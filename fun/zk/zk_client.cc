...
#else
# pragma comment(lib, "ThirdParty/zookeeper-3.3.0/src/c/x64/Release/zookeeper.lib")
#endif
#endif
...
#define THROW_IF_FAILED(rc) \
  if (ZK_FAILED(rc)) { \
    ...
    throw ZookeeperException(String::Format("zookeeper failed({}): {}", rc, zerror(rc))); \
  }
...
struct DataAsStringCompletionContext {
  ...
  DataAsStringCompletionContext(const Function<void(bool, const ByteArray&, const NodeStat&)>& callback)
    : callback(callback) {}
};
...
struct DataAsRawCompletionContext {
  ...
  DataAsRawCompletionContext(const Function<void(bool, const Array<uint8>&, const NodeStat&)>& callback)
    : callback(callback) {}
};
...
struct StringCompletionContext {
  ...
  StringCompletionContext(const Function<void(bool, const ByteArray&)>& callback)
    : callback(callback) {}
};
...
struct StringsCompletionContext {
  ...
  StringsCompletionContext(const Function<void(bool, const Array<ByteArray>&)>& callback)
    : callback(callback) {}
};
...
struct StringStatCompletionContext {
  ...
  StringStatCompletionContext(const Function<void(bool, const ByteArray&, const NodeStat&)>& callback)
    : callback(callback) {}
};
...
struct StringsStatCompletionContext {
  ...
  StringsStatCompletionContext(const Function<void(bool, const Array<ByteArray>&, const NodeStat&)>& callback)
    : callback(callback)
  {}
};
...
void DataAsStringCompletionCallback(int rc,
                                    const char* value,
                                    int value_len,
                                    ...
                                    const void* data)
{
  ...
  if (context->callback) {
    context->callback(ZK_SUCCEEDED(rc), ByteArray(value, value_len), FromZooStat(*stat));
  }
  delete context;
}

//제거해도 될듯...
void DataAsRawCompletionCallback( int rc,
                                  const char* value,
                                  int value_len,
                                  const struct Stat* stat,
                                  const void* data)
...
void StringsCompletionCallback( int rc,
...
void StringStatCompletionCallback(int rc,
                                  const char* string,
                                  const struct Stat* stat,
                                  const void* data)
{
...
void StringsStatCompletionCallback( int rc,
                                    const struct String_vector* strings,
                                    const struct Stat* stat,
                                    const void* data)
{
...
void GlobalWatchFunc( zhandle_t* handle,
                      int type,
                      int state,
                      const char* path,
                      void* context)
...
Client::Client()
  : zk_handle_(nullptr)
  , global_watcher_(nullptr)
{}


Client::Client(const ByteArray& server_host, IWatcher* global_watcher, const Timespan& timeout)
  : zk_handle_(nullptr)
  , global_watcher_(nullptr)
{
...
void Client::Connect(const ByteArray& server_host, IWatcher* global_watcher, const Timespan& timeout)
{
  Disconnect();
  ...
  zk_handle_ = zookeeper_init(server_host.ConstData(), GlobalWatchFunc, (int32)timeout.TotalMilliseconds(), nullptr, this, 0);
...
{
  return zk_handle_ && zoo_state((zhandle_t*)zk_handle_) == ZOO_EXPIRED_SESSION_STATE;
}

void Client::WatchHandler_INTERNAL(int type, int state, const char* path)
{
  LOG(LogZookeeper, Trace, "zookeeper-event: {}.{}: {}", *Type2String(type), *State2String(state), *String(path));
...
ByteArray Client::Create(const ByteArray& path, const ByteArray& value, const NodeTypes node_types)
{
  ByteArray path_buf(path.Len() + 64, NoInit);

  auto zc = zoo_create((zhandle_t*)zk_handle_,
              path.ConstData(),
              value.ConstData(),
              value.Len(),
              &ZOO_OPEN_ACL_UNSAFE,
              NodesTypeToZkFlags(node_types),
              path_buf.MutableData(),
              path_buf.Len());
  THROW_IF_FAILED(zc);
...
{
  auto zc = zoo_acreate((zhandle_t*)zk_handle_,
              path.ConstData(),
              value.ConstData(),
              value.Len(),
              &ZOO_OPEN_ACL_UNSAFE,
...
ByteArray Client::Create2(NodeStat& out_stat,
                          ...
                          const NodeTypes node_types)
{
  ...
  Stat zoo_stat;
  auto zc = zoo_create2((zhandle_t*)zk_handle_,
              path.ConstData(),
              value.ConstData(),
              value.Len(),
              &ZOO_OPEN_ACL_UNSAFE,
...
bool Client::Create2Async(const ByteArray& path,
                          const Function<void(bool, const ByteArray&, const NodeStat&)>& callback,
                          const ByteArray& value,
                          const NodeTypes node_types)
{
  auto zc = zoo_acreate2((zhandle_t*)zk_handle_,
    path.ConstData(),
    value.ConstData(),
    value.Len(),
    &ZOO_OPEN_ACL_UNSAFE,
    NodesTypeToZkFlags(node_types),
    StringStatCompletionCallback,
    new StringStatCompletionContext(callback));
  THROW_IF_FAILED(zc);
  return true;
}
...
bool Client::Delete(const ByteArray& path)
{
  ...
  auto zc = zoo_delete((zhandle_t*)zk_handle_, path.ConstData(), NodeStat.version);
  THROW_IF_FAILED(zc);
  return true;
}

bool Client::DeleteAsync(const ByteArray& path, const Function<void(bool)>& callback)
{
  NodeStat node_stat;
  ...
  auto zc = zoo_adelete((zhandle_t*)zk_handle_, path.ConstData(), node_stat.version, VoidCompletionCallback, new VoidCompletionContext(callback));
  THROW_IF_FAILED(zc);
  return true;
}
...
bool Client::DeleteIfExists(const ByteArray& path)
{
  ...
  auto zc = zoo_delete((zhandle_t*)zk_handle_, path.ConstData(), node_stat.version);
  if (zc == ZNONODE) {
    return false;
  }
...
bool Client::Exists(const ByteArray& path, bool watch, NodeStat* out_node_stat)
{
  Stat zoo_stat;
  auto zc = zoo_exists((zhandle_t*)zk_handle_, path.ConstData(), watch, out_node_stat != nullptr ? (&zoo_stat) : nullptr);
  if (zc == ZNONODE) {
    return false;
  }
...
bool Client::ExistsAsync( const ByteArray& path,
                          ...
                          bool watch)
{
  auto zc = zoo_aexists((zhandle_t*)zk_handle_, path.ConstData(), watch, StatCompletionCallback, new StatCompletionContext(callback));
...
{
  Stat zoo_stat;
  auto zc = zoo_exists((zhandle_t*)zk_handle_, path.ConstData(), false, &zoo_stat);
...
bool Client::GetStatAsync(const ByteArray& path, const Function<void(bool, const NodeStat&)>& callback)
{
  auto zc = zoo_aexists((zhandle_t*)zk_handle_, path.ConstData(), false, StatCompletionCallback, new StatCompletionContext(callback));
  THROW_IF_FAILED(zc);
  return true;
}
...
bool Client::Set(const ByteArray& path, const ByteArray& value)
{
  ...
  auto zc = zoo_set((zhandle_t*)zk_handle_, path.ConstData(), value.ConstData(), value.Len(), NodeStat.version);
  THROW_IF_FAILED(zc);
  return true;
}

bool Client::SetAsync(const ByteArray& path,
                      const ByteArray& value,
                      const Function<void(bool, const NodeStat&)>& callback)
{
  NodeStat node_stat;
  ...
  auto zc = zoo_aset((zhandle_t*)zk_handle_, path.ConstData(), value.ConstData(), value.Len(), NodeStat.version, StatCompletionCallback, new StatCompletionContext(callback));
  THROW_IF_FAILED(zc);
  return true;
}
...
bool Client::Get(const ByteArray& path, ByteArray& out_value, bool watch)
{
  ...
  auto zc = zoo_get((zhandle_t*)zk_handle_, path.ConstData(), watch, out_value.MutableData(), &buffer_len, &zoo_stat);
...
bool Client::GetAsync(const ByteArray& path,
                      const Function<void(bool, const ByteArray&, const NodeStat&)>& callback,
                      bool watch)
{
  auto zc = zoo_aget((zhandle_t*)zk_handle_, path.ConstData(), watch, DataAsStringCompletionCallback, new DataAsStringCompletionContext(callback));
  THROW_IF_FAILED(zc);
  return true;
}

bool Client::GetChildren( const ByteArray& parent_path,
                          Array<ByteArray>& out_children,
                          bool watch)
{
  struct String_vector ChildrenVec;
  auto zc = zoo_get_children((zhandle_t*)zk_handle_, parent_path.ConstData(), watch, &ChildrenVec);
...
bool Client::GetChildrenAsync(const ByteArray& parent_path,
                              const Function<void(bool, const Array<ByteArray>&)>& callback,
                              bool watch)
{
  auto zc = zoo_aget_children((zhandle_t*)zk_handle_, parent_path.ConstData(), watch, StringsCompletionCallback, new StringsCompletionContext(callback));
  THROW_IF_FAILED(zc);
  return true;
}
                          ...
                          bool watch)
{
  struct String_vector ChildrenVec;
  Stat zoo_stat;
  auto zc = zoo_get_children2((zhandle_t*)zk_handle_, parent_path.ConstData(), watch, &ChildrenVec, &zoo_stat);
...
bool Client::GetChildren2Async( const ByteArray& parent_path,
                                const Function<void(bool, const Array<ByteArray>&, const NodeStat&)>& callback,
                                bool watch)
{
  auto zc = zoo_aget_children2((zhandle_t*)zk_handle_, parent_path.ConstData(), watch, StringsStatCompletionCallback, new StringsStatCompletionContext(callback));
  THROW_IF_FAILED(zc);
  return true;
}

ByteArray Client::RecursiveCreate(const ByteArray& path,
                                  const ByteArray& value,
                                  const NodeTypes node_types)
{
  int32 pos = 0;
...
bool Client::SyncAsync( const ByteArray& path,
                        const Function<void(bool, const ByteArray&)>& callback)
{
  auto zc = zoo_async((zhandle_t*)zk_handle_, path.ConstData(), StringCompletionCallback, new StringCompletionContext(callback));
  THROW_IF_FAILED(zc);
  return true;
}
