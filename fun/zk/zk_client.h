...
//TODO 예외는 던지지 말도록 하자. 로깅만 처리하고, 결과를 반환하는 형태로 처리하도록 하자.
...
class FUN_ZOOKEEPER_API Client : public Noncopyable
{
 public:
   ...
   * Constructor.
   *
   * \param TimeoutMS - 서버에 전달되는 timeout 값으로, client사이드에서 사용되는 되는 값이 아님.
   *     Zookeeper에서 받아들일때 4000msec 밑으로는 설정이 안된다. (즉, 최소 값이 4000임)
   */
  Client( const ByteArray& server_host_list,
  ...
  //typedef Function<void(int32 RC, const ByteArray& CreatedNodeName)> CCreateCallback;
  //typedef Function<void(int32 RC, const ByteArray& CreatedNodeName, const CNodeStat& NodeStat)> CCreate2Callback;
  //typedef Function<void(int32 RC, const CNodeStat& NodeStat)> CExistsCallback;
  //typedef Function<void(int32 RC, const CNodeStat& NodeStat)> CSetCallback;

  ByteArray Create( const ByteArray& path,
  ...
  ByteArray Create2(CNodeStat& out_stat,
                    const ByteArray& path,
                    const ByteArray& value = ByteArray(),
                    const NodeTypes node_types = NodeType::Persistent);

  bool Create2Async(const ByteArray& path,
                    const Function<void(bool, const ByteArray&, const CNodeStat&)>& callback,
                    const ByteArray& value = ByteArray(),
                    const NodeTypes node_types = NodeType::Persistent);

  ByteArray CreateIfNotExists(const ByteArray& path,
                              const ByteArray& value = ByteArray(),
                              const NodeTypes node_types = NodeType::Persistent);
  ...
  bool Exists(const ByteArray& path,
              bool watch = false,
  ...
  bool SetAsync(const ByteArray& path,
                const ByteArray& value,
                const Function<void(bool, const CNodeStat&)>& callback);

  bool Get(const ByteArray& path, ByteArray& out_value, bool watch = false);

  bool GetAsync(const ByteArray& path,
                const Function<void(bool, const ByteArray&, const CNodeStat&)>& callback,
                bool watch = false);

  bool GetChildren( const ByteArray& parent_path,
                    Array<ByteArray>& out_children,
                    bool watch = false);

  bool GetChildrenAsync(const ByteArray& parent_path,
  ...
  bool GetChildren2Async( const ByteArray& parent_path,
                          const Function<void(bool, const Array<ByteArray>&, const CNodeStat&)>& callback,
                          bool watch = false);
