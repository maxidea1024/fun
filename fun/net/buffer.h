...
class FUN_NET_API Buffer {
 public:
  ...
  size_t WritableLength() const {
    return buffer_.Count() - writer_index_;
  }

  size_t PrependableLength() const {
  ...
  const char* Find(const char* sub, size_t len) const {
    const char* found = std::search(ReadablePtr(), WritablePtr(), sub, sub + len);
  ...
  String ReadAllAsString() {
    return ReadAsString(ReadableLength());
  }
 ...
 private:
  ...
  void MakeSpace(size_t len) {
    if (WritableLength() + PrependabeLength() < len + DISCARDABLE_PREPEND_LENGTH) {
