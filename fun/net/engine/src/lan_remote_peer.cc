...
LanRemotePeer_C::LanRemotePeer_C(LanClientImpl* owner)
    : tcp_transport_(this, nullptr, InetAddress::None),
      owner_(owner) {
  dispose_caller_ = nullptr;
  created_time_ = owner_->GetAbsoluteTime();//owner_->clock_.AbsoluteSeconds();
  last_ping_ = 0;
...
ResultCode
LanRemotePeer_C::ExtractMessagesFromTcpStream(ReceivedMessageList& out_result) {
...
void LanRemotePeer_C::WarnTooShortDisposal(const char* where) {
  if (dispose_caller_ == nullptr) {
    ...
    if ((owner_->GetAbsoluteTime() - created_time_) < 0.3) { // TODO 수치가 하드코딩 되어 있음, 별도로 빼주는것도 좋을듯...
      if (owner_->intra_logger_) {
        const String text = String::Format( "A suspicious part has been found.  "
                                            ...
                                            "a short time after connecting. : %s", where);
...
bool LanRemotePeer_C::IsDispose() {
  return dispose_waiter_.IsValid();
}

double LanRemotePeer_C::GetAbsoluteTime() {
...
void AcceptedInfo::ExtractMessagesFromTcpStream(ReceivedMessageList& out_result) {
  owner_->LockMain_AssertIsNotLockedByCurrentThread();
  ...
  if (added_count < 0) {
    ...
    //owner_->(this, rc, ResultCode::TCPConnectFailure, ByteArray(), __FUNCTION__, SocketErrorCode::Ok); //InduceDisconnect();
  }
}
