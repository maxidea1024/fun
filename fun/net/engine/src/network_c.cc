...
void NetClientWorker::ProcessEveryMessageOrMoveToFinalRecvQueue(
    UdpSocket_C* udp_socket) {
...
NetClientWorker::~NetClientWorker() {
  const bool is_disconnected =
      (GetState() == NetClientWorker::State::Disconnected);
  const bool is_unregistered = (!owner_->manager_->IsRegistered(this));

  if (!is_disconnected || !is_unregistered) {
    ErrorReporter::Report(
        String::Format("NetClientWorker.dtor - assert fail: %d %d",
                       is_disconnected, is_unregistered));
  }
}
...
void NetClientWorker::Heartbeat() {
  ...
  // 10ms는 눈에도 띄지 않고 send completion event시 즉시 다음 send issue를
  // 하므로 랙에 민감하지도 않다. 게다가 TCP Nagle OFF나 UDP coalesce에서는
  // 초송신이 오히려 약간의 시간은 대기 후 전송이니까.
  if (owner_->process_send_ready_remotes_alarm_.TakeElapsedTime(
          owner_->GetElapsedTime())) {
...
void NetClientWorker::Heartbeat_ConnectedCase() {
  ...
  if (owner_->last_tick_invoked_time_ > 0 &&
      (absolute_time - owner_->last_tick_invoked_time_) >
          30) {  // TODO 하드코딩된 이 숫자를 외부로 빼주도록 하자.
    if (owner_->IsIntraLoggingOn()) {
      owner_->IntraLogToServer(LogCategory::System,
                               "WARNING: NetClient.Tick is not called in 30 "
                               "seconds.  Is this your intention?");
    }
    ...
    if (to_server_tcp) {
      if (!to_server_tcp->recv_issued_) {
        if (to_server_tcp->last_recv_invoke_warning_time_ != 0) {
          if ((owner_->GetAbsoluteTime() -
               to_server_tcp->last_recv_invoke_warning_time_) >
              3) {  // TODO: NetConfig에 값으로 빼주는게...
            to_server_tcp->last_recv_invoke_warning_time_ = absolute_time;
            owner_->IntraLogToServer(
                LogCategory::System,
                "WARNING: Recvissue is not called in 3 seconds.");
          }
        } else {
          to_server_tcp->last_recv_invoke_warning_time_ = absolute_time;
        }
      ...
      if (!to_server_tcp->send_issued_) {
        if (to_server_tcp->last_send_invoked_warning_time_ != 0) {
          if ((absolute_time - to_server_tcp->last_send_invoked_warning_time_) >
              30) {  // TODO: NetConfig에 값으로 빼주는게...
            to_server_tcp->last_send_invoked_warning_time_ = absolute_time;
            owner_->IntraLogToServer(
                LogCategory::System,
                "WARNING: Sendissue is not called in 30 seconds.");
          }
        } else {
          to_server_tcp->last_send_invoked_warning_time_ = absolute_time;
        }
  ...
  if ((absolute_time - owner_->last_tcp_stream_recv_time_) >
      owner_->settings_.default_timeout_sec) {
    if (owner_->worker_) {
      if (owner_->IsIntraLoggingOn()) {
        const String text = String::Format(
            "Connect to server timed out.  default_timeout_time: %lf, "
            "absolute_time: %lf, last_tcp_stream_received_time: %lf",
            owner_->settings_.default_timeout_sec, absolute_time,
            owner_->last_tcp_stream_recv_time_);
        owner_->IntraLogToServer(LogCategory::System, *text);
      }

      // Disconnect
      owner_->EnqueueDisconnectionEvent(ResultCode::DisconnectFromLocal,
  ...
  // 중간에 컴플리션이 뜨지 않으면 남아있는 preFinal이 Final로 이동되지
  // 않으므로.. 반드시 필요하다!!! 일단 사용중지.
...
void NetClientWorker::Heartbeat_EveryRemotePeer() {
  ...
  const double p2p_attempt_under_progress_count =
      MathBase::Max(1.0, (double)p2p_conn_count);

  owner_->p2p_holepunch_interval_sec_ =
      NetConfig::p2p_holepunch_interval_sec * p2p_attempt_under_progress_count;
  owner_->p2p_connection_attempt_end_time_ =
      NetConfig::GetP2PHolepunchEndTime() * p2p_attempt_under_progress_count;
...
void NetClientWorker::UdpRecvCompletionCase(UdpSocket_C* udp_socket,
  ...
  if (udp_socket->recycle_binned_time_ == 0 &&
      completion.completed_length > 0) {
    const double absolute_time = owner_->GetAbsoluteTime();
    ...
    const UdpPacketDefragger::AssembledPacketError assembled_result =
        udp_socket->packet_defragger_->PushFragmentAndPopAssembledPacket(
            ...
            owner_->GetSrcHostIdByAddrAtDestSide_NOLOCK(
                completion.recvfrom_addr),
            absolute_time, assembled_packet, out_error);
    ...
    if (assembled_result == UdpPacketDefragger::AssembledPacketError::Ok) {
      ...
      owner_->ExtractMessagesFromUdpRecvQueue(
          assembled_packet.ConstData(), assembled_packet.Len(),
          assembled_packet.sender_addr, extracted_msg_list, extract_result);
      if (extract_result == ResultCode::Ok) {
        ...
        if (udp_socket->owner_peer_ ||
            udp_socket == owner_->Get_ToServerUdpSocket()) {
  ...
  if (completion.completed_length < 0) {
    owner_->emergency_log_data_.last_error_completion_length =
        (int32)::GetLastError();  //@todo 길이에 에러코드를 담는다??
  }
...
bool NetClientWorker::ProcessMessage_EngineLayer(
  ...
  //  ByteArray HexDump;
  //  ByteArray::BytesToDebuggableString(HexDump, msg.ToReadableBytesCopy(),
  //  false, "  >> ");
  //#endif
  ...
  if (msg_type == MessageType::RPC) {
    ...
    LOG(LogNetEngine, Info,
        ">> NetClient.MSG [%s] [%05d] [%s] RPC#%d (LEN: %d)",
        udp_socket ? "UDP" : "TCP", (int32)received_msg.remote_id,
        received_msg.relayed ? "Relayed" : "NonRelayed", (int32)rpc_id,
        msg.ReadableLength());
    //*String(HexDump));
  } else {
  ...
  switch (msg_type) {
    ...
    case MessageType::Encrypted_Unreliable: {
      ...
      if (owner_->DecryptMessage(msg_type, received_msg,
        ...
        msg_processed |= ProcessMessage_EngineLayer(
            udp_socket, decrypted_received_msg);  // Recursive call
      }
      break;
    }
    ...
    case MessageType::Compressed: {
      ...
      if (owner_->DecompressMessage(received_msg,
        ...
        msg_processed |= ProcessMessage_EngineLayer(
            udp_socket, decompressed_received_msg);  // Recursive call
      }
      break;
    }

    case MessageType::RequestReceiveSpeedAtReceiverSide_NoRelay:
      ProcessMessage_RequestReceiveSpeedAtReceiverSide_NoRelay(udp_socket,
                                                               received_msg);
      msg_processed = true;
      break;
  ...
  if (msg_processed && l1 != l2 &&
    ...
    //@note 에러 상황이지만, fail-over를 위해서 에러만 기록하고 계속 진행하도록
    //한다.
    msg_processed = true;
    ...
    comment +=
        String::Format(", Message=(Name=%s(%d), Length=%d, ReadOffset=%d)",
                       *ToString(msg_type), (int32)msg_type, l1, l2);

    // 딱히 copy를 하지 않아도 될듯한데...
    // ByteArray ProblemMessage(msg.GetData(), msg.GetLength());
    // ByteArray ProblemMessage = msg.GetSharableBuffer(); //TODO 오프셋
    // 관련하여 문제가 없으려나??? 우선은 안전하게 전체 데이터의 사본을 넘겨주는
    // 형태로 처리하도록 하자.

    // 일단은 안전하게 전체 내용 복사해서 처리.. 차후에 복사를 제거하는 쪽으로
    // 개선해야함. 대량으로 계속 패킷이 꺠져온다는 가정을 한다면, 이 복사비용도
    // 부담이될터이니, 추후에 참조 형태로 넘길 수 있도록 변경하자.
    const ByteArray spurious_msg = msg.ToAllBytesCopy();
    owner_->EnqueueError(ResultInfo::From(ResultCode::InvalidPacketFormat,
                                          received_msg.remote_id, comment,
                                          spurious_msg));
...
void NetClientWorker::ProcessMessage_PeerUdp_PeerHolepunch(
...
void NetClientWorker::ProcessMessage_PeerHolepunchAck(
    ReceivedMessage& received_msg) {
  owner_->LockMain_AssertIsLockedByCurrentThread();
...
void NetClientWorker::ProcessMessage_RUdp_Frame(UdpSocket_C* udp_socket,
  ...
  if (peer && !peer->to_peer_rudp.panic_) {
    // TODO 매우 큰 데이터(1MB남짓)를 연속해서 보내면, 데이터의 변형이 생기는
    // 경우가 있음.

    ReceivedMessageList udp_received_msgs;
    ResultCode extract_result;
    peer->to_peer_rudp.EnqueueReceivedFrameAndGetFlushedMessages(
        received_msg.unsafe_message, udp_received_msgs, extract_result);
    if (extract_result == ResultCode::Ok) {
      ...
      // TODO 에러가 안나는게 맞는건가?
      LOG(LogNetEngine, Error, "Extraction failed: %s",
          *ToString(extract_result));
      owner_->EnqueueError(
          ResultInfo::From(extract_result, peer->host_id_,
                           "Stream extract error at reliable UDP"));
      // TODO 접속을 끊어야하지 않을까??
      // SetState(State::Disconnecting);
    }
  } else {
    // Symmetric NAT 홀펀칭을 하는 경우 등에서, 의도하지 않은 다른 peer로부터의
    // 메시지 수신이 있을 수 있다. 이런 경우, 즉 대응되지 않는 패킷인 경우라도
    // 끝까지 다 읽은셈 쳐야 한다. 안그러면 엄한데서 온 UDP frame message가 있는
    // 경우 bad stream exception이 발생하기 때문이다.
    received_msg.unsafe_message.SeekToEnd();
  }
}

// 서버와의 홀펀칭이 완료 되었을때 수신받는 메시지
void NetClientWorker::ProcessMessage_NotifyClientServerUdpMatched(
    MessageIn& msg) {
  ...
  if (owner_->IsIntraLoggingOn()) {
    owner_->IntraLogToServer(LogCategory::SP2P,
                             "Holepunch to server UDP successful.");
  }
}
...
//
// -> 해당 버그는 고쳐졌음.
//    UDP 소켓 생성시 TCP 접속으로 얻어낸 클라 주소로 Dummy(MessageType::Ignore)
//    패킷을 보내도록 되어 있었는데, 이 패킷을 보낸 후로 서버의 Send쪽이 막히는
//    문제가 있었음. (wireshark를 통해서 확인.) 해당 부분의 코드를 막아준 이후로
//    아주 잘됨.
void NetClientWorker::ProcessMessage_ServerHolepunchAck(
    ReceivedMessage& received_msg) {
  auto& msg = received_msg.unsafe_message;

  Uuid holepunch_tag;               // Key to confirm arrival from desired.
  InetAddress here_addr_at_server;  // The address of this client that is
                                    // recognized by the server (this address is
  ...
  // it came from the wrong place, not where I wanted to be. Ignore it.
  if (owner_->to_server_udp_fallbackable_->server_addr_ !=
      received_msg.remote_addr_udp_only) {
    TRACE_SOURCE_LOCATION();
    return;
  }
  ...
  owner_->Get_ToServerTcp()->SendWhenReady(SendFragRefs(msg_to_send),
                                           TcpSendOption());

  if (owner_->IsIntraLoggingOn()) {
    const String text = String::Format(
        "Message_ServerHolepunchAck.  addr_of_here_at_server: %s",
        *here_addr_at_server_.ToString());
    owner_->IntraLogToServer(LogCategory::SP2P, *text);
  }
}

void NetClientWorker::ProcessMessage_PeerUdp_ServerHolepunchAck(
    ReceivedMessage& received_msg) {
  auto& msg = received_msg.unsafe_message;
...
// debugging
  ...
  if (peer && !peer->garbaged_ && peer->p2p_holepunch_attempt_context_) {
    peer->p2p_holepunch_attempt_context_
        ->ProcessMessage_PeerUdp_ServerHolepunchAck(
            received_msg, holepunch_tag, here_addr_at_server, peer_id);
  }
}
...
bool NetClientWorker::ProcessMessage_S2CRoutedMulticast1(
    UdpSocket_C* udp_socket, ReceivedMessage& received_msg) {
  ...
  // Peer가 relayed thru server 상태인 경우 다시 릴레이하면 효율성이
  // 무시되어버린다. 일단, 서버에서 direct P2P라는걸로 인식된 상태이므로 그걸
  // 받아들이고 직빵 전송해야 한다.
  for (int32 i = 0; i < host_id_list.Count(); ++i) {
    if (host_id_list[i] != owner_->local_host_id_) {
      ...
      if (peer && !peer->garbaged_ && peer->IsDirectP2P() &&
          peer->udp_socket_) {
        // 상대가 relay일 때 이는 무시되겠지만 그래도 상관없다. 어차피
        // unreliable이고 조만간 서버에서는 제대로 조정된 것으로 보내질 터이니.
        const auto filter_tag =
            FilterTag::Make(owner_->GetLocalHostId(), host_id_list[i]);
        peer->Get_ToPeerUdpSocket()->SendWhenReady(
            host_id_list[i], filter_tag,
            peer->p2p_holepunched_local_to_remote_addr_, routed_msg,
            owner_->GetAbsoluteTime(), send_opt);
      }
    }
  }
...
bool NetClientWorker::ProcessMessage_S2CRoutedMulticast2(
    UdpSocket_C* udp_socket, ReceivedMessage& received_msg) {
  auto& msg = received_msg.unsafe_message;
...
void NetClientWorker::ProcessMessage_RequestStartServerHolepunch(
    MessageIn& msg) {
...
void NetClientWorker::ProcessMessage_NotifyServerConnectSuccess(
    MessageIn& msg) {
  owner_->LockMain_AssertIsLockedByCurrentThread();
  ...
  if (!lf::Reads(msg, local_host_id, server_instance_tag_, user_data,
                 local_addr_at_server)) {
    auto error =
        ResultInfo::From(ResultCode::ProtocolVersionMismatch, HostId_Server,
                         String::Format("Bad format in %s", __FUNCTION__));
  ...
  // 경우에 따라서는 resolve하느라 느려질 수 있지 않을까?? 뭐 어짜피 ip
  // literal로 오므로 상관 없으려나???
  owner_->Get_ToServerTcp()->local_addr_at_server_ =
      local_addr_at_server.ToInetAddress();

  // Connection hint까지 왔으면, TCP 연결을 add port mapping upnp를 걸기 위해,
  // 여기서부터 미리 켜도록 한다.
  owner_->ConditionalStartupUPnP();

  const InetAddress server_addr = InetAddress(
      owner_->connect_args_.server_ip, owner_->connect_args_.server_port);
  {
    CScopedLock2 owner_guard(owner_->GetMutex());
    ...
    //  owner_->viz_agent_->c2s_proxy_.NotifyClient_ConnectionState(HostId_Server,
    //  GReliableSend_INTERNAL, owner_->GetServerConnectionState(ignorant));
    //}
  }

  if (owner_->IsIntraLoggingOn()) {
    const String text = String::Format(
        "Connecting to server successful.  host_id: %d, local_addr: %s, "
        "addr_at_server: %s, server_addr: %s, server_instance_tag: %s",
        ...
        *server_addr.ToString(), *server_instance_tag.ToString());
    owner_->IntraLogToServer(LogCategory::System, *text);
  }
}
...
void NetClientWorker::ProcessMessage_NotifyServerConnectionHint(
    MessageIn& msg) {
  owner_->LockMain_AssertIsLockedByCurrentThread();
  ...
  if (!CryptoRSA::CreateRandomBlock(
          strong_random_block,
          owner_->settings_.strong_encrypted_message_key_length) ||
      !CryptoAES::ExpandFrom(
          owner_->SelfP2PSessionKey.aes_key,
          (const uint8*)strong_random_block.ConstData(),
          owner_->settings_.strong_encrypted_message_key_length / 8) ||

      !CryptoRSA::CreateRandomBlock(
          weak_random_block,
          owner_->settings_.weak_encrypted_message_key_length) ||
      !CryptoRC4::ExpandFrom(
          owner_->SelfP2PSessionKey.rc4_key,
          (const uint8*)weak_random_block.ConstData(),
          owner_->settings_.weak_encrypted_message_key_length / 8)) {
    owner_->EnqueueDisconnectionEvent(ResultCode::EncryptFail,
                                      ResultCode::TCPConnectFailure);

    if (owner_->callbacks_) {
      owner_->callbacks_->OnException(
          HostId_None, Exception("Failed to create session-key"));
    }
  ...
  // 아예 전역 값도 수정하도록 한다.
  {
    ...
    NetConfig::message_max_length = MathBase::Max<int32>(
        NetConfig::message_max_length, settings_.message_max_length);
  }
  ...
  owner_->Get_ToServerTcp()->SetEnableNagleAlgorithm(
  ...
  if (!CryptoRSA::CreateRandomBlock(
          ...
          owner_->settings_.strong_encrypted_message_key_length) ||
      !CryptoAES::ExpandFrom(
          owner_->ToServerSessionKey.aes_key,
          (const uint8*)strong_random_block.ConstData(),
          owner_->settings_.strong_encrypted_message_key_length / 8) ||
      ...
      !CryptoRSA::CreateRandomBlock(
          ...
          owner_->settings_.weak_encrypted_message_key_length) ||
      !CryptoRC4::ExpandFrom(
          owner_->ToServerSessionKey.rc4_key,
          (const uint8*)weak_random_block.ConstData(),
          owner_->settings_.weak_encrypted_message_key_length / 8) ||
      !CryptoAES::Encrypt(owner_->ToServerSessionKey.aes_key, weak_random_block,
                          encrypted_rc4_key)) {
    owner_->EnqueueDisconnectionEvent(ResultCode::EncryptFail,
                                      ResultCode::TCPConnectFailure);
    SetState(State::Disconnecting);
  ...
  owner_->Get_ToServerTcp()->SendWhenReady(SendFragRefs(response),
                                           TcpSendOption());
}

void NetClientWorker::ProcessMessage_NotifyProtocolVersionMismatch(
    MessageIn& msg) {
  owner_->EnqueueConnectFailEvent(ResultCode::ProtocolVersionMismatch);
  SetState(State::Disconnecting);
}

void NetClientWorker::ProcessMessage_NotifyServerDeniedConnection(
    MessageIn& msg) {
...
void NetClientWorker::TcpSendCompletionCase(CompletionStatus& completion) {
  ...
  if (completion.completed_length < 0) {  // 0은 허용?
    ...
    owner_->EnqueueDisconnectionEvent(ResultCode::DisconnectFromRemote,
                                      ResultCode::TCPConnectFailure,
                                      "completed_length < 0 SendComplete");
    // Disconnecting으로 상태를 전환하지 않아도 되나??
  } else {
    if (completion.completed_length > 0) {
      // 송신 큐에서 완료된 만큼의 데이터를 제거한다. 그리고 다음 송신을 건다.
      // (최종 completion 상황이라 하더라도 이 과정은 필수!)
      owner_->Get_ToServerTcp()->send_queue_.DequeueNoCopy(
          completion.completed_length);

      // Stats
      owner_->stats_.total_tcp_send_bytes += completion.completed_length;
      // owner_->RecentBackedUpStats.total_tcp_send_bytes =
      // owner_->stats_.total_tcp_send_bytes;
    }

    // 이 루틴이 실행중일떄는 아직 owner가 건재함을 전제한다.
    // 이 비교가 불필요할 듯. socket is NOT closed and 'Disconnecting' state인
    // 경우 난감한데다 ConditionalIssueSend 자체가 dead socket이면 skip하므로.
...
bool NetClientWorker::LoopbackRecvCompletionCase() {
  ...
  if (!owner_->loopback_final_recv_message_queue_.IsEmpty()) {
    ...
    received_msg.unsafe_message =
        owner_->loopback_final_recv_message_queue_.CutFront();
...
void NetClientWorker::TcpRecvCompletionCase(CompletionStatus& completion) {
  ...
  if (completion.completed_length <= 0) {
    ...
    if (owner_->worker_->GetState() < NetClientWorker::State::Disconnecting) {
      // TCP 연결이 끊어졌음을 의미한다. 따라서 에러 처리한다.
      const String text = String::Format(
          "completed_length(%d) <= 0 RecvComplete, comp_error_code: %d, "
          "last_error: %u, socket_error: %d",
          completion.completed_length, (int32)completion.socket_error,
          last_error, (int32)completion.socket_error);
      owner_->EnqueueDisconnectionEvent(ResultCode::DisconnectFromRemote,
                                        ResultCode::TCPConnectFailure, text);
      SetState(State::Disconnecting);
    }

    if (owner_->IsIntraLoggingOn()) {
      const String text = String::Format(
          "[Error] completed_length(%d) <= 0 RecvComplete, last_error: %u, "
          "socket_error: %d",
          completion.completed_length, last_error,
    ...
    owner_->last_tcp_stream_recv_time_ =
        owner_->GetAbsoluteTime();  // 타임스탬핑
    owner_->Get_ToServerTcp()->recv_stream_.EnqueueCopy(
        owner_->Get_ToServerTcp()->socket_->GetRecvBufferPtr(),
        completion.completed_length);  // TODO 복사가 불필요해보이는데, 차후에
                                       // 개선하도록 하자.
    owner_->stats_.total_tcp_recv_bytes +=
        completion.completed_length;  // Stats
  ...
  // 다음 recv를 건다.  여기까지 왔을떄는 시스템이 파괴되지 않고 건재함을
  // 전제해야 한다.
  if (owner_->worker_->GetState() < NetClientWorker::State::Disconnecting) {
    const SocketErrorCode socket_error =
        owner_->Get_ToServerTcp()->IssueRecvAndCheck();
    if (socket_error != SocketErrorCode::Ok) {
      const String text = String::Format("%d TCP IssueRecv failed socket_error",
                                         (int32)socket_error);
      owner_->EnqueueDisconnectionEvent(ResultCode::TCPConnectFailure,
                                        ResultCode::Unexpected, text);
      SetState(State::Disconnecting);
    }
  }
}

void NetClientWorker::ProcessMessageOrMoveToFinalRecvQueue(
    UdpSocket_C* udp_socket, ReceivedMessage& received_msg) {
  fun_check(received_msg.unsafe_message.AtBegin());
  ProcessMessage_EngineLayer(udp_socket, received_msg);
}

// UnreliableRelay1(sender RemoteClient) -> Server -> UnreliableRelay2(Server)
// -> local
void NetClientWorker::ProcessMessage_UnreliableRelay2(
    UdpSocket_C* udp_socket, ReceivedMessage& received_msg) {
  // 서버로부터 온 메시지가 아니면 무시한다.
  if (received_msg.remote_id != HostId_Server) {
    TRACE_SOURCE_LOCATION();
...
void NetClientWorker::ProcessMessage_ReliableRelay2(MessageIn& msg) {
  ...
  if (content_length < 0 ||
      content_length >= owner_->settings_.message_max_length) {
    TRACE_SOURCE_LOCATION();
    return;
  }
  ...
  if (peer && !peer->garbaged_ && !peer->to_peer_rudp.panic_) {
    ...
    RUdpHelper::BuildRelayed2LongDataFrame(frame_number, SendFragRefs(content),
                                           long_frame);

    ResultCode extract_result;
    ReceivedMessageList received_msg_list;
    peer->to_peer_rudp.EnqueueReceivedFrameAndGetFlushedMessages(
        long_frame, received_msg_list, extract_result);
    if (extract_result == ResultCode::Ok) {
      for (auto& received_msg : received_msg_list) {
        ...
        // TCP로 수신 받은 메시지들이므로, 첫번째 인자인 UdpSocket은 null로
        // 지정.
...
void NetClientWorker::ProcessMessage_LingerDataFrame2(
    UdpSocket_C* udp_socket, ReceivedMessage& received_msg) {
  ...
  // 아니면, 아래처럼 별도의 길이 체크용 함수를 하나 만들어서 사용하는것도
  // 좋을듯 싶다.
  // if (!msg.IsValidMessageLength(frame_length))
  if (frame_length < 0 ||
      frame_length >= owner_->settings_.message_max_length) {
    TRACE_SOURCE_LOCATION();
    return;
  }

  // Hack or engine bug.
  fun_check(msg.GetReadableLength() ==
            frame_length);  // 정확히 맞아떨어져야함. (부족해도 남아도 안됨)

  // ReadRawBytes 함수를 통해서 복사를 할 경우에는 문제가 없으나,
  // ReadAsShared 함수를 통해서 복사 없이 처리할 경우에는 문제가 있음.
  //-> ByteArray(const char*, int32)에서 문자열에 '\0'이 있을 경우, 빈문자열로
  //할당되던 코드가 있어서 발생했었음.
  ...
  if (peer && !peer->garbaged_ && !peer->to_peer_rudp.panic_) {
    ...
    // frame.data = frame_data.ToReadableBytesCopy(); //TODO frame.Data를
    // CMessageIn형태로 변경해주다면, 복사를 제거할 수 있을듯.  현재는
    // frame.Data가 ByteArray 타입임. frame.data =
    // frame_data.ToReadableBytesRaw(); recevier windows에서 참조를 가지고
    // 유지했다가 해당 프레임이 왔을 경우 스트림에 반영되므로, 복사를 해서
    //넘겨주어야함. frame.data = ByteArray::FromRawData((const
    // char*)frame_data.GetReadableData(), frame_data.GetReadableLength());
    // frame.data = frame_data;

    //주의:
    // recevier windows에서 참조를 가지고 유지했다가 해당 프레임이 왔을 경우
    // 스트림에 반영되므로, 복사를 해서 넘겨주어야함. frame.data = frame_data;
    frame.data = frame_data.ToReadableBytesCopy();

    ReceivedMessageList received_msg_list;
    ResultCode extract_result;
    peer->to_peer_rudp.EnqueueReceivedFrameAndGetFlushedMessages(
        Frame, received_msg_list, extract_result);
    if (extract_result == ResultCode::Ok) {
      for (auto& received_msg : received_msg_list) {
        received_msg.relayed = true;
        // received_msg.action_time = owner_->GetAbsoluteTime();
...
//
// (혹은 아직 서버와의 연결 처리가 다 끝나지 않은 상태이면 HostId_None에서 오는
// 경우일테고 이때는 무시)
bool NetClientWorker::IsFromRemoteClientPeer(
    ReceivedMessage& received_msg) const {
...
bool NetClientWorker::Main_IssueConnect(SocketErrorCode& out_socket_error) {
  const SocketErrorCode socket_error =
      owner_->Get_ToServerTcp()->socket_->Connect(
          owner_->connect_args_.server_ip, owner_->connect_args_.server_port);
  if (socket_error == SocketErrorCode::Ok ||
...
void NetClientWorker::IssueTcpFirstRecv() {
  ...
  const SocketErrorCode socket_error =
      owner_->Get_ToServerTcp()->IssueRecvAndCheck();
  if (socket_error != SocketErrorCode::Ok) {  //@note 첫번째이므로, wouldblock가
                                              //온다면 그것 또한 의미 없으리..
    owner_->EnqueueConnectFailEvent(ResultCode::TCPConnectFailure,
                                    socket_error);
    SetState(State::Disconnecting);
  }
}
...
void NetClientWorker::Heartbeat_IssueConnect() {
  ...
  //
  // winsock connect()는 TCP 연결 제한시간이 1초이다. 이를 API 딴에서 고칠
  // 방법은 없다. 따라서 TCP connect()를 수차례 반복후 안되면 즐쳐줘야 한다.
  // 통상적인 소켓은 1초지만 어떤 유저들은 이 값을 윈도 레지스트리를 통해
  // 바꿔놓으므로 타임 기준으로 체크한다.
  ...
  if (!owner_->Get_ToServerTcp()->socket_->Bind()) {
    owner_->EnqueueError(
        ResultInfo::From(ResultCode::TCPConnectFailure, HostId_None,
                         "Cannot bind TCP socket to a local address."));
  ...
  // 연결 시작 시각 기록
  connect_issued_time_ =
      owner_->GetAbsoluteTime();  // clock_.AbsoluteSeconds();

  // 소켓 기본 설정
  const bool old_verbose = owner_->Get_ToServerTcp()->socket_->GetVerboseFlag();
  owner_->Get_ToServerTcp()->socket_->SetVerboseFlag(
      false);  //소켓 경고를 잠시 꺼준다.
  owner_->Get_ToServerTcp()->socket_->SetBlockingMode(
      false);  // 소켓을 넌블럭킹 모드로 바꾼 후 연결 시도를 한다. 연결 시도가
               // 끝나면 블럭킹 모드로 다시 바꾼다.
...
void NetClientWorker::Heartbeat_Connecting() {
  ...
  if ((T1 - connect_issued_time_) > NetConfig::tcp_socket_connect_timeout_sec) {
    ...
    // 기다리지 않고 폴링한다. 어차피 이 함수는 타이머에 의해 일정 시간마다
    // 호출되니까.
    select_context.Wait(0);

    SocketErrorCode socket_error;
    const bool did = select_context.GetConnectResult(
        *owner_->Get_ToServerTcp()->socket, socket_error);
    if (did) {
      if (socket_error == SocketErrorCode::Ok) {
        // 연결 완료!

        owner_->Get_ToServerTcp()->socket_->SetBlockingMode(
            true);  //@todo blocking?? (연결중일때만 Non-blocking으로 함)

        // 연결이 정상적으로 완료 되었으니,
        // 이제 로컬 IP주소도 얻자. 127.0.0.1 아니면 연결된 NIC의 주소가 나올
        // 것이다.
        owner_->Get_ToServerTcp()->RefreshLocalAddress();

        if (!owner_->Get_ToServerTcp()->local_addr.IsUnicast()) {
          LOG(LogNetEngine, Warning,
              "서버와 TCP 연결이 되었는데도 로컬 소켓 주소를 얻을 수 없다니!");
        }
        ...
        // char msg[1024];
        // LOG(LogNetEngine, Warning, "connection failure: %s",
        // CPlatformMisc::GetSystemErrorMessage(msg, countof(msg),
        // (int32)socket_error));
...
void NetClientWorker::Heartbeat_JustConnected() {
  ...
  // 이 메시지를 받은 서버는 접속 정보등에 관련해서 알려주게 된다. (서버내
  // 설정등을 동기화)
  MessageOut msg;
...
void NetClientWorker::Heartbeat_Connected() {
  ...
  // 예전에는 Heartbeat_ConnectedCase가 아래 while 문 안에 있었으나, 그게 아닌
  // 듯 하다. 아래는 더 이상 수신된 시그널이 없을 때까지 도는 루프니까.
  Heartbeat_ConnectedCase();

  // Nat 장치 이름을 얻어옴. (바로 얻어오지 못하고, 주기적으로 조회해서
  // 가져와야함.)
  Heartbeat_DetectNatDeviceName();

  while (true) {
    // TODO 루프를 돌면서 처리하는데, 여기서 시간이 흘러갈란가?
    // owner_->GetAbsoluteTime()이 확실히 리얼타이인지? 아니면, Heartbeat시에만
    //갱신되는 steppedtime인지 명확히 할 필요가 있어보인다. const double
    // absolute_time =
    // owner_->GetMorePrecisionAbsoluteTime();//owner_->GetAbsoluteTime();
    const double absolute_time = owner_->GetAbsoluteTime();

    // 서버와의 연결 해제 시작 상황이 된 후 서버와 오랫동안 연결이 안끊어지면
    // 강제 스레드 종료를 한다. 접속해제를 요청한(NetClient.Disconnect() 함수
    // 호출) 시각으로부터 오랜 시간동안 응답이 없으면 바로 종료 절차로
    // 넘어가도록 합니다. 대기시간이 있는 이유는, 정상적으로 TCP 접속을 서버가
    // 인지한 후에 안전하게 끊을 수 있도록 하기 위함인데, 스트레스 테스팅을 하는
    // 과정에서는 지연만 생길 수 있으므로, graceful_disconnect_timeout_ 값을
    ...
    if (shutdown_issued_time_ > 0) {
      ...
      if (elapsed_time > graceful_disconnect_timeout_) {
        if (graceful_disconnect_timeout_ > 0.0) {
          LOG(LogNetEngine, Warning,
              "Graceful disconnection timedout.  elapsed: %fsec", elapsed_time);
        }
...
void NetClientWorker::Heartbeat_Disconnecting() {
  ...
  if (disconnecting_mode_heartbeat_count_ == 1) {
    ...
    if (owner_->Get_ToServerUdpSocket()) {  // UDP 소켓은 일단 바로 끊어버린다.
                                            // (소켓 객체만 닫아줌!)
    ...
    // TCP 소켓도 바로 끊어야 한다.
    // 소켓을 끊으면서 async IO가 모두 종료되는데, 이것부터 먼저 한 후에 worker
    // thread unregister를 하는 것이 안전하기 때문이다.

    if (owner_->Get_ToServerTcp()) {
      // owner_->Get_ToServerTcp()->socket_->Shutdown(ShutdownFlag_Both);
      // 이거 안해도 된다. 이미 일전에 RPC를 통해 shutdown을 서버에
      // 노티했으니까.
      ...
      if (owner_->IsIntraLoggingOn()) {
        owner_->IntraLogToServer(
            LogCategory::UDP,
            "Heartbeat_Disconnecting, CloseSocketHandleOnly called.");
      }
    }
  }
  ...
  if (!owner_->Garbages.IsEmpty()) {
    ...
    safe_disconnect = false;
             ...
             (!owner_->Get_ToServerUdpSocket()->socket_->IsClosed() ||
              owner_->Get_ToServerUdpSocket()->send_issued_ ||
              owner_->Get_ToServerUdpSocket()->recv_issued_)) {
    unsafe_disconnect_reason = 2;
    safe_disconnect = false;
  } else if (owner_->Get_ToServerTcp() && owner_->Get_ToServerTcp()->socket_ &&
             (!owner_->Get_ToServerTcp()->socket_->IsClosed() ||
              owner_->Get_ToServerTcp()->send_issued_ ||
              owner_->Get_ToServerTcp()->recv_issued_)) {
  ...
  if (safe_disconnect) {
    ...
    owner_->CleanupEvenUnstableSituation(
        false);  // 아직 처리되지 않은 패킷들은 지우지 않음.
    ...
    // if (owner_->IsIntraLoggingOn()) {
    //  owner_->IntraLogToServer(LogCategory::System, "Networker thread is
    //  stopped.");
    //}
  ...
  else if ((owner_->GetAbsoluteTime() - disconnecting_mode_start_time_) > 5 &&
           !disconnecting_mode_warned_) {
    ErrorReporter::Report(
        String::Format("Too long time elapsed since disconnecting mode.  "
...
void NetClientWorker::ProcessMessage_NotifyCSSessionKeySuccess(MessageIn& msg) {
  ...
  owner_->Get_ToServerTcp()->SendWhenReady(SendFragRefs(request_msg),
                                           TcpSendOption());
}

void NetClientWorker::ProcessMessage_P2PIndirectServerTimeAndPing(
    ReceivedMessage& received_msg) {
  auto& msg = received_msg.unsafe_message;
  ...
  if (!peer ||
      peer->host_id_ ==
          HostId_Server) {  //??? RP에 서버가 들어갈 수 있나?? (서버가 P2P멤버로
                            //참여 가능한 설정이 활성화 되었을 경우에는 가능?)
    TRACE_SOURCE_LOCATION();
    return;
  }
  ...
  peer->to_peer_udp.SendWhenReady(
      SendFragRefs(pong),
      UdpSendOption(MessagePriority::Ring0, EngineOnlyFeature));
}

void NetClientWorker::ProcessMessage_P2PIndirectServerTimeAndPong(
    ReceivedMessage& received_msg) {
  auto& msg = received_msg.unsafe_message;
...
void NetClientWorker::ProcessMessage_ReplyServerTime(MessageIn& msg) {
  ...
  // TODO 리얼타임을 써야하는지, owner_->GetAbsoluteTime()를 써야하는지 좀
  // 헷갈림... 꼭 리얼타임을 써야함!! const double client_time =
  // owner_->GetAbsoluteTime();//clock_.AbsoluteSeconds();
  ...
  if (owner_->server_udp_recent_ping_ != 0) {
    ...
    owner_->server_udp_recent_ping_ =
        MathBase::Lerp(owner_->server_udp_recent_ping_, lag,
  ...
  const double server_time =
      server_local_time + owner_->server_udp_recent_ping_;
...
void NetClientWorker::SetState(State new_state) {
  if (new_state != state_ &&
      new_state > state_) {  // 뒤 상태로는 이동할 수 없어야 한다.
    disconnecting_mode_heartbeat_count_ = 0;
    state_ = new_state;
...
void NetClientWorker::ProcessMessage_RPC(ReceivedMessage& received_msg,
  ...
  // 엔진 내부 Stub에서 처리되지 않은 경우에는, 유저 스레드로 넘겨주어서
  // 처리하도록 한다.

  //여기서 잘못 읽혔다는건 시리얼라이징이 잘못되었다는건데...
  // const ByteArray S = ByteArray((const
  // char*)received_msg.unsafe_message.GetReadableData() - 1,
  // received_msg.unsafe_message.GetReadableLength() + 1);
  // LOG(LogNetEngine,Warning,"DUMP: %s", *String(S.ToHex()));

  // 엔진 내부 S2CStub에서 처리 시도.
  ref_msg_processed |=
      owner_->s2c_stub_.ProcessReceivedMessage(received_msg, host_tag);

  // 엔진 내부 C2CStub에서 처리 시도.
  if (!ref_msg_processed) {
    content.Seek(saved_read_pos);  // Rewind
    ref_msg_processed |=
        owner_->c2c_stub_.ProcessReceivedMessage(received_msg, host_tag);
  }

  // 유저 스레드로 넘겨주어서 처리 시도.
  if (!ref_msg_processed) {
    // 엔진 내부 RPC에서 처리가 안되었으므로, 이제 유저 RPC임.
    // 그런데, 유저 RPC가 호출되는 시점에서 무조건 JIT P2P가 트리거 되는게
    // 맞는걸까??
    ...
    received_msg_to_user_thread.remote_addr_udp_only =
        received_msg.remote_addr_udp_only;
    received_msg_to_user_thread.remote_id = received_msg.remote_id;

    owner_->LockMain_AssertIsLockedByCurrentThread();
    owner_->FinalUserWorkItemQueue.Enqueue(FinalUserWorkItem(
        received_msg_to_user_thread, FinalUserWorkItemType::RPC));

    // LOG(LogNetEngine,Info,"UserRPC: %d",
    // (int32)received_msg_to_user_thread.remote_id);

    // 보낸 호스트가 Server/None이 아닌 경우, 즉 피어에게서 받은 경우에는 P2P를
    // 개시하게 됨. 단, 이때 해당 피어가 같은 그룹내에 있어야함. 그룹에 들어있지
    // 않은 경우에는 릴레이를 통해서 서버를 경유해서 보내지게됨.
    if (received_msg_to_user_thread.remote_id != HostId_Server &&
        received_msg_to_user_thread.remote_id != HostId_None) {
      CScopedLock2 owner_guard(owner_->GetMutex());

      auto peer =
          owner_->GetPeerByHostId(received_msg_to_user_thread.remote_id);
      if (peer && !peer->garbaged_) {
...
void NetClientWorker::Heartbeat_DetectNatDeviceName() {
  ...
  if (owner_->nat_device_name_detected_ == false &&
      owner_->HasServerConnection() && owner_->local_host_id_ != HostId_None) {
    const String name = owner_->GetNatDeviceName();
    if (!name.IsEmpty()) {
      owner_->nat_device_name_detected_ = true;
      owner_->c2s_proxy_.NotifyNatDeviceNameDetected(
          HostId_Server, GReliableSend_INTERNAL, name);
    }
  }
}
...
void NetClientWorker::ProcessMessage_RequestReceiveSpeedAtReceiverSide_NoRelay(
  ...
  // 수신속도를 리플한다.
  // 그러나 수신 UDP 주소로 바로 에코하지 않는다. 드물지만 src->dest시 src과
  // dst->src 리플시 src가 서로 다를 수 있으니까.
  const double receive_speed =
      udp_socket->packet_defragger_->GetRecentReceiveSpeed(
          received_msg.remote_addr_udp_only);
  if (receive_speed > 0) {
    MessageOut msg_to_send;
    ...
    if (received_msg.remote_id == HostId_Server) {
      const auto filter_tag =
          FilterTag::Make(owner_->GetLocalHostId(), received_msg.remote_id);
      const UdpSendOption send_opt(MessagePriority::Ring1, EngineOnlyFeature);
      owner_->Get_ToServerUdpSocket()->SendWhenReady(
          received_msg.remote_id, filter_tag,
          owner_->to_server_udp_fallbackable_->server_addr_, msg_to_send,
          owner_->GetAbsoluteTime(), send_opt);
      ...
      if (peer && !peer->garbaged_ && peer->udp_socket_) {
        ...
        peer->Get_ToPeerUdpSocket()->SendWhenReady(
            received_msg.remote_id, filter_tag,
            peer->p2p_holepunched_local_to_remote_addr_, msg_to_send,
            owner_->GetAbsoluteTime(), send_opt);
      }
    }
  }
...
void NetClientWorker::ProcessMessage_ReplyReceiveSpeedAtReceiverSide_NoRelay(
    ReceivedMessage& received_msg) {
  owner_->LockMain_AssertIsLockedByCurrentThread();
...
void NetClientWorker::ProcessMessage_FreeformMessage(
    ReceivedMessage& received_msg, bool& ref_msg_processed) {
  auto& content = received_msg.unsafe_message;
  ...
  ReceivedMessage received_msg_to_user_thread;
  // received_msg_to_user_thread.action_time = received_msg.action_time;
  received_msg_to_user_thread.unsafe_message =
      content.ToReadableMessage();  // shared
  received_msg_to_user_thread.relayed = received_msg.relayed;
  received_msg_to_user_thread.remote_addr_udp_only =
      received_msg.remote_addr_udp_only;
  received_msg_to_user_thread.remote_id = received_msg.remote_id;
  ...
  owner_->LockMain_AssertIsLockedByCurrentThread();
  owner_->final_user_work_queue_.Enqueue(FinalUserWorkItem(
      received_msg_to_user_thread, FinalUserWorkItemType::FreeformMessage));
  ...
  if (received_msg_to_user_thread.remote_id != HostId_Server &&
    ...
    // RP가 목록에 들어 있어야만 처리가 되는데, 이말은 그룹핑이 된 이후에만
    // 처리가 된다는 얘기? 그룹핑이 안되어도 되게끔할 수는 없으려나??
...
void NetClientWorker::WarnTooLongElapsedTime() {
  if (NetConfig::starvation_warning_enabled) {
    static const double COMPARE_WARNING_TIME =
        double(NetConfig::client_heartbeat_interval_msec) / 1000 * 10;

    const double average = owner_->manager_->average_elapsed_time_;
    const double first_average_calc_time_ =
        owner_->manager_->first_average_calc_time_;

    if (average > COMPARE_WARNING_TIME &&
        (owner_->GetAbsoluteTime() - first_average_calc_time_) >
            10) {  // 최소 10초는 지난 후에 체크하지 않으면 넷클라 처음 뜨는
                   // 동안 버벅대는 시간 등이 모두 모아지므로 false positive가
                   // 생김.
      const String text =
          String::Format("%lf average heartbeat elapsed-time", average);

      // note: EnqueueWarning() 내부에서 락이 걸려있는 상태이므로, 구태여 락을
      // 걸필요 없음. CScopedLock2 owner_guard(owner_->GetMutex());
      owner_->EnqueueWarning(ResultInfo::From(
          ResultCode::TooSlowHeartbeatWarning, owner_->GetLocalHostId(), text));
...
NetClientManager::NetClientManager() {
  ...
  completion_port_.Reset(new CompletionPort(
      this, true, CPlatformMisc::NumberOfCoresIncludingHyperthreads()));
  ...
  const uint32 Interval =
      MathBase::Max(NetConfig::client_heartbeat_interval_msec, 1u);
  heartbeat_signal_timer_id_ =
      timer_.Schedule(Timespan::FromMilliseconds(Interval),
...
void NetClientManager::Run() {
  // 게임 렌더링 코어 등에 의해 많은 시간을 뺏기면 대량 통신시 UDP 타임아웃이
  // 자주 발생하는 것 같아 이 설정이 필요
  ...
  try {
    ...
    while (!should_stop_thread_) {
      ...
      //아니다 위에서 대기는 이벤트가 없을 경우에만 주어진 시간까지
      //대기하는거니까 이벤트가 있을 경우에는 바로 블럭에서 해제될 수 있으므로,
      //타이밍 체크는 별도로 해야함.
      ...
      if (heartbeat_pulse_.GetValue()) {
        heartbeat_pulse_.Set(0);
        {
          ...
          //변수 선언시 volatile이 지정되어 잇어서, MathBase::Lerp에서
          //문법오류(?) 나고 있음. 강제로 double로 캐스팅 했더니 문제는
          //없는데... 흠..
          recent_elapsed_time_ = MathBase::Lerp((double)recent_elapsed_time_,
                                                (double)elapsed_time, 0.3);
    ...
    const String text = String::Format("std.exception(%s)",
                                       (const char*)UTF8_TO_TCHAR(e.what()));
    CatchThreadUnexpectedExit(__FUNCTION__, *text);
  }  // catch (_com_error& e) {
  //  const String text = String::Format("_com_error(%s)", (const
  //  char*)e.Description()); CatchThreadUnexpectedExit(__FUNCTION__, text);
  //} catch (void*) {
...
void NetClientManager::CatchThreadUnexpectedExit(const char* where,
                                                 const char* reason) {
  LOG(LogNetEngine, Warning, "(%s): Unexpected thread exit with (%s)", where,
      reason);
}
...
bool NetClientManager::ProcessIoCompletion_TCP(CompletionStatus& completion) {
  ...
  if (tcp_transport) {
    CScopedLock2 owner_guard(
        mutex_);  // TODO main lock을 잡은 상태에서 해야하나??
    ...
    if (completion.type == CompletionType::Send) {
      ...
      owner_guard.Unlock();
      ErrorReporter::Report(
          *String::Format("Unexpected at client IOCP TCP completion. type: %d",
                          (int32)completion.type));
    }
    return true;
  }
...
bool NetClientManager::ProcessIoCompletion_UDP(CompletionStatus& completion) {
  ...
  if (udp_socket /* && !udp_socket->garbaged_*/) {  // Garbage인 경우에도 I/O
                                                    // 처리는 해야함.
    CScopedLock2 owner_guard(
        mutex_);  // TODO main lock을 잡은 상태에서 해야하나??
    ...
    if (completion.type == CompletionType::Send) {
      if (udp_socket->main_->worker_) {
        udp_socket->main_->worker_->UdpSendCompletionCase(udp_socket,
                                                          completion);
      }
    } else if (completion.type == CompletionType::Receive) {
      if (udp_socket->main_->worker_) {
        udp_socket->main_->worker_->UdpRecvCompletionCase(udp_socket,
                                                          completion);
      }
    } else {
      owner_guard.Unlock();
      ErrorReporter::Report(
          *String::Format("Unexpected at client IOCP UDP completion. type: %d",
                          (int32)completion.type));
    }
...
void NetClientManager::OnMeasureComplete(double speed) {
  for (auto instance = instances_.Front(); instance;
       instance = instance->GetNextNode()) {
    instance->owner_->c2s_proxy_.NotifySendSpeed(HostId_Server,
                                                 GReliableSend_INTERNAL, speed);
  }
}
...
void NetClientManager::CalculateAverageElapsedTime() {
  if (NetConfig::starvation_warning_enabled) {
    ...
    if (last_heartbeat_time_ != 0) {
      ...
      // 최대 샘플까지만 제한함.
      if (elapsed_time_queue_.Count() >
          NetConfig::manager_average_elapsed_time_sample_count) {
        elapsed_time_queue_.RemoveFront();
      }

      // 최대 샘플까지 수집이 된경우에만 정확한 값을 측정할 수 있음.
      if (elapsed_time_queue_.Count() ==
          NetConfig::manager_average_elapsed_time_sample_count) {  // TODO
                                                                   // 이름을 좀
                                                                   // 바꾸는데...
        // dword/1000 *10 dword를 double로 바꿔 10배로 바꿈 이것이 warning의
        // 기준이 된다.
        ...
        const double average =
            accum_elapsed_time / (double)elapsed_time_queue_.Count();
...
void NetClientManager::HasOverlappedIoToGarbage(
    IHasOverlappedIoPtr overlapped_io) {
  // Induce dispose.
...
void NetClientManager::DoGarbageFree() {
  ...
  if ((absolute_time - last_garbage_free_time_) >
      NetConfig::manager_garbage_free_interval_sec) {
