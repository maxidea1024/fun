...
bool CryptoRSAKey::ToBlob(ByteArray& out_blob) {
  ...
  if (rsa_export((uint8*)out_blob.MutableData(), &length, PK_PUBLIC, key) != CRYPT_OK) {
...
bool CryptoRSA::CreatePublicAndPrivateKey(CryptoRSAKey& out_xchg_key, ByteArray& out_public_key_blob) {
  if (rsa_make_key(&g_rsa_provider.prng, g_rsa_provider.prngIndex, 1024 / 8, 65537, out_xchg_key.key) != CRYPT_OK) {
    // RSA MAKE KEY ERROR
    TRACE_SOURCE_LOCATION();
    return false;
  }
...
bool CryptoRSA::EncryptSessionKeyByPublicKey(ByteArray& out_encrypted_session_key, const ByteArray& random_block, const ByteArray& public_key_blob) {
  ...
  if (rsa_encrypt_key(
      (const uint8*)random_block.ConstData(), BlobLen,
      (uint8*)out_encrypted_session_key.MutableData(), &DataSize,
      nullptr, 0,
      &g_rsa_provider.prng, g_rsa_provider.prngIndex,
      g_rsa_provider.hashIndex, PublicKey.key) != CRYPT_OK) {
    TRACE_SOURCE_LOCATION();
    return false;
  }
...
ResultInfoPtr CryptoRSA::DecryptSessionKeyByPrivateKey(
      ...
      const ByteArray& encrypted_session_key,
  ...
  if (rsa_decrypt_key(
      (const uint8*)encrypted_session_key.ConstData(), encrypted_session_key.Len(),
      (uint8*)out_random_block.MutableData(), &len,
      nullptr, 0,
      g_rsa_provider.hashIndex, &stat, private_key.key) != CRYPT_OK) {
  ...
  if (stat != 1) {
    ...
    return ResultInfo::From(ResultCode::DecryptFail, HostId_None, "incorrect packet");
  }

  if (len > (unsigned long)encrypted_session_key.Len()) {
    TRACE_SOURCE_LOCATION();

    const String text = "decryption result is larger. memory corruption may occur.";
...
bool CryptoRSA::CreateRandomBlock(ByteArray& output, int32 length) {
  ...
  if (fortuna_read((uint8*)output.MutableData(), length, &g_rsa_provider.prng) <= 0) {
