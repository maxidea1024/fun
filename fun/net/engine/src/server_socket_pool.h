...
class ServerSocketPool
  : public Singleton<ServerSocketPool>
  , public Runnable {
 public:
  ServerSocketPool();
  ~ServerSocketPool();

  InternalSocket* NewTcpSocket( IInternalSocketDelegate* Delegate,
 ...
 private:
  ...
  FastMutex mutex_;
  Array<SOCKET> newbie_sockets_; //List로 하는건 어떨런지?? 삽입삭제가 빈번하지 않으려나??
};
