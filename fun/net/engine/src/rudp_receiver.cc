//수신 윈도우
// 일단 받은 프레임들을 보존하는 역활임.  일단 받고, 빼나갈 상황이 되면
// 빼나가므로 버퍼의 역활임. 그런 이유로, 수신 윈도우의 내용들은 빼나가기 전까지
// 보존되어야함.
//
...
void RUdpReceiver::SendGatheredAcks() {
  ...
  while (!acks_to_send_.IsEmpty()) {
    ...
    frame.acked_frame_numbers.Reset(
        new CompressedFrameNumbers());  // 계속 생성하는게 좀 걸리는데....

    int32 insertion_count = 0;  // acks_to_send_.Count();

    // 보낼 ack list를 모두 정렬한다. 설령 다 넣지 못하더라도.
    // HeuristicQuickSort(acks_to_send_.MutableData(), acks_to_send_.Count(),
    // 2000);
    acks_to_send_.Sort();
    ...
    for (int32 i = 0; i < acks_to_send_.Count(); ++i) {
      ...
      if (frame.acked_frame_numbers->Count() >=
          RUdpConfig::max_ack_count_in_one_frame) {
        break;
      }
    }
...
void RUdpReceiver::ProcessDataFrame(RUdpFrame& frame) {
  ...
  // ack list에 추가한다. 단, 타 reliable channel을 통해 송신 가능
  // 상황이면(예컨대 tcp 릴레이 모드) ack에 포함시키지 않는다. 또한, expected
  // frame number 과거 것들도 추가를 하지 않는다. 추후 보낼 ack frame에서
  // expected frame number를 같이 보내기 때문에 안 보내도 ok.
  if (!owner_->delegate_->IsReliableChannel()) {
    if (FrameNumberUtil::Compare(frame.frame_number, expected_frame_number) >=
        0) {
...
void RUdpReceiver::AddToReceivedFrames(RUdpFrame& received_frame) {
  ...
  for (auto it = receiver_window_.CreateIterator(); it; ++it) {
    ...
    // 바로 앞에 삽입함.
    if (FrameNumberUtil::Compare(received_frame.frame_number,
  ...
  while (!receiver_window_.IsEmpty()) {
    ...
    if (receiver_frame.frame_number == expected_frame_number) {
      ...
      expected_frame_number =
          FrameNumberUtil::NextFrameNumber(expected_frame_number);
    } else {
      break;
    }
  }
...
void RUdpReceiver::ProcessAckFrame(RUdpFrame& frame) {
  ...
  if (!ack_list.IsEmpty()) {
    ...
    owner_->sender.last_received_ack_time_ =
        owner_->delegate_->GetAbsoluteTime();
    owner_->sender.total_ack_frame_recv_count_++;
  }

  // ack를 보낸 측(즉 data frame의 수신자)에서 보내준 expected frame number 이하
  // 것들은 미리 걸러내버린다.
  owner_->sender.RemoveSpecifiedAndItsPastsFromSenderWindow(
      frame.expected_frame_number);
  owner_->sender.last_expected_frame_number_at_sender_ =
      frame.expected_frame_number;
  ...
  owner_->sender.remote_recv_speed_ =
      (int32)MathBase::Lerp((double)owner_->sender.remote_recv_speed_,
                            (double)frame.recent_receive_speed, 0.9) +
      1;
}
...
void RUdpReceiver::Tick(float elapsed_time) {
  ...
  if ((absolute_time - recent_receive_frame_count_start_time_) >
      RUdpConfig::calc_recent_receive_interval) {
    const double T =
        0.1 / (absolute_time - recent_receive_frame_count_start_time_);
    recent_receive_speed = (int32)MathBase::Lerp(
        (double)recent_receive_speed, (double)recent_receive_frame_count_, T);
    recent_receive_frame_count_ = 0;
    recent_receive_frame_count_start_time_ = absolute_time;
  }
  ...
  if ((absolute_time - last_send_gathered_acks_time_) >
      RUdpConfig::stream_to_sender_window_coalesce_interval * 0.2) {
    SendGatheredAcks();
