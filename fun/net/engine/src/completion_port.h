...
class CompletionStatus {
 public:
  /** 완료된 결과와 관련된 객체. 이 객체 값은
   * completion_port_.AssociateSocket에서 주어진 object이다. */
  ...
  /** recv, recvfrom에서 수신된 flags값. overlapped I/O에도 업뎃되므로 유지해야
   * 하기에 멤버로 선언했다. */
  DWORD recv_flags;
...
class ICompletionPortCallbacks {
 public:
  ...
  virtual void OnCompletionPortWarning(CompletionPort* port,
                                       const char* msg) = 0;
};
 ...
 * 사용법:
 * socket.SetCompletionContext=>completion.AssociateSocket=>socket.Issue* or
 * socket.AcceptEx
 */
class CompletionPort {
 public:
  ...
  bool Dequeue(Array<CompletionStatus>& out_statuses, int32& dequeue_count,
