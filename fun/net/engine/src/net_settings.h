...
class NetSettings {
 public:
  ...
  // RC4 대칭키의 사이즈(bit) 입니다. 최대 0,512,1024,2048을 지원합니다. 키값이 길어도 속도에 지장을 주지 않습니다.
...
template <> struct MessageFieldTypeTraits<NetSettings> {
  ...
  static bool Read(IMessageIn& input, CppValueType& out_value) {
    ...
    FUN_DO_CHECKED(LiteFormat::Read(input, out_value.direct_p2p_start_condition));
    FUN_DO_CHECKED(LiteFormat::Read(input, out_value.over_send_suspecting_threshold_in_byte));
    FUN_DO_CHECKED(LiteFormat::Read(input, out_value.bEnableNagleAlgorithm));
    FUN_DO_CHECKED(LiteFormat::Read(input, out_value.strong_encrypted_message_key_length));
    FUN_DO_CHECKED(LiteFormat::Read(input, out_value.weak_encrypted_message_key_length));
    FUN_DO_CHECKED(LiteFormat::Read(input, out_value.server_as_p2p_group_member_allowed));
    FUN_DO_CHECKED(LiteFormat::Read(input, out_value.p2p_encrypted_messaging_enabled));
    FUN_DO_CHECKED(LiteFormat::Read(input, out_value.upnp_detect_nat_device));
    FUN_DO_CHECKED(LiteFormat::Read(input, out_value.upnp_tcp_addr_port_mapping));
    FUN_DO_CHECKED(LiteFormat::Read(input, out_value.look_ahead_p2p_send_enabled));
...
inline String ToString(const NetSettings& value)
{
  ...
  ret << ", direct_p2p_start_condition: " << ToString(value.direct_p2p_start_condition);
  ret << ", over_send_suspecting_threshold_in_byte: " << ToString(value.over_send_suspecting_threshold_in_byte);
  ret << ", enable_nagle_algorithm: " << ToString(value.bEnableNagleAlgorithm);
  ret << ", strong_encrypted_message_key_length: " << ToString(value.strong_encrypted_message_key_length);
  ret << ", weak_encrypted_message_key_length: " << ToString(value.weak_encrypted_message_key_length);
  ret << ", enable_p2p_encrypted_messaging: " << ToString(value.p2p_encrypted_messaging_enabled);
  ret << ", allow_server_as_p2p_group_member: " << ToString(value.server_as_p2p_group_member_allowed);
  ret << ", upnp_detect_nat_device: " << ToString(value.upnp_detect_nat_device);
  ret << ", upnp_tcp_add_port_mapping: " << ToString(value.upnp_tcp_addr_port_mapping);
  ret << ", emergency_log_line_count: " << ToString(value.emergency_log_line_count);
  ret << ", enable_lookahead_p2p_send: " << ToString(value.look_ahead_p2p_send_enabled);
  ret << ", enable_ping_test: " << ToString(value.ping_test_enabled);
  ret << ", ignore_failed_bind_port: " << ToString(value.ignore_failed_bind_port);
  ret << "}");
  return ret;
}
