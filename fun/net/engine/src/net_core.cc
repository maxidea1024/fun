...
//const char* DuplicatedRpcIdErrorText = "Duplicated RPC ID is found. Review RPC ID declaration in .IDL files.";
//const char* BadRpcIdErrorText = "Wrong RPC ID is found. RPC ID should be >=1000 or <65530.";
//const char* AsyncCallbackMayOccurErrorText = "Already async callback may occur! server start or client connection should have not been done before here.";
...
static bool IsOverlapped(const RpcId* a, int32 a_count, const RpcId* b, int32 b_count) {
  //겹치는지 여부만 알려주는게 아니라, 어떤것이 겹치는지 알려주는것이 좀더 좋을듯함.
  //단, 이때 번호만 알려주는게 아니라 이름까지 알려줄 수 있다면 좋을듯 한데...
...
void NetCoreImpl::AttachProxy(RpcProxy* proxy_to_attach) {
  ...
  //@todo 이놈은 선형적이지 않은 목록을 추려내어서 개별로 충돌여부를 확인하도록 하자.
  // 만약 중복되는 ID가 있으면 에러 처리한다.
  for (int32 proxy_index = 0; proxy_index < proxies_nolock_.Count(); ++proxy_index) {
    ...
    //}
    //
    //if (!IsCombinationEmpty((uint16)proxy->GetFirstRpcId(), (uint16)(proxy->GetLastRpcId() + 1), (uint16)proxy_to_attach->GetFirstRpcId(), (uint16)(proxy_to_attach->GetLastRpcId() + 1))) {
...
void NetCoreImpl::AttachStub(RpcStub* stub_to_attach) {
  ...
  for (int32 stub_index = 0; stub_index < stubs_nolock_.Count(); ++stub_index) {
    ...
    //TODO
    //if (!IsCombinationEmpty((uint16)stub->GetFirstRpcId(), (uint16)(stub->GetLastRpcId() + 1), (uint16)stub_to_attach->GetFirstRpcId(), (uint16)(stub_to_attach->GetLastRpcId() + 1))) {
    //  throw Exception(DuplicatedRpcIdErrorText);
    //}
  }
...
void NetCoreImpl::ShowNotImplementedRpcWarning(RpcId rpc_id, const char* rpc_name) {
  LOG(LogNetEngine, Warning, "unimplemented RPC function is called. %s(%d)", rpc_name, (int32)rpc_id);
}

void NetCoreImpl::PostCheckReadMessage(IMessageIn& msg, RpcId rpc_id, const char* rpc_name) {
  if (!msg.AtEnd()) {
    LOG(LogNetEngine, Warning, "failed to read RPC incoming message %s", rpc_name);
  }
  ...
  //FIXME
  //예외처리를 해주어야할듯 싶은데..
  //여기서 로그만 출력하면, 실제 stub 함수가 엉망인상태로 호출되는 불상사가 발생하므로,
  //예외를 던지던지 하는 형태로 처리를 해야한다.
}
...
bool NetCoreImpl::Send_CompressLayer( const SendFragRefs& payload,
  ...
  // Because it has 12 bytes header, it is rather easy to increase the size below 50 bytes.
  if (send_opt.compression_mode == CompressionMode::None || payload.GetTotalLength() <= COMPRESSION_ATLEAST_LENGTH) {
  ...
  //note: 압축을 위해서는 어쩔수 없이 복사를 해야하나? 스트림 형태로 처리한다면 복사 비용을 줄일수도 있을듯 싶은데...
  ...
  int32 actual_compressed_len = Compressor::GetMaxCompressedLength(send_opt.compression_mode, payload_bytes.Len());
  compressed_msg.SetLength(actual_compressed_len);

  // Let's rolling.
  String compression_error;
  const bool compression_ok = Compressor::Compress(send_opt.compression_mode,
                                compressed_msg.MutableData(),
                                &actual_compressed_len,
                                (const uint8*)payload_bytes.ConstData(),
                                payload_bytes.Len(),
                                &compression_error);
  if (!compression_ok) {
    // Compression is failed.
    const String text = String::Format("packet compression failed. error: %s", *compression_error);
    EnqueueError(ResultInfo::From(ResultCode::CompressFail, sendto_list[0], text));
  ...
  if ((actual_compressed_len + 10) < payload.GetTotalLength()) {
    ...
    return Send_SecureLayer(compressed_payload, send_opt, send_to, sendto_count);
  } else {
...
bool NetCoreImpl::DecompressMessage(ReceivedMessage& received_msg, MessageIn& decompressed_output) {
  ...
  // Read header
  if (!lf::Reads(compressed_msg,  compression_mode, compressed_payload_len, decompressed_payload_len)) {
  ...
  // Check the length first because it is difficult to overkill memory due to bug size or hacked size.
  // 실제로는 0미만이 아닌 최소값 이상이어야함.  왜냐하면, 최소값 이하는 압축을 하지 않았을테니...
  if (decompressed_payload_len < COMPRESSION_ATLEAST_LENGTH || decompressed_payload_len > GetMessageMaxLength()) {
    //TRACE_SOURCE_LOCATION();
    //LOG(LogNetEngine,Warning,"decompressed_payload_len=%d, message_max_length=%d", decompressed_payload_len, GetMessageMaxLength());
  ...
  if (!Compressor::Decompress(compression_mode,
            (uint8*)decompressed_payload.MutableData(),
            &actual_decompressed_len,
            compressed_msg.GetReadableData(),
            compressed_payload_len,
            &decompression_error) ||
      actual_decompressed_len != decompressed_payload_len) {
    const String text = String::Format("packet decompression failed. error: %s", *decompression_error);
...
bool NetCoreImpl::Send_SecureLayer( const SendFragRefs& payload,
                                    ...
                                    const HostId* sendto_list,
                                    int32 sendto_count) {
  if (send_opt.encryption_mode == EncryptionMode::None || send_opt.reliability == MessageReliability::Last) {
  ...
  {
    // ConvertGroupToIndividualsAndUnion() 함수가 락을 걸고 호출하는 것을 전재로 하였기 때문에 호출전에 락을 걸어주어야 한다.
    CScopedLock2 main_guard(GetMutex());
    CheckCriticalSectionDeadLock(__FUNCTION__);

    ConvertGroupToIndividualsAndUnion(sendto_count, sendto_list, flatten_send_dest_list);
  }

  String out_error;
  bool ok = false;
  for (int32 send_dest_index = 0; send_dest_index < flatten_send_dest_list.Count(); ++send_dest_index) {
    ...
    // 상대측에서 패킷 해킹으로 오인함!
      ...
      if (send_opt.reliability == MessageReliability::Reliable) {
        ...
        if (!NextEncryptCount(send_dest, encrypt_count)) {
          EnqueueError(ResultInfo::From(ResultCode::EncryptFail, send_dest, "NextEncryptCount is failed."));
          return false;
        }
      ...
      for (int32 frag_index = 0; frag_index < payload.Count(); ++frag_index) {
        input_payload.WriteRawBytes(payload[frag_index].data, payload[frag_index].length);
      }
      ...
      switch (send_opt.encryption_mode) {
        case EncryptionMode::Strong:
          encryption_ok = CryptoAES::Encrypt(session_key_->aes_key, ByteStringView(input_payload.ConstData(),input_payload.GetLength()), output_payload);
          break;
        case EncryptionMode::Weak:
          encryption_ok = CryptoRC4::Encrypt(session_key_->rc4_key, ByteStringView(input_payload.ConstData(),input_payload.GetLength()), output_payload);
          break;
        ...
        case EncryptionMode::None:
          ...
          break;
        default:
          EnqueueError(ResultInfo::From(ResultCode::EncryptFail, send_dest, String::Format("unsupported encryption method: %d", (int32)send_opt.encryption_mode)));
          break;
      }
      ...
      if (encryption_ok) {
        ...
        // 위에서, Reliable일 경우에만 페이로드 앞쪽에 카운터 값을 추가 했으므로..
        const MessageType msg_type = send_opt.reliability == MessageReliability::Reliable ? MessageType::Encrypted_Reliable : MessageType::Encrypted_Unreliable;
        lf::Write(header, msg_type);
        lf::Write(header, send_opt.encryption_mode);
        lf::Write(header, OptimalCounter32(output_payload.Len()));

        //LOG(LogNetEngine, Warning, "<< ENCRYPTION:  plain=%d bytes, cipher=%d bytes", EncryptedPayload.GetLength(), output_payload.Len());
        ...
        EnqueueError(ResultInfo::From(ResultCode::EncryptFail, send_dest, "encryption is failed."));

        // 암호화가 실패한 경우에는 EncryptCount를 원래대로 돌려주어야함. (Rollback)
        if (send_opt.reliability == MessageReliability::Reliable) {
      ...
      if (!out_error.IsEmpty()) {
        EnqueueError(ResultInfo::From(ResultCode::EncryptFail, send_dest, out_error));
      } else {
        EnqueueError(ResultInfo::From(ResultCode::EncryptFail, send_dest, "StartServerArgs.p2p_encrypted_messaging_enabled = false. P2P Messaging can not encrypted."));
      }
    }
  }
...
bool NetCoreImpl::DecryptMessage( MessageType msg_type,
  ...
  // 넘겨진 페이로드 길이만큼 정확하게 데이터가 있는지 확인(더도 말고 덜도말고 정확히 일치해야함)
  // -> 암호화된 경우에는 길이의 왜곡이 있을 수 있으므로, 정확히 비교하는대신 최소 길이만 체크함.
  ...
  String out_error;
  SessionKey* session_key = GetCryptSessionKey(received_msg.remote_id, out_error);
  if (session_key) { //세션키가 있어야지만, 복호화를 수행할 수 있음.
    switch (encryption_mode) {
      case EncryptionMode::Strong:
        decryption_ok = CryptoAES::Decrypt(session_key->aes_key, ByteStringView(msg.GetReadableData(),msg.GetReadableLength()), decryption_payload);
        break;
      case EncryptionMode::Weak:
        decryption_ok = CryptoRC4::Decrypt(session_key->rc4_key, ByteStringView(msg.GetReadableData(),msg.GetReadableLength()), decryption_payload);
        break;
      //case EncryptionMode::Fast:
      //  fun_check(0); //TODO
      //  break;
      case EncryptionMode::None: //발생할 수 없는 경우임.
        LOG(LogNetEngine,Trace,"%s: encryption_mode.None is not allowed at here", __FUNCTION__);
        break;
      default:
        LOG(LogNetEngine,Trace,"%s: unsupported encryption method: %d", __FUNCTION__, (int32)encryption_mode);
        break;
    }
  }
  ...
  if (!decryption_ok) {
    ...
    EnqueueError(ResultInfo::From(ResultCode::DecryptFail, received_msg.remote_id, out_error));
    msg.Seek(saved_read_pos);
    return false;
  }
...
//debugging
#if 1
  //LOG(LogNetEngine,Warning,"DecryptedMessageLength: %d", decryption_payload.Len());
  //LOG(LogNetEngine, Warning, ">> DECRYPTION:  plain=%d bytes, cipher=%d bytes", decryption_payload.Len(), encrypted_payload_len);
#endif

  decrypted_output = MessageIn(decryption_payload);

  // Reliable일 경우에 한해서, 앞쪽에 카운터가 있으므로, 카운터 값이 유효한지 체크함.
  if (msg_type == MessageType::Encrypted_Reliable) {
    ...
    if (!lf::Read(decrypted_output, tossed_decrypt_count)) {
      ...
      EnqueueError(ResultInfo::From(ResultCode::DecryptFail, received_msg.remote_id, "tossed_decrypt_count read failed."));
      msg.Seek(saved_read_pos);
      return false;
    }

    if (!GetExpectedDecryptCount(received_msg.remote_id, expected_decrypt_count)) {
      CScopedLock2 main_guard(GetMutex());
      CheckCriticalSectionDeadLock(__FUNCTION__);

      EnqueueError(ResultInfo::From(ResultCode::DecryptFail, received_msg.remote_id, "GetExpectedDecryptCount failed."));
      msg.Seek(saved_read_pos);
      return false;
    }
    ...
    if (tossed_decrypt_count != expected_decrypt_count) {
      ...
      const String text = String::Format("tossed_decrypt_count(%d) != expected_decrypt_count(%d)", tossed_decrypt_count, expected_decrypt_count);
      EnqueueError(ResultInfo::From(ResultCode::DecryptFail, received_msg.remote_id, text));
      msg.Seek(saved_read_pos);
      return false;
    }
...
bool NetCoreImpl::SendFreeform( const HostId* sendto_list,
                                int32 sendto_count,
  ...
  const bool ok = Send(frag_refs, SendOption(rpc_call_opt), sendto_list, sendto_count);
