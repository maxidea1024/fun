...
class UdpSocket_C : public ICompletionContext, public IHasOverlappedIo {
 public:
  ...
  void SendWhenReady(HostId final_dest_id, FilterTag::Type filter_tag,
  ...
  // owner_peer_ == nullptr이지만 main_ != nullptr인 경우가 있으므로 이런 변수가
  // 별도 필요. owner_peer_ == nullptr이지만 main_ != nullptr인 경우란, 홀펀칭
  // 재사용을 위해 재사용통에 들어가 있는 경우이다.
  NetClientImpl* main_;
  ...
  // FunNet.RemotePeer_C.UdpAddrFromServer와 다름.
