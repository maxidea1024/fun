//@deprecated
//@note NetClient, LanClient 모두 Disconnect()호출하여 접속을 끊을시에는
//"LeftFromServer" 콜백이 호출되지 않음.
//      호출되게 해줄까??
...
IMPLEMENT_RPCSTUB_LanS2C_P2PGroup_MemberJoin(LanClientImpl::S2CStub) {
  ...
  owner_->UpdateP2PGroup_MemberJoin(group_id, member_id, custom_field, event_id,
                                    p2p_aes_session_key, p2p_rc4_session_key,
                                    connection_tag);
  return true;
}

IMPLEMENT_RPCSTUB_LanS2C_P2PGroup_MemberJoin_Unencrypted(
    LanClientImpl::S2CStub) {
  CScopedLock2 main_guard(owner_->GetMutex());
  owner_->CheckCriticalSectionDeadLock(__FUNCTION__);

  // P2P Group에 새로운 member가 들어온것을 update한다
  owner_->UpdateP2PGroup_MemberJoin(group_id, member_id, custom_field, event_id,
                                    ByteArray(), ByteArray(), connection_tag);
  return true;
}
...
IMPLEMENT_RPCSTUB_LanS2C_ReliablePong(LanClientImpl::S2CStub) {
  ...
  // 받은 시간을 키핑한다.
  // owner_->last_reliable_pong_received_time_ =
  // owner_->clock_.AbsoluteSeconds();
...
IMPLEMENT_RPCSTUB_LanS2C_P2PConnectStart(LanClientImpl::S2CStub) {
  ...
  // 체크 방법이 잘못된듯 싶은데...
  // 미인증 목록에서 찾는게 아니라 인증목록에서 찾은 결과를 토대로
  // 처리해야하는거 아닌지???
  ...
  lp->tcp_transport_.socket_.Reset(
      owner_->server_socket_pool_->NewTcpSocket(owner_, owner_));

  // Nagle Algorithm 적용
  lp->tcp_transport_.SetEnableNagleAlgorithm(
  ...
  // 상대방에게 커넥션을 걸자. 커넥션 플래그는 커넥트 overlapped안의 issued를
  // 사용하자. 로컬 어드레스로 바인딩... NIC가 여러개일경우를 지칭한다.
  if (!lp->tcp_transport_.socket_->Bind(*owner_->local_nic_addr_, 0)) {
    // 서버에게도 노티하자.
    owner_->EnqueueError(ResultInfo::From(ResultCode::TCPConnectFailure,
                                          owner_->local_host_id_,
                                          "ConnectSocket bind failure."));
    return true;
  }
  //@maxidea: debugging
  // else {
  ...
  const SocketErrorCode socket_error =
      lp->tcp_transport_.socket_->ConnectEx(lp->listen_addr_from_server_);
  fun_check(socket_error == SocketErrorCode::Ok);

  //@maxidea: debugging
  //_tprintf("[!] Connecting to remote-peer (%s)    socket_error=%s\n",
  //*lp->listen_addr_from_server_.ToString(),
  //SocketErrorCode::ToString(socket_error));
  return true;
}
...
IMPLEMENT_RPCSTUB_LanS2C_ShutdownTcpAck(LanClientImpl::S2CStub) {
  ...
  if (owner_->listener_thread_ && owner_->shutdown_issued_time_ > 0 &&
      owner_->graceful_disconnect_timeout_ != 0.0) {
    owner_->shutdown_issued_time_ =
        owner_->GetAbsoluteTime() - (owner_->graceful_disconnect_timeout_ * 2);
  }

  // 서버에게 shutdown요청에 대한 응답을 보냄.
  owner_->c2s_proxy_.ShutdownTcpHandshake(HostId_Server,
...
IMPLEMENT_RPCSTUB_LanS2C_RequestAutoPrune(LanClientImpl::S2CStub) {
  ...
  if (owner_->intra_logger_) {
    owner_->intra_logger_->WriteLine(LogCategory::System,
                                     "RequestAutoPrune receive.");
  }

  // 서버와의 연결을 당장 끊는다.
  // Shutdown-shake 과정을 할 필요가 없다. 클라는 디스가 불특정 시간에 일어나는
  // 셈이므로.
  if (owner_->GetState() <= ConnectionState::Connected) {
    // Disconnect
    owner_->EnqueueDisconnectionEvent(ResultCode::DisconnectFromRemote,
...
void LanClientImpl::RemoveRemotePeerIfNoGroupRelationDetected(
    LanRemotePeer_C* member_lp) {
  ...
  IssueDisposeRemotePeer(member_lp, ResultCode::DisconnectFromRemote,
                         ResultCode::Ok, ByteArray(), __FUNCTION__,
                         SocketErrorCode::Ok);
}
...
LanClientImpl::LanClientImpl()
    : server_as_send_dest_(this),
      ...
      purge_too_old_unmature_peer_alarm_(
          NetConfig::purge_too_old_joining_timeout_interval_sec),
      remove_tool_old_tcp_send_packet_queue_alarm_(
          NetConfig::udp_packet_board_long_interval_sec),  //@temp
      dispose_issued_remote_peers_alarm_(
      ...
      accepted_peer_dispost_alarm_(
          NetConfig::dispose_issued_remote_clients_timeout_sec),
      remove_lookahead_message_alarm_(
          NetConfig::remove_lookahead_msg_timeout_interval_sec),
...
LanClientImpl::~LanClientImpl() {
  ...
  // 아래 과정은 unsafe heap을 쓰는 것들을 mutex_ lock 상태에서 청소하지 않으면
  // 괜한 assert fail이 발생하므로 필요
  CScopedLock2 main_guard(mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);

  // CompletionPort와 ToServerTcp는 어차피 Disconnect내부에서 삭제되므로 여기서
  // 따로 하지 않음.
  // completion_port_.Reset();
  p2p_groups_.Clear();  // 확인사살
  // to_server_tcp_.Reset(); // 이것들도 unsafe heap을 참조하는 것들을
  // 보유하므로
...
bool LanClientImpl::Connect(LanConnectionArgs& args,
  ...
  LockMain_AssertIsNotLockedByCurrentThread();
  {
    ...
    if (state != ConnectionState::Disconnected) {
      ...
      const String text = String::Format(
          "Already connected.  (current_state: %s)", *ToString(state));
      out_error =
          ResultInfo::From(ResultCode::AlreadyConnected, HostId_None, text);
      return false;
    }
  }
  ...
  // DisconnectNotWait함수가 호출된후 connect를 하게 되면 문제가 생길수
  // 있는경우가 있어 코딩되었다.
  ...
  // ConnectionState::Disconnecting단계에서, 처리가 완료되면
  // ConnectionState::Disconnected로 넘어가는것인지 확인할 필요가 있어보임.
  //이곳에서 호출하면 문제가 발생함.
  ...
  //@todo 이게 해제되는게 Cleanup()함수에 있는데, 명시적으로 Disconnect()하기
  //전에는
  ...
  if (has_multiple_nics && args.local_nic_addr_.IsEmpty()) {
    ...
    // "Server has multiple network devices, however, no device is specified for
    // listening. Is it your intention?"));

    // TODO 이걸 보고해야할 오류라고 봐야하나??
    //경고정도로 끝내도 좋을듯 싶은데...
    ErrorReporter::Report(
        String::Format("LanClientImpl.Connect - No NIC binding though multiple "
                       "NIC detected##Process=%s",
  ...
  // (disconnected state에서도 이게 안 비어있을 수 있다. 왜냐하면 서버에서
  // 추방직전 쏜 RPC를 클라가 모두 처리하려면 disconnected state에서도 미처리
  // 항목을 유지해야 하기 때문이다.)
  ...
  purge_too_old_unmature_peer_alarm_ =
      IntervalAlaram(NetConfig::purge_too_old_joining_timeout_interval_sec);
  remove_tool_old_tcp_send_packet_queue_alarm_ =
  ...
  disconnect_remote_peer_on_timeout_alarm_ =
      IntervalAlaram(NetConfig::cs_ping_interval_sec);
  accepted_peer_dispost_alarm_ =
      IntervalAlaram(NetConfig::dispose_issued_remote_clients_timeout_sec);
  remove_lookahead_message_alarm_ =
      IntervalAlaram(NetConfig::remove_lookahead_msg_timeout_interval_sec);
  ...
  if (args.network_thread_count == 0) {
    args.network_thread_count =
        CPlatformMisc::NumberOfCoresIncludingHyperthreads();  //@todo 이게 좀
                                                              //맞는 처리인지??
  }
  ...
  // 이 에러 처리가 필요 하다.
  if (!CreateTcpConnectSocketAndInit(connection_params_.server_ip,
                                     connection_params_.server_port,
                                     out_error)) {
    //@todo 정리 차원에서 할뿐..
    state_ = ConnectionState::Disconnected;
    ...
    out_error = ResultInfo::From(ResultCode::Unexpected, HostId_None,
                                 "Connect socket create failed.");
    return false;
  }
  ...
  // 주의 : 모든 타이머는 초기화 이슈등을 피하기 위해서, 0.1초 이후부터 작동하기
  // 시작함. 구지 이렇게 해야할까 싶긴한데...
  ...
  // 3ms ?
  // 좀 짧은 감이 없지 않아 있는데, 즉 0.003초안의 coalescing을 수행하겠다는
  // 얘기임. (0.003초 안의 전송은 모아서 하겠다는 거고, 대기 시간을 너무 길게
  // 잡을 경우에는 반응성이 떨어질 수 있음.)
  ...
  //@todo 나중에는 이런 초딩짓이 아니라 모든 스레드의 시작이 준비되는 순간에
  //리턴하게 만들자. CountdownLatch를 사용하면 좋을듯...
...
bool LanClientImpl::CreateTcpListenSocketAndInit(
    SharedPtr<ResultInfo>& out_error) {
  ...
  if (!tcp_listening_socket_->Bind(*local_nic_addr_,
                                   connection_params_.p2p_listening_port)) {
...
bool LanClientImpl::CreateTcpConnectSocketAndInit(
    const String& ip, int32 port, SharedPtr<ResultInfo>& out_error) {
  ...
  if (!to_server_tcp_->socket_->Bind(*local_nic_addr_, 0)) {
    out_error = ResultInfo::From(ResultCode::TCPConnectFailure, local_host_id_,
                                 "ConnectSocket bind failure.");
    return false;
  }
...
bool LanClientImpl::Send_BroadcastLayer(const SendFragRefs& payload,
  ...
  if (!to_server_tcp_.IsValid() ||
      local_host_id_ == HostId_None) {  // 이미 서버와 연결 해제된 상태면 즐
  ...
  Array<LanRemotePeer_C*, InlineAllocator<256>> p2p_send_list(
  ...
  for (int32 send_dest_index = 0; send_dest_index < send_dest_list_count;
    ...
    if (send_dest == this && send_opt.bounce) {
      ...
      switch (msg_type) {
        case MessageType::RPC:
          final_user_work_item_type = FinalUserWorkItemType::RPC;
          break;
        case MessageType::FreeformMessage:
          final_user_work_item_type = FinalUserWorkItemType::FreeformMessage;
          break;
        default:
          fun_check(0);
          break;
      ...
      final_user_work_queue_.Enqueue(FinalUserWorkItem_S(
          payload_without_msg_type, final_user_work_item_type));
    ...
    // NOTE: truely lookahead는 아니지만 어쨌거나 서버에서 P2P member join
    // 요청에 대한 반응을 한 적이 있다면 유효한 remote Peer로 처리한다. P2P
    // member join 요청이 안온 것에 대해서까지 lookahead하지는 않고 있다.
    else if (settings_.look_ahead_p2p_send_enabled && send_dest &&
      ...
      AddLookaheadP2PSendQueueMap(send_dest->GetHostId(),
                                  payload.ToBytes());  // copy
  ...
  for (int32 send_index = 0; send_index < p2p_send_count; ++send_index) {
    ...
    {
      CScopedLock2 lp_tcp_send_queue_guard(
          lp->tcp_transport_.GetSendQueueMutex());
...
void LanClientImpl::Send_ToServer_Directly_Copy(
    MessageReliability Reliability, const SendFragRefs& data_to_send) {
  CScopedLock2 to_server_tcp_guard(to_server_tcp_mutex_);
  to_server_tcp_->SendWhenReady(data_to_send, TcpSendOption());
}

void LanClientImpl::ConvertGroupToIndividualsAndUnion(int32 sendto_count,
                                                      const HostId* sendto_list,
                                                      HostIdArray& output) {
  CScopedLock2 main_guard(mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
...
void LanClientImpl::ConvertGroupToIndividualsAndUnion(
...
bool LanClientImpl::ConvertAndAppendP2PGroupToPeerList(
    HostId sendto, ILanSendDestList_C& SendTo2) {
  LockMain_AssertIsLockedByCurrentThread();
...
ResultCode LanClientImpl::ExtractMessagesFromTcpStream(
  ...
  const int32 added_count = MessageStream::ExtractMessagesAndFlushStream(
      to_server_tcp_->recv_stream_, output, HostId_Server,
      settings_.message_max_length, extract_result);
  if (added_count < 0) {
    // 서버와의 TCP 연결에 문제가 있다는 뜻이므로 연결 해제를 유도한다.
    // 데드락의 문제가 있어 삭제.
    // EnqueueError(ResultInfo::From(extract_result, HostId_Server, "Received
    // stream from TCP server became inconsistent.")); //InduceDisconnect();연결
    // 해제 유도.
...
void LanClientImpl::EnqueueDisconnectionEvent(ResultCode result_code,
...
INetCoreCallbacks* LanClientImpl::GetCallbacks_NOLOCK() { return callbacks_; }
...
void LanClientImpl::ProcessOneLocalEvent(LocalEvent& event) {
  ...
  if (callbacks_) {
    try {
      //@maxidea: LanClient는 싱글 스레드가 아니므로, 아래의 플래그들에 해당하는
      //동작들은 유효하지 않을듯 싶은데??

      callbacks_->bHolsterMoreCallback_FORONETHREADEDMODEL =
          false;  // one thread model인 클라라서 안전
      ...
      switch (event.type) {
        ...
        case LocalEventType::P2PAddMember:
          callbacks_->OnP2PMemberJoined(event.member_id, event.group_id,
                                        event.member_count, event.custom_field);
          break;
        ...
        case LocalEventType::Error:
          callbacks_->OnError(ResultInfo::From(event.result_info->result_code,
                                               event.remote_id,
        ...
        case LocalEventType::Warning:
          callbacks_->OnWarning(ResultInfo::From(event.result_info->result_code,
                                                 event.remote_id,
                                                 event.result_info->comment)
                                    .Get());
          break;
        ...
        case LocalEventType::P2PDisconnected:
          callbacks_->OnP2PDisconnected(event.remote_id,
...
bool LanClientImpl::NextEncryptCount(HostId remote_id,
...
bool LanClientImpl::GetExpectedDecryptCount(HostId remote_id,
                                            CryptoCountType& out_count) {
  CScopedLock2 main_guard(mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
...
SessionKey* LanClientImpl::GetCryptSessionKey(HostId remote_id,
                                              String& out_error) {
  CScopedLock2 main_guard(mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
  ...
  if (local_host_id_ == remote_id) {
    ...
    if (key == nullptr) {
      out_error =
          String::Format("%d remote lp is %s in LanClient.", (int32)remote_id,
                         lp == nullptr ? "NULL" : "not NULL");
    }
  }
...
bool LanClientImpl::AsyncCallbackMayOccur() { return listener_thread_; }
...
void LanClientImpl::IssueDisposeRemotePeer(
    LanRemotePeer_C* lp, ResultCode result_code, ResultCode detail_code,
...
void LanClientImpl::DisconnectRemotePeerOnTimeout() {
  ...
  for (auto it = authed_remote_peers_.CreateIterator(); it; ++it) {
    ...
    if ((absolute_time - lp->last_tcp_stream_recv_time_) >
        settings_.default_timeout_sec) {
      NotifyP2PDisconnected(lp, ResultCode::TimeOut);

      // if (IssueDisposeRemotePeer(lp, ResultCode::DisconnectFromRemote,
      // ResultCode::ConnectServerTimeout, ByteArray(), __FUNCTION__,
      // SocketErrorCode::Ok, false)) {
...
void LanClientImpl::DisposeIssuedRemotePeers() {
  ...
  for (auto it = dispose_issued_remote_peers_map_.CreateIterator(); it; ++it) {
    ...
    const bool tcp_close_safe =
        !lp->tcp_transport_.recv_issued_ && !lp->tcp_transport_.send_issued_;

    // 아직 처리중이라면, 대기를 해야함.
    const bool works_remain =
        (lp->task_subject_node_.GetListOwner() || lp->IsTaskRunning());
...
void LanClientImpl::ProcessOnPeerDisposeCanSafe(LanRemotePeer_C* lp) {
  ...
  // delete되기 전에 enque하는 것이므로 다른 쓰레드에서 이 RemoteClient에 대한
  // 이벤트가 나오지 않을 것이므로, 이렇게 처리해도 무방하다.
  // if (lp->dispose_waiter_) {
  //  EnqueuePeerLeaveEvent(lp, lp->dispose_waiter_->reason,
  //  lp->dispose_waiter_->detail, lp->dispose_waiter_->comment,
  //  lp->dispose_waiter_->socket_error);
  //}
  // else {
  //  EnqueuePeerLeaveEvent(lp, ResultCode::DisconnectFromLocal,
  //  ResultCode::ConnectServerTimeout, ByteArray(), SocketErrorCode::Ok);
  //}
...
void LanClientImpl::EnqueuePeerLeaveEvent(LanRemotePeer_C* lp,
...
void LanClientImpl::EnqueuePeerDisconnectEvent(LanRemotePeer_C* lp,
                                               ResultCode result_code) {
  LockMain_AssertIsLockedByCurrentThread();
...
void LanClientImpl::Disconnect(double graceful_disconnect_timeout,
                               const ByteArray& comment) {
  CScopedLock2 connect_disconnect_phase_guard(connect_disconnect_phase_mutex_);
...
//{
//  DisconnectNoWait(NetConfig::default_graceful_disconnect_timeout_sec,
//  ByteArray());
//}
//
//
//
// void LanClientImpl::DisconnectNoWait(double graceful_disconnect_timeout_sec,
// const ByteArray& comment)
//{
...
void LanClientImpl::Disconnect_INTERNAL(double graceful_disconnect_timeout_sec,
                                        const ByteArray& comment) {
  LockMain_AssertIsNotLockedByCurrentThread();
  ...
  const uint32 timeout = MathBase::Max<uint32>(
      (uint32)(graceful_disconnect_timeout_sec * 2 * 1000), 10000);
  ...
  // TODO 하트비트가 안불리므로, 시간 갱신이 안될터인데... 문제점을 좀 짚어볼
  // 필요가 있음.
  ...
  while (true) {
    ...
    if (GetState() == ConnectionState::Disconnected &&
        remote_peer_instances_.IsEmpty()) {
      safe_disconnect = true;
      break;
    }
    ...
    if ((Clock::Milliseconds() - t0) > timeout) {
      const String text = String::Format(
          "LanClient.Disconnect seems to be freezed ## process: %s",
          CPlatformProcess::ExecutableName());
    ...
    if (!issue_disconnect_done) {
      ...
      if (GetState() == ConnectionState::Connected) {
        // 언제 디스 작업을 시작할건가를 지정
        shutdown_issued_time_ =
            GetAbsoluteTime();  // 이거에 의해 Networker State가
                                // disconnecting으로 곧 바뀐다.
...
void LanClientImpl::EnqueueConnectFailEvent(ResultCode result_code,
                                            SharedPtr<ResultInfo> result_info) {
  CScopedLock2 main_guard(mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
  ...
  if (!suppress_subsequenct_disconnection_events_) {
    ...
    event.result_info->comment = result_info->comment;
    event.remote_id = HostId_Server;
    event.remote_addr = InetAddress(connection_params_.server_ip,
                                    connection_params_.server_port);
...
void LanClientImpl::EnqueueHackSuspectEvent(LanRemotePeer_C* lp,
...
String LanClientStats::ToString() const {
  return String::Format(
      "remote_peer_count: %d, total_recv_bytes: %I64d, total_send_bytes: %I64d",
...
void LanClientImpl::UpdateP2PGroup_MemberJoin(
    ...
    uint32 event_id, const ByteArray& p2p_aes_session_key,
    const ByteArray& p2p_rc4_session_key, const Uuid& connection_tag) {
  LockMain_AssertIsLockedByCurrentThread();
    ...
    if (member_id != local_host_id_ && member_lp == nullptr) {
      ...
      if (!p2p_aes_session_key.IsEmpty()) {
        if (!CryptoAES::ExpandFrom(
                member_lp->p2p_session_key.aes_key,
      ...
      if (!p2p_rc4_session_key.IsEmpty()) {
        if (!CryptoRC4::ExpandFrom(
                member_lp->p2p_session_key.rc4_key,
...
void LanClientImpl::ConditionalAssignRemotePeerInfo(LanRemotePeer_C* lp) {
  ...
  if (auto dispose_lp =
          dispose_issued_remote_peers_map_.FindRef(lp->host_id_)) {
    lp->task_running_ = dispose_lp->task_running_;

    // 일거리를 옮긴다.
    // DisposeRP의 FinalUserworkItemList를 lp->FinalUserworkItemList쪽으로 옮겨
    // 주는건가???
...
bool LanClientImpl::Send(const SendFragRefs& data_to_send,
  ...
  //@note
  // Send_CompressLayer -> Send_SecureLayer -> Send_BroadcastLayer 순으로
  // 전파됨. 이름을 살짝 변경해주는게 좋지 않을까? Send_RootLayer? 좋은 이름이
  // 생각이 안난다???
...
void LanClientImpl::EnableVizAgent(const char* viz_server_ip,
...
void LanClientImpl::Viz_NotifySendByProxy(const HostId* sento_list,
  ...
  //  UnsafeMemory::Memcpy(targets.MutableData(), sento_list, sizeof(HostId) *
  //  sendto_count);
  //
  //  viz_agent_->c2s_proxy_.NotifyCommon_SendRpc(HostId_Server,
  //  GReliableSend_INTERNAL, targets, rpc_name, rpc_id);
  //}
}

void LanClientImpl::Viz_NotifyRecvToStub(HostId rpc_recvfrom, RpcId rpc_id,
                                         const char* rpc_name,
...
//@todo 왜 이래해야 하는걸까??
void LanClientImpl::ShowNotImplementedRpcWarning(RpcId rpc_id,
                                                 const char* rpc_name) {
  NetCoreImpl::ShowNotImplementedRpcWarning(rpc_id, rpc_name);
}

//@todo 왜 이래해야 하는걸까??
void LanClientImpl::PostCheckReadMessage(IMessageIn& msg, RpcId rpc_id,
                                         const char* rpc_name) {
...
InetAddress LanClientImpl::GetServerAddress() {
  ...
  return to_server_tcp_.IsValid() ? to_server_tcp_->socket_->GetPeerName()
                                  : InetAddress::None;
}
...
double LanClientImpl::GetServerTimeDiff() { return dx_server_time_diff_; }
...
double LanClientImpl::GetLastPingSec(HostId remote_id) {
  ...
  if (auto group = GetP2PGroupByHostId_INTERNAL(remote_id)) {
    ...
    double sum = 0;
    for (const auto& member_pair : group->members_) {
      //@maxidea: 핑만 조회해도 JIT P2P를 시도함...(뭐 나쁜 선택은 아닌듯 보이긴
      //한데...)
...
uint32 LanClientImpl::GetInternalVersion() { return internal_version_; }

InetAddress LanClientImpl::GetPublicAddress() {
  CScopedLock2 main_guard(mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);

  return to_server_tcp_ ? to_server_tcp_->local_addr_at_server_
                        : InetAddress::None;
}
...
void LanClientImpl::OnCompletionPortWarning(CompletionPort* port,
                                            const char* msg) {
  CScopedLock2 main_guard(mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
...
HostId LanClientImpl::GetHostId() const { return local_host_id_; }
...
void LanClientImpl::ConditionalRequestServerTime() {
  if (local_host_id_ != HostId_None) {
    ...
    if ((absolute_time - last_request_server_time_time_) >
      ...
      {
        CScopedLock2 to_server_tcp_guard(to_server_tcp_mutex_);
        to_server_tcp_->SendWhenReady(SendFragRefs(msg_to_send),
                                      TcpSendOption());
      }

      if (intra_logger_) {
        intra_logger_->WriteLine(LogCategory::System,
                                 "SendRequestServerTimeAndKeepAlive");
      }
    }
...
void LanClientImpl::ConditionalSyncIndirectServerTime() {
  ...
  Array<LanRemotePeer_C*, InlineAllocator<256>> collected_lp_list(peer_count,
                                                                  NoInit);
  ...
  for (auto& pair : authed_remote_peers_) {
    ...
    if (lp && lp->host_id_ != HostId_Server) {
      ...
      if (lp->sync_indirect_server_time_diff_cooltime_ <= 0) {
        ...
        lp->sync_indirect_server_time_diff_cooltime_ =
...
#pragma omp parallel for
    ...
    for (request_index = 0; request_index < request_count; ++request_index) {
      ...
      CScopedLock2 lp_tcp_send_queue_guard(
          lp->tcp_transport_.GetSendQueueMutex(), false);
      ...
      if (request_index != 0) {
        if (const bool lock_ok = lp_tcp_send_queue_guard.TryLock()) {
          ...
          // 동기화된 시간, 랙)을 보낸다. 이걸 받은 상대는 해당 peer 기준으로의
          // time diff 값을 갖고 있는다. 모든 Peer로부터 이값을 받으면 그리고
          // Peer가 속한 각 P2P group 범위 내에서의 time diff 평균값을 계산한다.
          //
          ...
          lp->DecreaseUseCount();
          collected_lp_list[request_index] =
              collected_lp_list[--request_count];  // 맨 끝에 있는 것을 앞으로
                                                   // 땡겨온다.
        }
      } else {
...
void LanClientImpl::Tcp_LongTick() {
  ...
  const int32 total_remote_peer_count =
      authed_remote_peers_.Count() + candidate_remote_peers_.Count();
  if (total_remote_peer_count <= 0) {
    return;
  }

  Array<LanRemotePeer_C*, InlineAllocator<256>> collected_lp_list(
      total_remote_peer_count, NoInit);
...
void LanClientImpl::ConditionalReportLanP2PPeerPing() {
  ...
  for (auto& pair : authed_remote_peers_) {
    ...
    if (lp->last_report_p2p_peer_ping_cooltime_ <= 0) {
      lp->last_report_p2p_peer_ping_cooltime_ +=
          NetConfig::report_lan_p2p_peer_ping_interval_sec;
      c2s_proxy_.ReportP2PPeerPing(HostId_Server, GReliableSend_INTERNAL,
                                   lp->host_id_,
                                   (uint32)(lp->recent_ping_ * 1000));
    }
  }
}
...
void LanClientImpl::PurgeTooOldUnmaturePeer() {
  ...
  for (auto& pair : candidate_remote_peers_) {
    ...
    // 서버에 노티하도록 하자.
    if ((absolute_time - lp->created_time_) >
            NetConfig::lan_peer_connect_peer_timeout_sec &&
        !lp->dispose_requested_) {
...
//타임아웃 된것들은 아래서 제거를 하긴 하는데, 정상적으로 처리된건 어디에서
//삭제를 하는거지??
void LanClientImpl::PurgeTooOldUnmatureAcceptedPeers() {
  ...
  for (auto it = accepted_peers_.CreateIterator(); it; ++it) {
    ...
    if (!accepted_info->socket_) {
      // 소켓 객체가 해제 되어서 제거한다??  그런데, 소켓 객체가 왜 무효화
      // 된걸까??
      ...
      if (accepted_info->GetUseCount() == 0) {
        ...
        //@maxidea: 이경우, 계속 처리가 안되고 대기를 하게 된다면, 누수로
        //이어질듯 한데??? _tprintf("    Pended: UseCount=%d\n"),
    ...
    // 바로 폐기하지는 않고, 다음 턴에 폐기되도록 표시만 해둔다.(is_timedout =
    // true)
    else if ((absolute_time - accepted_info->accepted_time_) >
             NetConfig::lan_peer_connect_peer_timeout_sec) {
...
void LanClientImpl::NotifyP2PDisconnected(LanRemotePeer_C* lp,
                                          ResultCode reason) {
  LockMain_AssertIsLockedByCurrentThread();

  if (!lp->dispose_requested_ &&
      !dispose_issued_remote_peers_map_.Contains(lp->host_id_)) {
    MessageOut reply;
...
double LanClientImpl::GetReliablePingTimerInterval() {
  // 연속 두세번 보낸 것이 일정 시간 안에 도착 안하면 사실상 막장이므로 이정도
  // 주기면 OK.
...
void LanClientImpl::Cleanup() {
  ...
  dispose_issued_remote_peers_map_
      .Clear();  // connect, disconnect시에 이것을 clear하는게 맞는듯하다.
  CleanupPeerGet();
...
void LanClientImpl::EveryClientDispose() {
  ...
  for (auto& pair : candidate_remote_peers_) {
    ...
    IssueDisposeRemotePeer(lp, ResultCode::DisconnectFromLocal,
                           ResultCode::TCPConnectFailure, ByteArray(),
                           __FUNCTION__, SocketErrorCode::Ok, false);
  }
  candidate_remote_peers_.Clear();
...
void LanClientImpl::AddLookaheadP2PSendQueueMap(HostId peer_id,
...
ITaskSubject* LanClientImpl::GetTaskSubjectByHostId_NOLOCK(
  ...
  if (subject == nullptr)  // && settings_.look_ahead_p2p_send_enabled)
                           // //HostTag에서 candidate를 못빼서 문제가 생김...
  {
    ...
    if (auto dispose =
            dispose_issued_remote_peers_map_.FindRef(subject_host_id)) {
      return dispose;
    }
  }
...
void LanClientImpl::CheckCriticalSectionDeadLock_INTERNAL(const char* where) {
  ...
  for (auto lp : remote_peer_instances_) {
    if (lp->tcp_transport_.GetMutex().IsLockedByCurrentThread()) {
      LOG(LogNetEngine, Warning, "LanClient LanRemtePeer DeadLock! - %s",
          where);
    }
  }
  ...
  for (auto& pair : accepted_peers_) {
    ...
    if (accepted_info->GetMutex().IsLockedByCurrentThread()) {
      LOG(LogNetEngine, Warning, "LanClient AcceptedInfo DeadLock! - %s",
          where);
    }
  }

  if (to_server_tcp_mutex_.IsLockedByCurrentThread()) {
    LOG(LogNetEngine, Warning, "LanClient to_server_tcp_mutex_ DeadLock! - %s",
        where);
  }

  if (tcp_issue_queue_mutex_.IsLockedByCurrentThread()) {
    LOG(LogNetEngine, Warning, "LanClient tcpissuequeuecs DeadLock! - %s",
        where);
  }
}
...
void LanClientImpl::IoCompletion_ToServerTcp(
    CompletionStatus& completion, ReceivedMessageList& received_msg_list) {
  LockMain_AssertIsNotLockedByCurrentThread();
  to_server_tcp_mutex_.AssertIsNotLockedByCurrentThread();

  // CTcpLayer_C로 캐스팅이 성공하면 ToServer. CLanRemotePeer_C로 캐스팅이
  // 성공하면 remote peer.
  auto tcp_transport =
      LeanDynamicCastTcpTransport_C(completion.completion_context);
    ...
    if (completion.type == CompletionType::Send) {
      ...
      if (completion.completed_length < 0) {
        ...
        if (GetState() < ConnectionState::Disconnecting) {
          ...
          EnqueueDisconnectionEvent(ResultCode::DisconnectFromRemote,
                                    ResultCode::TCPConnectFailure);
          ...
          if (intra_logger_) {
            const String text = String::Format(
                "Send CompletedDatalength<=0.  host_id: %d, socket_error: %d",
                (int32)GetLocalHostId(), (int32)completion.socket_error);
            intra_logger_->WriteLine(LogCategory::System, *text);
          }
        }
      } else {
        // 보내기 완료
        if (completion.completed_length > 0) {
          // 송신 큐에서 완료된 만큼의 데이터를 제거한다. 그리고 다음 송신을
          // 건다. (최종 completion 상황이라 하더라도 이 과정은 필수!)
      ...
      try {
        ...
        if (completion.completed_length <= 0) {
          ...
          if (GetState() < ConnectionState::Disconnecting) {
            ...
            if (intra_logger_) {
              const String text = String::Format(
                  "Recv CompletedDatalength<=0.  host_id: %d, socket_error: %d",
                  (int32)GetLocalHostId(), (int32)completion.socket_error);
              intra_logger_->WriteLine(LogCategory::System, *text);
            }
          }
          ...
          // 수신 큐에서 받은 데이터를 꺼낸 후 ...
          tcp_transport->recv_stream_.EnqueueCopy(
              tcp_transport->socket_->GetRecvBufferPtr(),
          ...
          const ResultCode result_code =
              ExtractMessagesFromTcpStream(received_msg_list);

          to_server_tcp_guard.Unlock();

          if (result_code != ResultCode::Ok) {
            EnqueueError(
                ResultInfo::From(result_code, HostId_Server,
                                 "Received stream from TCP server became "
                                 "inconsistent."));  // InduceDisconnect();연결
                                                     // 해제 유도.
          }

          {
            CScopedLock2 pre_final_recv_queue_guard(
                pre_final_recv_queue_mutex_);

            for (auto& received_msg : received_msg_list) {
              // PreFinalRecvQueue는 따로 lock을 사용
              // received_msg.ActionName = owner_->GetAbsoluteTime() +
              // owner_->GetSimLag();
        ...
        if (GetState() < ConnectionState::Disconnecting) {
          ...
          if (socket_error != SocketErrorCode::Ok) {
            const String text = String::Format(
                "%d TCP IssueRecv failed socket_error", (int32)socket_error);
            EnqueueDisconnectionEvent(ResultCode::TCPConnectFailure,
        ...
        CatchThreadUnexpectedExit(
            __FUNCTION__,
            *String::Format("std.exception(%s)",
                            (const char*)UTF8_TO_TCHAR((e.what()))));
      }
      // catch (_com_error& e) {
      //  CatchThreadUnexpectedExit(__FUNCTION__,
      //  *String::Format("_com_error(%s)", (const char*)e.Description()));
      //}
      // catch (void*) {
      ...
      // 커넥트 완료 되었음
      const SocketErrorCode socket_error =
          tcp_transport->socket_->ConnectExComplete();
...
void LanClientImpl::IoCompletion_PerRemotePeer(
    CompletionStatus& completion, ReceivedMessageList& received_msg_list) {
  LockMain_AssertIsNotLockedByCurrentThread();
...
void LanClientImpl::IoCompletion_TcpRecvCompletionCase(
  ...
  if (completion.completed_length <= 0) {
    // const String comment = String::Format("%s:%d", __FUNCTION__,
    ...
    // owner_->IssueDisposeRemotePeer(lp, ResultCode::DisconnectFromRemote,
    // ResultCode::TCPConnectFailure, ByteArray(), __FUNCTION__,
    // completion.socket_error); lp->WarnTooShortDisposal(__FUNCTION__);

    // if (owner_->intra_logger_) {
    //  const String text = String::Format("서버: 클라이언트 연결 해제. host_id:
    //  %d, addr: %s, error_code: %d",
    //              lp->host_id_,
    ...
    // Console.WriteLine(String.Format("서버: {0}바이트 받았음.",
    // completion.completed_length));
    ...
    // 수신 큐에서 받은 데이터를 꺼낸 후 ...
    lp->tcp_transport_.recv_stream_.EnqueueCopy(
        lp->tcp_transport_.socket_->GetRecvBufferPtr(),
        completion.completed_length);
    ...
    const ResultCode result_code =
        lp->ExtractMessagesFromTcpStream(received_msg_list);

#ifdef TRACE_LOOKAHEADSEND
    // FUN_TRACE("Receive lp %d->%d\n", (int)lp->GetHostId(),
    // (int)owner_->GetLocalHostId());
#endif
    ...
    if (result_code != ResultCode::Ok) {
      ...
      EnqueueError(ResultInfo::From(ResultCode::TCPConnectFailure, lp->host_id_,
                                    "Extraction failed"));
...
void LanClientImpl::IoCompletion_TcpSendCompletionCase(
    CompletionStatus& completion, LanRemotePeer_C* lp) {
  LockMain_AssertIsNotLockedByCurrentThread();
...
void LanClientImpl::IoCompletion_TcpConnectExCompletionCase(
    CompletionStatus& completion, LanRemotePeer_C* lp) {
  LockMain_AssertIsNotLockedByCurrentThread();
  //여기서는 MainLock로 진행해도 무방할듯.그리 많은 connectex가 일어나지
  //않을테니까.
  CScopedLock2 main_guard(mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
  ...
  if (completion.socket_error != SocketErrorCode::Ok) {
    ...
    const SocketErrorCode result_code =
        lp->tcp_transport_.socket_->ConnectExComplete();
    if (result_code != SocketErrorCode::Ok) {
...

      {
        CScopedLock2 lp_tcp_send_queue_guard(
            lp->tcp_transport_.GetSendQueueMutex());
        lp->tcp_transport_.SendWhenReady(SendFragRefs(msg_to_send),
                                         TcpSendOption());
      }
...
void LanClientImpl::IoCompletion_AcceptedInfo(
    CompletionStatus& completion, ReceivedMessageList& received_msg_list) {
  LockMain_AssertIsNotLockedByCurrentThread();
  ...
  try {
    ...
    if (accepted_info) {
      ...
      if (completion.type == CompletionType::ReferCustomValue) {
        ...
        IoCompletion_TcpRecvCompletionCase(completion, received_msg_list,
                                           accepted_info);
      }
    }
  } catch (std::exception&) {
...
void LanClientImpl::IoCompletion_TcpPeerAcceptedCase(
    CompletionStatus& completion, AcceptedInfo* accepted_info) {
  LockMain_AssertIsNotLockedByCurrentThread();
...
void LanClientImpl::IoCompletion_NewAcceptedPeerCase(
...
void LanClientImpl::IoCompletion_TcpRecvCompletionCase(
  ...
  if (completion.completed_length <= 0) {
    ...
    // 수신 큐에서 받은 데이터를 꺼낸 후 ...
    accepted_info->recv_stream_.EnqueueCopy(
        accepted_info->socket_->GetRecvBufferPtr(),
        completion.completed_length);
    ...
    for (auto& received_msg : received_msg_list) {
      ...
      ProcessMessage_EngineLayer(
          received_msg,
          accepted_info);  // AccpetedInfo는 TaskSubject를 사용치 않는다.
    }
    ...
    if (accepted_info->socket_.IsValid()) {
      // 여기서 Socket에 대해 Issue Recv를 하게되면 CTcpLayer_S의 m_issueRecv가
      // 갱신되지 않는 문제가 있다. 따라서 CAcceptInfo -> CRemotePeer_S로 이양이
      // 된뒤에 IssueRecv하는걸로 수정되었음. 다음 recv를 건다.소켓이
      // RemotePeer이 인양될수 있으므로 이렇게 처리.
      const SocketErrorCode socket_error =
          accepted_info->socket_->IssueRecv(NetConfig::tcp_issue_recv_length);
      if (socket_error != SocketErrorCode::Ok) {
        ...
        EnqueueError(ResultInfo::From(
            ResultCode::Unexpected, GetLocalHostId(),
            "FATAL: Cannot new peer accept socket receive issue."));
...
void LanClientImpl::ProcessMessage_NotifyConnectionPeerRequestData(
  ...
  if (!lf::Reads(msg, msg_remote_host_id, msg_server_instance_tag,
                 msg_internal_version, msg_signature)) {
    //검증 실패를 노티한다.
  ...
  // 일정시간 후 AcceptInfo 객체는 PurgeTooOldUnmatureAcceptedPeers()에서
  // 제거됨.
  ...
  if (accepted_info->recv_stream_.GetLength() > 0) {
    ...
    // CMessage MoveStream;
    // MoveStream.UseExternalBuffer(accepted_info->recv_stream_.GetData(),
    // accepted_info->recv_stream_.GetLength());
    ...
    SendFragRefs data;
    data.Add(accepted_info->recv_stream_.ConstData(),
             accepted_info->recv_stream_.GetLength());
...
void LanClientImpl::CatchThreadExceptionAndPurgeClient(LanRemotePeer_C* lp,
                                                       const char* where,
                                                       const char* reason) {
  if (lp) {
    if (callbacks_) {
      const String text =
          String::Format("%s에서 %s 이유로 클라이언트 %d를 추방합니다.", where,
                         reason, (int32)lp->host_id_);
      EnqueueError(
          ResultInfo::From(ResultCode::Unexpected, lp->host_id_, text));
    }

    IssueDisposeRemotePeer(lp, ResultCode::Unexpected,
                           ResultCode::TCPConnectFailure, ByteArray(),
...
void LanClientImpl::CatchThreadUnexpectedExit(const char* where,
                                              const char* reason) {
  if (callbacks_) {
    LockMain_AssertIsNotLockedByCurrentThread();
...
void LanClientImpl::Heartbeat_ConnectFailCase(SocketErrorCode socket_error) {
  ...
  if (GetState() < ConnectionState::Disconnecting) {
    ...
    //@warning Send_BroadcastLayer에서 괜시리 경고만 나오는 경우가 있어서, 일단
    //막아둠. local_host_id_ = HostId_None; // 서버와 연결이 해제됐으므로
    // 초기화한다.
...
void LanClientImpl::Heartbeat_Disconnecting() {
  ...
  if (disconnecting_mode_heartbeat_count_ == 1) {
    ...
    if (tcp_listening_socket_.IsValid() &&
      ...
      if (intra_logger_) {
        const String text = String::Format(
            "CloseSocketHandleOnly() called at %s (Listen)", __FUNCTION__);
        intra_logger_->WriteLine(LogCategory::System, *text);
      }
    }
    ...
    // TCP 소켓도 바로 끊어야 한다.
    if (to_server_tcp_.IsValid() &&
        to_server_tcp_->socket_->IsConnectExComplete()) {
      to_server_tcp_->socket_->CloseSocketHandleOnly();

      if (intra_logger_) {
        const String text = String::Format(
            "CloseSocketHandleOnly() called at %s (ToServer)", __FUNCTION__);
        intra_logger_->WriteLine(LogCategory::System, *text);
      }
    }
  ...
  // 어차피 async io는 이 스레드 하나에서만 이슈하기 떄문에 이 스레드 issue 없는
  // 상태인 것을 확인하면 더 이상 async io가 진행중일 수 없다.
  ...
  if (remote_peer_instances_.Count() > 0 || remote_peer_garbages_.Count() > 0) {
    ...
    safe_disconnect = false;
             ...
             (!to_server_tcp_->socket_->IsClosed() ||
  ...
  if (safe_disconnect) {
    ...
    // 서버측에서 RPC직후추방시 클라에서는 수신RPC를 폴링해야하는데 그게
    // 증발해버리니까. 대안을 찾자. 이러면 disconnected 상황이 되어도 미수신
    // 콜백을 유저가 폴링할 수 있다. 이래야 서버측에서 디스 직전 보낸 RPC를
    // 클라가 처리할 수 있다.

    //@warning Send_BroadcastLayer에서 괜시리 경고만 나오는 경우가 있어서, 일단
    //막아둠. local_host_id_ = HostId_None;
    ...
    //@fixme
    // net_thread_pool_
    // UserThreadPool에 여전히 접근할 경우, 서버에서 접속을 강제로 끊을 경우
    // 간혹 크래쉬가 발생함.
    ...
    //@maxidea:
    /*
    쓰레드풀 종료 루틴을 추가했음.  종료하지 않아서, Access Violation이
    일어나는것으로 보여서.. 스레드 풀을 종료해버리니, 다시 재접속이 안되는
    상황이 발생하네... 이걸 어떤식으로 접근해서 수정해야 할지???
    */
    // if (!net_thread_external_use_)
    //{
    //  if (net_thread_pool_ == user_thread_pool_) {
    ...
    */
    ...
    /*
    //tcp_accept_cp_.Reset();
    //.Reset(); //@maxidea: 이걸 닫아줘야 ThreadPool에서 무한 대기를 하지 않을듯
    싶은데??
  ...
  if (safe_disconnect == false &&
    ...
    const String text = String::Format(
        "Too long time elapsed since disconnecting mode.  "
        "unsafe_disconnect_reason: %d",
        unsafe_disconnect_reason);
    ErrorReporter::Report(*text);
...
void LanClientImpl::Heartbeat_Connected() {
  ...
  if ((GetAbsoluteTime() - last_tcp_stream_recv_time_) >
      settings_.default_timeout_sec) {
    CScopedLock2 main_guard(mutex_);
    CheckCriticalSectionDeadLock(__FUNCTION__);

    EnqueueDisconnectionEvent(ResultCode::DisconnectFromLocal,
  ...
  if (remove_tool_old_tcp_send_packet_queue_alarm_.TakeElapsedTime(
          elapsed_time)) {
    Tcp_LongTick();
  }
  ...
  //@maxidea: debug: 테스트를 위해서 임시로 막음... 단지, 디버깅이 곤란한점을
  //해소하기 위해서 임시로 막은것 뿐이니 반듯이 원래대로 돌려주어야함!
  ...
  while (true) {
    ...
    // TODO 오랜시간 대기를 할 수 있는건가???  시간 갱신을 이 루프 안에서
    // 해야하는지????
    UpdateFixedSteppedTimeVars();

    const double absolute_time = GetAbsoluteTime();

    // 서버와의 연결 해제 시작 상황이 된 후 서버와 오랫동안 연결이 안끊어지면
    // 강제 스레드 종료를 한다.
    if (shutdown_issued_time_ > 0 && (absolute_time - shutdown_issued_time_) >
                                         graceful_disconnect_timeout_) {
      CScopedLock2 main_guard(mutex_);
      CheckCriticalSectionDeadLock(__FUNCTION__);
    ...
    anything_did |=
        LoopbackRecvCompletionCase();  //@maxidea: 한번이라도 성공하면 계속
                                       //루푸를 돈다는 의미일까??
    if (GetState() == ConnectionState::Disconnecting) {
      return;
    }
...
bool LanClientImpl::ProcessMessage_EngineLayer(ReceivedMessage& received_msg,
  ...
  switch (msg_type) {
    ...
    case MessageType::NotifyConnectPeerRequestDataSucess:
      ProcessMessage_NotifyConnectPeerRequestSuccess(received_msg.remote_id,
                                                     msg);
      msg_processed = true;
      break;  //@todo 원래 break가 없었는데... 아래 루틴을 호출해주는게
              //맞는거였나???
    ...
    case MessageType::Encrypted_Unreliable: {
      ...
      if (DecryptMessage(msg_type, received_msg, decrypted_msg)) {
        ...
        msg_processed |=
            ProcessMessage_EngineLayer(decrypted_received_msg, subject);
      }
    } break;
    ...
    case MessageType::Compressed: {
      ...
      if (DecompressMessage(received_msg, decompressed_msg)) {
        ...
        msg_processed |=
            ProcessMessage_EngineLayer(decompressed_received_msg, subject);
      }
      break;
    }
    ...
    default:
      ...
      // (int32)subject->GetHostId(), (int32)GetLocalHostId());
      break;
  }

  // 만약 잘못된 메시지가 도착한 것이면 이미 FunNetNet 계층에서 처리한 것으로
  ...
  // 암호화된 메시지는 별도 버퍼에서 복호화된 후 처리되므로
  if (msg_processed == true && l1 != l2 &&
      msg_type != MessageType::Encrypted_Reliable &&
      msg_type != MessageType::Encrypted_Unreliable) {
    msg_processed = true;
    EnqueueError(ResultInfo::From(ResultCode::InvalidPacketFormat,
                                  received_msg.remote_id, __FUNCTION__,
                                  msg.ToAllBytesCopy()));  // copy
  }
...
void LanClientImpl::ProcessMessage_NotifyServerConnectionHint(MessageIn& msg) {
  ...
  if (!lf::Read(msg, intra_logging_on)) {
    EnqueueDisconnectionEvent(ResultCode::ProtocolVersionMismatch,
                              ResultCode::TCPConnectFailure);
    SetState(ConnectionState::Disconnecting);
    return;
  }
  ...
  if (!CryptoRSA::CreateRandomBlock(
          strong_random_block, settings_.strong_encrypted_message_key_length) ||
      !CryptoAES::ExpandFrom(
          self_p2p_session_key_.aes_key,
          (const uint8*)strong_random_block.ConstData(),
          settings_.strong_encrypted_message_key_length / 8) ||

      !CryptoRSA::CreateRandomBlock(
          weak_encrypt_random_block,
          settings_.weak_encrypted_message_key_length) ||
      !CryptoRC4::ExpandFrom(
          self_p2p_session_key_.rc4_key,
          (const uint8*)weak_encrypt_random_block.ConstData(),
          settings_.weak_encrypted_message_key_length / 8)) {
    EnqueueDisconnectionEvent(ResultCode::EncryptFail,
                              ResultCode::TCPConnectFailure);

    if (callbacks_) {
      callbacks_->OnException(HostId_None,
                              Exception("Failed to create session_key_"));
    }
  ...
  }
  {
    ...
    NetConfig::message_max_length = MathBase::Max<int32>(
        NetConfig::message_max_length,
        settings_.message_max_length);  // 아예 전역 값도 수정하도록 한다.
  }

  // NamedInetAddress UdpServerAddr;
  // if (!msg.Read(UdpServerAddr)) {
  //  owner_->EnqueueDisconnectionEvent(ResultCode::ProtocolVersionMismatch,
  //  ResultCode::TCPConnectFailure); SetState(Disconnecting);
  //
  //  //XXAACheck(owner);
  //  return;
  //}
  ...
  if (!CryptoRSA::CreateRandomBlock(
          strong_random_block, settings_.strong_encrypted_message_key_length) ||
      !CryptoAES::ExpandFrom(
          to_server_session_key_.aes_key,
          (const uint8*)strong_random_block.ConstData(),
          settings_.strong_encrypted_message_key_length / 8) ||
      !CryptoRSA::EncryptSessionKeyByPublicKey(
          encrypted_aes_key_blob, strong_random_block, public_key_blob_) ||

      !CryptoRSA::CreateRandomBlock(
          weak_encrypt_random_block,
          settings_.weak_encrypted_message_key_length) ||
      !CryptoRC4::ExpandFrom(
          to_server_session_key_.rc4_key,
          (const uint8*)weak_encrypt_random_block.ConstData(),
          settings_.weak_encrypted_message_key_length / 8) ||
      !CryptoAES::Encrypt(to_server_session_key_.aes_key,
                          weak_encrypt_random_block, encrypted_rc4_key)) {
    EnqueueDisconnectionEvent(ResultCode::EncryptFail,
                              ResultCode::TCPConnectFailure);

    if (callbacks_) {
      callbacks_->OnException(HostId_None,
                              Exception("Failed to create session_key"));
    }
  ...
  // send session key, user data and protocol version via TCP, public-key
  // encrypted
  MessageOut msg_to_send;
...
void LanClientImpl::ProcessMessage_NotifyCSSessionKeySuccess(MessageIn& msg) {
  ...
  lf::Write(msg_to_send, internal_version_);
  //@maxidea: 서버에 접속할 P2P용 Listen 서버 접속 주소를 알려주어야함.  그래야
  //다른 Peer가 이 Peer에 접속할 수 있을테니..
  // TODO 캐싱된 값을 사용해야할까?  가끔 직접 구하면 주소값이 제대로 조회가
  // 안되는 경우가 있는데.. (스트레스 테스트시)
...
void LanClientImpl::ProcessMessage_NotifyProtocolVersionMismatch(
    MessageIn& msg) {
  LockMain_AssertIsNotLockedByCurrentThread();
...
void LanClientImpl::ProcessMessage_ReplyServerTime(MessageIn& msg) {
  ...
  if (server_tcp_recent_ping_ == 0) {
    ...
    server_tcp_recent_ping_ = MathBase::Lerp(
        server_tcp_recent_ping_, lag,
        NetConfig::log_linear_programming_factor);  //! 급격하게 변하지 않도록
                                                    //! 하는 완화 장치?
  }
...
void LanClientImpl::ProcessMessage_NotifyServerDeniedConnection(
    MessageIn& msg) {
  LockMain_AssertIsNotLockedByCurrentThread();
...
void LanClientImpl::ProcessMessage_NotifyServerConnectSuccess(MessageIn& msg) {
  ...
  if (!lf::Reads(msg, msg_local_host_id, msg_server_instance_tag, msg_user_data,
                 msg_local_addr_at_server)) {
    EnqueueConnectFailEvent(
        ResultCode::InvalidPacketFormat,
        ResultInfo::From(ResultCode::ProtocolVersionMismatch, HostId_Server,
                         "Bad format in NotifyServerConnectSuccess"));
    SetState(ConnectionState::Disconnecting);
    return;
  }
  ...
  event.remote_id = HostId_Server;
  event.remote_addr =
      InetAddress(connection_params_.server_ip, connection_params_.server_port);
  EnqueueLocalEvent(event);
  ...
  if (intra_logger_) {
    intra_logger_->WriteLine(LogCategory::LP2P,
                             *String::Format("서버와의 연결 성공.  host_id: %d",
                                             (int32)local_host_id_));
  }

  //@note 기본에는 ConnectEx가 성공하면(물리적으로 tcp 커넥션 연결됨) 연결됨으로
  //변경해주었는데,
  //      LocalHostId를 서버에서 받은 이후가 연결된 상황이라고 간주하는게
  //      좋을듯하다.
  SetState(ConnectionState::Connected);
}

void LanClientImpl::ProcessMessage_NotifyConnectPeerRequestSuccess(
    HostId peer_id, MessageIn& msg) {
  LockMain_AssertIsNotLockedByCurrentThread();
...
void LanClientImpl::ProcessMessage_P2PIndirectServerTimeAndPing(
    ReceivedMessage& received_msg) {
  LockMain_AssertIsNotLockedByCurrentThread();
...
void LanClientImpl::ProcessMessage_P2PIndirectServerTimeAndPong(
    ReceivedMessage& received_msg) {
  auto& msg = received_msg.unsafe_message;
  ...
  if (peer->recent_ping_ > 0) {
    peer->recent_ping_ = MathBase::Lerp(
        peer->recent_ping_, lag, NetConfig::log_linear_programming_factor);
  } else {
...
void LanClientImpl::ProcessMessage_RPC(ReceivedMessage& received_msg,
                                       ITaskSubject* subject,
                                       bool& ref_msg_processed) {
  LockMain_AssertIsNotLockedByCurrentThread();

  auto& payload = received_msg.unsafe_message;
  const int32 saved_read_pos =
      payload.Tell();  // 이 값은 MessageType::RPC 다음의 offset이다.

  CScopedLock2 main_guard(mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);

  // FunNetNet layer의 RPC이면 아래 구문에서 true가 리턴되고 결국 user thread로
  // 넘어가지 않는다. 따라서 아래처럼 하면 된다.

  // 1st
  ref_msg_processed |=
      s2c_stub_.ProcessReceivedMessage(received_msg, subject->host_tag_);

  // 2nd
  if (!ref_msg_processed) {
    payload.Seek(saved_read_pos);
    ref_msg_processed |=
        c2c_stub_.ProcessReceivedMessage(received_msg, subject->host_tag_);
  }
  ...
  if (!ref_msg_processed) {
    ...
    if (subject == this) {
      final_user_work_queue_.Enqueue(FinalUserWorkItem_S(
          received_msg2.unsafe_message, FinalUserWorkItemType::RPC));
    } else {
      ((LanRemotePeer_C*)subject)
          ->final_user_work_queue_.Enqueue(FinalUserWorkItem_S(
              received_msg2.unsafe_message, FinalUserWorkItemType::RPC));
    }
...
void LanClientImpl::ProcessMessage_FreeformMessage(
  ...
  // CReceivedMessage파괴되면서 refcount꼬임.
  {
    ...
    if (subject == this) {
      final_user_work_queue_.Enqueue(
          FinalUserWorkItem_S(received_msg2.unsafe_message,
...
void LanClientImpl::OnIoCompletion(Array<IHostObject*>& send_issued_pool,
                                   ReceivedMessageList& msg_list,
                                   CompletionStatus& completion) {
...
void LanClientImpl::SetState(ConnectionState new_state) {
  if (new_state > state_) {  // 뒤 상태로는 이동할 수 없어야 한다.
    ...
    state_ = new_state;
    disconnecting_mode_start_time_ =
        GetAbsoluteTime();  //@todo Disconnecting이 아닐때도 불리는데 귀찮아서
                            //그런걸까? 상관 없으려나??
    disconnecting_mode_warned_ = false;
  } else {
    //@todo 이전 상태로 돌아간다는건데... 의도한건지 어쩐건지 확인을
    //해봐야할듯함.
...
void LanClientImpl::UserWork_FinalReceiveRPC(FinalUserWorkItem& uwi,
                                             void* host_tag) {
  LockMain_AssertIsNotLockedByCurrentThread();
  ...
  if (lf::Read(msg_content, rpc_id)) {
    ...
    for (int32 stub_index = 0; stub_index < stub_count; ++stub_index) {
      ...
      //@note 여러개의 Stub에서 접근할 것이므로, Stub에서 접근할때마다 원래의
      //위치로 옮겨주어야함.
      msg_content.Seek(saved_read_pos);

      // 이렇게 해줘야 막판에 task running flag를 수정할 수 있으니까 try-catch
      // 구문이 있는거다.
      try {
...
void LanClientImpl::UserWork_FinalReceiveFreeform(FinalUserWorkItem& uwi,
                                                  void* host_tag) {
  LockMain_AssertIsNotLockedByCurrentThread();
    ...
    try {
      ...
      if (!lf::Read(msg_content, payload_length) ||
        ...
        callbacks_->OnReceiveFreeform(
            uwi.unsafe_message.remote_id, rpc_hint,
            msg_content.ToReadableBytesCopy());  // copy
      }
    } catch (Exception& e) {
      if (callbacks_ /* && !owner_->tear_down_*/) {
        callbacks_->OnException(uwi.unsafe_message.remote_id, e);
...
void LanClientImpl::UserWork_FinalUserTask(FinalUserWorkItem& uwi,
                                           void* host_tag) {
  LockMain_AssertIsNotLockedByCurrentThread();
...
LanClient* LanClient::New() { return new LanClientImpl(); }
