...
#define ASSERT_OR_HACKED(X)  { if (!(X)) { EnqueueHackSuspectEvent(lc, #X, HackType::PacketRig); } }
...
IMPLEMENT_RPCSTUB_LanC2S_P2PGroup_MemberJoin_Ack(LanServerImpl::C2SStub) {
  ...
  if (auto group = owner_->GetP2PGroupByHostId_NOLOCK(group_id)) {
    ...
    if (added_member_id == rpc_recvfrom) {
      if (auto p2p_state = owner_->lan_p2p_connection_pair_list_.GetPair(rpc_recvfrom, added_member_id)) {
        p2p_state->p2p_connect_state = LanP2PConnectionState::P2PConnectState::P2PConnected;
      }
    ...
    if (group->add_member_ack_waiters_.RemoveEqualItem(rpc_recvfrom, added_member_id, event_id) == true &&
        group->add_member_ack_waiters_.AckWaitingItemExists(added_member_id, event_id) == false) {
      ...
      for (auto& old_member_pair : group->members_) {
        ...
        try {
          old_member = dynamic_cast<LanClient_S*>(old_member_pair.value.ptr); // 기존 그룹의 각 멤버
        ...
        auto p2p_state = owner_->lan_p2p_connection_pair_list_.GetPair(old_member->host_id_, added_member_id);

        // 자기보다 큰 호스트아이디, 혹은 이미 연결되어있다면 건너뛴다.
        if (old_member->host_id_ >= added_member_id || !p2p_state.IsValid() || p2p_state->dup_count > 1) {
          continue;
        }

        // 이미 연결중이라면 다시 보내지 않는다.
        if (p2p_state->p2p_connect_state != LanP2PConnectionState::P2PConnectState::P2PJoining) {
          continue;
        }
        ...
        InetAddress external_addr = p2p_state->GetExternalAddr(old_member->host_id_);
        owner_->s2c_proxy_.P2PConnectStart(added_member_id, GReliableSend_INTERNAL, old_member->host_id_, external_addr);

        // P2P connect state 바꿈
        p2p_state->p2p_connect_state = LanP2PConnectionState::P2PConnectState::P2PConnecting;
      }
    }
  } else {
...
IMPLEMENT_RPCSTUB_LanC2S_ReliablePing(LanServerImpl::C2SStub) {
  ...
  if (auto lc = owner_->GetAuthedLanClientByHostId_NOLOCK(rpc_recvfrom)) {
    ...
    if (owner_->intra_logger_) {
      const String text = String::Format("Reliable ping receive.  %d host %lf last_tcp_stream_recv_time_", (int32)rpc_recvfrom, lc->last_tcp_stream_recv_time_);
...
bool LanServerImpl::Start(const LanStartServerArgs& InParams, SharedPtr<ResultInfo>& out_error) {
  ...
  if (settings_.strong_encrypted_message_key_length != (int32)StrongEncryptionLevel::Low &&
      settings_.strong_encrypted_message_key_length != (int32)StrongEncryptionLevel::Middle &&
      settings_.strong_encrypted_message_key_length != (int32)StrongEncryptionLevel::High) {
  ...
  if (settings_.weak_encrypted_message_key_length != (int32)WeakEncryptionLevel::None &&
      settings_.weak_encrypted_message_key_length != (int32)WeakEncryptionLevel::Low &&
      settings_.weak_encrypted_message_key_length != (int32)WeakEncryptionLevel::Middle &&
      settings_.weak_encrypted_message_key_length != (int32)WeakEncryptionLevel::High) {
    throw Exception("LanStartServerArgs::weak_encrypted_message_key_length incorrect key length, MaxSize is 2048, MinSize is 8");
  }
  ...
  if (!CryptoRSA::CreatePublicAndPrivateKey(self_xchg_key_, public_key_blob_) ||
      !CryptoRSA::CreateRandomBlock(random_block, settings_.strong_encrypted_message_key_length) ||
      !CryptoAES::ExpandFrom(self_session_key_.aes_key, (const uint8*)random_block.ConstData(), settings_.strong_encrypted_message_key_length / 8) ||
  ...
  //completion_port_.Reset(new CompletionPort(this, true)); // AcceptEx때문에 무조건 true
  ...
  if (!CreateTcpListenSocketAndInit(args.tcp_port, out_error)) {
    // 앞서 시행했던 것들을 모두 롤백한다.
    //@maxidea: 앞서하지 말고, 이후에 하면 다시 돌려야하는 부담도 없을듯 싶은데...
    timer_.Stop();
  ...
  if (args.HostIdGenerationPolicy == HostIdGenerationPolicy::NoRecycle) {
    ...
    host_id_factory_.Reset(new RecycleHostIdFactory(NetConfig::host_id_recycle_allow_time_sec));
  } else {
    throw Exception("Invalid parameter not HostIdGenerationPolicy::Assign in Lanserver");
  }
  ...
  pure_too_old_unmature_client_alarm_ = IntervalAlaram(NetConfig::purge_too_old_add_member_ack_timeout_sec);
  purge_too_old_add_member_ack_item_alarm_ = IntervalAlaram(NetConfig::purge_too_old_add_member_ack_timeout_sec);
  dispose_issued_remote_clients_alarm_ = IntervalAlaram(NetConfig::dispose_issued_remote_clients_timeout_sec);
  remove_too_old_tcp_send_packet_queue_alarm_ = IntervalAlaram(NetConfig::tcp_packet_board_long_interval_sec);
  disconnect_remote_client_on_timeout_alarm_ = IntervalAlaram(NetConfig::cs_ping_interval_sec);
...
void LanServerImpl::Stop() {
  ...
  }
  {
    ...
    if (tcp_listening_socket_) {
      ...
      if (intra_logger_) {
        intra_logger_->WriteLine(LogCategory::System, *String::Format("CloseSocketHandleOnly() called at %s", __FUNCTION__));
      }
    }
    ...
    for (auto& pair : candidate_lan_clients_) {
      auto lc = pair.value;
      IssueDisposeLanClient(lc, ResultCode::DisconnectFromLocal, ResultCode::TCPConnectFailure, ByteArray(), __FUNCTION__, SocketErrorCode::Ok);
    }
  ...
  for (int32 i = 0; i < 10000; ++i) {
    ...
    if (authed_lan_clients_.Count() == 0 && candidate_lan_clients_.Count() == 0 && dispose_issued_lan_clients_map_.Count() == 0) {
      break;
    }
  ...
  // 아래 과정은 unsafe heap을 쓰는 것들을 main_mutex_ lock 상태에서 청소하지 않으면 괜한 assert fail이 발생하므로 필요
  CScopedLock2 main_guard(main_mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
...
bool LanServerImpl::CreateTcpListenSocketAndInit( int32 tcp_port,
...
void LanServerImpl::PurgeTooOldAddMemberAckItem() {
  ...
  for (auto& group_pair : lan_p2p_groups_) {
    ...
    for (int32 member_index = 0; member_index < group->add_member_ack_waiters_.Count(); ++member_index) {
      ...
      if ((absolute_time - ack.event_time) > NetConfig::purge_too_old_add_member_ack_timeout_sec) {
        ...
        if (!group->add_member_ack_waiters_.AckWaitingItemExists(joining_peer_id)) {
...
void LanServerImpl::PurgeTooOldUnmatureClient() {
  ...
  //@todo 대상이 너무 많으면 어똫게 되는가.. 다른 메모리 할당자를 쓰는게 좋을듯 싶음.
  //Array<LanClient_S*,InlineAllocator<256>> timeout_lc_list(list_count, NoInit);
  Array<LanClient_S*> timeout_lc_list(list_count, NoInit);

  for (auto& pair : candidate_lan_clients_) {
    auto lc = pair.value;

    if (!lc->purge_requested_ && (absolute_time - lc->created_time_) > NetConfig::client_connect_server_timeout_sec) {
...
#pragma omp parallel for
    ...
    for (int32 timeout_index = 0; timeout_index < timeout_count; ++timeout_index) {
      ...
      CScopedLock2 lc_tcp_send_queue_guard(lc->to_client_tcp_->GetSendQueueMutex(), false);

      if (timeout_index != 0) {
        if (const bool lock_ok = lc_tcp_send_queue_guard.TryLock()) {
          lc->to_client_tcp_->SendWhenReady(SendFragRefs(msg_to_send), TcpSendOption());
...
SessionKey* LanServerImpl::GetCryptSessionKey(HostId remote_id, String& out_error) {
  CScopedLock2 main_guard(main_mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
  ...
  if (key == nullptr) {
    out_error = String::Format("%d remote LanClient is %s in LanServer.", (int32)remote_id, lc == nullptr ? "NULL" : "not NULL");
  }
...
bool LanServerImpl::Send_BroadcastLayer(const SendFragRefs& payload,
  ...
  //Array<LanClient_S*,InlineAllocator<256>> send_list(send_dest_list_count, NoInit);
  ...
  for (int32 lc_index = 0; lc_index < send_count; ++lc_index) {
    ...
    {
      CScopedLock2 lc_tcp_send_queue_guard(lc->to_client_tcp_->GetSendQueueMutex());
  ...
  if (send_opt.reliability == MessageReliability::Unreliable) {
    ...
    EnqueueError(ResultInfo::From(ResultCode::Unexpected, HostId_Server, "An unreliable message cannot use LanServer and LanClient."));
    return false;
  }
...
bool LanServerImpl::CloseConnection(HostId client_id) {
  ...
  if (auto lc = GetAuthedLanClientByHostId_NOLOCK(client_id)) {
    ...
    if (intra_logger_) {
      intra_logger_->WriteLine(LogCategory::System, "CloseConnection() called.");
    }
...
void LanServerImpl::CloseAllConnections() {
  ...
  if (intra_logger_) {
    intra_logger_->WriteLine(LogCategory::System, "CloseAllConnections() called.");
  }
...
LanClient_S* LanServerImpl::GetAuthedLanClientByHostId_NOLOCK(HostId client_id) {
  AssertIsLockedByCurrentThread();

  LanClient_S* lc = authed_lan_clients_.FindRef(client_id);
  return (lc && lc->dispose_waiter_.IsValid() == false) ? lc : nullptr; // disposing 중인것은 제외함.
}
...
bool LanServerImpl::LeaveP2PGroup(HostId member_id, HostId group_id) {
  ...
  if (member_id != HostId_Server) {
    for (const auto& member_pair : group->members_) {
      if (member_pair.value.ptr->GetHostId() != HostId_Server && member_lc->GetHostId() != HostId_Server) {
        lan_p2p_connection_pair_list_.ReleasePair(this, (LanClient_S*)member_lc, (LanClient_S*)member_pair.value.ptr);
      }
    }
  }
...
HostId LanServerImpl::CreateP2PGroup( const HostId* ClientHostIdList0,
  ...
  if (count > 0) {
    ...
    for (int32 i = 0; i < count; ++i) {
      ...
      if (!lc->session_key_.KeyExists()) {
        EnqueueError(ResultInfo::From(ResultCode::Unexpected, lc->host_id_, "CreateP2P 실패: session key 관련 오류."));
      }
    }
  }
  ...
  for (int32 i = 0; i < count; ++i) {
    JoinP2PGroup_INTERNAL(client_host_id_list[i],
                          new_group->group_id_,
                          custom_field,
                          joined_p2p_group_key_gen_);
  }
...
bool LanServerImpl::JoinP2PGroup_INTERNAL(HostId member_id,
                                          HostId group_id,
                                          const ByteArray& custom_field,
  ...
  if (settings_.server_as_p2p_group_member_allowed && member_id == HostId_Server) {
    ...
    member_lc = GetAuthedLanClientByHostId_NOLOCK(member_id);
    if (member_lc == nullptr || IsDisposeLanClient_NOLOCK((LanClient_S*)member_lc)) {
      return false;
    }
  }
  ...
  for (auto& old_member_pair : group->members_) {
    ...
    if (old_member->GetHostId() != HostId_Server && member_lc->GetHostId() != HostId_Server) {
      ...
      p2p_state = lan_p2p_connection_pair_list_.GetPair(old_member_as_lc->host_id_, new_member_as_lc->host_id_);
      if (!p2p_state.IsValid()) {
        p2p_state.Reset(new LanP2PConnectionState(this));
        p2p_state->p2p_connect_state = LanP2PConnectionState::P2PConnectState::P2PJoining;
        p2p_state->dup_count = 1;
        // 각 피어간 외부주소를 입력해준다.
        p2p_state->SetExternalAddr(old_member_as_lc->host_id_, old_member_as_lc->external_addr);
        p2p_state->SetExternalAddr(new_member_as_lc->host_id_, new_member_as_lc->external_addr);

        if (settings_.p2p_encrypted_messaging_enabled) {
          if (!CryptoRSA::CreateRandomBlock(p2p_state->p2p_aes_session_key, settings_.strong_encrypted_message_key_length) ||
              !CryptoRSA::CreateRandomBlock(p2p_state->p2p_rc4_session_key, settings_.weak_encrypted_message_key_length)) {
            if (intra_logger_) {
              const String text = String::Format("%d와 %d간의 P2P session key make failed", (int32)old_member->GetHostId(), (int32)member_lc->GetHostId());
        ...
        lan_p2p_connection_pair_list_.AddPair(old_member_as_lc, new_member_as_lc, p2p_state);
        //FUN_TRACE("%d-%d pair Add\n", old_member_as_lc->HostID, new_member_as_lc->HostID);

        old_member_as_lc->lan_p2p_connection_pairs_.Add(p2p_state);
        new_member_as_lc->lan_p2p_connection_pairs_.Add(p2p_state);

        if (intra_logger_) {
          const String text = String::Format("Prepare P2P pair for %d <-> %d", (int32)old_member->GetHostId(), (int32)member_lc->GetHostId());
          intra_logger_->WriteLine(LogCategory::LP2P, *text);
        }
      } else {
        p2p_state->dup_count++;
    ...
    if (intra_logger_ && p2p_state) {
      const String text = String::Format("서버에서 발급한 holepunch tag: %s", *p2p_state->holepunch_tag_.ToString());
      intra_logger_->WriteLine(LogCategory::LP2P, *text);
    }
  }
...
bool LanServerImpl::JoinP2PGroup( HostId member_id,
                                  HostId group_id,
                                  const ByteArray& custom_field) {
  CScopedLock2 main_guard(main_mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
...
void LanServerImpl::EnqueueClientLeaveEvent(LanClient_S* lc,
  ...
  //  viz_agent_->C2SProxy.NotifySrv_Clients_Remove(HostId_Server, GReliableSend_INTERNAL, lc->host_id_);
  //}
}
...
LanServerImpl::LanServerImpl()
  : main_mutex_(),
    settings_(),
    pure_too_old_unmature_client_alarm_(NetConfig::purge_too_old_add_member_ack_timeout_sec),
    purge_too_old_add_member_ack_item_alarm_(NetConfig::purge_too_old_add_member_ack_timeout_sec),
    dispose_issued_remote_clients_alarm_(NetConfig::dispose_issued_remote_clients_timeout_sec),
    remove_too_old_tcp_send_packet_queue_alarm_(NetConfig::tcp_packet_board_long_interval_sec),
    disconnect_remote_client_on_timeout_alarm_(NetConfig::cs_ping_interval_sec),
...
//TODO 이어붙인다고???
void LanServerImpl::ConvertAndAppendP2PGroupToPeerList( HostId sendto,
                                                        HostIdArray& additive_output) {
  AssertIsLockedByCurrentThread();

  // ConvertP2PGroupToPeerList는 한번만 호출되는 것이 아니다. 추가하는 형태로 동작하므로, 여기서 리셋하면 안됨.
...
bool LanServerImpl::GetP2PConnectionStats(HostId remote_id, P2PConnectionStats& out_stats) {
  CScopedLock2 main_guard(main_mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
  ...
  if (auto group = GetP2PGroupByHostId_NOLOCK(remote_id)) {
    ...
    visited_connection_states.Reserve((int32)MathBase::Max(pow((double)group->members_.Count(), 2) * 0.5, 1.0));
    for (auto member_pair : group->members_) {
      // 서버와의 연결은 통계에서 제외. (서버와 P2P가 허용된 경우에는 대상이 서버가 될 수 있음.)
      ...
      for (auto p2p_connection : member_as_lc->lan_p2p_connection_pairs_) {
        ...
        if (!group->members_.Contains(p2p_connection->first_client->host_id_) ||
            !group->members_.Contains(p2p_connection->second_client->host_id_)) {
          continue;
        }
...
// 이 함수는 마구 호출해도 괜찮다.
void LanServerImpl::IssueDisposeLanClient(LanClient_S* lc,
                                          ResultCode result_code,
                                          ResultCode detail_code,
                                          const ByteArray& comment,
                                          const char* where,
  ...
  if (intra_logger_) {
    intra_logger_->WriteLine(LogCategory::System, *String::Format("IssueDisposeLanClient() called at %s", where));
  }
  ...
  if (!dispose_issued_lan_clients_map_.Contains(lc)) {
    ...
    if (lc->dispose_waiter_.IsValid()) {
      EnqueueClientLeaveEvent(lc,
                              lc->dispose_waiter_->reason,
                              lc->dispose_waiter_->detail,
                              lc->dispose_waiter_->comment,
                              lc->dispose_waiter_->socket_error);
    } else {
      EnqueueClientLeaveEvent(lc,
...
//#endif
  ...
  for (auto& group_pair : lc->joined_lan_p2p_groups_) {
    ...
    AddMemberAckWaiters_RemoveRelated_MayTriggerJoinP2PMemberCompleteEvent(group.Get(), lc->host_id_, ResultCode::DisconnectFromRemote);

    // notify member leave to related group members
    // Server는 P2PGroup_MemberLeave를 받을 필요가 없다.
    for (const auto& member_pair : group->members_) {
      if (member_pair.key != HostId_Server) {
        s2c_proxy_.P2PGroup_MemberLeave(member_pair.key, GReliableSend_INTERNAL, lc->host_id_, group->group_id_);
      }
    }
...
void LanServerImpl::DisposeIssuedLanClients() {
  ...
  for (auto it = dispose_issued_lan_clients_map_.CreateIterator(); it; ++it) {
    ...
    const bool tcp_close_safe = !lc->to_client_tcp_->recv_issued_ && !lc->to_client_tcp_->send_issued_;

    // 일거리가 남아있다면 지우지 아니 한다.
    const bool works_remain = (lc->task_subject_node_.GetListOwner() || lc->IsTaskRunning() || lc->GetListOwner());
...
NamedInetAddress LanServerImpl::GetRemoteIdentifiableLocalAddr() {
  if (tcp_listening_socket_.IsValid()) {
    NamedInetAddress result = NamedInetAddress(tcp_listening_socket_->GetSockName());
...
void LanServerImpl::Tcp_LongTick() {
  ...
  int32 count = 0;
  const int32 total_count = authed_lan_clients_.Count() + candidate_lan_clients_.Count();
  if (total_count <= 0) {
    return;
  }

  //TODO inline allocator를 사용하는 array로 대체하는게 좋을듯함.
  //LanClient_S** lan_clients = (LanClient_S**)ALLOCA(sizeof(LanClient_S*) * total_count);
  //Array<LanClient_S*,InlineAllocator<256>> lan_clients;
...
void LanServerImpl::EnableIntraLog(const char* log_filename) {
  ...
  //@maxidea: NetServer에서는 지원하고 있지만, LanServer에서는 구지 필요 없어서 뺀듯..?
  // 모든 클라이언트들에게 로그를 보내라는 명령을 한다.
  //for (auto client_it = authed_lan_clients_.begin(); client_it != authed_lan_clients_.end(); ++client_it) {
...
void LanServerImpl::Heartbeat_PerClient() {
  ...
  for (auto pair : authed_lan_clients_) {
    ...
    if ((absolute_time - lc->last_tcp_stream_recv_time_) > settings_.default_timeout_sec) {
      if (intra_logger_) {
        const String text = String::Format("Timeout %d host %lf delta %lf default_timeout_sec",
                                          (int32)lc->host_id_,
                                          absolute_time - lc->last_tcp_stream_recv_time_,
                                          settings_.default_timeout_sec);
        intra_logger_->WriteLine(LogCategory::System, *text);
      }

      IssueDisposeLanClient(lc,
                            ResultCode::DisconnectFromRemote,
                            ResultCode::ConnectServerTimeout,
                            ByteArray(),
                            __FUNCTION__,
...
void LanServerImpl::SetMessageMaxLength(int32 max_length) {
  ...
  // 아예 전역 값도 수정하도록 한다.
  {
    ...
    NetConfig::message_max_length = MathBase::Max(NetConfig::message_max_length, max_length);
  }
...
bool LanServerImpl::SetDirectP2PStartCondition(DirectP2PStartCondition condition) {
  CScopedLock2 main_guard(main_mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
...
InetAddress LanServerImpl::GetTcpListenerLocalAddr() {
  return tcp_listening_socket_.IsValid() ? tcp_listening_socket_->GetSockName() : InetAddress::None;
}

void LanServerImpl::ConvertGroupToIndividualsAndUnion(int32 sendto_count,
                                                      const HostId* sendto_list,
  ...
  for (int32 sendto_index = 0; sendto_index < sendto_count; ++sendto_index) {
    if (sendto_list[sendto_index] != HostId_None) {
      ConvertAndAppendP2PGroupToPeerList(sendto_list[sendto_index], out_send_dest_list);
    }
  }
...
bool LanServerImpl::SendWithSplitter_DirectlyToClient_Copy( HostId client_id,
  ...
  if (auto lc = GetAuthedLanClientByHostId_NOLOCK(client_id)) {
    ...
    if (reliability != MessageReliability::Reliable) {
      EnqueueError(ResultInfo::From(ResultCode::Unexpected, HostId_Server, "An unreliable message connot use LanServer and LanClient."));
      return false;
    }
...
void LanServerImpl::Convert_NOLOCK(LanSendDestInfoList_S& to, HostIdArray& from) {
  AssertIsLockedByCurrentThread();
...
bool LanServerImpl::NextEncryptCount(HostId remote_id, CryptoCountType& out_count) {
  CScopedLock2 main_guard(main_mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
...
bool LanServerImpl::GetExpectedDecryptCount(HostId remote_id, CryptoCountType& out_count) {
  CScopedLock2 main_guard(main_mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
...
void LanServerImpl::OnSocketWarning(InternalSocket* socket, const String& text) {
  //@warning Deadlock
  //CScopedLock2 main_guard(main_mutex_);
...
uint32 LanServerImpl::GetInternalVersion() {
...
void LanServerImpl::EnqueueWarning(const InetAddress& sender, const char* text) {
  CScopedLock2 main_guard(main_mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);

  //@todo Tcp어드레스가 유니크할까??
  auto lc = GetCandidateLanClientByTcpAddr(sender);
  EnqueueWarning(ResultInfo::From(ResultCode::InvalidPacketFormat, lc ? lc->host_id_ : HostId_None, text));
}

void LanServerImpl::EnqueueClientJoinApproveDetermine(const InetAddress& client_tcp_addr,
...
void LanServerImpl::ProcessOnClientJoinRejected(LanClient_S* lc, const ByteArray& response) {
  AssertIsLockedByCurrentThread();
  lc->to_client_tcp_->AssertIsSendQueueNotLockedByCurrentThread();

  MessageOut msg;
  lf::Write(msg, MessageType::NotifyServerDeniedConnection);
  lf::Write(msg, response); //TODO 참조로도 넘길 수 있을터인데,... 뭐 호출 빈도는 크지 않을테니 상관 없으려나??
  {
...
void LanServerImpl::ProcessOnClientJoinApproved(LanClient_S* lc, const ByteArray& response) {
  //CScopedLock2 main_guard(main_mutex_);
  AssertIsLockedByCurrentThread();
...
LanClient_S* LanServerImpl::GetCandidateLanClientByTcpAddr(const InetAddress& client_addr) {
  AssertIsLockedByCurrentThread();
  ...
  if (callbacks_/* && !tear_down_*/) {
    try {
      ...
      case LocalEventType::ClientLeaveAfterDispose:
        callbacks_->OnClientLeft(event.NetClientInfo.Get(), event.result_info.Get(), event.comment);
        break;
      ...
      case LocalEventType::DirectP2PEnabled:
        callbacks_->OnP2PConnectionEstablished(event.member_id, event.remote_id);
        break;

      case LocalEventType::P2PDisconnected:
        callbacks_->OnP2PDisconnected(event.member_id, event.remote_id, event.result_info->result_code);
        break;
      ...
      case LocalEventType::ClientJoinCandidate: {
          ByteArray response;
          const bool is_approved = callbacks_->OnConnectionRequest(event.remote_addr, event.connection_request, response);
...
HostId LanServerImpl::GetLocalHostId() {
...
void LanServerImpl::RequestAutoPrune(LanClient_S* lc) {
  ...
  if (lc->request_auto_prune_start_time_ == 0) { // Once!
    lc->request_auto_prune_start_time_ = GetAbsoluteTime(); //@note 설정 여부에 시간을 기록해두는게 좀더 좋을듯..
...
void LanServerImpl::HardDisconnect_AutoPruneGoesTooLongClient(LanClient_S* lc) {
  ...
  if (lc->request_auto_prune_start_time_ != 0.0 && (GetAbsoluteTime() - lc->request_auto_prune_start_time_) > 5) {
    lc->request_auto_prune_start_time_ = 0.0;

    // Caller에서는 FunNet.LanServerImpl.AuthedLanClients를 iter중이 아니므로 안전
    IssueDisposeLanClient(
        lc,
        ResultCode::DisconnectFromLocal,
        ResultCode::ConnectServerTimeout,
        ByteArray(),
...
void LanServerImpl::AddMemberAckWaiters_RemoveRelated_MayTriggerJoinP2PMemberCompleteEvent(LanP2PGroup_S* group, HostId member_id, ResultCode reason)
{
  Array<int32> DelList;
  ...
  for (int32 i = group->add_member_ack_waiters_.Count() - 1; i >= 0; --i) {
    ...
    if (ack.joining_member_host_id == member_id || ack.old_member_host_id == member_id) {
      DelList.Add(i);
  ...
  // MemberId에 대한 OnP2PGroupJoinMemberAckComplete 대기에 대한 콜백에 대한 정리.
  ...
  for (auto JoiningMemberId : JoiningMemberDelList) {
    if (!group->add_member_ack_waiters_.AckWaitingItemExists(JoiningMemberId)) {
      EnqueueP2PAddMemberAckCompleteEvent(group->group_id_, JoiningMemberId, reason);
    }
  }
}
  ...
  if (auto group = GetP2PGroupByHostId_NOLOCK(group_id)) {
    for (auto& member_pair1 : group->members_) {
      ...
      for (auto& member_pair2 : group->members_) {
        ...
        auto state = lan_p2p_connection_pair_list_.GetPair(member_pair1.value.ptr->GetHostId(), member_pair2.value.ptr->GetHostId());
        if (state->p2p_connect_state != LanP2PConnectionState::P2PConnectState::P2PConnected) {
          return false;
        }
      }
    }
    ...
    for (auto& member_pair : group->members_) {
      s2c_proxy_.GroupP2PConnectionComplete(member_pair.value.ptr->GetHostId(), GReliableSend_INTERNAL, group_id);
    }
  }
...
void LanServerImpl::EnqueueP2PConnectionEstablishedEvent(HostId member_id, HostId remote_id) {
  // Enqueue event
...
ITaskSubject* LanServerImpl::GetTaskSubjectByHostId_NOLOCK(HostId subject_host_id) {
  AssertIsLockedByCurrentThread();
  CheckCriticalSectionDeadLock(__FUNCTION__);
...
void LanServerImpl::CheckCriticalSectionDeadLock_INTERNAL(const char* where) {
  ...
  if (tcp_issue_queue_mutex_.IsLockedByCurrentThread() == true) {
    LOG(LogNetEngine, Warning, "LanServer TcpSendIssueQueueCS DeadLock!! - %s", where);
  }
}
...
void LanServerImpl::OnSocketIoCompletion( Array<IHostObject*>& send_issued_pool,
  ...
  try {
    ...
    if (lc) {
      ...
      } else if (completion.type == CompletionType::ReferCustomValue) { // Custom(User)
...
void LanServerImpl::IoCompletion_TcpCustomValueCase(CompletionStatus& completion, LanClient_S* lc) {
  AssertIsNotLockedByCurrentThread();
...
void LanServerImpl::IoCompletion_NewClientCase(LanClient_S* lc) {
  ...
  if (socket_error != SocketErrorCode::Ok) {
    ...
    lc->IssueDispose(ResultCode::DisconnectFromRemote, ResultCode::TCPConnectFailure, ByteArray(), __FUNCTION__, socket_error);
...
void LanServerImpl::IoCompletion_TcpSendCompletionCase(CompletionStatus& completion, LanClient_S* lc) {
  AssertIsNotLockedByCurrentThread();
  lc->AssertIsNotLockedByCurrentThread();
  ...
  if (completion.completed_length < 0) { // Don't care completion.socket_error
    ...
    IssueDisposeLanClient(lc, ResultCode::DisconnectFromRemote, ResultCode::TCPConnectFailure, ByteArray(), *comment, completion.socket_error);
  } else {
    // 송신 큐에서 완료된 만큼의 데이터를 제거한다. 그리고 다음 송신을 건다.
    {
      CScopedLock2 lc_tcp_send_queue_guard(lc->to_client_tcp_->GetSendQueueMutex());
      lc->to_client_tcp_->send_queue_.DequeueNoCopy(completion.completed_length);
    }

    const SocketErrorCode socket_error = lc->to_client_tcp_->ConditionalIssueSend(GetAbsoluteTime());
    if (socket_error != SocketErrorCode::Ok) {
      lc_tcp_guard.Unlock();
...
void LanServerImpl::IoCompletion_TcpRecvCompletionCase( CompletionStatus& completion,
                                                        LanClient_S* lc,
                                                        ReceivedMessageList& msg_list) {
  AssertIsNotLockedByCurrentThread();
  ScopedUseCounter counter(*lc);
  ...
  if (completion.completed_length <= 0) {
    ...
    IssueDisposeLanClient(lc, ResultCode::DisconnectFromRemote, ResultCode::TCPConnectFailure, ByteArray(), __FUNCTION__, completion.socket_error);
    lc->WarnTooShortDisposal(__FUNCTION__);

    if (intra_logger_) {
      const String text = String::Format("서버: 클라이언트 연결 해제.  host_id: %d, addr: %s, socket_error: %d",
          (int32)lc->host_id_, *lc->to_client_tcp_->remote_addr_.ToString(), (int32)completion.socket_error);
      intra_logger_->WriteLine(LogCategory::System, *text);
    }
  } else {
    // 수신 큐에서 받은 데이터를 꺼낸 후 ...
    lc->to_client_tcp_->recv_stream_.EnqueueCopy(lc->to_client_tcp_->socket_->GetRecvBufferPtr(), completion.completed_length);

    //msg_list.EmptyAndKeepCapacity();
    msg_list.Reset(); // keep capacity

    const ResultCode extract_result = lc->ExtractMessagesFromTcpStream(msg_list);
    if (extract_result != ResultCode::Ok) {
      ...
      IssueDisposeLanClient(lc, extract_result, ResultCode::TCPConnectFailure, ByteArray(), __FUNCTION__, SocketErrorCode::Ok);
      return;
    }
...
void LanServerImpl::IoCompletion_ProcessMessageOrMoveToFinalRecvQueue(LanClient_S* lc,
...
bool LanServerImpl::IoCompletion_ProcessMessage_EngineLayer(ReceivedMessage& received_msg, LanClient_S* lc) {
  AssertIsNotLockedByCurrentThread();
  ...
  case MessageType::FreeformMessage:
    IoCompletion_ProcessMessage_FreeformMessage(received_msg, msg_processed, lc);
    break;

  case MessageType::Encrypted_Reliable:
  case MessageType::Encrypted_Unreliable: {
      ReceivedMessage decrypted_msg;
      if (DecryptMessage(msg_type, received_msg, decrypted_msg.unsafe_message)) {
        ...
        msg_processed |= IoCompletion_ProcessMessage_EngineLayer(decrypted_msg, lc);
      }
    }
    break;
  ...
  case MessageType::Compressed: {
    ...
    if (DecompressMessage(received_msg, decompressed_msg.unsafe_message)) {
      ...
      decompressed_msg.remote_addr_udp_only = received_msg.remote_addr_udp_only;
      decompressed_msg.remote_id = received_msg.remote_id;

      msg_processed |= IoCompletion_ProcessMessage_EngineLayer(decompressed_msg, lc);
    }
    break;
  }
  ...
  if (msg_processed &&
      received_msg.unsafe_message.GetLength() != received_msg.unsafe_message.Tell() &&
      msg_type != MessageType::Encrypted_Reliable && msg_type != MessageType::Encrypted_Unreliable) { // 암호화된 메시지는 별도 버퍼에서 복호화된 후 처리되므로
    //@todo 이게 맞는건지??
    msg_processed = true;

    // 에러시에 마지막 메세지를 저장한다.
    EnqueueError(ResultInfo::From(ResultCode::InvalidPacketFormat, received_msg.remote_id, __FUNCTION__, msg.ToReadableBytesCopy())); //copy
  }
...
void LanServerImpl::IoCompletion_ProcessMessage_NotifyCSEncryptedSessionKey(MessageIn& msg, LanClient_S* lc) {
  lc->AssertIsZeroUseCount();
  AssertIsNotLockedByCurrentThread();
  ...
  // self_xchg_key_ 는 서버 시작시에 한번만 만드므로 락을 안걸어도 괜찮을것 같습니다.
  if (SharedPtr<ResultInfo> error = CryptoRSA::DecryptSessionKeyByPrivateKey(out_random_block, encrypted_aes_key_blob, self_xchg_key_)) {
  ...
  if (!CryptoAES::ExpandFrom(lc->session_key_.aes_key, (const uint8*)out_random_block.ConstData(), settings_.strong_encrypted_message_key_length / 8) ||
      !CryptoAES::Decrypt(lc->session_key_.aes_key, encrypted_rc4_key_blob, out_rc4_random_block) ||
      !CryptoRC4::ExpandFrom(lc->session_key_.rc4_key, (const uint8*)out_rc4_random_block.ConstData(), settings_.weak_encrypted_message_key_length / 8)) {
    callbacks_->OnException(HostId_None, Exception("Create session-key failed"));
...
void LanServerImpl::IoCompletion_ProcessMessage_NotifyServerConnectionRequestData(MessageIn& msg, LanClient_S* lc) {
  AssertIsNotLockedByCurrentThread();
  lc->AssertIsZeroUseCount();
  lc->AssertIsNotLockedByCurrentThread();
  ...
  if (!lf::Reads(msg, msg_user_data, msg_protocol_version, msg_internal_version, msg_external_addr)) {
    NotifyProtocolVersionMismatch(lc);
    return;
  }

  // Protocol version match and assign received session key
  if (msg_protocol_version != protocol_version_ || msg_internal_version != internal_version_) {
    NotifyProtocolVersionMismatch(lc);
    return;
  }
  ...
  //msg_external_addr.BinaryAddress = lc->to_client_tcp_->remote_addr_.BinaryAddress; //포트 번호는???
  msg_external_addr = InetAddress(lc->to_client_tcp_->remote_addr_.GetHost(), msg_external_addr.GetPort()); //TODO SetHost 함수를 하나 만들어야할듯...
  ...
  // 이 스레드는 이미 main_mutex_ locked이므로 여기서 하지 말고 스레드 풀에서 콜백받은 후 후반전으로 ㄱㄱ

  if (callbacks_) {
    EnqueueClientJoinApproveDetermine(lc->to_client_tcp_->remote_addr_, msg_user_data);
  } else {
    // 무조건 성공 처리
...
void LanServerImpl::IoCompletion_ProcessMessage_RequestServerTimeAndKeepAlive(MessageIn& msg, LanClient_S* lc) {
  ...
  if (intra_logger_) {
    const String text = String::Format("RequestServerTimeAndKeepAlive %d host %lf last_tcp_stream_recv_time_", (int32)lc->host_id_, lc->last_tcp_stream_recv_time_);
    intra_logger_->WriteLine(LogCategory::System, *text);
  }
...
void LanServerImpl::IoCompletion_ProcessMessage_NotifyCSConnectionPeerSuccess(MessageIn& msg, LanClient_S* lc) {
  AssertIsNotLockedByCurrentThread();
  lc->AssertIsZeroUseCount();
  ...
  if (auto p2p_state = lan_p2p_connection_pair_list_.GetPair(peer_id, lc->host_id_)) {
    // 연결이 성사되었다.
    p2p_state->p2p_connect_state = LanP2PConnectionState::P2PConnectState::P2PConnected;
...
void LanServerImpl::IoCompletion_ProcessMessage_NotifyCSP2PDisconnected(MessageIn& msg, LanClient_S* lc) {
  AssertIsNotLockedByCurrentThread();
  ...
  // P2P 상태변경
  if (auto p2p_state = lan_p2p_connection_pair_list_.GetPair(lc->host_id_, peer_id)) {
...
void LanServerImpl::IoCompletion_ProcessMessage_RPC(ReceivedMessage& received_msg, bool msg_processed, LanClient_S* lc) {
  // FunNet layer의 RPC이면 아래 구문에서 true가 리턴되고 결국 user thread로 넘어가지 않는다.
  // 따라서 아래처럼 하면 된다.
  {
    ...
    const int32 rpc_header_pos = payload.Tell(); // 이 값은 MessageType::RPC 다음의 offset이다.

    // 내부 RPC Stub에 먼저 처리 기회를 주고, 처리되지 않았다면 유저 RPC Stub로 전달하도록 함.
    msg_processed |= c2s_stub_.ProcessReceivedMessage(received_msg, lc->host_tag_);
...
void LanServerImpl::IoCompletion_ProcessMessage_FreeformMessage(
  ...
  // 따라서 아래처럼 하면 된다.
   ...
   //TODO 딱히 블럭을 유지해야할 이유가 없어보이는데??
   {
    ...
    UserTaskQueue_Add(lc, received_msg2, FinalUserWorkItemType::FreeformMessage);
  }
}
...
void LanServerImpl::UserTaskQueue_Add(LanClient_S* lc,
  ...
  lc->final_user_work_queue_.Enqueue(FinalUserWorkItem_S(received_msg.unsafe_message, type));
...
void LanServerImpl::CatchThreadExceptionAndPurgeClient( LanClient_S* lc,
  ...
  if (lc) {
    if (callbacks_) {
      const String text = String::Format("LanServerImpl.CatchThreadExceptionAndPurgeClient: where: %s, reason: %s, client_id: %d", where, reason, (int32)lc->host_id_);
      EnqueueError(ResultInfo::From(ResultCode::Unexpected, lc->host_id_, text));
    }

    IssueDisposeLanClient(lc, ResultCode::Unexpected, ResultCode::TCPConnectFailure, ByteArray(), __FUNCTION__, SocketErrorCode::Ok);
  } else {
    if (callbacks_) {
...
void LanServerImpl::PostHeartbeatIssue() {
  net_thread_pool_->PostCompletionStatus(this, (uintptr_t)IocpCustomValue::Heartbeat);
}
...
void LanServerImpl::Heartbeat_One(double elapsed_sec) {
  ...
  if (remove_too_old_tcp_send_packet_queue_alarm_.TakeElapsedTime(elapsed_sec)) {
    Tcp_LongTick();
  }
...
void LanServerImpl::EveryRemote_IssueSendOnNeed(Array<IHostObject*>& pool) {
  ...
  //수집이 덜될수도 있으려나??
  //아래 코드를 보니 실질적으로 불필요한 코드인듯... 나중에 좀더 살펴보고 삭제하도록 하자.
...
#endif
#endif
    for (int32 parallel_index = 0; parallel_index < total_parallel_count; ++parallel_index) {
...
void LanServerImpl::PostEveryRemote_IssueSend() {
  net_thread_pool_->PostCompletionStatus(this, (uintptr_t)IocpCustomValue::SendEnqueued);
}
...
void LanServerImpl::UserWork_FinalReceiveRPC(FinalUserWorkItem& uwi, void* host_tag) {
  AssertIsNotLockedByCurrentThread();
  ...
  if (saved_read_pos != 0) {
    EnqueueHackSuspectEvent(nullptr, __FUNCTION__, HackType::PacketRig); // bug or hacked
  }
...
void LanServerImpl::UserWork_FinalReceiveFreeformMessage(FinalUserWorkItem& uwi, void* host_tag) {
  AssertIsNotLockedByCurrentThread();
  ...
  if (callbacks_/* && !owner_->tear_down_*/) {
    ...
    try {
      ...
      if (!lf::Read(content, payload_length) || payload_length != content.GetReadableLength()) {
        ...
        callbacks_->OnReceiveFreeform(uwi.unsafe_message.remote_id, rpc_hint, content.ToReadableBytesCopy()); //copy
      }
    } catch (Exception& e) {
      if (callbacks_/* && !owner_->tear_down_*/) {
        callbacks_->OnException(uwi.unsafe_message.remote_id, e);
...
void LanServerImpl::UserWork_FinalUserTask(FinalUserWorkItem& uwi, void* host_tag) {
  AssertIsNotLockedByCurrentThread();
...
#ifdef TRACE_NEW_DISPOSE_ROUTINE
      //FUN_TRACE("%d-%d pair Remove...\n", (int)state->first_client->host_id_, (int)state->second_client->host_id_);
#endif
...
void LanP2PPairList::AddPair(LanClient_S* lc_a, LanClient_S* lc_b, LanP2PConnectionStatePtr state) {
...
void LanP2PPairList::ReleasePair(LanServerImpl* owner, LanClient_S* lc_a, LanClient_S* lc_b) {
  ...
  //    List.erase(pair_it); // 이게 여기 있어야 한다. 위에 있으면 안되고.
    //
  //    if (owner_->intra_logger_) {
  //      const String text = String::Format("[서버] Client %d-Client %d간의 P2P 연결 쌍 제거", lc_a->host_id_, lc_b->host_id_);
  ...
  if (auto pair = list.FindRef(pair_key)) {
    ...
    if (pair->dup_count == 0) {
      ...
      if (owner_->intra_logger_) {
        const String text = String::Format("Remove pair of connections between client %d and %d.", (int32)lc_a->host_id_, (int32)lc_b->host_id_);
        owner_->intra_logger_->WriteLine(LogCategory::LP2P, *text);
      }
    }
  }
...
LanServer* LanServer::New() {
  return new LanServerImpl();
}

void LanP2PConnectionState::SetExternalAddr(HostId peer_id, const InetAddress& external_addr) {
  for (int32 i = 0; i < 2; ++i) {
