...
void NetSettings::Reset() {
  ...
  over_send_suspecting_threshold_in_byte =
      NetConfig::default_over_send_suspecting_threshold_in_byte;
  //@remarks Nagle 알고리즘을 활성화 했을경우, 문제가 발생할 수 있다함.
  //         직접처리하는게 더 효율적이다. 이건가..
  //         FunNet affords better anti-silly window syndrome technology than
  //         TCP Nagle Algorithm.
  ...
  // 기본적으로 꺼놈... 어짜피 서버 기동시에 파라메터에 설정하므로, 거기에서
  // 설정 하도록 하자.
