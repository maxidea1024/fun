...
TcpTransport_S::TcpTransport_S(ITcpTransportOwner_S* owner,
  ...
  if (socket) {
    ...
    int32 send_buffer_length int32 recv_buffer_length;
...
void TcpTransport_S::SendWhenReady(const SendFragRefs& data_to_send,
  ...
  // 바로 보내지 않고 send-qeueue에 집어 넣어주고, 스케줄러가 필요에 따라서
  // 보내는 형태로 처리함. 복사를 제거할 수 있는 방법이 있다면 좋을듯.. 그렇게
  // 하기 위해서는, CSendFragRefs가 raw포인터가 아닌 참조포인터를 가져야할듯..
  ...
  if (!send_issued_) {
    ...
    //보내야할 대상에 링크시킴. Send 스케줄러는 정해진 간격(0.003초)마다
    //모아서(coalesce) 전송할것임.
    owner_->EnqueueIssueSendReadyRemotes();
  }
}

// SendBreak(Throttling)는 사용하지 않지만, 일정시간동안 모아서 전송하는 역활은
// 하므로, 아예 필요 없지는 않을듯 싶음.

// 이놈은 가끔씩 불리면서, 큐에 들어있는 데이터를 네트워크 너머로 보내는 역활을
// 수행.
SocketErrorCode TcpTransport_S::ConditionalIssueSend(double absolute_time) {
  ...
  // 만약 닫힌 소켓에 issue하면 completion이 어쨌거나 또 발생한다. 메모리도
  // 긁으면서. 송신할게 있는지 체크하기 전에 이걸 먼저 체크해야 한다. 반드시!
...
TcpTransport_S::~TcpTransport_S() {
  ...
  //      가만, 그런데 의미 없다. 조 위의 fun_check(!recv_issued_ &&
  //      !send_issued_) 는 머냐??
  // if
  // (!owner_->IsValidEnd())//owner_->owner_->tcp_listening_socket_.IsValid()) {
