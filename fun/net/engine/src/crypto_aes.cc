...
bool CryptoAES::ExpandFrom( CryptoAESKey& out_key,
                            const uint8* key,
                            int32 key_length,
                            int32 block_size) {
  ...
  switch (key_length) {
    case 16:
      out_key.Rounds = (out_key.block_size == 16) ? 10 : (out_key.block_size == 24 ? 12 : 14);
      break;
...
int32 CryptoAES::GetEncryptSize(const CryptoAESKey& key, int32 input_length) {
  ...
  const uint8 padding_size = (uint8)(key.GetBlockSize() - ((input_length + sizeof(uint32) + sizeof(padding_size)) % key.GetBlockSize()));
  return input_length + sizeof(uint32) + sizeof(padding_size) + padding_size;
}

bool CryptoAES::Encrypt(const CryptoAESKey& key,
                        const uint8* input,
                        int32 input_length,
                        uint8* output,
                        int32& output_length,
  ...
  // 암호화할 데이터는 block_size 로 나누어져야 하므로 padding 사이즈를 구합니다.
  const uint8 padding_size = (uint8)(key.GetBlockSize() - ((input_length + sizeof(uint32) + sizeof(padding_size)) % key.GetBlockSize()));

  if (output_length == 0 || output_length < input_length + (int32)(sizeof(uint32) + sizeof(padding_size) + padding_size)) {
    TRACE_SOURCE_LOCATION();
    return false;
  }

  // 암호화가 끝난후의 데이터 사이즈입니다.
  output_length = input_length + sizeof(uint32) + sizeof(padding_size) + padding_size;
  ...
  UnsafeMemory::Memcpy(output + input_length + padding_size + sizeof(crc), &padding_size, sizeof(padding_size));
  ...
  switch (encryption_mode) {
    ...
    case CFB: {
        ...
        for (int32 i = 0; i < Total; ++i) {
          EncryptBlock(key, chain, output + Position);
          Xor(output + Position, (uint8*)OutputClone.MutableData() + Position, key.block_size);
          UnsafeMemory::Memcpy(chain, output + Position, key.block_size);
          Position += key.GetBlockSize();
        }
      }
...
bool CryptoAES::Decrypt(const CryptoAESKey& key,
                        const uint8* input,
                        int32 input_length,
                        uint8* output,
                        int32& output_length,
  ...
  UnsafeMemory::Memcpy(&crc, output + input_length - sizeof(crc) - sizeof(padding_size), sizeof(crc));
  UnsafeMemory::Memcpy(&padding_size, output + input_length - sizeof(padding_size), sizeof(padding_size));

  output_length = input_length - padding_size - sizeof(crc) - sizeof(padding_size);
  if (output_length < 0) {
    TRACE_SOURCE_LOCATION();
    return false;
  }
...
bool CryptoAES::Encrypt(const CryptoAESKey& key,
  ...
  return Encrypt(key, (const uint8*)input.ConstData(), (int32)input.Len(), (uint8*)output.MutableData(), output_length, encryption_mode);
}
...
bool CryptoAES::Decrypt(const CryptoAESKey& key,
  ...
  output.ResizeUninitialized(output_length);
  if (!Decrypt(key, (const uint8*)input.ConstData(), output_length, (uint8*)output.MutableData(), output_length, encryption_mode)) {
    TRACE_SOURCE_LOCATION();
    return false;
  }
...
int32 CryptoAES::Mul4(int32 A, char B[]) {
  ...
  A = _Log[A & 0xFF];
  const int32 A0 = (B[0] != 0) ? _ALog[(A + _Log[B[0] & 0XFF]) % 255] & 0xFF : 0;
  const int32 A1 = (B[1] != 0) ? _ALog[(A + _Log[B[1] & 0XFF]) % 255] & 0xFF : 0;
  const int32 A2 = (B[2] != 0) ? _ALog[(A + _Log[B[2] & 0XFF]) % 255] & 0xFF : 0;
  const int32 A3 = (B[3] != 0) ? _ALog[(A + _Log[B[3] & 0XFF]) % 255] & 0xFF : 0;
...
bool CryptoAES::DefaultEncryptBlock(const CryptoAESKey& key,
                                    const uint8* input,
                                    uint8* output) {
  if (!key.KeyExists()) {
    // AES key is not initialized.
  ...
  for (int32 r = 1; r < key.Rounds; ++r) {
    ...
    A0 = (_T1[(T0 >> 24) & 0xFF] ^
      _T2[(T1 >> 16) & 0xFF] ^
      _T3[(T2 >>  8) & 0xFF] ^
      _T4[T3 & 0xFF]) ^ KERound[0];
    A1 = (_T1[(T1 >> 24) & 0xFF] ^
      _T2[(T2 >> 16) & 0xFF] ^
      _T3[(T3 >>  8) & 0xFF] ^
      _T4[T0 & 0xFF]) ^ KERound[1];
    A2 = (_T1[(T2 >> 24) & 0xFF] ^
      _T2[(T3 >> 16) & 0xFF] ^
      _T3[(T0 >>  8) & 0xFF] ^
      _T4[T1 & 0xFF]) ^ KERound[2];
    A3 = (_T1[(T3 >> 24) & 0xFF] ^
      _T2[(T0 >> 16) & 0xFF] ^
      _T3[(T1 >>  8) & 0xFF] ^
      _T4[T2 & 0xFF]) ^ KERound[3];
...
bool CryptoAES::DefaultDecryptBlock(const CryptoAESKey& key,
                                    const uint8* input,
                                    uint8* output) {
  if (!key.KeyExists()) {
    // AES key is not initialized.
...
bool CryptoAES::EncryptBlock( const CryptoAESKey& key,
                              const uint8* input,
                              uint8* output) {
  if (!key.KeyExists()) {
    // AES key is not initialized.
...
// input    - the cipher-text.
// output   - the plain-text generated from a cipher-text using the session key.
bool CryptoAES::DecryptBlock( const CryptoAESKey& key,
                              const uint8* input,
                              uint8* output) {
  if (!key.KeyExists()) {
    // AES key is not initialized.
  ...
  for (int32 r = 1; r < key.Rounds; ++r) {
    for (i = 0; i < BlockCount; ++i) {
      a[i] = (_T5[(t[i] >> 24) & 0xFF] ^
        ...
        _T8[ t[(i + s3) % BlockCount] & 0xFF]) ^ key.KD[r][i];
    }
...
int32 CryptoAESKey::GetRounds() const {
  return KeyExists() ? Rounds : -1;
}

bool CryptoAESKey::KeyExists() const {
  return key_length > 0;
}

void CryptoAESKey::Reset() {
  key_length = 0;
}
...
const int32 CryptoAES::_Shifts[3][4][2] = { { { 0, 0 }, { 1, 3 }, { 2, 2 }, { 3, 1 } }, { { 0, 0 }, { 1, 5 }, { 2, 4 }, { 3, 3 } }, { { 0, 0 }, { 1, 7 }, { 3, 5 }, { 4, 4 } }
};
