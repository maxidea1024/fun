...
UdpSocket_S::UdpSocket_S(NetServerImpl* owner) {
  ...
  // You do not have to deal with send brakes on the server side. it only causes a bad bug.
  // In addition, if you want to solve the problem with a brake, short interval processing is required, which is a bad server load.
...
void UdpSocket_S::ConditionalIssueSend() {
  // The main lock should not get hung up, because what it does is not involved in the main lock.
  ...
  // If you issue a closed socket, completion occurs anyway. Scraping the memory.
  // You should check this before checking if you want to send. must!
  if (!send_issued_ && !socket_->IsClosedOrClosing()) {
    ...
    if (packet_fragger_->PopAnySendQueueFilledOneWithCoalesce(*XXX, owner_->GetAbsoluteTime()) &&
        send_issued_frag_->send_frag_frag.Buffer.Count() > 0) {
      lock.Unlock();

      send_issued_ = true;
      const SocketErrorCode socket_error = socket_->IssueSendTo_NoCopy_TempTtl(send_issued_frag_->send_frag_frag, send_issued_frag_->send_to, send_issued_frag_->ttl);
...
NamedInetAddress UdpSocket_S::GetRemoteIdentifiableLocalAddr(RemoteClient_S* rc) {
  AssertIsNotLockedByCurrentThread(); // mainlock만으로 진행이 가능할듯.

  NamedInetAddress result = NamedInetAddress(socket_->GetSockName());

  // The address of the socket that accepted the TCP connection. The most successful connection rate is low.
  // Even if there are two or more NICs, the address of the TCP connection is likely to receive the UDP connection, so OK.
  result.OverwriteHostNameIfExists(rc->to_client_tcp_->socket_->GetSockName().GetHost().ToString());

  // If the server has more than one NIC and if the NIC address is specified, first choose the NIC address.
...
double UdpSocket_S::GetAbsoluteTime() {
  return owner_->GetAbsoluteTime();
}

void UdpSocket_S::RequestReceiveSpeedAtReceiverSide_NoRelay(const InetAddress& dest_addr) {
  ...
  const auto filter_tag = FilterTag::Make(HostId_Server, HostId_None); // Server -> None
...
void UdpSocket_S::EnqueuePacketDefragWarning(
      const InetAddress& sender, const char* text) {
...
HostId UdpSocket_S::GetSrcHostIdByAddrAtDestSide_NOLOCK(const InetAddress& address) {
  return owner_->GetSrcHostIdByAddrAtDestSide_NOLOCK(address);
}

HostId UdpSocket_S::GetLocalHostId() {
  return owner_->GetLocalHostId();
}

void UdpSocket_S::Decrease() {
  DecreaseUseCount();
}
...
CCriticalSection2& UdpSocket_S::GetMutex() {
  return mutex_;
}
