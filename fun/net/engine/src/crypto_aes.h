...
class CryptoAES {
 public:
  enum EncryptionMode { ECB = 0, CBC = 1, CFB = 2 };

  enum { DEFAULT_BLOCK_SIZE = 16 };
  enum { MAX_BLOCK_SIZE = 32, MAX_ROUNDS = 14, MAX_KC = 8, MAX_BC = 8 };

  static bool ExpandFrom(CryptoAESKey& out_key, const uint8* input_key,
                         int32 key_length = DEFAULT_BLOCK_SIZE,
                         int32 block_size = DEFAULT_BLOCK_SIZE);
  static int GetEncryptSize(const CryptoAESKey& key, int32 input_length);
  static bool Encrypt(const CryptoAESKey& key, const uint8* input,
                      int32 input_length, uint8* output, int32& output_length,
                      EncryptionMode encryption_mode = ECB);
  static bool Decrypt(const CryptoAESKey& key, const uint8* input,
                      int32 input_length, uint8* output, int32& output_length,
                      EncryptionMode encryption_mode = ECB);
  static bool Encrypt(const CryptoAESKey& key, ByteStringView input,
                      ByteArray& output, EncryptionMode encryption_mode = ECB);
  static bool Decrypt(const CryptoAESKey& key, ByteStringView input,
                      ByteArray& output, EncryptionMode encryption_mode = ECB);

 private:
  static bool DefaultEncryptBlock(const CryptoAESKey& key, const uint8* input,
                                  uint8* output);
  static bool DefaultDecryptBlock(const CryptoAESKey& key, const uint8* input,
                                  uint8* output);
  static bool EncryptBlock(const CryptoAESKey& key, const uint8* input,
                           uint8* output);
  static bool DecryptBlock(const CryptoAESKey& key, const uint8* input,
                           uint8* output);
