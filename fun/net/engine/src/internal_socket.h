 ...
 * WSABUF에는 보낼 데이터의 참조만 기록되는데, 실제로 보내기가 완료되거나
 * 중지될때까지 참조를 유지해주어야한다.
 */
...
/**
 * overlapped I/O를 할 경우 validation 값을 여기서 가지므로 소멸 자체도 worker
 * thread에서 하는 것이 필요하다.
 *
 * IOCP에 포인터 값으로 직접 넘어가므로, 가상소멸자가 있으면 안됨.
 * 더불어, ListNode<>, ICompletionKey에도 가상소멸자가 있으면 안됨.
 *
 * 넘기기는 InternalSocket*을 어드레스 값(uintptr_t)으로 넘기는데, IOCP에서
 * 넘겨받은 어드레스 값(uintptr_t)을 ICompletionKey*로 캐스팅 후 액세스하는
 * 부분이 있는데, 여기서 어드레스 왜곡이 생김.
 *
 * 이부분에 대해서는 좀더 면밀히 살펴봐야할것으로 보인다.
 */
class InternalSocket
    : public ListNode<InternalSocket>  // Faked IOCP 구현을 위해서만 필요함.
                                       // (차후에 reactor구현시 필요할수도..)
    ,
 ...
 private:
  ...
  FUN_ALIGNED_VOLATILE bool
      socket_closed_or_closing_mutex_protected_;  // FUN_ALIGNED_VOLATILE:
                                                  // IsClosedOrClosing 때문

  void InitOthers();

  OverlappedEx accept_ex_ovl_;
  Array<uint8> accept_ex_buffer_;
  DWORD accept_ex_length_;  //@todo AcceptEx함수의 인자로 사용되는데, DWORD*
                            //타입을 요구함..
  ...
  //@note 꼭 필요한 값은 아니지만, 디버깅을 위해서 변수 형태로 가지고 있게함.
  //차후에 제거해도 될듯..
  SOCKADDR_IN sendto_addr_;
  int32 sendto_addr_len_;

  DWORD recv_flags_;  // recv, recvfrom에서 수신된 flags값. overlapped I/O에도
                      // 업뎃되므로 유지해야 하기에 멤버로 선언했다.
 ...
 public:
  ...
  // TODO 실질적으로 CompletionEvent, bIssued는 의미가 없음. 사용도 되지 않고,
  // 이 함수는 차라리 없애는것도 좋을듯... static void
  // ResetOverlappedStruct(OverlappedEx& obj, Event* completion_event = nullptr,
  // bool issued = false);
 ...
 public:
  ...
  bool GetConnectExOverlappedResult(bool wait_until_complete,
                                    OverlappedResult& out_result);
#endif

 public:
  //@deprecated 제거하도록 하자.
  // 테스트 용도이다. overlapped io 미완료로 인한 메모리 긁기가 발생하는지
  // 체크를 위함
  ...
  /**
   * 소켓 내부의 버퍼 또는 변수들은 그대로 유지한채, 소켓만 재생성할 경우에
   * 사용됨. 이렇게 내부 변수들은 그대로 둔채, 소켓 객체만 다시 생성하는 동작을
   * Restore라 칭함.
   */
  ...
  void FinalizeAcceptEx(InternalSocket* tcp_listening_socket,
                        InetAddress& local_addr, InetAddress& remote_addr);
  ...
  SocketErrorCode IssueSendTo(const uint8* data, int32 length,
                              const InetAddress& sendto);
  inline SocketErrorCode IssueSendTo_NoCopy(FragmentedBuffer& send_buffer,
                                            const InetAddress& sendto) {
    return IssueSendTo_NoCopy_TempTtl(send_buffer, sendto, -1);
  }
  SocketErrorCode IssueSendTo_NoCopy_TempTtl(FragmentedBuffer& send_buffer,
                                             const InetAddress& sendto,
                                             int32 ttl);
  ...
  SocketErrorCode BlockedSendTo(const uint8* data, int32 length,
                                const InetAddress& sendto);
  SocketErrorCode BlockedRecvFrom(uint8* buff, int32 buff_length,
  ...
  static bool IsUdpStopErrorCode(
      SocketErrorCode socket_error);  // TODO 쓰이지 않고 있음.
  static bool IsTcpStopErrorCode(
      SocketErrorCode socket_error);  // TODO 쓰이지 않고 있음.
  static bool IsPendingErrorCode(SocketErrorCode socket_error);
};

// SSDP socket에서도 참조하며 Socket같은건 자주 만드는게 아니니까 이렇게 해도
// OK. 자주 접근하므로 thread safe로 만들면 곤란. 웬만하면 쓰지 말자. RefCount이
// assign op는 thread unsafe하니까.
 ...
 *
 * 주의: Wait 호출 후에는 FD_SET의 내용이 바뀐다.  따라서 이 객체는 1회성으로
 * 쓰여야 한다.
 */
