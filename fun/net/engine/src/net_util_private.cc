...
Array<IpAddress>& NetUtil::LocalAddresses()
{
  ...
  if (CachedLocalAddresses.IsEmpty()) {
    try {
      //주의 : Windows의 경우 아래의 함수가 호출되기전에 윈속이 초기화되어 있어야함.
      
      //TODO warning: 참조로 가져오면 문제가 발생함. 왜일까?? 나중에 분석을 좀 해봐야할듯...
...
void NetUtil::AssertCloseSocketWillReturnImmediately(SOCKET socket)
{
  ...
  if (getsockopt(socket, SOL_SOCKET, SO_LINGER, (char*)&Value, &ValueLen) == 0) {
    if (Value.l_onoff && Value.l_linger) {
      ErrorReporter::Report("FATAL: socket which has behavior of some waits in closesocket() has been detected.");
    }
  }
}
...
void NetUtil::SetTcpDefaultBehavior(InternalSocket* socket)
{
  ...
  if (NetConfig::socket_tcp_keep_alive_option_enabled) {
    int32 Value = 1;
    setsockopt(socket_->socket_, SOL_SOCKET, SO_KEEPALIVE, (char*)&Value, sizeof(Value));
  }
}
