...
void CompletionPort::PostWin32Warning(const char* where) {
  event_->OnCompletionPortWarning(
      this, *String::Format("failure at %s: %d", where, GetLastError()));
}
...
void CompletionPort::AssociateSocket(InternalSocket* sock) {
  ...
  if (!::CreateIoCompletionPort((HANDLE)sock_->sock_, iocp_handle_,
                                (ULONG_PTR)socket, 0)) {
...
bool CompletionPort::GetQueuedCompletionStatus(CompletionStatus* out_result,
  ...
  *out_result = CompletionStatus();
  ...
  ::GetQueuedCompletionStatus(iocp_handle_, &bytes, &key,
                              (OVERLAPPED**)&overlapped, timeout_msec);

  // Overlapped가 null일 경우에는 timeout등이 발생한 경우이므로, 일단은 false를
  // 반환하고 다음 호출을 처리.
  ...
  if (spi && overlapped == &spi->recv_ovl_) {
    ...
    // out_result->sendto_addr = InetAddress((sockaddr*)&spi->sendto_addr,
    // spi->SendToAddrLen); //@note 이건 테스트용...
  ...
  if (GetLastError() == WSAEMSGSIZE) {
    // 해커가 보낸 메시지로도 생길 수 있으므로 ShowUserMisuseError는 과잉진압.
    // 하지만 분명히 경고는 남겨서 해결을 도모해야 할지도.
    //@todo
    // LOG(LogNetEngine, Warning, "FunNet always send MTU-length aware packets,
    // thus, never occurs WSAEMSGSIZE! You are missing something! WSAEMSGSIZE
    // will cause ResultCode::InvalidPacketFormat!");
  }
...
bool CompletionPort::GetQueuedCompletionStatusEx(
    Array<CompletionStatus>& out_result, int32& dequeue_count,
    uint32 timeout_msec) {
  ...
  if (!::GetQueuedCompletionStatusEx(
          iocp_handle_, overlapped_entry, GQCS_EX_REMOVED_COUNT,
          &removed_entry_count, timeout_msec, FALSE)) {
  ...
  for (uint32 i = 0; i < removed_entry_count; ++i) {
    ...
    if (IocpCustomValueInRange((INT_PTR)overlapped_entry[i].lpOverlapped)) {
      ...
      status.completed_length =
          overlapped_entry[i].dwNumberOfBytesTransferred;  // 0이 아니라.
    ...
    if (spi && overlapped_entry[i].lpOverlapped == &spi->recv_ovl_) {
      ...
      spi->send_ovl_.issued = false;
               ...
               overlapped_entry[i].lpOverlapped == &spi->accept_ex_ovl_) {
      status.type = CompletionType::AcceptEx;
      spi->accept_ex_ovl_.issued = false;
    } else if (spi &&
               overlapped_entry[i].lpOverlapped == &spi->connect_ex_ovl_) {
...
void CompletionPort::Post(ICompletionKey* key, uintptr_t value) {
  ...
  ::PostQueuedCompletionStatus(iocp_handle_, 0, (ULONG_PTR)key,
                               (OVERLAPPED*)value);
}
