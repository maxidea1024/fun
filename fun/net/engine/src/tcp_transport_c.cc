...
TcpTransport_C::TcpTransport_C(ITcpTransportOwner_C* owner)
  ...
  socket_.Reset(new InternalSocket(SocketType::Tcp, owner_,
                                   owner_->GetIoCompletionDelegate()));
  ...
  // 안전하다. recv buffer 제거는 백해무익이므로 즐
  ...
  // recv buffer를 크게 잡을수록 OS 부하가 커진다. 특별한 경우가 아니면 이것은
  // 건드리지 않는 것이 좋다.
...
void TcpTransport_C::SendWhenReady(const SendFragRefs& data_to_send,
  ...
  // 송신큐에 넣음
  // 최초 송신 이슈는 10ms마다 하므로(사람이 못느끼며 TCP nagle off시 coalesce
  // 효과) iocp post는 불필요. 결론적으로 10ms 내의 송신 데이터들은
  // 뭉쳐서(coalesce) 보낸다는 얘기..
  send_queue_.EnqueueCopy(final_send_data, send_opt);

  //// 워커스레드에서 10ms마다 최초 송신 이슈를 걸기 때문에 신호를 던질 필요
  ///없음 / 그러나 최초 송신 필요함 리스트에는 등록을 해야 함
...
void TcpTransport_C::ConditionalIssueSend() {
  ...
  if (!socket_->IsClosed() && !send_issued_) {
    const int32 braked_send_amount =
        GetBrakedSendAmount(owner_->GetAbsoluteTime());
    if (braked_send_amount > 0) {
      // 상한선이 넘어도 문제 없다. 어차피 버퍼는 safe하고 TCP socket이 가득
      // 차면 어차피 completion만 늦을 뿐이니까.
      ...
      //   => TCP issue recv or recv completion에서 소켓 닫힘이 감지되며,
      //   OnLeftFromServer로 이어질 것임.
      if (result != SocketErrorCode::Ok) {
        send_issued_ = false;
        socket_->CloseSocketHandleOnly();  // induce disconnecting.

        owner_->OnSocketWarning(
            socket_.Get(), String::Format("%s : issue-send failed -error:%d",
                                          __FUNCTION__, (int32)result));
      } else {
        send_queue_.send_brake_.Accumulate(braked_send_amount);
        send_queue_.send_speed_.Accumulate(braked_send_amount,
...
int32 TcpTransport_C::GetBrakedSendAmount(double absolute_time) {
  ...
  return MathBase::Min<int32>(
      send_queue_.send_brake_.GetMaxSendAmount(
          absolute_time, send_queue_.allowed_max_send_speed_.GetValue()),
      queued_data_length);
}
...
void TcpTransport_C::RefreshLocalAddress() {
  ...
  if (!local_addr_.IsUnicast()) {
    ...
    //그러나, binding된 port번호를 얻기만 하면 되므로 이 러한 상황이 필요할 수
    //있음. 다른곳에서 오류가 안나도록 로컬 주소를 찾아서 대입시켜주자. 단, 포트
    //번호는 유지해야함.
