...
double NetConfig::client_connect_server_timeout_sec = tcp_socket_connect_timeout_sec * 2 + 3;
...
double NetConfig::server_udp_repunch_interval_sec = NetConfig::server_holepunch_interval_sec * 3;
...
//@maxidea:
// 주의 : 서버에 트래픽을 야기할 수 있으므로, 꼭 필요한 상황이 아니라면 꺼주는게 바람직할듯함.
bool NetConfig::report_real_udp_count_enabled = true;

//@warning
// 테스트를 위해서 임시로 인터벌을 짧게 가져감. (원래는 30초였음)
// 통계관련된 부분이므로 report_real_udp_count_enabled를 false로 해서 아예 기능을 끌수도 있어야할듯함.
// 실 서버시에는 그다지 필요없는 기능이므로..
// 서버에 접속시 설정값을 내려주는것도 좋을듯 싶음.
double NetConfig::report_real_udp_count_interval_sec = 30; // 30초 마다 보고

const bool NetConfig::speedhack_detector_enabled_by_default = false; // 테스트에 방해가 되어서 일단 막아둠.
...
//주의:
// 이 값을 수정하려거든, C#의 같은 값도 맞춰줘야함.  그렇지 않으면, PacketFragging에서 문제가 발생함.
// PacketFragging시에 MTU 단위로 fragmentation이 일어나기 때문임.
int32 NetConfig::MTU = 500;

double NetConfig::remove_too_old_udp_send_packet_queue_timeout_sec = 3*60; // 3분
double NetConfig::assemble_fragged_packet_timeout_sec = 10; // 10초

double NetConfig::default_graceful_disconnect_timeout_sec = 2; // 2초 이상 접속해제 대기
...
CCriticalSection2& NetConfig::GetWriteMutex() {
  return write_mutex;
}
...
//TODO 테스트임.  구태여 미리 홀펀칭할 이유는 없음.  그러나 과도기이므로 테스트를 위해서 미리 켜두는 형태로 처리하자.
//추후에 반듯이 JIT로 변경하도록 하자.
//DirectP2PStartCondition NetConfig::default_direct_p2p_start_condition = DirectP2PStartCondition::Jit;
DirectP2PStartCondition NetConfig::default_direct_p2p_start_condition = DirectP2PStartCondition::Always;
...
double NetConfig::viz_reconnect_try_interval_sec = 3 + client_connect_server_timeout_sec;
