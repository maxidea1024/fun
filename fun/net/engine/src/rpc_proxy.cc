...
RpcProxy::~RpcProxy() {
  if (core_) {
    LOG(LogNetEngine, Warning,
        "RPC proxy which is still in use by FunNet core cannot be destroyed! "
        "destroy NetClient or NetServer instance first.");
  }
}

bool RpcProxy::RpcCall(const HostId* rpc_sendto_list, int32 rpc_sendto_count,
                       const RpcCallOption& rpc_call_opt,
                       const MessageOut& message, const char* rpc_name,
                       RpcId rpc_id, const char* arguments) {
  if (core_ == nullptr) {
  ...
  const bool result = core_->Send(frag_refs, SendOption(rpc_call_opt),
                                  rpc_sendto_list, rpc_sendto_count);
  ...
  if (arguments && (notify_send_by_proxy_enabled_ && !engine_specific_only_)) {
    ...
    core_->Viz_NotifySendByProxy(rpc_sendto_list, rpc_sendto_count, summary,
                                 rpc_call_opt);
  }
...
bool RpcProxy::RpcFailure(const HostId* rpc_sendto_list, int32 rpc_sendto_count,
  ...
  if (core_ == nullptr) {
    ...
    RpcHeader::Write(header, result_code,
                     error_message ? error_message : "");  // header

    const bool result =
        core_->Send(SendFragRefs(header), SendOption(rpc_call_opt),
                    rpc_sendto_list, rpc_sendto_count);
