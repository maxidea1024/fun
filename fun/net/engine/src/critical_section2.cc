...
CCriticalSection2::CCriticalSection2(bool bFasterForWindows6) {
  ...
  if (bFasterForWindows6) {  // CSingleton에서는 이걸 접근할 때 false이므로 아래
                             // Kernel32Api 싱글톤을 또 접근하는 우가 없음
    if (Kernel32Api::Get().InitializeCriticalSectionEx !=
        nullptr) {  // 반드시 이걸 나중에 체크해야 한다. Instance를 접근하는
                    // 순간 critsec을 접근하므로!
      if (!Kernel32Api::Get().InitializeCriticalSectionEx(
              &CS, 0,
              bFasterForWindows6 ? CRITICAL_SECTION_NO_DEBUG_INFO : 0)) {
        throw Exception(TEXT("InitializeCriticalSectionEx failure."),
                        __FUNCTION__);
      } else {
        return;
      }
...
CCriticalSection2::~CCriticalSection2() {
  ...
  if (IsCriticalSectionLocked(CS)) {
    // throw Exception(TEXT("critical-section is still in use. destruction may
    // cause problems."));
    fun_check(0);
  }
...
#ifdef _DEBUG
  UnsafeMemory::Memset(&CS, 0x00,
                       sizeof(CS));  // 이렇게 지워버려야 디버깅중 무효화된
                                     // 객체인지 체크가 가능하다.
#endif
}
...
void CCriticalSection2::Lock() {
  ...
  if (ValidKey != ValidKeyValue) {
    // LOG(LogNetEngine, Warning, TEXT("cannot enter critical-section which has
    // been already destroyed! NOTE: this may be solved by deleting NetClient
    // instance before your WinMain() finishes working."));
    assert(0);
  }
...
bool CCriticalSection2::TryLock() {
  if (ValidKey != ValidKeyValue) {
    // LOG(LogNetEngine, Warning, TEXT("lock invalid critical-section is
    // prohibited! NOTE: this may be solved by manually destroying FunNet related
    // objects if they are defined as global objects."));
    assert(0);
  }
...
void CCriticalSection2::Unlock() {
  ...
  // unlock 단계에서는 체크하지 말자. safe lock, unsafe lock 중 어느걸 했는지
  // 모르므로, lock에서 엄한 사태인데 unlock이 무슨 의미리요. 게다가 파괴자에서
  // throw? 이거 짜증남 따라서 즐. 특히 BST 케이스에서 이게 문제됐다!
...
bool CCriticalSection2::IsValid() { return ValidKeyValue == ValidKey; }
...
bool IsCriticalSectionLocked(const CRITICAL_SECTION& CS) {
  // fun_check(IsIocpSupported()); 막은 이유: 이 함수가 CKernel32API singleton을
  // 접근하니까.
