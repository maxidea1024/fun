...
class IHostIdFactory {
 public:
  ...
  virtual HostId Create(double absolute_time,
...
class HostIdFactory : public IHostIdFactory {
 public:
  ...
  HostId Create(double absolute_time,
                HostId assigned_host_id = HostId_None) override;
...
class RecycleHostIdFactory : public IHostIdFactory {
 ...
 private:
  struct Node {
    ...
    Node() : host_id(HostId_None), recycle_count(0), dropped_time(0) {}
  };
