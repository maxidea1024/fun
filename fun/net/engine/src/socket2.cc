...
bool SocketSelectContextImpl::GetConnectResult(Socket2& socket, SocketErrorCode& out_code) {
  Socket2Impl* ptr = nullptr;
  ...
  if (ptr) {
    return selection_context_.GetConnectResult(*(ptr->internal_socket_.Get()), out_code);
  }
  return false;
}
...
Socket2Impl::Socket2Impl(SOCKET exisiting_socket, ISocketDelegate* delegate)
  ...
  internal_socket_.Reset(new internal_socket_(exisiting_socket, (IInternalSocketDelegate*)this));
}

Socket2Impl::Socket2Impl(SocketType socket_type, ISocketDelegate* delegate)
  : internal_delegate_(delegate) {
  internal_socket_.Reset(new internal_socket_(sockettype, (IInternalSocketDelegate*)this));
}

bool Socket2Impl::Bind() {
  return internal_socket_->Bind();
}

bool Socket2Impl::Bind(int32 port) {
  return internal_socket_->Bind(port);
}
...
SocketErrorCode Socket2Impl::IssueSendTo(const uint8* data, int32 len, const InetAddress& sendto) {
...
bool Socket2Impl::GetRecvOverlappedResult(bool wait_until_complete,
                                          OverlappedResult& out_overlapped_result) {
  return internal_socket_->GetRecvOverlappedResult(wait_until_complete, out_overlapped_result);
}

bool Socket2Impl::GetSendOverlappedResult(bool wait_until_complete,
                                          OverlappedResult& out_overlapped_result) {
  return internal_socket_->GetSendOverlappedResult(wait_until_complete, out_overlapped_result);
}

bool Socket2Impl::GetAcceptExOverlappedResult(bool wait_until_complete,
                                              OverlappedResult& out_overlapped_result) {
  return internal_socket_->GetAcceptExOverlappedResult(wait_until_complete, out_overlapped_result);
}
