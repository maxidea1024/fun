...
/**
 * Reliable UDP 수신기
 * window size나 ack, frame 등의 용어는 일반적인 data link layer 및 TCP 프로토콜의 용어에서와 같다.
 * 단독으로는 쓰이지 못하고, RUdpSender와 쌍을 맺어야 한다.
 * RUdpSender를 통해 ack를 보내니까.
 * 주요 메서드: ProcessRawReceivedFrames, Tick, FlushAckAccumulatedToOutput
 */
class RUdpReceiver {
 private:
  RUdpHost* owner_;
 ...
 public:
  ...
  /**
  이번에 플러쉬될 프레임의 기대하는 ID. 즉 수신자에서 이 값의 패킷이 올 차례가 되어야 한다.
  */
  ...
  /**
  수신된 데이터 프레임에 대한 ack를 보내되 한번에 여러개를 보내기 위해 일단 모아놓는 공간
  짧은 순간에만 사용되는 변수
  */
