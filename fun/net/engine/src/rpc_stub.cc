...
void RpcStub::ShowUnknownHostIdWarning(HostId remote_id) {
  //LOG(Warning) << Substitute("경고: ProcessReceivedMessage에서 unknown host_id $0!", (int32)remote_id);
  //LOG(Warning) << String::Format("ProcessReceivedMessage에서 unknown host_id %d!", (int32)remote_id);
}
...
RpcStub::~RpcStub() {
  if (core_) {
    LOG(LogNetEngine, Warning, "RPC stub which is still in use by FunNet core cannot be destroyed! Destroy NetClient or NetServer instance first.");
  }
}
