...
LanListener_C::~LanListener_C() { StopListening(); }
...
bool LanListener_C::IsListening() const { return thread_.IsValid(); }
...
uint32 LanListener_C::Run() {
  try {
        ...
        if (new_socket) {
          ...
          if (remote_addr.IsUnicast()) {
            if (owner_->intra_logger_) {
              const String text = String::Format("TCP accept from address: %s",
            ...
            // 접속하는 중에 원격지에서 접속을 끊을 경우, 주소가 제대로 인식되지
            // 않을 수 있습니다. 이 경우 접속을 받아줘봐야 의미가 없으므로,
            // 소켓을 drop합니다.
          }
    ...
    if (owner_->intra_logger_) {
      const String text =
          String::Format("%s thread is terminated.", __FUNCTION__);
      owner_->intra_logger_->WriteLine(LogCategory::System, *text);
    }
...
bool LanListener_C::AcceptNewSocket(InternalSocket* new_socket,
  ...
  // 제거된다.
  // 성공 했을 경우에는 completion Context가 AcceptedInfo에서 RemotePeer로 교체
  // 된다. 즉, Accept 과정중에만 사용되는 임시 객체다.
  auto accepted_info = new AcceptedInfo(owner_);

  //@maxidea: 여기서 구지 락을 해야할까?  contention이 일어나는 상황도 아닌데
  //말이지...
  ...
  // socket 객체를 Attach 시켜준다.  Accept가 성공 여부에 상관 없이 완료되면
  // Detach되며, Socket이 Detach된 AcceptedInfo 객체는 제거 된다.
  ...
  accepted_info->socket_->SetCompletionContext(
      (ICompletionContext*)accepted_info);
  ...
  owner_->net_thread_pool_->PostCompletionStatus(
      accepted_info->socket_.Get(), (UINT_PTR)IocpCustomValue::NewPeerAccepted);

  if (owner_->intra_logger_) {
    const String text = String::Format(
        "New peer TCP socket is accepted.  accepted_info: %ph, tcp_socket: %s",
        accepted_info, *remote_addr.ToString());
    owner_->intra_logger_->WriteLine(LogCategory::System, *text);
  }
      ...
      default:
        if (socket_error != SocketErrorCode::NotSocket) {
          ...
          owner_->EnqueueError(ResultInfo::From(
              ResultCode::Unexpected, owner_->GetLocalHostId(), text));
        }
        delete new_socket;
        return nullptr;
    }
...
void LanListener_C::CatchThreadUnexpectedExit(const char* where,
                                              const char* reason) {
  if (owner_ && owner_->callbacks_) {
    ...
    auto result_info = ResultInfo::From(
        ...
        String::Format("(%s): Unexpected thread exit with (%s)", where,
                       reason));
