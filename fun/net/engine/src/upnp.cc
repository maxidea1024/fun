//@deprecated simple-socket을 사용해서 간단히 만들도록 하자. 아니면,
//Http::CSession을 사용해도 좋을듯..
...
static const InetAddress SSDP_REQUEST_TO =
    InetAddress(String("239.255.255.250"), 1900);

static const ByteArray SSDP_REQUEST = ByteArray(
    "M-SEARCH * HTTP/1.1\r\n"
...
void UPnP::Heartbeat_Ssdp_RequestingCase(SsdpContext* ssdp) {
  while (true) {
    ...
    if (overlapped_result.socket_error != SocketErrorCode::Ok ||
        overlapped_result.completed_length == 0) {
...
void UPnP::Heartbeat_Ssdp_ResponseWaitingCase(SsdpContext* ssdp) {
  while (true) {
    ...
    // 수신 완료. (ByteArray 자체가 대부분의 경우 nul-term을 보장하므로, 끝에
    // nul을 추가로 붙여줄필요 없음.)
    ByteArray reponse(overlapped_result.completed_length, NoInit);
    UnsafeMemory::Memcpy(reponse.MutableData(),
                         ssdp->socket_->GetRecvBufferPtr(),
                         overlapped_result.completed_length);
    ...
    if (new_nat->TakeResponse(reponse)) {
      if (discovering_nat_devices_.Count() < 20) {  // THIS IS NEEDED
        ...
        if (!discovering_nat_devices_.Contains(
                Host)) {  // 같은 이름의 디바이스가 이미 등록되어 있으면 제외
    ...
    const double time_diff =
        manager_->GetCachedAbsoluteTime() - ssdp->response_wait_start_time_;
    if (time_diff > 3) {
...
void UPnP::Ssdp_IssueRequest(SsdpContext* ssdp) {
  ...
  const SocketErrorCode socket_error =
      ssdp->socket_->IssueSendTo((const uint8*)SSDP_REQUEST.ConstData(),
                                 SSDP_REQUEST.Len() + 1, SSDP_REQUEST_TO);
  if (socket_error != SocketErrorCode::Ok &&
    ...
    char error_msg[2048];
    CPlatformMisc::GetSystemErrorMessage(error_msg, countof(error_msg),
                                         (int32)socket_error);
    TRACE_SOURCE_LOCATION_MSG(
        *String::Format("sendto failed: %s (socker_error: %d)", error_msg,
                        (int32)socket_error));
  } else {  // OK
...
void UPnP::Ssdp_IssueResponseWait(SsdpContext* ssdp) {
  ...
  UnsafeMemory::Memset(ssdp->response_buffer_.MutableData(), 0x00,
                       ssdp->response_buffer_.Len());

  ssdp->recv_issued_ = true;

  if (ssdp->socket_->IssueRecvFrom(ssdp->response_buffer_.Len() - 1) ==
      SocketErrorCode::Ok) {
...
void UPnP::Heartbeat_Discovering(DiscoveringNatDevice* device) {
  switch (device->GetState()) {
    case HttpState::Connecting: {
      if (device->GetCurrentStateTurnCounter() ==
          0) {  // first chance in this state
        device->socket_.Reset(new InternalSocket(SocketType::Tcp, device));
        device->socket_->Bind();
        device->socket_->SetBlockingMode(false);

        // const ByteArray URI = device->uri_.GetHost() + "/" +
        // device->uri_.GetPath(); const ByteArray URI =
        // ByteArray::Format("%s:%d",
        //        *device->uri_.GetHost(),
        //        device->uri_.GetPort());
        if (!device->IssueConnect(String(device->uri_.GetHost()),
                                  device->uri_.GetPort())) {
      ...
      if (select_context.GetConnectResult(*device->socket_, socket_error)) {
        if (socket_error ==
            SocketErrorCode::Ok) {  // 정상적으로 접속이 이루어졌으므로, 요청을
                                    // 보내도록 합니다.
    ...
    case HttpState::Requesting: {
      if (device->GetCurrentStateTurnCounter() ==
        ...
        // TODO 버그인지는 모르겠으나, Uri::GetPath()에서 반환된 경로명에 앞에가
        // '/'로 시작하지 않아서, 요청이 안되는 현상이 있음.
        if (!path.StartsWith('/')) {
          path.Prepend('/');
        }
      ...
      if (device->socket_->GetSendOverlappedResult(0, overlapped_result)) {
        ...
        if (overlapped_result.completed_length < 0) {
          ...
          device->http_request_issue_offset_ +=
              overlapped_result.completed_length;

          const int32 length = device->http_request_.Len() -
                               device->http_request_issue_offset_ + 1;
          if (length <= 0) {
    ...
    case HttpState::ReponseWaiting: {
      ...
      if (device->GetCurrentStateTurnCounter() ==
        ...
        if (device->socket_->IssueRecv(HTTP_RESPONSE_BUFFER_LENGTH) !=
            SocketErrorCode::Ok) {
      ...
      if (device->socket_->GetRecvOverlappedResult(0, overlapped_result)) {
        ...
        if (overlapped_result.completed_length <= 0) {
          ...
          device->http_response_buffer_.Append(
              device->socket_->GetRecvBufferPtr(),
              overlapped_result.completed_length);
          device->http_response_issue_offset_ +=
              overlapped_result.completed_length;

          const int32 length =
              HTTP_RESPONSE_BUFFER_LENGTH - device->http_response_issue_offset_;
          if (length <= 0) {
            device->SetState(HttpState::ParsingHttpResponse);
            return;
          } else {
    ...
    case HttpState::ParsingHttpResponse:
      if (!device->http_response_buffer_.IsEmpty()) {
        device->http_response_ =
            ByteArray((const char*)device->http_response_buffer_.ConstData(),
                      device->http_response_buffer_.Count());
        ...
        // FriendlyName + '##' + ModelDescription 형태로 되어 있는데, 그냥
        // FriendlyName만 있어도 좋지 않을까 싶은데... nat_device_name =
        // GetXmlNodeValue(device->http_response_, "FRIENDLYNAME") +
        // ByteArray("##") + GetXmlNodeValue(device->http_response_,
        // "MODELDESCRIPTION");
        nat_device_name =
            GetXmlNodeValue(device->http_response_, "FRIENDLYNAME");
        control_url = GetXmlNodeValueAfter(device->http_response_, "CONTROLURL",
                                           "WANIPCONNECTION");

        if (!nat_device_name.IsEmpty()) {
          LOG(LogNetEngine, Info, "FOUND device.  name: %s, control_url: %s",
              *String(nat_device_name), *String(ControlUrl));
        }
...
ByteArray UPnP::GetXmlNodeValue(const ByteArray& xml_doc,
  ...
  ByteArray xml_doc_cap = xml_doc;
  xml_doc_cap.MakeUpper();
  ByteArray node_name_cap = node_name;
  node_name_cap.MakeUpper();
...
ByteArray UPnP::GetXmlNodeValueAfter(const ByteArray& xml_doc,
  ...
  ByteArray node_name_cap = node_name;
  node_name_cap.MakeUpper();
  ByteArray find_after_cap = find_after;
  find_after_cap.MakeUpper();
...
void UPnP::IssueTermination() {
  ...
  // DeletePortMapping 명령은 웬만하면 반드시 모두 실행된 후에 게임 클라
  // 프로세스가 종료되어야 한다. 이를 위패 DeletePortMapping Task가 남아 있는 한
  // issue termination을 유보해야 한다. 그러나 마냥 유보할 수는 없으므로 일정
  // 시간 기다리도록 하자.
...
void UPnP::WaitUntilNoIssueGuaranteed() {
  while (true) {
    ...
    for (auto it = ssdp_contextes_.CreateConstIterator(); it; ++it) {
      ...
      if (context->socket_) {
        if (context->socket_->GetRecvOverlappedResult(false,
                                                      overlapped_result)) {
          context->recv_issued_ = false;
        }

        if (context->socket_->GetSendOverlappedResult(false,
                                                      overlapped_result)) {
...
void UPnP::AddTcpPortMapping(const InetAddress& lan_addr,
  ...
  if (auto device = GetCommandAvailableNatDevice()) {
    // 공유기의 task queue에, TCP연결-송신-수신-완료 과정을 수행할 첫 issue를
    // 건다. 건 이슈는 netClient manager worker thread에 의해서 issue connect가
    // 될 것이다. 이를 위한 heartbeat도 이미 작동하고 있다.
    ...
    LOG(LogNetEngine, Warning,
        "AddTcpPortMapping.  lan_address: %s, wan_address: %s",
        *lan_addr.ToString(), *wan_addr.ToString());
  }
}
...
void UPnP::Heartbeat_Task_InitState(UPnPTask* task) {
  ...
  if (!task->IssueConnect(String(task->owner_device_->uri_.GetHost()),
                          task->owner_device_->uri_.GetPort())) {
    task->state_ = HttpState::Disposing;
    return;
  }
...
void UPnP::Heartbeat_Task_Requesting(UPnPTask* task) {
  ...
  if (task->socket_->GetSendOverlappedResult(false, overlapped_result)) {
    ...
    if (overlapped_result.completed_length < 0) {  // error?
      ...
      const int32 length =
          task->http_request_.Len() - task->http_request_issue_offset_ + 1;
      if (length <= 0) {
        task->recv_issued_ = true;

        if (task->socket_->IssueRecv(HTTP_RESPONSE_BUFFER_LENGTH) !=
            SocketErrorCode::Ok) {
...
void UPnP::Heartbeat_Task_ResponseWaiting(UPnPTask* task) {
  ...
  if (task->socket_->GetRecvOverlappedResult(false, overlapped_result)) {
    ...
    if (overlapped_result.completed_length <= 0) {  // error?
      ...
      task->http_response_buffer_.Append(task->socket_->GetRecvBufferPtr(),
                                         overlapped_result.completed_length);
      task->http_response_issue_offset_ += overlapped_result.completed_length;

      const int32 length =
          HTTP_RESPONSE_BUFFER_LENGTH - task->http_response_issue_offset_;
      if (length <= 0) {  // done?
        task->state_ = HttpState::ParsingHttpResponse;
      } else {
        task->recv_issued_ = true;
...
void UPnP::Heartbeat_Task_ParsingHttpResponse(UPnPTask* task) {
  task->http_response_ =
      ByteArray((const char*)task->http_response_buffer_.GetData(),
                task->http_response_buffer_.Count());
  task->state_ = HttpState::Disposing;
}

ByteArray UPnP::GetPortMappingRandomDesc() {
  // 문자열이 너무 길게되면 일부 공유기에서 'bad request' 오류가 나오게 되므로
  // 가능한 짧게 유지해야합니다.
...
void UPnP::DeleteTcpPortMapping(const InetAddress& lan_addr,
  ...
  // 공유기의 task queue에, TCP연결-송신-수신-완료 과정을 수행할 첫 issue를
  // 건다. 건 이슈는 netClient manager worker thread에 의해서 issue connect가 될
  // 것이다. 이를 위한 heartbeat도 이미 작동하고 있다.
  UPnPTaskPtr task(new UPnPTask);
  task->owner_upnp_ = this;
...
bool HttpSession::IssueRequestSend() {
  ...
  if (socket_->IssueSend((const uint8*)*http_request_ + offset, length) !=
      SocketErrorCode::Ok) {
    send_issued_ = false;
    return false;
  } else {
...
void UPnPTask::BuildUPnPRequestText_AddPortMapping() {
  ...
  soap_cmd_token[10] =
      ...
      "dt:dt=\"ui4\">0</NewLeaseDuration></m:AddPortMapping></SOAP-ENV:Body></"
      "SOAP-ENV:Envelope>";
  ...
  ByteArray header = ByteArray::Format(
      ...
      "Content-length: %d\r\n"
      "Host: %s:%d\r\n\r\n",
      *device->control_url_, soap_cmd.Len(), *device->uri_.GetHost(),
      device->uri_.GetPort());
...
void UPnPTask::BuildUPnPRequestText_DeletePortMapping() {
  ...
  soap_cmd_token[0] =
      ...
      "xmlns:SOAP-ENV=\"http://schemas.xmlsoap.org/Soap/envelope/\" "
      "SOAP-ENV:encodingStyle=\"http://schemas.xmlsoap.org/Soap/encoding/\">"
      "<SOAP-ENV:Body><m:DeletePortMapping "
      "xmlns:m=\"urn:schemas-upnp-org:service:WANIPConnection:1\">"
      "<NewRemoteHost xmlns:dt=\"urn:schemas-microsoft-com:datatypes\" "
  ...
  soap_cmd_token[3] = is_tcp ? "TCP" : "UDP";
  soap_cmd_token[4] =
      "</NewProtocol></m:DeletePortMapping></SOAP-ENV:Body></"
      "SOAP-ENV:Envelope>";
