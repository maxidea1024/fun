...
//typedef TUnsafeFastPtr<struct CompressedFrameNumbers> CompressedFrameNumbersPtr;
...
class RUdpFrame {
 public:
  ...
  //TODO 처리되기전까지는 유지되어야하므로, raw -> bytestring으로 처리하면 안되므로
  //sharedbytestring을 하나 만들어서 유지시켜주어야함.  이름을 뭘로할까??
  //CByteStringRef에서 참조를 유지하는 형태로 처리하는건 어떨런지??
