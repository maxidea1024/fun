...
ResultInfo::ResultInfo() : last_received_msg() {
  result_code = ResultCode::Ok;  // unexpected로 바꾸지 말것! 생성자 값을 그대로
                                 // 쓰는데가 여기저기 있으니!
...
SharedPtr<ResultInfo> ResultInfo::FromSocketError(
...
SharedPtr<ResultInfo> ResultInfo::From(ResultCode result_code, HostId remote,
                                       const String& comment,
                                       const ByteArray& last_received_msg) {
  SharedPtr<ResultInfo> ret(new ResultInfo());
  ret->result_code = result_code;
  ret->remote = remote;
  ret->comment = comment;
...
String ResultInfo::ToString() const {
  ...
  result =
      String::Format("result: (%s", (const char*)TypeToString(result_code));

  if (result_code != detail_code && detail_code != ResultCode::Ok) {
    result << String::Format("), detail: (%s)",
                             (const char*)TypeToString(detail_code));
    ;
  }
...
const char* ResultInfo::TypeToString(ResultCode result_code) {
  switch (result_code) {
    ...
    case ResultCode::ServerPortListenFailure:
      return "server socket listen failure.  make sure that the TCP or UDP "
             "listening port is not already in use.";
    ...
    case ResultCode::LocalSocketCreationFailed:
      return "unable to start listening of client socket.  must check if "
             "either TCP or UDP socket is already in use.";
    case ResultCode::NoneAvailableInPortPool:
      return "failed binding to local port that defined in port pool.  please "
             "check number of values in port pool are sufficient.";
    case ResultCode::InvalidPortPool:
      return "range of user defined port is wrong.  set port to 0(random port "
             "binding) or check if it is overlapped.";
  }
  return "<undefined>";
}
