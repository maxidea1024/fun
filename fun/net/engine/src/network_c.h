...
class NetClientWorker : public ListNode<NetClientWorker> {
 ...
 public:
  ...
  bool ProcessMessage_S2CRoutedMulticast1(UdpSocket_C* udp_socket, ReceivedMessage& received_msg);
  bool ProcessMessage_S2CRoutedMulticast2(UdpSocket_C* udp_socket, ReceivedMessage& received_msg);
  bool ProcessMessage_EngineLayer(UdpSocket_C* udp_socket, ReceivedMessage& received_msg);

  void ProcessMessage_RPC(ReceivedMessage& received_msg, bool& ref_msg_processed);
  void ProcessMessage_FreeformMessage(ReceivedMessage& received_msg, bool& ref_msg_processed);
  ...
  void ProcessMessage_RUdp_Frame(UdpSocket_C* udp_socket,ReceivedMessage& received_msg);
  ...
  void ProcessMessage_UnreliableRelay2(UdpSocket_C* udp_socket, ReceivedMessage& received_msg);
  void ProcessMessage_LingerDataFrame2(UdpSocket_C* udp_socket,ReceivedMessage& received_msg);
  void ProcessMessage_RequestReceiveSpeedAtReceiverSide_NoRelay(UdpSocket_C* udp_socket, ReceivedMessage& received_msg);
  void ProcessMessage_ReplyReceiveSpeedAtReceiverSide_NoRelay(ReceivedMessage& received_msg);

  void TcpRecvCompletionCase(CompletionStatus& completion);
  void TcpSendCompletionCase(CompletionStatus& completion);
  void UdpRecvCompletionCase(UdpSocket_C* udp_socket, CompletionStatus& completion);
  void UdpSendCompletionCase(UdpSocket_C* udp_socket, CompletionStatus& completion);

  bool LoopbackRecvCompletionCase();

  void ProcessEveryMessageOrMoveToFinalRecvQueue(UdpSocket_C* udp_socket);
  void ProcessMessageOrMoveToFinalRecvQueue(UdpSocket_C* udp_socket, ReceivedMessage& received_msg);
};
...
class NetClientManager
 ...
 private:
  ...
  /* 아래 시간 관련 변수들을 보호.
  코드 프로필링 결과 FunNet.NetClientManager.GetCachedAbsoluteTime가 상당한 시간을 먹으므로 잠금 영역 세분화를 한다.
  */
 ...
 public:
  ...
  double GetCachedElapsedTime() const {
    return elapsed_time;
  }
