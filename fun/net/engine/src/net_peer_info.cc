...
String PeerInfo::ToString(bool at_server) {
  if (at_server) {
    return String::Format(
        "{host_id: %d, joined_p2p_group_count: %d, is_behind_nat: %d, "
        "real_udp_enabled: %d, to_remote_peer_send_udp_message_attempt_count: "
        "%u, to_remote_peer_send_udp_message_success_count: %u}",
        (int32)host_id, joined_p2p_groups_.Count(), behind_nat,
        real_udp_enabled, to_remote_peer_send_udp_message_attempt_count,
        to_remote_peer_send_udp_message_success_count);
  } else {
    return String::Format(
        "{host_id: %d, relayed_p2p: %d, joined_p2p_group_count: %d, "
        "is_behind_nat: %d, real_udp_enabled: %d}",
