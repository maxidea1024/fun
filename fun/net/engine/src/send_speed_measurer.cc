...
uint8 SendSpeedMeasurer::g_send_packet[SendSpeedMeasurer::PAKCET_LENGTH] = {
    0,
};
InetAddress SendSpeedMeasurer::g_sendto =
    InetAddress("211.214.161.39", 80);  // TODO: 나중에 제대로 고칠것
...
void SendSpeedMeasurer::Heartbeat() {
  ...
  if (measuring_) {
    ...
    if (measure_heartbeat_count_ == 1) {
      ...
      if ((GetAbsoluteTime() - start_time_) >
              NetConfig::measure_send_speed_duration_sec ||
          force_measure_now_) {
        // 총계를 산정한다.
...
void SendSpeedMeasurer::OnSocketWarning(InternalSocket* socket,
                                        const string& msg) {
  int32 x = 0;
}
...
void SendSpeedMeasurer::ProcessIoCompletion(CompletionStatus& completion) {
  ...
  fun_check(NetConfig::measure_send_speed_duration_sec <
            1.0);  // 너무 짧으면 측정 부정확. 너무 길면 호스트 과부하.
...
void SendSpeedMeasurer::IssueSendTo() {
  ...
  send_issued_ = true;
  if (send_socket_->IssueSendTo(g_send_packet, packet_len_, g_sendto) !=
      SocketErrorCode::Ok) {
    send_issued_ = false;
    measuring_ = false;
  }
...
void SendSpeedMeasurer::RequestStopProcess() { measuring_ = false; }
