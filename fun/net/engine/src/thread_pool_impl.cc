...
void ThreadPoolImpl::Run() {
  ...
  //COM 초기화는 per-thread이므로, CRunnableThread내에서 처리해주는게 바람직해보임.
  //CCoInitializer COI; // Per thread.
  ...
  if (IsGqcsExSupported()) {
    ...
    while (stop_all_threads_ == false) {
      fun_check(NetConfig::wait_completion_timeout_msec < 20); // 20ms보다는 작아야 coalesce가 지나침으로 인한 랙이 예방.
      const bool succeeded = completion_port_->GetQueuedCompletionStatusEx(completions, dequeued_count, NetConfig::wait_completion_timeout_msec);
      if (succeeded && dequeued_count > 0) {
        for (int32 i = 0; i < dequeued_count; ++i) {
          completions[i].key->OnIoCompletion(send_issue_pool, recv_message_list, completions[i]);
        }
      }
    }
  } else {
    while (stop_all_threads_ == false) {
      ...
      if (completion_port_->GetQueuedCompletionStatus(&completion, NetConfig::wait_completion_timeout_msec)) {
        fun_check(NetConfig::wait_completion_timeout_msec < 20); // 20ms보다는 작아야 coalesce가 지나침으로 인한 랙이 예방.
        completion.key->OnIoCompletion(send_issue_pool, recv_message_list, completion);
      }
    }
  }
...
void ThreadPoolImpl::SetCallbacks(IThreadPoolCallbacks* callbacks) {
  if (!thread_pool_worker_.IsEmpty()) { // threadpool이 시작하기 전에만 설정가능. 일단 시작한 후에는 설정이 불가능.
    throw Exception("Already async callback may occur.  Thread-pool start should have not been done before here.");
  }
...
void ThreadPoolImpl::Start(int32 thread_count) {
  ...
  // Worker 스레드 풀 준비
  {
    ...
    // completion_port_ 준비.
    completion_port_.Reset(new CompletionPort(this, true, thread_count)); // AcceptEx때문에 무조건 true
    ...
    for (int32 i = 0; i < thread_count; ++i) {
      ...
      thread_pool_worker_.Add(UniquePtr<RunnableThread>(RunnableThread::Create(this, *thread_name)));
      //thread_pool_worker_.Add(SharedPtr<RunnableThread>(RunnableThread::Create(this, *thread_name)));
    }
  }
...
void ThreadPoolImpl::Stop() {
  ...
  //AssertIsNotLockedByCurrentThread();
 {
    ...
    if (!referers_.IsEmpty()) {
      throw Exception("Referrer exist.  Thread-pool stop should have all referrers clear before here.");
    }
    ...
    //어짜피 RunnableThread 소멸자에서 대기했다가 종료하므로, 아래 코드는 불필요함.
...
void ThreadPoolImpl::GetThreadInfos(Array<ThreadInfo>& out_info_list) {
  ...
  for (int32 i = 0; i < thread_pool_worker_.Count(); ++i) {
    ...
    info.thread_handle = thread_pool_worker_[i]->GetHandle(); //TODO 핸들을 구지 가져가야하나??
    out_info_list.Add(info);
  }
}

bool ThreadPoolImpl::IsActive() {
  return start_flag_;
}

//TODO 사용 목적이 부족하다면, 제거하도록 하자.
void ThreadPoolImpl::OnCompletionPortWarning(CompletionPort* completion_port, const char* msg) {
  // nothing to do... yet?
}
...
void ThreadPoolImpl::PostCompletionStatus(ICompletionKey* key, uintptr_t custom_value) {
