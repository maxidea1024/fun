...
void RUdpSender::SendViaReliableUDP(const uint8* StreamToAdd, int32 Length) {
  ...
  //LOG(LogNetEngine,Warning,"send_stream_ length: %d", send_stream_.GetLength());
}
...
void RUdpSender::Tick(float elapsed_time) {
  ...
  if ((send_speed * RUdpConfig::brake_max_send_speed_threshold) > remote_recv_speed) {
    send_speed_limit_ = (int32)remote_recv_speed + 1;
  } else {
    send_speed_limit_ = RUdpConfig::max_send_speed_in_frame_count;
  }

  // 마지막 stream enque 직후 not on need에 의해 stream to frames가 실행되지 않아있을 수 있다.
  // 따라서 매 짧은 순간마다 아래 메서드의 호출은 필수.
  ConditionalStreamToSenderWindow(false);

  //이 두개 함수의 순서는 바뀌어선 안된다. 초송신 되었던것이, 재송신이 일어나면 안되므로...
...
void RUdpSender::CalcRecentSendSpeed() {
  ...
  if ((absolute_time - recent_send_frame_to_udp_start_time_) > RUdpConfig::calc_recent_receive_interval) {
    const double T = 0.1 / (absolute_time - recent_send_frame_to_udp_start_time_);
    recent_send_frame_to_udp_speed_ = (int32)MathBase::Lerp((double)recent_send_frame_to_udp_speed_, (double)recent_send_frame_to_udp_count_, T);
...
void RUdpSender::ConditionalFirstSenderWindowToUdpSender() {
  ...
  fun_check(RUdpConfig::first_resend_cooltime <= RUdpConfig::max_resend_cooltime);

  const double absolute_time = owner->Delegate->GetAbsoluteTime();

  // 상대측과의 랙이 처음 재송신 조건에 영향을 줌
  //double recent_ping = owner->Delegate->GetRecentPing() * 2; // round trip이므로 *2
  ...
  for (auto it = first_sender_window_.CreateIterator(); it; ++it) {
...
#else
    // 초기 재송신 인터벌을 핑 등 너무 짧은 값을 쓰니 패킷 로스 발생이 조금이라도 나면 트래픽이 확 증가.
    // 따라서 핑이 아니라 제법 긴 값을 그냥 쓰도록 하자.
    frame.resend_cooltime = RUdpConfig::first_resend_cooltime;
#endif
    // 그래도 상한선은 지키도록 하자. 핑 시간이 잘못 측정된 경우를 위해.
    frame.resend_cooltime = MathBase::Min(frame.resend_cooltime, RUdpConfig::max_resend_cooltime);
    ...
    // 만약 일정시간 즉, frame.ResendCoolTime안에 ACK가 오게 되면 정상적으로 수신된걸로
    // 처리하고 ResendWindow에서 해당 프레임을 제거한다.
...
void RUdpSender::ConditionalResendWindowToUdpSender(double elapsed_time) {
  ...
  double fLimitCount = double(resend_window_.Count()) * RUdpConfig::resend_limit_ratio;

  // 초당 재송신 가능한 최대 & 최소 범위로 조절
  fLimitCount = MathBase::Max(fLimitCount, double(RUdpConfig::min_resend_limit_count));
  fLimitCount = MathBase::Min(fLimitCount, double(RUdpConfig::max_resend_limit_count));

  // 초당 재송신 가능한거니까
  fLimitCount *= elapsed_time;

  // 클라가 버벅대고 있는 상황(즉 스레드 기아화)에서는 elapTime이 상당히 길어진다.
  // 이런 상황에서는 재전송할 것들이 계속 누적되는데, 이를 계속 재전송하려 하면 악순한의 고리가 이어진다.
  // 이 루틴은 client_heartbeat_interval_msec 간격으로 콜 되고 있다.
  // 그리고 기아화가 발생하면 elapsed time은 길어질 것이다.
  // 기아화 상황에서는 초당 보내는 양을 감하도록 하자.
  const double ExpectedElapTime = NetConfig::rudp_heartbeat_interval_sec;
  if (elapsed_time > ExpectedElapTime * 5) { // 어느 정도 제법 격차가 차이나야 기아화를 의심할 수 있으므로 이런 조건문을 넣는다.
    const double DecayRatio = elapsed_time / ExpectedElapTime; // 5보다는 클 것임
  ...
  for (auto it = resend_window_.CreateIterator(); it && LimitCount > 0; ++it) {
    ...
    if ((absolute_time - FrameToResend.last_send_time) > FrameToResend.resend_cooltime) { // 재송신 조건을 만족시
      // 최악의 재송신 시도라면, 기록을 남긴다.
      if (FrameToResend.first_send_time > 0) {
        max_resend_elapsed_time_ = MathBase::Max(max_resend_elapsed_time_, absolute_time - FrameToResend.first_send_time);
      }

      // 재송신

      // 재송신 횟수가 실패할수록 재송신을 좀 더 느긋하게 보내도록 한다.
      //FrameToResend.resend_cooltime *= RUdpConfig::enlarge_resend_cooltime_ratio;
      FrameToResend.resend_cooltime = MathBase::Min(FrameToResend.resend_cooltime, RUdpConfig::max_resend_cooltime);
      ...
      // 타 reliable 채널로 보내진거면(TCP 릴레이) resend가 불필요하므로 송신큐에서 제거한다.
...
void RUdpSender::ConditionalStreamToSenderWindow(bool move_now) {
  ...
  if (!move_now) {
    /*
    아직 UDP layer에서 미송출된 데이터가 남아있는 상황(UDP layer filled) 이라면 stream에 있는 것을 sender window로
    보내는 것이 불필요하다.

    아직 미송출 초송신 frame이 남아있는 판국에 stream에 조금이라도 쌓인것을 족족 frame으로 변환해봤자 frame의 payload
    크기만 작은 것이 다수 생길 뿐이니까.

    이때는 차라리 stream이 더 많이 쌓이게 냅둬야 coalesce 효과가 느므로 ok.

    그렇다고 UDP layer filled의 종료를 마냥 기다리면 starvation이 발생하므로 최대 대기 한계 시간을 두도록 한다.
    */
    if ((absolute_time - last_do_stream_to_sender_window_time_) > RUdpConfig::stream_to_sender_window_coalesce_interval
  ...
  if (move_now) {
    ...
    TODO: (나중에 위 루틴을 1초동안 총 보낸 양이 속도 제한을 넘으면 안되게 구현할 것. 즉
    silly window syndrome을 제거할 것) *

    V회사 제보에 의하면 클라20개가 대량 통신시 이게 막히는 경우가 있었다고 한다!
    분석 결과, 클라 20개에서 send_speed_limit_<102까지 떨어진다!
    잘못 측정된 SendSpeedLimit는 없느니만 못하다. 따라서 아래와 같이 불필요 부분을 막았다.
    */
    while (send_stream_.GetLength() > 0
      /*&& MovedToFrameCountTotal < (send_speed_limit_ - CurrentSendQueueTotalInFrameCount)*/) {
      const int32 Length = MathBase::Min(RUdpConfig::frame_length, send_stream_.GetLength());
      ...
      //frame.data = MessageIn(ByteArray((const char*)send_stream_.ConstData(), Length)); // copy
...
FrameNumber RUdpSender::NextFrameNumber() {
  ...
  current_frame_number_ = FrameNumberUtil::NextFrameNumber(current_frame_number_);
  return Next;
}
...
void RUdpSender::SendOneFrame(RUdpFrame& frame, bool bResend) {
  ...
  if (frame.type == RUdpFrameType::Data) {
    //if (bResend) {
    //  LOG(LogNetEngine,Info,"* RUdpSender.SendOneFrame: [RESEND]    type=%s, frame=%d, len=%d", *ToString(frame.type), (int32)frame.frame_number, frame.data.Len());
    //} else {
    //  LOG(LogNetEngine,Info,"* RUdpSender.SendOneFrame: [FRISTSEND] type=%s, frame=%d, len=%d", *ToString(frame.type), (int32)frame.frame_number, frame.data.Len());
    //}
...
bool RUdpSender::RemoveFromSenderWindow(FrameNumber frame_id) {
  return resend_window_.RemoveOneIf(
        [frame_id](const SenderFrame& frame) {
          return frame.frame_number == frame_id;
        });
}

// 주어진 프레임 넘버보다 이전 프레임 모두 제거.
void RUdpSender::RemoveSpecifiedAndItsPastsFromSenderWindow(FrameNumber frame_id) {
