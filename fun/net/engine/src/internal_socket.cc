...
//@maxidea: todo: 소켓 작업 실패시 오류 메시지를 좀더 자세하게 보여줄수 있도록 하자.

//IPV6_V6ONLY 이슈
//윈도우즈에서는 기본값이 0이 아니라함. 즉, IPv4 주소는 허용하지 않는다는... 쩝...
//http://www.minzkn.com/moniwiki/wiki.php/DualStackIPForWindows
...
//NOTE 이렇게 잡아주는게 맞을래나??
//InetAddress::MAX_ADDRESS_LENGTH + 16로 잡아주었더니, 주소가 null이 나오네... 아후..
...
//이게 더 안전할것 같은데...??
//const int32 AddrLengthInTdi = sizeof(SOCKADDR_STORAGE) + 16;
//실제로 sizeof(SOCKADDR_STORAGE)이 InetAddress::MAX_ADDRESS_LENGTH와 같은 값일까???
//SOCKADDR_STORAGE이 표준에 의한 형태인지??
...
void InternalSocket::Restore(bool bIsTcp) {
  ...
  if (bIsTcp) {
    socket_ = WSASocket(SOCKET_ADDR_FAMILY, SOCK_STREAM, 0, nullptr, 0, WSA_FLAG_OVERLAPPED);
  } else {
    socket_ = WSASocket(SOCKET_ADDR_FAMILY, SOCK_DGRAM, 0, nullptr, 0, WSA_FLAG_OVERLAPPED);
  }

  if (socket_ == INVALID_SOCKET) {
    throw Exception(String::Format("socket creation failure: error={0}", WSAGetLastError()));
  }
...
InternalSocket::InternalSocket( SocketType type,
  ...
  switch (type) {
    ...
    case SocketType::Raw:
      socket_ = WSASocket(SOCKET_ADDR_FAMILY, SOCK_RAW, 0, nullptr, 0, WSA_FLAG_OVERLAPPED);
      break;
  ...
  if (socket_ == INVALID_SOCKET) {
    throw Exception(String::Format("socket creation failure: error=%d", WSAGetLastError()));
  }
...
InternalSocket::InternalSocket( SOCKET socket,
                                IInternalSocketDelegate* delegate,
...
void InternalSocket::PostSocketWarning(DWORD error_code, const char* where) {
  if (!InternalSocket::IsPendingErrorCode((SocketErrorCode)error_code)) {
    //if (verbose_) {
      const String text = String::Format("{0}: fails (error_code: {1})", where, error_code);
...
SocketErrorCode InternalSocket::Connect(const String& host, int32 port) {
  try {
    ...
    //TODO Networker_C에서 connect가 여러번 불리는 문제가 있는데... 의도인건가??
    LOG(LogNetEngine,Warning,"%s: connect to %s", __FUNCTION__, *addr.ToString());
    ...
    if (connect(socket_, (const sockaddr*)&native_addr, sizeof(native_addr)) == SOCKET_ERROR) {
...
SocketErrorCode InternalSocket::IssueRecv(int32 length) {
  if (RecvIssued()) {
    // 경고만 낼 뿐 무시 처리는 안한다. 이건 검사 목적일 뿐이고 정확하지 않으니까.
    delegate_->OnSocketWarning(this, "WARNING: IssueRecv is duplicated!");
  }

  if (length == 0) {
    return SocketErrorCode::InvalidArgument; // 0 이하의 수신 자체는 무의미한데다 socket 오동작을 유발하므로
  }
  ...
  DWORD error;
  const int32 rc = WSARecv(socket_, &buf, 1, &recv_byte_count, &recv_flags_, &recv_ovl_, nullptr);
...
SocketErrorCode InternalSocket::IssueRecvFrom(int32 length) {
  ...
  while (true) {
    ...
    const int32 rc = WSARecvFrom( socket_,
                                  &buf,
                                  1,
                                  &out_done,
                                  &recv_flags_,
    ...
    switch (error) {
      ...
      case WSAEMSGSIZE:
        ...
        // 부분이 잘린 패킷을 받았으므로 사용하는 것도 거시기하다. 그냥 버려야 한다.
        // 차피 RZ 자체가 UDP coalesce를 MTU 크기 기반으로 하므로 정상적 상황에서는 이런 오류가 올 일이 없어야 한다.
        // DN에서는 WSA_IO_PENDING 아니면 no completion signal이므로
        // 시도로 이어져야 한다.
      ...
      case WSAECONNRESET:
        ...
        // } else {
        //  String text;
        //  text.Format("건재한 UDP 소켓에서 WSAECONNRESET이 옴. 재시도 횟수={0}\n", retry_count);
...
//데이터를 보내기전에 복사가 일어나는데 구지 이렇게 해야하나 싶은데...  줄일수도 있지 않을까??
SocketErrorCode InternalSocket::IssueSend(const uint8* data, int32 data_len) {
  ...
  if (data_len <= 0) { // 0 이하의 수신 자체는 무의미한데다 socket 오동작을 유발하므로
    return SocketErrorCode::InvalidArgument;
  }

  //@todo 여기서 카피를 꼭해야하나??  상위레벨에서 버퍼를 유지하는게 좋을듯 싶은데...
  ...
  DWORD out_done = 0;
  DWORD flags = 0;
  const int32 rc = WSASend(socket_, &buf, 1, &out_done, flags, &send_ovl_, nullptr);
  if (rc < 0 && (error = WSAGetLastError()) != WSA_IO_PENDING) {
...
//주의:
//FragmentedBuffer 안의 메모리 참조들이 가리키고 있는 메모리들은 전송이 완료되거나, 중단하는 경우까지
//유지하고 있어야함.
//그렇지 않을 경우, 메모리 access violation이 발생함!!
SocketErrorCode InternalSocket::IssueSend_NoCopy(FragmentedBuffer& send_buffer) {
  if (SendIssued()) {
    delegate_->OnSocketWarning(this, "WARNING: IssueSend is duplicated!");
  }
  ...
  DWORD out_done = 0;
  DWORD flags = 0;
  const int32 rc = WSASend( socket_,
                            send_buffer_.buffer_.ConstData(),
                            send_buffer_.buffer_.Count(),
                            &out_done,
                            flags,
                            &send_ovl_,
                            nullptr);
  if (rc < 0 && (error = WSAGetLastError()) != WSA_IO_PENDING) {
    ...
    io_pending_count_++;
    //delegate_->OnSocketWarning(this, String::Format("iopendingcount:{0}", io_pending_count_));
  }
...
SocketErrorCode InternalSocket::IssueSendTo_NoCopy_TempTtl(
  ...
  if (ttl >= 0) {
    ...
    int32 OrigTtl;
    if (GetTtl(OrigTtl) == SocketErrorCode::Ok && SetTTL(ttl) == SocketErrorCode::Ok) {
  ...
  DWORD out_done = 0;
  DWORD flags = 0;
  const int32 rc = WSASendTo( socket_,
                              send_buffer_.buffer_.ConstData(),
                              send_buffer_.buffer_.Count(),
                              &out_done,
                              flags,
...
SocketErrorCode InternalSocket::BlockedSendTo(const uint8* data,
                                              int32 length,
  ...
  return (SocketErrorCode)sendto( socket_,
                                  (const char*)data,
                                  length,
                                  0,
                                  (const sockaddr*)&sa,
...
SocketErrorCode InternalSocket::BlockedRecvFrom(uint8* data,
                                                int32 length,
  ...
  //TODO 오류처리를 해야하나??
  const SocketErrorCode rc = (SocketErrorCode)recvfrom( socket_,
                                                        (char*)data,
                                                        length,
                                                        0,
                                                        (sockaddr*)&sa,
...
SocketErrorCode InternalSocket::IssueSendTo(const uint8* data,
                                            int32 length,
                                            const InetAddress& sendto_addr) {
  if (SendIssued()) {
    // 경고만 낼 뿐 무시 처리는 안한다. 이건 검사 목적일 뿐이고 정확하지 않으니까.
...
// 만약 완료 성공 또는 소켓 에러 등의 실패가 생기면 객체를 리턴하되 ErrorCode가 채워져 있다.
...
bool InternalSocket::GetRecvOverlappedResult(
  ...
  */
  ...
  DWORD done = 0;
  DWORD flags = 0;
  const BOOL result = WSAGetOverlappedResult( socket_,
                                              &recv_ovl_,
                                              &done,
                                              wait_until_complete,
                                              &flags);
...
bool InternalSocket::GetAcceptExOverlappedResult(
  ...
  //fun_check(!(result && WSAGetLastError() == WSA_IO_INCOMPLETE)); // 이런 경우가 없어야 하는데 테스트 로비 클라에서 이런 경우가 있는듯
...
void InternalSocket::Listen() {
  ...
  {
    BOOL on = TRUE;
    setsockopt(socket_, SOL_SOCKET, SO_CONDITIONAL_ACCEPT, (char*)&on, sizeof(on));
  }
...
int32 InternalSocket::SetSendBufferSize(int32 size) {
  int32 value = size;
  const int32 rc = setsockopt(socket_, SOL_SOCKET, SO_SNDBUF, (const char*)&value, sizeof(value));
  if (rc != 0) {
    PostSocketWarning(WSAGetLastError(), __FUNCTION__);
  }
...
int32 InternalSocket::SetRecvBufferSize(int32 size) {
  int32 value = size;
  const int32 rc = setsockopt(socket_, SOL_SOCKET, SO_RCVBUF, (const char*)&value, sizeof(value));
  if (rc != 0) {
    PostSocketWarning(WSAGetLastError(), __FUNCTION__);
  }
...
void InternalSocket::CloseSocketHandleOnly() {
  ...
  socket_closed_guard.UnsafeLock(); // 드물지만 파괴자에서 잘못된 CS를 접근하는 경우 ShowUserMisuseError는 쐣
  ...
  if (!socket_closed_or_closing_mutex_protected_) {
    ...
    // socket을 닫으면 Win32는 IOCP와의 연계도 없앤다. 따라서 이 성향을 따라가도록 한다.
    if (associated_iocp_) {
      // fake IOCP에 연계중이고 issue가 된 상태이면 completion with error를 흉내낸다.
      if (!associated_iocp_->RealIocpEnabled()) {
        if (RecvIssued()) {
          recv_ovl_.Internal = STATUS_ABANDONED_WAIT_0; // 하여튼 STATUS_PENDING만 아니면 됨
          associated_iocp_->PostSocketCloseEvent_INTERNAL(this, CompletionType::Receive);
        }

        if (SendIssued()) {
          send_ovl_.Internal = STATUS_ABANDONED_WAIT_0; // 하여튼 STATUS_PENDING만 아니면 됨
          associated_iocp_->PostSocketCloseEvent_INTERNAL(this, CompletionType::Send);
        }
      }
...
bool InternalSocket::GetVerboseFlag() {
  return verbose_;
}

void InternalSocket::SetVerboseFlag(bool flag) {
  verbose_ = flag;
}
...
SocketErrorCode InternalSocket::AcceptEx(InternalSocket* exisiting_socket)
{
  ...
  const BOOL result = lpfnAcceptEx(
      socket_,                          // SOCKET       sListenSocket
  ...
  if (!result) {
    ...
    if (ignore_not_socket_error_ && (SocketErrorCode)error == SocketErrorCode::NotSocket) {
      return SocketErrorCode::Ok;
    }
...
SocketErrorCode InternalSocket::ConnectEx(const String& host, int32 port) {
  ////TODO InetAddress 클래스 자체에 resolving 기능이 추가되어 있으므로, 아래 코드는 불필요함.
  ////만약, InetAddress 주소가 잘못된 경우에는 예외가 던져지는 형태인데 이걸 어떤 형태로 핸들링하는게 좋으려나??
    //
...
SocketErrorCode InternalSocket::ConnectEx(const InetAddress& addr) {
  ...
  if (!IsBoundSocket()) {
    delegate_->OnSocketWarning(this, "WARNING: ConnectEX socket Not Bind! Auto Bind!");
  ...
  const BOOL result = lpfnConnectEx(socket_,
                                    (struct sockaddr*)&sa,
                                    sizeof(sa),
                                    nullptr,
                                    0,
  ...
  if (!result) {
    ...
    if (error != WSA_IO_PENDING) {
      return (SocketErrorCode)error; //@maxidea: 정확히 WSAGetLastError()의 반환값과 ESocketError이 1:1 대응할까??
    }
  }
...
SocketErrorCode InternalSocket::ConnectExComplete() {
  ...
  if (setsockopt(socket_, SOL_SOCKET, SO_UPDATE_CONNECT_CONTEXT, nullptr, 0) < 0) {
...
bool InternalSocket::GetConnectExOverlappedResult(
  ...
  //fun_check(!(b && WSAGetLastError()==WSA_IO_INCOMPLETE)); // 이런 경우가 없어야 하는데 테스트 로비 클라에서 이런 경우가 있는듯
  ...
  //recvfrom_addr_
  out_overlapped_result.received_from = InetAddress(recvfrom_addr_); // 불필요할듯
...
void InternalSocket::FinalizeAcceptEx(InternalSocket* tcp_listening_socket,
  ...
  if (setsockopt(socket_, SOL_SOCKET, SO_UPDATE_ACCEPT_CONTEXT,
        (char*)&tcp_listening_socket_->socket_, sizeof(tcp_listening_socket_->socket_)) == SOCKET_ERROR) {
    // sink error.
    WSAGetLastError();
  }
  ...
  GetAcceptExSockaddrs(
      tcp_listening_socket_->accept_ex_buffer_.MutableData(), // PVOID       lpOutputBuffer
      0,                      // DWORD       dwReceiveDataLength
      AddrLengthInTdi,        // DWORD       dwLocalAddressLength
      AddrLengthInTdi,        // DWORD       dwRemoteAddressLength
  ...
  // 이걸로 하자. GetAcceptExSockaddrs가 잘 작동 안하지만, 속도 희생해서라도, 어쩔 수 없다.
  // GetAcceptExSockaddrs가 제대로 작동을 안하는건가??
  // -> 인자 타입이 잘못 넘어갔었음.. 값이 아닌 포인터로 넘겨야 하는데, 주소를 받아오는곳에 값으로 넘겼었더라는...

  //NOTE 만약, 주소가 제대로 안나온다면 아래 두 라인을 다시 활성화 해야함..

  //가끔씩 주소가 제대로 인식이 안되는데,.. 어떤 상황인지??
  //접속을 받는 과정중에, 원격지에서 접속을 끊을 경우 주소를 얻지 못하는 경우도 있음.
  //그러므로, 여기에 assertion을 걸 필요는 없어보임.
...
void InternalSocket::DisableUdpConnResetReport() {
  ...
  const DWORD status = WSAIoctl(socket_,
                                SIO_UDP_CONNRESET,
                                &new_behaviour,
...
uint8* InternalSocket::GetRecvBufferPtr() {
  return recv_buffer_.MutableData();
}
...
SocketErrorCode InternalSocket::EnableBroadcastOption(bool flag) {
  ...
  const int32 rc = setsockopt(socket_, SOL_SOCKET, SO_BROADCAST, (const char*)&value, sizeof(value));
  if (rc != 0) {
...
SocketErrorCode InternalSocket::SetTTL(int32 ttl) {
  ...
  int32 rc;
  if ((rc = getsockopt(socket_, IPPROTO_IP, IP_TTL, (char*)&value, &value_len)) != 0) {
    delegate_->OnSocketWarning(this, "This socket doesn't support ttl change!");
    return (SocketErrorCode)rc;
  } else {
    value = ttl;
    if ((rc = setsockopt(socket_, IPPROTO_IP, IP_TTL, (const char*)&value, value_len)) != 0) {
...
bool InternalSocket::IsUdpStopErrorCode(SocketErrorCode socket_error) {
  // 주의: SocketErrorCode::ConnectResetByRemote 는 ICMP connreset에서 오는건데 이건 계속 UDP 통신을 할 수 있는 조건이므로 false이어야 함!
  return socket_error == SocketErrorCode::NotSocket || socket_error == SocketErrorCode::OperationAborted;
}
...
bool InternalSocket::IsPendingErrorCode(SocketErrorCode socket_error) {
  return socket_error == SocketErrorCode::WouldBlock || socket_error == SocketErrorCode::IoPending;
}

SocketErrorCode InternalSocket::AllowPacketFragmentation(bool allow) {
  int32 value = allow ? 0 : 1;

  if (setsockopt(socket_, IPPROTO_IP, IP_DONTFRAGMENT, (char*)&value, sizeof(value)) != 0) {
    return (SocketErrorCode)WSAGetLastError();
  } else {
    return SocketErrorCode::Ok;
  }
...
void InternalSocket::EnableNagleAlgorithm(bool enable) {
  ...
  const int32 rc = setsockopt(socket_, IPPROTO_TCP, TCP_NODELAY, (char*)&value, sizeof(value));
  if (rc == SOCKET_ERROR) {
    //LOG(LogNetEngine, Warning, "%s: set tcp-nagle option failure: error=%d", __FILE__, WSAGetLastError());
    PostSocketWarning(WSAGetLastError(), __FUNCTION__);
  }
}

//TODO 실질적으로 사용안되고 있음. BlockedSendTo 함수에서 사용하고는 있으나, 정작 이 함수 자체가 사용안되고 있음.
//상황 봐서 제거하던지, 소켓 유틸쪽으로 빼주던지 하자.
//UDP로 255.255.255.255.65535로 쏘면 블럭이 되는 현상때문에 그러한데... 흠..
//브로드 캐스팅 할 경우에는 239.255.255.250으로 해야하나?? UPnP처럼??
bool InternalSocket::EnsureUnicastEndpoint(const InetAddress& sendto_addr) {
  //if (sendto_addr.BinaryAddress == 0xFFFFFFFF) { // OS에 의해 차단되지 않으려면 필수
  //  delegate_->OnSocketWarning(this, "Sending to 255.255.255.255 is not permitted!");
  //  return false;
  //} else if (sendto_addr.Port == 0xFFFF || sendto_addr.Port == 0) { // OS에 의해 차단되지 않으려면 필수
  //  delegate_->OnSocketWarning(this, "Sending to prohibited port is not permitted!");
  //  return false;
  //} else if (sendto_addr.BinaryAddress == 0) { // OS에 의해 차단되지 않으려면 필수
  //  delegate_->OnSocketWarning(this, "Sending to 0.0.0.0 is not permitted!");
  //  return false;
  //}

  // NOTE: 255.255.255.255:65535로 쏘면 Win32는 막아버린다. 굳이 보내려면 239.255.255.250으로 브로드캐스트하덩가.
  //TODO 일단 막고, 정리하면서 검토.
  if (!(sendto_addr.IsUnicast() || sendto_addr.IsMulticast())) {
    delegate_->OnSocketWarning(this, String::Format("{0} endpoint is not permitted.", *sendto_addr.ToString()));
    return false;
  }
...
int32 InternalSocket::GetSendBufferSize(int32& out_size) {
  ...
  const int32 rc = getsockopt(socket_, SOL_SOCKET, SO_SNDBUF, (char*)&out_size, &out_size_len);
  if (rc != 0) {
    PostSocketWarning(WSAGetLastError(), __FUNCTION__);
  }
...
int32 InternalSocket::GetRecvBufferSize(int32& out_size) {
  int32 out_size_len = sizeof(out_size);
  const int32 rc = getsockopt(socket_, SOL_SOCKET, SO_RCVBUF, (char*)&out_size, &out_size_len);
  if (rc != 0) {
    PostSocketWarning(WSAGetLastError(), __FUNCTION__);
  }
...
SocketErrorCode InternalSocket::GetTtl(int32& out_ttl) {
  ...
  int32 rc;
  if ((rc = getsockopt(socket_, IPPROTO_IP, IP_TTL, (char*)&out_ttl, &out_ttl_len)) != 0) {
    delegate_->OnSocketWarning(this, "This socket doesn't support ttl change!");
  }
...
void InternalSocket::OnIoCompletion(Array<IHostObject*>& send_issued_pool,
  ...
  io_completion_delegate_->OnSocketIoCompletion(send_issued_pool, msg_list, completion);
}

void InternalSocket::SetIPv6Only(SOCKET socket, bool flag) {
#if !FUN_DISABLE_IPV6
  int32 value = flag ? 1 : 0;
  const int32 rc = setsockopt(socket_,
                              IPPROTO_IPV6,
                              IPV6_V6ONLY,
                              reinterpret_cast<const char*>(&value),
                              sizeof(value));
  if (rc == -1) {
    //delegate_->OnSocketWarning(this, "This socket doesn't support IPV6_V6ONLY change!");
  }
#endif
}

void InternalSocket::SetIPv6Only(bool flag) {
  SetIPv6Only(socket_, flag);
}
...
void InternalSocketSelectContext::Wait(uint32 timeout_msec) {
  if (timeout_msec != INFINITE) {
    ...
    select(0, &read_socket_list_, &write_socket_list_, &error_socket_list_, &tv);
  } else {
    // Infinite
    select(0, &read_socket_list_, &write_socket_list_, &error_socket_list_, nullptr);
  }
}

bool InternalSocketSelectContext::GetConnectResult(InternalSocket& socket, SocketErrorCode& out_code) {
