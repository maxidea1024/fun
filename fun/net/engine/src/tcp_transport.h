...
// 송신 속도 조절기도 포함됨
//
// NOTE 결과적으로 송싱속도 제어기는 사용하지 않는다는 얘기임. 그러므로,
// 제거해야한다는 얘기가 됨. SendBrake, AllowedMaxSendSpeed, RecentSpeedMeasurer
// 자체를 제거하는건 어떨런지? 아울러 TcpSendQueue 자체가 필요한가? coalescing을
// 위해서 잔존시켜야할까??
class TcpSendQueue {
 public:
  TcpSendQueue();
 ...
 public:
  // TCP 자체가 ack에 의한 sender window slide 기능이 존재->송신량 자연
  // 조절->따라서 불필요함.
