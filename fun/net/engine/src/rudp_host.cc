...
StreamQueue* RUdpHost::GetReceivedStream() { return &receiver_.recv_stream_; }
...
void RUdpHost::GetStats(RUdpHostStats& out_stats) {
  ...
  out_stats.total_received_stream_length =
      receiver_.total_received_stream_length_;
  ...
  out_stats.last_received_data_frame_number =
      receiver_.last_received_data_frame_number_;
  ...
  out_stats.recent_send_frame_to_udp_speed =
      sender_.recent_send_frame_to_udp_speed_;
  out_stats.send_speed_limit = sender_.send_speed_limit_;
  out_stats.total_receive_data_count =
      receiver_.total_receive_data_frame_count_;

  if (!sender_.first_sender_window_.IsEmpty()) {
    out_stats.first_sender_window_last_frame =
        sender_.first_sender_window_.Back().frame_number;
  } else {
    out_stats.first_sender_window_last_frame = (FrameNumber)0;
  }

  if (!sender_.resend_window_.IsEmpty()) {
    out_stats.resend_window_last_frame =
        sender_.resend_window_.Back().frame_number;
  } else {
    out_stats.resend_window_last_frame = (FrameNumber)0;
  }

  out_stats.last_expected_frame_number_at_sender =
      sender_.last_expected_frame_number_at_sender_;
}
...
//
// (이 함수는 주의해서 사용해야 한다. 잘못 쓰면 reliable UDP 통신을 끊어버리는
// 원인이 된다.)
//
