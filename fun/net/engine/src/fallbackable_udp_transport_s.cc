...
void FallbackableUdpTransport_S::SetUdpAddrFromHere(const InetAddress& addr) {
  if (udp_addr_from_here_ != addr) {
    ...
    owner_->owner_->udp_addr_to_remote_client_index_.Remove(udp_addr_from_here_);

    udp_addr_from_here_ = addr;

    // Insert into new collection.
    if (udp_addr_from_here_ != InetAddress::None) {
      owner_->owner_->udp_addr_to_remote_client_index_.Add(udp_addr_from_here_, owner_);
    }
  }
}
...
void FallbackableUdpTransport_S::SendWhenReady( HostId remote_id,
  ...
  if (real_udp_enabled_) {
    ...
    udp_socket_->SendWhenReady(remote_id, filter_tag, GetUdpAddrFromHere(), data, send_opt);
  } else { // Fallback mode
    // 서버와의 홀펀칭이 안되었을 경우에는 대안으로 TCP를 통해서 보내도록 한다.
    CScopedLock2 tcp_send_queue_guard(owner_->to_client_tcp_->GetSendQueueMutex());
