...
RemoteClient_S::RemoteClient_S( NetServerImpl* owner,
  ...
  max_direct_p2p_connection_count_ = NetConfig::default_max_direct_p2p_multicast_count;
...
ResultCode RemoteClient_S::ExtractMessagesFromTcpStream(ReceivedMessageList& out_result) {
...
void RemoteClient_S::GetClientInfo(NetClientInfo& out_info) {
  ...
  out_info.udp_addr_from_server = to_client_udp_fallbackable_.GetUdpAddrFromHere();
  ...
  out_info.to_server_send_udp_message_attempt_count = to_server_send_udp_message_attempt_count_;
  out_info.to_server_send_udp_message_success_count = to_server_send_udp_message_success_count_;
}

bool RemoteClient_S::IsBehindNAT() {
  return to_client_udp_fallbackable_.GetUdpAddrFromHere().GetHost() != to_client_udp_fallbackable_.udp_addr_internal_.GetHost();
}
...
RemoteClient_S::~RemoteClient_S() {
  ...
  if (owned_udp_socket_) {
    fun_check(owner_->local_addr_to_udp_socket_map_.Contains(owned_udp_socket_->cached_local_addr_));
    owner_->local_addr_to_udp_socket_map_.Remove(owned_udp_socket_->cached_local_addr_);
...
void RemoteClient_S::DetectSpeedHack() {
  ...
  if (speed_hack_detector_) {
    ...
    if (speed_hack_detector_->first_ping_recv_time_ == 0) {
      ...
      const double reckness = MathBase::Clamp(owner_->speed_hack_detector_reck_ratio_, 0.2, 0.9);

      speed_hack_detector_->recent_ping_interval_ = MathBase::Lerp(current_ping - last_ping_, speed_hack_detector_->recent_ping_interval_, reckness);

      // 클라 생성 후 충분한 시간이 지났으며

      // 현재 스핵은 주기가 짧아지는 경우만 체크한다. 주기가 긴 경우는 TCP fallback의 의심 상황일 수 있으므로 넘어간다.
      if ((current_ping - speed_hack_detector_->first_ping_recv_time_) > NetConfig::speedhack_detector_ping_interval_sec * 10) {
        if (speed_hack_detector_->recent_ping_interval_ < NetConfig::speedhack_detector_ping_interval_sec * 0.7) {
...
void RemoteClient_S::WarnTooShortDisposal(const char* where) {
  ...
  if (dispose_caller_ == nullptr) {
    ...
    if ((owner_->GetAbsoluteTime() - created_time_) < 0.3) { // TODO 0.3초는 NetConfig 쪽으로 빼주도록 하자.
      if (owner_->intra_logger_) {
        const String text = String::Format("WARNING: As soon as the client (remote: %d) comes in, it is banned immediately.  Is it intent or bug ? : %s", (int32)host_id_, where);
...
double RemoteClient_S::GetP2PGroupTotalRecentPing(HostId group_id) {
  ...
  if (auto group = owner_->GetP2PGroupByHostId_NOLOCK(group_id)) {
    ...
    for (const auto& member_pair : group->members_) {
      const double ping = owner_->GetRecentPingSec(member_pair.key); // Touch JIT P2P
      if (ping >= 0) {
        count++;
        sum += ping;
      }
...
bool RemoteClient_S::IsDispose() {
  return dispose_waiter_.IsValid();
}

double RemoteClient_S::GetAbsoluteTime() {
  return owner_->GetAbsoluteTime();
}

void RemoteClient_S::IssueDispose(
      ResultCode result_code,
      ResultCode detail_code,
      const ByteArray& comment,
      const char* where,
      SocketErrorCode socket_error) {
  owner_->IssueDisposeRemoteClient(this, result_code, detail_code, comment, where, socket_error);
}
...
void RemoteClient_S::Decrease() {
  DecreaseUseCount();
}

void RemoteClient_S::OnIssueSendFail(const char* where, SocketErrorCode socket_error) {
  IssueDispose(ResultCode::DisconnectFromRemote, ResultCode::TCPConnectFailure, ByteArray(), where, socket_error);
...
bool RemoteClient_S::IsLockedByCurrentThread() {
  return to_client_tcp_->IsLockedByCurrentThread() || to_client_tcp_->IsSendQueueLockedByCurrentThread();
}

void RemoteClient_S::SendWhenReady(
      HostId sender_id,
