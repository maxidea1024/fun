...
#define ASSERT_OR_HACKED(X)                                 \
  {                                                         \
    if (!(X)) {                                             \
      EnqueueHackSuspectEvent(rc, #X, HackType::PacketRig); \
    }                                                       \
  }
...
IMPLEMENT_RPCSTUB_NetC2S_P2PGroup_MemberJoin_Ack(NetServerImpl::C2SStub) {
  ...
  if (auto group = owner_->GetP2PGroupByHostId_NOLOCK(group_id)) {
    ...
    if (group->add_member_ack_waiters.RemoveEqualItem(
            rpc_recvfrom, added_member_id, event_id) == true &&
        group->add_member_ack_waiters.AckWaitingItemExists(added_member_id,
                                                           event_id) == false) {
  ...
  // 두 피어를 연결 정보를 얻는다.
  auto state =
      owner_->p2p_connection_pair_list_.GetPair(rpc_recvfrom, added_member_id);
  if (!state) {
    // 뒷북으로 오는 경우가 있으므로 warning를 띄우지 말자...
    // if (added_member_id != HostId_Server && added_member_id != remote) {
    //  // 엄청난 랙이 걸린 경우라면 용서되지만 그게 아닌 이상 이런 일은 없어야
    //  한다. owner_->EnqueueWarning(ResultInfo::From(ResultCode::RUdpFailed,
    //  remote, "경고:대응하는 P2PConnectionState가 없습니다."));
    //}
  ...
  if (state->member_join_acked_start_) {
    ...
    if (state->MemberJoinAckedAllComplete()) {
      if (state->release_time_ != 0 &&
          state->GetServerHolepunchOkCount() == 2) {
        // absolute_time that you have completed all the steps, let me start
        // reusing it.

        owner_->s2c_proxy_.P2PRecycleComplete(
            rpc_recvfrom, GReliableSend_INTERNAL, added_member_id, true,
        ...
        owner_->s2c_proxy_.P2PRecycleComplete(
            added_member_id, GReliableSend_INTERNAL, rpc_recvfrom, true,
        ...
        // Since the process did not succeed, initialize the state and tell it
        // to start from scratch.

        owner_->s2c_proxy_.P2PRecycleComplete(
            rpc_recvfrom, GReliableSend_INTERNAL, added_member_id, false,
            InetAddress::None, InetAddress::None, InetAddress::None,
            InetAddress::None);

        owner_->s2c_proxy_.P2PRecycleComplete(
            added_member_id, GReliableSend_INTERNAL, rpc_recvfrom, false,
            InetAddress::None, InetAddress::None, InetAddress::None,
            InetAddress::None);
...
IMPLEMENT_RPCSTUB_NetC2S_NotifyP2PHolepunchSuccess(NetServerImpl::C2SStub) {
  ...
  if (auto state = owner_->p2p_connection_pair_list_.GetPair(a, b)) {
    ...
    if (state->GetRelayed()) {  // switch from Relayed To Direct P2P mode..
      ...
      owner_->s2c_proxy_.NotifyDirectP2PEstablish(a, send_opt, a, b,
                                                  a2b_send_addr, a2b_recv_addr,
                                                  b2a_send_addr, b2a_recv_addr);
      owner_->s2c_proxy_.NotifyDirectP2PEstablish(b, send_opt, a, b,
                                                  a2b_send_addr, a2b_recv_addr,
                                                  b2a_send_addr, b2a_recv_addr);

      if (owner_->intra_logger_) {
        const String text = String::Format(
            "Holepunching between client %d and %d is completed successfully.  "
            "a2b_send_addr: %s, a2b_recv_addr: %s, b2a_send_addr: %s, "
            "b2a_recv_addr: %s",
            (int32)a, (int32)b, *a2b_send_addr.ToString(),
            *a2b_recv_addr.ToString(), *b2a_send_addr.ToString(),
            *b2a_recv_addr.ToString());
        owner_->intra_logger_->WriteLine(LogCategory::PP2P, *text);
      }
    }
  } else {
    // FUN_TRACE("경고: P2P 연결 정보가 존재하지 않으면서 홀펀칭이
    // 성공했다고라고라?\n");
  }
...
IMPLEMENT_RPCSTUB_NetC2S_P2P_NotifyDirectP2PDisconnected(
    NetServerImpl::C2SStub) {
  CScopedLock2 main_guard(owner_->GetMutex());
  owner_->CheckCriticalSectionDeadLock(__FUNCTION__);

  // Check validation
  if (auto rc = owner_->GetAuthedClientByHostId_NOLOCK(peer_id)) {
    // Update the connection status information between two clients stored in
    // the server.
    auto state =
        owner_->p2p_connection_pair_list_.GetPair(rpc_recvfrom, peer_id);
    if (state && !state->GetRelayed()) {
      ...
      // Send ack.
      owner_->s2c_proxy_.P2P_NotifyDirectP2PDisconnected2(
          peer_id, GReliableSend_INTERNAL, rpc_recvfrom, reason);

      if (owner_->intra_logger_) {
        const String text = String::Format(
            "Disconnected P2P UDP holepunching between client %d and %d.",
            (int32)rpc_recvfrom, (int32)peer_id);
        owner_->intra_logger_->WriteLine(LogCategory::PP2P, *text);
      }
    }
  }
...
// TODO PeerBID -> PeerBID 이름에 신경을 써야할듯... IDL쪽 문제인데 고민이
// 필요하다.
IMPLEMENT_RPCSTUB_NetC2S_NotifyPeerUdpSocketRestored(NetServerImpl::C2SStub) {
  ...
  if (auto rc = owner_->GetAuthedClientByHostId_NOLOCK(PeerBID)) {
    // Fully initialize the connection state information between two classes
    // stored in the server. even if it is already specified as Direct P2P.
    if (auto state =
            owner_->p2p_connection_pair_list_.GetPair(rpc_recvfrom, PeerBID)) {
      // Start from the beginning, start with Relayed.
      state->SetRelayed(true);

      // For both clients, reset the P2P connection state completely and let it
      // start as it did when it first started. This is necessary because the
      // socket is different.
      const auto& send_opt = GReliableSend_INTERNAL;
      owner_->s2c_proxy_.RenewP2PConnectionState(PeerBID, send_opt,
                                                 rpc_recvfrom);
      owner_->s2c_proxy_.RenewP2PConnectionState(rpc_recvfrom, send_opt,
                                                 PeerBID);

      if (owner_->intra_logger_) {
        const String text = String::Format(
            "P2P UDP hole punching between client %d and %d is completely "
            "restarted from the beginning.  One UDP socket is being restored",
            (int32)rpc_recvfrom, (int32)PeerBID);
        owner_->intra_logger_->WriteLine(LogCategory::PP2P, *text);
      }
    }
  }
...
IMPLEMENT_RPCSTUB_NetC2S_NotifyJitDirectP2PTriggered(NetServerImpl::C2SStub) {
  ...
  if (auto rc = owner_->GetAuthedClientByHostId_NOLOCK(PeerBID)) {
    ...
    if (auto state =
            owner_->p2p_connection_pair_list_.GetPair(rpc_recvfrom, PeerBID)) {
      if (state->first_client->p2p_connection_pairs_.Count() <
              state->first_client->max_direct_p2p_connection_count_ &&
          state->second_client->p2p_connection_pairs_.Count() <
              state->second_client->max_direct_p2p_connection_count_) {
        // UDP Socket이 생성되어있지 않다면, UDP socket 생성을 요청한다.
        owner_->RemoteClient_NewLocalUdpSocketAndRequestNewRemoteUdpSocket(
            state->first_client);
        owner_->RemoteClient_NewLocalUdpSocketAndRequestNewRemoteUdpSocket(
            state->second_client);

        // (요청했으며 reliable messaging이므로 연이어 아래 요청을 해도
        // 안전하다. 아래 요청을 받는 클라는 이미 UDP socket이 있을테니까.) 양측
        // 클라이언트 모두에게, Direct P2P 시작을 해야하는 조건이 되었으니 서로
        // 홀펀칭을 시작하라는 지령을 한다.
        const auto& send_opt = GReliableSend_INTERNAL;
        owner_->s2c_proxy_.NewDirectP2PConnection(PeerBID, send_opt,
                                                  rpc_recvfrom);
        owner_->s2c_proxy_.NewDirectP2PConnection(rpc_recvfrom, send_opt,
                                                  PeerBID);

        state->jit_direct_p2p_requested = true;

        if (owner_->intra_logger_) {
          // const String text = String::Format("P2P UDP hole punching start
          // condition between client %d and %d has been established.
          // absolute_time it is time to start the hole punching between the two
          // peers.", (int32)rpc_recvfrom, (int32)PeerBID);
          const String text = String::Format(
              "P2P UDP hole punching start condition between client %d and %d "
...
IMPLEMENT_RPCSTUB_NetC2S_NotifyLog(NetServerImpl::C2SStub) {
  ...
  if (owner_->intra_logger_) {
    const String FormattedText =
        String::Format(">> [Client %d] %s", (int32)rpc_recvfrom, *text);
...
IMPLEMENT_RPCSTUB_NetC2S_NotifyLogHolepunchFreqFail(NetServerImpl::C2SStub) {
  ...
  if (owner_->intra_logger_) {
    const String prefix = String::Format(
        ">> [Client %d] pair=%d/%d##CCU=%d##", (int32)rpc_recvfrom,
        stats.p2p_direct_connection_pair_count, stats.p2p_connection_pair_count,
        stats.client_count);
    owner_->intra_logger_->WriteLine(LogCategory::P2P, *(prefix + text));
  }

  // 랭크가 가장 높은것만 지속적으로 유지해줌. (차후에 가장 랭크가 높은것으로
  // 기록된 것을 실제로 저장할 것임)
  RemoteClient_S* rc;
  if (owner_->freq_fail_log_most_rank_ < rank &&
      (rc = owner_->GetAuthedClientByHostId_NOLOCK(rpc_recvfrom))) {
    const String prefix =
        String::Format("[Client %d %s] pair=%d/%d##CCU=%d", (int32)rpc_recvfrom,
                       *rc->to_client_tcp_->remote_addr_.ToString(),
                       stats.p2p_direct_connection_pair_count,
                       stats.p2p_connection_pair_count, stats.client_count);
...
IMPLEMENT_RPCSTUB_NetC2S_NotifyUdpToTcpFallbackByClient(
    NetServerImpl::C2SStub) {
  CScopedLock2 main_guard(owner_->GetMutex());
  owner_->CheckCriticalSectionDeadLock(__FUNCTION__);
...
IMPLEMENT_RPCSTUB_NetC2S_ReliablePing(NetServerImpl::C2SStub) {
  ...
  if (auto rc = owner_->GetAuthedClientByHostId_NOLOCK(rpc_recvfrom)) {
    ...
    rc->last_application_hint_.recent_frame_rate =
        recent_frame_rate;  // update frame-rate
...
// 클라이언트에서 안전하게 접속을 종료하기를 요청받은 경우, 이에 대한 처리를
// 하고 응답을 보내준다.
...
IMPLEMENT_RPCSTUB_NetC2S_ShutdownTcpHandshake(NetServerImpl::C2SStub) {
  ...
  if (auto conn = owner_->GetAuthedClientByHostId_NOLOCK(rpc_recvfrom)) {
    owner_->IssueDisposeRemoteClient(
        rc, ResultCode::DisconnectFromRemote, ResultCode::TCPConnectFailure,
        rc->shutdown_comment_, __FUNCTION__, SocketErrorCode::Ok);
  }
...
settings_.bEnablePingTest이 true인 경우에만 클라이언트가 보고를 하게됨에
주의해야함.
*/
...
IMPLEMENT_RPCSTUB_NetC2S_C2S_RequestCreateUdpSocket(NetServerImpl::C2SStub) {
  ...
  if (auto conn = owner_->GetAuthedClientByHostId_NOLOCK(rpc_recvfrom)) {
    ...
    if (ok) {
      sock_addr = conn->to_client_udp_fallbackable_.udp_socket_
                      ->GetRemoteIdentifiableLocalAddr(conn);
    }

    // End of UDP socket creation by partner request. Your opponent is already
    // there. Therefore, ACK is sent.
    owner_->s2c_proxy_.S2C_CreateUdpSocketAck(
        rpc_recvfrom, GReliableSend_INTERNAL, ok, sock_addr);
...
IMPLEMENT_RPCSTUB_NetC2S_C2S_CreateUdpSocketAck(NetServerImpl::C2SStub) {
  ...
  if (auto rc = owner_->GetAuthedClientByHostId_NOLOCK(rpc_recvfrom)) {
    ...
    if (ok) {
      ...
      // PerClient일때는 OwnedUdpSocket이 있고 static일때는 UdpSockets가
      // 있기때문..

      if (owner_->intra_logger_) {
        const String text = String::Format(
            "Client UDP socket creation failed.  host_id: %d, "
            "udp_socket_ref_count: %d",
            (int32)rc->host_id_,
            rc->to_client_udp_fallbackable_.udp_socket_
...
IMPLEMENT_RPCSTUB_NetC2S_ReportC2CUdpMessageCount(NetServerImpl::C2SStub) {
  ...
  if (rc && peer_id != HostId_None) {
    ...
    rc->to_remote_peer_send_udp_message_attempt_count +=
        udp_message_attempt_count;
    rc->to_remote_peer_send_udp_message_success_count +=
        udp_message_success_count;
    // TODO Loss비율 계산

    for (auto& pair : rc->p2p_connection_pairs_) {
      if (pair->ContainsHostId(peer_id)) {
        pair->to_remote_peer_send_udp_message_attempt_count =
            udp_message_attempt_count;
        pair->to_remote_peer_send_udp_message_success_count =
            udp_message_success_count;
        // TODO Loss비율 계산
      }
    }
...
bool NetServerImpl::Start(const StartServerArgs& InParams,
  ...
  if (net_thread_pool_ || user_thread_pool_) {
    out_error = ResultInfo::From(ResultCode::ServerPortListenFailure,
                                 HostId_None, "Already thread-pool exists.");
    return false;
  }
  ...
  if (args.udp_assign_mode == ServerUdpAssignMode::PerClient &&
      args.udp_ports.Count() > 0 && args.udp_ports.Count() < 1000) {
    LOG(LogNetEngine, Warning,
        "WARNING: You specified too few UDP ports with "
        "ServerUdpAssignMode::PerClient.  This may cause inefficient P2P "
        "communication and unreliable messaging.");
  }
  ...
  if (has_zero_port_num && has_non_zero_port_num) {
    throw Exception(
        "Cannot assign non-zero UDP port number with zero UDP port number.  "
        "All non-zero or all zero must be guaranteed.");
  }
  ...
  for (int32 i = 0; i < dup_ports_check_list.Count() - 1; ++i) {
    if (dup_ports_check_list[i] == dup_ports_check_list[i + 1] &&
        dup_ports_check_list[i] != 0) {
      // 겹친 포트 번호가 있을 경우는 예외
  ...
  // Send coalsecse 시간을 임의로 지정하는 경우에 대한 처리.(테스트 용도 혹은
  // 튜닝용)
  if (args.TunedNetworkerSendInterval_TEST > 0) {
    every_remote_issue_send_on_need_internal_ =
        args.TunedNetworkerSendInterval_TEST;
  } else {
    every_remote_issue_send_on_need_internal_ =
        NetConfig::every_remote_issue_send_on_need_internal_;
  }
  ...
  if (settings_.bEnableNagleAlgorithm) {
    LOG(LogNetEngine, Warning,
        "Setting bEnableNagleAlgorithm to false is RECOMMENDED.  FunNet "
        "affords better anti-silly window syndrome technology than TCP nagle "
        "algorithm.");
  }
  ...
  if (settings_.weak_encrypted_message_key_length !=
          (int32)WeakEncryptionLevel::None &&  // 0
      settings_.weak_encrypted_message_key_length !=
          (int32)WeakEncryptionLevel::Low &&  // 512
      settings_.weak_encrypted_message_key_length !=
          (int32)WeakEncryptionLevel::Middle &&  // 1024
      settings_.weak_encrypted_message_key_length !=
          (int32)WeakEncryptionLevel::High) {  // 2048
    throw Exception(
        "StartServerArgs::weak_encrypted_message_key_length incorrect key "
        "length, MaxSize is 2048, MinSize is 512");
  }
  ...
  if (settings_.strong_encrypted_message_key_length !=
          (int32)StrongEncryptionLevel::None &&  // 0
      settings_.strong_encrypted_message_key_length !=
          (int32)StrongEncryptionLevel::Low &&  // 128
      settings_.strong_encrypted_message_key_length !=
          (int32)StrongEncryptionLevel::Middle &&  // 192
      settings_.strong_encrypted_message_key_length !=
          (int32)StrongEncryptionLevel::High) {  // 256
    throw Exception(
        "StartServerArgs::EncryptedMessageKeyLength incorrect key length, only "
        "128, 192, 256");
  }

  // Generate public and private keys in advance and generate AES key for
  // loopback ..
  ...
  if (!CryptoRSA::CreatePublicAndPrivateKey(self_xchg_key_, public_key_blob_) ||
      !CryptoRSA::CreateRandomBlock(
          rsa_random_block, settings_.strong_encrypted_message_key_length) ||
      !CryptoAES::ExpandFrom(
          self_session_key_.aes_key, (const uint8*)rsa_random_block.ConstData(),
  ...
  if (args.external_user_worker_thread_pool) {
    ...
    user_thread_pool_->SetCallbacks(
        this);  // 내장일 경우 event-callbacks는 this가 된다. (스레드풀 셋팅전에
                // 해야함)
  ...
  if (!CreateAndInitUdpSockets(args.udp_assign_mode, args.udp_ports,
                               args.failed_bind_ports, out_error) ||
      !CreateTcpListenSocketAndInit(args.tcp_ports, out_error)) {
    // Because the sockets could not be created, it is no longer possible to
    // operate normally, Cancel all initializations that were done before.
    clock_.Stop();
    ...
    out_error =
        ResultInfo::From(ResultCode::ServerPortListenFailure, HostId_None,
                         "Listening tcp or udp socket creation is failed.");
    return false;
  }
    ...
    case HostIdGenerationPolicy::Assign:
      ...
      if (args.pre_created_p2p_group_start_host_id > HostId_Last) {
        ...
        for (int32 i = 0; i < args.pre_create_p2p_group_count; ++i) {
          CreateP2PGroup(
              nullptr, 0, ByteArray(), args.pre_create_p2p_group_option,
              (HostId)(args.pre_created_p2p_group_start_host_id + i));
        }
      }
      break;
  ...
  // PurgeTooOldUnmatureClient_Alarm =
  // IntervalAlaram(NetConfig::purge_too_old_add_member_ack_timeout_sec);
  // PurgeTooOldAddMemberAckItem_Alarm =
  // IntervalAlaram(NetConfig::purge_too_old_add_member_ack_timeout_sec);
  // DisposeIssuedRemoteClients_Alarm =
  // IntervalAlaram(NetConfig::dispose_issued_remote_clients_timeout_sec);
  // ElectSuperPeer_Alarm =
  // IntervalAlaram(NetConfig::elect_super_peer_interval_sec);
  // RemoveTooOldUdpSendPacketQueue_Alarm =
  // IntervalAlaram(NetConfig::udp_packet_board_long_interval_sec);
  // DisconnectRemoteClientOnTimeout_Alarm =
  // IntervalAlaram(NetConfig::cs_ping_interval_sec);
  // RemoveTooOldRecyclePair_Alarm =
  // IntervalAlaram(NetConfig::remove_too_old_recycle_pair_interval_sec);
  ...
  if (timer_callback_interval_ > 0) {
    tick_timer_id_ = timer_.Schedule(
        ...
        [&](const TimerTaskContext&) { PostOnTick(); }, "NetServer.Tick");
  } else {
    tick_timer_id_ = 0;
  }
  ...
  issue_send_on_need_timer_id_ = timer_.Schedule(
      Timespan::FromMilliseconds(100),
      Timespan::FromMilliseconds(every_remote_issue_send_on_need_internal_ *
                                 1000),
...
void NetServerImpl::Stop() {
  ...
  if (user_thread_pool_ && user_thread_pool_->IsCurrentThread()) {
    ...
    // 않는 현상이 생긴다. 그런데, userworkerthread내에서 Stop()을 호출하는게
    // 말이되나?
  ...
  {
    // Because the server is in the process of shutting down, the connection of
    // the new client is no longer meaningful, This closes the listener socket
    // and blocks new client connections. If you allow it, only errors will be
    // generated and the process will not be clean.
    ...
    // Disconnect all clients that are already connected. However, if you force
    // off immediately, Because it is lost, tell it to disconnect, and then wait
    // until the process is completed normally.
    ...
    for (auto& pair : candidate_remote_clients_) {
      auto& rc = pair.value;
      IssueDisposeRemoteClient(rc, ResultCode::DisconnectFromLocal,
    ...
    // Close the socket before leaving the overlapped I/O thread (leaving the
    // buffer) no more worker's issue should be made. Then there is no memory
    // violation to exit.
    for (int32 socket_index = 0; socket_index < udp_sockets_.Count();
         ++socket_index) {
  ...
  // Instructs the thread pool to terminate, wait until it is terminated
  // gracefully, and terminate safely.
  ...
  // Since all the threads have been properly terminated, remove the remaining
  // objects here.
  {
    CScopedLock2 main_guard(main_mutex_);
    CheckCriticalSectionDeadLock(__FUNCTION__);
...
bool NetServerImpl::CreateTcpListenSocketAndInit(
    const Array<int32>& tcp_ports, SharedPtr<ResultInfo>& out_error) {
  ...
  if (!bound) {
    // TCP listening can not start itself. In this case, the server can not
    // function properly.
    out_error = ResultInfo::From(ResultCode::ServerPortListenFailure,
                                 HostId_Server, "");
    tcp_listening_socket_.Reset();
...
bool NetServerImpl::CreateAndInitUdpSockets(ServerUdpAssignMode udp_assign_mode,
  ...
  for (int32 port_index = 0; port_index < udp_ports.Count(); ++port_index) {
    ...
    if (udp_socket->cached_local_addr_.GetPort() == 0 ||
        udp_socket->cached_local_addr_.GetPort() == 0xFFFF) {
      throw Exception("cached_local_addr_ has an unexpected value.");
    }
    udp_sockets_.Add(udp_socket);
    local_addr_to_udp_socket_map_.Add(udp_socket->cached_local_addr_,
                                      udp_socket);

    // IOCP에 연계한다.
    udp_socket->socket_->SetCompletionContext(
        (ICompletionContext*)udp_socket.Get());
    AssertIsLockedByCurrentThread();
    net_thread_pool_->AssociateSocket(udp_socket->socket_.Get());

    // overlapped send를 하므로 송신 버퍼는 불필요하다.
    // socket의 send buffer를 없앤다. CSocketBuffer가 non swappable이므로
    // 안전하다. send buffer 없애기는 coalsecse, throttling을 위해 필수다. recv
    // buffer 제거는 백해무익이므로 즐

// zero copy send는 빠르지만 너무 많은 nonpaged를 유발 위험. 따라서 이걸로 막자.
// 막으니 성능도 더 나은데?
...
RemoteClient_S* NetServerImpl::GetRemoteClientByUdpEndPoint_NOLOCK(
...
void NetServerImpl::PurgeTooOldAddMemberAckItem() {
  ...
  for (auto& pair : p2p_groups_) {
    ...
    for (int32 member_index = 0;
         member_index < group->add_member_ack_waiters.Count(); ++member_index) {
      ...
      if ((absolute_time - ack.event_time) >
        ...
        // 이어져서는 안되므로 아래가 필요.
        if (!group->add_member_ack_waiters.AckWaitingItemExists(
                joining_peer_id)) {
...
void NetServerImpl::PurgeTooOldUnmatureClient() {
  ...
  Array<RemoteClient_S*, InlineAllocator<256>> collected_timeout_rc_list(
      list_count, NoInit);
  ...
  for (auto& rc_pair : candidate_remote_clients_) {
    ...
    if ((absolute_time - rc->created_time_) >
...
#pragma omp parallel for
    ...
    for (int32 timeout_index = 0; timeout_index < timeout_count;
      ...
      CScopedLock2 rc_tcp_send_queue_guard(
          rc->to_client_tcp_->GetSendQueueMutex(), false);

      if (timeout_index != 0) {
        if (rc_tcp_send_queue_guard.TryLock()) {
          rc->to_client_tcp_->SendWhenReady(SendFragRefs(msg_to_send),
                                            TcpSendOption());
          rc_tcp_send_queue_guard.Unlock();

          rc->DecreaseUseCount();
          collected_timeout_rc_list[timeout_index] =
              collected_timeout_rc_list[--timeout_count];
        }
      } else {
...
void NetServerImpl::ProcessOnClientDisposeCanSafe(RemoteClient_S* rc) {
  ...
  // 인덱스 정리
  udp_addr_to_remote_client_index_.Remove(
      rc->to_client_udp_fallbackable_.GetUdpAddrFromHere());
  ...
  if (rc->to_client_tcp_->socket_) {
    ...
    if (intra_logger_) {
      const String text = String::Format(
          "%s CloseSocketHandleOnly() called.  remote_client: %d", __FUNCTION__,
          (int32)rc->host_id_);
  ...
  if (rc->to_client_udp_fallbackable_.udp_socket) {
    // CScopedLock2
    // udp_socket_guard(rc->to_client_udp_fallbackable_.udp_socket_->GetMutex());
  ...
  udp_addr_to_remote_client_index_.Remove(
      rc->to_client_udp_fallbackable_.udp_addr_from_here_);
}

SessionKey* NetServerImpl::GetCryptSessionKey(HostId remote_id,
                                              String& out_error) {
  CScopedLock2 main_guard(main_mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
  ...
  if (key == nullptr) {
    out_error = String::Format("%d rc is %s in NetServer.", (int32)remote_id,
                               rc == nullptr ? "null" : "not null");
  }
...
bool NetServerImpl::Send(const SendFragRefs& data_to_send,
                         ...
                         int32 sendto_count) {
  // The send is meaningless when the server is shutting down and does not do
  // anything because it may cause errors during the sending process.
  if (!listener_) {
    return false;
  }
...
bool NetServerImpl::Send_BroadcastLayer(const SendFragRefs& payload,
  ...
  const int32 unreliable_s2c_routed_multicast_max_count =
      MathBase::Max(send_opt.unreliable_s2c_routed_multicast_max_count, 0);
  const double unreliable_s2c_routed_multicast_max_ping =
      MathBase::Max(send_opt.unreliable_s2c_routed_multicast_max_ping, 0.0);
  ...
  //@todo SendDestList가 비대하지는 않을런지? 스택 공간을 할당받아서 사용해야
  //하므로 대량은 곤란하다. 흠..
  ...
  if (send_opt.reliability == MessageReliability::Reliable) {
    ...
    Array<ISendDest_S*, InlineAllocator<256>> reliable_send_list(
        send_dest_list_count, NoInit);
    for (int32 dst_index = 0; dst_index < send_dest_list_count; ++dst_index) {
      ...
      if (send_dest == this && send_opt.bounce) {
        // 즉시 final user work로 넣어버리자. 루프백 메시지는 사용자가 발생하는
        // 것 말고는 없는 것을 전제한다. 메시지 타입을 제거한 후 페이로드만
        // 온전히 남겨준다음 넘겨줌.
        ...
        switch (msg_type) {
          case MessageType::RPC:
            user_work_type = FinalUserWorkItemType::RPC;
            break;
          case MessageType::FreeformMessage:
            user_work_type = FinalUserWorkItemType::FreeformMessage;
            break;
          default:
            fun_check(0);
            break;
        ...
        final_user_work_queue_.Enqueue(
            FinalUserWorkItem_S(payload_without_msg_type, user_work_type));
    ...
    // host_id 리스트간에, P2P route를 할 수 있는 것들끼리 묶는다. 단,
    // unreliable 메시징인 경우에 한해서만. 여기에서 SendDestList가 재조정 될수
    // 있으므로, SendDestListCount를 갠신해야함.
    Array<SendDestInfo_S*, InlineAllocator<256>> unreliable_send_info_list(
        send_dest_list_count, NoInit);
    MakeP2PRouteLinks(send_dest_list, unreliable_s2c_routed_multicast_max_count,
    ...
    for (int32 dst_index = 0; dst_index < send_dest_list_count; ++dst_index) {
      ...
      if (send_dest == this && send_opt.bounce) {
        ...
        if (rc->to_client_udp_fallbackable_.real_udp_enabled_) {
          ...
          send_dest_list[dst_index].host_object =
              rc->to_client_udp_fallbackable_.udp_socket_.Get();
          send_dest_list[dst_index].sendto_addr =
              rc->to_client_udp_fallbackable_.GetUdpAddrFromHere();
        } else {
          rc->IncreaseUseCount();
    ...
    for (int32 dst_index = 0; dst_index < unreliable_count; ++dst_index) {
      ...
      // 항목의 P2P route prev link가 있으면, 즉 이미 P2P route로 broadcast가 된
      // 상태이다. 그러므로, 넘어간다.
      if (send_dest_info->p2p_route_prev_link) {
        // 아무것도 안함
      }
      // 항목의 P2P next link가 있으면, link의 끝까지 찾아서 P2P route 메시지
      // 내용물에 추가한다.
        ...
        Array<HostId,
            ...
            p2p_route_list;
        for (; send_dest_info;
             send_dest_info = send_dest_info->p2p_route_next_link) {
        ...
        CScopedLock2 host_send_guard(
            org_send_dest_info->host_object->GetSendMutex());
        org_send_dest_info->host_object->SendWhenReady(
            HostId_None, org_send_dest_info->sendto_addr, rc->GetHostId(),
            s2c_routed_multicast_msg, send_opt);
      }
      // 항목의 P2P link가 전혀 없다. 그냥 보내도록 한다.
        ...
        CScopedLock2 host_send_guard(
            org_send_dest_info->host_object->GetSendMutex());
        org_send_dest_info->host_object->SendWhenReady(
            rc->GetHostId(), org_send_dest_info->sendto_addr, rc->GetHostId(),
            payload, send_opt);
      }
...
bool NetServerImpl::CloseConnection(HostId client_id) {
  ...
  if (auto conn = GetAuthedClientByHostId_NOLOCK(client_id)) {
    if (intra_logger_) {
      const String text = String::Format(
          "NetServer::CloseConnection(%d) is called.", (int32)client_id);
      intra_logger_->WriteLine(LogCategory::System, *text);
    }

    // Handle it in a request form so that it can be disconnected in a safe
    // state without disconnecting the connection.
    RequestAutoPrune(rc);
    return true;
  }
...
RemoteClient_S* NetServerImpl::GetRemoteClientByHostId_NOLOCK(
    HostId client_id) {
  AssertIsLockedByCurrentThread();
  return authed_remote_clients_.FindRef(client_id);
}

RemoteClient_S* NetServerImpl::GetAuthedClientByHostId_NOLOCK(
    HostId client_id) {
  AssertIsLockedByCurrentThread();
  ...
  if (member_id != HostId_Server) {
    for (const auto& member_pair : group->members_) {
      if (member_pair.value.ptr->GetHostId() != HostId_Server &&
          member_rc->GetHostId() != HostId_Server) {
        p2p_connection_pair_list_.ReleasePair(
            this, (RemoteClient_S*)member_rc,
            (RemoteClient_S*)member_pair.value.ptr);
      }
    }
  }
...
HostId NetServerImpl::CreateP2PGroup(const HostId* ClientHostIdList,
                                     int32 Count, const ByteArray& custom_field,
  ...
  if (Count > 0) {
    ...
    for (int32 i = 0; i < Count; ++i) {
      ...
      if (!rc->session_key_.KeyExists()) {
        EnqueueError(
            ResultInfo::From(ResultCode::Unexpected, rc->host_id_,
                             "CreateP2PGroup failed: session-key is missing."));
      }
    }
  }
  ...
  const HostId group_id =
      host_id_factory_->Create(GetAbsoluteTime(), assigned_host_id);
  ...
  for (int32 i = 0; i < Count; ++i) {
    JoinP2PGroup_INTERNAL(UniqueClientHostIdList[i], NewGroup->group_id_,
...
bool NetServerImpl::JoinP2PGroup_INTERNAL(HostId member_id, HostId group_id,
  ...
  if (settings_.server_as_p2p_group_member_allowed &&
    ...
    member_rc = GetAuthedClientByHostId_NOLOCK(member_id);
    if (member_rc == nullptr ||
        IsDisposeRemoteClient_NOLOCK((RemoteClient_S*)member_rc)) {
      return false;
    }
  }
  ...
  for (auto old_member_it = group->members_.CreateIterator(); old_member_it;
       ++old_member_it) {
    ...
    if (old_member->GetHostId() != HostId_Server &&
      ...
      p2p_state = p2p_connection_pair_list_.GetPair(old_member_as_rc->host_id_,
                                                    new_member_as_rc->host_id_);
      if (!p2p_state) {
        p2p_state = p2p_connection_pair_list_.GetRecyclePair(
            old_member_as_rc->host_id_, new_member_as_rc->host_id_, true);

        fun_check(!p2p_state || p2p_state->release_time_ > 0);

        if (!p2p_state ||
            (p2p_state && (absolute_time - p2p_state->release_time_ >
                           NetConfig::recycle_pair_reuse_time_sec))) {
          p2p_state.Reset(new P2PConnectionState(
              this, old_member_as_rc->host_id_ == new_member_as_rc->host_id_));

          p2p_state->SetRelayed(true);

          if (settings_.p2p_encrypted_messaging_enabled) {
            //@note 암호화된 P2P통신을 사용해야 할 경우, 인증키를 준비함.
            //랜덤블럭 준비.
            if (!CryptoRSA::CreateRandomBlock(
                    p2p_state->p2p_aes_session_key_,
                    settings_.strong_encrypted_message_key_length) ||
                !CryptoRSA::CreateRandomBlock(
                    p2p_state->p2p_rc4_session_key_,
                    settings_.weak_encrypted_message_key_length)) {
              if (intra_logger_) {
                const String text = String::Format(
                    "P2P session-key make failed between client %d and %d.",
                    (int32)old_member->GetHostId(),
                    (int32)member_rc->GetHostId());
          ...
          p2p_state->p2p_fist_frame_number_ = RUdpHelper::GetRandomFrameNumber(
              random_);  //첫번째 프레임은 임의대로함. 약간의 보안을 위해서..
        ...
        p2p_connection_pair_list_.AddPair(old_member_as_rc, new_member_as_rc,
                                          p2p_state);

        p2p_state->MemberJoinStart(old_member_as_rc->host_id_,
                                   new_member_as_rc->host_id_);
        ...
        if (intra_logger_) {
          const String text = String::Format(
              "Prepare P2P pair for client %d and %d.",
    ...
    if (old_member != this) {
      if (recycle && p2p_state) {
        bind_port =
            p2p_state->GetHolepunchedInternalAddr(old_member->GetHostId())
                .port();
      }
      ...
      if (settings_.p2p_encrypted_messaging_enabled) {  // P2P with encryption
        ...
        s2c_proxy_.P2PGroup_MemberJoin(
            ...
            group->Option.direct_p2p_enabled, bind_port);
    ...
    if (intra_logger_ && p2p_state) {
      const String text = String::Format("P2P tag created by server. tag: %s",
...
bool NetServerImpl::JoinP2PGroup(HostId member_id, HostId group_id,
                                 const ByteArray& custom_field) {
  CScopedLock2 main_guard(main_mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
...
void NetServerImpl::EnqueueClientLeaveEvent(RemoteClient_S* rc,
  ...
  if (callbacks_ && rc->host_id_ != HostId_None) {
    ...
    // if (socket_error == 121) {
    //  String text;
    //  text = String::Format("socket_error=121! source={%s, %d},
    //  client_info={%s}, CCU = %d, SrvConfig={%s}",
  ...
  //  GReliableSend_INTERNAL, rc->host_id_);
  //}
}
...
bool NetServerImpl::GetP2PGroupInfo(HostId group_id,
                                    P2PGroupInfo& out_group_info) {
  CScopedLock2 main_guard(main_mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
...
void NetServerImpl::ConvertAndAppendP2PGroupToPeerList(HostId send_to,
                                                       HostIdArray& SendTo2) {
  AssertIsLockedByCurrentThread();
...
bool NetServerImpl::GetP2PConnectionStats(HostId remote_id,
  ...
  if (auto rc = GetAuthedClientByHostId_NOLOCK(remote_id)) {
    ...
    out_stats.to_remote_peer_send_udp_message_success_count =
        rc->to_remote_peer_send_udp_message_success_count_;
    out_stats.to_remote_peer_send_udp_message_attempt_count =
        rc->to_remote_peer_send_udp_message_attempt_count_;
    ...
    // visited.SetMinCapacity((int32)MathBase::Max(pow((double)group->members_.size(),
    // 2) * 0.5, 1.0));
    visited.Reserve((int32)MathBase::Max(
    ...
    for (auto& member_pair : group->members_) {
      if (member_pair.value.ptr->GetHostId() != HostId_Server) {
        ...
        out_stats.to_remote_peer_send_udp_message_success_count +=
            member_as_rc->to_remote_peer_send_udp_message_success_count_;
        out_stats.to_remote_peer_send_udp_message_attempt_count +=
            member_as_rc->to_remote_peer_send_udp_message_attempt_count_;

        for (const auto& conn_pair_of_member :
             member_as_rc->p2p_connection_pairs_) {
          // 다른 그룹일경우 제외하자.
          if (!group->members_.Contains(
                  conn_pair_of_member->first_client->host_id_) ||
              !group->members_.Contains(
                  conn_pair_of_member->second_client->host_id_)) {
            continue;
          }
...
bool NetServerImpl::GetP2PConnectionStats(HostId remote_a, HostId remote_b,
  ...
  if (rc_a && rc_b) {
    for (const auto& pair : rc_a->p2p_connection_pairs_) {
      if (pair->ContainsHostId(rc_b->host_id_)) {
        out_stats.to_remote_b_send_udp_message_success_count =
            pair->to_remote_peer_send_udp_message_success_count_;
        out_stats.to_remote_b_send_udp_message_attempt_count =
            pair->to_remote_peer_send_udp_message_attempt_count_;
      }
    }

    for (const auto& pair : rc_b->p2p_connection_pairs_) {
      if (pair->ContainsHostId(rc_a->host_id_)) {
        out_stats.to_remote_a_send_udp_message_success_count =
            pair->to_remote_peer_send_udp_message_success_count_;
        out_stats.to_remote_a_send_udp_message_attempt_count =
            pair->to_remote_peer_send_udp_message_attempt_count_;
      }
    }
...
// 하니까. 이 함수는 마구 호출해도 괜찮다.
void NetServerImpl::IssueDisposeRemoteClient(
    RemoteClient_S* rc, ResultCode result_code, ResultCode detail_code,
  ...
  if (!rc->dispose_waiter_) {  // just once
    ...
    if (intra_logger_) {
      const String text =
          String::Format("Call IssueDisposeRemoteClient() client=%d in %s.",
                         (int32)rc->host_id_, where);
      intra_logger_->WriteLine(LogCategory::System, *text);
    }
  }
  ...
  if (!dispose_issued_remote_clients_map_.Contains(rc)) {
    ...
    if (rc->dispose_waiter_) {
      EnqueueClientLeaveEvent(
          rc, rc->dispose_waiter_->reason, rc->dispose_waiter_->detail,
          rc->dispose_waiter_->comment, rc->dispose_waiter_->socket_error);
    } else {
      EnqueueClientLeaveEvent(rc, ResultCode::DisconnectFromLocal,
    ...
    return;  // 이렇게 하면 아래 CloseSocketHandleOnly가 자주호출되는 문제를
             // 피할 듯.
  }

  // When you close the socket like this, all the issues that were in the middle
  // of the issue are terminated. And then you try to retry. it will not be
  // because there is DisposeWaiter. Then you can safely destroy the object.
  // Additionaly, record the total amount sent to this remote before closing the
  // socket.
  ...
  if (intra_logger_) {
    ...
    // const String text = String::Format("RemoteClient stats in connected.
    // remote=%d, TotalSentBytes=%u bytes, TotalRecvBytes=%u bytes.",
    //        rc->host_id_, rc->to_client_tcp_->total_tcp_issued_send_bytes_,
    //        rc->to_client_tcp_->TotalTcpIssueRecvBytes);
    const String text = String::Format(
        "RemoteClient stats in connected.  remote_id: %d, total_sent_bytes: %u "
        "bytes",
        (int32)rc->host_id_, rc->to_client_tcp_->total_tcp_issued_send_bytes_);
    intra_logger_->WriteLine(LogCategory::System, *text);
  }

  rc->to_client_tcp_->socket_->CloseSocketHandleOnly();

  if (rc->owned_udp_socket_) {  // per rc udp 인경우 즉, RC당 UDPSocket이 할당된
                                // 경우, 할당된 UDP 소켓을 닫아줌.
  ...
  for (auto& group_pair : rc->joined_p2p_groups_) {
    ...
    AddMemberAckWaiters_RemoveRelated_MayTriggerJoinP2PMemberCompleteEvent(
        group.Get(), rc->host_id_, ResultCode::DisconnectFromRemote);

    // Notify member leave to related group members
    for (const auto& member_pair : group->members_) {
      if (member_pair.key != HostId_Server) {  // server does not need to
                                               // receive P2PGroup_MemberLeave.
        s2c_proxy_.P2PGroup_MemberLeave(member_pair.key, GReliableSend_INTERNAL,
                                        rc->host_id_, group->group_id_);
      }
    }
...
void NetServerImpl::DisposeIssuedRemoteClients() {
  ...
  for (auto it = dispose_issued_remote_clients_map_.CreateIterator(); it;
       ++it) {
    auto rc = it->key;
    ...
    if (rc->to_client_udp_fallbackable_.udp_socket_) {
      ...
      udp_socket_guard.SetMutex(
          rc->to_client_udp_fallbackable_.udp_socket_->GetMutex(), true);
      udp_socket_fragger_guard.SetMutex(
          rc->to_client_udp_fallbackable_.udp_socket_->GetFraggerMutex(), true);
    }
    ...
    const bool tcp_close_safe =
        !rc->to_client_tcp_->recv_issued_ && !rc->to_client_tcp_->send_issued_;

    bool udp_close_safe;
    if (!rc->owned_udp_socket_) {
      udp_close_safe = true;
    } else {
      udp_close_safe = !rc->owned_udp_socket_->recv_issued_ &&
                       !rc->owned_udp_socket_->send_issued_ &&
                       rc->owned_udp_socket_->GetUseCount() == 0;
    }

    const bool works_remain = (rc->task_subject_node_.GetListOwner() ||
                               rc->IsTaskRunning() || rc->GetListOwner());
    ...
    if (use_count_safe && tcp_close_safe && udp_close_safe && !works_remain) {
      ...
      if (udp_socket_guard.IsLocked()) {
        // lock의 파괴자가 나중에 호출 되므로 이경우는 unlock를 먼저 해줘야
        // 하겠다.
...
double NetServerImpl::GetTime() { return GetAbsoluteTime(); }
...
// Listening 바인딩된 주소 기준으로 IPv4 인지 IPv6인지 구분해서 처리하는게
// 바람직해보임. Listening 바인딩된 주소 기준으로 IPv4 인지 IPv6인지 구분해서
// 처리하는게 바람직해보임. Listening 바인딩된 주소 기준으로 IPv4 인지 IPv6인지
// 구분해서 처리하는게 바람직해보임. Listening 바인딩된 주소 기준으로 IPv4 인지
// IPv6인지 구분해서 처리하는게 바람직해보임. Listening 바인딩된 주소 기준으로
// IPv4 인지 IPv6인지 구분해서 처리하는게 바람직해보임. Listening 바인딩된 주소
// 기준으로 IPv4 인지 IPv6인지 구분해서 처리하는게 바람직해보임. Listening
// 바인딩된 주소 기준으로 IPv4 인지 IPv6인지 구분해서 처리하는게 바람직해보임.
...
void NetServerImpl::TcpAndUdp_LongTick() {
  ...
  if (udp_socket_list_count > 0) {
    ...
    udp_socket_list_count =
        authed_remote_client_list_count + candidate_remote_client_list_count;
    udp_socket_list.ResizeUninitialized(udp_socket_list_count);
  }
  ...
  if (authed_remote_client_list_count > 0) {
    collected_authed_rc_list.ResizeUninitialized(
        authed_remote_client_list_count);
  ...
  if (candidate_remote_client_list_count > 0) {
    int32 count = 0;
    collected_candidate_rc_list.ResizeUninitialized(
        candidate_remote_client_list_count);
    for (auto& rc_pair : candidate_remote_clients_) {
      auto rc = rc_pair.value;
...
#endif
#endif
    for (int32 socket_index = 0; socket_index < owned_socket_count;
         ++socket_index) {
...
#endif
#endif
    for (int32 rc_index = 0; rc_index < authed_remote_client_list_count;
         ++rc_index) {
      ...
      if (rc_index != 0) {
        if (rc_tcp_send_queue_guard.TryLock()) {
          ...
          rc->DecreaseUseCount();
          collected_authed_rc_list[rc_index] =
              collected_authed_rc_list[--authed_remote_client_list_count];
        }
      } else {
        rc_tcp_send_queue_guard.Lock();
        rc->to_client_tcp_->LongTick(absolute_time);
...
#endif
#endif
    for (int32 rc_index = 0; rc_index < candidate_remote_client_list_count;
         ++rc_index) {
      ...
      if (rc_index != 0) {
        if (rc_tcp_send_queue_guard.TryLock()) {
          ...
          rc->DecreaseUseCount();
          collected_candidate_rc_list[rc_index] =
              collected_candidate_rc_list[--candidate_remote_client_list_count];
        }
      } else {
        rc_tcp_send_queue_guard.Lock();
        rc->to_client_tcp_->LongTick(absolute_time);
...
void NetServerImpl::DisableIntraLogging() {
  ...
  if (intra_logger_) {
    ...
    for (auto& rc_pair : authed_remote_clients_) {
      if (!rc_pair.value->dispose_waiter_) {  // 종료 대기중인 것들은 제외..
                                              // 시도 해봐야 전송도 안될터...
...
void NetServerImpl::Heartbeat_PerClient() {
  ...
  for (auto& rc_pair : authed_remote_clients_) {
      ...
      if (rc->to_client_tcp) {
        if (!rc->to_client_tcp_->recv_issued_) {
          if (rc->to_client_tcp_->last_recv_issue_warning_time_ != 0.0) {
            if ((absolute_time -
                 rc->to_client_tcp_->last_recv_issue_warning_time_) >
                3.0) {  // TODO: NetConfig로 빼주는게 좋을듯...
              rc->to_client_tcp_->last_recv_issue_warning_time_ = absolute_time;

              const String text =
                  String::Format("host_id:%d - recvissued delay 3 second",
                                 (int32)rc->host_id_);
              intra_logger_->WriteLine(LogCategory::System, *text);
            }
          } else {
        ...
        if (!rc->to_client_tcp_->send_issued_) {
          if (rc->to_client_tcp_->last_send_issue_warning_time_ != 0.0) {
            if ((absolute_time -
                 rc->to_client_tcp_->last_send_issue_warning_time_) >
                5.0) {  // TODO: NetConfig로 빼주는게 좋을듯...
              rc->to_client_tcp_->last_send_issue_warning_time_ = absolute_time;

              const String text = String::Format(
                  "host_id: %d - sendissued delay 5 second, InReadyList: %s",
                  (int32)rc->host_id_, rc->GetListOwner() ? "true" : "false");
              intra_logger_->WriteLine(LogCategory::System, *text);
            }
          } else {
            rc->to_client_tcp_->last_send_issue_warning_time_ = absolute_time;
    ...
    if ((absolute_time - rc->last_tcp_stream_recv_time_) >
        settings_.default_timeout_sec) {
      if (!rc->dispose_waiter_) {
        if (intra_logger_) {
          const String text = String::Format(
              "The TCP receive from the client %d no longer.  close the "
              "socket.",
              (int32)rc->GetHostId());
          intra_logger_->WriteLine(LogCategory::System, *text);
        }

        IssueDisposeRemoteClient(rc, ResultCode::DisconnectFromRemote,
                                 ResultCode::ConnectServerTimeout, ByteArray(),
                                 __FUNCTION__, SocketErrorCode::Ok);
      }
...
void NetServerImpl::LocalProcessForFallbackUdpToTcp(RemoteClient_S* conn) {
  ...
  if (conn->to_client_udp_fallbackable_.real_udp_enabled_) {
    ...
    if (intra_logger_) {
      const String text = String::Format(
          "Client %d cancel UDP hole punching with server.  client_local_addr: "
          "%s",
          (int32)conn->host_id_,
          *conn->to_client_tcp_->remote_addr_.ToString());
      // TODO 카테고리를 뭘로 해야할까??
...
void NetServerImpl::EnqueueHackSuspectEvent(RemoteClient_S* conn,
...
void NetServerImpl::SetMessageMaxLength(int32 max_length) {
...

  {
    ...
    NetConfig::message_max_length =
        MathBase::Max(NetConfig::message_max_length,
                      max_length);  // 아예 전역 값도 수정하도록 한다.
  }
...
bool NetServerImpl::SetDirectP2PStartCondition(
...
InetAddress NetServerImpl::GetTcpListenerLocalAddr() {
  return tcp_listening_socket_ ? tcp_listening_socket_->GetSockName()
                               : InetAddress::None;
}
...
void NetServerImpl::GetUdpListenerLocalAddrs(Array<InetAddress>& output) {
  ...
  if (udp_assign_mode != ServerUdpAssignMode::Static) {
    throw Exception(
        "Cannot call GetUdpListenerLocalAddrs unless "
        "ServerUdpAssignMode::Static is used.");
  }
...
void NetServerImpl::ConvertGroupToIndividualsAndUnion(
...
void NetServerImpl::MakeP2PRouteLinks(
    SendDestInfoList_S& tgt, int32 unreliable_s2c_routed_multicast_max_count,
  ...
  for (i = 0; i < tgt_count; ++i) {
    ...
    for (j = i; j < tgt_count; ++j) {
      ...
      if (i != j && (state = p2p_connection_pair_list_.GetPair(
                         tgt[i].host_id, tgt[j].host_id))) {
        if (!state->GetRelayed() &&
            state->recent_ping_ < routed_send_max_ping) {
  ...
  for (i = 0; i < connection_info_list_.Count(); ++i) {
    ...
    if (router_index_list_.Contains(info2.host_index0) ||
        router_index_list_.Contains(info2.host_index1)) {
      continue;
    }
  ...
  for (int32 router_index = 0; router_index < router_index_list_count;
       ++router_index) {
    ...
    int32 count = 0;
    for (i = 0; i < connection_info_list_count &&
                count < unreliable_s2c_routed_multicast_max_count;
         ++i) {
      if (host_index == connection_info_list_[i].host_index0) {
        ...
        if (next_linked_item->p2p_route_prev_link ||
            next_linked_item->p2p_route_next_link) {
          continue;
        }
...
bool NetServerImpl::NextEncryptCount(HostId remote_id,
...
bool NetServerImpl::GetExpectedDecryptCount(HostId remote_id,
                                            CryptoCountType& out_count) {
  CScopedLock2 main_guard(main_mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
...
bool NetServerImpl::NextDecryptCount(HostId remote_id) {
  ...
  if (remote_id == HostId_Server) {  //@maxidea: todo: Hit 비중으로 보자면,
                                     //RC조회가 더 많을듯 싶은데... 하지만, 이게
                                     //연산이 없으니 이게 먼저 오는게 나으려나..
    ++self_decrypt_count_;
    return true;
  } else if (auto rc = GetAuthedClientByHostId_NOLOCK(remote_id)) {
    ++rc->decrypt_count;
...
bool NetServerImpl::IsConnectedClient(HostId client_id) {
  ...
  return GetAuthedClientByHostId_NOLOCK(
      client_id);  // 종료 되었거나, 종료 중일 경우에는 nullptr을 반환함.
}
  ...
  if (auto rc = GetAuthedClientByHostId_NOLOCK(client_id)) {
    if (enable) {
      if (!rc->SpeedHackDetector) {
        ...
        s2c_proxy_.NotifySpeedHackDetectorEnabled(client_id,
                                                  GReliableSend_INTERNAL, true);
      }
    } else {
      if (rc->speed_hack_detector_) {
        rc->speed_hack_detector_.Reset();
        s2c_proxy_.NotifySpeedHackDetectorEnabled(
            client_id, GReliableSend_INTERNAL, false);
      }
    }
...
void NetServerImpl::ConditionalFallbackServerUdpToTcp(RemoteClient_S* rc,
                                                      double absolute_time) {
  // [CaseCMN] 간혹 섭->클 UDP 핑은 되면서 반대로의 핑이 안되는 경우로 인해 UDP
  // fallback이 계속 안되는 경우가 있는 듯. 그러므로 서버에서도 클->섭 UDP 핑이
  // 오래 안오면 fallback한다.

  // if ((absolute_time - rc->last_udp_packet_recv_time_) > default_timeout_sec)
  // 이게 아니라
  if (rc->to_client_udp_fallbackable_.real_udp_enabled_ &&
      (absolute_time - rc->last_udp_packet_recv_time_) >
    ...
    s2c_proxy_.NotifyUdpToTcpFallbackByServer(rc->host_id_,
                                              GReliableSend_INTERNAL);
  }
}

// 홀 펀칭된 포트를 유지하기 위해 더미 패킷을 쏴줌..
void NetServerImpl::ConditionalArbitaryUdpTouch(RemoteClient_S* rc,
                                                double absolute_time) {
  AssertIsLockedByCurrentThread();

  fun_check(NetConfig::GetFallbackServerUdpToTcpTimeout() >
            NetConfig::cs_ping_interval_sec * 2.5);

  // 클라로부터 최근까지도 UDP 수신은 됐지만 정작 ping만 안오면 다른 형태의 pong
  // 보내기. 클라에서 서버로 대량으로 UDP를 쏘느라 정작 핑이 늑장하는 경우
  // 필요하다.
  if ((absolute_time - rc->last_udp_ping_recv_time_) >
          NetConfig::cs_ping_interval_sec * 1.5 &&
      (absolute_time - rc->last_udp_packet_recv_time_) <
          NetConfig::cs_ping_interval_sec) {
    if ((absolute_time - rc->arbitrary_udp_touched_time_) >
        NetConfig::cs_ping_interval_sec) {
      rc->arbitrary_udp_touched_time_ = absolute_time;

      MessageOut msg_to_send;
      lf::Write(msg_to_send, MessageType::ArbitaryTouch);
      rc->to_client_udp_fallbackable_.SendWhenReady(
          rc->GetHostId(), SendFragRefs(msg_to_send),
          UdpSendOption(MessagePriority::Ring0, EngineOnlyFeature));
    }
  }
}

void NetServerImpl::SetMaxDirectP2PConnectionCount(HostId client_id,
                                                   int32 max_count) {
  CScopedLock2 main_guard(main_mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
...
struct SuperPeerCandidate {
  ...
  bool operator==(const SuperPeerCandidate& rhs) const {
    return GetSuperPeerRatingC(peer, group) ==
           GetSuperPeerRatingC(rhs.peer, group);
  }
 ...
 private:
  static double GetSuperPeerRatingC(const RemoteClient_S* rc,
                                    const P2PGroup_S* group) {
    if (group->ordered_super_peer_suitables.Count() > 0 &&
...
void NetServerImpl::P2PGroup_RefreshMostSuperPeerSuitableClientId(
    P2PGroup_S* group) {
  AssertIsLockedByCurrentThread();
  ...
  if (group->members_.IsEmpty() ||
      !group->super_peer_selection_policy_is_valid) {
    ...
    for (auto& member_pair : group->members_) {
      ...
      //코드를 살짝 수정해야할듯.. 멤버가 서버이면 건너뛰는것을 먼저 간단히
      //해주어야할듯...
      ...
      // 수퍼피어가 아닐 당시의 계산이 된 적이 있고 현 수퍼피어이면 계산을 하지
      // 않는다. 이유: 수퍼피어가 되면 트래픽이 확 늘어남=>평가 점수가 급락하게
      // 됨=>다른 피어에 비해서 평가 점수가 크게 하락할 수 있음
      ...
      if (rc->to_client_udp_fallbackable_.real_udp_enabled_) {
        rc->super_peer_rating_ +=
            group->super_peer_selection_policy.real_udp_weight;  // 10000
      }

      // 리얼 IP이면 장땡이다!
      if (!rc->IsBehindNAT()) {
        rc->super_peer_rating_ +=
            group->super_peer_selection_policy.no_nat_device_weight;  // 500;
      }
      ...
      rc->super_peer_rating_ -=
          rc->recent_ping_ *
          group->super_peer_selection_policy.server_lag_weight;  // 200;

      // 사용자가 입력한 프레임 속도만큼 가산점 처리한다. 40~60 프레임 * 4 = 240
      // 점 정도
      rc->super_peer_rating_ +=
          rc->last_application_hint_.recent_frame_rate *
          group->super_peer_selection_policy.frame_rate_weight;

      // 피어 평균 핑만큼 감점 처리한다.
      // (제아무리 서버와의 핑이 좋아도 타 피어와의 핑이 나쁘면 심각한 수준인
      // 경우도 있고 아닌 경우도 있다. 디폴트는 피어 갯수당 0점. >0이면 JIT
      // 홀펀칭이 무의미하므로.)
      rc->super_peer_rating_ -=
          MathBase::Max(0.0, rc->GetP2PGroupTotalRecentPing(group->group_id_)) *
          group->super_peer_selection_policy.peer_lag_weight;

      // 트래픽 속도만큼 가산점 처리 (VDSL 수준(100Mbps) 이면 매우 높은
      // 수준이므로 400점 가산점)
      rc->super_peer_rating_ +=
          group->super_peer_selection_policy.send_speed_weight *
          rc->send_speed * (1024 * 1024 * 10);  // 400

      // P2P 그룹에 들어온지 얼마 안됐으면 왕창 감점한다.
      if (group->super_peer_selection_policy.exclude_newjoinee_duration_time >
              0 &&
          (absolute_time - rc0.joined_time) <
              group->super_peer_selection_policy
                  .exclude_newjoinee_duration_time) {
    ...
    Array<SuperPeerCandidate, InlineAllocator<256>> member_list(
        group->members_.Count());
    for (auto& member_pair : group->members_) {
    ...
    group->ordered_super_peer_suitables.Resize(
        count);  // TODO ResizeUninitialized로 해도 되지 않을까? 물론,
                 // 이경우에는 멤버가 모두 초기화 되었을 경우에 한해서...
    for (int32 i = 0; i < count; ++i) {
      ...
      group->ordered_super_peer_suitables[i].real_udp_enabled =
          peer->to_client_udp_fallbackable_.real_udp_enabled_;
      ...
      group->ordered_super_peer_suitables[i].p2p_group_total_recent_ping =
          peer->GetP2PGroupTotalRecentPing(group->group_id_);
      group->ordered_super_peer_suitables[i].send_speed = peer->send_speed_;
      group->ordered_super_peer_suitables[i].joined_time =
          member_list[i].joined_time;
      group->ordered_super_peer_suitables[i].frame_rate =
          peer->last_application_hint_.recent_frame_rate;
    }
  }
}
...
HostId NetServerImpl::GetMostSuitableSuperPeerInGroup(
...
HostId NetServerImpl::GetMostSuitableSuperPeerInGroup(
    HostId group_id, const SuperPeerSelectionPolicy& policy, HostId excludee) {
...
void NetServerImpl::GetUdpSocketAddrList(Array<InetAddress>& output) {
  ...
  if (udp_assign_mode != ServerUdpAssignMode::Static) {
    throw Exception(
        "Cannot call GetUdpSocketAddrList unless ServerUdpAssignMode::Static "
        "is used.");
  }
...
void NetServerImpl::OnSocketWarning(InternalSocket* socket,
                                    const String& text) {
...
uint32 NetServerImpl::GetInternalVersion() { return internal_version_; }
...
void NetServerImpl::EnqueuePacketDefragWarning(const InetAddress& sender,
                                               const char* text) {
  CScopedLock2 main_guard(main_mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
  ...
  EnqueueWarning(ResultInfo::From(ResultCode::InvalidPacketFormat,
                                  rc ? rc->host_id_ : HostId_None, text));
}

void NetServerImpl::EnqueueClientJoinApproveDetermine(
...
void NetServerImpl::ProcessOnClientJoinRejected(RemoteClient_S* rc,
...
void NetServerImpl::ProcessOnClientJoinApproved(RemoteClient_S* rc,
                                                const ByteArray& response) {
  AssertIsLockedByCurrentThread();
  ...
  // 클라이언트가 서버에 성공적으로 접속되자 마자, 아래의 메시지를 보내줌.
  {
    ...
    {
      CScopedLock2 rc_tcp_send_queue_guard(
          rc->to_client_tcp_->GetSendQueueMutex());
      rc->to_client_tcp_->SendWhenReady(SendFragRefs(msg_to_send),
                                        TcpSendOption());
    }
  }
  ...
  // if (viz_agent_) {
  //  CScopedLock2 viz_agent_guard(viz_agent_->main_mutex_);
  //  viz_agent_->C2SProxy.NotifySrv_Clients_AddOrEdit(HostId_Server,
  //  GReliableSend_INTERNAL, rc->host_id_);
  //}
...
RemoteClient_S* NetServerImpl::GetCandidateRemoteClientByTcpAddr(
    const InetAddress& client_addr) {
  AssertIsLockedByCurrentThread();
...
bool NetServerImpl::AsyncCallbackMayOccur() { return listener_.IsValid(); }
...
void NetServerImpl::ProcessOneLocalEvent(LocalEvent& event) {
  ...
  if (callbacks_ /* && !tear_down_*/) {
    try {
      switch (event.type) {
        case LocalEventType::ClientLeaveAfterDispose:
          callbacks_->OnClientLeft(event.client_info.Get(),
        ...
        case LocalEventType::ClientJoinCandidate: {
          ByteArray response;
          const bool approved = callbacks_->OnConnectionRequest(
    ...
    //  }
    //}
#ifdef ALLOW_CATCH_UNHANDLED_EVEN_FOR_USER_ROUTINE  // 사용자가 잡아내는게 더
                                                    // 바람직하지 않을런지??
    catch (...) {
      if (callbacks_ /* && !tear_down_*/) {
        Exception e;
...
void NetServerImpl::ConditionalLogFreqFail() {
  ...
  if (free_fail_need_ &&
      (GetAbsoluteTime() - last_holepunch_freq_fail_logged_time_) > 30) {
    LogFreqFailNow();
  }
}
...
HostId NetServerImpl::GetLocalHostId() { return HostId_Server; }

HostId NetServerImpl::GetSrcHostIdByAddrAtDestSide_NOLOCK(
...
void NetServerImpl::HardDisconnect_AutoPruneGoesTooLongClient(
    RemoteClient_S* rc) {
  // 5초로 잡아야 서버가 클라에게 많이 보내고 있던 중에도 직전 RPC의 확실한
  // 송신이 되므로 시간으로 하는건 좀 아니지 싶은데.. 어쩔 수 없으려나??
  if (rc->request_auto_prune_start_time_ > 0 &&
      (GetAbsoluteTime() - rc->request_auto_prune_start_time_) >
          5.0) {  // TODO 5.0 이값도 NetConfig로 빼주는게 좋을듯...
...
void NetServerImpl::TEST_SetOverSendSuspectingThresholdInBytes(
    int32 threshold) {
  settings_.over_send_suspecting_threshold_in_byte = MathBase::Clamp(
      threshold, 0, NetConfig::default_over_send_suspecting_threshold_in_byte);
}

void NetServerImpl::TEST_SetTestping(HostId HostId_A, HostId HostId_B,
                                     double ping) {
...
void NetServerImpl::RefreshSendQueuedAmountStat(RemoteClient_S* rc) {
  ...
  if (rc->to_client_udp_fallbackable_.udp_socket &&
      rc->to_client_udp_fallbackable_.real_udp_enabled_) {
    CScopedLock2 rc_udp_fragger_guard(
        rc->to_client_udp_fallbackable_.udp_socket_->GetFraggerMutex());
    udp_queued_amount =
        rc->to_client_udp_fallbackable_.udp_socket_->packet_fragger_
            ->FromTotalPacketInByteByAddr(
                rc->to_client_udp_fallbackable_.GetUdpAddrFromHere());
  }
  ...
  if (rc->send_queue_warning_start_time_ != 0) {
    if (rc->send_queued_amount_in_byte_ >
        NetConfig::send_queue_heavy_warning_capacity) {
      if ((absolute_time - rc->send_queue_warning_start_time_) >
          NetConfig::send_queue_heavy_warning_time_sec) {
        EnqueueWarning(
            ResultInfo::From(ResultCode::SendQueueIsHeavy, rc->GetHostId(),
                             String::Format("send_queue_ %dBytes",
                                            rc->send_queued_amount_in_byte_)));
        rc->send_queue_warning_start_time_ = absolute_time;
      }
    } else {
      rc->send_queue_warning_start_time_ = 0;
    }
  } else if (rc->send_queued_amount_in_byte_ >
             NetConfig::send_queue_heavy_warning_capacity) {
    rc->send_queue_warning_start_time_ = absolute_time;
  }
}
...
void NetServerImpl::AllowEmptyP2PGroup(bool allow) {
  ...
  //@warning 서버가 이미 동작중일 경우에는 설정을 변경할 수 없다. 설정할 수 있게
  //한다고 하여도, 혼란만 가중시킬 것이다.
  if (AsyncCallbackMayOccur())  //@todo 이름이 좀?? {
    throw Exception(
        "cannot set AllowEmptyP2PGroup after the server has started.");
}

if (!allow && start_create_p2p_group_ &&
    dynamic_cast<AssignHostIdFactory*>(host_id_factory.Get()) == nullptr) {
...
int32 NetServerImpl::GetSuitableSuperPeerRankListInGroup(
    ...
    int32 out_ratings_buffer_count, const SuperPeerSelectionPolicy& policy,
    const Array<HostId>& excludees) {
  CScopedLock2 main_guard(main_mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
  ...
  if (auto group = GetP2PGroupByHostId_NOLOCK(group_id)) {
    ...
    for (int32 i = 0; i < group->ordered_super_peer_suitables.Count(); ++i) {
      if (out_rating_count < out_ratings_buffer_count &&
          excludees.Contains(group->ordered_super_peer_suitables[i].host_id) ==
              false) {
        out_ratings[out_rating_count++] =
            group->ordered_super_peer_suitables[i];
      }
    }
...
void NetServerImpl::TouchSuitableSuperPeerCalcRequest(
  ...
  for (auto& member_pair : group->members_) {
    ...
    if (rc) {
      if (rc->last_request_measure_send_speed_time_ == 0 ||
          (absolute_time - rc->last_request_measure_send_speed_time_ >
           NetConfig::measure_client_send_speed_interval_sec)) {
        s2c_proxy_.RequestMeasureSendSpeed(rc->host_id_, GReliableSend_INTERNAL,
                                           policy.send_speed_weight > 0);
  ...
  // 일단 부정확하더라도 수퍼피어 목록은 넣어야 최초 콜 결과가 HostId_None이
  // 리턴되는 불상사는 없다.
  if (group->ordered_super_peer_suitables.Count() == 0 &&
      group->members_.Count() > 0) {
    P2PGroup_RefreshMostSuperPeerSuitableClientId(group);
  }
}

void NetServerImpl::EnableVizAgent(const char* viz_server_ip,
...
void NetServerImpl::Viz_NotifySendByProxy(const HostId* rpc_sendto_list,
  ...
  //  UnsafeMemory::Memcpy(target.GetData(), rpc_sendto_list, sizeof(HostId) *
  //  rpc_sendto_count);
  //
  //  viz_agent_->C2SProxy.NotifyCommon_SendRpc(HostId_Server,
  //  GReliableSend_INTERNAL, target, summary);
  //}
}

void NetServerImpl::Viz_NotifyRecvToStub(HostId rpc_recvfrom, RpcId rpc_id,
                                         const char* rpc_name,
                                         const char* params_as_string) {
  // TODO
  // if (viz_agent_) {
  //  viz_agent_->C2SProxy.NotifyCommon_ReceiveRpc(HostId_Server,
  //  GReliableSend_INTERNAL, rpc_recvfrom, rpc_name, rpc_id);
  //}
}
...
bool NetServerImpl::RemoteClient_New_ToClientUdpSocket(RemoteClient_S* rc) {
  ...
  if (udp_assign_mode == ServerUdpAssignMode::PerClient) {
    ...
    if (!bound && port_num > 0) {
      ...
      EnqueueWarning(ResultInfo::From(
          ResultCode::AlreadyExists, rc->host_id_,
          "All UDP port numbers specified at server start are deleted.  An "
          "arbitrary port number is assigned instead."));
    ...
    // socket local 주소를 얻고 그것에 대한 매핑 정보를 설정한다.
    assigned_udp_socket->cached_local_addr_ =
        assigned_udp_socket->socket_->GetSockName();
    fun_check(assigned_udp_socket->cached_local_addr_.GetPort() > 0);
    local_addr_to_udp_socket_map_.Add(assigned_udp_socket->cached_local_addr_,
                                      assigned_udp_socket);

    assigned_udp_socket->socket_->SetCompletionContext(
        (ICompletionContext*)assigned_udp_socket.Get());
...
#ifdef ALLOW_ZERO_COPY_SEND  // zero copy send는 빠르지만 너무 많은 nonpaged를
                             // 유발 위험. 따라서 이걸로 막자. 막으니 성능도 더
                             // 나은데?
  ...
  /*
  Windows 2003의 경우는 아래와 같은지 모르겠으나, Windows 2016 Server에서는 Tcp
  접속시에 알아낸 클라이언트 주소로 UDP Dummy 패킷을 보내면 UDP 소켓이 차단되어,
  Receive는 되나 Send가 안된다. 즉, Send쪽이 차단 되고 있다. (wireshark를 통해서
  알아냄)
  ...
  if (!using_over_block_icmp_environment_) {
    opt.ttl = 0; // NRM1 공유기 등은 알수없는 인바운드 패킷이 오면 멀웨어로
  감지해 버린다. 따라서 거기까지 도착할 필요는 없다.
  }

  // Windows 2003의 경우, (포트와 상관없이) 서버쪽에서 클라에게 UDP패킷을 한번
  쏴줘야 해당 클라로부터 오는 패킷을 블러킹하지 않는다. MessageOut ignorant;
  ...
  */
  ...
  /*
  확인해 본바, TCP Accept시에 받은 주소로 UDP 메시지를 보내면 Send쪽이 먹통이
  되는 현상이 있었다. 이것 때문에 서버 홀펀칭이 안되는 문제가 발생한다. 포트를
  변경해서 해보자. 그런데, 예측한 포트가 UDP가 아닌 TCP 프로토콜 형태로 점유하고
  있다면 이 또한 Send쪽이 블럭이 되지 않을까 싶다.
  ...
  if (intra_logger_) {
    const String text = String::Format("Send dummy UDP message to %s(avoiding
  server blocking)", *send_to.ToString());
...
void NetServerImpl::RemoteClient_RequestStartServerHolepunch_OnFirst(
    RemoteClient_S* rc) {
  AssertIsLockedByCurrentThread();
...
void NetServerImpl::RemoteClient_NewLocalUdpSocketAndRequestNewRemoteUdpSocket(
    RemoteClient_S* rc) {
  if (rc->to_client_udp_fallbackable_.udp_socket_.IsValid() == false &&
    ...
    if (RemoteClient_New_ToClientUdpSocket(rc)) {
      // 보내기전 만들어서 보낸다.
      const auto addr = rc->to_client_udp_fallbackable_.udp_socket_
                            ->GetRemoteIdentifiableLocalAddr(rc);
      s2c_proxy_.S2C_RequestCreateUdpSocket(rc->host_id_,
                                            GReliableSend_INTERNAL, addr);
...
void NetServerImpl::EnqueueP2PAddMemberAckCompleteEvent(HostId group_id,
...
// MemberHostId이 들어있는 AddMemberAckWaiter 객체를 목록에서 찾아서 모두
// 제거한다.
void NetServerImpl::
    AddMemberAckWaiters_RemoveRelated_MayTriggerJoinP2PMemberCompleteEvent(
  ...
  // index i 를 통해서 group->add_member_ack_waiters.RemoveSwap 제거를 하는
  // 동작을 하므로, 아래와 같은 형태로 루프를 돌게 되면 문제가 생김.
  ...
  for (int32 i = group->add_member_ack_waiters_.Count() - 1; i >= 0; --i) {
    ...
    if (ack_waiter.joining_member_host_id == member_id ||
        ack_waiter.old_member_host_id == member_id) {
      deletee_list.Add(i);
  ...
  // MemberHostId에 대한 OnP2PGroupJoinMemberAckComplete 대기에 대한 콜백에 대한
  ...
  for (auto joining_member_id : joining_member_deletee_list) {
    if (!group->add_member_ack_waiters_.AckWaitingItemExists(
            joining_member_id)) {
      EnqueueP2PAddMemberAckCompleteEvent(group->group_id_, joining_member_id,
                                          reason);
    }
  }
}
...
void P2PPairList::AddPair(RemoteClient_S* rc_a, RemoteClient_S* rc_b,
                          P2PConnectionStatePtr state) {
...
void P2PPairList::ReleasePair(NetServerImpl* owner, RemoteClient_S* rc_a,
                              RemoteClient_S* rc_b) {
    ...
    if (--pair->dup_count == 0) {
      ...
      if (owner_->intra_logger_) {
        const String text = String::Format(
            ...
            (int32)rc_a->host_id_, (int32)rc_b->host_id_);
        owner_->intra_logger_->WriteLine(LogCategory::PP2P, *text);
      }
    }
  }
...
void P2PPairList::AddRecyclePair(RemoteClient_S* rc_a, RemoteClient_S* rc_b,
                                 P2PConnectionStatePtr state) {
  HostId a = rc_a->host_id_;
  HostId b = rc_b->host_id_;
...
void P2PPairList::RemoveTooOldRecyclePair(double absolute_time) {
  for (auto it = recyclable_pairs.CreateIterator(); it; ++it) {
    ...
    if ((absolute_time - state->release_time_) >
        NetConfig::remove_too_old_recycle_pair_time_sec) {
      // 아마도 이미 다 해제된 후로 들어가니까 안해도 될거다.
      // state->first_client->p2p_connection_pairs_.Remove(state);
      // state->second_client->p2p_connection_pairs_.Remove(state);
...
P2PConnectionStatePtr P2PPairList::GetRecyclePair(HostId a, HostId b,
                                                  bool remove) {
  // TODO 이건 왜해야할까??
  if (a > b) {
    Swap(a, b);
...
NetServer* NetServer::New() { return new NetServerImpl(); }
...
bool P2PConnectionState::SetRecycleSuccess(HostId peer_id) {
  for (int32 i = 0; i < 2; ++i) {
    if (peer_holepunched_infos[i].id == peer_id) {
      SetServerHolepunchOk(peer_id, peer_holepunched_infos[i].internal_addr,
                           peer_holepunched_infos[i].external_addr);
      return true;
    }
  }
...
void P2PConnectionState::SetServerHolepunchOk(
    HostId peer_id, const InetAddress& internal_addr,
...
P2PConnectionState::P2PConnectionState(NetServerImpl* owner,
...
bool P2PConnectionState::GetRelayed() { return is_relayed_; }
...
void P2PConnectionState::SetPeerHolepunchOk(HostId peer_id,
...
//@todo SuperPeerSelectionPolicy::Ordinary, SuperPeerSelectionPolicy::Null로
//대체 하도록하자. 함수 이름이 다소 길다!
...
SuperPeerSelectionPolicy SuperPeerSelectionPolicy::GetOrdinary() {
  ...
  // 게임방 멤버가 4명 이상인 경우 서버와의 핑보다 피어와의 핑의 가중치가
  // 상회함을 의미한다.
  ...
  // ServerLagWeight보다 약간더 우선순위가 크다. (최고 60프레임정도로 보았을때
  // 240)
...
ITaskSubject* NetServerImpl::GetTaskSubjectByHostId_NOLOCK(
...
void NetServerImpl::CheckCriticalSectionDeadLock_INTERNAL(const char* where) {
  ...
  if (tcp_issue_queue_mutex_.IsLockedByCurrentThread()) {
    LOG(LogNetEngine, Warning, "NetServer tcp_issue_queue_mutex_ deadlock - %s",
        where);
  }

  if (udp_issue_queue_mutex_.IsLockedByCurrentThread()) {
    LOG(LogNetEngine, Warning, "NetServer udp_issue_queue_mutex_ deadlock - %s",
        where);
  }
}

void NetServerImpl::OnSocketIoCompletion(Array<IHostObject*>& send_issued_pool,
...
void NetServerImpl::IoCompletion_PerRemoteClient(
    CompletionStatus& completion, ReceivedMessageList& msg_list) {
  AssertIsNotLockedByCurrentThread();

  RemoteClient_S* rc = nullptr;

  // NOTE: Debug모드에서는 디버거에 의존해서 작업하는게 훨씬더 편하므로, 일단
  // 제껴둠..
#ifndef _DEBUG
  try
#endif {
  ...
  CatchThreadExceptionAndPurgeClient(rc, __FUNCTION__,
                                     (const char*)UTF8_TO_TCHAR(e.what()));
}
// catch (_com_error& e) {
//  CatchThreadExceptionAndPurgeClient(rc, __FUNCTION__, (const
//  char*)e.Description());
//} catch (void* e) {
//  e;
...
void NetServerImpl::IoCompletion_TcpCustomValueCase(
    CompletionStatus& completion, RemoteClient_S* rc) {
  AssertIsNotLockedByCurrentThread();
...
void NetServerImpl::IoCompletion_NewClientCase(RemoteClient_S* rc) {
  ...
  // 굳이 zero copy send로 수정하고자 한다면 아래 보내기 기능도 non blocked
  // send로 로직을 바꾸어야 하겠다.
  //@todo 의도가 무엇인가??? {
...
// 이부분을 안보냈더니, 클라이언트에 이미 첫번째 policy문자열을 읽어주는 부분이
// 있으므로, 메시지를 제대로 수신하지 못하고 있었음. 진짜로 필요한지 여부를
// 재판단해보고, 만약 필요 없다면, 서버와 클라이언트 모두 수정해주어야함!!
...
while (send_offset < poicy_file_text_length) {
  ...
  const SocketErrorCode socket_error = rc->to_client_tcp_->socket_->BlockedSend(
      ((uint8*)POLICY_FILE_TEXT.ConstData()) + send_offset,
      poicy_file_text_length - send_offset);
  if (socket_error == SocketErrorCode::error) {
    ...
    rc->IssueDispose(ResultCode::DisconnectFromRemote,
  ...
  send_offset += (int32)socket_error;  //@todo 음?? 에러코드 이기전에, 전송한
                                       //길이이기도 하다. (에러가 아닐 경우)
}
#endif
...
void NetServerImpl::IoCompletion_TcpSendCompletionCase(
    CompletionStatus& completion, RemoteClient_S* conn) {
  AssertIsNotLockedByCurrentThread();
  rc->AssertIsNotLockedByCurrentThread();
  ...
  if (completion.completed_length < 0) {  // 0도 인정?
    ...
    IssueDisposeRemoteClient(rc, ResultCode::DisconnectFromRemote,
                             ResultCode::TCPConnectFailure, ByteArray(),
                             *comment, completion.socket_error);
  } else {
    // 송신 큐에서 완료된 만큼의 데이터를 제거한다. 그리고 다음 송신을 건다.
    {
      CScopedLock2 rc_tcp_send_queue_guard(
          rc->to_client_tcp_->GetSendQueueMutex());
      rc->to_client_tcp_->send_queue_.DequeueNoCopy(
          completion.completed_length);
    }

    // 보낼게 남아있다면, 재 송신요청.
    const SocketErrorCode socket_error =
        rc->to_client_tcp_->ConditionalIssueSend(GetAbsoluteTime());
    if (socket_error != SocketErrorCode::Ok) {
...
void NetServerImpl::IoCompletion_TcpRecvCompletionCase(
    CompletionStatus& completion, RemoteClient_S* rc,
    ReceivedMessageList& msg_list) {
  AssertIsNotLockedByCurrentThread();
  rc->AssertIsNotLockedByCurrentThread();
  ...
  if (completion.completed_length <= 0) {  // disconnected
    ...
    if (intra_logger_) {
      const String text = String::Format(
          "Disconnect connection.  host_id: %d, remote_addr: %s, socket_error: "
          "%s",
          (int32)rc->host_id_, *rc->to_client_tcp_->remote_addr_.ToString(),
          *ToString(completion.socket_error));
      intra_logger_->WriteLine(LogCategory::System, *text);
    }
  } else {
    // 수신 큐에서 받은 데이터를 꺼낸 후 ...
    const uint8* RecviedData = rc->to_client_tcp_->socket_->GetRecvBufferPtr();
    rc->to_client_tcp_->recv_stream_.EnqueueCopy(RecviedData,
                                                 completion.completed_length);

    // 완전한 msg가 도착한 것들을 모두 추려서 final recv queue로 옮기거나 여기서
    // 처리한다.
    ...
    if (error != ResultCode::Ok) {
      ...
      IssueDisposeRemoteClient(rc, error, ResultCode::TCPConnectFailure,
                               ByteArray(), __FUNCTION__,
                               SocketErrorCode::Ok);  // InduceDisconnect()
      return;  // return 하는게 맞는듯 하다...어차피 dis니까..문제가 있으면
               // 수정토록 하자.
    }
    ...
    // 다음 recv를 건다.
    const SocketErrorCode socket_error =
        rc->to_client_tcp_->IssueRecvAndCheck();
    if (socket_error != SocketErrorCode::Ok) {
...
// FIXME 현재 UDP를 통해서 수신한 경우, extracted_msg_list 객체가 깨지는 문제가
// 있음.  왜일까??
void NetServerImpl::IoCompletion_ProcessMessageOrMoveToFinalRecvQueue(
...
bool NetServerImpl::IoCompletion_ProcessMessage_EngineLayer(
  ...
  const String debug_str = String::Format(
      "[%05d] [%s] [%s] [%s] %s (LEN=%d)", (int32)received_msg.remote_id,
      *remote_ip, is_real_udp ? "UDP" : "TCP",
      received_msg.relayed ? "[Relayed]" : "NonRelayed", *ToString(msg_type),
      msg.GetLength());
  ...
  switch (msg_type) {
    case MessageType::ServerHolepunch:
      // 이 메시지는 아직 RemoteClient가 배정되지 않은 클라이언트에 대해서도
      // 작동한다.
      ASSERT_OR_HACKED(received_msg.remote_addr_udp_only.IsUnicast());
      IoCompletion_ProcessMessage_ServerHolepunch(
          received_msg.unsafe_message, received_msg.remote_addr_udp_only,
          udp_socket);
      msg_processed = true;
      break;
    ...
    case MessageType::UnreliableRelay1_RelayDestListCompressed:
      IoCompletion_ProcessMessage_UnreliableRelay1_RelayDestListCompressed(msg,
                                                                           rc);
      msg_processed = true;
      break;
    ...
    case MessageType::RPC:
      IoCompletion_ProcessMessage_RPC(received_msg, msg_processed, rc,
                                      is_real_udp);
      break;
    ...
    case MessageType::Encrypted_Unreliable: {
      ...
      if (DecryptMessage(msg_type, received_msg,
                         decrypted_received_msg.unsafe_message)) {
        decrypted_received_msg.relayed = received_msg.relayed;
        decrypted_received_msg.remote_addr_udp_only =
            received_msg.remote_addr_udp_only;
        decrypted_received_msg.remote_id = received_msg.remote_id;
        msg_processed |= IoCompletion_ProcessMessage_EngineLayer(
            decrypted_received_msg, rc, udp_socket);  // recursive call
      }
    } break;

    case MessageType::Compressed: {
      ReceivedMessage decompressed_received_msg;
      if (DecompressMessage(received_msg,
                            decompressed_received_msg.unsafe_message)) {
        decompressed_received_msg.relayed = received_msg.relayed;
        decompressed_received_msg.remote_addr_udp_only =
            received_msg.remote_addr_udp_only;
        decompressed_received_msg.remote_id = received_msg.remote_id;
        msg_processed |= IoCompletion_ProcessMessage_EngineLayer(
            decompressed_received_msg, rc, udp_socket);  // recursive call
      }
      break;
    }

    case MessageType::RequestReceiveSpeedAtReceiverSide_NoRelay:
      IoCompletion_ProcessMessage_RequestReceiveSpeedAtReceiverSide_NoRelay(msg,
                                                                            rc);
      msg_processed = true;
      break;

    case MessageType::ReplyReceiveSpeedAtReceiverSide_NoRelay:
      IoCompletion_ProcessMessage_ReplyReceiveSpeedAtReceiverSide_NoRelay(
          received_msg, rc);
      msg_processed = true;
      break;
  ...
  if (msg_processed && l1 != l2 &&
    ...
    // 에러시에 마지막 메세지를 저장한다.
    // 복사 오버헤드가 있겠지만, 에러시에만 하는 동작이므로 크게 문제가 있어
    // 보이지는 않음. (단 에러가 자주 발생해서는 안되겠지..)
    // ByteArray last_received_msg(msg.GetData(), msg.GetLength());
    // EnqueueError(ResultInfo::From(ResultCode::InvalidPacketFormat,
    // received_msg.remote_id, __FUNCTION__, last_received_msg));

    // TODO 문제를 일으킨 패킷 데이터를 넘길때 전체를 넘기는게 맞는건지? 아니면
    // 유효한 부분만 넘겨야하는건지? 최종적으로 검증용이니, 전체를 넘기는게
    //맞겠지??
...
클라이언트 -> 서버로 서버와의 홀펀칭을 하고 싶다고 요청한 상태에서 호출되는
녀석임.
...
*/
void NetServerImpl::IoCompletion_ProcessMessage_ServerHolepunch(
  ...
  const auto filter_tag =
      FilterTag::Make(HostId_Server, HostId_None);  // server -> unknown
  const UdpSendOption send_opt(MessagePriority::Holepunch, EngineOnlyFeature);
  udp_socket->SendWhenReady(HostId_None, filter_tag, from,
                            SendFragRefs(msg_to_send), send_opt);
  ...
  if (intra_logger_) {
    const String text =
        String::Format("ServerHolepunchAck from %s", *from.ToString());
    intra_logger_->WriteLine(LogCategory::SP2P, *text);
    ;
  }
}
...
*/
void NetServerImpl::IoCompletion_ProcessMessage_PeerUdp_ServerHolepunch(
    MessageIn& msg, const InetAddress& from, UdpSocket_S* udp_socket) {
  AssertIsNotLockedByCurrentThread();

  if (udp_socket == nullptr) {
    // 버그 혹은 패킷이 깨져서 발생하는 경우임.
    // 패킷이 깨진 경우, 메시지 타입 오식별로 인해서 UdpSocket이 null인 상태에서
    // 도달가능함.
    TRACE_SOURCE_LOCATION();
    return;
  }
  ...
  if (intra_logger_) {
    const String text =
        String::Format("PeerUdp_ServerHolepunch from %s", *from.ToString());
    intra_logger_->WriteLine(LogCategory::SP2P, *text);
  }
}

void NetServerImpl::IoCompletion_ProcessMessage_NotifyCSEncryptedSessionKey(
  ...
  SharedPtr<ResultInfo> error = CryptoRSA::DecryptSessionKeyByPrivateKey(
  ...
  // 하자. Candidate 목록이 비대 하지는 않을테니, 검색 속도에 문제는 없으려나??
  ...
  if (!CryptoAES::ExpandFrom(
          rc->session_key_.aes_key, (const uint8*)out_random_block.ConstData(),
          settings_.strong_encrypted_message_key_length / 8) ||
      !CryptoAES::Decrypt(rc->session_key_.aes_key, encrypted_rc4_key_blob,
                          out_rc4_random_block) ||
      !CryptoRC4::ExpandFrom(rc->session_key_.rc4_key,
                             (const uint8*)out_rc4_random_block.ConstData(),
                             settings_.weak_encrypted_message_key_length / 8)) {
    callbacks_->OnException(HostId_None,
                            Exception("failed to create session-key"));
...
void NetServerImpl::
  ...
  // 미인증 클라이언트 여부 체크
  //@note 이곳에서 미인증에서 인증 상태의 클라이언트로 바뀌게 되므로, 우선
  //미인증 클라이언트인지 부터 체크 하도록 하자. if
  // (!candidate_remote_clients_.ContainsValue(rc)) //@warning 결과적으로, 본
  // 메시지는 한번만 와야함. 두번째 부터는 실패함.
  if (candidate_remote_clients_.FindKey(rc) ==
      nullptr) {  //@warning 결과적으로, 본 메시지는 한번만 와야함. 두번째
                  //부터는 실패함.
    NotifyProtocolVersionMismatch(rc);
    return;  // 오류로 인해서 리턴하면, 해당 RC는 방치 상태가 되는데, 방치된
             // RC는 일정시간 이후 시스템에서 제거 될것이므로 괜찮음.
  }
  ...
  if (!lf::Reads(msg, msg_user_data, msg_protocol_version,
                 msg_internal_version)) {
    NotifyProtocolVersionMismatch(rc);
    return;  // 오류로 인해서 리턴하면, 해당 RC는 방치 상태가 되는데, 방치된
             // RC는 일정시간 이후 시스템에서 제거 될것이므로 괜찮음.
  }
  ...
  if (callbacks_) {
    // 콜백이 지정된 경우, 콜백내에서 다소 무거운(DB query 같은) 작업을 통해서
    // 인증을 처리해야할 수 있으므로, 여기서 바로 처리하지 않고, 유저
    // 스레드풀에서 처리가 이루어지도록 함.
    EnqueueClientJoinApproveDetermine(rc->to_client_tcp_->remote_addr_,
                                      msg_user_data);
  } else {
...
// 있도록 힌트를 제공함.
void NetServerImpl::IoCompletion_ProcessMessage_RequestServerConnectionHint(
    MessageIn& msg, RemoteClient_S* rc) {
  CScopedLock2 main_guard(main_mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
...
void NetServerImpl::IoCompletion_ProcessMessage_RequestServerTimeAndKeepAlive(
    MessageIn& msg, RemoteClient_S* rc) {
  AssertIsNotLockedByCurrentThread();
  rc->AssertIsZeroUseCount();
  ...
  rc->to_client_udp_fallbackable_.SendWhenReady(
      rc->GetHostId(), SendFragRefs(msg_to_send), send_opt);
}

void NetServerImpl::IoCompletion_ProcessMessage_SpeedHackDetectorPing(
    MessageIn& msg, RemoteClient_S* rc) {
  AssertIsNotLockedByCurrentThread();
  rc->AssertIsZeroUseCount();
  rc->AssertIsNotLockedByCurrentThread();
...
void NetServerImpl::IoCompletion_ProcessMessage_UnreliableRelay1(
    MessageIn& msg, RemoteClient_S* rc) {
  AssertIsNotLockedByCurrentThread();
  ...
  if (payload_length < 0 || payload_length >= settings_.message_max_length) {
    // 엔진 버그 아니면 해커의 소행일 수 있음. 해커의 소행이라면 릴리즈 빌드로
    // 띄우자.
    TRACE_SOURCE_LOCATION();
  ...
  CScopedLock2 main_guard(main_mutex_);
  IoCompletion_MulticastUnreliableRelay2_AndUnlock(
      &main_guard, relay_dest_list, rc->host_id_, payload, priority, unique_id);
}

void NetServerImpl::
    IoCompletion_ProcessMessage_UnreliableRelay1_RelayDestListCompressed(
        MessageIn& msg, RemoteClient_S* rc) {
  AssertIsNotLockedByCurrentThread();
  ...
  Array<P2PGroupSubset_S, InlineAllocator<256>> p2p_group_subset_list(
      p2p_group_subset_list_count);
  ...
  for (int32 i = 0; i < p2p_group_subset_list.Count(); ++i) {
    ...
    if (auto group = GetP2PGroupByHostId_NOLOCK(p2p_dest.group_id)) {
      ...
      // auto ItA = p2p_dest.excludee_host_id_list.begin(); // DirectP2P로 이미
      // 보낸 리스트 auto ItB = group->members_.begin(); // P2P 전체 인원 리스트
      //
      //// merge sort와 비슷한 방법으로, group member들로부터 excludee를 제외한
      ///것들을 찾는다.
      // for (; ItA != p2p_dest.excludee_host_id_list.end() || ItB !=
      // group->members_.end();) {
      ...
      // TODO 이미 정렬된 상태에서는 처리가 위에서처럼 빠르지만, 코드 정리가
      // 아직 안되었고 group->Members가 아직 ordered-map이 아니므로 일단은
      // 아래처럼 처리.

      for (auto member_pair : group->members_) {
        if (!p2p_dest.excludee_host_id_list.Contains(
                member_pair
                    .key)) {  // 제외 대상에 없는 경우에만 최종 목록에 추가함.
  ...
  IoCompletion_MulticastUnreliableRelay2_AndUnlock(
      &main_guard, final_relay_dest_list, rc->host_id_, payload, priority,
      unique_id);
}

void NetServerImpl::IoCompletion_ProcessMessage_ReliableRelay1(
    MessageIn& msg, RemoteClient_S* rc) {
  AssertIsNotLockedByCurrentThread();
  ...
  if (relay_count == 0) {
    TRACE_SOURCE_LOCATION();
    // TODO 실질적으로는 오류임.  왜냐 릴레이를 시켜주어야하는데, 릴레이 대상이
    // 지정되지 않았다는 것이니...
    return;
  }
  ...
  for (int32 i = 0; i < relay_count; ++i) {
    ...
    if (dest_info.sendto_rc) {
      ...
      // LOG(LogNetEngine,Warning,"cannot find out relay target. remote: %d",
      // (int32)dst.send_to);
    }
  }
...
#pragma omp parallel for
    ...
    for (int32 dst_index = 0; dst_index < add_count; ++dst_index) {
      ...
      CScopedLock2 rc_tcp_send_queue_guard(
          dst.sendto_rc->to_client_tcp_->GetSendQueueMutex(), false);

      // LOG(LogNetEngine,Warning,"MessageType::ReliableRelay2: sender: %d,
      // receiver: %d, frame: %d, payload_length: %d", (int32)remote,
      // (int32)dst.sendto_id, (int32)dst.frame_number, payload_length);
      ...
      if (dst_index != 0) {  // DestIndex가 0이 아닌 경우가 더 많을테니 0이 아닌
                             // 경우를 먼저 체크함.
        if (rc_tcp_send_queue_guard.TryLock()) {
          dst.sendto_rc->to_client_tcp_->SendWhenReady(data_to_send,
                                                       TcpSendOption());
          rc_tcp_send_queue_guard.Unlock();
...
void NetServerImpl::IoCompletion_ProcessMessage_LingerDataFrame1(
    MessageIn& msg, RemoteClient_S* rc) {
  AssertIsNotLockedByCurrentThread();
  rc->AssertIsZeroUseCount();
  ...
  // ReliableUDP 프레임 전송중 Relay로 전환된 상태에서, 해당 피어에게 릴레이
  // 시켜주는 것이므로, 목적지 피어에게 TCP를 통해서 보내줌.
  MessageOut header;
  ...
  {
    CScopedLock2 rc_tcp_send_queue_guard(
        dest_conn->to_client_tcp_->GetSendQueueMutex());
...
void NetServerImpl::IoCompletion_ProcessMessage_NotifyHolepunchSuccess(
    MessageIn& msg, RemoteClient_S* rc) {
  AssertIsNotLockedByCurrentThread();
  rc->AssertIsZeroUseCount();
  ...
  if (rc->to_client_udp_fallbackable_.holepunch_tag_ == tag &&
    ...
    if (intra_logger_) {
      const String text = String::Format(
          "Client %d succeeded in hole punching to the server.  "
          "client_local_addr: %s, client_addr_at_server: %s",
          (int32)rc->host_id_, *client_local_addr.ToString(),
          *client_addr_from_here.ToString());
      intra_logger_->WriteLine(LogCategory::SP2P, *text);
    }
  }
}

void NetServerImpl::IoCompletion_ProcessMessage_PeerUdp_NotifyHolepunchSuccess(
    MessageIn& msg, RemoteClient_S* rc) {
  AssertIsNotLockedByCurrentThread();
  rc->AssertIsZeroUseCount();
  ...
  auto p2p_conn_state =
      p2p_connection_pair_list_.GetPair(peer_id, rc->host_id_);
  ...
  const int32 holepunch_ok_count_old =
      p2p_conn_state->GetServerHolepunchOkCount();

  p2p_conn_state->SetServerHolepunchOk(rc->host_id_, client_local_addr,
                                       client_addr_from_here);
  if (p2p_conn_state->GetServerHolepunchOkCount() != 2 ||
      holepunch_ok_count_old != 1) {
    // LOG(LogNetEngine,Trace, "#### %d %d ####",
    // p2p_conn_state->GetServerHolepunchOkCount(), holepunch_ok_count_old);
    return;
  }
  ...
  s2c_proxy_.RequestP2PHolepunch(
      rc->host_id_, GReliableSend_INTERNAL, rc2->host_id_,
      p2p_conn_state->GetInternalAddr(rc2->host_id_),
      p2p_conn_state->GetExternalAddr(rc2->host_id_));

  // send to rc2
  s2c_proxy_.RequestP2PHolepunch(rc2->host_id_, GReliableSend_INTERNAL,
                                 rc->host_id_,
  ...
  if (intra_logger_) {
    const String text = String::Format(
        "Completed first step for hole punching between client "
        "%d(external_addr: %s) and client %d(external_addr: %s).",
        (int32)rc->host_id_,
...
void NetServerImpl::IoCompletion_ProcessMessage_RPC(
  ...
  // 일단은, 내부 RPC 메시지 일 수 있으므로, C2SStub에게 먼저 처리할 기회를
  // 줘보고, 처리가 되지 않는 다면(관심없는) 유저 RPC Stub쪽에 기회를 주도록
  // 한다.
  msg_processed |=
      c2s_stub_.ProcessReceivedMessage(received_msg, rc->host_tag_);

  // 내부 RPC C2SStub에서 처리되지 않았으므로, 유저 RPC Sutb쪽으로 넘겨주도록
  // 한다.
  if (!msg_processed) {
    // c2s_stub_.ProcessReceivedMessage에서 처리가 안되었을 경우에도, 읽기
    // 위치가 변경되었을 수 있으므로, 저장된 위치로 복원해주어야함. 그러나,
    // 실제로는 c2s_stub_.ProcessReceivedMessage에서 처리가 안되면, 내부적으로
    // 읽기 위치를 원래대로 복원해주므로 이중으로 복원해주는 꼴이다. 만약을
    // 위해서 일단은 유지하도록 하자.
...
void NetServerImpl::IoCompletion_ProcessMessage_FreeformMessage(
  ...
  // 이거 치우거나 MainLock아래로 내리지 말것...no lock
  // CReceivedMessage파괴되면서 refcount꼬임.
  // TODO 딱히 블럭을 잡아줄 필요가 없어보이는데??
  {
    ...
    UserTaskQueue_Add(rc, user_msg, FinalUserWorkItemType::FreeformMessage,
                      is_real_udp);
  }
}

void NetServerImpl::
    IoCompletion_ProcessMessage_RequestReceiveSpeedAtReceiverSide_NoRelay(
        MessageIn& msg, RemoteClient_S* rc) {
  AssertIsNotLockedByCurrentThread();
  ...
  if (rc->to_client_udp_fallbackable_.udp_socket) {
    double speed = 0;
    {
      CScopedLock2 udp_fragger_guard(
          rc->to_client_udp_fallbackable_.udp_socket_->GetFraggerMutex());
      speed = rc->to_client_udp_fallbackable_.udp_socket_->packet_defragger_
                  ->GetRecentReceiveSpeed(
                      rc->to_client_udp_fallbackable_.GetUdpAddrFromHere());
    }
    ...
    if (speed > 0) {
      ...
      lf::Write(msg_to_send,
                MessageType::ReplyReceiveSpeedAtReceiverSide_NoRelay);
...
void NetServerImpl::
  ...
  // TODO 패킷이 깨져서 온 경우, rc->to_client_udp_fallbackable_.udp_socket
  // 객체가 null일 경우 서버가 크래쉬 될 수 있음. 밑에 부분에서 해당 객체를
  //접근하고 있으므로, access-violation이 발생함.
  //
  ...
  CScopedLock2 udp_fragger_guard(
      rc->to_client_udp_fallbackable_.udp_socket_->GetFraggerMutex());
  rc->to_client_udp_fallbackable_.udp_socket_->packet_fragger_
      ->SetReceiveSpeedAtReceiverSide(
          rc->to_client_udp_fallbackable_.GetUdpAddrFromHere(), recv_speed);
}

void NetServerImpl::UserTaskQueue_Add(RemoteClient_S* rc,
  ...
  rc->final_user_work_queue_.Enqueue(
...
void NetServerImpl::IoCompletion_UdpConnectee(CompletionStatus& completion,
                                              ReceivedMessageList& msg_list) {
...
void NetServerImpl::IoCompletion_UdpSendCompletionCase(
  ...
  // wsasend자체가 udpsocket의 lock로 보호되며, 1번 wsasend에 1번 completion이
  // 올것이므로,
...
// TODO message_list를 인자로 넘겨진것을 사용해야할지? 로컬을 잡아서
// 처리해야할지?
void NetServerImpl::IoCompletion_UdpRecvCompletionCase(
    CompletionStatus& completion, UdpSocket_S* udp_socket,
    ReceivedMessageList& MessageList00) {
  AssertIsNotLockedByCurrentThread();
  ...
  if (completion.completed_length > 0) {
...

    {
      ...
      src_host_id =
          GetSrcHostIdByAddrAtDestSide_NOLOCK(completion.recvfrom_addr);
...

    {
      ...
      assembling_result =
          udp_socket->packet_defragger_->PushFragmentAndPopAssembledPacket(
              ...
              completion.completed_length, completion.recvfrom_addr,
              src_host_id, absolute_time, assembled_packet, out_error);
    }
    ...
    if (assembling_result == UdpPacketDefragger::AssembledPacketError::Ok) {
      if (rc) {
        try {
          ...
          // 완전한 msg가 도착한 것들을 모두 추려서 final recv queue로 옮기거나
          // 여기서 처리한다.
          ...
          rc->ExtractMessagesFromUdpRecvQueue(packet_data, packet_len,
                                              udp_addr_from_here_,
                                              message_max_length, msg_list);

          IoCompletion_ProcessMessageOrMoveToFinalRecvQueue(rc, msg_list,
                                                            udp_socket);
        } catch (Exception& e) {
          CatchThreadExceptionAndPurgeClient(
              rc, __FUNCTION__, *String::Format("Exception(%s)", *e.Message()));
        } catch (std::exception& e) {
          CatchThreadExceptionAndPurgeClient(
              rc, __FUNCTION__,
        ...
        // 아직 등록 안된 remote로부터 도착한거다. 단순 에코 등의 메시지일 수
        // 있으므로 별도 처리한다. coalesce를 감안해서 처리한다.
        // ReceivedMessageList extracted_msg_list;
        // msg_list.EmptyAndKeepCapacity();
        ...
        if (added_count >= 0) {
          for (auto& received_msg : msg_list) {
            ...
            IoCompletion_ProcessMessage_FromUnknownClient(
                assembled_packet.SenderAddr, received_msg.unsafe_message,
                udp_socket);
          }
        } else {
          // 잘못된 스트림 데이터이다. UDP인 경우에는 모두 처리된 것처럼
          // 간주하고 그냥 무시해버린다.
        }
      }
    } else if (assembling_result ==
               UdpPacketDefragger::AssembledPacketError::error) {
      CScopedLock2 main_guard(main_mutex_);
      CheckCriticalSectionDeadLock(__FUNCTION__);
...
bool NetServerImpl::IoCompletion_ProcessMessage_FromUnknownClient(
    const InetAddress& from, MessageIn& incomming_msg,
    UdpSocket_S* udp_socket) {
  AssertIsNotLockedByCurrentThread();
  ...
  switch (msg_type) {
    case MessageType::ServerHolepunch:
      IoCompletion_ProcessMessage_ServerHolepunch(incomming_msg, from,
                                                  udp_socket);
      msg_processed = true;
      break;

    case MessageType::PeerUdp_ServerHolepunch:
      IoCompletion_ProcessMessage_PeerUdp_ServerHolepunch(incomming_msg, from,
                                                          udp_socket);
      msg_processed = true;
      break;
  }
...
// caller는 이미 main 잠금을 only 1 recursion 상태로 이 함수를 콜 한 상태이어야
// 한다.
void NetServerImpl::IoCompletion_MulticastUnreliableRelay2_AndUnlock(
  ...
  Array<UnreliableDestInfo, InlineAllocator<256>> relay_dest_list(
      relay_count);  // warning: must be initialized!

  // RC를 얻어내야하기 때문에 main lock이 필요하다.
  // 단, 콜러에서 이미 잠금을 한 상태이면(당연히 recursion count = 1이어야) 굳이
  // 또 잠그지 않는다. 이 함수의 하단에서 멀티코어를 사용하기 위해 잠금을 풀어야
  // 하니까.
  ...
  for (int32 relay_index = 0; relay_index < relay_count; ++relay_index) {
    ...
    if (auto dest_conn = GetAuthedClientByHostId_NOLOCK(dest_id)) {
      ...
      if (dest_conn->to_client_udp_fallbackable_.real_udp_enabled_) {
        ...
        dest_info.sendto_rc =
            dest_conn->to_client_udp_fallbackable_.udp_socket_.Get();
        dest_info.sendto_addr =
            dest_conn->to_client_udp_fallbackable_.GetUdpAddrFromHere();
      } else {
  ...
  // 잠금을 푼다.
  main_mutex->Unlock();
  // 이 스레드는 잠그고 있지 않음이 보장되어야 아래 멀티코어 멀티캐스트가 효력을
  // 낸다.
  AssertIsNotLockedByCurrentThread();
  ...
  // TODO 참조로 넘길 수 있다면 복사 비용을 대폭 줄일 수 있을터...
  // CSendFragRefs는 전반적으로 고민을 해보아야할듯!!
  SendFragRefs data_to_send;
  data_to_send.Add(header);
  data_to_send.Add(payload);
...
#pragma omp parallel for
    ...
    for (int32 dst_index = 0; dst_index < add_count; ++dst_index) {
      ...
      if (dst_index != 0) {
        if (SendLock.TryLock()) {
          dst.sendto_rc->SendWhenReady(dst.sendto_id, dst.sendto_addr,
                                       dst.sendto_id, data_to_send, send_opt);
          SendLock.Unlock();
...
void NetServerImpl::CatchThreadExceptionAndPurgeClient(RemoteClient_S* rc,
                                                       const char* where,
                                                       const char* reason) {
  CScopedLock2 main_guard(main_mutex_);
  CheckCriticalSectionDeadLock(__FUNCTION__);
  ...
  if (rc) {
    if (callbacks_) {
      // TODO 메시지를 영문으로 수정하도록 하자.  오류 코드를 구체화할수 있어도
      // 좋을듯...
      const String text =
          String::Format("%s에서 %s 이유로 클라이언트 %d를 추방합니다.", where,
                         reason, (int32)rc->host_id_);
      EnqueueError(
          ResultInfo::From(ResultCode::Unexpected, rc->host_id_, text));
    }

    IssueDisposeRemoteClient(rc, ResultCode::Unexpected,
                             ResultCode::TCPConnectFailure, ByteArray(),
                             __FUNCTION__, SocketErrorCode::Ok);
  } else {
    if (callbacks_) {
...
void NetServerImpl::PostHeartbeatIssue() {
  ...
  net_thread_pool_->PostCompletionStatus(this,
                                         (UINT_PTR)IocpCustomValue::Heartbeat);
}
...
// 사용자가 핸들링하지 않은 에러는 차라리 unhandled exception으로 이어지게 해서
// 미니덤프를 남기는 것이 conceal보다 훨씬 문제해결을 빨리 해낸다.
#if EXPOSE_CRASH_THAN_CONCEAL_EXCEPTION  //예외를 숨기는것 보다는 차라리
                                         //노출하는게 좀더 유리하다?
  } catch (Exception& e) {
...
void NetServerImpl::EveryRemote_IssueSendOnNeed(Array<IHostObject*>& pool) {
  ...
  int32 total_parallel_count = udp_issued_send_ready_list_.Count() +
                               tcp_issue_send_ready_remote_clients_.Count();
  ...
  // 일단은 임시큐 두개로, 병렬 for을 한다. 하지만 1개 큐가 더나을수도 있으므로
  // 수정여지는 있음.
...
#endif
#endif
    for (int32 object_index = 0; object_index < total_parallel_count;
         ++object_index) {
...
void NetServerImpl::PostEveryRemote_IssueSend() {
  net_thread_pool_->PostCompletionStatus(
      this, (UINT_PTR)IocpCustomValue::SendEnqueued);
}

// TODO 사용자 정의 루틴을 호출하므로 막아주어야할듯 싶은데??
void NetServerImpl::OnTick() {
  if (callbacks_ /*&& !owner_->tear_down_*/) {
    // NOTE: 중복 실행되는 경우가 없도록 설계 되었으므로, 아래와 같은 처리를 할
    // 필요가 없음. 중복 호출이 안되도록 막아주어야함.
...
void NetServerImpl::DoUserTask() {
  ...
  do {
    void* host_tag = nullptr;
    {
      ...
      running =
          user_task_queue_.PopAnyTaskNotRunningAndMarkAsRunning(uwi, &host_tag);
    }
    ...
    if (running) {
      ...
      switch (uwi.type) {
        ...
        case FinalUserWorkItemType::FreeformMessage:
          ...
          // 동시에 다른 작업도 병행이 가능한건가?? 아무리 스레드 콜백이라고
          // 해도 말이다.. 조금더 확인해보도록 하자. 콜백을 받아서 처리하는건
          // 아무래도 싱크를 맞추기가 쉽지 않다는 문제는 여전히 있다.
...
void NetServerImpl::UserWork_FinalReceiveRPC(FinalUserWorkItem& uwi,
                                             void* host_tag) {
  AssertIsNotLockedByCurrentThread();
...
void NetServerImpl::UserWork_FinalReceiveFreeformMessage(FinalUserWorkItem& uwi,
                                                         void* host_tag) {
  AssertIsNotLockedByCurrentThread();
  ...
  if (callbacks_ /*&& !owner_->tear_down_*/) {
    ...
    try {
      ...
      if (!lf::Read(msg, payload_length) ||
          payload_length !=
              msg.GetReadableLength()) {  // 뒷쪽에 더 추가가 될수도 있남??
        // if (!lf::Read(msg, payload_length) || !msg.CanRead(payload_length)) {
        // // 뒷쪽에 더 추가가 될수도 있남??
        ...
        ByteArray payload = ByteArray::FromRawData(
            (const char*)msg.GetReadableData(), msg.GetReadableLength());
...
void NetServerImpl::UserWork_FinalUserTask(FinalUserWorkItem& uwi,
                                           void* host_tag) {
  AssertIsNotLockedByCurrentThread();
...
void NetServerImpl::EndCompletion() {
  if (net_thread_pool_->IsCurrentThread()) {  //@maxidea:
                                              //net_thread_pool_->IsCurrentThread()가
                                              //true여야, net_thread_pool_()
                                              //내에서 EndCompletion이 일어남.
    net_thread_pool_->UnregisterReferer(this);
    net_thread_pool_unregisted_ = true;
  }

  if (user_thread_pool_->IsCurrentThread()) {  //@maxidea:
                                               //user_thread_pool_->IsCurrentThread()가
                                               //true여야, net_thread_pool_()
                                               //내에서 EndCompletion이 일어남.
...
NetServerStats::NetServerStats() { Reset(); }
...
String NetServerStats::ToString() const {
  ...
  ret << ", p2p_direct_connection_pair_count=" <<
  ToString(p2p_direct_connection_pair_count); ret << ", total_tcp_recv_bytes="
  << ToString(total_tcp_recv_bytes); ret << ", total_tcp_recv_count=" <<
  ToString(total_tcp_recv_count); ret << ", total_tcp_send_bytes=" <<
  ToString(total_tcp_send_bytes); ret << ", total_tcp_send_count=" <<
  ToString(total_tcp_send_count); ret << ", total_udp_recv_bytes=" <<
  ToString(total_udp_recv_bytes); ret << ", total_udp_recv_count=" <<
  ToString(total_udp_recv_count); ret << ", total_udp_send_bytes=" <<
  ToString(total_udp_send_bytes); ret << ", total_udp_send_count=" <<
  ToString(total_udp_send_count); ret << ", client_count=" <<
  ToString(client_count); ret << ", real_udp_enabled_client_count=" <<
  ToString(real_udp_enabled_client_count); ret << ", occupied_udp_port_count="
  << ToString(occupied_udp_port_count);
  */
  ret << "{";
  ...
  ret << ", \"real_udp_enabled_client_count\": "
      << ToString(real_udp_enabled_client_count);
  ret << ", \"occupied_udp_port_count\": " << ToString(occupied_udp_port_count);
  ret << ", \"p2p_group_count\": " << ToString(p2p_group_count);
  ret << ", \"p2p_connection_pair_count\": "
      << ToString(p2p_connection_pair_count);
  ret << ", \"p2p_direct_connection_pair_count\": "
      << ToString(p2p_direct_connection_pair_count);
  ret << "}";
  return ret;
}
