...
// 전에는 이 값이 1.3이었으나, FirstResendCoolTime를 2로 상향한 후 이 값은 변경할 필요가 없어졌음.
...
int32 RUdpConfig::max_ack_count_in_one_frame = MathBase::Max((frame_length - 10) / 5, 10);
...
int32 RUdpConfig::max_send_speed_in_frame_count = 1024 * 1024 * 10 / frame_length;
...
// VTG case: unreliable RPC가 안가지는 현상 제보와 관련일 듯 해서 false로 바꾸었다.(...취소!)
...
//  resend가 너무 많이 일어나면, recv측에서 처리가 안되어 계속 delay되는 현상이 있습니다.
// 그래서 resend양을 조절합니다.
