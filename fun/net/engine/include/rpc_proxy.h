...
class FUN_NETX_API RpcProxy {
 public:
  ...
  virtual bool RpcCall(const HostId* rpc_sendto_list,
  ...
  bool RpcFailure(const HostId* rpc_sendto_list,
                  int32 rpc_sendto_count,
                  RpcId rpc_id,
                  int32 result_code,
                  const char* error_message = nullptr);
...
inline bool RpcProxy::RpcFailure( HostId remote,
  ...
  return RpcFailure(remote,
                    RpcCallOption::Reliable,
                    rpc_id,
                    result_code,
                    error_message);
}
...
template <typename Allocator>
inline bool RpcProxy::RpcFailure( const Array<HostId, Allocator>& rpc_sendto_list,
                                  RpcId rpc_id,
                                  int32 result_code,
                                  const char* error_message) {
  return RpcFailure(rpc_sendto_list,
                    RpcCallOption::Reliable,
                    rpc_id,
                    result_code,
                    error_message);
...
inline bool RpcProxy::RpcFailure( const Array<HostId, Allocator>& rpc_sendto_list,
                                  ...
                                  int32 result_code,
                                  const char* error_message) {
  return RpcFailure(rpc_sendto_list.ConstData(),
