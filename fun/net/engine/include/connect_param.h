...
class NetConnectionArgs {
 public:
  ...
  FUN_NETX_API void LoadFromINI(const String& filename,
                                const String& section_name);
  FUN_NETX_API void SaveToINI(const String& filename,
                              const String& section_name);
};
...
class StartServerArgs {
 public:
   ...
   * 다만, L4 장비 뒤에 놓인 서버일 경우에 주소인식을 제대로 못할 수 있으므로,
   * 직접 지정해주어야함.
   */
   ...
   * 서버의 리스닝 바인딩을 위한 주소.
   *
   * 비워두어도 된다. 다만, NIC가 두개 이상일 경우에는 첫번째 주소로 바인딩
   * 되는데, 두번째 NIC의 주소를 사용해야한다던지 하는 경우에는 직접 지정해야
   * 원하는 NIC로 접속을 받아낼 수 있다.
   */
   ...
   * 목록을 채우지 않으면 0이 하나 들어가게 된다. 0을 지정하면 TCP 리스닝 포트
   * 한개가 자동 할당 된다.  자동 할당이 유용한 경우는, 클라이언트가 서버에
   * 접속하기 전에 자동 할당된 포트 번호를 받은 경우, 예를들어 스트레스 테스트를
   * 위해 불특정 다수의 서버를 띄워야 하는 경우가 되겠다. 이때 클라이언트에게
   * 자동할당된 포트 주소를 알려주어야 한다. 자동 할당된 포트 번호는
   * GetServerAddress()로 얻을 수 있다.
   */
  ...
  /**
   * True이면, UdpPorts에 이미 사용중인 Port가 있더라도, 실패하지 않고
   * 건너띄면서 다음 포트를 Bind함. 즉, 개별 Udp Port binding에 실패 했다고 해도
   * 나머지 포트들에 대해서 binding을 지속한다.
   */
  ...
  /**
   * 송신주기를 임의로 변경하고 싶을 경우 사용한다. 기본은 0.03초로 설정되어
   * 있는데, 이 주기를 변경하고 싶을 경우에 아래의 값을 변경하도록 한다.
   */
