  ...
  for (struct addrinfo* it = addr_info; it; it = it->ai_next) {
    ...
    if (it->ai_addrlen > 0 && it->ai_addr) {
      switch (it->ai_addr->sa_family) {
        case AF_INET:
          //host_addresses_.Add(IpAddress(&reinterpret_cast<struct sockaddr_in*>(it->ai_addr)->sin_addr, sizeof(in_addr)));
          host_addresses_.Add(IpAddress(reinterpret_cast<struct sockaddr_in*>(it->ai_addr)->sin_addr));
          break;

        case AF_INET6:
          //host_addresses_.Add(IpAddress(&reinterpret_cast<struct sockaddr_in6*>(it->ai_addr)->sin6_addr, sizeof(in6_addr), reinterpret_cast<struct sockaddr_in6*>(it->ai_addr)->sin6_scope_id));
          host_addresses_.Add(IpAddress(reinterpret_cast<struct sockaddr_in6*>(it->ai_addr)->sin6_addr, reinterpret_cast<struct sockaddr_in6*>(it->ai_addr)->sin6_scope_id));
          break;
