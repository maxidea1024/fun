...
void TimerQueue::HandleRead() {
  ...
  //for (std::vector<Entry>::iterator it = expireds.begin(); it != expireds.end(); ++it) {
  //  it->second->Run();
  //}
...
TimerQueue::GetExpireds(const Timestamp& now) {
  fun_check(timers_.Count() == active_timers_.Count());
...
void TimerQueue::Reset( const std::vector<Entry>& expireds,
  ...
  for (std::vector<Entry>::const_iterator it = expireds.begin(); it != expireds.end(); ++it) {
    ActiveTimer timer(it->second, it->second->GetSequence());

    if (it->second->ShouldRepeat()
        && cancelling_timers_.find(timer) == cancelling_timers_.end()) { // 실행중일때 cancel한 경우에는 재시작하면 안됨.
