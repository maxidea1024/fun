...
class FUN_NET_API NamedInetAddress {
 public:
  ...
  //FUN_ALWAYS_INLINE friend Archive& operator & (Archive& ar, NamedInetAddress& addr)
  //{
...
FUN_ALWAYS_INLINE NamedInetAddress::NamedInetAddress(const String& host_and_port) {
  ...
  if (colon_index != INVALID_INDEX) {
    ...
    if (!ok || port_value < 0 || port_value > 0xFFFF) {
      throw InvalidArgumentException(String::Format("Invalid port value: %s", *port_str));
    }
...
FUN_ALWAYS_INLINE NamedInetAddress::NamedInetAddress(const String& host, uint16 port)
  : address_(host), port_(port) {
  //Host 문자열에는 ":port"가 있어서는 안됨.
  address_.Trim();
}

FUN_ALWAYS_INLINE NamedInetAddress::NamedInetAddress(const IpAddress& host, uint16 port)
...
FUN_ALWAYS_INLINE NamedInetAddress::NamedInetAddress(const NamedInetAddress& rhs)
...
FUN_ALWAYS_INLINE void NamedInetAddress::OverwriteHostNameIfExists(const String& host_name) {
...
FUN_ALWAYS_INLINE bool NamedInetAddress::operator == (const NamedInetAddress& rhs) const {
...
FUN_ALWAYS_INLINE String NamedInetAddress::ToString() const {
  ...
  if (addr.GetAddressFamily() == AddressFamily::IPv6) {
    return AsciiString("[") + address_ + AsciiString("]:") + String::FromNumber(port_);
  } else {
...
FUN_ALWAYS_INLINE bool NamedInetAddress::IsLoopback() const {
  //if (address_ == "localhost" || address_ == "127.0.0.1" || address_ == "::1")
  if (address_ == "localhost" || address_.StartsWith("127.") || address_ == "::1") {
    return true;
  }
