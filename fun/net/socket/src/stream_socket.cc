...
void StreamSocket::Connect(const InetAddress& address, const Timespan& timeout) {
...
int32 StreamSocket::ReceiveBytes(FIFOBuffer& buffer) {
  ...
  const int32 received_byte_count = GetImpl()->ReceiveBytes(buffer.Next(), (int32)buffer.Available());
  if (received_byte_count > 0) {
    buffer.Advance(received_byte_count); // 받은만큼 큐에 데이터를 넣어줌. (enqueue)
  }
  return received_byte_count;
}

void StreamSocket::SendUrgent(uint8 data) {
