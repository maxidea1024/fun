...
TcpClient::~TcpClient() { Disconnect(true); }
...
bool TcpClient::IsConnected() const { return is_connected_; }

bool TcpClient::IsConnecting() const { return is_connecting_; }
...
void TcpClient::ConnectSync(const String& addr, uint32 port) {
  ...
  try {
    ...
    // 예외 발생시, 소켓을 닫아줍니다. 재접속 시도시 Socket::Connect()에서
    // 소켓이 닫힌 경우 다시 생성하므로 문제 될것은 없습니다.
    socket_.Close();
...
// pending connecting일 경우에도 operation이 가능 하도록 수정하는게
// 바람직해보임. pending에 대한 처리도 해주어야할듯함. connect 요청시에 요청큐를
// 비워주어야 할것이고, connect 실패시에도 실패 사실을 알 수 있어야함.
...
void TcpClient::AsyncRead(const ReadRequest& request) {
  ...
  if (IsConnected()) {
    io_service_->SetReadCallback(socket_,
                                 [this](SOCKET fd) { OnReadAvailable(fd); });
...
void TcpClient::AsyncWrite(const WriteRequest& request) {
  ...
  if (IsConnected()) {
    io_service_->SetWriteCallback(socket_,
                                  [this](SOCKET fd) { OnWriteAvailable(fd); });
...
void TcpClient::OnConnected(SOCKET fd) {
  // TODO
  // connect에 실패할수도 있으므로, 그에 따른 성공여부를 호출자 쪽에서도 알 수
  // 있어야함.
}
...
TcpClient::AsyncReadCallback TcpClient::ProcessRead(ReadResult& result) {
  ...
  auto cb =
      request.async_read_cb;  //이름을 그냥 Callback으로 하는게 좋을듯도...
  ...
  try {
    ...
    int32 received_byte_count =
        socket_.ReceiveBytes(result.buffer.GetData(), result.buffer.Count());
...
void TcpClient::OnReadAvailable(SOCKET fd) {
  //소켓 버퍼내에 읽기가 가능한 데이터가 있으므로, 읽어들인 후 읽기요청에 대한
  //처리를 한다.
  ReadResult result;
...
TcpClient::AsyncWriteCallback TcpClient::ProcessWrite(WriteResult& result) {
  ...
  try {
    result.size =
        socket_.SendBytes(request.buffer.GetData(), request.buffer.Count());
    result.success = true;
  } catch (Exception&) {
    result.success = false;
  }
