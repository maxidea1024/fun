...
void TcpServer::Start(const String& addr, uint32 port,
...
bool TcpServer::IsRunning() const { return is_running_; }

void TcpServer::OnReadAvailable(SOCKET fd) {
  try {
    SharedPtr<TcpClient> client =
        MakeShareable<TcpClient>(new TcpClient(socket_.AcceptConnection()));

    if (!new_connection_cb_ || !new_connection_cb_(client)) {
      client->SetDisconnectedCallback(
          [client, this]() { OnClientDisconnected(client); });
      clients_.Add(client);
    }
  } catch (Exception&) {
    Stop();
