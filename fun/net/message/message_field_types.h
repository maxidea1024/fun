...
#define FUN_MESSAGEFIELD_ENUM_FXED8(EnumType)                        \
  ...
  struct MessageFieldTypeTraits<                                     \
      T, typename EnableIf<IsSame<T, EnumType>::Value>::Type> {      \
    typedef EnumType CppValueType;                                   \
    FUN_ALWAYS_INLINE static void Write(IMessageOut& output,         \
                                        const CppValueType& value) { \
      output.WriteFixed8((uint8)value);                              \
    }                                                                \
    FUN_ALWAYS_INLINE static bool Read(IMessageIn& input,            \
                                       CppValueType& out_value) {    \
      uint8 tmp;                                                     \
 ...
 */
template <>
struct MessageFieldTypeTraits<NamedInetAddress> {
 ...
 */
template <>
struct MessageFieldTypeTraits<IpAddress> {
 ...
 */
template <>
struct MessageFieldTypeTraits<InetAddress> {
