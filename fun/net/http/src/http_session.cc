...
#ifdef _DEBUG
#pragma comment( \
    lib, "ThirdParty/curl-7.52.1/build/Win64/VC14/DLL Debug/libcurld.lib")
#else
#pragma comment( \
    lib, "ThirdParty/curl-7.52.1/build/Win64/VC14/DLL Release/libcurl.lib")
#endif
#endif
...
SessionImpl::~SessionImpl() { FreeCurlContext(curl_context_); }
...
void SessionImpl::SetUri(const Uri& uri) { uri_ = uri; }
...
void SessionImpl::SetProxies(const Proxies& proxies) { proxies_ = proxies; }
  ...
  if (curl) {
    ...
    for (auto& part : multipart.parts) {
      ...
      if (part.is_buffer) {
        ...
        form_data.Add({CURLFORM_CONTENTSLENGTH,
                       reinterpret_cast<const char*>(part.data_len)});
      ...
      curl_formadd(&form_post, &last_ptr, CURLFORM_ARRAY, form_data.GetData(),
                   CURLFORM_END);
    }
  ...
  if (curl) {
    ...
    for (auto& part : multipart.parts) {
      ...
      if (part.is_buffer) {
        ...
        form_data.Add({CURLFORM_BUFFERLENGTH,
                       reinterpret_cast<const char*>(part.data_len)});
      } else if (part.is_file) {
        form_data.Add({CURLFORM_FILE, part.value.ConstData()});
      } else {
...
struct CurlDebugContext {
  ...
  CurlDebugContext() : ascii_only(false), output(nullptr) {}
};

void DumpOperation(String& output, const char* operation, const uint8* data,
                   int32 length, bool ascii_only) {
  const int32 width =
      ascii_only ? (80 - 2)
                 : 16;  // hex 출력시에는 한줄당 16 octets 단위로 출력합니다.
                        // ascii만 출력시에는 78글자를 출력합니다.
  ...
  for (int32 i = 0; i < length; i += width) {
    ...
    for (int32 c = 0; (c < width) && (i + c < length); ++c) {
      ...
      if (ascii_only && (i + c + 1 < length) && data[i + c] == 0x0D &&
          data[i + c + 1] == 0x0A) {
        i += (c + 2 - width);
        break;
      }
      ...
      if (ascii_only && (i + c + 2 < length) && data[i + c + 1] == 0x0D &&
          data[i + c + 2] == 0x0A) {
        i += (c + 3 - width);
        break;
      }
...
// CURLINFO_END
static int32 TraceCurlOperation(Uri*, curl_infotype type, char* data,
  ...
  switch (type) {
    ...
    case CURLINFO_HEADER_OUT:
      operation = "=> SEND HEADER";
      break;
    case CURLINFO_DATA_OUT:
      operation = "=> SEND DATA";
      break;
    case CURLINFO_SSL_DATA_OUT:
      operation = "=> SEND SSL DATA";
      break;
    case CURLINFO_HEADER_IN:
      operation = "<= RECV HEADER";
      break;
    case CURLINFO_DATA_IN:
      operation = "<= RECV DATA";
      break;
    case CURLINFO_SSL_DATA_IN:
      operation = "<= RECV SSL DATA";
      break;
  }

  *context->output << String::Format("%s, %d bytes\n", operation,
                                     (int32)length);
  String::BytesToDebuggableString(*context->output,
                                  String::FromRawData(data, length),
                                  context->ascii_only, "   ");
...
Response SessionImpl::MakeRequest(Uri* curl) {
  ...
  if (trace_curl) {
    ...
    curl_easy_setopt(
        ...
        1L);  // the DEBUGFUNCTION has no effect until we enable VERBOSE
  ...
  return Response{(int32)response_code,
                  response_string,
                  headers,
                  raw_url,
                  elapsed,
...
void Session::SetParameters(Parameters&& parameters) {
  impl_->SetParameters(MoveTemp(parameters));
}
...
void Session::SetMultipart(Multipart&& multipart) {
  impl_->SetMultipart(MoveTemp(multipart));
}
void Session::SetRedirect(const Redirect& redirect) {
  impl_->SetRedirect(redirect);
}
void Session::SetMaxRedirects(const max_redirects& max_redirects) {
...
void Session::SetLowSpeed(const LowSpeed& low_speed) {
  impl_->SetLowSpeed(low_speed);
}
void Session::SetVerifySsl(const VerifySsl& verify_ssl) {
  impl_->SetVerifySsl(verify_ssl);
}

void Session::SetOption_INTERNAL(const Uri& uri) { impl_->SetUri(uri); }
void Session::SetOption_INTERNAL(const Parameters& parameters) {
  impl_->SetParameters(parameters);
}
void Session::SetOption_INTERNAL(Parameters&& parameters) {
  impl_->SetParameters(MoveTemp(parameters));
}
void Session::SetOption_INTERNAL(const Headers& headers) {
  impl_->SetHeaders(headers);
}
void Session::SetOption_INTERNAL(const Timeout& timeout) {
  impl_->SetTimeout(timeout);
}
void Session::SetOption_INTERNAL(const Auth& auth) { impl_->SetAuth(auth); }
void Session::SetOption_INTERNAL(const Digest& digest) {
  impl_->SetDigest(digest);
}
void Session::SetOption_INTERNAL(const Payload& payload) {
  impl_->SetPayload(payload);
}
void Session::SetOption_INTERNAL(Payload&& payload) {
  impl_->SetPayload(MoveTemp(payload));
}
void Session::SetOption_INTERNAL(const Proxies& proxies) {
  impl_->SetProxies(proxies);
}
void Session::SetOption_INTERNAL(Proxies&& proxies) {
  impl_->SetProxies(MoveTemp(proxies));
}
void Session::SetOption_INTERNAL(const Multipart& multipart) {
  impl_->SetMultipart(multipart);
}
void Session::SetOption_INTERNAL(Multipart&& multipart) {
  impl_->SetMultipart(MoveTemp(multipart));
}
void Session::SetOption_INTERNAL(const Redirect& redirect) {
  impl_->SetRedirect(redirect);
}
void Session::SetOption_INTERNAL(const max_redirects& max_redirects) {
  impl_->SetMaxRedirects(max_redirects);
}
void Session::SetOption_INTERNAL(const Cookies& cookies) {
  impl_->SetCookies(cookies);
}
void Session::SetOption_INTERNAL(const Body& body) { impl_->SetBody(body); }
void Session::SetOption_INTERNAL(Body&& body) {
  impl_->SetBody(MoveTemp(body));
}
void Session::SetOption_INTERNAL(const LowSpeed& low_speed) {
  impl_->SetLowSpeed(low_speed);
}
void Session::SetOption_INTERNAL(const VerifySsl& verify_ssl) {
  impl_->SetVerifySsl(verify_ssl);
}
