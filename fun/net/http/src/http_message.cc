...
CMessage::CMessage() : Version(HTTP_1_0) {}

CMessage::CMessage(const ByteString& Version) : Version(Version) {}
...
void CMessage::SetTransferEncoding(const ByteString& Encoding) {
  if (Encoding.Compare(IDENTITY_TRANSFER_ENCODING,
                       CaseSensitivity::IgnoreCase) == 0) {
...
bool CMessage::GetChunkedTransferEncoding() const {
  return GetTransferEncoding().Compare(CHUNKED_TRANSFER_ENCODING,
                                       CaseSensitivity::IgnoreCase) == 0;
}
...
bool CMessage::GetKeepAlive() const {
  ...
  if (!Connection.IsEmpty()) {
    return Connection.Compare(CONNECTION_CLOSE, CaseSensitivity::IgnoreCase) !=
           0;
  } else {
