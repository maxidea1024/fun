...
inline String ToString(const http::Protocol value) {
  switch (value) {
  case http::Protocol::None: return StringLiteral("None");
  case http::Protocol::HTTP: return StringLiteral("HTTP");
  case http::Protocol::HTTPS: return StringLiteral("HTTPS");
...
inline bool FromString(const UNICHAR* str, http::Protocol& out_value) {
  ...
  */
  ...
  if (StringCmp::Compare(str, "None", CaseSensitivity::IgnoreCase) == 0) {
    out_value = http::Protocol::None;
    return true;
  } else if (StringCmp::Compare(str, "HTTP", CaseSensitivity::IgnoreCase) == 0) {
    out_value = http::Protocol::HTTP;
    return true;
  } else if (StringCmp::Compare(str, "HTTPS", CaseSensitivity::IgnoreCase) == 0) {
