#include "fun/net/http/http_server.h"
#include "fun/net/http/http_request.h"
#include "fun/net/http/http_response.h"
...
void OnRequest(const HttpRequest& req, HttpResponse* resp) {
  LOG_INFO << "Headers " << req.MethodString() << " " << req.GetPath();
  ...
  std::map<String, String>::const_iterator it = redirections.find(req.GetPath());
...
int main(int argc, char* argv[]) {
  ...
  EventLoop loop;
  EventLoopThreadPool thread_pool(&loop, "shorturl");
  if (thread_count > 1) {
    thread_pool.SetThreadCount(thread_count);
  }
  else {
    thread_count = 1;
  }
