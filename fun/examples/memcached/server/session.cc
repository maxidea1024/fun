  ...
  while (buf->GetReadableLength() > 0) {
    if (state_ == kNewCommand) {
      if (protocol_ == kAuto) {
        ...
        protocol_ =
            IsBinaryProtocol(buf->GetReadablePtr()[0]) ? kBinary : kAscii;
      }
...
void Session::ReceiveValue(fun::net::Buffer* buf) {
  ...
  const size_t avail =
      std::min(buf->GetReadableLength(), curr_item_->neededBytes());
...
bool Session::ProcessRequest(StringPiece request) {
  ...
  if (command_ == "set" || command_ == "add" || command_ == "replace" ||
    ...
    if (conn_->GetOutputBuffer()->writableBytes() >
        65536 + output_buf_.GetReadableLength()) {
