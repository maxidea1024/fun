...
struct RawMessage
{
  ...
  void updateId()
  {
    ...
    UnsafeMemory::Memcpy(const_cast<char*>(body + bodylen - ProtobufCodecLite::kChecksumLen), &be32, sizeof(be32));
  }
...
{
 public:
  BackendSession(EventLoop* loop, const InetAddress& backend_addr, const String& name)
    : loop_(loop),
  ...
  bool Send(MSG& msg, const TcpConnectionPtr& client_conn)
  {
    ...
    if (conn_)
    {
      ...
      msg.set_id(id);
      SendTo(conn_, msg);
      // LOG_DEBUG << "forward " << r.origId << " from " << client_conn->GetName()
 ...
 private:
  ...
  void OnMessageT(MSG& msg)
  {
    ...
    if (it != outstandings_.end())
    {
      ...
      if (client_conn)
      {
        // LOG_DEBUG << "Send back " << origId << " of " << client_conn->GetName()
