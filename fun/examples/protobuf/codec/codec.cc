...
void ProtobufCodec::FillEmptyBuffer(Buffer* buf,
  ...
  int32_t chksum = static_cast<int32_t>(
      ::adler32(1, reinterpret_cast<const Bytef*>(buf->GetReadablePtr()),
                static_cast<int>(buf->GetReadableLength())));
  buf->AppendInt32(chksum);
  fun_check(buf->GetReadableLength() ==
            sizeof name_len + name_len + byte_size + sizeof chksum);
  int32_t len =
      sockets::hostToNetwork32(static_cast<int32_t>(buf->GetReadableLength()));
...
void ProtobufCodec::OnMessage(const TcpConnectionPtr& conn, Buffer* buf,
  ...
  while (buf->GetReadableLength() >= kMinMessageLen + kHeaderLen) {
    ...
    if (len > kMaxMessageLen || len < kMinMessageLen) {
      ...
      break;
    } else if (buf->GetReadableLength() >=
               ImplicitCast<size_t>(len + kHeaderLen)) {
      ErrorCode error_code = kNoError;
      MessagePtr message =
          Parse(buf->GetReadablePtr() + kHeaderLen, len, &error_code);
