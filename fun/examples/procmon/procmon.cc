...
class Procmon : Noncopyable {
 public:
  Procmon(EventLoop* loop, pid_t pid, uint16_t port, const char* procname)
      : kClockTicksPerSecond_(fun::ProcessInfo::clockTicksPerSecond()),
        ...
        procname_(
            procname ? procname
                     : ProcessInfo::procname(readProcFile("stat")).as_string()),
  ...
  void Start() {
    tick();
    server_.GetLoop()->ScheduleEvery(kPeriod_,
                                     boost::bind(&Procmon::tick, this));
    server_.Start();
  }
 ...
 private:
    ...
    */
    ...
    if (req.path() == "/") {
      ...
      for (size_t i = 0; i < cpu_usage_.size(); ++i)
        cpu_usage.push_back(
            cpu_usage_[i].cpuUsage(kPeriod_, kClockTicksPerSecond_));
  ...
  void fillOverview(const String& query) {
    ...
    appendResponse("<p>Page generated at %s (UTC)",
    ...
    appendTableRow(
        "Started at",
        started.ToFormattedString(false /*showMicroseconds*/) + " (UTC)");
    appendTableRowFloat(
        "Uptime (s)",
        TimeDifference(now, started));  // FIXME: format as days+H:M:S
    ...
    appendTableRow("CPU usage",
                   "<img src=\"/cpu.png\" height=\"100\" witdh=\"640\">");
  ...
  static int dirFilter(const struct dirent* d) { return (d->d_name[0] != '.'); }
  ...
  void listFiles() {
    ...
    for (int i = 0; i < result; ++i) {
      ...
      if (::lstat(namelist[i]->d_name, &stat) == 0) {
        ...
        appendResponse(
            "%c %9ld %s %s", getDirType(namelist[i]->d_type), stat.st_size,
  ...
  void tick() {
    ...
    if (ticks_ > 0) {
      CpuTime time;
      time.userTime_ =
          std::max(0, static_cast<int>(statData.utime - lastStatData_.utime));
      time.sysTime_ =
          std::max(0, static_cast<int>(statData.stime - lastStatData_.stime));
