...
void receive(const Options& opt) {
  try {
    ...
    size_t nr = boost::asio::read(
        socket, boost::asio::buffer(&sessionMessage, sizeof sessionMessage),
        error);
    ...
    fun::SharedPtr<PayloadMessage> freeIt(payload, ::free);
    fun_check(payload);
    ...
    for (int i = 0; i < sessionMessage.number; ++i) {
      ...
      if (boost::asio::read(
              socket,
              boost::asio::buffer(&payload->length, sizeof(payload->length)),
      ...
      if (boost::asio::write(socket, boost::asio::buffer(&ack, sizeof(ack))) !=
          sizeof(ack)) {
