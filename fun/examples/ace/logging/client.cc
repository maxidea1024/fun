 ...
 private:
  void OnConnection(const TcpConnectionPtr& conn) {
    ...
    if (conn->IsConnected()) {
      ...
      hb->set_process_start_time(
          ProcessInfo::startTime().microSecondsSinceEpoch());
  ...
  void OnMessage(const TcpConnectionPtr&, const MessagePtr& message,
                 const Timestamp&) {
    // SHOULD NOT HAPPEN
...
int main(int argc, char* argv[]) {
  if (argc < 3) {
    ...
    CurrentThread::sleepUsec(
        1000 * 1000);  // wait for Disconnect, then safe to destruct LogClient
                       // (esp. TcpClient). Otherwise mutex_ is used after dtor.
  }
