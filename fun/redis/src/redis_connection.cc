...
Connection::~Connection() {
  tcp_client_->Disconnect(true);
}

void Connection::ConnectSync( const String& host,
                              int32 port,
  ...
  try {
    ...
    TcpClient::ReadRequest request = { READ_BUFFER_SIZE, [this](const TcpClient::ReadResult& result) { OnTcpReceive(result); } };
    tcp_client_->AsyncRead(request);
  } catch (Exception& e) {
    //아래와 같이 "throw e"를 통해서 rethrow할 경우, CException으로 복사되어서 예외가 전달됩니다.
    //이때 예외 클래스의 정보가 생략되어, CException의 정보로 대체되는 부작용이 있을 수 있습니다.
    //throw Ex;
    e.Rethrow();
  }
...
void Connection::ConnectAsync(const String& host,
                              int32 port,
...
bool Connection::IsConnecting() const {
  return tcp_client_->IsConnecting();
}
...
void Connection::OnTcpReceive(const TcpClient::ReadResult& result) {
  ...
  try {
    ...
    //복사를 줄이기 위해서, Raw 형태로 넘겨줌. 단 이때 result.Buffer는 완료시점까지 파괴되어서는 안됨.
    builder_ << String::FromRawData(result.buffer_.GetData(), result.buffer_.Count());
  } catch (Exception&) {
    //TODO 오류 발생시 Disconnection Handler를 호출할뿐, Disconnect()를 호출하지 않는다.
    //어떤식으로 처리하는게 바람직할까??
