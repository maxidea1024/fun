...
Reply::Reply(const String& value, StringType reply_type)
    : type_(static_cast<Type>(reply_type)), string_value_(value) {}

Reply::Reply(int64 value) : type_(Type::Integer), integer_value_(value) {}

Reply::Reply(const TArray<Reply>& rows) : type_(Type::Array), rows_(rows) {}

bool Reply::IsArray() const { return type_ == Type::Array; }

bool Reply::IsString() const { return IsSimpleString() || IsBulkString(); }

bool Reply::IsSimpleString() const { return type_ == Type::SimpleString; }

bool Reply::IsBulkString() const { return type_ == Type::BulkString; }

bool Reply::IsError() const { return type_ == Type::Error; }

bool Reply::IsInteger() const { return type_ == Type::Integer; }

bool Reply::IsNil() const { return type_ == Type::Nil; }

bool Reply::Ok() const { return type_ != Type::Error; }
...
int64 Reply::AsInteger() const {
  ...
  // if (!IsInteger()) {
  //  throw RedisException(String::Format("integer 타입이 요구되는데, {0}
  //  타입입니다."));
  //}
...
Reply::Type Reply::GetType() const { return type_; }
...
const String Reply::GetTypeName() const {
  switch (type_) {
    ...
    case Type::BulkString:
      return StringLiteral("BulkString");
    case Type::SimpleString:
      return StringLiteral("SimpleString");
    case Type::Nil:
      return StringLiteral("Nil");
    case Type::Integer:
      return StringLiteral("Integer");
    case Type::Array:
