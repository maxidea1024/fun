...
class Client {
 public:
  ...
  enum class ClientType { normal, master, pubsub, slave };
 ...
 public:
  ...
  typedef Function<void(const String& host, int32 port, ConnectState status)>
      ConnectCallback;
  ...
  /// (may be null) \param timeout_msecs maximum time to connect \param
  ...
  void Connect(const String& host = "127.0.0.1", int32 port = 6379,
               const ConnectCallback& connect_callback = nullptr,
               uint32 timeout_msecs = 0, int32 max_reconnects = 0,
 ...
 public:
  ...
  /// the command is actually pipelined and only buffered, so nothing is sent to
  /// the network please call Commit() / SyncCommit() to flush the buffer \param
  /// redis_cmd command to be sent \param callback callback to be called on
  /// received reply \return current instance
  Client& Send(const Array<String>& redis_cmd, const ReplyCallback& callback);

  /// same as the other Send method
  /// but Future based: does not take any callback and return an tl:;Future to
  /// handle the reply \param redis_cmd command to be sent \return Future to
  /// handler redis reply
  ...
  /// calling Commit while the Client is not connected, something that throws an
  /// exception). Commit() works asynchronously: it returns immediately after
  /// sending the queued requests and replies are processed asynchronously.
  /// Please note that, while Commit() can safely be called from inside a reply
  /// callback, calling SyncCommit() from inside a reply callback is not
  ...
  Client& SyncCommit(const tl::chrono::duration<Rep, Period>& timeout) {
    ...
    if (!sync_cv_.wait_for(lock_callback, timeout, [=] {
          return callbacks_running_ == 0 && commands_.empty();
        })) {
 ...
 private:
  ...
  void UnprotectedSend(const Array<String>& redis_cmd,
 ...
 public:
  ...
  /// get_master_addr_by_name() when autoconnect is enabled. \param host
  /// Sentinel host \param port Sentinel port \param timeout_msecs maximum time
  /// to connect
 ...
 public:
  ...
  enum class AggregateMethod { sum, min, max, server_default };
 ...
 public:
  ...
  /// use server_default if you are not willing to specify this parameter and
  /// let the server defaults
  enum class OverflowType { wrap, sat, fail, server_default };
 ...
 public:
  ...
  struct BitfieldOperation {
    ...
    /// do not want to specify it) \return corresponding get BitfieldOperation
    static BitfieldOperation get(
        const String& type, int offset,
        OverflowType overflow = OverflowType::server_default);
    ...
    /// \param overflow overflow specification (leave to server_default if you
    /// do not want to specify it) \return corresponding set BitfieldOperation
    static BitfieldOperation set(
        const String& type, int offset, int value,
        OverflowType overflow = OverflowType::server_default);
    ...
    /// \param overflow overflow specification (leave to server_default if you
    /// do not want to specify it) \return corresponding incrby
    /// BitfieldOperation
    static BitfieldOperation incrby(
        const String& type, int offset, int increment,
        OverflowType overflow = OverflowType::server_default);
  };
 ...
 public:
  ...
  Client& bitfield(const String& key,
                   const Array<BitfieldOperation>& operations,
                   const ReplyCallback& reply_cb);
  Future<Reply> bitfield(const String& key,
                         const Array<BitfieldOperation>& operations);

  Client& bitop(const String& operation, const String& destkey,
                const Array<String>& keys, const ReplyCallback& reply_cb);
  Future<Reply> bitop(const String& operation, const String& destkey,
  ...
  Client& client_kill(const String& host, int port, const T& arg,
                      const Ts&... args);
  ...
  Client& cluster_setslot(const String& slot, const String& mode,
                          const String& node_id, const ReplyCallback& reply_cb);
  Future<Reply> cluster_setslot(const String& slot, const String& mode,
  ...
  Client& eval(const String& script, int numkeys, const Array<String>& keys,
               const Array<String>& args, const ReplyCallback& reply_cb);
  Future<Reply> eval(const String& script, int numkeys,
                     const Array<String>& keys, const Array<String>& args);

  Client& evalsha(const String& sha1, int numkeys, const Array<String>& keys,
                  const Array<String>& args, const ReplyCallback& reply_cb);
  Future<Reply> evalsha(const String& sha1, int numkeys,
                        const Array<String>& keys, const Array<String>& args);
  ...
  Client& geoadd(const String& key,
  ...
  Client& geodist(const String& key, const String& member_1,
                  const String& member_2, const ReplyCallback& reply_cb);
  Client& geodist(const String& key, const String& member_1,
                  const String& member_2, const String& unit,
                  const ReplyCallback& reply_cb);
  Future<Reply> geodist(const String& key, const String& member_1,
                        const String& member_2, const String& unit = "m");

  Client& georadius(const String& key, double longitude, double latitude,
                    double radius, GeoUnit unit, bool with_coord,
                    bool with_dist, bool with_hash, bool asc_order,
                    const ReplyCallback& reply_cb);
  Client& georadius(const String& key, double longitude, double latitude,
                    double radius, GeoUnit unit, bool with_coord,
                    bool with_dist, bool with_hash, bool asc_order, int32 count,
                    const ReplyCallback& reply_cb);
  Client& georadius(const String& key, double longitude, double latitude,
                    double radius, GeoUnit unit, bool with_coord,
                    bool with_dist, bool with_hash, bool asc_order,
                    const String& store_key, const ReplyCallback& reply_cb);
  Client& georadius(const String& key, double longitude, double latitude,
                    double radius, GeoUnit unit, bool with_coord,
                    bool with_dist, bool with_hash, bool asc_order,
  ...
  Client& georadius(const String& key, double longitude, double latitude,
                    ...
                    const String& store_key, const String& storedist_key,
                    const ReplyCallback& reply_cb);
  Future<Reply> georadius(const String& key, double longitude, double latitude,
                          double radius, GeoUnit unit, bool with_coord = false,
                          bool with_dist = false, bool with_hash = false,
                          bool asc_order = false, int32 count = 0,
                          const String& store_key = "",
                          const String& storedist_key = "");

  Client& georadiusbymember(const String& key, const String& member,
                            double radius, GeoUnit unit, bool with_coord,
                            bool with_dist, bool with_hash, bool asc_order,
                            const ReplyCallback& reply_cb);
  Client& georadiusbymember(const String& key, const String& member,
  ...
  Client& georadiusbymember(const String& key, const String& member,
                            ...
                            const String& storedist_key,
                            const ReplyCallback& reply_cb);
  Future<Reply> georadiusbymember(
      const String& key, const String& member, double radius, GeoUnit unit,
      bool with_coord = false, bool with_dist = false, bool with_hash = false,
      bool asc_order = false, int32 count = 0, const String& store_key = "",
      const String& storedist_key = "");
  ...
  Client& hincrbyfloat(const String& key, const String& field, float incr,
                       const ReplyCallback& reply_cb);
  Future<Reply> hincrbyfloat(const String& key, const String& field,
                             float incr);
  ...
  Client& hmset(const String& key, const Array<Pair<String, String>>& field_val,
                const ReplyCallback& reply_cb);
  Future<Reply> hmset(const String& key,
  ...
  Client& hset(const String& key, const String& field, const String& value,
               const ReplyCallback& reply_cb);
  Future<Reply> hset(const String& key, const String& field,
                     const String& value);

  Client& hsetnx(const String& key, const String& field, const String& value,
                 const ReplyCallback& reply_cb);
  Future<Reply> hsetnx(const String& key, const String& field,
                       const String& value);
  ...
  Client& linsert(const String& key, const String& before_after,
                  const String& pivot, const String& value,
                  const ReplyCallback& reply_cb);
  Future<Reply> linsert(const String& key, const String& before_after,
                        const String& pivot, const String& value);
  ...
  Client& migrate(const String& host, int port, const String& key,
                  const String& dest_db, int timeout,
                  const ReplyCallback& reply_cb);
  Client& migrate(const String& host, int port, const String& key,
                  const String& dest_db, int timeout, bool copy, bool replace,
                  const Array<String>& keys, const ReplyCallback& reply_cb);
  Future<Reply> migrate(const String& host, int port, const String& key,
                        const String& dest_db, int timeout, bool copy = false,
                        bool replace = false, const Array<String>& keys = {});
  ...
  Client& restore(const String& key, int ttl, const String& serialized_value,
                  const ReplyCallback& reply_cb);
  Future<Reply> restore(const String& key, int ttl,
                        const String& serialized_value);

  Client& restore(const String& key, int ttl, const String& serialized_value,
                  const String& replace, const ReplyCallback& reply_cb);
  Future<Reply> restore(const String& key, int ttl,
  ...
  Client& rpoplpush(const String& source, const String& destination,
                    const ReplyCallback& reply_cb);
  ...
  Client& sdiffstore(const String& destination, const Array<String>& keys,
                     const ReplyCallback& reply_cb);
  ...
  Client& set_advanced(const String& key, const String& value,
                       const ReplyCallback& reply_cb);
  Client& set_advanced(const String& key, const String& value, bool ex,
                       int ex_sec, bool px, int px_milli, bool nx, bool xx,
                       const ReplyCallback& reply_cb);
  Future<Reply> set_advanced(const String& key, const String& value,
  ...
  Client& sinterstore(const String& destination, const Array<String>& keys,
                      const ReplyCallback& reply_cb);
  ...
  Client& smove(const String& source, const String& destination,
                const String& member, const ReplyCallback& reply_cb);
  Future<Reply> smove(const String& src, const String& dst,
                      const String& member);

  Client& sort(const String& key, const ReplyCallback& reply_cb);
  Future<Reply> sort(const String& key);

  Client& sort(const String& key, const Array<String>& get_patterns,
               bool asc_order, bool alpha, const ReplyCallback& reply_cb);
  Future<Reply> sort(const String& key, const Array<String>& get_patterns,
                     bool asc_order, bool alpha);

  Client& sort(const String& key, int32 offset, int32 count,
               const Array<String>& get_patterns, bool asc_order, bool alpha,
               const ReplyCallback& reply_cb);
  Future<Reply> sort(const String& key, int32 offset, int32 count,
                     const Array<String>& get_patterns, bool asc_order,
                     bool alpha);

  Client& sort(const String& key, const String& by_pattern,
               const Array<String>& get_patterns, bool asc_order, bool alpha,
               const ReplyCallback& reply_cb);
  Future<Reply> sort(const String& key, const String& by_pattern,
                     const Array<String>& get_patterns, bool asc_order,
                     bool alpha);
  ...
  Client& sort(const String& key, int32 offset, int32 count,
               const Array<String>& get_patterns, bool asc_order, bool alpha,
               const String& store_dest, const ReplyCallback& reply_cb);
  Future<Reply> sort(const String& key, int32 offset, int32 count,
                     const Array<String>& get_patterns, bool asc_order,
  ...
  Client& sort(const String& key, const String& by_pattern, int32 offset,
               int32 count, const Array<String>& get_patterns, bool asc_order,
               bool alpha, const ReplyCallback& reply_cb);
  Future<Reply> sort(const String& key, const String& by_pattern, int32 offset,
  ...
  Client& sunionstore(const String& destination, const Array<String>& keys,
                      const ReplyCallback& reply_cb);
  ...
  Client& zadd(const String& key, const Array<String>& options,
               const tl::multimap<String, String>& score_members,
               const ReplyCallback& reply_cb);
  Future<Reply> zadd(const String& key, const Array<String>& options,
                     const tl::multimap<String, String>& score_members);
  ...
  Client& zincrby(const String& key, const String& incr, const String& member,
                  const ReplyCallback& reply_cb);
  Future<Reply> zincrby(const String& key, const String& incr,
                        const String& member);

  Client& zinterstore(const String& destination, int32 numkeys,
                      const Array<String>& keys, const Array<int32> weights,
                      AggregateMethod method, const ReplyCallback& reply_cb);
  Future<Reply> zinterstore(const String& destination, int32 numkeys,
                            const Array<String>& keys,
  ...
  Client& zlexcount(const String& key, const String& min, const String& max,
                    const ReplyCallback& reply_cb);
  Future<Reply> zlexcount(const String& key, const String& min,
                          const String& max);

  Client& zrange(const String& key, int start, int stop,
                 const ReplyCallback& reply_cb);
  Client& zrange(const String& key, int start, int stop, bool withscores,
                 const ReplyCallback& reply_cb);
  Future<Reply> zrange(const String& key, int start, int stop,
                       bool withscores = false);

  Client& zrange(const String& key, double start, double stop,
                 const ReplyCallback& reply_cb);
  Client& zrange(const String& key, double start, double stop, bool withscores,
                 const ReplyCallback& reply_cb);
  Future<Reply> zrange(const String& key, double start, double stop,
                       bool withscores = false);

  Client& zrange(const String& key, const String& start, const String& stop,
                 const ReplyCallback& reply_cb);
  Client& zrange(const String& key, const String& start, const String& stop,
                 bool withscores, const ReplyCallback& reply_cb);
  Future<Reply> zrange(const String& key, const String& start,
                       const String& stop, bool withscores = false);

  Client& zrangebylex(const String& key, int min, int max,
                      const ReplyCallback& reply_cb);
  Client& zrangebylex(const String& key, int min, int max, bool withscores,
                      const ReplyCallback& reply_cb);
  Future<Reply> zrangebylex(const String& key, int min, int max,
                            bool withscores = false);

  Client& zrangebylex(const String& key, double min, double max,
                      const ReplyCallback& reply_cb);
  Client& zrangebylex(const String& key, double min, double max,
                      bool withscores, const ReplyCallback& reply_cb);
  Future<Reply> zrangebylex(const String& key, double min, double max,
                            bool withscores = false);

  Client& zrangebylex(const String& key, const String& min, const String& max,
                      const ReplyCallback& reply_cb);
  Client& zrangebylex(const String& key, const String& min, const String& max,
                      bool withscores, const ReplyCallback& reply_cb);
  Future<Reply> zrangebylex(const String& key, const String& min,
                            const String& max, bool withscores = false);

  Client& zrangebylex(const String& key, int min, int max, int32 offset,
                      int32 count, const ReplyCallback& reply_cb);
  Client& zrangebylex(const String& key, int min, int max, int32 offset,
                      int32 count, bool withscores,
                      const ReplyCallback& reply_cb);
  Future<Reply> zrangebylex(const String& key, int min, int max, int32 offset,
                            int32 count, bool withscores = false);

  Client& zrangebylex(const String& key, double min, double max, int32 offset,
                      int32 count, const ReplyCallback& reply_cb);
  Client& zrangebylex(const String& key, double min, double max, int32 offset,
                      int32 count, bool withscores,
                      const ReplyCallback& reply_cb);
  Future<Reply> zrangebylex(const String& key, double min, double max,
                            int32 offset, int32 count, bool withscores = false);

  Client& zrangebylex(const String& key, const String& min, const String& max,
                      int32 offset, int32 count, const ReplyCallback& reply_cb);
  Client& zrangebylex(const String& key, const String& min, const String& max,
  ...
  Client& zrangebyscore(const String& key, int min, int max,
                        const ReplyCallback& reply_cb);
  Client& zrangebyscore(const String& key, int min, int max, bool withscores,
                        const ReplyCallback& reply_cb);
  Future<Reply> zrangebyscore(const String& key, int min, int max,
                              bool withscores = false);

  Client& zrangebyscore(const String& key, double min, double max,
                        const ReplyCallback& reply_cb);
  Client& zrangebyscore(const String& key, double min, double max,
                        bool withscores, const ReplyCallback& reply_cb);
  Future<Reply> zrangebyscore(const String& key, double min, double max,
                              bool withscores = false);

  Client& zrangebyscore(const String& key, const String& min, const String& max,
                        const ReplyCallback& reply_cb);
  Client& zrangebyscore(const String& key, const String& min, const String& max,
                        bool withscores, const ReplyCallback& reply_cb);
  Future<Reply> zrangebyscore(const String& key, const String& min,
                              const String& max, bool withscores = false);

  Client& zrangebyscore(const String& key, int min, int max, int32 offset,
                        int32 count, const ReplyCallback& reply_cb);
  Client& zrangebyscore(const String& key, int min, int max, int32 offset,
                        int32 count, bool withscores,
                        const ReplyCallback& reply_cb);
  Future<Reply> zrangebyscore(const String& key, int min, int max, int32 offset,
                              int32 count, bool withscores = false);

  Client& zrangebyscore(const String& key, double min, double max, int32 offset,
                        int32 count, const ReplyCallback& reply_cb);
  Client& zrangebyscore(const String& key, double min, double max, int32 offset,
                        int32 count, bool withscores,
                        const ReplyCallback& reply_cb);
  ...
  Client& zremrangebylex(const String& key, const String& min,
                         const String& max, const ReplyCallback& reply_cb);
  Future<Reply> zremrangebylex(const String& key, const String& min,
                               const String& max);
  ...
  Client& zremrangebyrank(const String& key, const String& start,
                          const String& stop, const ReplyCallback& reply_cb);
  Future<Reply> zremrangebyrank(const String& key, const String& start,
                                const String& stop);
  ...
  Client& zremrangebyscore(const String& key, const String& min,
                           const String& max, const ReplyCallback& reply_cb);
  Future<Reply> zremrangebyscore(const String& key, const String& min,
                                 const String& max);

  Client& zrevrange(const String& key, int start, int stop,
                    const ReplyCallback& reply_cb);
  Client& zrevrange(const String& key, int start, int stop, bool withscores,
                    const ReplyCallback& reply_cb);
  Future<Reply> zrevrange(const String& key, int start, int stop,
                          bool withscores = false);

  Client& zrevrange(const String& key, double start, double stop,
                    const ReplyCallback& reply_cb);
  Client& zrevrange(const String& key, double start, double stop,
                    bool withscores, const ReplyCallback& reply_cb);
  Future<Reply> zrevrange(const String& key, double start, double stop,
                          bool withscores = false);

  Client& zrevrange(const String& key, const String& start, const String& stop,
                    const ReplyCallback& reply_cb);
  Client& zrevrange(const String& key, const String& start, const String& stop,
                    bool withscores, const ReplyCallback& reply_cb);
  Future<Reply> zrevrange(const String& key, const String& start,
                          const String& stop, bool withscores = false);

  Client& zrevrangebylex(const String& key, int max, int min,
                         const ReplyCallback& reply_cb);
  Client& zrevrangebylex(const String& key, int max, int min, bool withscores,
                         const ReplyCallback& reply_cb);
  Future<Reply> zrevrangebylex(const String& key, int max, int min,
                               bool withscores = false);

  Client& zrevrangebylex(const String& key, double max, double min,
                         const ReplyCallback& reply_cb);
  Client& zrevrangebylex(const String& key, double max, double min,
                         bool withscores, const ReplyCallback& reply_cb);
  Future<Reply> zrevrangebylex(const String& key, double max, double min,
                               bool withscores = false);

  Client& zrevrangebylex(const String& key, const String& max,
                         const String& min, const ReplyCallback& reply_cb);
  Client& zrevrangebylex(const String& key, const String& max,
                         const String& min, bool withscores,
                         const ReplyCallback& reply_cb);
  Future<Reply> zrevrangebylex(const String& key, const String& max,
                               const String& min, bool withscores = false);

  Client& zrevrangebylex(const String& key, int max, int min, int32 offset,
                         int32 count, const ReplyCallback& reply_cb);
  Client& zrevrangebylex(const String& key, int max, int min, int32 offset,
                         int32 count, bool withscores,
                         const ReplyCallback& reply_cb);
  ...
  Client& zrevrangebyscore(const String& key, int max, int min,
                           const ReplyCallback& reply_cb);
  Client& zrevrangebyscore(const String& key, int max, int min, bool withscores,
                           const ReplyCallback& reply_cb);
  Future<Reply> zrevrangebyscore(const String& key, int max, int min,
                                 bool withscores = false);

  Client& zrevrangebyscore(const String& key, double max, double min,
                           const ReplyCallback& reply_cb);
  Client& zrevrangebyscore(const String& key, double max, double min,
                           bool withscores, const ReplyCallback& reply_cb);
  Future<Reply> zrevrangebyscore(const String& key, double max, double min,
                                 bool withscores = false);

  Client& zrevrangebyscore(const String& key, const String& max,
                           const String& min, const ReplyCallback& reply_cb);
  Client& zrevrangebyscore(const String& key, const String& max,
                           const String& min, bool withscores,
                           const ReplyCallback& reply_cb);
  Future<Reply> zrevrangebyscore(const String& key, const String& max,
                                 const String& min, bool withscores = false);

  Client& zrevrangebyscore(const String& key, int max, int min, int32 offset,
                           int32 count, const ReplyCallback& reply_cb);
  Client& zrevrangebyscore(const String& key, int max, int min, int32 offset,
                           int32 count, bool withscores,
                           const ReplyCallback& reply_cb);
  ...
  Client& zunionstore(const String& destination, int32 numkeys,
                      const Array<String>& keys, const Array<int32> weights,
                      AggregateMethod method, const ReplyCallback& reply_cb);
  Future<Reply> zunionstore(const String& destination, int32 numkeys,
                            const Array<String>& keys,
                            const Array<int32> weights, AggregateMethod method);
 ...
 private:
  /// Client kill impl
  template <typename T>
  typename EnableIf<IsSame<T, ClientType>::Value>::Type client_kill_unpack_arg(
      Array<String>& redis_cmd, ReplyCallback&, ClientType type);

  template <typename T>
  typename EnableIf<IsSame<T, bool>::Value>::Type client_kill_unpack_arg(
      Array<String>& redis_cmd, ReplyCallback&, bool skip);

  template <typename T>
  typename EnableIf<IsIntegral<T>::Value>::Type client_kill_unpack_arg(
      Array<String>& redis_cmd, ReplyCallback&, uint64 id);

  template <typename T>
  typename EnableIf<IsClass<T>::Value>::Type client_kill_unpack_arg(
      Array<String>&, ReplyCallback& reply_cb, const T& cb);

  template <typename T, typename... Ts>
  void client_kill_impl(Array<String>& redis_cmd, ReplyCallback& reply,
 ...
 private:
  /// sort impl
  Client& sort(const String& key, const String& by_pattern, bool limit,
               int32 offset, int32 count, const Array<String>& get_patterns,
               bool asc_order, bool alpha, const String& store_dest,
               const ReplyCallback& reply_cb);
  ...
  /// zrangebyscore impl
  Client& zrangebyscore(const String& key, const String& min, const String& max,
                        bool limit, int32 offset, int32 count, bool withscores,
                        const ReplyCallback& reply_cb);
 ...
 private:
  /// redis connection receive handler, triggered whenever a reply has been read
  /// by the redis connection \param conn redis_connection instance \param reply
  /// parsed reply
  void OnConnectionReceive(Connection& conn, Reply& reply);

  /// redis_connection disconnection handler, triggered whenever a disconnection
  /// occurred \param conn redis_connection instance
...
Client::client_kill_unpack_arg(Array<String>& redis_cmd, ReplyCallback&,
                               ClientType type) {
  redis_cmd.Add("TYPE");
  String type_string;
...
template <typename T>
typename EnableIf<IsSame<T, bool>::Value>::Type Client::client_kill_unpack_arg(
    Array<String>& redis_cmd, ReplyCallback&, bool skip) {
...
template <typename T>
typename EnableIf<IsIntegral<T>::Value>::Type Client::client_kill_unpack_arg(
    Array<String>& redis_cmd, ReplyCallback&, uint64 id) {
  redis_cmd.Add("ID");
...
template <typename T>
typename EnableIf<IsClass<T>::Value>::Type Client::client_kill_unpack_arg(
    Array<String>&, ReplyCallback& reply_cb, const T& cb) {
  reply_cb = cb;
}

template <typename T, typename... Ts>
void Client::client_kill_impl(Array<String>& redis_cmd, ReplyCallback& reply,
                              const T& arg, const Ts&... args) {
  static_assert(!IsClass<T>::Value,
...
inline Client& Client::client_kill(const T& arg, const Ts&... args) {
  // TODO
  // static_assert(helpers::is_different_types<T, Ts...>::Value, "Should only
  // have one distinct value per filter type"); static_assert(!(IsClass<T>::Value
  // && IsSame<T, typename helpers::back<T, Ts...>::Type>::Value), "Should have
  // at least one filter");
...
template <typename T, typename... Ts>
inline Client& Client::client_kill(const String& host, int port, const T& arg,
                                   const Ts&... args) {
  // TODO
  // static_assert(helpers::is_different_types<T, Ts...>::Value, "Should only
  // have one distinct value per filter type");
  Array<String> redis_cmd({"CLIENT", "KILL"});
...
Future<Reply> Client::client_kill_future(const T arg, const Ts... args) {
  ...
  return Execute(std::bind(
      [this](T arg, Ts... args, const ReplyCallback& cb) -> Client& {
