...
MemoryAllocator* MemoryAllocator::GetGlobalMalloc() {
  ...
  if (!global_malloc_instance->IsInternallyThreadSafe()) {
    global_malloc_instance = new MemoryAllocatorThreadSafeProxy(global_malloc_instance);
  }

#if defined(FUN_MALLOC_VERIFY)
  global_malloc_instance = new MemoryAllocatorVerifyProxy(global_malloc_instance);
#endif

#if defined(FUN_MALLOC_LEAKDETECTION)
  global_malloc_instance = new MemoryAllocatorLeakDetectionProxy(global_malloc_instance);
#endif

#if defined(FUN_USE_MALLOC_FILL_BYTES)
  global_malloc_instance = new MemoryAllocatorPoisonProxy(global_malloc_instance);
#endif
}
...
void* UnsafeMemory::ReallocExternal(void* ptr, size_t new_count, uint32 alignment) {
...
void UnsafeMemory::Trim() {
  MemoryAllocator::GetGlobalMalloc()->Trim();
}
