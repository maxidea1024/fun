...
AndroidTimeZoneImpl::AndroidTimeZoneImpl() : TimeZoneImpl() {
  // start with system time zone
  android_time_zone_ = JNIObjectImpl::callStaticObjectMethod(
...
void AndroidTimeZoneImpl::Init(const String& iana_id) {
  JNIObjectImpl jo_ianaId =
      JNIObjectImpl::fromString(String::fromUtf8(iana_id));
  android_time_zone_ = JNIObjectImpl::callStaticObjectMethod(
      "java.util.TimeZone", "getTimeZone",
      "(Ljava/lang/String;)Ljava/util/TimeZone;",
      static_cast<jstring>(jo_ianaId.object()));
  ...
  JNIObjectImpl jname =
      android_time_zone_.callObjectMethod("getID", "()Ljava/lang/String;");
  bool found = (jname.toString().toUtf8() == iana_id);
  for (int32 style = 1; !found && style-- > 0;) {
    for (int32 dst = 1; !found && dst-- > 0;) {
      jname = android_time_zone_.callObjectMethod(
...
String AndroidTimeZoneImpl::GetDisplayName(TimeZone::TimeType time_type,
                                           TimeZone::NameType name_type,
                                           const Locale& locale) const {
  String name;

  if (android_time_zone_.IsValid()) {
    jboolean daylight_time =
        (time_type == TimeZone::DaylightTime);  // treat TimeZone::GenericTime
                                                // as TimeZone::StandardTime

    // treat all NameTypes as java TimeZone style LONG (value 1), except of
    // course TimeZone::ShortName which is style SHORT (value 0);
    jint style = (name_type == TimeZone::ShortName ? 0 : 1);

    JNIObjectImpl jlanguage = JNIObjectImpl::fromString(
        Locale::LanguageToString(locale.GetLanguage()));
    JNIObjectImpl jcountry =
        JNIObjectImpl::fromString(Locale::CountryToString(locale.Getcountry()));
    JNIObjectImpl jvariant =
        JNIObjectImpl::fromString(Locale::ScriptToString(locale.GetScript()));
    JNIObjectImpl jlocale(
        "java.util.Locale",
    ...
    JNIObjectImpl jname = android_time_zone_.callObjectMethod(
        "getDisplayName", "(ZILjava/util/Locale;)Ljava/lang/String;",
        daylight_time, style, jlocale.object());
...
int32 AndroidTimeZoneImpl::GetOffsetFromUtc(int64 at_msecs_since_epoch) const {
  // GetOffsetFromUtc() is invoked when android_time_zone_ may not yet be set at
  // startup, so a validity test is needed here
  if (android_time_zone_.IsValid()) {
    // the java method getOffset() returns milliseconds, but TimeZone returns
    // seconds
    return android_time_zone_.callMethod<jint>(
               "getOffset", "(J)I", static_cast<jlong>(at_msecs_since_epoch)) /
           1000;
  } else {
    return 0;
  }
...
int32 AndroidTimeZoneImpl::GetStandardTimeOffset(
    int64 at_msecs_since_epoch) const {
  // the java method does not use a reference time
  FUN_UNUSED(at_msecs_since_epoch);

  if (android_time_zone_.IsValid()) {
    // the java method getRawOffset() returns milliseconds, but TimeZone returns
    // seconds
...
int32 AndroidTimeZoneImpl::GetDaylightTimeOffset(
    int64 at_msecs_since_epoch) const {
  return GetOffsetFromUtc(at_msecs_since_epoch) -
         GetStandardTimeOffset(at_msecs_since_epoch);
}
...
bool AndroidTimeZoneImpl::IsDaylightTime(int64 at_msecs_since_epoch) const {
  if (android_time_zone_.IsValid()) {
    JNIObjectImpl jDate("java/util/Date", "(J)V",
                        static_cast<jlong>(at_msecs_since_epoch));
    return android_time_zone_.callMethod<jboolean>(
        "inDaylightTime", "(Ljava/util/Date;)Z", jDate.object());
  } else {
    return false;
  }
}

TimeZoneImpl::Data AndroidTimeZoneImpl::GetData(
...
TimeZoneImpl::Data AndroidTimeZoneImpl::NextTransition(
...
TimeZoneImpl::Data AndroidTimeZoneImpl::PreviousTransition(
...
String AndroidTimeZoneImpl::GetSystemTimeZoneId() const {
  JNIObjectImpl android_system_time_zone =
      JNIObjectImpl::callStaticObjectMethod("java.util.TimeZone", "getDefault",
                                            "()Ljava/util/TimeZone;");
  JNIObjectImpl system_tz_id_android =
      android_system_time_zone.callObjectMethod<jstring>("getID");
...
Array<String> AndroidTimeZoneImpl::AvailableTimeZoneIds() const {
  ...
  JNIObjectImpl android_available_id_list =
      JNIObjectImpl::callStaticObjectMethod(
          "java.util.TimeZone", "getAvailableIDs", "()[Ljava/lang/String;");

  QJNIEnvironmentPrivate jni_env;
  int32 android_tz_count = jni_env->GetArrayLength(
      static_cast<jarray>(android_available_id_list.object()));

  // need separate jobject and QAndroidJniObject here so that we can delete
  // (DeleteLocalRef) the reference to the jobject (or else the JNI reference
  // table fills after 512 entries from GetObjectArrayElement)
  jobject android_tz_object;
  JNIObjectImpl android_tz;
  for (int32 i = 0; i < android_tz_count; ++i) {
    android_tz_object = jni_env->GetObjectArrayElement(
        static_cast<jobjectArray>(android_available_id_list.object()), i);
