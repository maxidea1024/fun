...
static void ResetCodecConverterStateHelper(TextEncoding::ConverterState* state)
{
...
bool TextStream::FillReadBuffer(int64 max_len)
{
  ...
  if (device_->IsSequential()) {
    if (max_len != -1) {
      bytes_read = device_->ReadLine(buf, MathBase::Min<int64>(sizeof(buf), max_len));
    }
    else {
  ...
  {
    if (max_len != -1) {
      bytes_read = device_->Read(buf, MathBase::Min<int64>(sizeof(buf), max_len));
    }
    else {
  ...
  if (!encoding_ || auto_detect_unicode_) {
    ...
    encoding_ = TextEncoding::EncodingForUtfText(ByteArray::FromRawData(buf, bytes_read), encoding_);
    if (!encoding_) {
      encoding_ = TextEncoding::EncodingForLocale();
      write_converter_state_.flags |= TextEncoding::IgnoreHeader;
    }
  ...
  // convert to unicode
  read_buffer_ += encoding_ ? encoding_->ToUnicode(buf, bytes_read, &read_converter_state_)
...
void TextStreamImpl::FlushWriteBuffer()
{
...
#if defined(FUN_TEXTSTREAM_DEBUG)
  qDebug("TextStreamImpl::FlushWriteBuffer(), using %s encoding_ (%s generating BOM)",
         encoding_ ? encoding_->GetName().ConstData() : "no",
         !encoding_ || (write_converter_state_.flags & TextEncoding::IgnoreHeader) ? "not" : "");
#endif

  // convert from unicode to raw data
  // encoding_ might be null if we're already inside global destructors (QTestCodec::EncodingForLocale returned null)
  ByteArray data = encoding_  ? encoding_->FromUnicode(write_buffer_.ConstData(), write_buffer_.Len(), &write_converter_state_)
  ...
  qDebug("TextStreamImpl::FlushWriteBuffer(), device_->Write(\"%s\") == %d",
        qt_prettyDebug(data.ConstData(), MathBase::Min(data.Len(), 32), data.Len()).ConstData(), int32(bytes_written));
#endif
...
#if defined(FUN_TEXTSTREAM_DEBUG)
  qDebug("TextStreamImpl::FlushWriteBuffer() wrote %d bytes", int32(bytes_written));
#endif
...
UString TextStreamImpl::Read(int32 max_len)
{
  ...
  if (string_) {
    ...
    while (read_buffer_.Len() - read_buffer_offset_ < max_len && FillReadBuffer()) ;

    last_token_len_ = MathBase::Min(max_len, read_buffer_.Len() - read_buffer_offset_);
...
bool TextStreamImpl::Scan(const UNICHAR** ptr,
                          int32* token_length,
                          int32 max_len,
                          TokenDelimiter delimiter)
{
  ...
  do {
    ...
    for (; !found_token && start_offset < end_offset && (!max_len || total_size < max_len); ++start_offset) {
  ...
  if (delimiter == EndOfLine && total_size > 0 && !found_token) {
    if (((string_ && string_offset_ + total_size == string_->Len()) || (device_ && device_->AtEnd()))
        && last_char == '\r') {
      consume_delimiter = true;
      ++delim_size;
    }
...
#if defined(FUN_TEXTSTREAM_DEBUG)
  qDebug("TextStreamImpl::Scan(%p, %p, %d, %x) token length = %d, delimiter = %d",
          ptr, token_length, max_len, (int32)delimiter, total_size - delim_size, delim_size);
#endif
...
void TextStreamImpl::RestoreToSavedConverterState()
{
  if (read_converter_saved_state_) {
    ...
    CopyConverterStateHelper(&read_converter_state_, read_converter_saved_state_);
  }
  else {
...
void TextStreamImpl::PutString(const UNITHCAR* str, int32 len, bool is_number)
{
  if (params_.field_width > len) {
    ...
    if (params_.field_alignment == TextStream::AlignAccountingStyle && is_number) {
...
void TextStreamImpl::PutNumber(uint64 number, bool is_negative)
{
  ...
  if (is_negative && base == 10) {
    result = dd->Int64ToString(-static_cast<int64>(number), -1, base, -1, flags);
  }
  else if (is_negative) {
    ...
    if (number == 0 && base == 8 && params_.number_flags & TextStream::ShowBase
...
TextStreamImpl::NumberParsingStatus TextStreamImpl::GetNumber(uint64* ret)
{
  ...
  if (base == 0) {
    ...
    else if (ch == locale_.GetNegativeSign() || ch == locale_.GetPositiveSign() || ch.IsDigit()) {
      base = 10;
    }
    else {
      UngetChar(ch);
  ...
  case 10: {
    ...
    if (sign != locale_.GetNegativeSign() && sign != locale_.GetPositiveSign()) {
      if (!sign.IsDigit()) {
        UngetChar(sign);
        return NumberParsingStatus::MissingDigit;
      }
    ...
    while (GetChar(&ch)) {
      ...
      else if (locale_ != Locale::C() && ch == locale_.GetGroupSeparator()) {
        continue;
      }
      else {
        UngetChar(ch);
...
int64 TextStream::Tell() const
{
  if (impl_->device_) {
    ...
    int old_read_buffer_offset = impl_->read_buffer_offset_ + impl_->read_converter_saved_state_offset_;
...
UString* TextStream::GetString() const
{
...
UNICHAR TextStream::GetPadChar() const
{
  return impl_->params_.pad_char;
}

void TextStream::SetPadChar(UNICHAR ch)
{
  return impl_->params_.pad_char = ch;
}
...
int32 TextStream::GetIntegerBase() const
{
  return impl_->params_.integer_base;
}
...
Status TextStream::GetStatus() const
{
...
bool TextStream::ReadLineInto(String* buf, int64 max_len)
{
  ...
  int32 len;
  if (!impl_->Scan(&read_ptr, &len, int32(max_len), TextStreamImpl::EndOfLine)) {
    if (buf && !buf->IsNull()) {
      buf->Resize(0);
    }
...
TextEncoding* TextStream::GetEncoding() const
{
...
{
  if (impl_->write_buffer_.IsEmpty()) {
    impl_->write_converter_state_.flags.SetFlags(TextEncoding::IgnoreHeader, !generate);
  }
}

bool TextStream::GetGenerateByteOrderMark() const
{
  return (impl_->write_converter_state_.flags & TextEncoding::IgnoreHeader) == 0;
}
...
TextStream& TextStream::operator << (double d)
{
  ...
  if (GetLocale() != Locale::C() && !(number_options & Locale::OmitGroupSeparator)) {
    flags |= LocaleData::ThousandsGroup;
  }
  ...
  UString num = dd->DoubleToString(d, impl_->params_.real_number_precision, form, -1, flags);
