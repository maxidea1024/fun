...
class FUN_BASE_API TextStream {
 public:
  ...
  TextStream(const ByteArray& array,
             IoDevice::OpenMode mode = IoDevice::ReadOnly);
  virtual ~TextStream();
  ...
  void SetString(UString* string,
                 IoDevice::OpenMode open_mode = IoDevice::ReadWrite);
...
class FUN_BASE_API TextStreamManipulator {
 public:
  TextStreamManipulator(TSMFI m, int32 a) : mi(m), arg(a), ch(0) {}

  TextStreamManipulator(TSMFC m, UNICHAR ch) : mc(m), arg(-1), ch(ch) {}
