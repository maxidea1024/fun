...
CustomVersion unused_custom_version_(Uuid(0, 0, 0, 0xF99D40C1), 0,
                                     "Unused custom version");
...
struct EnumCustomVersion_DEPRECATED {
  ...
  CustomVersion ToCustomVersion() const {
    ...
    return CustomVersion(Uuid(0, 0, 0, tag), version,
                         String::Format("EnumTag{0}", tag));
  }
};
...
const String CustomVersion::GetFriendlyName() const {
  if (friendly_name_ == "") {
    friendly_name_ =
        CustomVersionContainer::GetRegistered().GetFriendlyName(key);
  }
...
void CustomVersionContainer::Clear() { versions_.Clear(); }
...
String CustomVersionContainer::ToString(const String& indent) const {
  ...
  for (const CustomVersion& version : versions_) {
    ...
    versions_as_string += String::Format(
        "key={0}  version={1}  friendly name={2}\n", version.key.ToString(),
        version.version, version.GetFriendlyName());
  }
...
void CustomVersionContainer::Serialize(
    Archive& ar, CustomVersionSerializationFormat::Type format) {
  switch (format) {
    default:
      fun_check(false);
    ...
    case CustomVersionSerializationFormat::Enums: {
      ...
      // They should never be saved - they only exist for backward
      // compatibility.
...
const CustomVersion* CustomVersionContainer::GetVersion(const Uuid& key) const {
  // A testing tag was written out to a few archives during testing so we need
  // to handle the existence of it to ensure that those archives can still be
  // loaded.
...
void CustomVersionContainer::SetVersion(const Uuid& custom_key, int32 version,
...
CustomVersionRegistration::CustomVersionRegistration(
    const Uuid& key, int32 version, const String& friendly_name) {
  ...
  if (CustomVersion* existing_registration = versions.Find(key)) {
    // We don't allow the registration details to change across registrations -
    // this code path only exists to support hotreload
    ...
    // TODO
    // ENSURE_MSGF(
    //    existing_registration->version == version &&
    //    existing_registration->GetFriendlyName() == friendly_name, "Custom
    //    version registrations cannot change between hotreloads - \"%s\"
    //    version %d is being reregistered as \"%s\" version %d",
