...
ArchiveProxy::ArchiveProxy(Archive& inner_archive)
    : Archive(inner_archive), inner_archive_(inner_archive) {}
...
Archive::Archive(const Archive& rhs) {
  ...
  // Don't know why this is set to false, but this is what the original copying
  // code did
  // ArIsFilterEditorOnly  = false;

  custom_version_container_ =
      new CustomVersionContainer(*rhs.custom_version_container_);
}
...
Archive::~Archive() { delete custom_version_container_; }
...
rhs.ArIsModifyingWeakAndStrongReferences; ArIsCountingMemory =
rhs.ArIsCountingMemory;
...
String Archive::GetArchiveName() const { return "Archive"; }

//#if FUN_WITH_EDITOR
// Archive::ScopeAddDebugData::ScopeAddDebugData(Archive& InAr, const Name&
// debug_data)
//  : ar(InAr)
//{
...
//{
//  // The base Archive does not implement this method. Use ArchiveFObject
//  instead. fun_log(LogSerialization, Fatal, "Archive does not support
//  LazyObjectPtr serialization. Use CArchiveUObject instead."); return *this;
//}
//
// Archive& Archive::operator & (class AssetPtr& asset_ptr)
//{
//  // The base Archive does not implement this method. Use ArchiveFObject
//  instead. fun_log(LogSerialization, Fatal, "Archive does not support AssetPtr
//  serialization. Use CArchiveUObject instead."); return *this;
//}
//
...
const CustomVersionContainer& Archive::GetCustomVersions() const {
  if (custom_versions_are_reset_) {
    ...
    // If the archive is for reading then we want to use currently registered
    // custom versions, otherwise we expect serialization code to use
    // UsingCustomVersion to populate the container.
    if (IsLoading()) {
...
void Archive::ResetCustomVersions() { custom_versions_are_reset_ = true; }
...
void Archive::UsingCustomVersion(const Uuid& key) {
  ...
  auto* registered_version =
      CustomVersionContainer::GetRegistered().GetVersion(key);

  // Ensure that the version has already been registered.
  // If this fails, you probably don't have an CustomVersionRegistration
  // variable defined for this GUID.
  fun_check_ptr(registered_version);

  const_cast<CustomVersionContainer&>(GetCustomVersions())
      .SetVersion(key, registered_version->version,
                  registered_version->GetFriendlyName());
}
...
void Archive::SetCustomVersion(const Uuid& key, int32 version,
                               const String& friendly_name) {
  const_cast<CustomVersionContainer&>(GetCustomVersions())
      .SetVersion(key, version, friendly_name);
}
...
class AsyncCompressionChunk : public NonAbandonableTask {
 public:
  ...
  void DoWork() {
    ...
    fun_verify(Compression::CompressMemory(flags, compressed_buffer,
                                           compressed_size, uncompressed_buffer,
                                           uncompressed_size));
  }

  ALWAYS_INLINE TStatId GetStatId() const {
    RETURN_QUICK_DECLARE_CYCLE_STAT(AsyncCompressionChunk,
                                    STATGROUP_ThreadPoolAsyncTasks);
  }
};
#endif  // WITH_MULTI_THREADED_COMPRESSION

void Archive::SerializeCompressed(void* v, int64 length, CompressionFlags flags,
                                  bool treat_buffer_as_file_reader) {
  // TODO
  /*
  if (IsLoading()) {
    ...
    *this & package_file_tag;
    bool was_byte_swapped = package_file_tag.compressed_size !=
PACKAGE_FILE_TAG;
    ...
    *this & summary;
    ...
    if (was_byte_swapped) {
      ...
      package_file_tag.uncompressed_size =
ByteOrder::Flip(package_file_tag.uncompressed_size); } else {
...
CompressionConstants::LOADING_COMPRESSION_CHUNK_SIZE;
    }

    // Figure out how many chunks there are going to be based on uncompressed
size and compression chunk size. int64 total_chunk_count =
(summary.uncompressed_size + loading_compression_chunk_size - 1) /
loading_compression_chunk_size;

    // Allocate compression chunk infos and serialize them, keeping track of max
size of compression chunks used. CompressedChunkInfo* compression_chunks = new
CompressedChunkInfo[total_chunk_count]; int64 max_compressed_size = 0; for
(int32 chunk_index = 0; chunk_index < total_chunk_count; chunk_index++) { *this
& compression_chunks[chunk_index]; if (was_byte_swapped) {
...
ByteOrder::Flip(compression_chunks[chunk_index].uncompressed_size);
      ...
      max_compressed_size =
MathBase::Max(compression_chunks[chunk_index].compressed_size,
max_compressed_size);
    }

    int64 padding = 0;

    // Set up destination pointer and allocate memory for compressed chunk[s]
(one at a time). uint8* dst = (uint8*)v; void* compressed_buffer =
UnsafeMemory::Malloc(max_compressed_size + padding);

    // Iterate over all chunks, serialize them into memory and decompress them
directly into the destination pointer for (int64 chunk_index = 0; chunk_index <
total_chunk_count; chunk_index++) { const CompressedChunkInfo& chunk =
compression_chunks[chunk_index];
      // Read compressed data.
      Serialize(compressed_buffer, chunk.compressed_size);
      // Decompress into dst pointer directly.
      fun_verify(Compression::UncompressMemory(flags, dst,
chunk.uncompressed_size, compressed_buffer, chunk.compressed_size, (padding > 0)
? true : false));
    ...
    // Serialize package file tag used to determine endianess in
LoadCompressedData. CompressedChunkInfo package_file_tag;
    package_file_tag.compressed_size = PACKAGE_FILE_TAG;
    package_file_tag.uncompressed_size = GSavingCompressionChunkSize;
    *this & package_file_tag;

    // Figure out how many chunks there are going to be based on uncompressed
size and compression chunk size. int64 total_chunk_count = (length +
GSavingCompressionChunkSize - 1) / GSavingCompressionChunkSize + 1;

    // Keep track of current position so we can later seek back and overwrite
stub compression chunk infos. int64 start_pos = Tell();

    // Allocate compression chunk infos and serialize them so we can later
overwrite the data. CompressedChunkInfo* compression_chunks = new
CompressedChunkInfo[total_chunk_count]; for (int64 chunk_index = 0; chunk_index
< total_chunk_count; chunk_index++) { *this & compression_chunks[chunk_index];
    }

    // The uncompressd size is equal to the passed in length.
    compression_chunks[0].uncompressed_size = length;
    // Zero initialize compressed size so we can update it during chunk
compression. compression_chunks[0].compressed_size = 0;
  ...
  #define MAX_COMPRESSION_JOBS  (16)
    ...
    if (GNumUnusedThreads_SerializeCompressed == (uint32)-1) {
      ...
      if (Parse::Param(CommandLine::Get(), TEXT("USEALLAVAILABLECORES")) ==
true) { GNumUnusedThreads_SerializeCompressed = 0;
      }
    }

    // Maximum number of concurrent async tasks we're going to kick off. This is
based on the number of processors
    // available in the system.
    int32 MaxConcurrentAsyncChunks =
MathBase::Clamp<int32>(CPlatformMisc::NumberOfCores() -
GNumUnusedThreads_SerializeCompressed, 1, MAX_COMPRESSION_JOBS); if
(Parse::Param(CommandLine::Get(), TEXT("MTCHILD"))) {
      // throttle this back when doing MT cooks
      MaxConcurrentAsyncChunks =
MathBase::Min<int32>(MaxConcurrentAsyncChunks,4);
    }

    // Number of chunks left to finalize.
    int64 num_chunks_left_to_finalize = (length + GSavingCompressionChunkSize -
1) / GSavingCompressionChunkSize;
    ...
    // Loop while there is work left to do based on whether we have finalized
all chunks yet. while (num_chunks_left_to_finalize) {
      // If true we are waiting for async tasks to complete and should wait to
complete some
      ...
      if (num_chunks_left_to_kick_off > 0) {
        // Find free index based on looking at uncompressed size. We can't use
the thread counter
        ...
        for (int32 i = 0; i < MaxConcurrentAsyncChunks; ++i) {
          if (!async_chunk_index[i]) {
            free_index = i;
            fun_check(async_chunks[free_index].IsIdle()); // this is not
supposed to be in use break;
          }
        }
        ...
        if (free_index != INVALID_INDEX) {
          ...
          // 2 times the uncompressed size should be more than enough; the
compressed data shouldn't be that much larger new_chunk.compressed_size = 2 *
GSavingCompressionChunkSize;
          // Allocate compressed buffer placeholder on first use.
          if (new_chunk.compressed_buffer == nullptr) {
            new_chunk.compressed_buffer =
UnsafeMemory::Malloc(new_chunk.compressed_size);
          }

          // By default everything is chunked up into
GSavingCompressionChunkSize chunks. new_chunk.uncompressed_size   =
MathBase::Min(byte_remaining_to_kick_off, (int64)GSavingCompressionChunkSize);
          ...
          if (treat_buffer_as_file_reader) {
            // Allocate memory on first use. We allocate the maximum amount to
allow reuse. if (new_chunk.uncompressed_buffer == nullptr) {
              new_chunk.uncompressed_buffer =
UnsafeMemory::Malloc(GSavingCompressionChunkSize);
            }
            ((Archive*)v)->Serialize(new_chunk.uncompressed_buffer,
new_chunk.uncompressed_size);
          }
          ...
          // Update status variables for tracking how much work is left, what to
do next. byte_remaining_to_kick_off -= new_chunk.uncompressed_size;
      ...
      for (int32 i = 0; i < MaxConcurrentAsyncChunks; ++i) {
        fun_check(async_chunk_index[i] == 0 || async_chunk_index[i] >=
retire_chunk_index); fun_check(async_chunk_index[i] < retire_chunk_index +
MaxConcurrentAsyncChunks); if (async_chunk_index[i] == retire_chunk_index) {
          oldest_async_chunk_index = i;
        }
      }
      fun_check(oldest_async_chunk_index != INVALID_INDEX);  // the retire chunk
better be outstanding

      bool is_chunk_ready;
      if (need_to_wait_for_async_task) {
        // This guarantees that the async work has finished, doing it on this
thread if it hasn't been started
      ...
      if (is_chunk_ready) {
        AsyncCompressionChunk& done_chunk =
async_chunks[oldest_async_chunk_index].GetTask();
        ...
        compression_chunks[compression_chunk_index].compressed_size =
done_chunk.compressed_size;
        compression_chunks[compression_chunk_index].uncompressed_size =
done_chunk.uncompressed_size;

        // Keep track of total compressed size, stored in first chunk.
        compression_chunks[0].compressed_size += done_chunk.compressed_size;

        // Clean up chunk. src and dst buffer are not touched as the contain
allocations we keep till the end. async_chunk_index[oldest_async_chunk_index] =
0; done_chunk.compressed_size = 0; done_chunk.uncompressed_size = 0;
    ...
    // 2 times the uncompressed size should be more than enough; the compressed
data shouldn't be that much larger int64 compressed_buffer_size = 2 *
GSavingCompressionChunkSize; void* compressed_buffer =
UnsafeMemory::Malloc(compressed_buffer_size);

    while (bytes_remaining > 0) {
      int64 bytes_to_compress = MathBase::Min(bytes_remaining,
(int64)GSavingCompressionChunkSize); int64 compressed_size =
compressed_buffer_size;
      ...
      fun_verify(Compression::CompressMemory(flags, compressed_buffer,
compressed_size_int, src, bytes_to_compress)); compressed_size =
compressed_size_int;
...
bytes_to_compress; current_chunk_index++;
    ...
    // Overrwrite chunk infos by seeking to the beginning, serializing the data
and then
...
void Archive::SerializeIntPacked(uint32& value) {
  if (IsLoading()) {
    ...
    Serialize(packed_bytes.MutableData(),
              packed_bytes.Count());  // Actually serialize the bytes we made
  }
}
...
VARARG_BODY(void, Archive::Printf, const TCHAR*, VARARG_NONE) {
  // We need to use malloc here directly as GMalloc might not be safe, e.g. if
called from within GMalloc! int32 BufferSize  = 1024; TCHAR* Buffer = nullptr;
  int32 Result = -1;
...
ArchiveSaveCompressedProxy::ArchiveSaveCompressedProxy(
    Array<uint8>& compressed_data, CompressionFlags compression_flags)
    : compressed_data_(compressed_data), compression_flags_(compression_flags) {
  SetPersistent();
  SetSaving();
  ...
  tmp_data_start_ = (uint8*)UnsafeMemory::Malloc(
      CompressionConstants::LOADING_COMPRESSION_CHUNK_SIZE);
  tmp_data_end_ =
      tmp_data_start_ + CompressionConstants::LOADING_COMPRESSION_CHUNK_SIZE;
...
void ArchiveSaveCompressedProxy::Flush() {
  if ((tmp_data_ - tmp_data_start_) > 0) {
    // This will call Serialize so we need to indicate that we want to serialize
    // to array.
    should_serialize_to_array_ = true;
    SerializeCompressed(tmp_data_start_, tmp_data_ - tmp_data_start_,
                        compression_flags_);
    should_serialize_to_array_ = false;
...
void ArchiveSaveCompressedProxy::Serialize(void* v, int64 len) {
  ...
  // Regular call to serialize, queue for compression.
    ...
    while (len) {
      const int32 len_to_copy =
          MathBase::Min<int32>(len, (int32)(tmp_data_end_ - tmp_data_));
      ...
      // Tmp buffer fully exhausted, compress it.
        ...
        // Flush existing data to array after compressing it. This will call
        // Serialize again so we need to handle recursion.
        Flush();
      }
    }
  }
...
void ArchiveSaveCompressedProxy::Seek(int64 pos) {
  ...
  if (should_serialize_to_array_) {
    ...
    // TODO
    // fun_log(Serialization, Fatal, "Seeking not supported with
...
ArchiveLoadCompressedProxy::ArchiveLoadCompressedProxy(
    const Array<uint8>& compressed_data, CompressionFlags compression_flags)
    : compressed_data_(compressed_data), compression_flags_(compression_flags) {
  SetPersistent();
  SetLoading();
...
void ArchiveLoadCompressedProxy::DecompressMoreData() {
  ...
  SerializeCompressed(
      tmp_data_start_,
      CompressionConstants::LOADING_COMPRESSION_CHUNK_SIZE /** it's ignored, but
                                                              that's how much we
                                                              serialize */
      ,
      compression_flags_);
...
void ArchiveLoadCompressedProxy::Serialize(void* v, int64 len) {
  ...
  // Regular call to serialize, read from temp buffer
    ...
    while (len) {
      ...
      // Enough room in buffer to copy some data.
      if (len_to_copy) {
        // We pass in a NULL pointer when forward seeking. In that case we don't
        // want to copy the data but only care about pointing to the proper
        // spot.
        if (dst_data) {
      ...
      // Tmp buffer fully exhausted, decompress new one.
        ...
        // Decompress more data. This will call Serialize again so we need to
        // handle recursion.
        DecompressMoreData();
      }
    }
...
void ArchiveLoadCompressedProxy::Seek(int64 pos) {
  ...
  fun_check(diff >= 0);
  // Seek by serializing data, albeit with NULL destination so it's just
  // decompressing data.
  Serialize(nullptr, diff);
}

int64 ArchiveLoadCompressedProxy::Tell() { return raw_bytes_serialized_; }
