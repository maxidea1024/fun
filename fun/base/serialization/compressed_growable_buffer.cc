...
CompressedGrowableBuffer::CompressedGrowableBuffer(
    int32 max_pending_buffer_size, CompressionFlags compression_flags)
...
int32 CompressedGrowableBuffer::Append(void* data, int32 size) {
  ...
  if (max_pending_buffer_size_ - pending_compression_buffer_.Count() < size) {
    // Allocate temporary buffer to hold compressed data. It is bigger than the
    // uncompressed size as compression is not guaranteed to create smaller data
    // and we don't want to handle that case so we simply assert if it doesn't
    // fit. For all practical purposes this works out fine and is what other
    // code in the engine does as well.
    ...
    // TODO
    // fun_verify(Compression::Compress(compression_flags_, tmp_buffer,
    // compressed_size, pending_compression_buffer_.ConstData(),
    // pending_compression_buffer_.Count()));
    Compression::Compress(compression_flags_, tmp_buffer, compressed_size,
                          pending_compression_buffer_.ConstData(),
                          pending_compression_buffer_.Count());

    // Append the compressed data to the compressed buffer and delete temporary
    // data.
    int32 start_index = compressed_buffer_.AddUninitialized(compressed_size);
    UnsafeMemory::Memcpy(&compressed_buffer_[start_index], tmp_buffer,
                         compressed_size);
    ...
    info.uncompressed_offset =
        current_offset_ - pending_compression_buffer_.Count();
...
void* CompressedGrowableBuffer::Access(int32 offset) {
  ...
  if (decompressed_buffer_book_keeping_info_index_ != INVALID_INDEX) {
    const BufferBookKeeping& info =
        book_keeping_info_[decompressed_buffer_book_keeping_info_index_];
    // Cache HIT.
    if ((info.uncompressed_offset <= offset) &&
        (info.uncompressed_offset + info.uncompressed_size > offset)) {
      // Figure out index into uncompressed data and set it. DecompressionBuffer
      // (return value) is going to be valid till the next call to Access or
      // Unlock.
  ...
  if (uncompressed_data == nullptr) {
    for (int32 info_index = 0; info_index < book_keeping_info_.Count();
         ++info_index) {
      ...
      if ((info.uncompressed_offset <= offset) &&
        ...
        // TODO
        // fun_verify(Compression::Uncompress(compression_flags_,
        // decompressed_buffer_.MutableData(), info.uncompressed_size,
        // &compressed_buffer_[info.compressed_offset], info.compressed_size));
        Compression::Uncompress(
            compression_flags_, decompressed_buffer_.MutableData(),
            info.uncompressed_size, &compressed_buffer_[info.compressed_offset],
            info.compressed_size);

        // Figure out index into uncompressed data and set it.
        // DecompressionBuffer (return value) is going to be valid till the next
        // call to Access or Unlock.
        const int32 internal_offset = offset - info.uncompressed_offset;
        uncompressed_data = &decompressed_buffer_[internal_offset];
  ...
  // If we still haven't found the data it might be in the pending compression
  // buffer.
  if (uncompressed_data == nullptr) {
    const int32 uncompressed_start_offset =
        current_offset_ - pending_compression_buffer_.Count();
    if ((uncompressed_start_offset <= offset) && (current_offset_ > offset)) {
      // Figure out index into uncompressed data and set it.
      // pending_compression_buffer_ (return value) is going to be valid till
      // the next call to Access, Unlock or Append.
