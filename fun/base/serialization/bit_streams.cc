...
// Table.
static const uint8 SHIFT_TABLE[8] = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80 };
static const uint8 MASK_TABLE[8]  = { 0x00, 0x01, 0x03, 0x07, 0x0f, 0x1f, 0x3f, 0x7f };

// Optimized arbitrary bit range memory copy routine.

//TODO UnsafeMemory 쪽으로 옮겨주는게 어떨런지??
void appBitsCpy(uint8* dst, int32 dest_bit, const uint8* src, int32 src_bit, int32 bit_count)
{
  if (bit_count == 0) {
    return;
  }
  ...
  if (bit_count <= 8) {
    ...
    uint32 last_mask = 0xFE << ((dest_bit + bit_count - 1) & 7) ; // Pre-shifted left by 1.
    uint32 accu;

    if (src_index == last_src) {
      accu = (src[src_index] >> shift_src);
    } else {
      accu =((src[src_index] >> shift_src) | (src[last_src ] << (8-shift_src)));
    }

    if (dst_index == last_dest) {
      uint32 multi_mask = first_mask & ~last_mask;
      dst[dst_index] = ((dst[dst_index] & ~multi_mask) | ((accu << shift_dest) & multi_mask));
    } else {
      dst[dst_index] = (uint8)((dst[dst_index] & ~first_mask) | ((accu << shift_dest) & first_mask)) ;
      dst[last_dest ] = (uint8)((dst[last_dest ] & last_mask)  | ((accu >> (8-shift_dest)) & ~last_mask)) ;
    }
  ...
  // Lead-in - first copy.
  dst[dst_index] = (uint8) ((bit_accu & first_src_mask) | (dst[dst_index] &  ~first_src_mask));
  src_index++;
  dst_index++;
  ...
  for (; full_loop > 1; full_loop--) {
    ...
    bit_accu = (((uint32)src[src_index] << shift_count) + (bit_accu)) >> 8; // Copy in the new, discard the old.
    src_index++;
  ...
  if (last_src_mask != 0xFF) {
    if ((uint32)(src_bit+bit_count-1)/8 == src_index) { // Last legal byte ?
      bit_accu = (((uint32)src[src_index] << shift_count) + (bit_accu)) >> 8;
    } else {
      bit_accu = bit_accu >> 8;
    }

    dst[dst_index] = (uint8)((dst[dst_index] & last_src_mask) | (bit_accu & ~last_src_mask));
  }
}
...
BitWriter::BitWriter()
  : count_(0), max_(0), allow_resize_(false) {}
...
void BitWriter::SerializeInt(uint32& value, uint32 value_max) {
  ...
  if (write_value >= value_max) {
    //TODO
    //fun_log(LogSerialization, Error, "BitWriter::SerializeInt(): value out of bounds (value: %u, value_max: %u)", write_value, value_max);
    //ENSURE_MSGF(false, "BitWriter::SerializeInt(): value out of bounds (value: %u, value_max: %u)", write_value, value_max);
  ...
  if (AllowAppend((int32)MathBase::CeilLogTwo(value_max))) {
    ...
    for (uint32 mask = 1; new_value + mask < value_max && mask; mask *= 2, local_count++) {
      if (write_value & mask) {
...
void BitWriter::WriteIntWrapped(uint32 value, uint32 value_max) {
  if (AllowAppend((int32)MathBase::CeilLogTwo(value_max))) {
    uint32 new_value = 0;
    for (uint32 mask = 1; new_value + mask < value_max && mask; mask *= 2, count_++) {
      if (value & mask) {
        buffer_[count_>>3] += SHIFT_TABLE[count_&7];
        new_value += mask;
      }
...
void BitWriterMark::Copy(BitWriter& writer, Array<uint8>& buffer) {
  ...
  if (byte_count > 0) {
    ...
    buffer[byte_count-1] = 0; // Make sure the last byte is 0 out, because appBitsCpy wont touch the last bits
    appBitsCpy(buffer.MutableData(), 0, writer.buffer_.ConstData(), count_, writer.count_ - count_);
  }
}
...
BitReader::BitReader(uint8* src, int64 bit_count)
  : count_(bit_count), pos_(0) {
...
void BitReaderMark::Copy(BitReader& reader, Array<uint8>& buffer) {
  ...
  if (byte_count > 0) {
    buffer.ResizeUninitialized(byte_count); // This makes room but doesnt zero
    buffer[byte_count-1] = 0;               // Make sure the last byte is 0 out, because appBitsCpy wont touch the last bits
    appBitsCpy(buffer.MutableData(), 0, reader.buffer_.ConstData(), pos_, reader.pos_ - pos_);
  }
}
