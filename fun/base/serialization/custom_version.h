...
class FUN_BASE_API CustomVersion {
 public:
  ...
  inline bool operator==(const Uuid& key) const { return this->key == key; }
 ...
 private:
  /** Friendly name for error messages or whatever. Lazy initialized for
   * serialized versions */
...
struct CustomVersionKeyFuncs : BaseKeyFuncs<CustomVersion, Uuid, false> {
  ...
  static inline bool Matches(const Uuid& a, const Uuid& b) { return a == b; }
...
class FUN_BASE_API CustomVersionContainer {
 ...
 public:
  ...
  inline const CustomVersionSet& GetAllVersions() const { return versions_; }
   ...
   *
   * @return The CustomVersion for the specified custom key, or nullptr if the
   * key doesn't exist in the container.
   */
   ...
   * @param custom_key - Custom key for which to retrieve the version.
   *
   * @return The friendly name for the specified custom key, or NAME_None if the
   * key doesn't exist in the container.
   */
   ...
   */
  void SetVersion(const Uuid& custom_key, int32 version,
                  const String& friendly_name);

  /** Serialization. */
  void Serialize(Archive& ar, CustomVersionSerializationFormat::Type format =
                                  CustomVersionSerializationFormat::Latest);
...
/**
 * This class will cause the registration of a custom version number and key
 * with the global CustomVersionContainer when instantiated, and unregister when
 * destroyed.  It should be instantiated as a global variable somewhere in the
 * module which needs a custom version number.
 */
class CustomVersionRegistration : Noncopyable {
 public:
  CustomVersionRegistration(const Uuid& key, int32 version,
                            const String& friendly_name);
