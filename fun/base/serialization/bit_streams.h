...
FUN_BASE_API void appBitsCpy(uint8* dst, int32 dest_bit, const uint8* src,
                             int32 src_bit, int32 bit_count);
...
class FUN_BASE_API BitWriter : public Archive {
 public:
   ...
   */
  inline uint8* MutableData() { return buffer_.MutableData(); }

  inline const uint8* ConstData() const { return buffer_.ConstData(); }

  inline const Array<uint8>* GetBuffer() const { return &buffer_; }

  /**
   * Returns the number of bytes written.
   */
  inline int64 GetByteCount() const { return (count_ + 7) >> 3; }

  /**
   * Returns the number of bits written.
   */
  inline int64 GetBitCount() const { return count_; }

  /**
   * Returns the number of bits the buffer supports.
   */
  inline int64 GetMaxBits() const { return max_; }

  /**
   * Marks this bit writer as overflowed.
   */
  inline void SetOverflowed() { SetError(); }
  ...
  inline void SetAllowResize(bool allow) { allow_resize_ = allow; }
  ...
  inline void WriteAlign() { count_ = (count_ + 7) & (~7); }
...
class FUN_BASE_API BitWriterMark {
 public:
  ...
  BitWriterMark(BitWriter& writer) { Init(writer); }

  int64 GetBitCount() { return count_; }
...
class FUN_BASE_API BitReader : public Archive {
 ...
 public:
  ...
  FUN_ALWAYS_INLINE_DEBUGGABLE void SerializeBits(void* dst, int64 bit_count) {
    if (GetError() || pos_ + bit_count > count_) {
      if (!GetError()) {
        SetOverflowed();
        // TODO?
        // fun_log(LogNetSerialization, Error, "BitReader::SerializeBits: pos_ +
        // bit_count > count_");
      }
  ...
  void SerializeInt(uint32& out_value, uint32 value_max) {
    ...
    for (uint32 mask = 1; value + mask < value_max && mask;
         mask *= 2, ++local_pos) {
      if (local_pos >= local_count) {
        SetOverflowed();
        // fun_log(LogNetSerialization, Error, "BitReader::SerializeInt:
  ...
  FUN_ALWAYS_INLINE_DEBUGGABLE int64 GetBitCount() const { return count_; }

  FUN_ALWAYS_INLINE_DEBUGGABLE int64 GetPosBits() const { return pos_; }

  FUN_ALWAYS_INLINE_DEBUGGABLE void EatByteAlign() {
    // Skip over remaining bits in current byte
    pos_ = (pos_ + 7) & (~0x07);
    if (pos_ > count_) {
      // fun_log(LogNetSerialization, Error, "BitReader::EatByteAlign: pos_ >
      // count_");
      SetOverflowed();
    }
  }
 ...
 private:
  inline uint8 Shift(const uint8 n) const { return 1 << n; }
};
