...
//@todo 컴파일러는 영문으로 사용하는게 좋을듯 싶네.. 한글로 경고가 나오니 좀...
#ifdef _MSC_VER
#pragma warning(disable : 4838) // warning C4838: 'double'에서 'float'(으)로의 변환에는 축소 변환이 필요합니다.
#pragma warning(disable : 4305) // '초기화 중': 'double'에서 'float'(으)로 잘립니다.
#endif
...
LinearColor::LinearColor(const Color& color) {
  ...
  a = float(color.a) * ONE_OVER_255;
}

LinearColor::LinearColor(const Vector& vector)
  : r(vector.x),
    g(vector.y),
    b(vector.z),
    a(1.f) {}

LinearColor::LinearColor(const Float16Color& color) {
  r = color.r.GetFloat();
  g = color.g.GetFloat();
  b = color.b.GetFloat();
  a = color.a.GetFloat();
...
LinearColor LinearColor::FromSRGBColor(const Color& color) {
  LinearColor ret;
  ret.r = sRGBToLinearTable[color.r];
  ret.g = sRGBToLinearTable[color.g];
  ret.b = sRGBToLinearTable[color.b];
  ret.a = float(color.a) * ONE_OVER_255;
  return ret;
}

LinearColor LinearColor::FromPow22Color(const Color& color) {
  LinearColor ret;
  ret.r = Pow22OneOver255Table[color.r];
  ret.g = Pow22OneOver255Table[color.g];
  ret.b = Pow22OneOver255Table[color.b];
  ret.a = float(color.a) * ONE_OVER_255;
  return ret;
}
...
Color LinearColor::ToRGBE() const {
  ...
  if (primary < 1E-32) {
    ...
    color.r = Math::Clamp(Math::TruncToInt(r * scale), 0, 255);
    color.g = Math::Clamp(Math::TruncToInt(g * scale), 0, 255);
    color.b = Math::Clamp(Math::TruncToInt(b * scale), 0, 255);
    color.a = Math::Clamp(Math::TruncToInt(exponent), -128, 127) + 128;
  }
...
Color LinearColor::ToColor(const bool bSRGB) const {
  float float_r = Math::Clamp(r, 0.f, 1.f);
  float float_g = Math::Clamp(g, 0.f, 1.f);
  float float_b = Math::Clamp(b, 0.f, 1.f);
  float float_a = Math::Clamp(a, 0.f, 1.f);

  if (bSRGB) {
    float_r = float_r <= 0.0031308f ? float_r * 12.92f : Math::Pow(float_r, 1.f / 2.4f) * 1.055f - 0.055f;
    float_g = float_g <= 0.0031308f ? float_g * 12.92f : Math::Pow(float_g, 1.f / 2.4f) * 1.055f - 0.055f;
    float_b = float_b <= 0.0031308f ? float_b * 12.92f : Math::Pow(float_b, 1.f / 2.4f) * 1.055f - 0.055f;
  }

  Color rv;
  rv.a = Math::FloorToInt(float_a * 255.999f);
  rv.r = Math::FloorToInt(float_r * 255.999f);
  rv.g = Math::FloorToInt(float_g * 255.999f);
  rv.b = Math::FloorToInt(float_b * 255.999f);
  return rv;
}

Color LinearColor::Quantize() const {
  return Color(
    (uint8)Math::Clamp0255<int32>(Math::TruncToInt(r*255.f)),
    (uint8)Math::Clamp0255<int32>(Math::TruncToInt(g*255.f)),
    (uint8)Math::Clamp0255<int32>(Math::TruncToInt(b*255.f)),
    (uint8)Math::Clamp0255<int32>(Math::TruncToInt(a*255.f)));
}

LinearColor LinearColor::Desaturate(float desaturation) const {
  float luminancy = ComputeLuminance();
  return Math::Lerp(*this, LinearColor(luminancy, luminancy, luminancy, 0), desaturation);
}
...
Color Color::FromHex(const String& hex_string) {
  int32 start_index = (!hex_string.IsEmpty() && hex_string[0] == char('#')) ? 1 : 0;

  if (hex_string.Len() == 3 + start_index) {
    const int32 r = Parse::HexDigit(hex_string[start_index++]);
    const int32 g = Parse::HexDigit(hex_string[start_index++]);
    const int32 b = Parse::HexDigit(hex_string[start_index]);

    return Color((r << 4) + r, (g << 4) + g, (b << 4) + b, 255);
  }

  if (hex_string.Len() == 6 + start_index) {
    Color rv;
    rv.r = (Parse::HexDigit(hex_string[start_index+0]) << 4) + Parse::HexDigit(hex_string[start_index+1]);
    rv.g = (Parse::HexDigit(hex_string[start_index+2]) << 4) + Parse::HexDigit(hex_string[start_index+3]);
    rv.b = (Parse::HexDigit(hex_string[start_index+4]) << 4) + Parse::HexDigit(hex_string[start_index+5]);
    rv.a = 255;
    return rv;
  }

  if (hex_string.Len() == 8 + start_index) {
    Color rv;
    rv.r = (Parse::HexDigit(hex_string[start_index+0]) << 4) + Parse::HexDigit(hex_string[start_index+1]);
    rv.g = (Parse::HexDigit(hex_string[start_index+2]) << 4) + Parse::HexDigit(hex_string[start_index+3]);
    rv.b = (Parse::HexDigit(hex_string[start_index+4]) << 4) + Parse::HexDigit(hex_string[start_index+5]);
    rv.a = (Parse::HexDigit(hex_string[start_index+6]) << 4) + Parse::HexDigit(hex_string[start_index+7]);
    return rv;
  }
...
LinearColor Color::FromRGBE() const {
  if (a == 0) {
    ...
    return LinearColor(r * scale, g * scale, b * scale, 1.f);
  }
}
...
LinearColor LinearColor::FGetHSV(uint8 H, uint8 S, uint8 v) {
  ...
  const Vector color_vector = (hue + s/255.f * (Vector(1, 1, 1) - hue)) * brightness;
  return LinearColor(color_vector.x, color_vector.y, color_vector.z, 1);
}

LinearColor LinearColor::LinearRGBToHSV() const {
  const float rgb_min = Math::Min3(r, g, b);
  const float rgb_max = Math::Max3(r, g, b);
  const float rgb_range = rgb_max - rgb_min;

  const float hue = ( rgb_max == rgb_min ? 0.f :
            rgb_max == r      ? fmod((((g - b) / rgb_range) * 60.f) + 360.f, 360.f) :
            rgb_max == g      ?      (((b - r) / rgb_range) * 60.f) + 120.f :
            rgb_max == b      ?      (((r - g) / rgb_range) * 60.f) + 240.f :
            0.f);

  const float saturation = (rgb_max == 0.f ? 0.f : rgb_range / rgb_max);
...
LinearColor LinearColor::HSVToLinearRGB() const {
  ...
  const float h_div60 = hue / 60.f;
  const float h_div60_floor = floorf(h_div60);
  const float h_div60_fraction = h_div60 - h_div60_floor;

  const float rgb_values[4] = {
    value,
    value * (1.f - saturation),
    value * (1.f - (h_div60_fraction * saturation)),
    value * (1.f - ((1.f - h_div60_fraction) * saturation)),
  };

  const uint32 rgb_swizzle[6][3] = { { 0, 3, 1 }, { 2, 0, 1 }, { 1, 0, 3 }, { 1, 2, 0 }, { 3, 1, 0 }, { 0, 1, 2 }, };

  const uint32 swizzle_index = ((uint32)h_div60_floor) % 6;

  return LinearColor( rgb_values[rgb_swizzle[swizzle_index][0]],
...
LinearColor LinearColor::LerpUsingHsv(const LinearColor& from, const LinearColor& to, const float progress) {
  const LinearColor from_hsv = from.LinearRGBToHSV();
  const LinearColor to_hsv = to.LinearRGBToHSV();

  float from_hue = from_hsv.r;
  float to_hue = to_hsv.r;

  // Take the shortest path to the new hue
  if (Math::Abs(from_hue - to_hue) > 180.f) {
    if (to_hue > from_hue) {
      from_hue += 360.f;
    } else {
      to_hue += 360.f;
  ...
  float new_hue = Math::Lerp(from_hue, to_hue, progress);

  new_hue = Math::Fmod(new_hue, 360.f);
  if (new_hue < 0.f) {
    new_hue += 360.f;
  }

  const float new_saturation = Math::Lerp(from_hsv.g, to_hsv.g, progress);
  const float new_value = Math::Lerp(from_hsv.b, to_hsv.b, progress);
  LinearColor interpolated = LinearColor(new_hue, new_saturation, new_value).HSVToLinearRGB();

  const float new_alpha = Math::Lerp(from.a, to.a, progress);
  interpolated.a = new_alpha;

  return interpolated;
}

LinearColor LinearColor::MakeRandomColor() {
  const uint8 hue = (uint8)(Math::FRand()*255.f);
...
LinearColor LinearColor::MakeFromColorTemperature(float tmp) {
  tmp = Math::Clamp(tmp, 1000.f, 15000.f);

  // Approximate Planckian locus in CIE 1960 UCS
  float u = (0.860117757f + 1.54118254e-4f * tmp + 1.28641212e-7f * tmp*tmp) / (1.f + 8.42420235e-4f * tmp + 7.08145163e-7f * tmp*tmp);
  float v = (0.317398726f + 4.22806245e-5f * tmp + 4.20481691e-8f * tmp*tmp) / (1.f - 2.89741816e-5f * tmp + 1.61456053e-7f * tmp*tmp);

  float x1 = 3.f * u / (2.f * u - 8.f * v + 4.f);
  float y1 = 2.f * v / (2.f * u - 8.f * v + 4.f);
  float z1 = 1.f - x1 - y1;
  ...
  const float r =  3.2404542f * x2 + -1.5371385f * y2 + -0.4985314f * z2;
  const float g = -0.9692660f * x2 +  1.8760108f * y2 +  0.0415560f * z2;
  const float b =  0.0556434f * x2 + -0.2040259f * y2 +  1.0572252f * z2;

  return LinearColor(r, g, b);
}

Color Color::MakeFromColorTemperature(float tmp) {
  return LinearColor::MakeFromColorTemperature(tmp).ToColor(true);
}

Color Color::MakeRedToGreenColorFromScalar(float scalar) {
  float red_sclr = Math::Clamp01<float>((1.f - scalar)/0.5f);
  float green_sclr = Math::Clamp01<float>((scalar/0.5f));
  int32 r = Math::TruncToInt(255 * red_sclr);
  int32 g = Math::TruncToInt(255 * green_sclr);
  int32 b = 0;
  return Color(r, g, b);
}

void ComputeAndFixedColorAndIntensity(const LinearColor& linear_color, Color& out_color, float& out_intensity) {
  float max_component = Math::Max(DELTA, Math::Max(linear_color.r, Math::Max(linear_color.g, linear_color.b)));
  out_color = (linear_color / max_component).ToColor(true);
  out_intensity = max_component;
}
