...
static CAutoConsoleVariableRef GMallocBinned2PerThreadCachesCVar(
  ...
  TEXT("Enables per-thread caches of small (<= 32768 byte) allocations from MemoryAllocatorBinned2")
  );
...
static CAutoConsoleVariableRef GMallocBinned2BundleSizeCVar(
  TEXT("MemoryAllocatorBinned2.BundleSize"),
...
static CAutoConsoleVariableRef GMallocBinned2BundleCountCVar(
  TEXT("MemoryAllocatorBinned2.BundleCount"),
...
static CAutoConsoleVariableRef GMallocBinned2AllocExtraCVar(
  TEXT("MemoryAllocatorBinned2.AllocExtra"),
...
uint16 MemoryAllocatorBinned2::SmallBlockSizesReversed[BINNED2_SMALL_POOL_COUNT] = { 0 };
...
MemoryAllocatorBinned2* MemoryAllocatorBinned2::MemoryAllocatorBinned2 = nullptr;
// Mapping of sizes to small table indices
uint8 MemoryAllocatorBinned2::MemSizeToIndex[1 + (BINNED2_MAX_SMALL_POOL_SIZE >> BINNED2_MINIMUM_ALIGNMENT_SHIFT)] = { 0 };


MemoryAllocatorBinned2::CPoolList::CPoolList()
  : front(nullptr)
{
}

MemoryAllocatorBinned2::CPoolTable::CPoolTable()
  : block_size(0)
{
}
...
struct MemoryAllocatorBinned2::CPoolInfo
{
  ...
  CFreeBlock* first_free_block; // Pointer to first free memory in this pool or the OS Allocation size in bytes if this allocation is not binned
  CPoolInfo*  next;     // Pointer to next pool
  CPoolInfo** ptr_to_prev_next;  // Pointer to whichever pointer points to this pool
 ...
 public:
  ...
  {
    if (canary != should_be) {
      fun_log(LogMemory, Fatal, TEXT("MemoryAllocatorBinned2 Corruption canary was 0x%x, should be 0x%x"), int32(canary), int32(should_be));
    }
  }
  ...
  void SetCanary(ECanary should_be, bool pre_existing, bool bGuarnteedToBeNew)
  {
    if (pre_existing) {
      if (bGuarnteedToBeNew) {
        fun_log(LogMemory, Fatal, TEXT("MemoryAllocatorBinned2 Corruption canary was 0x%x, should be 0x%x. This block is both preexisting and guaranteed to be new; which makes no sense."), int32(canary), int32(should_be));
      }

      if (should_be == ECanary::Unassigned) {
        if (canary != ECanary::FirstFreeBlockIsOSAllocSize && canary != ECanary::FirstFreeBlockIsPtr) {
          fun_log(LogMemory, Fatal, TEXT("MemoryAllocatorBinned2 Corruption canary was 0x%x, will be 0x%x because this block should be preexisting and in use."), int32(canary), int32(should_be));
        }
      }
      else if (canary != should_be) {
        fun_log(LogMemory, Fatal, TEXT("MemoryAllocatorBinned2 Corruption canary was 0x%x, should be 0x%x because this block should be preexisting."), int32(canary), int32(should_be));
      }
    }
    else {
      if (bGuarnteedToBeNew) {
        if (canary != ECanary::Unassigned) {
          fun_log(LogMemory, Fatal, TEXT("MemoryAllocatorBinned2 Corruption canary was 0x%x, will be 0x%x. This block is guaranteed to be new yet is it already assigned."), int32(canary), int32(should_be));
        }
      }
      else if (canary != should_be && canary != ECanary::Unassigned) {
        fun_log(LogMemory, Fatal, TEXT("MemoryAllocatorBinned2 Corruption canary was 0x%x, will be 0x%x does not have an expected value."), int32(canary), int32(should_be));
      }
    }
    canary = should_be;
  }
  ...
  void SetOSAllocationSizes(uint32 InRequestedBytes, uintptr_t InAllocatedBytes)
  {
    CheckCanary(ECanary::FirstFreeBlockIsOSAllocSize);
    fun_check(InRequestedBytes != 0);                // Shouldn't be pooling zero byte allocations
    fun_check(InAllocatedBytes >= InRequestedBytes); // We must be allocating at least as much as we requested
 ...
 private:
  ...
  {
    if (first_free_block->GetNumFreeRegularBlocks() == 0) { // && (first_free_block->GetShortBlockSize() == 0 || !first_free_block->HasFreeShortBlock()))
      first_free_block = (CFreeBlock*)first_free_block->nextFreeBlock;
    }
    fun_check(!first_free_block || first_free_block->GetNumFreeRegularBlocks() != 0);
  }
};
...
struct MemoryAllocatorBinned2::Private
{
   ...
   */
  static CPoolInfo* GetOrCreatePoolInfo(MemoryAllocatorBinned2& allocator, void* ptr, MemoryAllocatorBinned2::CPoolInfo::ECanary Kind, bool pre_existing)
  {
    /**
    ...
    */
    ...
    uint32 pool_index;
    allocator.PtrToPoolMapping.GetHashBucketAndPoolIndices(ptr, bucket_index, bucket_index_collision, pool_index);
    ...
    if (!allocator.hash_bucket_free_list_) {
      allocator.hash_bucket_free_list_ = (PoolHashBucket*)PlatformMemory::BinnedAllocFromOS(MemoryAllocatorBinned2::PAGE_SIZE);

      for (uintptr_t i = 0, n = PAGE_SIZE / sizeof(PoolHashBucket); i < n; ++i) {
        allocator.hash_bucket_free_list_->Link(new (allocator.hash_bucket_free_list_ + i) PoolHashBucket());
      }
    }
  ...
  struct CGlobalRecycler
  {
    bool PushBundle(uint32 pool_index, CBundleNode* bundle) {
      uint32 cahced_bundle_count = MathBase::Min<uint32>(GMallocBinned2MaxBundlesBeforeRecycle, BINNED2_MAX_GMallocBinned2MaxBundlesBeforeRecycle);
      for (uint32 slot = 0; slot < cahced_bundle_count; slot++) {
        if (!Bundles[pool_index].free_bundles[slot]) {
          if (!Atomics::CompareExchangePointer((void**)&Bundles[pool_index].free_bundles[slot], bundle, nullptr)) {
            return true;
          }
        }
      }
    ...
    CBundleNode* PopBundle(uint32 pool_index) {
      ...
      for (uint32 slot = 0; slot < cahced_bundle_count; slot++) {
        ...
        if (result) {
          if (Atomics::CompareExchangePointer((void**)&Bundles[pool_index].free_bundles[slot], nullptr, result) == result) {
            return result;
          }
        }
      }
  ...
  private:
    struct CPaddedBundlePointer {
      CBundleNode* free_bundles[BINNED2_MAX_GMallocBinned2MaxBundlesBeforeRecycle];
#define BUNDLE_PADDING (PLATFORM_CACHE_LINE_SIZE - sizeof(CBundleNode*) * BINNED2_MAX_GMallocBinned2MaxBundlesBeforeRecycle)
#if (4 + (4 * FUN_64_BIT)) * BINNED2_MAX_GMallocBinned2MaxBundlesBeforeRecycle < PLATFORM_CACHE_LINE_SIZE
      uint8 Padding[BUNDLE_PADDING];
#endif
      CPaddedBundlePointer() {
        DefaultConstructItems<CBundleNode*>(free_bundles, BINNED2_MAX_GMallocBinned2MaxBundlesBeforeRecycle);
      }
    };
    static_assert(sizeof(CPaddedBundlePointer) == PLATFORM_CACHE_LINE_SIZE, "CPaddedBundlePointer should be the same size as a cache line");
    MS_ALIGN(PLATFORM_CACHE_LINE_SIZE) CPaddedBundlePointer Bundles[BINNED2_SMALL_POOL_COUNT] GCC_ALIGN(PLATFORM_CACHE_LINE_SIZE);
  };

  static CGlobalRecycler GGlobalRecycler;

  static void free_bundles(MemoryAllocatorBinned2& allocator, CBundleNode* bundles_to_recycle, uint32 block_size, uint32 pool_index)
  {
    ...
    while (bundle) {
      ...
      do {
        ...
        if (!node_pool) {
          fun_log(LogMemory, Fatal, TEXT("MemoryAllocatorBinned2 Attempt to free an unrecognized small block %p"), node);
        }
        ...
        if (--node_pool->taken == 0) {
          ...
          // Free the OS memory.
          node_pool->Unlink();
          allocator.CachedOSPageAllocator.Free(base_ptr_of_node, allocator.PAGE_SIZE);
        }
...
MemoryAllocatorBinned2::Private::CGlobalRecycler MemoryAllocatorBinned2::Private::GGlobalRecycler;
...
inline MemoryAllocatorBinned2::CPoolInfo& MemoryAllocatorBinned2::CPoolList::GetFrontPool()
{
  fun_check(!IsEmpty());
  return *front;
}

inline const MemoryAllocatorBinned2::CPoolInfo& MemoryAllocatorBinned2::CPoolList::GetFrontPool() const
{
  fun_check(!IsEmpty());
  return *front;
}
...
MemoryAllocatorBinned2::CPoolInfo& MemoryAllocatorBinned2::CPoolList::PushNewPoolToFront(MemoryAllocatorBinned2& allocator, uint32 block_size, uint32 pool_index)
{
  const uint32 LocalPageSize = allocator.PAGE_SIZE;

  // Allocate memory.
  CFreeBlock* free = new (allocator.CachedOSPageAllocator.Allocate(LocalPageSize)) CFreeBlock(LocalPageSize, block_size, pool_index);
  if (!free)
  {
  ...
  // Create pool
  CPoolInfo* result = Private::GetOrCreatePoolInfo(allocator, free, CPoolInfo::ECanary::FirstFreeBlockIsPtr, false);
  result->Link(front);
  result->taken = 0;
  ...
  : hash_bucket_free_list_(nullptr)
{
  ...
  fun_check(!bOnce); // this is now a singleton-like thing and you cannot make multiple copies
  bOnce = true;
  ...
  OsAllocationGranularity = Constants.OsAllocationGranularity ? Constants.OsAllocationGranularity : PAGE_SIZE;
  NumPoolsPerPage = PAGE_SIZE / sizeof(CPoolInfo);
  PtrToPoolMapping.Init(PAGE_SIZE, NumPoolsPerPage, Constants.AddressLimit);

  CHECKF(MathBase::IsPowerOfTwo(PAGE_SIZE), TEXT("OS page size must be a power of two"));
  CHECKF(MathBase::IsPowerOfTwo(Constants.AddressLimit), TEXT("OS address limit must be a power of two"));
  CHECKF(Constants.AddressLimit > PAGE_SIZE, TEXT("OS address limit must be greater than the page size")); // Check to catch 32 bit overflow in AddressLimit
  ...
  CHECKF(PAGE_SIZE % BINNED2_LARGE_ALLOC == 0, TEXT("OS page size must be a multiple of BINNED2_LARGE_ALLOC"));
  CHECKF(sizeof(MemoryAllocatorBinned2::CFreeBlock) <= SmallBlockSizes[0], TEXT("pool header must be able to fit into the smallest block"));
  static_assert(countof(SmallBlockSizes) == BINNED2_SMALL_POOL_COUNT, "Small block size array size must match BINNED2_SMALL_POOL_COUNT");
  static_assert(countof(SmallBlockSizes) <= 256, "Small block size array size must fit in a byte");
  static_assert(sizeof(CFreeBlock) <= BINNED2_MINIMUM_ALIGNMENT, "Free block struct must be small enough to fit into a block.");

  // Init pool tables.
  for (uint32 index = 0; index != BINNED2_SMALL_POOL_COUNT; ++index) {
    CHECKF(index == 0 || SmallBlockSizes[index - 1] < SmallBlockSizes[index], TEXT("Small block sizes must be strictly increasing"));
    CHECKF(SmallBlockSizes[index] <= PAGE_SIZE, TEXT("Small block size must be small enough to fit into a page"));
  ...
  hash_buckets_ = (PoolHashBucket*)PlatformMemory::BinnedAllocFromOS(Align(max_hash_buckets_ * sizeof(PoolHashBucket), OsAllocationGranularity));
...
bool MemoryAllocatorBinned2::IsInternallyThreadSafe() const
{
  return true;
}
...
void* MemoryAllocatorBinned2::MallocExternal(size_t size, uint32 alignment)
{
  ...
  if ((size <= BINNED2_MAX_SMALL_POOL_SIZE) & (alignment <= BINNED2_MINIMUM_ALIGNMENT)) { // one branch, not two
    ...
    if (!table.ActivePools.IsEmpty()) {
      ...
      pool = &table.ActivePools.PushNewPoolToFront(*this, table.BlockSize, pool_index);
    }
  ...
  // Create pool.
  CPoolInfo* pool = Private::GetOrCreatePoolInfo(*this, result, CPoolInfo::ECanary::FirstFreeBlockIsOSAllocSize, false);
  fun_check(size > 0 && size <= AlignedSize && AlignedSize >= OsAllocationGranularity);
  pool->SetOSAllocationSizes(size, AlignedSize);

  return result;
}

void* MemoryAllocatorBinned2::ReallocExternal(void* ptr, size_t new_size, uint32 alignment)
{
  if (new_size == 0) {
  ...
  if (!IsOSAllocation(ptr)) {
    ...
    if (
      ((new_size <= BlockSize) & (alignment <= BINNED2_MINIMUM_ALIGNMENT)) && // one branch, not two
  ...
  if (!pool)
  {
    fun_log(LogMemory, Fatal, TEXT("MemoryAllocatorBinned2 Attempt to realloc an unrecognized block %p"), ptr);
  }
  uintptr_t PoolOsBytes = pool->GetOsAllocatedBytes();
  uint32 PoolOSRequestedBytes = pool->GetOSRequestedBytes();
  CHECKF(PoolOSRequestedBytes <= PoolOsBytes, TEXT("MemoryAllocatorBinned2::ReallocExternal %d %d"), int32(PoolOSRequestedBytes), int32(PoolOsBytes));
  if (new_size > PoolOsBytes || // can't fit in the old block
    (new_size <= BINNED2_MAX_SMALL_POOL_SIZE && alignment <= BINNED2_MINIMUM_ALIGNMENT) || // can switch to the small block allocator
    Align(new_size, OsAllocationGranularity) < PoolOsBytes) { // we can get some pages back
    // Grow or shrink.
    void* result = MemoryAllocatorBinned2::MallocExternal(new_size, alignment);
    UnsafeMemory::Memcpy(result, ptr, MathBase::Min<size_t>(new_size, PoolOSRequestedBytes));
    MemoryAllocatorBinned2::FreeExternal(ptr);
    return result;
  }
...
void MemoryAllocatorBinned2::FreeExternal(void* ptr)
{
  if (!IsOSAllocation(ptr)) {
    ...
    CPoolInfo* pool = Private::FindPoolInfo(*this, ptr);
    if (!pool) {
      fun_log(LogMemory, Fatal, TEXT("MemoryAllocatorBinned2 Attempt to free an unrecognized block %p"), ptr);
    }
    uintptr_t PoolOsBytes = pool->GetOsAllocatedBytes();
    uint32 PoolOSRequestedBytes = pool->GetOSRequestedBytes();
    CHECKF(PoolOSRequestedBytes <= PoolOsBytes, TEXT("MemoryAllocatorBinned2::FreeExternal %d %d"), int32(PoolOSRequestedBytes), int32(PoolOsBytes));
...
bool MemoryAllocatorBinned2::GetAllocationSizeExternal(void* ptr, size_t& out_allocation_size)
{
  if (!IsOSAllocation(ptr)) {
    fun_check(ptr); // null is 64k aligned so we should not be here
  ...
  CPoolInfo* pool = Private::FindPoolInfo(*this, ptr);
  if (!pool) {
    fun_log(LogMemory, Fatal, TEXT("MemoryAllocatorBinned2 Attempt to GetAllocationSizeExternal an unrecognized block %p"), ptr);
  }
  uintptr_t PoolOsBytes = pool->GetOsAllocatedBytes();
  uint32 PoolOSRequestedBytes = pool->GetOSRequestedBytes();
  CHECKF(PoolOSRequestedBytes <= PoolOsBytes, TEXT("MemoryAllocatorBinned2::GetAllocationSizeExternal %d %d"), int32(PoolOSRequestedBytes), int32(PoolOsBytes));
...
{
  for (CPoolInfo** pool_ptr = &Front; *pool_ptr; pool_ptr = &(*pool_ptr)->next) {
    CPoolInfo* pool = *pool_ptr;
    fun_check(pool->ptr_to_prev_next == pool_ptr);
    fun_check(pool->first_free_block);
    for (CFreeBlock* Free = pool->first_free_block; Free; Free = (CFreeBlock*)Free->nextFreeBlock) {
...
void MemoryAllocatorBinned2::FlushCurrentThreadCache()
{
  ...
  if (Lists) {
    CScopedLock guard(Mutex);
    for (int32 pool_index = 0; pool_index != BINNED2_SMALL_POOL_COUNT; ++pool_index) {
      CBundleNode* Bundles = Lists->PopBundles(pool_index);
      if (Bundles) {
        Private::free_bundles(*this, Bundles, PoolIndexToBlockSize(pool_index), pool_index);
      }
    }
  }
}
...
void MemoryAllocatorBinned2::Trim()
{
  ...
  //  };
  //  CTaskGraphInterface::BroadcastSlow_OnlyUseForSpecialPurposes(false, Broadcast);
  //  //fun_log(LogTemp, Info, TEXT("Trim Broadcast = %6.2fms"), 1000.0f * float(SystemTime::Seconds() - StartTime));
  //}
  {
    //double StartTime = SystemTime::Seconds();
    CScopedLock guard(Mutex);
    CachedOSPageAllocator.FreeAll();
    //fun_log(LogTemp, Info, TEXT("Trim CachedOSPageAllocator = %6.2fms"), 1000.0f * float(SystemTime::Seconds() - StartTime));
  }
}
...
bool MemoryAllocatorBinned2::CFreeBlockList::ObtainPartial(uint32 pool_index)
{
  if (!PartialBundle.Head) {
    ...
    PartialBundle.Head = MemoryAllocatorBinned2::Private::GGlobalRecycler.PopBundle(pool_index);
    if (PartialBundle.Head) {
...
MemoryAllocatorBinned2::CBundleNode* MemoryAllocatorBinned2::CFreeBlockList::RecyleFull(uint32 pool_index)
{
  MemoryAllocatorBinned2::CBundleNode* result = nullptr;
  if (FullBundle.Head) {
    FullBundle.Head->Count = FullBundle.Count;
    if (!MemoryAllocatorBinned2::Private::GGlobalRecycler.PushBundle(pool_index, FullBundle.Head)) {
...
MemoryAllocatorBinned2::CBundleNode* MemoryAllocatorBinned2::CFreeBlockList::PopBundles(uint32 pool_index)
{
...
{
  fun_check(MemoryAllocatorBinned2::Binned2TlsSlot);
  CPerThreadFreeBlockLists* ThreadSingleton = (CPerThreadFreeBlockLists*)CPlatformTLS::GetTlsValue(MemoryAllocatorBinned2::Binned2TlsSlot);
  if (!ThreadSingleton) {
    ThreadSingleton = new (PlatformMemory::BinnedAllocFromOS(Align(sizeof(CPerThreadFreeBlockLists), MemoryAllocatorBinned2::OsAllocationGranularity))) CPerThreadFreeBlockLists();
    CPlatformTLS::SetTlsValue(MemoryAllocatorBinned2::Binned2TlsSlot, ThreadSingleton);
  }
}
...
{
  fun_log(LogMemory, Fatal, TEXT("MemoryAllocatorBinned2 Attempt to realloc an unrecognized block %p   canary == 0x%x != 0x%x"), (void*)this, (int32)canary, (int32)MemoryAllocatorBinned2::CFreeBlock::CANARY_VALUE);
}
