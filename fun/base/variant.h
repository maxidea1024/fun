...
class FUN_BASE_API Variant {
 public:
  ...
  Variant(const Array<uint8>& byte_array)
      : value_(byte_array), type_(VariantTypes::ByteArray) {}

  Variant(const Variant& rhs) : type_(rhs.type_), value_(rhs.value_) {}
  ...
  bool IsNull() const { return type_ == VariantTypes::Empty; }

  const Array<uint8>& ByteArray() const { return value_; }

  int32 Len() const { return value_.Count(); }
  ...
  typename EnableIf<!IsEnum<T>::Value, T>::Type Value() const {
    ...
    // ((VariantTraitsCall<T>::Type() == VariantTypes::UInt8) && (type_ ==
    // VariantTypes::Enum)));
    fun_check(type_ == VariantTraitsCall<T>::Type());
...
template <>
struct VariantTraits<Rotator> {
  static const int32 Type = VariantTypes::Rotator;
};

template <>
struct VariantTraits<String> {
