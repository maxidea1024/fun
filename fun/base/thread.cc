...
class CallableHolder : public Runnable {
 public:
  ...
  void Run() override { callable_(data_); }
...
Thread::Thread()
    : id_(UniqueId()), name_(MakeName()), tls_(nullptr), event_() {}

Thread::Thread(const String& name)
    : id_(UniqueId()), name_(name), tls_(nullptr), event_() {}

Thread::~Thread() { delete tls_; }

void Thread::SetPriority(Priority prior) { SetPriorityImpl(prior); }
...
bool Thread::TryJoin(int32 milliseconds) { return JoinImpl(milliseconds); }
...
String Thread::MakeName() { return String("#") + id_; }
