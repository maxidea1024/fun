...
// when modifying the global allocator stats, if we are using COARSE locks, then
// all callsites for stat modification are covered by the allocator-wide access
// guard. Thus the stats can be modified directly. If we are using FINE locks,
// then we must modify the stats through atomics as the locks are either not
// actually covering the stat callsites, or are locking specific table locks
// which is not sufficient for stats.
#if STATS
...
#define BINNED_PEAK_STATCOUNTER(PeakCounter, CompareVal) \
  ((PeakCounter) = MathBase::Max((PeakCounter), (CompareVal)))
#else
...
#define BINNED_PEAK_STATCOUNTER(PeakCounter, CompareVal)                      \
  {                                                                           \
    ...
    } while (Atomics::CompareExchange(&(PeakCounter), NewPeak, NewCompare) != \
             NewCompare);                                                     \
  }
#endif
#else
...
DECLARE_MEMORY_STAT_EXTERN(TEXT("Binned Os Current"), STAT_Binned_OsCurrent,
                           STATGROUP_MemoryAllocator, FUN_BASE_API);
DECLARE_MEMORY_STAT_EXTERN(TEXT("Binned Os Peak"), STAT_Binned_OsPeak,
                           STATGROUP_MemoryAllocator, FUN_BASE_API);
DECLARE_MEMORY_STAT_EXTERN(TEXT("Binned Waste Current"),
                           STAT_Binned_WasteCurrent, STATGROUP_MemoryAllocator,
                           FUN_BASE_API);
DECLARE_MEMORY_STAT_EXTERN(TEXT("Binned Waste Peak"), STAT_Binned_WastePeak,
                           STATGROUP_MemoryAllocator, FUN_BASE_API);
DECLARE_MEMORY_STAT_EXTERN(TEXT("Binned Used Current"), STAT_Binned_UsedCurrent,
                           STATGROUP_MemoryAllocator, FUN_BASE_API);
DECLARE_MEMORY_STAT_EXTERN(TEXT("Binned Used Peak"), STAT_Binned_UsedPeak,
                           STATGROUP_MemoryAllocator, FUN_BASE_API);
...
DECLARE_MEMORY_STAT_EXTERN(TEXT("Binned Slack Current"),
                           STAT_Binned_SlackCurrent, STATGROUP_MemoryAllocator,
                           FUN_BASE_API);
...
class MemoryAllocatorBinned : public MemoryAllocator {
 ...
 private:
  ...
  struct CFreePageBlock {
    ...
    CFreePageBlock() : ptr(nullptr), byte_size(0) {}
  };
#endif
  ...
  /**
   * We can't call the constructor to TLockFreePointerList in the BinnedMalloc
   * constructor as it attempts to allocate memory. We push this back and
   * initialize it later but we set aside the memory before hand
   */
  ...
  CPoolTable* mem_size_to_pool_table_[MAX_POOLED_ALLOCATION_SIZE +
 ...
 public:
  ...
  // page_size - First parameter is page size, all allocs from
  // BinnedAllocFromOS() MUST be aligned to this size AddressLimit - Second
  // parameter is estimate of the range of addresses expected to be returns by
  // BinnedAllocFromOS(). Binned Malloc will adjust its internal structures to
  // make lookups for memory allocations O(1) for this range. It is ok to go
  // outside this range, lookups will just be a little slower
  ...
  /** Called once per frame, gathers and sets all memory allocator statistics
   * into the corresponding stats. MUST BE THREAD SAFE. */
