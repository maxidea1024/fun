 ...
 *
 *  Multi-cast delegates are also supported, using the 'FUN_DECLARE_MULTICAST_DELEGATE...' macros.
 ...
 *
 *  This creates a delegate type called 'StringDelegate' that takes a single parameter of type 'String'.
 *
 ...
 *  and the actual function address of the method.  So, here we'll create an instance of our 'LogWriter'
 ...
 *  such as BindRaw() and BindFObject().  You can bind to global function pointers with BindStatic().
 *
 *  Now, your 'WriteToLog' method can be called by MyClass without it even knowing anything about
 *  the 'LogWriter' class!  To call a your delegate, just use the 'Execute()' method:
 *
 ...
 *
 *    write_to_log_delegate.ExecuteIfBound("Only executes if a function was bound!");
 *
 ...
 *  using these objects.  If you bind a delegate to a member of a FObject or shared pointer class, the
 ...
 *  ------------------------------------------------------------------------------------------------------------------------------------------------------------
 *  void Function()                               | FUN_DECLARE_DELEGATE(DelegateName)
 *  void Function(<Param1>)                       | FUN_DECLARE_DELEGATE_OneParam(DelegateName, Param1Type)
 *  void Function(<Param1>, <Param2>)             | FUN_DECLARE_DELEGATE_TwoParams(DelegateName, Param1Type, Param2Type)
 *  void Function(<Param1>, <Param2>, ...)        | FUN_DECLARE_DELEGATE_<Num>Params(DelegateName, Param1Type, Param2Type, ...)
 *  <RetVal> Function()                           | FUN_DECLARE_DELEGATE_RetVal(RetType, DelegateName)
 *  <RetVal> Function(<Param1>)                   | FUN_DECLARE_DELEGATE_RetVal_OneParam(RetType, DelegateName, Param1Type)
 *  <RetVal> Function(<Param1>, <Param2>)         | FUN_DECLARE_DELEGATE_RetVal_TwoParams(RetType, DelegateName, Param1Type, Param2Type)
 *  <RetVal> Function(<Param1>, <Param2>, ...)    | FUN_DECLARE_DELEGATE_RetVal_<Num>Params(RetType, DelegateName, Param1Type, Param2Type, ...)
 *  ------------------------------------------------------------------------------------------------------------------------------------------------------------
 *
 ...
 *                        Multi-cast delegates:  FUN_DECLARE_MULTICAST_DELEGATE...()
 *   Dynamic (FObject, serializable) delegates:  FUN_DECLARE_DYNAMIC_DELEGATE...()
 */
...
//#define FUN_FUN_FUNC_FUN_DECLARE_DYNAMIC_DELEGATE(WeakPtr, DynamicDelegateName, ExecFunction, FuncParamList, FuncParamPassThru, ...) \
...
//#define FUN_FUNC_FUN_DECLARE_DYNAMIC_MULTICAST_DELEGATE(WeakPtr, DynamicMulticastDelegateName, ExecFunction, FuncParamList, FuncParamPassThru, ...) \
//  class DynamicMulticastDelegateName : public fun::TBaseDynamicMulticastDelegate<WeakPtr, __VA_ARGS__> { \
//   public: \
//    /** Default constructor */ \
//    DynamicMulticastDelegateName() \
//    {} \
//    \
//    /** Construction from an FMulticastScriptDelegate must be explicit.  This is really only used by FObject system internals. */ \
//    explicit DynamicMulticastDelegateName(const fun::TMulticastScriptDelegate<>& multicast_script_delegate) \
//      : fun::TBaseDynamicMulticastDelegate<WeakPtr, __VA_ARGS__>(multicast_script_delegate) \
//    {} \
//    \
...
namespace fun {
  namespace NStrAfterLastDoubleColon_internal {
    ...
    /**
     * Metafunction which evaluates to a IntegerSequence of chars containing only the function name.
     *
     * Example:
     * StrAfterLastDoubleColon<"&SomeClass::SomeNestedClass::SomeFunc"_intseq>::Type == IntegerSequence<char, 'S', 'o', 'm', 'e', 'F', 'u', 'n', 'c'>
     */
    template <typename T>
    ...
    struct StrAfterLastDoubleColon<IntegerSequence<T, Chars...>> {
      typedef typename NStrAfterLastDoubleColon_internal::TypedImpl<T>::template Inner<true, Chars...>::Type Type;
    };
  }

  /**
   * Custom literal operator which converts a string into a IntegerSequence of chars.
   */
  template <typename T, T... Chars>
  ...
  #define FUN_STATIC_FUNCTION_NAME(str)  fun::NStrAfterLastDoubleColon_internal::StaticStringFromCharSequence<typename fun::NStrAfterLastDoubleColon_internal::StrAfterLastDoubleColon<decltype(PREPROCESSOR_JOIN(str, _intseq))>::Type>::Get()

#else

  #define FUN_STATIC_FUNCTION_NAME(str)  fun::Delegate_internal::GetTrimmedMemberFunctionName(str)

#endif

//// Helper macro for calling BindDynamic() on dynamic delegates.  Automatically generates the function name string.
//#define BindDynamic(UserObject, FuncName)  __Internal_BindDynamic(UserObject, FuncName, FUN_STATIC_FUNCTION_NAME(#FuncName))
//
//// Helper macro for calling AddDynamic() on dynamic multi-cast delegates.  Automatically generates the function name string.
//#define AddDynamic(UserObject, FuncName)  __Internal_AddDynamic(UserObject, FuncName, FUN_STATIC_FUNCTION_NAME(#FuncName))
//
...
//#define AddUniqueDynamic(UserObject, FuncName)  __Internal_AddUniqueDynamic(UserObject, FuncName, FUN_STATIC_FUNCTION_NAME(#FuncName))
//
//// Helper macro for calling RemoveDynamic() on dynamic multi-cast delegates.  Automatically generates the function name string.
//#define RemoveDynamic(UserObject, FuncName)  __Internal_RemoveDynamic(UserObject, FuncName, FUN_STATIC_FUNCTION_NAME(#FuncName))
//
//// Helper macro for calling IsAlreadyBound() on dynamic multi-cast delegates.  Automatically generates the function name string.
//#define IsAlreadyBound(UserObject, FuncName)  __Internal_IsAlreadyBound(UserObject, FuncName, FUN_STATIC_FUNCTION_NAME(#FuncName))
...
namespace Delegate_internal {
  ...
  inline String GetTrimmedMemberFunctionName(const char* function_name) {
    ...
    fun_check_msg(result && result[2] != '0', "'{0}' does not look like a member function", function_name);
