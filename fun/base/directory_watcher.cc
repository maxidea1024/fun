...
class DirectoryWatcherStrategy {
 public:
  ...
  DirectoryWatcher& GetOwner() { return owner_; }
 ...
 protected:
  ...
  void Compare(ItemInfoMap& old_entries, ItemInfoMap& new_entries) {
    for (ItemInfoMap::iterator itn = new_entries.begin();
         itn != new_entries.end(); ++itn) {
      ...
      if (ito != old_entries.end()) {
        if ((GetOwner().GetEventMask() & DirectoryWatcher::DW_ITEM_MODIFIED) &&
            !GetOwner().GetEventsSuspended()) {
          if (itn->second.size != ito->second.size ||
              itn->second.last_modified != ito->second.last_modified) {
            fun::File f(itn->second.path);
            DirectoryWatcher::DirectoryEvent ev(
                f, DirectoryWatcher::DW_ITEM_MODIFIED);
            GetOwner().item_modified(&GetOwner(), ev);
          }
        }
        old_entries.erase(ito);
      } else if ((GetOwner().GetEventMask() &
    ...
    if ((GetOwner().GetEventMask() & DirectoryWatcher::DW_ITEM_REMOVED) &&
        !GetOwner().GetEventsSuspended()) {
      for (ItemInfoMap::iterator it = old_entries.begin();
           it != old_entries.end(); ++it) {
        fun::File f(it->second.path);
        DirectoryWatcher::DirectoryEvent ev(f,
...
class WindowsDirectoryWatcherStrategy : public DirectoryWatcherStrategy {
 public:
  ...
  ~WindowsDirectoryWatcherStrategy() { CloseHandle(hstopped_); }
  ...
  void Stop() { SetEvent(hstopped_); }

  bool SupportsMoveEvents() const { return false; }
...
class LinuxDirectoryWatcherStrategy : public DirectoryWatcherStrategy {
 public:
  LinuxDirectoryWatcherStrategy(DirectoryWatcher& owner)
      : DirectoryWatcherStrategy(owner), fd_(-1), stopped_(false) {
    fd_ = inotify_init();
    if (fd_ == -1) {
  ...
  ~LinuxDirectoryWatcherStrategy() { close(fd_); }
  ...
  void Run() {
    ...
    int wd = inotify_add_watch(fd_, GetOwner().GetDirectory().GetPath().c_str(),
                               mask);
    if (wd == -1) {
      try {
        ...
        if (n > 0) {
          while (n > 0) {
            struct inotify_event* event =
                reinterpret_cast<struct inotify_event*>(buffer.begin() + i);
            ...
            if (event->len > 0) {
              if (!GetOwner().GetEventsSuspended()) {
                ...
                if ((event->mask & IN_CREATE) &&
                    (GetOwner().GetEventMask() &
                     DirectoryWatcher::DW_ITEM_ADDED)) {
                  DirectoryWatcher::DirectoryEvent ev(
                      f, DirectoryWatcher::DW_ITEM_ADDED);
                  GetOwner().item_added(&GetOwner(), ev);
                }
                ...
                if ((event->mask & IN_DELETE) &&
                    (GetOwner().GetEventMask() &
                     DirectoryWatcher::DW_ITEM_REMOVED)) {
                  DirectoryWatcher::DirectoryEvent ev(
                      f, DirectoryWatcher::DW_ITEM_REMOVED);
                  GetOwner().item_removed(&GetOwner(), ev);
                }
                ...
                if ((event->mask & IN_MOVED_FROM) &&
                    (GetOwner().GetEventMask() &
                     DirectoryWatcher::DW_ITEM_MOVED_FROM)) {
                  DirectoryWatcher::DirectoryEvent ev(
                      f, DirectoryWatcher::DW_ITEM_MOVED_FROM);
                ...
                if ((event->mask & IN_MOVED_TO) &&
                  ...
                  DirectoryWatcher::DirectoryEvent ev(
                      f, DirectoryWatcher::DW_ITEM_MOVED_TO);
  ...
  bool SupportsMoveEvents() const { return true; }
 ...
 public:
  ...
  void Run() {
    ...
    while (!stopped_) {
      ...
      EV_SET(&event, dir_fd_, EVFILT_VNODE, EV_ADD | EV_CLEAR, eventFilter, 0,
             0);
      ...
      if (event_count < 0 || event_data.flags == EV_ERROR) {
        try {
          ...
          GetOwner().ScanError(&GetOwner(), e);
                 ...
                 ((GetOwner().GetEventMask() &
                   DirectoryWatcher::DW_ITEM_MODIFIED) &&
                  last_scan.isElapsed(GetOwner().GetScanInterval() *
                                      1000000))) {
        ItemInfoMap new_entries;
        Scan(new_entries);
        Compare(entries, new_entries);
...
DirectoryWatcher::DirectoryWatcher(const String& path, int event_mask,
                                   int scan_interval, bool force_scan)
    : directory_(path),
...
DirectoryWatcher::DirectoryWatcher(const fun::File& directory, int event_mask,
                                   int scan_interval, bool force_scan)
...
void DirectoryWatcher::ResumeEvents() { events_suspended_++; }
...
void DirectoryWatcher::Init() {
  ...
  if (!directory_.IsDirectory()) {
    throw fun::InvalidArgumentException("not a directory",
                                        directory_.GetPath());
  }
...
void DirectoryWatcher::Run() { strategy_->Run(); }
