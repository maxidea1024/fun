...
TimeZone::TimeZone(const Timespan& offset) {
  ...
  if (offset_in_second >= -50400 && offset_in_second <= 50400) {
    impl_ =
        SharedPtr<TimeZoneImpl>(new UtcTimeZoneImpl((int32)offset_in_second));
  }
}

TimeZone::TimeZone(const String& iana_id, const Timespan& offset,
                   const String& name, const String& abbreviation,
                   Locale::Country country, const String& comment) {
  if (!IsTimeZoneAvailable(iana_id)) {
    impl_ = SharedPtr<TimeZoneImpl>(
        new UtcTimeZoneImpl(iana_id, (int32)offset.TotalSeconds(), name,
                            abbreviation, country, comment));
  }
}
...
bool TimeZone::IsValid() const { return impl_.IsValid() && impl_->IsValid(); }
...
String TimeZone::GetDisplayName(const DateTime& at_date_time,
  ...
  if (IsValid()) {
    return impl_->GetDisplayName(at_date_time.ToUtcTicksSinceEpoch(), name_type,
                                 locale);
  } else {
    return String();
  }
...
Timespan TimeZone::GetOffsetFromUtc(const DateTime& at_date_time) const {
  return Timespan::FromSeconds(
      IsValid() ? impl_->GetOffsetFromUtc(at_date_time.ToUtcTicksSinceEpoch())
                : 0);
}
...
Timespan TimeZone::GetStandardTimeOffset(const DateTime& at_date_time) const {
  return Timespan::FromSeconds(
      IsValid()
          ? impl_->GetStandardTimeOffset(at_date_time.ToUtcTicksSinceEpoch())
          : 0);
}
...
Timespan TimeZone::GetDaylightTimeOffset(const DateTime& at_date_time) const {
  return Timespan::FromSeconds(
      IsValid()
          ? impl_->GetDaylightTimeOffset(at_date_time.ToUtcTicksSinceEpoch())
          : 0);
}
...
bool TimeZone::IsDaylightTime(const DateTime& at_date_time) const {
  return IsValid() ? impl_->IsDaylightTime(at_date_time.ToUtcTicksSinceEpoch())
                   : false;
}

TimeZone::OffsetData TimeZone::GetOffsetData(
    const DateTime& for_date_time) const {
  if (HasTransitions()) {
    return TimeZoneImpl::ToOffsetData(
        impl_->GetData(for_date_time.ToUtcTicksSinceEpoch()));
  } else {
...
TimeZone::OffsetData TimeZone::NextTransition(
    const DateTime& after_date_time) const {
  if (HasTransitions()) {
    return TimeZoneImpl::ToOffsetData(
        impl_->NextTransition(after_date_time.ToUtcTicksSinceEpoch()));
  } else {
    return TimeZoneImpl::InvalidOffsetData();
  }
}

TimeZone::OffsetData TimeZone::PreviousTransition(
    const DateTime& before_date_time) const {
  if (HasTransitions()) {
    return TimeZoneImpl::ToOffsetData(
        impl_->PreviousTransition(before_date_time.ToUtcTicksSinceEpoch()));
  } else {
    return TimeZoneImpl::InvalidOffsetData();
  }
}

TimeZone::OffsetDataList TimeZone::GetTransitions(
    const DateTime& from_date_time, const DateTime& to_date_time) const {
  OffsetDataList result;
  if (HasTransitions()) {
    const auto list =
        impl_->GetTransitions(from_date_time.ToUtcTicksSinceEpoch(),
                              to_date_time.ToUtcTicksSinceEpoch());
...
static Array<String> SetUnion(const Array<String>& list1,
                              const Array<String>& list2) {
  Array<String> result;
...
Array<String> TimeZone::AvailableTimeZoneIds() {
  return SetUnion(UtcTimeZoneImpl().AvailableTimeZoneIds(),
                  g_global_timezone.GetBackend()->AvailableTimeZoneIds());
}

Array<String> TimeZone::AvailableTimeZoneIds(Locale::Country country) {
  return SetUnion(
      UtcTimeZoneImpl().AvailableTimeZoneIds(country),
      g_global_timezone.GetBackend()->AvailableTimeZoneIds(country));
}

Array<String> TimeZone::AvailableTimeZoneIds(int32 offset_seconds) {
  return SetUnion(
      UtcTimeZoneImpl().AvailableTimeZoneIds(offset_seconds),
      g_global_timezone.GetBackend()->AvailableTimeZoneIds(offset_seconds));
}
...
String TimeZone::LocalId() { return GetSystemTimeZoneId(); }

TimeZone TimeZone::Local() { return GetSystemTimeZone(); }

uint32 HashOf(const TimeZone& timezone) { return HashOf(timezone.GetId()); }
