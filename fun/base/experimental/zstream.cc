...
ZStream::ZStream(std::istream& istr, StreamType type, int level)
    : istr_(&istr), ostr_(nullptr), eof_(false) {
  zstr_.next_in = 0;
  zstr_.avail_in = 0;
  zstr_.total_in = 0;
  zstr_.next_out = 0;
  ...
  int rc =
      deflateInit2(&zstr_, level, Z_DEFLATED,
                   15 + (type == STREAM_GZIP ? 16 : 0), 8, Z_DEFAULT_STRATEGY);
  if (rc != Z_OK) {
    delete[] buffer_;
...
ZStream::ZStream(std::istream& istr, int windows_bits, int level)
  ...
  int rc = deflateInit2(&zstr_, level, Z_DEFLATED, windows_bits, 8,
                        Z_DEFAULT_STRATEGY);
  if (rc != Z_OK) {
    delete[] buffer_;
    throw IoException(zError(rc));
...
ZStream::ZStream(std::ostream& ostr, StreamType type, int level)
    : istr_(nullptr), ostr_(&ostr), eof_(false) {
  zstr_.zalloc = Z_NULL;
  zstr_.zfree = Z_NULL;
  zstr_.opaque = Z_NULL;
  zstr_.next_in = 0;
