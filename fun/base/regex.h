...
class FUN_BASE_API Regex {
 public:
   ...
   *   * Options marked [match] can be passed to match, Extract, Split and
   * subst.
  ...
  enum Options {  // These must match the corresponding options in pcre.h!
    ...
    RE_MULTILINE =
        0x00000002,  /// enable multi-line mode; affects ^ and $ (/m) [ctor]
    RE_DOTALL = 0x00000004,    /// dot matches all characters, including newline
                               /// (/s) [ctor]
    RE_EXTENDED = 0x00000008,  /// totally ignore whitespace (/x) [ctor]
    RE_ANCHORED =
        0x00000010,  /// treat pattern as if it starts with a ^ [ctor, match]
    RE_DOLLAR_ENDONLY = 0x00000020,  /// dollar matches end-of-string only, not
                                     /// last newline in string [ctor]
    RE_EXTRA = 0x00000040,  /// enable optional PCRE functionality [ctor]
    RE_NOTBOL =
        0x00000080,  /// circumflex does not match beginning of string [match]
    ...
    RE_UTF8 =
        0x00000800,  /// assume pattern and subject is UTF-8 encoded [ctor]
    RE_NO_AUTO_CAPTURE =
        0x00001000,  /// disable numbered capturing parentheses [ctor, match]
    RE_NO_UTF8_CHECK =
        0x00002000,  /// do not check validity of UTF-8 code sequences [match]
    RE_FIRSTLINE = 0x00040000,  /// an  unanchored  pattern  is  required  to
                                /// match before  or  at  the  first  newline in
                                /// the subject string, though the matched text
                                /// may continue over the newline [ctor]
    RE_DUPNAMES = 0x00080000,  /// names used to identify capturing  subpatterns
                               /// need not be unique [ctor]
    RE_NEWLINE_CR =
        0x00100000,  /// assume newline is CR ('\r'), the default [ctor]
    RE_NEWLINE_LF = 0x00200000,    /// assume newline is LF ('\n') [ctor]
    RE_NEWLINE_CRLF = 0x00300000,  /// assume newline is CRLF ("\r\n") [ctor]
    RE_NEWLINE_ANY = 0x00400000,   /// assume newline is any valid Unicode
                                   /// newline character [ctor]
    RE_NEWLINE_ANYCRLF =
        0x00500000,          /// assume newline is any of CR, LF, CRLF [ctor]
    RE_GLOBAL = 0x10000000,  /// replace all occurrences (/g) [subst]
    RE_NO_VARS = 0x20000000  /// treat dollar in replacement string as ordinary
                             /// character [subst]
  };

  struct MatchInfo {
    int32
        offset;  /// zero based offset (INVALID_INDEX if subexpr does not match)
 ...
 public:
  ...
  If no part of the subject matches the pattern, match.offset is INVALID_INDEX
  and match.length is 0. Throws a RegularExpressionException in case of an
  error. Returns the number of matches.
  */
  ...
  error. Returns the number of matches.
  */
  int Match(const String& subject, int32 offset, MatchInfo& match,
            int options = 0) const;
  ...
  The following entries identify matching subpatterns. See the PCRE
  documentation for a more detailed explanation. If no part of the subject
  matches the pattern, matches is empty. Throws a RegularExpressionException in
  case of an error. Returns the number of matches.
  */
  int Match(const String& subject, int32 offset, MatchList& matches,
            int options = 0) const;
  ...
  */
  ...
  Returns the number of matches.
  */
  int Extract(const String& subject, int32 offset, String& str,
              int options = 0) const;

  /**
  Matches the given subject string against the pattern.
  The first entry in captured is the captured substring.
  The following entries contain substrings matching subpatterns. See the PCRE
  documentation for a more detailed explanation. If no part of the subject
  matches the pattern, captured is empty. Throws a RegularExpressionException in
  case of an error. Returns the number of matches.
  */
  int Split(const String& subject, Array<String>& strings,
            int options = 0) const;
  ...
  case of an error. Returns the number of matches.
  */
  int Split(const String& subject, int32 offset, Array<String>& strings,
            int options = 0) const;
  ...
  */
  int Substitute(String& subject, const String& replacement,
                 int options = 0) const;
  ...
  If RE_GLOBAL is specified as option, all matches are replaced. Otherwise,
  only the first match is replaced.
  Unless RE_NO_VARS is specified, occurrences of $<n> (for example, $0, $1, $2,
  ... $9) in replacement are replaced with the corresponding captured string. $0
  is the captured substring. $1 ... $n are the substrings matching the
  subpatterns. Returns the number of replaced occurrences.
  */
  int Substitute(String& subject, int32 offset, const String& replacement,
                 int options = 0) const;

  /**
  Matches the given subject string against the regular expression given in
  pattern, using the given options.
  */
  static bool Match(const String& subject, const String& pattern,
                    int options = 0);

 protected:
  int32 SubstituteOne(String& subject, int32 offset, const String& replacement,
                      int options) const;

 private:
  // Note: to avoid a dependency on the pcre.h header the following are
  // declared as void* and casted to the correct type in the implementation
  // file.
...
inline int Regex::Match(const String& subject, MatchInfo& match,
                        int options) const {
  return Match(subject, 0, match, options);
}

inline int Regex::Split(const String& subject, Array<String>& strings,
                        int options) const {
  return Split(subject, 0, strings, options);
}

inline int Regex::Substitute(String& subject, const String& replacement,
                             int options) const {
...
class FUN_BASE_API URegex {
  ...
  */
  enum Options  // These must match the corresponding options in pcre.h! {
      ...
      RE_ANCHORED =
          0x00000010,  /// treat pattern as if it starts with a ^ [ctor, match]
      RE_DOLLAR_ENDONLY = 0x00000020,  /// dollar matches end-of-string only,
                                       /// not last newline in string [ctor]
      RE_EXTRA = 0x00000040,  /// enable optional PCRE functionality [ctor]
      RE_NOTBOL =
          0x00000080,  /// circumflex does not match beginning of string [match]
      RE_NOTEOL = 0x00000100,    /// $ does not match end of string [match]
      ...
      RE_NO_UTF8_CHECK =
          0x00002000,  /// do not check validity of UTF-8 code sequences [match]
      RE_FIRSTLINE = 0x00040000,  /// an  unanchored  pattern  is  required  to
                                  /// match before  or  at  the  first  newline
                                  /// in  the subject string, though the matched
                                  /// text may continue over the newline [ctor]
      RE_DUPNAMES = 0x00080000,  /// names used to identify capturing
      ...
      RE_NEWLINE_ANYCRLF =
          0x00500000,          /// assume newline is any of CR, LF, CRLF [ctor]
      RE_GLOBAL = 0x10000000,  /// replace all occurrences (/g) [subst]
      RE_NO_VARS = 0x20000000  /// treat dollar in replacement string as
...
Returns the number of matches.
*/
int Match(const UString& subject, int32 offset, MatchInfo& match,
          int options = 0) const;
...
Returns the number of matches.
*/
int Match(const UString& subject, int32 offset, MatchList& matches,
          int options = 0) const;
...
Returns the number of matches.
*/
int Extract(const UString& subject, int32 offset, UString& str,
            int options = 0) const;
...
The first entry in captured is the captured substring.
The following entries contain substrings matching subpatterns. See the PCRE
documentation for a more detailed explanation. If no part of the subject matches
the pattern, captured is empty. Throws a RegularExpressionException in case of
an error. Returns the number of matches.
*/
int Split(const UString& subject, Array<UString>& strings,
          int options = 0) const;
...
an error. Returns the number of matches.
*/
int Split(const UString& subject, int32 offset, Array<UString>& strings,
          int options = 0) const;
...
Returns the number of replaced occurrences.
*/
int Substitute(UString& subject, const UString& replacement,
               int options = 0) const;
...
subpatterns. Returns the number of replaced occurrences.
*/
int Substitute(UString& subject, int32 offset, const UString& replacement,
               int options = 0) const;
...
pattern, using the given options.
*/
static bool Match(const UString& subject, const UString& pattern,
                  int options = 0);

protected:
int32 SubstituteOne(UString& subject, int32 offset, const UString& replacement,
                    int options) const;
...
inline int URegex::Match(const UString& subject, MatchInfo& match,
                         int options) const {
  return Match(subject, 0, match, options);
}

inline int URegex::Split(const UString& subject, Array<UString>& strings,
                         int options) const {
  return Split(subject, 0, strings, options);
}

inline int URegex::Substitute(UString& subject, const UString& replacement,
                              int options) const {
  return Substitute(subject, 0, replacement, options);
}
