...
class FUN_BASE_API StackwalkWin : public Stackwalk {
 public:
  ...
  static void ProgramCounterToSymbolInfo(uint64 ProgramCounter, CProgramCounterSymbolInfo& OutSymbolInfo);
  static void CaptureStackBackTrace(uint64* backtrace, uint32 max_depth, void* Context = nullptr);
  ...
  static void ThreadStackwalkAndDump(ANSICHAR* HumanReadableString, size_t HumanReadableStringSize, int32 IgnoreCount, uint32 thread_id);

  static int32 GetProcessModuleCount();
  static int32 GetProcessModuleSignatures(CStackWalkModuleInfo* ModuleSignatures, const int32 ModuleSignaturesSize);
