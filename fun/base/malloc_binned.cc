...
struct MemoryAllocatorBinned::CPoolInfo
{
  ...
  /**
  index of pool. index into mem_size_to_pool_table_[]. Valid when < MAX_POOLED_ALLOCATION_SIZE, MAX_POOLED_ALLOCATION_SIZE is os_table_.
  When alloc_size is 0, this is the number of pages to step back to find the base address of an allocation. See FindPoolInfoInternal()
  */
  ...
  void SetAllocationSizes(uint32 InBytes, uintptr_t InOsBytes, uint32 InTableIndex, uint32 SmallAllocLimt)
  {
...
struct MemoryAllocatorBinned::CPoolHashBucket
{
  ...
  void Link(CPoolHashBucket* after)
  {
    Link(after, prev, this);
  }

  static void Link(CPoolHashBucket* node, CPoolHashBucket* before, CPoolHashBucket* after)
  {
    node->prev = before;
    node->next = after;
    before->next = node;
...
struct MemoryAllocatorBinned::Private
{
  ...
  static inline void TrackStats(CPoolTable* table, size_t size)
  {
    ...
    table->max_active_requests = MathBase::Max(table->max_active_requests, table->active_request_count);
  ...
  */
  ...
  {
    uint64 IndirectPoolBlockSizeBytes = allocator.indect_pool_block_size_ * sizeof(CPoolInfo);

    CHECK_SLOW(IndirectPoolBlockSizeBytes <= allocator.PAGE_SIZE);
    CPoolInfo* indirect = (CPoolInfo*)PlatformMemory::BinnedAllocFromOS(IndirectPoolBlockSizeBytes);
    if (indirect == nullptr) {
      OutOfMemory(IndirectPoolBlockSizeBytes);
    }
    UnsafeMemory::Memset(indirect, 0, IndirectPoolBlockSizeBytes);

    BINNED_PEAK_STATCOUNTER(allocator.os_peak_, BINNED_ADD_STATCOUNTER(allocator.os_current_, (int64)(Align(IndirectPoolBlockSizeBytes, allocator.PAGE_SIZE))));
    BINNED_PEAK_STATCOUNTER(allocator.waste_peak_, BINNED_ADD_STATCOUNTER(allocator.waste_current_, (int64)(Align(IndirectPoolBlockSizeBytes, allocator.PAGE_SIZE))));

    return indirect;
  }

  /**
  Gets the CPoolInfo for a memory address. If no valid info exists one is created.
  ...
  */
  static inline CPoolInfo* GetPoolInfo(MemoryAllocatorBinned& allocator, uintptr_t ptr)
  {
    if (allocator.hash_buckets_ == nullptr) {
      // Init tables.
      allocator.hash_buckets_ = (CPoolHashBucket*)PlatformMemory::BinnedAllocFromOS(Align(allocator.max_hash_buckets_ * sizeof(CPoolHashBucket), allocator.PAGE_SIZE));
    ...
    uintptr_t pool_index = ((uintptr_t)ptr >> allocator.pool_bit_shift_) & allocator.pool_mask_;
  ...
  static inline CPoolInfo* FindPoolInfo(MemoryAllocatorBinned& allocator, uintptr_t ptr1, uintptr_t& allocation_base)
  {
    uint16 next_step = 0;
    uintptr_t ptr = ptr1 &~ ((uintptr_t)allocator.PAGE_SIZE - 1);
    for (uint32 i = 0, N = (BINNED_ALLOC_POOL_SIZE / allocator.PAGE_SIZE) + 1; i < N; ++i) {
      CPoolInfo* pool = FindPoolInfoInternal(allocator, ptr, next_step);
      if (pool) {
        allocation_base = ptr;
        //CHECK_SLOW(ptr1 >= allocation_base && ptr1 < allocation_base + pool->GetBytes());
        return pool;
      }
      ptr = ((ptr - (allocator.PAGE_SIZE * next_step)) - 1) &~ ((uintptr_t)allocator.PAGE_SIZE - 1);
    }
    allocation_base = 0;
    return nullptr;
  }

  static inline CPoolInfo* FindPoolInfoInternal(MemoryAllocatorBinned& allocator, uintptr_t ptr, uint16& jump_offset)
  {
    CHECK_SLOW(allocator.hash_buckets_);

    uint32 key = ptr >> allocator.hash_key_shift_;
    uint32 hash = key & (allocator.max_hash_buckets_ - 1);
    uint32 pool_index = ((uintptr_t)ptr >> allocator.pool_bit_shift_) & allocator.pool_mask_;
   ...
   */
  static inline CPoolHashBucket* CreateHashBucket(MemoryAllocatorBinned& allocator)
  {
   ...
   */
  static inline void InitializeHashBucket(MemoryAllocatorBinned& allocator, CPoolHashBucket* bucket)
  {
  ...
  static CPoolHashBucket* AllocateHashBucket(MemoryAllocatorBinned& allocator)
  {
    if (allocator.hash_bucket_free_list_ == nullptr) {
      ...
      allocator.hash_bucket_free_list_ = (CPoolHashBucket*)PlatformMemory::BinnedAllocFromOS(PAGE_SIZE);
      BINNED_PEAK_STATCOUNTER(allocator.os_peak_, BINNED_ADD_STATCOUNTER(allocator.os_current_, PAGE_SIZE));
      BINNED_PEAK_STATCOUNTER(allocator.waste_peak_, BINNED_ADD_STATCOUNTER(allocator.waste_current_, PAGE_SIZE));

      for (uintptr_t i = 0, N = PAGE_SIZE / sizeof(CPoolHashBucket); i < N; ++i) {
        allocator.hash_bucket_free_list_->Link(new (allocator.hash_bucket_free_list_ + i) CPoolHashBucket());
      }
    }
  ...
  static CPoolInfo* AllocatePoolMemory(MemoryAllocatorBinned& allocator, CPoolTable* table, uint32 PoolSize, uint16 table_index)
  {
    const uint32 PAGE_SIZE = allocator.PAGE_SIZE;
    ...
    CPoolInfo* pool;
    {
      ...
      for (uintptr_t i = (uintptr_t)PAGE_SIZE, offset = 0; i < OsBytes; i += PAGE_SIZE, ++offset) {
        ...
        TrailingPool->SetAllocationSizes(0, 0, offset, allocator.binned_os_table_index_);
      }

      BINNED_PEAK_STATCOUNTER(allocator.os_peak_, BINNED_ADD_STATCOUNTER(allocator.os_current_, OsBytes));
      BINNED_PEAK_STATCOUNTER(allocator.waste_peak_, BINNED_ADD_STATCOUNTER(allocator.waste_current_, (OsBytes - Bytes)));
    }

    // Init pool.
    pool->Link(table->first_pool);
    pool->SetAllocationSizes(Bytes, OsBytes, table_index, allocator.binned_os_table_index_);
    pool->taken = 0;
    pool->first_mem = Free;

#if STATS
    table->active_pool_count++;
    table->max_active_pools = MathBase::Max(table->max_active_pools, table->active_pool_count);
#endif
  ...
  static inline CFreeMem* AllocateBlockFromPool(MemoryAllocatorBinned& allocator, CPoolTable* table, CPoolInfo* pool, uint32 alignment)
  {
    // Pick first available block and unlink it.
    pool->taken++;
    CHECK_SLOW(pool->table_index < allocator.binned_os_table_index_); // if this is false, first_mem is actually a size not a pointer
    ...
    CFreeMem* Free = (CFreeMem*)((uint8*)pool->first_mem + --pool->first_mem->free_block_count * table->BlockSize);
    ...
    BINNED_PEAK_STATCOUNTER(allocator.used_peak_, BINNED_ADD_STATCOUNTER(allocator.used_current_, table->BlockSize));
    return Align(Free, alignment);
  }

  /**
   * Releases memory back to the system. This is not protected from multi-threaded access and it's
   * the callers responsibility to guard access_mutex_ before calling this.
   */
  static void FreeInternal(MemoryAllocatorBinned& allocator, void* ptr)
  {
    ...
    if (pool == nullptr)
    {
      fun_log(LogMemory, Warning, TEXT("Attempting to free a pointer we didn't allocate!"));
      return;
    }
#endif
    CHECK_SLOW(pool);
    ...
    if (pool->table_index < allocator.binned_os_table_index_) {
      ...
      pool->first_mem = Free;
      BINNED_ADD_STATCOUNTER(allocator.used_current_, -(int64)(table->BlockSize));
...
#endif
        // Free the OS memory.
        size_t OsBytes = pool->GetOsBytes(allocator.PAGE_SIZE, allocator.binned_os_table_index_);
        BINNED_ADD_STATCOUNTER(allocator.os_current_, -(int64)OsBytes);
        BINNED_ADD_STATCOUNTER(allocator.waste_current_, -(int64)(OsBytes - pool->GetBytes()));
        pool->Unlink();
  ...
  {
#ifdef USE_LOCKFREE_DELETE
    if (allocator.pending_free_list_ == nullptr && !allocator.done_flush_list_init_)
    {
      allocator.done_flush_list_init_ = true;
      allocator.pending_free_list_ = new ((void*)allocator.pending_free_list_memory_) TLockFreePointerList<void>();
    }
  ...
  static inline void OSFree(MemoryAllocatorBinned& allocator, void* ptr, size_t size)
  {
    ...
    while (allocator.freed_page_block_count_ && (allocator.freed_page_block_count_ >= MAX_CACHED_OS_FREES || allocator.cached_total_ + size > MAX_CACHED_OS_FREES_BYTE_LIMIT)) {
      ...
      if (allocator.freed_page_block_count_ > 0) {
        UnsafeMemory::Memmove(&allocator.freed_page_blocks_[0], &allocator.freed_page_blocks_[1], sizeof(CFreePageBlock) * allocator.freed_page_block_count_);
      }
      PlatformMemory::BinnedFreeToOS(free_ptr, free_size);
    }
    allocator.freed_page_blocks_[allocator.freed_page_block_count_].ptr = ptr;
    allocator.freed_page_blocks_[allocator.freed_page_block_count_].ByteSize = size;
  ...
  static inline void* OSAlloc(MemoryAllocatorBinned& allocator, size_t new_size, size_t& OutActualSize)
  {
#ifdef CACHE_FREED_OS_ALLOCS
    {
#ifdef USE_FINE_GRAIN_LOCKS
      ...
      for (uint32 i = 0; i < allocator.freed_page_block_count_; ++i)
      {
        ...
        if (allocator.freed_page_blocks_[i].ByteSize == new_size)
        {
          ...
          {
            UnsafeMemory::Memmove(&allocator.freed_page_blocks_[i], &allocator.freed_page_blocks_[i + 1], sizeof(CFreePageBlock) * (allocator.freed_page_block_count_ - i - 1));
          }
          allocator.freed_page_block_count_--;
          return Ret;
        }
      ...
      for (uint32 i = 0; i < allocator.freed_page_block_count_; ++i)
      {
        ...
        if (allocator.freed_page_blocks_[i].ByteSize >= new_size && allocator.freed_page_blocks_[i].ByteSize * 3 <= new_size * 4)
        {
          void* Ret = allocator.freed_page_blocks_[i].ptr;
          OutActualSize = allocator.freed_page_blocks_[i].ByteSize;
          allocator.cached_total_ -= allocator.freed_page_blocks_[i].ByteSize;
  ...
  static void FlushAllocCache(MemoryAllocatorBinned& allocator)
  {
    ...
    {
      //Remove allocs
      PlatformMemory::BinnedFreeToOS(allocator.freed_page_blocks_[i].ptr, allocator.freed_page_blocks_[i].ByteSize);
  ...
  static void UpdateSlackStat(MemoryAllocatorBinned& allocator)
  {
    ...
    for (int32 pool_index = 0; pool_index < POOL_COUNT; pool_index++)
    {
      ...
      Waste += ((double)table.total_waste / (double)table.total_request_count) * (double)table.active_request_count;
      Waste += table.active_pool_count * (BINNED_ALLOC_POOL_SIZE - ((BINNED_ALLOC_POOL_SIZE / table.BlockSize) * table.BlockSize));
    }
    LocalWaste += (uint32)Waste;
    allocator.slack_current_ = allocator.os_current_ - LocalWaste - allocator.used_current_;
  #endif //STATS
  }
};
...
void MemoryAllocatorBinned::GetAllocatorStats(GenericMemoryStats& out_stats)
{
  ...
  out_stats.Add(GET_STATDESCRIPTION(STAT_Binned_WasteCurrent), local_waste_count);
  out_stats.Add(GET_STATDESCRIPTION(STAT_Binned_WastePeak), local_waste_peak);
  out_stats.Add(GET_STATDESCRIPTION(STAT_Binned_UsedCurrent), local_used_current);
  out_stats.Add(GET_STATDESCRIPTION(STAT_Binned_UsedPeak), local_used_peak);
  out_stats.Add(GET_STATDESCRIPTION(STAT_Binned_CurrentAllocs), local_current_allocs);
  out_stats.Add(GET_STATDESCRIPTION(STAT_Binned_TotalAllocs), local_total_allocs);
  out_stats.Add(GET_STATDESCRIPTION(STAT_Binned_SlackCurrent), local_slack_current);
#endif //STATS
}
...
MemoryAllocatorBinned::MemoryAllocatorBinned(uint32 InPageSize, uint64 AddressLimit)
...
#endif
{
  ...
  fun_check(AddressLimit > PAGE_SIZE); // Check to catch 32 bit overflow in AddressLimit

  // Shift to get the reference from the indirect tables
  pool_bit_shift_ = CPlatformMath::CeilLogTwo(PAGE_SIZE);
  indect_pool_block_shift_ = CPlatformMath::CeilLogTwo(PAGE_SIZE/sizeof(CPoolInfo));
  indect_pool_block_size_ = PAGE_SIZE/sizeof(CPoolInfo);

  max_hash_buckets_ = AddressLimit >> (indect_pool_block_shift_+pool_bit_shift_);
  ...
  max_book_keeping_overhead_ = ((AddressLimit/PAGE_SIZE)*sizeof(CPoolHashBucket))/(1024*1024);
  ...
  binned_os_table_index_ = binned_size_limit_+EXTENDED_PAGE_POOL_ALLOCATION_COUNT;
  ...
  page_pool_table_[0].BlockSize = PAGE_SIZE == Private::PAGE_SIZE_LIMIT ? binned_size_limit_+(binned_size_limit_/2) : 0;

  page_pool_table_[1].first_pool = nullptr;
  page_pool_table_[1].exhausted_pool = nullptr;
  page_pool_table_[1].BlockSize = PAGE_SIZE == Private::PAGE_SIZE_LIMIT ? PAGE_SIZE+binned_size_limit_ : 0;
  ...
  fun_check(MAX_POOLED_ALLOCATION_SIZE - 1 == pool_table_[POOL_COUNT - 1].BlockSize);
}
...
void* MemoryAllocatorBinned::Malloc(size_t size, uint32 alignment)
{
  ...
  alignment = MathBase::Max<uint32>(alignment, Private::DEFAULT_BINNED_ALLOCATOR_ALIGNMENT);
  size_t SpareBytesCount = MathBase::Min<size_t>(Private::DEFAULT_BINNED_ALLOCATOR_ALIGNMENT, size);
  size = MathBase::Max<size_t>(pool_table_[0].BlockSize, size + (alignment - SpareBytesCount));
  MEM_TIME(mem_time_ -= SystemTime::Seconds());
  ...
  if (size < binned_size_limit_) {
    ...
    if (pool == nullptr) {
      pool = Private::AllocatePoolMemory(*this, table, Private::BINNED_ALLOC_POOL_SIZE/*PAGE_SIZE*/, size);
    }

    Free = Private::AllocateBlockFromPool(*this, table, pool, alignment);
  }
  else if (((size >= binned_size_limit_ && size <= page_pool_table_[0].BlockSize) ||
    ...
    CPoolInfo* pool = table->first_pool;
    if (pool == nullptr) {
      pool = Private::AllocatePoolMemory(*this, table, PageCount*PAGE_SIZE, binned_size_limit_+BinType);
    }
    ...
    CPoolInfo* pool;
    {
      ...
      if ((uintptr_t)Free != ((uintptr_t)aligned_free & ~((uintptr_t)PAGE_SIZE - 1))) {
        // Mark the CPoolInfo for aligned_free to jump back to the CPoolInfo for ptr.
        for (uintptr_t i = (uintptr_t)PAGE_SIZE, offset = 0; i < AlignedSize; i += PAGE_SIZE, ++offset)
        {
          CPoolInfo* TrailingPool = Private::GetPoolInfo(*this, ((uintptr_t)Free) + i);
          fun_check(TrailingPool);
          //Set trailing pools to point back to first pool
          TrailingPool->SetAllocationSizes(0, 0, offset, binned_os_table_index_);
        }
      }
    }
    Free = (CFreeMem*)aligned_free;
    pool->SetAllocationSizes(size, AlignedSize, binned_os_table_index_, binned_os_table_index_);
    BINNED_PEAK_STATCOUNTER(os_peak_, BINNED_ADD_STATCOUNTER(os_current_, AlignedSize));
    BINNED_PEAK_STATCOUNTER(used_peak_, BINNED_ADD_STATCOUNTER(used_current_, size));
    BINNED_PEAK_STATCOUNTER(waste_peak_, BINNED_ADD_STATCOUNTER(waste_current_, (int64)(AlignedSize - size)));
  }
...
void* MemoryAllocatorBinned::Realloc(void* ptr, size_t new_size, uint32 alignment)
{
  // Handle DEFAULT_ALIGNMENT for binned allocator.
  if (alignment == DEFAULT_ALIGNMENT) {
    alignment = Private::DEFAULT_BINNED_ALLOCATOR_ALIGNMENT;
  ...
  size_t SpareBytesCount = MathBase::Min<size_t>(Private::DEFAULT_BINNED_ALLOCATOR_ALIGNMENT, new_size);
  if (new_size > 0) {
    new_size = MathBase::Max<size_t>(pool_table_[0].BlockSize, new_size + (alignment - SpareBytesCount));
  }
  MEM_TIME(mem_time_ -= SystemTime::Seconds());
  uintptr_t base_ptr;
  void* new_ptr = ptr;
  if (ptr && new_size > 0) {
    ...
    if (pool->table_index < binned_os_table_index_) {
      ...
      fun_check(pool->table_index > 0); // it isn't possible to allocate a size of 0, Malloc will increase the size to DEFAULT_BINNED_ALLOCATOR_ALIGNMENT
      if (NewSizeUnmodified > mem_size_to_pool_table_[pool->table_index]->BlockSize || NewSizeUnmodified <= mem_size_to_pool_table_[pool->table_index - 1]->BlockSize) {
        new_ptr = Malloc(NewSizeUnmodified, alignment);
        UnsafeMemory::Memcpy(new_ptr, ptr, MathBase::Min<size_t>(NewSizeUnmodified, mem_size_to_pool_table_[pool->table_index]->BlockSize - (alignment - SpareBytesCount)));
        Free(ptr);
      }
      ...
      // Allocated from OS.
      if (new_size > pool->GetOsBytes(PAGE_SIZE, binned_os_table_index_) || new_size * 3 < pool->GetOsBytes(PAGE_SIZE, binned_os_table_index_) * 2) {
        // Grow or shrink.
        new_ptr = Malloc(NewSizeUnmodified, alignment);
        UnsafeMemory::Memcpy(new_ptr, ptr, MathBase::Min<size_t>(NewSizeUnmodified, pool->GetBytes()));
        Free(ptr);
      }
      else {
//need a lock to cover the SetAllocationSizes()
        ...
        pool->SetAllocationSizes(NewSizeUnmodified, pool->GetOsBytes(PAGE_SIZE, binned_os_table_index_), binned_os_table_index_, binned_os_table_index_);
      }
    }
  }
  else if (ptr == nullptr) {
...
bool MemoryAllocatorBinned::GetAllocationSize(void* reported_ptr, size_t& out_allocation_size)
{
  ...
  uintptr_t base_ptr;
  CPoolInfo* pool = Private::FindPoolInfo(*this, (uintptr_t)reported_ptr, base_ptr);
  out_allocation_size = pool->table_index < binned_os_table_index_ ? mem_size_to_pool_table_[pool->table_index]->BlockSize : pool->GetBytes();
  return true;
}
...
bool MemoryAllocatorBinned::ValidateHeap()
{
  ...
  for (int32 i = 0; i < POOL_COUNT; i++) {
    ...
    for (CPoolInfo** pool_ptr = &table->first_pool; *pool_ptr; pool_ptr = &(*pool_ptr)->next) {
    ...
    for (CPoolInfo** pool_ptr = &table->exhausted_pool; *pool_ptr; pool_ptr = &(*pool_ptr)->next) {
      CPoolInfo* pool = *pool_ptr;
      fun_check(pool->prev_link == pool_ptr);
...
void MemoryAllocatorBinned::DumpAllocatorStats(class Printer& ar)
{
  CBufferedPrinter BufferedOutput;
  {
    ...
    BufferedOutput.CategorizedPrintf(LogMemory.GetCategoryName(), ELogVerbosity::Info, TEXT("allocator Stats for %s:"), GetDescriptiveName());
    // Waste is the total overhead of the memory system
    BufferedOutput.CategorizedPrintf(LogMemory.GetCategoryName(), ELogVerbosity::Info, TEXT("Current Memory %.2f MB used, plus %.2f MB waste"), used_current_ / (1024.0f * 1024.0f), (os_current_ - used_current_) / (1024.0f * 1024.0f));
    BufferedOutput.CategorizedPrintf(LogMemory.GetCategoryName(), ELogVerbosity::Info, TEXT("Peak Memory %.2f MB used, plus %.2f MB waste"), used_peak_ / (1024.0f * 1024.0f), (os_peak_ - used_peak_) / (1024.0f * 1024.0f));

    BufferedOutput.CategorizedPrintf(LogMemory.GetCategoryName(), ELogVerbosity::Info, TEXT("Current OS Memory %.2f MB, peak %.2f MB"), os_current_ / (1024.0f * 1024.0f), os_peak_ / (1024.0f * 1024.0f));
    BufferedOutput.CategorizedPrintf(LogMemory.GetCategoryName(), ELogVerbosity::Info, TEXT("Current Waste %.2f MB, peak %.2f MB"), waste_current_ / (1024.0f * 1024.0f), waste_peak_ / (1024.0f * 1024.0f));
    BufferedOutput.CategorizedPrintf(LogMemory.GetCategoryName(), ELogVerbosity::Info, TEXT("Current Used %.2f MB, peak %.2f MB"), used_current_ / (1024.0f * 1024.0f), used_peak_ / (1024.0f * 1024.0f));
    BufferedOutput.CategorizedPrintf(LogMemory.GetCategoryName(), ELogVerbosity::Info, TEXT("Current Slack %.2f MB"), slack_current_ / (1024.0f * 1024.0f));

    BufferedOutput.CategorizedPrintf(LogMemory.GetCategoryName(), ELogVerbosity::Info, TEXT("Allocs      % 6i Current / % 6i Total"), current_alloc_count_, total_alloc_count_);
    MEM_TIME(BufferedOutput.CategorizedPrintf(LogMemory.GetCategoryName(), ELogVerbosity::Info, TEXT("Seconds     % 5.3f"), mem_time_));
    MEM_TIME(BufferedOutput.CategorizedPrintf(LogMemory.GetCategoryName(), ELogVerbosity::Info, TEXT("MSec/Allc   % 5.5f"), 1000.0 * mem_time_ / MemAllocs));

    // This is the memory tracked inside individual allocation pools
    BufferedOutput.CategorizedPrintf(LogMemory.GetCategoryName(), ELogVerbosity::Info, TEXT(""));
    BufferedOutput.CategorizedPrintf(LogMemory.GetCategoryName(), ELogVerbosity::Info, TEXT("Block size Num Pools Max Pools Cur Allocs Total Allocs Min Req Max Req Mem Used Mem Slack Mem Waste Efficiency"));
    BufferedOutput.CategorizedPrintf(LogMemory.GetCategoryName(), ELogVerbosity::Info, TEXT("---------- --------- --------- ---------- ------------ ------- ------- -------- --------- --------- ----------"));
    ...
    for (int32 i = 0; i < binned_size_limit_ + EXTENDED_PAGE_POOL_ALLOCATION_COUNT; i++) {
      if (table == mem_size_to_pool_table_[i] || mem_size_to_pool_table_[i]->BlockSize == 0) {
        continue;
      }
      ...
      uint32 total_alloc_size = (table->BlockSize > binned_size_limit_ ? (((3 * (i - binned_size_limit_)) + 3)*Private::BINNED_ALLOC_POOL_SIZE) : Private::BINNED_ALLOC_POOL_SIZE);
      // The amount of memory allocated from the OS
      uint32 mem_allocated = (table->active_pool_count * total_alloc_size) / 1024;
      ...
      uint32 pool_mem_waste = table->active_pool_count * (total_alloc_size - ((total_alloc_size / table->BlockSize) * table->BlockSize)) / 1024;
      // Wasted memory due to individual allocation alignment. This is an estimate.
      uint32 mem_waste = (uint32)(((double)table->total_waste / (double)table->total_request_count) * (double)table->active_request_count) / 1024 + pool_mem_waste;
      // Memory that is reserved in active pools and ready for future use
      uint32 mem_slack = mem_allocated - mem_used - pool_mem_waste;

      BufferedOutput.CategorizedPrintf(LogMemory.GetCategoryName(), ELogVerbosity::Info, TEXT("% 10i % 9i % 9i % 10i % 12i % 7i % 7i % 7iK % 8iK % 8iK % 9.2f%%"),
                    table->BlockSize,
                    table->active_pool_count,
                    table->max_active_pools,
                    table->active_request_count,
                    (uint32)table->total_request_count,
                    table->min_request,
                    table->max_request,
                    mem_used,
                    mem_slack,
                    mem_waste,
                    mem_allocated ? 100.0f * (mem_allocated - mem_waste) / mem_allocated : 100.0f);
    ...
    BufferedOutput.CategorizedPrintf(LogMemory.GetCategoryName(), ELogVerbosity::Info, TEXT(""));
    BufferedOutput.CategorizedPrintf(LogMemory.GetCategoryName(), ELogVerbosity::Info, TEXT("%iK allocated in pools (with %iK slack and %iK waste). Efficiency %.2f%%"), total_memory, total_slack, total_waste, total_memory ? 100.0f * (total_memory - total_waste) / total_memory : 100.0f);
    BufferedOutput.CategorizedPrintf(LogMemory.GetCategoryName(), ELogVerbosity::Info, TEXT("Allocations %i Current / %i Total (in %i pools)"), total_active_requests, total_requests, total_pools);
    BufferedOutput.CategorizedPrintf(LogMemory.GetCategoryName(), ELogVerbosity::Info, TEXT(""));
#endif
#endif
  }
...
const char* MemoryAllocatorBinned::GetDescriptiveName()
{
  return "binned";
}
