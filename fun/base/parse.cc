...
//  // @param " must not be 0
//  static void OnConsoleVariable(const char* name, IConsoleObject* cvar,
//  Set<String>& sink) {
...
//};
//
// void ConsoleCommandLibrary_DumpLibrary(env* env, Exec& sub_system, const
// String& pattern, Printer& out)
//{
...
//
//  for (Set<String>::ConstIterator
//  it(local_console_command_library.known_names); it; ++it) {
...
//  if (executed) {
//    out.Printf("ERROR: The function was supposed to only find matching
//    commands but not have any side effect."); out.Printf("However Exec()
//    returned true which means we either executed a command or the command
//    parsing returned true where it shouldn't.");
//  }
//}
//
//
// void ConsoleCommandLibrary_DumpLibraryHTML(env* env, Exec& sub_system, const
// String& out_path)
//{
//  const String& pattern("*");
//  ConsoleCommandLibrary local_console_command_library(pattern);
...
//  PrinterNull Null;
//
//  bool executed = sub_system.Exec(env, *local_console_command_library.pattern,
//  Null);
//  {
//    IConsoleManager::Get().ForEachConsoleObject(
//      ConsoleObjectVisitor::CreateStatic<Set<String>&>(
...
//  local_console_command_library.known_names.Sort(Less<String>());
//
//  String template_filename = CPaths::Combine(CPlatformProcess::BaseDir(),
//  "../../Documentation/Extras", "ConsoleHelpTemplate.html"); String
//  template_file; if (FileHelper::ReadAllText(template_file,
//  *template_filename,
//    FileHelper::HashOptions(FileHelper::HashOption::EnableVerify) |
//    FileHelper::HashOption::ErrorMissingHash)) {
//    // todo: do we need to create the directory?
//    UniquePtr<Archive>
//    file(IFileManager::Get().CreateDebugFileWriter(*out_path)); if
//    (file.IsValid()) {
//      LazyPrintf lazy_printf(*template_file);
//
//      lazy_printf.PushParam("FUN Console Variables and Commands");    // title
//      lazy_printf.PushParam("FunConsole Variables and Commands");   //
//      headline lazy_printf.PushParam("FunConsole command 'Help'"); //
//      generated by lazy_printf.PushParam("0.95"); // version
...
//          }
//
//          //{name: "r.SetRes", help:"To change the screen/window
//          resolution."}, String data_line = String::Format("{name: \"%s\",
//          help:\"%s\", type:\"%s\"},\r\n", *name, *help, element_type);
//          all_data += data_line;
//        } else {
//          // Exec command (better we change them to use the new method as it
//          has better help and is more convenient to use)
//          //{name: "", help:"To change the screen/window resolution."},
//          String data_line = String::Format("{name: \"%s\", help:\"Sorry: Exec
//          commands have no help\", type:\"Exec\"},\r\n", *name); all_data +=
//          data_line;
//        }
//      }
//
...
bool Parse::Value(const char* stream, const char* match, char* value,
                  int32 max_len, bool should_stop_on_comma) {
  ...
  // Number of characters we can look back from found looking for first
  // parenthesis.
  ...
  const bool is_fully_quoted =
      ...
      // "-Option=Value1 Value2"
      //   ^~~~found
      || (allowed_backtrace_char_count > 1 &&
          (found[-1] == '-' && found[-2] == '"'));
  ...
  if (is_arg_quoted || is_fully_quoted) {
    ...
    if (tmp) *tmp = 0;
    tmp = CStringTraitsA::Strstr(value, "\r");
    if (tmp) *tmp = 0;
    tmp = CStringTraitsA::Strstr(value, "\n");
    if (tmp) *tmp = 0;
    tmp = CStringTraitsA::Strstr(value, "\t");
...
bool Parse::Value(const char* stream, const char* match, String& value,
...
bool Parse::QuotedString(const char* buffer, String& value,
                         int32* readed_char_count) {
...
bool Parse::Value(const char* stream, const char* match, uint32& value) {
  if (const char* found = CStringTraitsA::Strfind(stream, match)) {
    char* end;
    value =
        CStringTraitsA::Strtoi(found + CStringTraitsA::Strlen(match), &end, 10);
...
bool Parse::Value(const char* stream, const char* match, Text& value, const
char* ns) { stream = CStringTraitsA::Strfind(stream, match); if (stream) {
...
bool Parse::Command(const char** stream, const char* match,
  ...
  if (CStringTraitsA::Strnicmp(*stream, match, CStringTraitsA::Strlen(match)) ==
      0) {
    *stream += CStringTraitsA::Strlen(match);

    if (!CharTraitsA::IsAlnum(**stream)) {
      // if (!CharTraitsA::IsAlnum(**stream) && (**stream != '_') && (**stream
      // != '.')) { // more correct e.g. a cvar called "log.abc" should work but
      // breaks some code so commented out
      while (**stream == ' ' || **stream == 9) {
        ++(*stream);
      }
      return true;  // Success.
...
bool Parse::Token(const char*& str, char* result, int32 max_len,
                  bool use_escape) {
  int32 len = 0;
...
bool Parse::Token(const char*& str, String& arg, bool use_escape) {
  ...
  if (*str == '"') {
    ...
    // Get unquoted string (that might contain a quoted part, which will be left
    // intact). For example, -ARG="foo bar baz", will be treated as one token,
    // with quotes intact
    bool in_quote = false;
...
bool Parse::LineExtended(const char** stream, String& result,
                         int32& consumed_line_count, bool exact) {
  bool got_stream = false;
  bool is_quoted = false;
  bool ignore = false;
  ...
  while (**stream != 0 &&
         ((**stream != 10 && **stream != 13) || bracket_depth > 0)) {
    // start of comments.
    if (!is_quoted && !exact && (*stream)[0] == '/' && (*stream)[1] == '/') {
      ignore = true;
    }
    ...
    else if (!is_quoted && (*stream)[0] == '\\' &&
             ((*stream)[1] == 10 || (*stream)[1] == 13)) {
      result.Append(' ');
      ++consumed_line_count;
      (*stream) += 2;
      if (**stream == 10 || **stream == 13) {
  ...
  if (**stream == 0) {
    ...
    while (**stream == 10 || **stream == 13 || **stream == '|') {
      ...
      if (((*stream)[0] == 10 && (*stream)[1] == 13) ||
          ((*stream)[0] == 13 && (*stream)[1] == 10)) {
        ++(*stream);
      }
...
bool Parse::SchemeNameFromUri(const char* uri, String& out_scheme_name) {
  for (int32 i = 0;; ++i) {
    ...
    if (!CharTraitsA::IsAlpha(ch) && !CharTraitsA::IsDigit(ch) && ch != '+' &&
        ch != '.' && ch != '-') {
      if (ch == ':' && i > 0) {
