 ...
 */
template <typename _ElementType, typename _KeyType, bool _AllowDuplicateKeys = false>
struct BaseKeyFuncs
{
...
struct DefaultKeyFuncs : BaseKeyFuncs<ElementType, ElementType, AllowDuplicateKeys>
{
 ...
 * Should be in FunTemplate but isn't for Clang build reasons - will move later
 */
template <typename T>
...
class SetElementId
{
 public:
   ...
   * Default constructor.
   */
  FUN_ALWAYS_INLINE SetElementId()
   ...
   */
  FUN_ALWAYS_INLINE bool IsValidId() const
  {
   ...
   * Comparison operator.
   */
  FUN_ALWAYS_INLINE friend bool operator == (const SetElementId& x, const SetElementId& y)
  {
    return x.index_ == y.index_;
  }

  FUN_ALWAYS_INLINE int32 AsInteger() const
  {
    return index_;
  }
 ...
 private:
   ...
   */
  FUN_ALWAYS_INLINE SetElementId(int32 index)
    : index_(index)
  {
  }
   ...
   */
  FUN_ALWAYS_INLINE operator int32() const
  {
    return index_;
  }
};
...
class SetElement
{
 public:
  ...
  FUN_ALWAYS_INLINE SetElement(const InitType& value)
    : value(value)
  {
  }

  template <typename InitType>
  FUN_ALWAYS_INLINE SetElement(InitType&& value)
   ...
   * Serializer.
   */
  FUN_ALWAYS_INLINE friend Archive& operator & (Archive& ar, SetElement& element)
  {
...
class Set
{
 ...
 public:
   ...
   * Copy constructor.
   */
  FUN_ALWAYS_INLINE Set(const Set& rhs)
    : hash_size_(0)
  {
    *this = rhs;
  }
  ...
  FUN_ALWAYS_INLINE explicit Set(const Array<ElementType, ArrayAllocator>& array)
    : hash_size_(0)
  {
    Append(array);
  }
 ...
 private:
  ...
  static FUN_ALWAYS_INLINE typename EnableIf<ContainerTraits<SetType>::MoveWillEmptyContainer>::Type
 ...
 public:
   ...
   */
  Set(std::initializer_list<ElementType> init_list)
    : hash_size_(0)
  {
    Append(init_list);
  }
  ...
  {
    // Clear the elements array, and reallocate it for the expected number of elements.
    ...
    if (!ConditionalRehash(count, true)) {
      ...
      for (int32 hash_index = 0, local_hash_size = hash_size_; hash_index < local_hash_size; ++hash_index) {
  ...
  {
    // makes sense only when capacity > elements_.Count() since SparseArray::Reserve
   ...
   */
  FUN_ALWAYS_INLINE void Relax()
  {
    ConditionalRehash(elements_.Count(), true);
  }
  ...
  FUN_ALWAYS_INLINE void CountBytes(Archive& ar)
  {
    ...
    ar.CountBytes(hash_size_ * sizeof(int32), hash_size_ * sizeof(SetElementId));
  }
   ...
   *
   * Returns true if the element identifier refers to a valid element in this set.
   */
  FUN_ALWAYS_INLINE bool IsValidId(SetElementId id) const
  {
    return  id.IsValidId() &&
            id >= 0 &&
            id < elements_.GetMaxIndex() &&
   ...
   * \param out_is_already_in_set - [out] Optional pointer to bool that will be set depending on whether element is already in set
   *
   * Returns A pointer to the element stored in the set.
   */
  FUN_ALWAYS_INLINE SetElementId Add(const ElementType& element, bool* out_is_already_in_set = nullptr)
  {
    return Emplace(element, out_is_already_in_set);
  }

  FUN_ALWAYS_INLINE SetElementId Add(ElementType&& element, bool* out_is_already_in_set = nullptr)
  {
  ...
  SetElementId Emplace(ArgsType&& args, bool* out_is_already_in_set = nullptr)
  {
    ...
    auto& element = *new(element_allocation) SetElementType(Forward<ArgsType>(args));
  ...
  void Remove(SetElementId element_id)
  {
    if (elements_.Count()) {
      ...
      for ( SetElementId* next_element_id = &GetTypedHash(element_being_removed.hash_index);
   ...
   *
   * Returns The id of the set element matching the given key, or the NULL id if none matches.
   */
  SetElementId FindId(KeyInitType key) const
  {
    if (elements_.Count() > 0) {
      for ( SetElementId element_id = GetTypedHash(KeyFuncs::GetKeyHash(key));
        ...
        if (KeyFuncs::Matches(KeyFuncs::GetSetKey(elements_[element_id].value), key)) {
  ...
  //{
  //  out.Printf("Set: %i elements, %i hash slots", elements_.Count(), hash_size_);
  //  for (int32 hash_index = 0, local_hash_size = hash_size_; hash_index < local_hash_size; ++hash_index) {
  ...
  //    while (element_id.IsValidId()) {
  //      if (!IsValidId(element_id)) {
  //        out.Printf(TEXT("      !!INVALID!! element_id = %d"), element_id.index);
  //      }
  //      else {
   ...
   *
   * Returns True if the other set is entirely included in this set, false if it is not
   */
   ...
   * Used for implementations to check that reference arguments aren't going to be
   * invalidated by possible reallocation.
   *
 ...
 private:
  ...
  class ElementCompareClass
  {
   public:
    ...
    FUN_ALWAYS_INLINE bool operator()(const SetElementType& a, const SetElementType& b) const
    {
  ...
  {
    return ((SetElementId*)hash_.GetAllocation())[hash_index & (hash_size_ - 1)];
  }
   ...
   */
  FUN_ALWAYS_INLINE const SetElementType& GetInternalElement(const SetElementId& id) const
  {
    return elements_[id];
  }
   ...
   */
  FUN_ALWAYS_INLINE void HashElement(SetElementId element_id, const SetElementType& element) const
  {
    // Compute the hash bucket the element goes in.
    element.hash_index = KeyFuncs::GetKeyHash(KeyFuncs::GetSetKey(element.value)) & (hash_size_ - 1);
   ...
   */
  bool ConditionalRehash(int32 hashed_element_count, bool allow_shrinking = false) const
  {
    //TODO 강제로 고정된 해시 버킷수를 지정해서 사용하고, Rehash를 금지시키는 기능을 넣는건 어떨런지?

    // Calculate the desired hash size for the specified number of elements.
    const int32 desired_hash_size = Allocator::GetNumberOfHashBuckets(hashed_element_count);
  ...
  void Rehash() const
  {
    ...
    if (local_hash_size > 0) {
      ...
      for (typename ElementArrayType::ConstIterator element_it_(elements_); element_it_; ++element_it_) {
  ...
  class BaseIterator
  {
   ...
   public:
    using ElementItType = typename Conditional<IsConst, typename ElementArrayType::ConstIterator, typename ElementArrayType::Iterator>::Result;
 ...
 public:
  ...
  class ConstIterator : public BaseIterator<true>
  {
   ...
   public:
    FUN_ALWAYS_INLINE ConstIterator(const typename BaseIterator<true>::ElementItType& element_id)
  ...
  class Iterator : public BaseIterator<false>
  {
   ...
   public:
    FUN_ALWAYS_INLINE Iterator(Set& set, const typename BaseIterator<false>::ElementItType& element_id)
      : BaseIterator<false>(element_id)
      , set_(set)
    {
    }

    FUN_ALWAYS_INLINE Iterator(Set& set)
      : BaseIterator<false>(begin(set.elements_))
      , set_(set)
    {
    }
 ...
 private:
  ...
  FUN_ALWAYS_INLINE friend Iterator begin(Set& set) { return Iterator(set, begin(set.elements_)); }
  FUN_ALWAYS_INLINE friend ConstIterator begin(const Set& set) { return ConstIterator(begin(set.elements_)); }
  FUN_ALWAYS_INLINE friend Iterator end(Set& set) { return Iterator(set, end(set.elements_)); }
  FUN_ALWAYS_INLINE friend ConstIterator end(const Set& set) { return ConstIterator(end(set.elements_)); }
};
...
{
 public:
  static UntypedSetLayout GetUntypedLayout(int32 element_size, int32 element_alignment)
  {
    UntypedSetLayout result;

    // SetElement<TPair<key, value>>
    StructBuilder set_element_struct;
    result.element_offset = set_element_struct.AddMember(element_size, element_alignment);
    result.hash_next_id_offset = set_element_struct.AddMember(sizeof(SetElementId), alignof(SetElementId));
    result.hash_index_offset = set_element_struct.AddMember(sizeof(int32), alignof(int32));
    result.size = set_element_struct.GetSize();
    result.sparse_array_layout = UntypedSparseArray::GetUntypedLayout(set_element_struct.GetSize(), set_element_struct.GetAlignment());
  ...
  {
    return elements_.IsValidIndex(index);
  }
  ...
  void Clear(int32 slack, const UntypedSetLayout& layout)
  {
    ...
    for (auto* it = (SetElementId*)hash_.GetAllocation(), *end = it + hash_size_; it != end; ++it) {
      *it = SetElementId();
    }
  }
  ...
  void RemoveAt(int32 index, const UntypedSetLayout& layout)
  {
    ...
    auto* element_being_removed = elements_.MutableData(index, layout.sparse_array_layout);

    // Remove the element from the hash.
    for ( SetElementId* next_element_id = &GetTypedHash(GetHashIndexRef(element_being_removed, layout));
          next_element_id->IsValidId();
          next_element_id = &GetHashNextIdRef(elements_.MutableData(next_element_id->AsInteger(), layout.sparse_array_layout), layout)) {
  ...
  void Rehash(const UntypedSetLayout& layout, TFunctionRef<uint32 (const void*)> get_type_hash)
  {
    // Free the old hash.
    hash_.ResizeAllocation(0, 0, sizeof(SetElementId));
    ...
    if (hash_size_ > 0) {
      ...
      while (count > 0) {
        if (elements_.IsValidIndex(index)) {
          ...
          void* element = (uint8*)elements_.MutableData(index, layout.sparse_array_layout);
 ...
 private:
  ...
  static SetElementId& GetHashNextIdRef(const void* element, const UntypedSetLayout& layout)
  {
    return *(SetElementId*)((uint8*)element + layout.hash_next_id_offset);
  }

  static int32& GetHashIndexRef(const void* element, const UntypedSetLayout& layout)
  {
  ...
  static void CheckConstraints()
  {
    ...
    static_assert(sizeof (UntypedType) == sizeof (TypedType), "UntypedSet's size doesn't match Set");
    static_assert(alignof(UntypedType) == alignof(TypedType), "UntypedSet's alignment doesn't match Set");

    // Check member sizes
    static_assert(sizeof(DeclVal<UntypedType>().elements_ ) == sizeof(DeclVal<TypedType>().elements_ ), "UntypedSet's elements_ member size does not match Set's");
    static_assert(sizeof(DeclVal<UntypedType>().hash_     ) == sizeof(DeclVal<TypedType>().hash_     ), "UntypedSet's Hash member size does not match Set's");
    static_assert(sizeof(DeclVal<UntypedType>().hash_size_) == sizeof(DeclVal<TypedType>().hash_size_), "UntypedSet's hash_size_ member size does not match Set's");

    // Check member offsets
    static_assert(offsetof(UntypedType, elements_ ) == offsetof(TypedType, elements_ ), "UntypedSet's elements_ member offset does not match Set's");
    static_assert(offsetof(UntypedType, hash_     ) == offsetof(TypedType, hash_     ), "UntypedSet's Hash member offset does not match Set's");
    static_assert(offsetof(UntypedType, hash_size_) == offsetof(TypedType, hash_size_), "UntypedSet's FirstFreeIndex member offset does not match Set's");
  }
...
template <> struct IsZeroConstructible<UntypedSet> { enum { Value = true }; };
