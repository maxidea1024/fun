 ...
 * - A method IndexType Count() const that returns the number of items in the
 * container.
 * - A method bool IsValidIndex(IndexType index) which returns whether a given
 * index is valid in the container.
 * - A method ElementType& operator\[\](IndexType index) which returns a
 * reference to a contained object by index.
 */
template <typename ContainerType, typename ElementType, typename IndexType>
class IndexedContainerIterator {
 public:
  IndexedContainerIterator(ContainerType& container,
                           IndexType starting_index = 0)
      : container_(container), index_(starting_index) {}
  ...
  ElementType& operator*() const { return container_[index_]; }
  ...
  FUN_ALWAYS_INLINE IndexType GetIndex() const { return index_; }

  FUN_ALWAYS_INLINE void Reset() { index_ = 0; }

  FUN_ALWAYS_INLINE friend bool operator==(
      const IndexedContainerIterator& lhs,
      const IndexedContainerIterator& rhs) {
 ...
 */
template <typename ContainerType, typename ElementType, typename IndexType>
FUN_ALWAYS_INLINE IndexedContainerIterator<ContainerType, ElementType,
                                           IndexType>
operator+(int32 offset,
          IndexedContainerIterator<ContainerType, ElementType, IndexType> rhs) {
  return rhs + offset;
}
...
struct CheckedPointerIterator {
  ...
  explicit CheckedPointerIterator(const int32& count, ElementType* ptr)
      : ptr_(ptr), current_count_(count), initial_count_(count) {}

  FUN_ALWAYS_INLINE ElementType& operator*() const { return *ptr_; }
 ...
 private:
  ...
  friend bool operator!=(const CheckedPointerIterator& lhs,
                         const CheckedPointerIterator& rhs) {
    // We only need to do the check in this operator, because no other operator
    // will be called until after this one returns.
    //
    // Also, we should only need to check one side of this comparison - if the
    // other iterator isn't even from the same array then the compiler has
    // generated bad code.
    fun_check_msg(lhs.current_count_ == lhs.initial_count_,
...
class DereferencingIterator {
 public:
  explicit DereferencingIterator(IteratorType iter) : iter_(iter) {}
 ...
 private:
  ...
  FUN_ALWAYS_INLINE friend bool operator!=(const DereferencingIterator& lhs,
                                           const DereferencingIterator& rhs) {
...
/**
 * Base dynamic array.
 * An untyped data array; mirrors a Array's members, but doesn't need an exact
 * C++ type for its elements.
 */
class UntypedArray : protected HeapAllocator::ForAnyElementType {
 public:
  void* MutableData() { return this->GetAllocation(); }

  const void* ConstData() const { return this->GetAllocation(); }

  bool IsValidIndex(int32 index) const { return index >= 0 && index < count_; }
  ...
  void Clear(int32 slack, int32 bytes_per_element) {
    ...
    // only reallocate if we need to, I don't trust realloc to the same size to
    // work
    if (capacity_ != slack) {
  ...
  UntypedArray() : count_(0), capacity_(0) {}
   ...
   */
  int32 GetSlack() const { return capacity_ - count_; }
 ...
 protected:
  ...
  FUN_NO_INLINE void ResizeShrink(int32 bytes_per_element) {
    const int32 new_capacity =
        this->CalculateSlackShrink(count_, capacity_, bytes_per_element);
  ...
  FUN_NO_INLINE void ResizeTo(int32 new_capacity, int32 bytes_per_element) {
    if (new_capacity > 0) {
      new_capacity =
          this->CalculateSlackReserve(new_capacity, bytes_per_element);
    }
...
template <>
struct IsZeroConstructible<UntypedArray> {
  enum { Value = true };
};
...
class ReversePredicateWrapper {
 public:
  ReversePredicateWrapper(const Predicate& pred) : pred_(pred) {}

  FUN_ALWAYS_INLINE bool operator()(ElementType& a, ElementType& b) const {
    return pred_(b, a);
  }

  FUN_ALWAYS_INLINE bool operator()(const ElementType& a,
...
struct CanMoveTArrayPointersBetweenArrayTypes {
  ...
  enum {
    Value =
        IsSame<FromAllocatorType,
               ToAllocatorType>::Value &&  // Allocators must be equal
        ContainerTraits<FromArrayType>::
            MoveWillEmptyContainer &&  // A move must be allowed to leave the
                                       // source array empty
        (IsSame<ToElementType,
                FromElementType>::Value ||  // The element type of the container
                                            // must be the same, or...
         IsBitwiseConstructible<ToElementType, FromElementType>::
             Value  // ... the element type of the source container must be
                    // bitwise constructible from the element type in the
...
class Array {
 ...
 public:
  ...
  AGRESSIVE_ARRAY_FORCEINLINE Array& operator=(
      std::initializer_list<ElementType> init_list) {
 ...
 private:
  ...
  static FUN_ALWAYS_INLINE
      typename EnableIf<Array_internal::CanMoveTArrayPointersBetweenArrayTypes<
          FromArrayType, ToArrayType>::Value>::Type
      MoveOrCopy(ToArrayType& to_array, FromArrayType& from_array,
                 int32 prev_capacity) {
  ...
  static FUN_ALWAYS_INLINE
      ...
      MoveOrCopy(ToArrayType& to_array, FromArrayType& from_array,
                 int32 prev_capacity) {
    to_array.CopyToEmpty(from_array.ConstData(), from_array.Count(),
                         prev_capacity, 0);
  }
  ...
  static FUN_ALWAYS_INLINE
      typename EnableIf<Array_internal::CanMoveTArrayPointersBetweenArrayTypes<
          FromArrayType, ToArrayType>::Value>::Type
      MoveOrCopyWithSlack(ToArrayType& to_array, FromArrayType& from_array,
                          int32 prev_capacity, int32 extra_slack) {
  ...
  static FUN_ALWAYS_INLINE
      ...
      MoveOrCopyWithSlack(ToArrayType& to_array, FromArrayType& from_array,
                          int32 prev_capacity, int32 extra_slack) {
    to_array.CopyToEmpty(from_array.ConstData(), from_array.Count(),
                         prev_capacity, extra_slack);
  }

 public:
  FUN_ALWAYS_INLINE Array(Array&& other) { MoveOrCopy(*this, other, 0); }

  template <typename OtherElementType, typename OtherAllocator>
  FUN_ALWAYS_INLINE explicit Array(
      Array<OtherElementType, OtherAllocator>&& other) {
    MoveOrCopy(*this, other, 0);
  }

  template <typename OtherElementType>
  Array(Array<OtherElementType, Allocator>&& other, int32 extra_slack) {
    // We don't implement move semantics for general OtherAllocators, as there's
    // no way to tell if they're compatible with the current one.  Probably
    // going to be a pretty rare requirement anyway.
  ...
  FUN_ALWAYS_INLINE uint32 GetTypeSize() const { return sizeof(ElementType); }
  ...
  FUN_ALWAYS_INLINE void RangeCheck(int32 index) const {
    ...
    if (Allocator::RequireRangeCheck) {
      fun_check_msg(index >= 0 && index < count_,
                    "array index out of bounds: %i from an array of size {0}",
                    index, count_);  // & for one branch
    }
  }
  ...
  FUN_ALWAYS_INLINE int32 Count() const { return count_; }

  FUN_ALWAYS_INLINE bool IsEmpty() const { return count_ == 0; }

  FUN_ALWAYS_INLINE int32 Capacity() const { return capacity_; }
  ...
  FUN_ALWAYS_INLINE const ElementType& SafeAt(
      int32 index, const ElementType& default_value) const {
  ...
  void Push(ElementType&& item) { Add(MoveTemp(item)); }

  void Push(const ElementType& item) { Add(item); }
  ...
  const ElementType& Top() const { return Last(); }
  ...
  void PushBack(const ElementType& item) { Add(item); }
  void PushBack(ElementType&& item) { Add(MoveTemp(item)); }
  void PushFront(const ElementType& item) { Insert(0, item); }
  void PushFront(ElementType&& item) { Insert(0, MoveTemp(item)); }
  ...
  int32 Find(const ElementType& item) const {
    ...
    for (const ElementType* __restrict it = start,
                                       *__restrict data_end = start + count_;
         it != it; ++it) {
      if (*it == item) {
  ...
  int32 FindLastIf(const Predicate& pred, int32 starting_index) const {
    ...
    for (const ElementType* __restrict start = ConstData(),
                                       *__restrict it = start + starting_index;
         it != start;) {
      --it;
      if (pred(*it)) {
        return static_cast<int32>(it - start);
  ...
  ElementType* FindByKey(const KeyType& key) {
    for (ElementType* __restrict it = MutableData(),
                                 *__restrict data_end = it + count_;
         it != data_end; ++it) {
      if (*it == key) {
        return it;
      }
  ...
  Array<ElementType> FilterIf(const Predicate& pred) const {
    ...
    for (const ElementType* __restrict it = ConstData(),
                                       *__restrict data_end = it + count_;
         it != data_end; ++it) {
      if (pred(*it)) {
  ...
  bool operator==(const Array& other) const {
    ...
    return count == other.Count() &&
           CompareItems(ConstData(), other.ConstData(), count);
  }
  ...
  friend Archive& operator&(Archive& ar, Array& array) {
    ...
    if (sizeof(ElementType) == 1) {
      ...
      if (ar.IsLoading()) {
        ...
        array.allocator_.ResizeAllocation(0, array.capacity_,
                                          sizeof(ElementType));
      }
  ...
  //  /**
  //  Bulk serialize array as a single memory blob when loading. Uses regular
  //  serialization code for saving and doesn't serialize at all otherwise (e.g.
  //  transient, garbage collection, ...).
  //
  //  Requirements:
  //    - ElementType's & operator needs to serialize ALL member variables in
  //    the SAME order they are layed out in memory.
  //    - ElementType's & operator can NOT perform any fixup operations. This
  //    limitation can be lifted by manually copying
  //      the code after the BulkSerialize call.
  //    - ElementType can NOT contain any member variables requiring constructor
  //    calls or pointers
  //    - sizeof(ElementType) must be equal to the sum of sizes of it's member
  //    variables.
  //         - e.g. use pragma pack (push, 1)/ (pop) to ensure alignment
  //         - match up uint8/ WORDs so everything always end up being properly
  //         aligned
  //    - Code can not rely on serialization of ElementType if neither
  //    ArIsLoading nor ArIsSaving is true.
  //    - Can only be called platforms that either have the same endianness as
  //    the one the content was saved with
  //      or had the endian conversion occur in a cooking process like e.g. for
  //      consoles.
  //
  //  Notes:
  ...
  //
  //  IMPORTANT:
  //    - This is Overridden in XeD3dResourceArray.h Please make certain changes
  //    are propogated accordingly
  //
  //  \param ar - Archive to bulk serialize this Array to/from
  //  */
  //  void BulkSerialize(Archive& ar, bool bForcePerElementSerialization =
  //  false)
  //  {
  ...
  //      || (ar.IsSaving()     // if we are saving, we always do the ordinary
  //      serialize as a way to make sure it matches up with bulk serialization
  //      && !ar.IsCooking()      // but cooking and transacting is performance
  //      critical, so we skip that
  //      && !ar.IsTransacting())
  //      || ar.IsByteSwapping()    // if we are byteswapping, we need to do
  //      that per-element ) { ar & *this;
  //    }
  //    else {
  //      CountBytes(ar);
  //      if (ar.IsLoading()) {
  //        // Basic sanity checking to ensure that sizes match.
  //        fun_check_msg(SerializedElementSize == 0 || SerializedElementSize ==
  //        ElementSize, TEXT("Expected %i, Got: %i"), ElementSize,
  //        SerializedElementSize);
  //        // Serialize the number of elements, block allocate the right amount
  //        of memory and deserialize
  //        // the data as a giant memory blob in a single call to Serialize.
  //        Please see the function header
  ...
  void CountBytes(Archive& ar) {
    ar.CountBytes(count_ * sizeof(ElementType),
                  capacity_ * sizeof(ElementType));
  }
  ...
  template <typename OtherAllocator>
  int32 Insert(const Array<ElementType, OtherAllocator>& items,
               const int32 index) {
  ...
  FUN_ALWAYS_INLINE void CheckAddress(const ElementType* addr) const {
    fun_check_msg(addr < ConstData() || addr >= (ConstData() + capacity_),
                  "Attempting to add a container element (0x%08x) which "
                  "already comes from the container (0x%08x, capacity_: %d)!",
                  addr, ConstData(), capacity_);
  }
  ...
  void RemoveAt(int32 index, int32 count = 1, bool allow_shrinking = true) {
    ...
    if (move_count > 0) {
      UnsafeMemory::Memmove(
          (uint8*)allocator_.GetAllocation() + (index) * sizeof(ElementType),
          (uint8*)allocator_.GetAllocation() +
              (index + count) * sizeof(ElementType),
  ...
  void RemoveAtSwap(int32 index, int32 count = 1, bool allow_shrinking = true) {
    ...
    // Replace the elements in the hole created by the removal with elements
    // from the end of the array, so the range of indices used by the array is
    // contiguous.
    const int32 elements_in_hole = count;
    const int32 elements_after_hole = count_ - (index + count);
    const int32 elements_to_move_into_hole =
        MathBase::Min(elements_in_hole, elements_after_hole);
    if (elements_to_move_into_hole) {
      UnsafeMemory::Memcpy(
          (uint8*)allocator_.GetAllocation() + (index) * sizeof(ElementType),
          (uint8*)allocator_.GetAllocation() +
              (count_ - elements_to_move_into_hole) * sizeof(ElementType),
  ...
  void Resize(int32 new_count, bool allow_shrinking = true) {
    if (new_count > Count()) {
      ...
      DefaultConstructItems<ElementType>(
          (uint8*)allocator_.GetAllocation() + index * sizeof(ElementType),
          diff);
  ...
  template <typename OtherElementType, typename OtherAllocator>
  FUN_ALWAYS_INLINE void Append(
      const Array<OtherElementType, OtherAllocator>& src) {
    ...
    ConstructItems<ElementType>(MutableData() + count_, src.ConstData(),
                                src_count);
  ...
  FUN_ALWAYS_INLINE void Append(Array<OtherElementType, OtherAllocator>&& src) {
    ...
    RelocateConstructItems<ElementType>(MutableData() + count_, src.ConstData(),
                                        src_count);
    src.count_ = 0;
  ...
  FUN_ALWAYS_INLINE void Append(std::initializer_list<ElementType> init_list) {
    ...
    const int32 index = AddUninitialized(count);
    ConstructItems<ElementType>(MutableData() + index, init_list.begin(),
                                count);
  }
  ...
  FUN_ALWAYS_INLINE Array& operator+=(
      std::initializer_list<ElementType> init_list) {
    Append(init_list);
    return *this;
  }
 ...
 public:
  ...
  int32 RemoveSingle(const ElementType& item) {
    ...
    RelocateConstructItems<ElementType>(remove_ptr, remove_ptr + 1,
                                        count_ - (index + 1));
  ...
  int32 RemoveAll(const Predicate& pred) {
    ...
    int32 read_index = 0;
    bool not_match =
        !pred(MutableData()[read_index]);  // use a ! to guarantee it can't be
                                           // anything other than zero or one
    do {
      int32 run_start_index = read_index++;
      while (read_index < original_count &&
             not_match == !pred(MutableData()[read_index])) {
        read_index++;
      }
      ...
      if (not_match) {
        ...
        if (write_index != run_start_index) {
          UnsafeMemory::Memmove(&MutableData()[write_index],
                                &MutableData()[run_start_index],
                                sizeof(ElementType) * run_length);
        }
  ...
  // bool FindItemByClass(SearchType** item = nullptr, int32* ItemIndex =
  // nullptr, int32 starting_index = 0) const
  //{
  ...
  typedef IndexedContainerIterator<const Array, const ElementType, int32>
      ConstIterator;
 ...
 private:
...
#if FUN_ARRAY_RANGED_FOR_CHECKS
  FUN_ALWAYS_INLINE friend RangedForIteratorType begin(Array& array) {
    return RangedForIteratorType(array.count_, array.MutableData());
  }
  FUN_ALWAYS_INLINE friend RangedForConstIteratorType begin(
      const Array& array) {
    return RangedForConstIteratorType(array.count_, array.ConstData());
  }
  FUN_ALWAYS_INLINE friend RangedForIteratorType end(Array& array) {
    return RangedForIteratorType(array.count_,
                                 array.MutableData() + array.Count());
  }
  FUN_ALWAYS_INLINE friend RangedForConstIteratorType end(const Array& array) {
    return RangedForConstIteratorType(array.count_,
                                      array.ConstData() + array.Count());
  }
#else
  FUN_ALWAYS_INLINE friend RangedForIteratorType begin(Array& array) {
  ...
  FUN_ALWAYS_INLINE friend RangedForConstIteratorType end(const Array& array) {
    return array.ConstData() + array.Count();
  }
#endif
 ...
 private:
  FUN_NO_INLINE void ResizeGrow(int32 old_count) {
    capacity_ =
        allocator_.CalculateSlackGrow(count_, capacity_, sizeof(ElementType));
    allocator_.ResizeAllocation(old_count, capacity_, sizeof(ElementType));
  }

  FUN_NO_INLINE void ResizeShrink() {
    const int32 new_capacity =
        allocator_.CalculateSlackShrink(count_, capacity_, sizeof(ElementType));
    if (new_capacity != capacity_) {
      capacity_ = new_capacity;
      fun_check(capacity_ >= count_);
      allocator_.ResizeAllocation(count_, capacity_, sizeof(ElementType));
  ...
  FUN_NO_INLINE void ResizeTo(int32 new_capacity) {
    if (new_capacity) {
      new_capacity =
          allocator_.CalculateSlackReserve(new_capacity, sizeof(ElementType));
    }
  ...
  template <typename OtherElementType>
  FUN_ALWAYS_INLINE void CopyToEmpty(const OtherElementType* other_data,
                                     int32 other_count, int32 prev_capacity,
                                     int32 extra_slack) {
 ...
 public:
  ...
  void Heapify() { Heapify(Less<ElementType>()); }
  ...
  template <typename Predicate>
  void HeapPop(ElementType& out_item, const Predicate& pred,
  ...
  const ElementType& HeapTop() const { return (*this)[0]; }

  ElementType& HeapTop() { return (*this)[0]; }

  template <typename Predicate>
  void HeapRemoveAt(int32 index, const Predicate& pred,
                    bool allow_shrinking = true) {
    RemoveAtSwap(index, 1, allow_shrinking);
  ...
  void HeapSort() { HeapSort(Less<ElementType>()); }
 ...
 private:
  template <typename Predicate>
  FUN_ALWAYS_INLINE void SiftDown(int32 index, const int32 Count,
    ...
    while (!HeapIsLeaf(index, Count)) {
      ...
      if (right_child_index < Count) {
        min_child_index = pred(heap[left_child_index], heap[right_child_index])
                              ? left_child_index
                              : right_child_index;
      }
  ...
  template <typename Predicate>
  FUN_ALWAYS_INLINE int32 SiftUp(int32 root_index, int32 node_index,
                                 const Predicate& pred) {
    ElementType* heap = MutableData();
...
template <typename ElementType, typename Allocator>
struct IsTArray<Array<ElementType, Allocator>> {
  enum { Value = true };
};
template <typename ElementType, typename Allocator>
struct IsTArray<const Array<ElementType, Allocator>> {
  enum { Value = true };
};
template <typename ElementType, typename Allocator>
struct IsTArray<volatile Array<ElementType, Allocator>> {
  enum { Value = true };
};
template <typename ElementType, typename Allocator>
struct IsTArray<const volatile Array<ElementType, Allocator>> {
  enum { Value = true };
};
...
struct ContainerTraits<MruArray<ElementType, Allocator>>
  ...
  enum {
    MoveWillEmptyContainer = ContainerTraitsBase<typename MruArray<
        ElementType, Allocator>::BaseType>::MoveWillEmptyContainer
  };
};
...
class IndirectArray {
 public:
  ...
  FUN_ALWAYS_INLINE int32 Count() const { return array_.Count(); }
  ...
  uint32 GetTypeSize() const { return sizeof(ElementType*); }
  ...
  FUN_ALWAYS_INLINE const ElementType& Last(
      int32 index_from_the_end = 0) const {
    return *(ElementType*)array_.Last(index_from_the_end);
  }

  FUN_ALWAYS_INLINE void Shrink() { array_.Shrink(); }
  ...
  void CountBytes(Archive& ar) { array_.CountBytes(ar); }
  ...
  void Swap(int32 index1, int32 index2) { array_.Swap(index1, index2); }
  ...
  FUN_ALWAYS_INLINE int32 Add(ElementType* item) { return array_.Add(item); }
  ...
  uint32 GetAllocatedSize() const {
    return array_.Capacity() * sizeof(ElementType*) +
           array_.Count() * sizeof(ElementType);
  }

  // Iterators
  using Iterator = IndexedContainerIterator<IndirectArray, ElementType, int32>;
  using ConstIterator =
      IndexedContainerIterator<const IndirectArray, const ElementType, int32>;
 ...
 private:
  ...
  // DO NOT USE DIRECTLY
  // STL-like iterators to enable range-based for loop support.
  FUN_ALWAYS_INLINE friend DereferencingIterator<
      ElementType, typename InternalArrayType::RangedForIteratorType>
  begin(IndirectArray& array) {
    return DereferencingIterator<
        ElementType, typename InternalArrayType::RangedForIteratorType>(
        begin(array.array_));
  }
  FUN_ALWAYS_INLINE friend DereferencingIterator<
      const ElementType, typename InternalArrayType::RangedForConstIteratorType>
  begin(const IndirectArray& array) {
    return DereferencingIterator<
        const ElementType,
        typename InternalArrayType::RangedForConstIteratorType>(
        begin(array.array_));
  }
  FUN_ALWAYS_INLINE friend DereferencingIterator<
      ElementType, typename InternalArrayType::RangedForIteratorType>
  end(IndirectArray& array) {
    return DereferencingIterator<
        ElementType, typename InternalArrayType::RangedForIteratorType>(
        end(array.array_));
  }
...
struct ContainerTraits<IndirectArray<ElementType, Allocator>>
  ...
  enum {
    MoveWillEmptyContainer = ContainerTraitsBase<typename IndirectArray<
        ElementType, Allocator>::InternalArrayType>::MoveWillEmptyContainer
  };
};
...
void* operator new(size_t size,
                   fun::IndirectArray<ElementType, Allocator>& array) {
  using namespace fun;
  fun_check(size == sizeof(ElementType));
...
//
///**
// * NOTE: Right now, you can't use a custom allocation policy with
// transactional arrays. If
// * you need to do it, you will have to fix up FTransaction::CObjectRecord to
// use the correct Array<Allocator>.
// */
// template <typename ElementType>
...
//
//    if (g_undo) {
//      g_undo->SaveArray(owner_, (UntypedArray*)this, index, count, 1,
//      sizeof(ElementType), DefaultConstructItem, SerializeItem, DestructItem);
//    }
//    return index;
//  }
//
...
//  {
//    if (g_undo) {
//      g_undo->SaveArray(owner_, (UntypedArray*)this, 0, this->count_, -1,
//      sizeof(ElementType), DefaultConstructItem, SerializeItem, DestructItem);
//    }
//
...
//    const int32 index = AddUninitialized(count);
//
//    UnsafeMemory::Memzero(this->MutableData() + index,
//    count*sizeof(ElementType)); return index;
//  }
//
//  int32 AddUnique(const ElementType& item)
//  {
...
//  {
//    if (g_undo) {
//      g_undo->SaveArray(owner_, (UntypedArray*)this, index, 1, 0,
//      sizeof(ElementType), DefaultConstructItem, SerializeItem, DestructItem);
//    }
//  }
//
...
//  {
//    if (g_undo) {
//      g_undo->SaveArray(owner_, (UntypedArray*)this, 0, this->Count(), 0,
//      sizeof(ElementType), DefaultConstructItem, SerializeItem, DestructItem);
//    }
//  }
//
...
//
// template <typename ElementType>
// struct ContainerTraits<TransArray<ElementType>> : public
// ContainerTraitsBase<TransArray<ElementType>>
//{
//  enum { MoveWillEmptyContainer = ContainerTraitsBase<typename
//  TransArray<ElementType>::BaseType>::MoveWillEmptyContainer };
//};
//
// template <typename ElementType> void* operator new(size_t size,
// TransArray<ElementType>& array)
//{
//  fun_check(size == sizeof(ElementType));
...
//
// template <typename ElementType> void* operator new(size_t size,
// TransArray<ElementType>& array, int32 index)
//{
//  fun_check(size == sizeof(ElementType));
//  array.Insert(index);
//  return &array[index];
