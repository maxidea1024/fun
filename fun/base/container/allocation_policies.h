...
ReferencedType* IfPThenAElseB(Predicate pred, ReferencedType* a, ReferencedType* b);

//*NEW*
#if TRACK_ARRAY_SLACK
FUN_BASE_API void TrackSlack( int32 element_count,
                              int32 allocated_element_count,
                              size_t bytes_per_element,
                              int32 rv);
#endif

FUN_ALWAYS_INLINE int32 DefaultCalculateSlackShrink(
        int32 element_count,
        int32 allocated_element_count,
        size_t bytes_per_element,
        bool allow_quantize,
  ...
  const size_t current_slack_bytes = (allocated_element_count - element_count) * bytes_per_element;
  const bool too_many_slack_bytes = current_slack_bytes >= 16384;
  const bool too_many_slack_elements = 3 * element_count < 2 * allocated_element_count;
  if ((too_many_slack_bytes || too_many_slack_elements) && (current_slack_elements > 64 || !element_count)) { //  hard coded 64 :-(
    rv = element_count;
    if (rv > 0) {
      if (allow_quantize) {
        //TODO size_t / int32 어떤 타입으로 다루는게 좋을지...
        rv = (int32)(UnsafeMemory::QuantizeSize(rv * bytes_per_element, alignment) / bytes_per_element);
      }
    }
  } else {
...
#if TRACK_ARRAY_SLACK
  void TrackSlack(element_count, allocated_element_count, bytes_per_element, rv);
#endif
  return rv;
}

FUN_ALWAYS_INLINE int32 DefaultCalculateSlackGrow(
          int32 element_count,
          int32 allocated_element_count,
          size_t bytes_per_element,
          bool allow_quantize,
  ...
  if (allow_quantize) {
    //TODO size_t / int32 어떤 타입으로 다루는게 좋을지...
    rv = (int32)(UnsafeMemory::QuantizeSize(grow * bytes_per_element, alignment) / bytes_per_element);
  } else {
    //TODO size_t / int32 어떤 타입으로 다루는게 좋을지...
    rv = (int32)grow;
  }
...
FUN_ALWAYS_INLINE int32 DefaultCalculateSlackReserve(
  ...
  if (allow_quantize) {
    //TODO size_t / int32 어떤 타입으로 다루는게 좋을지...
    rv = (int32)(UnsafeMemory::QuantizeSize(size_t(rv) * size_t(bytes_per_element), alignment) / bytes_per_element);
    // element_count and MaxElements are stored in 32 bit signed integers
    // so we must be careful not to overflow here.
    if (element_count > rv) {
      rv = int32_MAX;
...
class ContainerAllocatorInterface {
 public:
  ...
  struct ForElementType {
     ...
     *
     * \param previous_element_count - The number of elements that were stored in the previous allocation.
     * \param element_count - The number of elements to allocate space for.
     * \param bytes_per_element - The number of bytes/element.
     */
     ...
     *
     * \param element_count - The number of elements to allocate space for.
     * \param current_slack_element_count - The current number of elements allocated.
     * \param bytes_per_element - The number of bytes/element.
     */
    int32 CalculateSlack(int32 element_count, int32 current_slack_element_count, size_t bytes_per_element) const;
     ...
     * \param bytes_per_element - The number of bytes/element.
     */
    int32 CalculateSlackShrink(int32 element_count, int32 current_slack_element_count, size_t bytes_per_element) const;
     ...
     * \param bytes_per_element - The number of bytes/element.
     */
    int32 CalculateSlackGrow(int32 element_count, int32 current_slack_element_count, size_t bytes_per_element) const;
...
class AlignedHeapAllocator {
 public:
  ...
  struct ForAnyElementType {
    ...
    void ResizeAllocation(int32 previous_element_count,
                          int32 element_count,
                          size_t bytes_per_element) {
...
#if FUN_PLATFORM_HAS_UMA
        data_ = (UntypedContainerElement*)UnsafeMemory::GPURealloc(data_, element_count * bytes_per_element, Alignment);
#else
        //fun_check_dbg(((uint64)element_count*(uint64)ElementTypeInfo.GetSize() < (uint64)INT_MAX));
        data_ = (UntypedContainerElement*)UnsafeMemory::Realloc(data_, element_count * bytes_per_element, Alignment);
#endif
      }
    }

    FUN_ALWAYS_INLINE int32 CalculateSlackReserve(
              int32 element_count, int32 bytes_per_element) const {
      return DefaultCalculateSlackReserve(element_count, bytes_per_element, true, Alignment);
    }

    FUN_ALWAYS_INLINE int32 CalculateSlackShrink( int32 element_count,
                                                  int32 allocated_element_count,
                                                  int32 bytes_per_element) const {
      return DefaultCalculateSlackShrink(element_count, allocated_element_count, bytes_per_element, true, Alignment);
    }

    FUN_ALWAYS_INLINE int32 CalculateSlackGrow( int32 element_count,
                                                int32 allocated_element_count,
                                                int32 bytes_per_element) const {
      return DefaultCalculateSlackGrow(element_count, allocated_element_count, bytes_per_element, true, Alignment);
    }
 ...
 public:
  ...
  struct FUN_BASE_API ForAnyElementType {
    ...
    void ResizeAllocation(int32 previous_element_count,
      ...
      if (data_ || element_count > 0) {
        //fun_check_dbg(((uint64)element_count*(uint64)ElementTypeInfo.GetSize() < (uint64)INT_MAX));
        data_ = (UntypedContainerElement*)UnsafeMemory::Realloc(data_, element_count*bytes_per_element);
      }
    }

    FUN_ALWAYS_INLINE int32 CalculateSlackReserve(int32 element_count,
                                                  int32 bytes_per_element) const {
      return DefaultCalculateSlackReserve(element_count, bytes_per_element, true);
    }

    FUN_ALWAYS_INLINE int32 CalculateSlackShrink( int32 element_count,
                                                  int32 allocated_element_count,
                                                  int32 bytes_per_element) const {
      return DefaultCalculateSlackShrink(element_count, allocated_element_count, bytes_per_element, true);
    }

    FUN_ALWAYS_INLINE int32 CalculateSlackGrow( int32 element_count,
                                                int32 allocated_element_count,
                                                int32 bytes_per_element) const {
      return DefaultCalculateSlackGrow(element_count, allocated_element_count, bytes_per_element, true);
    }
...
template <>
struct AllocatorTraits<HeapAllocator>
    : AllocatorTraitsBase<HeapAllocator> {
  enum { SupportsMove = true };
  enum { IsZeroConstruct = true };
};
...
/**
 * The FUN_ALWAYS_INLINE allocation policy allocates up to a specified number of elements in the same allocation as the container.
  ...
  struct ForElementType {
    ...
    FUN_ALWAYS_INLINE void MoveToEmpty(ForElementType& other) {
      ...
      if (!other.secondary_allocator_.GetAllocation()) {
        // Relocate objects from other FUN_ALWAYS_INLINE storage only if it was stored FUN_ALWAYS_INLINE in other
        RelocateConstructItems<ElementType>((void*)inline_data_, other.GetInlineElements(), NumInlineElements);
      }
    ...
    // ContainerAllocatorInterface
    FUN_ALWAYS_INLINE ElementType* GetAllocation() const {
      return IfAThenAElseB<ElementType>(secondary_allocator_.GetAllocation(), GetInlineElements());
    }

    void ResizeAllocation(int32 previous_element_count,
                          int32 element_count,
                          size_t bytes_per_element) {
      // Check if the new allocation will fit in the FUN_ALWAYS_INLINE data area.
      if (element_count <= NumInlineElements) {
        // If the old allocation wasn't in the FUN_ALWAYS_INLINE data area, relocate it into the FUN_ALWAYS_INLINE data area.
        if (secondary_allocator_.GetAllocation()) {
          RelocateConstructItems<ElementType>((void*)inline_data_, (ElementType*)secondary_allocator_.GetAllocation(), previous_element_count);
        ...
        if (secondary_allocator_.GetAllocation() == nullptr) {
          ...
          secondary_allocator_.ResizeAllocation(0, element_count, bytes_per_element);

          // Move the data out of the FUN_ALWAYS_INLINE data area into the new allocation.
          RelocateConstructItems<ElementType>((void*)secondary_allocator_.GetAllocation(), GetInlineElements(), previous_element_count);
        } else {
          // Reallocate the indirect data for the new size.
          secondary_allocator_.ResizeAllocation(previous_element_count, element_count, bytes_per_element);
        }
      }
    }
    ...
    FUN_ALWAYS_INLINE int32 CalculateSlackReserve(int32 element_count,
                                                  int32 bytes_per_element) const {
      // If the elements use less space than the FUN_ALWAYS_INLINE allocation, only use the FUN_ALWAYS_INLINE allocation as slack.
      return element_count <= NumInlineElements ?
                    NumInlineElements :
                              secondary_allocator_.CalculateSlackReserve(element_count, bytes_per_element);
    }
    ...
    FUN_ALWAYS_INLINE int32 CalculateSlackShrink( int32 element_count,
      ...
      return element_count <= NumInlineElements ?
                    NumInlineElements :
                              secondary_allocator_.CalculateSlackShrink(element_count, allocated_element_count, bytes_per_element);
    }

    FUN_ALWAYS_INLINE int32 CalculateSlackGrow(int32 element_count, int32 allocated_element_count, int32 bytes_per_element) const {
      // If the elements use less space than the FUN_ALWAYS_INLINE allocation, only use the FUN_ALWAYS_INLINE allocation as slack.
      return element_count <= NumInlineElements ?
                    NumInlineElements :
                              secondary_allocator_.CalculateSlackGrow(element_count, allocated_element_count, bytes_per_element);
    }

    size_t GetAllocatedSize(int32 allocated_element_count, int32 bytes_per_element) const {
      return secondary_allocator_.GetAllocatedSize(allocated_element_count, bytes_per_element);
    }
   ...
   private:
     ...
     */
    typename SecondaryAllocator::template ForElementType<ElementType> secondary_allocator_;
...
struct AllocatorTraits<InlineAllocator<NumInlineElements, SecondaryAllocator>>
  : AllocatorTraitsBase<InlineAllocator<NumInlineElements, SecondaryAllocator>> {
...
public:
  ...
  struct ForElementType {
    ...
    void ResizeAllocation(int32 previous_element_count, int32 element_count, size_t bytes_per_element) {
      // Ensure the requested allocation will fit in the FUN_ALWAYS_INLINE data area.
      fun_check(element_count <= NumInlineElements);
    }

    FUN_ALWAYS_INLINE int32 CalculateSlackReserve(int32 element_count, size_t bytes_per_element) const {
      // Ensure the requested allocation will fit in the FUN_ALWAYS_INLINE data area.
      fun_check(element_count <= NumInlineElements);
      return NumInlineElements;
    }

    FUN_ALWAYS_INLINE int32 CalculateSlackShrink(int32 element_count, int32 allocated_element_count, int32 bytes_per_element) const {
      // Ensure the requested allocation will fit in the FUN_ALWAYS_INLINE data area.
...
//
//      // Relocate objects from other FUN_ALWAYS_INLINE storage
//      RelocateConstructItems<ElementType>((void*)ExternalBuffer, other.ExternalBuffer, ExternalBufferLength);
//    }
//
//    // ContainerAllocatorInterface
//    FUN_ALWAYS_INLINE ElementType* GetAllocation() const {
...
//    void ResizeAllocation(int32 previous_element_count, int32 element_count, size_t bytes_per_element) {
//      // Ensure the requested allocation will fit in the FUN_ALWAYS_INLINE data area.
//      fun_check(element_count <= ExternalBufferLength);
//    }
//
//    FUN_ALWAYS_INLINE int32 CalculateSlackReserve(int32 element_count, size_t bytes_per_element) const
//    {
//      // Ensure the requested allocation will fit in the FUN_ALWAYS_INLINE data area.
//      fun_check_dbg(element_count <= ExternalBufferLength);
//      return ExternalBufferLength;
//    }
//
//    FUN_ALWAYS_INLINE int32 CalculateSlackShrink(int32 element_count, int32 allocated_element_count, int32 bytes_per_element) const
//    {
//      // Ensure the requested allocation will fit in the FUN_ALWAYS_INLINE data area.
//      fun_check_dbg(allocated_element_count <= ExternalBufferLength);
//      return ExternalBufferLength;
//    }
//
//    FUN_ALWAYS_INLINE int32 CalculateSlackGrow(int32 element_count, int32 allocated_element_count, int32 bytes_per_element) const
...
//};
//
//struct AllocatorTraits<ExternalBufferAllocator> : AllocatorTraitsBase<ExternalBufferAllocator>
//{
//  enum { SupportsMove = true };
//};
 ...
 */
template <typename _ElementAllocator = DefaultAllocator, typename _BitArrayAllocator = DefaultBitArrayAllocator>
...
/**
 * An FUN_ALWAYS_INLINE sparse array allocator that allows sizing of the FUN_ALWAYS_INLINE allocations for a set number of elements.
 */
template <
...
class SetAllocator {
 public:
   ...
   */
  static FUN_ALWAYS_INLINE uint32 GetNumberOfHashBuckets(uint32 num_hashed_elements) {
    if (num_hashed_elements >= MinNumberOfHashedElements) {
      return MathBase::RoundUpToPowerOfTwo(num_hashed_elements / AverageNumberOfElementsPerHashBucket + BaseNumberOfHashBuckets);
    }
...
/**
 * An FUN_ALWAYS_INLINE set allocator that allows sizing of the FUN_ALWAYS_INLINE allocations for a set number of elements.
 */
template <
    uint32 NumInlineElements,
...
class InlineSetAllocator {
 ...
 public:
   ...
   */
  static FUN_ALWAYS_INLINE uint32 GetNumberOfHashBuckets(uint32 num_hashed_elements) {
    const uint32 NumDesiredHashBuckets = MathBase::RoundUpToPowerOfTwo(num_hashed_elements / AverageNumberOfElementsPerHashBucket);
