...
// 실제로 해싱에 기반한 Map이 아니고, 정렬되어 있는 배열을 토대로
// BinarySearching으로 구현되어 있음.
...
class OrderedMap {
  template <typename OtherKeyType, typename OtherValueType,
            typename OtherArrayAllocator, typename OtherSortPredicate>
 ...
 public:
  ...
  OrderedMap(OrderedMap<KeyType, ValueType, OtherArrayAllocator,
                        SortPredicate>&& other)
   ...
   */
  template <typename OtherArrayAllocator>
  OrderedMap(const OrderedMap<KeyType, ValueType, OtherArrayAllocator,
                              SortPredicate>& other)
      : pairs_(other.pairs_) {}
   ...
   */
  template <typename OtherArrayAllocator>
  OrderedMap& operator=(OrderedMap<KeyType, ValueType, OtherArrayAllocator,
                                   SortPredicate>&& other) {
    pairs_ = MoveTemp(other.pairs_);
    return *this;
  }
  ...
  template <typename OtherArrayAllocator>
  OrderedMap& operator=(
      const OrderedMap<KeyType, ValueType, OtherArrayAllocator, SortPredicate>&
          other) {
    pairs_ = other.pairs_;
    return *this;
  }
   ...
   */
  FUN_ALWAYS_INLINE void Reserve(int32 count) { pairs_.Reserve(count); }
  ...
  ValueType& Emplace(InitKeyType&& key, InitValueType&& value) {
    ...
    new (data_ptr) ElementType(PairInitializer<InitKeyType&&, InitValueType&&>(
  ...
  ValueType& Emplace(InitKeyType&& key) {
    ElementType* data_ptr = AllocateMemoryForEmplace(key);
    new (data_ptr)
        ElementType(KeyInitializer<InitKeyType&&>(Forward<InitKeyType>(key)));
    return data_ptr->value;
  }
  ...
  const KeyType* FindKey(ValueInitType value) const {
    for (typename ElementArrayType::ConstIterator pair_it(pairs_); pair_it;
         ++pair_it) {
  ...
  FUN_ALWAYS_INLINE friend Archive& operator&(Archive& ar, OrderedMap& map) {
    ...
    if (ar.IsLoading()) {
      // We need to resort, in case the sorting is not consistent with what it
      // was before
  ...
  void Append(OrderedMap<KeyType, ValueType, OtherArrayAllocator,
                         OtherSortPredicate>&& other_map) {
   ...
   */
  template <typename OtherArrayAllocator, typename OtherSortPredicate>
  void Append(const OrderedMap<KeyType, ValueType, OtherArrayAllocator,
                               OtherSortPredicate>& other_map) {
    this->Reserve(this->Count() + other_map.Count());
 ...
 private:
  ...
  FUN_ALWAYS_INLINE ElementType* AllocateMemoryForEmplace(InitKeyType&& key) {
    const int32 insert_index =
        algo::LowerBoundBy(pairs_, key, KeyForward(), SortPredicate());
    ...
    if (pairs_.IsValidIndex(insert_index) &&
        !SortPredicate()(key, pairs_[insert_index].key)) {
  ...
  class BaseIterator {
   ...
   private:
    using MapType =
        typename Conditional<IsConst, const OrderedMap, OrderedMap>::Result;
    using ItKeyType =
    ...
    using PairType =
        typename Conditional<IsConst,
                             const typename ElementArrayType::ElementType,
                             typename ElementArrayType::ElementType>::Result;
   ...
   public:
    ...
    FUN_ALWAYS_INLINE explicit operator bool() const { return !!pair_it_; }
    ...
    FUN_ALWAYS_INLINE ItKeyType& Key() const { return pair_it_->key; }

    FUN_ALWAYS_INLINE ItValueType& Value() const { return pair_it_->value; }

    FUN_ALWAYS_INLINE PairType& operator*() const { return *pair_it_; }
 ...
 public:
  ...
  class Iterator : public BaseIterator<false> {
   public:
    ...
    FUN_ALWAYS_INLINE Iterator(
        const typename BaseIterator<false>::PairItType& pair_it)
   ...
   */
  using RangedForIteratorType =
      typename ElementArrayType::RangedForIteratorType;
  using RangedForConstIteratorType =
      typename ElementArrayType::RangedForConstIteratorType;

 private:
  // DO NOT USE DIRECTLY
  // STL-like iterators to enable range-based for loop support.
  FUN_ALWAYS_INLINE friend RangedForIteratorType begin(OrderedMap& map) {
    return begin(map.pairs_);
  }
  FUN_ALWAYS_INLINE friend RangedForConstIteratorType begin(
      const OrderedMap& map) {
    return begin(map.pairs_);
  }
  FUN_ALWAYS_INLINE friend RangedForIteratorType end(OrderedMap& map) {
    return end(map.pairs_);
  }
  FUN_ALWAYS_INLINE friend RangedForConstIteratorType end(
      const OrderedMap& map) {
    return end(map.pairs_);
  }
};

template <typename KeyType, typename ValueType, typename ArrayAllocator,
          typename SortPredicate>
struct ContainerTraits<
    OrderedMap<KeyType, ValueType, ArrayAllocator, SortPredicate>>
    : public ContainerTraitsBase<
          OrderedMap<KeyType, ValueType, ArrayAllocator, SortPredicate>> {
  enum {
    MoveWillEmptyContainer =
        ContainerTraits<typename OrderedMap<KeyType, ValueType, ArrayAllocator,
                                            SortPredicate>::ElementArrayType>::
            MoveWillEmptyContainer
  };
};
