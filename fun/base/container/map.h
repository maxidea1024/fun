...
/**
 * An initializer type for pairs that's passed to the pair set when adding a new
 * pair.
 */
...
/**
 * An initializer type for keys that's passed to the pair set when adding a new
 * key.
 */
...
class KeyInitializer {
 public:
   ...
   * Initialization constructor.
   */
  FUN_ALWAYS_INLINE explicit KeyInitializer(KeyInitType key) : key(key) {}
};
...
class TPair {
 public:
  ...
  FUN_ALWAYS_INLINE TPair(
      const PairInitializer<InitKeyType, InitValueType>& init)
      : key(StaticCast<InitKeyType>(init.key)),
        value(StaticCast<InitValueType>(init.value)) {
    // The seemingly-pointless casts above are to enforce a move (i.e.
    // equivalent to using MoveTemp) when the initializers are themselves rvalue
    // references.
  }
  ...
  FUN_ALWAYS_INLINE explicit TPair(const KeyInitializer<InitKeyType>& init)
      : key(StaticCast<InitKeyType>(init.key)), value() {
    // The seemingly-pointless cast above is to enforce a move (i.e. equivalent
    // to using MoveTemp) when the initializer is itself an rvalue reference.
  }
   ...
   */
  FUN_ALWAYS_INLINE operator PairInitializer<KeyInitType, ValueInitType>()
      const {
...
struct DefaultMapKeyFuncs
  ...
  using ElementInitType =
      const PairInitializer<typename TypeTraits<KeyType>::ConstInitType,
                            typename TypeTraits<ValueType>::ConstInitType>&;
...
/**
 * The base class of maps from keys to values.  Implemented using a Set of
 * key-value pairs with a custom KeyFuncs, with the same O(1) addition, removal,
 * and finding.
 */
...
class MapBase {
  template <typename OtherKeyType, typename OtherValueType,
            typename OtherSetAllocator, typename OtherKeyFuncs>
 ...
 protected:
  ...
  /**
   * Assignment operator for moving elements from a Map with a different
   * SetAllocator
   */
  template <typename OtherSetAllocator>
  MapBase& operator=(
      MapBase<KeyType, ValueType, OtherSetAllocator, KeyFuncs>&& other) {
    pairs_ = MoveTemp(other.pairs_);
    return *this;
  }

  /**
   * Assignment operator for copying elements from a Map with a different
   * SetAllocator
   */
  template <typename OtherSetAllocator>
  MapBase& operator=(
      const MapBase<KeyType, ValueType, OtherSetAllocator, KeyFuncs>& other) {
    pairs_ = other.pairs_;
    return *this;
  }

 public:
  // Legacy comparison operators.  Note that these also test whether the map's
  // key-value pairs were added in the same order!
  ...
  /**
   * Compare this map with another for equality. Does not make any assumptions
   * about key order. NOTE: this might be a candidate for operator== but it was
   * decided to make it an explicit function since it can potentially be quite
   * slow.
   *
   * \param other - The other map to compare against
   *
   * \returns True if both this and other contain the same keys with values that
   * compare ==
   */
  bool OrderIndependentCompareEqual(const MapBase& other) const {
    ...
    // since we know the counts are the same, we can just iterate one map and
    // check for existence in the other
    ...
    // all fields in A match B and A and B's counts are the same (so there can
    // be no fields in B not in A)
    return true;
  }

  /**
   * Removes all elements from the map, potentially leaving space allocated for
   * an expected number of elements about to be added.
   *
   * \param count - The number of elements about to be added to the set.
   */
  FUN_ALWAYS_INLINE void Clear(int32 count = 0) { pairs_.Clear(count); }

  /**
   * Efficiently empties out the map but preserves all allocations and
   * capacities
   */
  FUN_ALWAYS_INLINE void Reset() { pairs_.Reset(); }

  /**
   * Shrinks the pair set to avoid slack.
   */
  FUN_ALWAYS_INLINE void Shrink() { pairs_.Shrink(); }

  /**
   * Compacts the pair set to remove holes
   */
  FUN_ALWAYS_INLINE void Compact() { pairs_.Compact(); }

  /**
   * Compacts the pair set to remove holes.
   * Does not change the iteration order of the elements.
   */
  FUN_ALWAYS_INLINE void CompactStable() { pairs_.CompactStable(); }
   ...
   */
  FUN_ALWAYS_INLINE int32 Count() const { return pairs_.Count(); }

  FUN_ALWAYS_INLINE bool IsEmpty() const { return pairs_.IsEmpty(); }
   ...
   */
  FUN_ALWAYS_INLINE void CountBytes(Archive& ar) { pairs_.CountBytes(ar); }
   ...
   *
   * \return A reference to the value as stored in the map.  The reference is
   * only valid until the next change to any key in the map.
   */
   ...
   * only valid until the next change to any key in the map.
   */
  FUN_ALWAYS_INLINE ValueType& Add(const KeyType& key) { return Emplace(key); }
  ...
  ValueType& Emplace(InitKeyType&& key, InitValueType&& value) {
    const SetElementId pair_id =
        pairs_.Emplace(PairInitializer<InitKeyType&&, InitValueType&&>(
            Forward<InitKeyType>(key), Forward<InitValueType>(value)));
  ...
  ValueType& Emplace(InitKeyType&& key) {
    const SetElementId pair_id = pairs_.Emplace(
        KeyInitializer<InitKeyType&&>(Forward<InitKeyType>(key)));
    return pairs_[pair_id].value;
  }
  ...
  /**
   * Returns the key associated with the specified value.  The time taken is
   * O(N) in the number of pairs.
   *
   * \param value - The value to search for
   *
   * \return A pointer to the key associated with the specified value, or
   * nullptr if the value isn't contained in this map.  The pointer is only
   * valid until the next change to any key in the map.
   */
  const KeyType* FindKey(ValueInitType value) const {
    for (typename PairSetType::ConstIterator pair_it(pairs_); pair_it;
         ++pair_it) {
   ...
   *
   * \return  A pointer to the value associated with the specified key, or
   * nullptr if the key isn't contained in this map.  The pointer is only valid
   * until the next change to any key in the map.
   */
 ...
 public:
   ...
   * \param key - The key to search for.
   *
   * \return The value associated with the specified key, or triggers an
   * assertion if the key does not exist.
   */
  FUN_ALWAYS_INLINE const ValueType& FindChecked(
      KeyConstPointerType key) const {
   ...
   * for the ValueType if the key isn't contained in this map.
   */
  ...
  FUN_ALWAYS_INLINE bool TryGetValue(KeyConstPointerType key,
 ...
 protected:
  ...
  class BaseIterator {
   public:
    using PairItType =
        typename Conditional<IsConst, typename PairSetType::ConstIterator,
                             typename PairSetType::Iterator>::Result;

   private:
    using MapType =
        typename Conditional<IsConst, const MapBase, MapBase>::Result;
    using ItKeyType =
        typename Conditional<IsConst, const KeyType, KeyType>::Result;
    using ItValueType =
        typename Conditional<IsConst, const ValueType, ValueType>::Result;
    using PairType =
        typename Conditional<IsConst, const typename PairSetType::ElementType,
                             typename PairSetType::ElementType>::Result;
   ...
   public:
     ...
     */
    FUN_ALWAYS_INLINE explicit operator bool() const { return !!pair_it; }
    ...
    FUN_ALWAYS_INLINE ItKeyType& Key() const { return pair_it->key; }

    FUN_ALWAYS_INLINE ItValueType& Value() const { return pair_it->value; }

    FUN_ALWAYS_INLINE PairType& operator*() const { return *pair_it; }
  ...
  /**
   * The base type of iterators that iterate over the values associated with a
   * specified key.
   */
  template <bool IsConst>
  class BaseKeyIterator {
   private:
    using SetItType =
        typename Conditional<IsConst, typename PairSetType::ConstKeyIterator,
                             typename PairSetType::KeyIterator>::Result;
    using ItKeyType =
        typename Conditional<IsConst, const KeyType, KeyType>::Result;
    using ItValueType =
        typename Conditional<IsConst, const ValueType, ValueType>::Result;
   ...
   public:
     ...
     * conversion to "bool" returning true if the iterator is valid.
     */
    FUN_ALWAYS_INLINE explicit operator bool() const { return !!set_it_; }
    ...
    FUN_ALWAYS_INLINE ItKeyType& Key() const { return set_it_->key; }

    FUN_ALWAYS_INLINE ItValueType& Value() const { return set_it_->value; }
 ...
 public:
  ...
  class Iterator : public BaseIterator<false> {
   public:
     ...
     * Initialization constructor.
     */
    FUN_ALWAYS_INLINE Iterator(MapBase& map,
                               bool requires_rehash_on_removal = false)
    ...
    FUN_ALWAYS_INLINE Iterator(
        MapBase& map, const typename BaseIterator<false>::PairItType& pair_it)
  ...
  class ConstIterator : public BaseIterator<true> {
   public:
    ...
    FUN_ALWAYS_INLINE ConstIterator(
        const typename BaseIterator<true>::PairItType& pair_it)
  ...
  class ConstKeyIterator : public BaseKeyIterator<true> {
   public:
    FUN_ALWAYS_INLINE ConstKeyIterator(const MapBase& map, KeyInitType key)
        : BaseKeyIterator<true>(
              typename PairSetType::ConstKeyIterator(map.pairs_, key)) {}
  };
  ...
  class KeyIterator : public BaseKeyIterator<false> {
   public:
    FUN_ALWAYS_INLINE KeyIterator(MapBase& map, KeyInitType key)
        : BaseKeyIterator<false>(
              typename PairSetType::KeyIterator(map.pairs_, key)) {}
  ...
  /**
   * Creates an iterator over the values associated with a specified key in a
   * map
   */
  ...
  /**
   * Creates a const iterator over the values associated with a specified key in
   * a map
   */
  FUN_ALWAYS_INLINE ConstKeyIterator
  CreateConstKeyIterator(KeyInitType key) const {
 ...
 private:
  ...
  FUN_ALWAYS_INLINE friend Iterator begin(MapBase& map) {
    return Iterator(map, begin(map.pairs_));
  }
  FUN_ALWAYS_INLINE friend ConstIterator begin(const MapBase& map) {
    return ConstIterator(begin(map.pairs_));
  }
  FUN_ALWAYS_INLINE friend Iterator end(MapBase& map) {
    return Iterator(map, end(map.pairs_));
  }
  FUN_ALWAYS_INLINE friend ConstIterator end(const MapBase& map) {
    return ConstIterator(end(map.pairs_));
  }
};
...
template <typename KeyType, typename ValueType, typename SetAllocator,
          typename KeyFuncs>
class SortableMapBase
    : public MapBase<KeyType, ValueType, SetAllocator, KeyFuncs> {
 ...
 protected:
  ...
  template <typename OtherSetAllocator>
  SortableMapBase(
      SortableMapBase<KeyType, ValueType, OtherSetAllocator, KeyFuncs>&& other)
   ...
   */
  template <typename OtherSetAllocator>
  SortableMapBase(const SortableMapBase<KeyType, ValueType, OtherSetAllocator,
                                        KeyFuncs>& other)
      : BaseType(other) {}
   ...
   */
  template <typename OtherSetAllocator>
  SortableMapBase& operator=(
      SortableMapBase<KeyType, ValueType, OtherSetAllocator, KeyFuncs>&&
          other) {
  ...
  template <typename OtherSetAllocator>
  SortableMapBase& operator=(
      const SortableMapBase<KeyType, ValueType, OtherSetAllocator, KeyFuncs>&
          other) {
    (BaseType&)* this = other;
    return *this;
  }
 ...
 public:
  /**
   * Sorts the pairs array using each pair's key as the sort criteria, then
   * rebuilds the map's hash. Invoked using "MyMapVar.KeySort(Predicate());"
   */
  ...
  /**
   * Sorts the pairs array using each pair's value as the sort criteria, then
   * rebuilds the map's hash. Invoked using "MyMapVar.ValueSort(Predicate());"
   */
  template <typename Predicate>
 ...
 private:
  ...
  class KeyComparisonClass {
   public:
    FUN_ALWAYS_INLINE KeyComparisonClass(const Predicate& pred) : pred_(pred) {}

    FUN_ALWAYS_INLINE bool operator()(
        const typename BaseType::PairType& a,
        const typename BaseType::PairType& b) const {
...
class Map : public SortableMapBase<KeyType, ValueType, SetAllocator, KeyFuncs> {
  ...
  static_assert(
      !KeyFuncs::AllowDuplicateKeys,
      "Map cannot be instantiated with a KeyFuncs which allows duplicate keys");
 ...
 public:
  ...
  template <typename OtherSetAllocator>
  Map& operator=(
      const Map<KeyType, ValueType, OtherSetAllocator, KeyFuncs>& other) {
    (BaseType&)* this = other;
    return *this;
  }
   ...
   *
   * \param key - the key to search for
   * \param out_removed_value - if found, the value that was removed (not
   * modified if the key was not found)
   *
   * \return whether or not the key was found
   */
  FUN_ALWAYS_INLINE bool RemoveAndCopyValue(KeyInitType key,
                                            ValueType& out_removed_value) {
  ...
  template <typename OtherSetAllocator>
  void Append(
      Map<KeyType, ValueType, OtherSetAllocator, KeyFuncs>&& other_map) {
  ...
  template <typename OtherSetAllocator>
  void Append(
      const Map<KeyType, ValueType, OtherSetAllocator, KeyFuncs>& other_map) {
    this->Reserve(this->Count() + other_map.Count());
    for (auto& pair : other_map) {
  ...
  // FUN_ALWAYS_INLINE ValueType& operator[](const KeyType& key) { return
  // this->FindOrAdd(key); }
...
template <typename KeyType, typename ValueType,
          ...
          >
class MultiMap
    : public SortableMapBase<KeyType, ValueType, SetAllocator, KeyFuncs> {
  friend struct ContainerTraits<MultiMap>;

  static_assert(KeyFuncs::AllowDuplicateKeys,
                "MultiMap cannot be instantiated with a KeyFuncs which "
 ...
 public:
  ...
  template <typename OtherSetAllocator>
  MultiMap(
      const MultiMap<KeyType, ValueType, OtherSetAllocator, KeyFuncs>& other)
      : BaseType(other) {}
  ...
  template <typename OtherSetAllocator>
  MultiMap& operator=(
      MultiMap<KeyType, ValueType, OtherSetAllocator, KeyFuncs>&& other) {
    (BaseType&)* this = MoveTemp(other);
    return *this;
  }
   ...
   * \param out_values - Upon return, contains the values associated with the
   * key. \param keep_order - true if the Values array should be in the same
   * order as the map's pairs.
   */
  template <typename ArrayAllocator>
  void MultiFind(KeyInitType key, Array<ValueType, ArrayAllocator>& out_values,
                 bool keep_order = false) const {
    for (typename BaseType::PairSetType::ConstKeyIterator it(BaseType::pairs_,
                                                             key);
         it; ++it) {
   ...
   *
   * \param key - The key to find associated values for.
   * \param out_values - Upon return, contains pointers to the values associated
   * with the key. Pointers are only valid until the next change to any key in
   * the map. \param keep_order - true if the Values array should be in the same
   * order as the map's pairs.
   */
  template <typename ArrayAllocator>
  void MultiFindPointer(KeyInitType key,
                        Array<const ValueType*, ArrayAllocator>& out_values,
                        bool keep_order = false) const {
    for (typename BaseType::PairSetType::ConstKeyIterator it(BaseType::pairs_,
                                                             key);
         it; ++it) {
  ...
  /**
   * Adds a key-value association to the map.  The association doesn't replace
   * any of the key's existing associations. However, if both the key and value
   * match an existing association in the map, no new association is made and
   * the existing association's value is returned.
   *
   ...
   * only valid until the next change to any key in the map.
   */
  FUN_ALWAYS_INLINE ValueType& AddUnique(const KeyType& key,
  ...
  FUN_ALWAYS_INLINE ValueType& AddUnique(KeyType&& key,
                                         const ValueType& value) {
  ...
  ValueType& EmplaceUnique(InitKeyType&& key, InitValueType&& value) {
    ...
    // If there's no existing association with the same key and value, create
    // one.
    return BaseType::Add(Forward<InitKeyType>(key),
                         Forward<InitValueType>(value));
  }
  ...
  int32 Remove(KeyInitType key, ValueInitType value) {
    ...
    for (typename BaseType::PairSetType::KeyIterator it(BaseType::pairs_, key);
         it; ++it) {
  ...
  /**
   * Removes the first association between the specified key and value from the
   * map.
   *
   * \param key - The key part of the pair to remove.
   * \param value - The value part of the pair to remove.
   ...
   *
   * \return If the map contains a matching association, a pointer to the value
   * in the map is returned.  Otherwise nullptr is returned. The pointer is only
   * valid as long as the map isn't changed.
   */
  FUN_ALWAYS_INLINE const ValueType* FindPair(KeyInitType key,
  ...
  /**
   * Returns the number of values within this map associated with the specified
   * key
   */
  ...
  /**
   * Since we implement an overloaded Count() function in MultiMap, we need to
   * reimplement MapBase::Count to make it visible.
   */
  FUN_ALWAYS_INLINE int32 Count() const { return BaseType::Count(); }
};
...
class UntypedMap {
 public:
  static UntypedMapLayout GetUntypedLayout(int32 key_size, int32 key_alignment,
                                           int32 value_size,
                                           int32 value_alignment) {
    ...
    result.set_layout = UntypedSet::GetUntypedLayout(
        pair_struct.GetSize(), pair_struct.GetAlignment());
    return result;
  }

  UntypedMap() {}

  bool IsValidIndex(int32 index) const { return pairs_.IsValidIndex(index); }

  int32 Count() const { return pairs_.Count(); }

  int32 GetMaxIndex() const { return pairs_.GetMaxIndex(); }
  ...
  void Rehash(const UntypedMapLayout& layout,
              TFunctionRef<uint32(const void*)> get_key_hash) {
 ...
 private:
  ...
  static void CheckConstraints() {
    ...
    static_assert(sizeof(RawType) == sizeof(RealType),
                  "UntypedMap's size doesn't match Map");
    static_assert(alignof(RawType) == alignof(RealType),
                  "UntypedMap's alignment doesn't match Map");

    // Check member sizes
    static_assert(
        sizeof(DeclVal<RawType>().pairs_) == sizeof(DeclVal<RealType>().pairs_),
        "UntypedMap's pairs_ member size does not match Map's");

    // Check member offsets
    static_assert(offsetof(RawType, pairs_) == offsetof(RealType, pairs_),
                  "UntypedMap's pairs_ member offset does not match Map's");
  }

 public:
  // These should really be private, because they shouldn't be called, but
  // there's a bunch of code that needs to be fixed first.
  UntypedMap(const UntypedMap&) = delete;
  UntypedMap& operator=(const UntypedMap&) = delete;
};

template <>
struct IsZeroConstructible<UntypedMap> {
  enum { Value = true };
};
...
struct ContainerTraits<Map<KeyType, ValueType, SetAllocator, KeyFuncs>>
    : public ContainerTraitsBase<
          Map<KeyType, ValueType, SetAllocator, KeyFuncs>> {
  enum {
    MoveWillEmptyContainer = ContainerTraits<
        typename Map<KeyType, ValueType, SetAllocator,
                     KeyFuncs>::PairSetType>::MoveWillEmptyContainer
  };
};
...
struct ContainerTraits<MultiMap<KeyType, ValueType, SetAllocator, KeyFuncs>>
    : public ContainerTraitsBase<
          MultiMap<KeyType, ValueType, SetAllocator, KeyFuncs>> {
  enum {
    MoveWillEmptyContainer = ContainerTraits<
        typename MultiMap<KeyType, ValueType, SetAllocator,
                          KeyFuncs>::PairSetType>::MoveWillEmptyContainer
  };
};
