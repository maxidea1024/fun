 ...
 * or a Array with an arbitrary allocator as an argument when the function neither adds nor removes elements
 *
 * e.g.:
 ...
 *
 * Note:
 *   View classes are not const-propagating! If you want a view where the elements are const, you need "ArrayView<const T>" not "const ArrayView<T>"!
 *
 * Caution:
...
class ArrayView
{
 ...
 private:
  template <typename T>
  using IsCompatibleElementType = ArrayView_internal::IsCompatibleElementType<T, ElementType>;
 ...
 public:
  ...
  template <
      ...
      typename = typename EnableIf<IsCompatibleElementType<OtherElementType>::Value>::Type
    >
  FUN_ALWAYS_INLINE ArrayView(const ArrayView<OtherElementType>& other)
    : data_ptr_(other.MutableData())
    , array_count_(other.Count())
  {}
  ...
  FUN_ALWAYS_INLINE ArrayView(OtherElementType (&other)[Size])
    : data_ptr_(other)
    , array_count_((int32)Size)
  {
    static_assert(Size <= int32_MAX, "array size too large! size is only int32 for compatibility with Array!");
  }
  ...
  template <
      typename OtherElementType,
      size_t Size,
      typename = typename EnableIf<IsCompatibleElementType<const OtherElementType>::Value>::Type
    >
  ...
  template <
      ...
      typename = typename EnableIf<IsCompatibleElementType<const OtherElementType>::Value>::Type
    >
  FUN_ALWAYS_INLINE ArrayView(const Array<OtherElementType, OtherAllocator>& other)
    : data_ptr_(other.MutableData())
    , array_count_(other.Count())
  {}
  ...
  FUN_ALWAYS_INLINE ArrayView(std::initializer_list<OtherElementType> list)
    : data_ptr_(&*list.begin())
    , array_count_(list.size())
  {}
  ...
  template <
      ...
      typename = typename EnableIf<IsCompatibleElementType<OtherElementType>::Value>::Type
    >
  FUN_ALWAYS_INLINE ArrayView& operator = (const ArrayView<OtherElementType>& other)
  {
  ...
  FUN_ALWAYS_INLINE ElementType* MutableData() const
  {
    return data_ptr_;
  }
  ...
  {
    CheckInvariants();
    fun_check_msg(index >= 0 & index < array_count_, "array index out of bounds: %i from an array of size {0}", index, array_count_); // & for one branch
  }
   ...
   *
   * \param index_from_the_end - (Optional) index from the end of array.  Default is 0.
   *
  ...
  int32 Find(const ElementType& item) const
  {
    ...
    for (const ElementType* __restrict data = start, *__restrict data_end = data + array_count_; data != data_end; ++data) {
      if (*data == item) {
  ...
  /**
   * Finds element within the array starting from start_index and going backwards. Uses predicate to match element.
   *
  ...
  int32 FindLastIf(const Predicate& pred, int32 start_index) const
  {
    ...
    for (const ElementType* __restrict start = ConstData(), *__restrict data = start + start_index; data != start; ) {
      --data;
      if (pred(*data)) {
        return static_cast<int32>(data - start);
      }
   ...
   *
   * \returns index to the first matching element, or INVALID_INDEX if none is found.
   */
  template <typename KeyTy>
  int32 IndexOfByKey(const KeyTy& key) const
  {
    const ElementType* __restrict start = ConstData();
    for (const ElementType* __restrict data = start, *__restrict data_end = start + array_count_; data != data_end; ++data) {
      if (*data == key) {
        return static_cast<int32>(data - start);
      }
    }
  ...
  {
    for (ElementType* __restrict data = MutableData(), *__restrict data_end = data + array_count_; data != data_end; ++data) {
      if (*data == key) {
        return data;
      }
    }
   ...
   */
  template <typename Predicate>
  Array<typename RemoveConst<ElementType>::Type> FilterIf(const Predicate& pred) const
  {
    Array<typename RemoveConst<ElementType>::Type> filtered_results;
    for (const ElementType* __restrict data = ConstData(), *__restrict data_end = data + array_count_; data != data_end; ++data) {
      if (pred(*data)) {
 ...
 private:
  ...
  FUN_ALWAYS_INLINE friend ElementType* begin(const ArrayView& array) { return array.MutableData(); }
  FUN_ALWAYS_INLINE friend ElementType* end(const ArrayView& array) { return array.MutableData() + array.Count(); }
...
template <typename ElementType, typename Allocator>
ArrayView<const ElementType> MakeArrayView(const Array<ElementType, Allocator>& other)
{
