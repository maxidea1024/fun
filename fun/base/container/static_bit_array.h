...
class StaticBitReference {
 public:
  StaticBitReference(T& data, T mask) : data_(data), mask_(mask) {}
...
class ConstStaticBitReference {
 public:
  ConstStaticBitReference(const T& data, T mask) : data_(data), mask_(mask) {}
...
class StaticBitArray {
 ...
 public:
   ...
   */
  FUN_ALWAYS_INLINE StaticBitArray() { Clear_internal(); }

  /**
   * Constructor that allows initializing by assignment from 0
   */
  FUN_ALWAYS_INLINE StaticBitArray(UnspecifiedZeroType) { Clear_internal(); }
  ...
  FUN_ALWAYS_INLINE StaticBitArray(bool, uint32 bit_index) {
    ...
    // static_assert(bit_index >= 0 && bit_index < NumBits, "Invalid bit
    // value.");
  ...
  // Accessors.
  FUN_ALWAYS_INLINE static int32 Count() { return NumBits; }
  ...
  FUN_ALWAYS_INLINE const ConstStaticBitReference<WordType> operator[](
      int32 i) const {
    fun_check(i >= 0 && i < NumBits);
  ...
  friend FUN_ALWAYS_INLINE StaticBitArray<NumBits> operator~(
      const StaticBitArray<NumBits>& other) {
  ...
  friend FUN_ALWAYS_INLINE StaticBitArray<NumBits> operator|(
      const StaticBitArray<NumBits>& a, const StaticBitArray<NumBits>& b) {
    // is not calling |= because doing it in here has less LoadHitStores and is
    // therefore faster.
  ...
  friend FUN_ALWAYS_INLINE bool operator==(const StaticBitArray<NumBits>& a,
                                           const StaticBitArray<NumBits>& b) {
  ...
  /**
   * This operator only exists to disambiguate == in statements of the form
   * (flags == 0)
   */
  friend FUN_ALWAYS_INLINE bool operator==(const StaticBitArray<NumBits>& array,
  ...
  friend Archive& operator&(Archive& ar, StaticBitArray& bit_array) {
    ...
    if (ar.IsLoading()) {
      ...
      archive_word_count =
          MathBase::Min(bit_array.kNumWords, archive_word_count);
    }

    ar.Serialize(bit_array.words_,
                 archive_word_count * sizeof(bit_array.words_[0]));
    return ar;
  }
  ...
  // TODO
  ///**
  // * Converts the bitarray to a String representing the binary representation
  // of the array
  // */
 ...
 private:
  ...
  static const uint32 kNumWords =
      ((NumBits + kNumBitsPerWord - 1) & ~(kNumBitsPerWord - 1)) /
      kNumBitsPerWord;
