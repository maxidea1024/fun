 ...
 public:
  ...
  struct FUN_BASE_API ForAnyElementType {
    ...
    void ResizeAllocation(int32 previous_element_count, int32 element_count,
      ...
      if (element_count > 0) {
        // CHECK_SLOW(((uint64)element_count*(uint64)ElementTypeInfo.GetSize() <
        // (uint64)INT_MAX));
    ...
    int32 CalculateSlackReserve(int32 element_count,
    ...
    int32 CalculateSlackGrow(int32 element_count, int32 allocated_element_count,
                             int32 bytes_per_element) const {
    ...
    bool HasAllocation() { return data_ != nullptr; }
 ...
 */
class AnsiSetAllocator
    : public SetAllocator<AnsiAllocator, InlineAllocator<1, AnsiAllocator>> {};
