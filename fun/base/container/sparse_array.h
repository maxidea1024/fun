...
template <typename _ElementType, typename Allocator = DefaultSparseArrayAllocator>
class SparseArray;
...
class SparseArray {
 ...
 public:
  ...
  void RemoveAtUninitialized(int32 index, int32 count = 1) {
    for (; count > 0; --count) {
      ...
      index_data.next_free_index = free_index_count_ > 0 ? first_free_index_ : INVALID_INDEX;
  ...
  void Reserve(int32 count) {
    if (count > data_.Count()) {
      ...
      for (int32 free_index = element_index; free_index < count; ++free_index) {
        ...
        MutableData(free_index).prev_free_index = -1;
        MutableData(free_index).next_free_index = free_index_count_ > 0 ? first_free_index_ : INVALID_INDEX;
        first_free_index_ = free_index;
        ++free_index_count_;
      }
  ...
  void Shrink() {
    ...
    for (ConstSetBitIterator<typename Allocator::BitArrayAllocator> allocated_index_it(allocation_flags_); allocated_index_it; ++allocated_index_it) {
      max_allocated_index = MathBase::Max(max_allocated_index, allocated_index_it.GetIndex());
    }
    ...
    if (first_index_to_remove < data_.Count()) {
      ...
      data_.RemoveAt(first_index_to_remove, data_.Count() - first_index_to_remove);
      allocation_flags_.RemoveAt(first_index_to_remove, allocation_flags_.Count() - first_index_to_remove);
    }
  ...
  bool Compact() {
    ...
    while (free_index != INVALID_INDEX) {
      ...
      if (free_index < target_index) {
        ...
        RelocateConstructItems<ElementOrFreeListLink>(element_data + free_index, element_data + end_index, 1);
  ...
  uint32 GetAllocatedSize() const {
    return (data_.Count() + data_.GetSlack()) * sizeof(ElementOrFreeListLink) + allocation_flags_.GetAllocatedSize();
  }
  ...
  friend bool operator == (const SparseArray& x, const SparseArray& y) {
    ...
    for (int32 element_index = 0; element_index < x.GetMaxIndex(); ++element_index) {
  ...
  SparseArray& operator = (const SparseArray& other) {
    if (FUN_LIKELY(&other != this)) {
      ...
      if (TypeTraits<ElementType>::NeedsCopyConstructor) {
        ElementOrFreeListLink* src_data = (ElementOrFreeListLink*)data_.MutableData();
        const ElementOrFreeListLink* dest_data = (ElementOrFreeListLink*)other.data_.ConstData();
        ...
        for (int32 index = 0; index < other.GetMaxIndex(); ++index) {
          ...
          if (other.IsAllocated(index)) {
            ::new((uint8*)&dest_element.element_data) ElementType(*(ElementType*)&src_element.element_data);
          }
        ...
        UnsafeMemory::Memcpy(data_.MutableData(), other.data_.ConstData(), sizeof(ElementOrFreeListLink) * other.GetMaxIndex());
      }
    }
    return *this;
  }
 ...
 private:
  ...
  FUN_ALWAYS_INLINE static typename EnableIf<ContainerTraits<SparseArrayType>::MoveWillEmptyContainer>::Type
  MoveOrCopy(SparseArrayType& to_array, SparseArrayType& from_array) {
    to_array.data_ = (DataType&&)from_array.data_;
    to_array.allocation_flags_ = (AllocationBitArrayType&&)from_array.allocation_flags_;
 ...
 public:
  ...
  ElementType& operator[](int32 index) {
    fun_check_dbg(index >= 0 && index < data_.Count() && index < allocation_flags_.Count());
    //fun_check_dbg(allocation_flags_[index]); // Disabled to improve loading times -BZ
  ...
  bool IsAllocated(int32 index) const {
    return allocation_flags_[index];
  }
  ...
  int32 Count() const {
    return data_.Count() - free_index_count_;
  }
 ...
 private:
  ...
  class BaseIterator {
   public:
    using BitArrayItType = ConstSetBitIterator<typename Allocator::BitArrayAllocator>;

   private:
    using ArrayType = typename Conditional<IsConst, const SparseArray, SparseArray>::Result;
    using ItElementType = typename Conditional<IsConst, const ElementType, ElementType>::Result;

   public:
    explicit BaseIterator(ArrayType& array, const BitArrayItType& bit_array_it)
      : array_(array),
        bit_array_it_(bit_array_it) {}
    ...
    FUN_ALWAYS_INLINE friend bool operator == (const BaseIterator& lhs, const BaseIterator& rhs) {
      return lhs.bit_array_it_ == rhs.bit_array_it_ && &lhs.array_ == &rhs.array_;
    }
    ...
    FUN_ALWAYS_INLINE const RelativeBitReference& GetRelativeBitReference() const {
      return bit_array_it_;
    }
  ...
  class Iterator : public BaseIterator<false> {
   public:
    Iterator(SparseArray& array)
      : BaseIterator<false>(array, ConstSetBitIterator<typename Allocator::BitArrayAllocator>(array.allocation_flags_)) {}

    Iterator(SparseArray& array, const typename BaseIterator<false>::BitArrayItType& bit_array_it)
     ...
     */
    void RemoveCurrent() {
      this->array_.RemoveAt(this->GetIndex());
    }
  };
  ...
  class ConstIterator : public BaseIterator<true>
  {
   public:
    ConstIterator(const SparseArray& array)
      : BaseIterator<true>(array, ConstSetBitIterator<typename Allocator::BitArrayAllocator>(array.allocation_flags_))
    {
    }

    ConstIterator(const SparseArray& array, const typename BaseIterator<true>::BitArrayItType& bit_array_it)
 ...
 private:
  ...
  FUN_ALWAYS_INLINE friend Iterator begin(SparseArray& array) { return Iterator(array, ConstSetBitIterator<typename Allocator::BitArrayAllocator>(array.allocation_flags_)); }
  FUN_ALWAYS_INLINE friend ConstIterator begin(const SparseArray& array) { return ConstIterator(array, ConstSetBitIterator<typename Allocator::BitArrayAllocator>(array.allocation_flags_)); }
  FUN_ALWAYS_INLINE friend Iterator end(SparseArray& array) { return Iterator(array, ConstSetBitIterator<typename Allocator::BitArrayAllocator>(array.allocation_flags_, array.allocation_flags_.Count())); }
  FUN_ALWAYS_INLINE friend ConstIterator end(const SparseArray& array) { return ConstIterator(array, ConstSetBitIterator<typename Allocator::BitArrayAllocator>(array.allocation_flags_, array.allocation_flags_.Count())); }
 ...
 public:
  ...
  {
   public:
    ConstSubsetIterator(const SparseArray& array, const BitArray<SubsetAllocator>& bit_array)
      : array_(array)
      , bit_array_it_(array.allocation_flags_, bit_array)
    {
    }
   ...
   private:
    ...
    ConstDualSetBitIterator<typename Allocator::BitArrayAllocator, SubsetAllocator> bit_array_it_;
  };
 ...
 private:
   ...
   * compatible types.
   */
  using ElementOrFreeListLink = SparseArrayElementOrFreeListLink<AlignedStorage<sizeof(ElementType), alignof(ElementType)>>;
  ...
  {
   public:
    ElementCompareClass(const Predicate& pred)
      : pred_(pred)
    {
    }

    bool operator()(const ElementOrFreeListLink& x, const ElementOrFreeListLink& y) const
    {
      return pred_(*(ElementType*)&x.element_data, *(ElementType*)&y.element_data);
    }
  ...
  using DataType = Array<ElementOrFreeListLink, typename Allocator::ElementAllocator>;
  DataType data_;

  using AllocationBitArrayType = BitArray<typename Allocator::BitArrayAllocator>;
...
class UntypedSparseArray {
 public:
  static UntypedSparseArrayLayout GetUntypedLayout(int32 element_size, int32 element_alignment) {
    UntypedSparseArrayLayout result;
    result.element_offset = 0;
    result.alignment = MathBase::Max(element_alignment, (int32)alignof(FreeListLink));
  ...
  const void* ConstData(int32 index, const UntypedSparseArrayLayout& layout) const {
  ...
  int32 AddUninitialized(const UntypedSparseArrayLayout& layout) {
    ...
    if (free_index_count_ > 0) {
      // Remove and use the first index from the list of free elements.
      index = first_free_index_;
      first_free_index_ = GetFreeListLink(first_free_index_, layout)->next_free_index;
      --free_index_count_;
      if (free_index_count_ > 0) {
        GetFreeListLink(first_free_index_, layout)->prev_free_index = INVALID_INDEX;
      }
    } else {
      // Add a new element.
   ...
   */
  void RemoveAtUninitialized(const UntypedSparseArrayLayout& layout, int32 index, int32 count = 1) {
    for (; count > 0; --count) {
      fun_check(allocation_flags_[index]);
 ...
 private:
  ...
  static void CheckConstraints() {
    ...
    static_assert(sizeof (RawType) == sizeof (RealType), "UntypedSparseArray's size doesn't match SparseArray");
    static_assert(alignof(RawType) == alignof(RealType), "UntypedSparseArray's alignment doesn't match SparseArray");

    // Check member sizes
    static_assert(sizeof(DeclVal<RawType>().data_) == sizeof(DeclVal<RealType>().data_), "UntypedSparseArray's data_ member size does not match SparseArray's");
    static_assert(sizeof(DeclVal<RawType>().allocation_flags_) == sizeof(DeclVal<RealType>().allocation_flags_), "UntypedSparseArray's allocation_flags_ member size does not match SparseArray's");
    static_assert(sizeof(DeclVal<RawType>().first_free_index_) == sizeof(DeclVal<RealType>().first_free_index_), "UntypedSparseArray's first_free_index_ member size does not match SparseArray's");
    static_assert(sizeof(DeclVal<RawType>().free_index_count_) == sizeof(DeclVal<RealType>().free_index_count_), "UntypedSparseArray's free_index_count_ member size does not match SparseArray's");

    // Check member offsets
    static_assert(offsetof(RawType, data_) == offsetof(RealType, data_), "UntypedSparseArray's data_ member offset does not match SparseArray's");
    static_assert(offsetof(RawType, allocation_flags_) == offsetof(RealType, allocation_flags_), "UntypedSparseArray's allocation_flags_ member offset does not match SparseArray's");
    static_assert(offsetof(RawType, first_free_index_) == offsetof(RealType, first_free_index_), "UntypedSparseArray's first_free_index_ member offset does not match SparseArray's");
    static_assert(offsetof(RawType, free_index_count_) == offsetof(RealType, free_index_count_), "UntypedSparseArray's free_index_count_ member offset does not match SparseArray's");

    // Check free index offsets
    static_assert(offsetof(RawType::FreeListLink, prev_free_index) == offsetof(RealType::ElementOrFreeListLink, prev_free_index), "UntypedSparseArray's FreeListLink's prev_free_index member offset does not match SparseArray's");
    static_assert(offsetof(RawType::FreeListLink, next_free_index) == offsetof(RealType::ElementOrFreeListLink, next_free_index), "UntypedSparseArray's FreeListLink's next_free_index member offset does not match SparseArray's");
  }
   ...
   * Accessor for the element or free list data.
   */
  FUN_ALWAYS_INLINE FreeListLink* GetFreeListLink(int32 index, const UntypedSparseArrayLayout& layout) {
...
template <> struct IsZeroConstructible<UntypedSparseArray> { enum { Value = true }; };
 ...
 */
FUN_ALWAYS_INLINE void* operator new(fun::size_t size, const fun::SparseArrayAllocationInfo& allocation)
{
