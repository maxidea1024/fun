...
class ChunkedArray
{
 public:
   ...
   */
  ChunkedArray(int32 count = 0)
    : count_(count)
  {
    // Compute the number of chunks needed.
    const int32 chunk_count = (count_ + ElementsPerChunk - 1) / ElementsPerChunk;
 ...
 private:
  ...
  FUN_ALWAYS_INLINE static typename EnableIf<ContainerTraits<ArrayType>::MoveWillEmptyContainer>::Type
 ...
 public:
  ChunkedArray(ChunkedArray&& other)
  {
  ...
  {
    return chunks_.GetAllocatedSize();
  }
   ...
   *
   * \param min_capacity - The number of elements that the array should be able to contain after allocation.
   */
  void Reserve(int32 min_capacity)
  {
    // Compute the number of chunks needed.
    const int32 chunk_count = (min_capacity + ElementsPerChunk - 1) / ElementsPerChunk;
...
struct ContainerTraits<ChunkedArray<ElementType, TargetBytesPerChunk>>
  : public ContainerTraitsBase<ChunkedArray<ElementType, TargetBytesPerChunk>>
{
  enum { MoveWillEmptyContainer = ContainerTraits<typename ChunkedArray<ElementType, TargetBytesPerChunk>::ChunksType>::MoveWillEmptyContainer };
};
...
void* operator new(fun::size_t size, fun::ChunkedArray<T, TargetBytesPerChunk>& chuncked_array)
{
  using namespace fun;
