...
template <size_t Size> struct TypeStorage;

template <> struct TypeStorage<sizeof(uint8) > { using Type = uint8;  };
template <> struct TypeStorage<sizeof(uint16)> { using Type = uint16; };
template <> struct TypeStorage<sizeof(uint32)> { using Type = uint32; };
template <> struct TypeStorage<sizeof(uint64)> { using Type = uint64; };
...
struct HasNone {
  template <typename U> static auto Test(U *) -> decltype(U::None, TrueType());
  template <typename U> static auto Test(...) -> FalseType;

  static constexpr bool Value = IsSame<decltype(Test<Enum>(0)), TrueType>::Value;
};
...
class Flags {
 public:
  ...
  //TODO 이게 꼭 필요할까???
  //static_assert(Flags_internal::HasNone<T>::Value, "The enumeration needs a None value");
  //static_assert(static_cast<UnderlyingType>(T::None) == 0, "None should be 0");
...
#define __DEFINE_FLAGS_BITWISE_OP_CONST(op) \
  FUN_ALWAYS_INLINE Flags<T> operator op (T rhs) const { \
    return Flags<T>(static_cast<T>(static_cast<UnderlyingType>(value_) op static_cast<UnderlyingType>(rhs))); \
  } \
  FUN_ALWAYS_INLINE Flags<T> operator op (Flags<T> rhs) const { \
    return Flags<T>(static_cast<T>(static_cast<UnderlyingType>(value_) op static_cast<UnderlyingType>(rhs.value_))); \
  }
...
#define __DEFINE_FLAGS_BITWISE_OP(op) \
  FUN_ALWAYS_INLINE Flags<T>& operator op##= (T rhs) { \
    value_ = static_cast<T>(static_cast<UnderlyingType>(value_) op static_cast<UnderlyingType>(rhs)); \
    return *this; \
  } \
  FUN_ALWAYS_INLINE Flags<T>& operator op##= (Flags<T> rhs) { \
    value_ = static_cast<T>(static_cast<UnderlyingType>(value_) op static_cast<UnderlyingType>(rhs.value_)); \
    return *this; \
  }
...
#undef __DEFINE_FLAGS_BITWISE_OP
  ...
  FUN_ALWAYS_INLINE bool HasAny(T mask) const {
    return (static_cast<UnderlyingType>(value_) & static_cast<UnderlyingType>(mask)) != 0;
  }

  FUN_ALWAYS_INLINE bool HasAll(T mask) const {
    return (static_cast<UnderlyingType>(value_) & static_cast<UnderlyingType>(mask)) == static_cast<UnderlyingType>(mask);
  }
  ...
  FUN_ALWAYS_INLINE Flags<T>& SetBool(T flags, bool set) {
    if (set) {
      value_ = static_cast<T>(static_cast<UnderlyingType>(value_) | (static_cast<UnderlyingType>(flags)));
    } else {
      value_ = static_cast<T>(static_cast<UnderlyingType>(value_) & (~static_cast<UnderlyingType>(flags)));
    }
    return *this;
...
#define __DEFINE_FLAGS_BITWISE_OP(T, Op) \
    FUN_ALWAYS_INLINE T operator Op (T lhs, T rhs) { \
      ...
      return static_cast<T>(static_cast<UnderlyingType>(lhs) Op static_cast<UnderlyingType>(rhs)); \
    }
