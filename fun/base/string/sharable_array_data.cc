...
size_t CalculateBlockSize(size_t element_count, size_t element_size,
...
// util 함수로 빼주어도 좋을듯...
GrowingBlockSize CalculateGrowingBlockSize(size_t element_count,
                                           size_t element_size,
                                           size_t header_size) {
  GrowingBlockSize result = {std::numeric_limits<size_t>::max(),
                             std::numeric_limits<size_t>::max()};

  uint32 bytes =
      uint32(CalculateBlockSize(element_count, element_size, header_size));
  if (int32(bytes) < 0) {
    return result;
  }

  //버그가 있었음!
  // const uint32 more_bytes = MathBase::RoundUpToPowerOfTwo(bytes); //이게
  // 맞는건지?
...
//  { { RefCounter::PERSISTENT_COUNTER_VALUE }, 0, 0, 0,
//  sizeof(UntypedSharableArrayData) }, // shared null
//  /* zero initialized terminator */
//};

static const UntypedSharableArrayData g_empty_values[3] = {
    {{RefCounter::PERSISTENT_COUNTER_VALUE},
     0,
     0,
     0,
     sizeof(UntypedSharableArrayData)},  // shared empty
    {{RefCounter::UNSHARABLE_COUNTER_VALUE},
     0,
     0,
     0,
     sizeof(UntypedSharableArrayData)},  // unshared empty
    /* zero initialized terminator */
};
...
FUN_ALWAYS_INLINE size_t CalculateBlockSize_helper(size_t& capacity,
                                                   size_t object_size,
                                                   size_t header_size,
                                                   uint32 options) {
...
UntypedSharableArrayData* Reallocate_helper(UntypedSharableArrayData* header,
                                            size_t alloc_size, uint32 options) {
  header = static_cast<UntypedSharableArrayData*>(
      UnsafeMemory::Realloc(header, alloc_size));
  if (header) {
    header->capacity_reserved =
        !!(options & UntypedSharableArrayData::CapacityReserved);
  }
  return header;
}
...
UntypedSharableArrayData* UntypedSharableArrayData::Allocate(
  ...
  fun_check(alignment >= alignof(UntypedSharableArrayData) &&
  ...
  const size_t alloc_size =
      CalculateBlockSize_helper(capacity, object_size, header_size, options);

  // critical point:
  //  alignment가 생략되어서 엄청난 버그가 지속적으로 있었음.
  // alignment는 별도로 계산되므로, 여기서 alignment가 요구되지 않음.
  UntypedSharableArrayData* header =
      static_cast<UntypedSharableArrayData*>(UnsafeMemory::Malloc(alloc_size));
  if (header) {
...
UntypedSharableArrayData* UntypedSharableArrayData::ReallocateUnaligned(
  ...
  const size_t alloc_size =
      CalculateBlockSize_helper(capacity, object_size, header_size, options);
  UntypedSharableArrayData* header = static_cast<UntypedSharableArrayData*>(
      Reallocate_helper(old_data, alloc_size, options));
  if (header) {
    header->alloc = capacity;
  }
  return header;
...
void UntypedSharableArrayData::Free(UntypedSharableArrayData* data,
                                    size_t object_size,
