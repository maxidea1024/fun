...
struct IsCompatibleCharTypeHelper
  : IntegralConstant<bool,
    IsSame<CharType, UNICHAR>::Value ||
    IsSame<CharType, int16>::Value ||
    IsSame<CharType, uint16>::Value ||
    IsSame<CharType, char16_t>::Value ||
    (IsSame<CharType, wchar_t>::Value && sizeof(wchar_t) == sizeof(UNICHAR))> {};
...
class UStringView {
 ...
 private:
  template <typename CharType>
  using IfCompatibleChar = typename EnableIf<UniStringView_internal::IsCompatibleCharType<CharType>::Value, bool>::Type;

  template <typename Array>
  using IfCompatibleArray = typename EnableIf<UniStringView_internal::IsCompatibleArray<Array>::Value, bool>::Type;

  template <typename Pointer>
  using IfCompatiblePointer = typename EnableIf<UniStringView_internal::IsCompatiblePointer<Pointer>::Value, bool>::Type;

  template <typename T>
  using IfCompatibleStdString = typename EnableIf<UniStringView_internal::IsCompatibleStdBasicString<T>::Value, bool>::Type;

  template <typename T>
  using IfCompatibleUStringFamily = typename EnableIf<IsSame<T,UString>::Value || IsSame<T,UStringRef>::Value, bool>::Type;
 ...
 public:
  UStringView() noexcept
    : data_(nullptr), length_(0) {}

  UStringView(decltype(nullptr))
    : UStringView() {}
  ...
  template <typename Str, IfCompatibleUStringFamily<Str> = true> //string_inline.h
  ...
  inline UNICHAR FirstOr(const UNICHAR def = UNICHAR(0)) const { return Len() ? At(0) : def; }
  inline UNICHAR LastOr(const UNICHAR def = UNICHAR(0)) const { return Len() ? At(Len() - 1) : def; }

  inline const_pointer ConstData() const noexcept { return reinterpret_cast<const_pointer>(data_); }
  inline const storage_type* UTF16() const noexcept { return data_; }

  inline UNICHAR operator[](size_t index) const { fun_check(index < Len()); return UNICHAR(data_[index]); }
  inline UNICHAR At(size_t index) const { fun_check(index < Len()); return UNICHAR(data_[index]); }
  ...
  inline UStringView Mid(size_t pos) const { fun_check(pos <= length_); return UStringView(data_ + pos, length_ - pos); }
  inline UStringView Mid(size_t pos, size_t len) const { fun_check(pos + len <= length_); return UStringView(data_ + pos, len); }
  inline UStringView Left(size_t len) { fun_check(len <= length_); return UStringView(data_, len); }
  inline UStringView Right(size_t len) { fun_check(len <= length_); return UStringView(data_ + length_ - len, len); }
  //TODO Chopped

  UStringView& Truncate(size_t pos) { fun_check(pos <= Len()); length_ = pos; }
  ...
  inline UNICHAR front() const { fun_check(!empty()); return UNICHAR(data_[0]); }
  inline UNICHAR back() const { fun_check(!empty()); return UNICHAR(data_[length_ - 1]); }
...
template <
    typename StringFamily,
    typename EnableIf<
          IsSame<StringFamily, UString>::Value || IsSame<StringFamily, UStringRef>::Value,
          bool
        >::Type = true
  >
