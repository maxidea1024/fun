...
struct IsCompatibleCharTypeHelper
    : IntegralConstant<bool, IsSame<CharType, char>::Value ||
...
struct IsSupportByteStringView
    : IntegralConstant<
          ...
          ByteStringView_internal::IsCompatibleCharType<T>::Value ||
              ...
              IsSame<T, ByteString>::Value || IsSame<T, ByteStringRef>::Value> {
};
...
class ByteStringView {
 ...
 private:
  ...
  using IfCompatibleChar = typename EnableIf<
      ByteStringView_internal::IsCompatibleCharType<CharType>::Value,
      bool>::Type;

  template <typename Array>
  using IfCompatibleArray = typename EnableIf<
      ByteStringView_internal::IsCompatibleArray<Array>::Value, bool>::Type;

  template <typename Pointer>
  using IfCompatiblePointer = typename EnableIf<
      ByteStringView_internal::IsCompatiblePointer<Pointer>::Value, bool>::Type;
  ...
  template <typename T>
  using IfCompatibleByteStringFamily =
      typename EnableIf<IsSame<T, ByteString>::Value ||
 ...
 public:
  ByteStringView() noexcept : data_(nullptr), length_(0) {}

  ByteStringView(decltype(nullptr)) : ByteStringView() {}
  ...
  template <typename Str,
            IfCompatibleByteStringFamily<Str> = true>  // byte_string_inline.h
  ...
  FUN_ALWAYS_INLINE char FirstOr(const char def = char(0)) const {
    return Len() ? At(0) : def;
  }
  FUN_ALWAYS_INLINE char LastOr(const char def = char(0)) const {
    return Len() ? At(Len() - 1) : def;
  }

  FUN_ALWAYS_INLINE const_pointer ConstData() const noexcept {
    return reinterpret_cast<const_pointer>(data_);
  }

  FUN_ALWAYS_INLINE char operator[](size_t index) const {
    fun_check(index < Len());
    return char(data_[index]);
  }
  FUN_ALWAYS_INLINE char At(size_t index) const {
    fun_check(index < Len());
    return char(data_[index]);
  }
  ...
  FUN_ALWAYS_INLINE ByteStringView Mid(size_t pos) const {
    fun_check(pos <= length_);
    return ByteStringView(data_ + pos, length_ - pos);
  }
  FUN_ALWAYS_INLINE ByteStringView Mid(size_t pos, size_t len) const {
    fun_check(pos + len <= length_);
    return ByteStringView(data_ + pos, len);
  }
  FUN_ALWAYS_INLINE ByteStringView Left(size_t len) {
    fun_check(len <= length_);
    return ByteStringView(data_, len);
  }
  FUN_ALWAYS_INLINE ByteStringView Right(size_t len) {
    fun_check(len <= length_);
    return ByteStringView(data_ + length_ - len, len);
  }
  // TODO Chopped

  ByteStringView& Truncate(size_t pos) {
    fun_check(pos <= Len());
    length_ = pos;
  }

  FUN_ALWAYS_INLINE bool StartsWith(
      ByteStringView sub,
  ...
  FUN_ALWAYS_INLINE bool EndsWith(
      ByteStringView sub,
      CaseSensitivity casesense = CaseSensitivity::CaseSensitive) const {
  ...
  FUN_ALWAYS_INLINE const_iterator begin() const noexcept {
    return ConstData();
  }
  FUN_ALWAYS_INLINE const_iterator end() const noexcept {
  ...
  FUN_ALWAYS_INLINE const_reverse_iterator rbegin() const noexcept {
    return const_reverse_iterator(end());
  }
  FUN_ALWAYS_INLINE const_reverse_iterator rend() const noexcept {
    return const_reverse_iterator(begin());
  }
  FUN_ALWAYS_INLINE const_reverse_iterator crbegin() const noexcept {
    return rbegin();
  }
  FUN_ALWAYS_INLINE const_reverse_iterator crend() const noexcept {
    return rend();
  }

  FUN_ALWAYS_INLINE bool empty() const noexcept { return size() == 0; }
  FUN_ALWAYS_INLINE char front() const {
    fun_check(!empty());
    return char(data_[0]);
  }
  FUN_ALWAYS_INLINE char back() const {
    fun_check(!empty());
    return char(data_[length_ - 1]);
  }
...
template <typename ByteStringFamily,
          typename EnableIf<IsSame<ByteStringFamily, ByteString>::Value ||
                                IsSame<ByteStringFamily, ByteStringRef>::Value,
                            bool>::Type = true>
FUN_ALWAYS_INLINE ByteStringView
ToByteStringView(const ByteStringFamily& str) noexcept {
