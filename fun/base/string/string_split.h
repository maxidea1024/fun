...
template <
      typename ResultStringType,
      typename SourceStringType,
      typename SeparatorsType,
      typename MidMethod
    >
inline static int32 SplitByCharacters(Array<ResultStringType>& list,
  ...
  for (;;) {
    ...
    const int32 separator_pos = source.IndexOfAny(separators, casesense, start, nullptr, &separator_len);
    const bool term = (separator_pos == INVALID_INDEX || (max_splits > 0 && splitted_count == max_splits));
    ...
    if (split_options & StringSplitOption::Trimming) {
      ...
      token_len = StringAlgo<SourceStringType>::TrimmedPositions(token_begin, token_end);
    ...
    start = separator_pos + separator_len; //인코딩 변환이 있을 수 있으므로, 길이를 고려해야함.
  }
...
inline static int32 SplitByStrings( Array<ResultStringType>& list,
  ...
  for (;;) {
    ...
    const int32 separator_pos = IndexOfAnyStrings_helper(source, separators, separator_count, casesense, start, nullptr, &matched_separator_len);
    const bool term = (separator_pos == INVALID_INDEX || (max_splits > 0 && splitted_count == max_splits));
    const int32 end = term ? source.Len() : separator_pos;
    ...
    start = separator_pos + matched_separator_len; //인코딩 변환이 있을 수 있으므로, 길이를 고려해야함.
  }
