...
struct FUN_BASE_API UntypedSharableArrayData {
  ...
  FUN_ALWAYS_INLINE void* MutableData() {
    fun_check(length == 0 || offset < 0 || offset >= sizeof(UntypedSharableArrayData));
  ...
  //TODO 내부에서 헤더 길이를 계산할때 alignment를 고려한다면, 아래 조건을 달리해야한다.
   ...
   */
  FUN_ALWAYS_INLINE bool IsMutable() const {
    return alloc != 0;
  }
  ...
  static void Free( UntypedSharableArrayData* data,
                    size_t object_size,
...
struct TypedSharableArrayData : public UntypedSharableArrayData {
  ...
  FUN_ALWAYS_INLINE static TypedSharableArrayData*
    Allocate(size_t capacity, AllocationOptions options = Default) {
    static_assert(sizeof(TypedSharableArrayData) == sizeof(Super), "sizeof(TypedSharableArrayData) == sizeof(Super)");
    return static_cast<TypedSharableArrayData*>(Super::Allocate(sizeof(T), alignof(AlignmentHelper), capacity, options));
  }

  FUN_ALWAYS_INLINE static TypedSharableArrayData*
    ReallocateUnaligned(TypedSharableArrayData* old_data, size_t capacity, AllocationOptions options = Default) {
    static_assert(sizeof(TypedSharableArrayData) == sizeof(Super), "sizeof(TypedSharableArrayData) == sizeof(Super)");
    return static_cast<TypedSharableArrayData*>(Super::ReallocateUnaligned(old_data, sizeof(T), alignof(AlignmentHelper), capacity, options));
  }
  ...
  FUN_ALWAYS_INLINE static TypedSharableArrayData*
  FromRawData(const T* data, size_t len, AllocationOptions options = Default) {
    static_assert(sizeof(TypedSharableArrayData) == sizeof(Super), "sizeof(TypedSharableArrayData) == sizeof(Super)");
  ...
  FUN_ALWAYS_INLINE iterator begin(iterator = iterator()) { return MutableData(); }
  FUN_ALWAYS_INLINE iterator end(iterator = iterator()) { return MutableData() + length; }
  FUN_ALWAYS_INLINE const_iterator begin(const_iterator = const_iterator()) const { return MutableData(); }
  FUN_ALWAYS_INLINE const_iterator end(const_iterator = const_iterator()) const { return MutableData() + length; }
  FUN_ALWAYS_INLINE const_iterator cbegin(const_iterator = const_iterator()) const { return ConstData(); }
  FUN_ALWAYS_INLINE const_iterator cend(const_iterator = const_iterator()) const { return ConstData() + length; }
};
...
#define STATIC_SHARABLE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(size, offset) \ { { RefCounter::PERSISTENT_COUNTER_VALUE }, size, 0, 0, offset }

#define STATIC_SHARABLE_ARRAY_DATA_HEADER_INITIALIZER(type, size) \
  STATIC_SHARABLE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(Size, ((sizeof(UntypedSharableArrayData) + (alignof(type)-1)) & ~(alignof(type)-1)))
...
// accomplished by hiding a static const instance of TypedStaticSharableArrayData, which is
// POD.
...
#define SHARABLE_ARRAY_LITERAL_IMPL(type, ...) \
  union { Type TYPE_MUST_BE_POD; } dummy; (void)dummy; \
  type data[] = { __VA_ARGS__ }; (void)data; \
  static const TypedStaticSharableArrayData<type,size> literal = { STATIC_SHARABLE_ARRAY_DATA_HEADER_INITIALIZER(type, size), { __VA_ARGS__ }}; \
  TypedSharableArrayDataPointerRef<type> ref = { static_cast<TypedSharableArrayData<type>*>(const_cast<UntypedSharableArrayData*>(&literal.header)) };
