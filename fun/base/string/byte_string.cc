...
// NOTE data_->bCapacityReserved는 Reserve() 함수를 통해서, capacity를 일부러
// 잡아놓은 경우인지 아닌지를 판단하는 용도임.

/*! \class ByteString
  \brief ANSI / UTF8 문자열 뿐만 아니라, 바이너리 RAW 데이터도 다룰수 있는
  클래스.

  기본적으로 std::string 클래스와 유사한 기능들을 지원합니다.

  통신으로 오고가는 데이터처리에서는 유니코드 기반의 UString을 사용하기에는 다소
  비효율적 부분이 있으며, 처리가 애매한 부분이 있다.

  Redis, Zookeeper 등의 적지 않은 외부 솔류션들이 바이트
  스트링(std::string같은)을 사용하기 때문에, 단지 타입 때문에, 불필요한 변환과
  복사를 수반하게 된다.

  이럴 경우, 가급적 바이트 스트링을 그대로 액세스 할 수 있는 수단이 필요하게
  되는데, 이때 사용되는 클래스가 \c ByteString이다.

  기본 구조는 std::string과 유사한 성격을 띄고 있다.

  \c UString과 통합되어 있으므로, 상호 변환등의 작업을 명시적으로 하지 않아도
  자동으로 처리가 가능하다.


  \todo 검색관련해서, casesenseitive를 선택적으로 적용할 수 있도록 함.
  \todo 버퍼 공유, 외부 버퍼를 사용하는 등의 기능도 고려해야함. 기존의
  CByteArrayPtr이 좀 많이 후짐... \todo 작업이 어느정도 안정화되면, UString도
  유사한 형태로 정리하도록 한다.
...
ByteString::ByteString() : data_(Data::SharedEmpty()) {}
...
ByteString::ByteString(const ByteString& rhs) : data_(rhs.data_) {
  data_->ref.AddRef();
}
...
ByteString::ByteString(const UNICHAR* str, int32 len) {
  ...
  if (str_as_utf8.IsEmpty()) {
    ...
    StringCopy(data_->MutableData(), str_as_utf8.ConstData(),
               str_as_utf8.Len());
...
ByteString::ByteString(UNICHAR ch) {
  ...
  StringCopy(data_->MutableData(), char_as_utf8.ConstData(),
             char_as_utf8.Len());
...
ByteString& ByteString::operator=(char ch) {
  if (IsDetached() &&
      Capacity() >= 1) {  // capacity는 순수하게 글자 갯수임. 즉, 실제 할당은
                          // 글자수+1임 (단, 비어있는 경우에는 0)
    char* dst = data_->MutableData();
    dst[0] = ch;
    dst[1] = '\0';
    data_->length = 1;
...
ByteString::ByteString(ByteString&& rhs) : data_(rhs.data_) {
...
void ByteString::Swap(ByteString& rhs) { fun::Swap(data_, rhs.data_); }

int32 ByteString::Len() const { return data_->length; }
...
bool ByteString::IsEmpty() const { return data_->length == 0; }
...
void ByteString::ResizeZeroed(int32 new_len) { Resize(new_len, char(0)); }
...
const char* ByteString::operator*() const { return data_->ConstData(); }
...
const char* ByteString::ConstData() const { return data_->ConstData(); }
...
bool ByteString::IsNulTerm() const { return NulTermLen() == Len(); }
...
char ByteString::First() const { return At(0); }

ByteString::CharRef ByteString::First() { return operator[](0); }

char ByteString::Last() const { return At(Len() - 1); }

ByteString::CharRef ByteString::Last() { return operator[](Len() - 1); }
...
void ByteString::Reserve(int32 new_capacity) {
  if (data_->ref.IsShared() || (new_capacity + 1) > (int32)data_->alloc) {
    ...
    ReallocateData(new_capacity + 1,
                   data_->DetachFlags() | Data::CapacityReserved);
  } else {
...
void ByteString::Shrink() {
  if (data_->ref.IsShared() || data_->length + 1 < (int32)data_->alloc) {
    ReallocateData(
        data_->length + 1,
        data_->DetachFlags() &
            ~Data::CapacityReserved);  // CapacityReserved 플래그를 클리어. 즉,
                                       // 여분 없이 딱 핏팅되어 있는 상태임.
...
bool ByteString::IsDetached() const { return data_->ref.IsShared() == false; }

bool ByteString::IsSharedWith(const ByteString& rhs) const {
  return data_ == rhs.data_;
}

bool ByteString::IsRawData() const { return data_->IsRawData(); }

void ByteString::ReallocateData(int32 new_alloc,
                                Data::AllocationOptions options) {
...
int32 ByteString::IndexOfAny(const Array<char>& chars,
                             CaseSensitivity casesense, int32 from,
                             int32* matched_index, int32* matched_len) const {
  return IndexOfAny(ByteStringView(chars.ConstData(), chars.Count()), casesense,
                    from, matched_index, matched_len);
}

int32 ByteString::LastIndexOfAny(const Array<char>& chars,
                                 CaseSensitivity casesense, int32 from,
                                 int32* matched_index,
                                 int32* matched_len) const {
  return LastIndexOfAny(ByteStringView(chars.ConstData(), chars.Count()),
                        casesense, from, matched_index, matched_len);
}

int32 ByteString::IndexOfAny(const Array<const char*>& strings,
                             CaseSensitivity casesense, int32 from,
                             int32* matched_index, int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.ConstData(), strings.Count(),
                                  casesense, from, matched_index, matched_len);
}
int32 ByteString::IndexOfAny(const Array<ByteString>& strings,
                             CaseSensitivity casesense, int32 from,
                             int32* matched_index, int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.ConstData(), strings.Count(),
                                  casesense, from, matched_index, matched_len);
}
int32 ByteString::IndexOfAny(const Array<ByteStringRef>& strings,
                             CaseSensitivity casesense, int32 from,
                             int32* matched_index, int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.ConstData(), strings.Count(),
                                  casesense, from, matched_index, matched_len);
}
int32 ByteString::IndexOfAny(const Array<ByteStringView>& strings,
                             CaseSensitivity casesense, int32 from,
                             int32* matched_index, int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.ConstData(), strings.Count(),
                                  casesense, from, matched_index, matched_len);
}
int32 ByteString::IndexOfAny(const Array<AsciiString>& strings,
                             CaseSensitivity casesense, int32 from,
                             int32* matched_index, int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.ConstData(), strings.Count(),
                                  casesense, from, matched_index, matched_len);
...
int32 ByteString::IndexOfAny(std::initializer_list<const char*> strings,
                             CaseSensitivity casesense, int32 from,
                             int32* matched_index, int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.begin(), strings.size(),
                                  casesense, from, matched_index, matched_len);
}
int32 ByteString::IndexOfAny(std::initializer_list<ByteString> strings,
                             CaseSensitivity casesense, int32 from,
                             int32* matched_index, int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.begin(), strings.size(),
                                  casesense, from, matched_index, matched_len);
}
int32 ByteString::IndexOfAny(std::initializer_list<ByteStringRef> strings,
                             CaseSensitivity casesense, int32 from,
                             int32* matched_index, int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.begin(), strings.size(),
                                  casesense, from, matched_index, matched_len);
}
int32 ByteString::IndexOfAny(std::initializer_list<ByteStringView> strings,
                             CaseSensitivity casesense, int32 from,
                             int32* matched_index, int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.begin(), strings.size(),
                                  casesense, from, matched_index, matched_len);
}
int32 ByteString::IndexOfAny(std::initializer_list<AsciiString> strings,
                             CaseSensitivity casesense, int32 from,
                             int32* matched_index, int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.begin(), strings.size(),
                                  casesense, from, matched_index, matched_len);
...
int32 ByteString::IndexOf(ByteStringView sub, CaseSensitivity casesense,
  ...
  if (sub.Len() == 1) {  // 한글자 찾기로 전환.. 이게 좀더 빠름...
    return ByteStringMatcher::FastFindChar(ConstData(), Len(), *sub.ConstData(),
                                           from, casesense);
  }
  ...
  return ByteStringMatcher::FastFind(ConstData(), this_len, from,
                                     sub.ConstData(), sub.Len(), casesense);
}

int32 ByteString::LastIndexOf(ByteStringView sub, CaseSensitivity casesense,
                              int32 from, int32* matched_len) const {
  if (matched_len) {
    *matched_len = sub.Len();
  }
  ...
  if (sub.Len() == 1) {
    return ByteStringMatcher::FastLastFindChar(
        ConstData(), Len(), *sub.ConstData(), from, casesense);
  }

  return ByteStringMatcher::FastLastFind(ConstData(), Len(), from,
                                         sub.ConstData(), sub.Len(), casesense);
}
...
ByteString ByteString::Reversed() const { return ByteString(*this).Reverse(); }
...
bool ByteString::StartsWith(ByteStringView sub,
...
bool ByteString::GlobMatch(ByteStringView pattern,
                           CaseSensitivity casesense) const {
  // TODO
  fun_check(0);
  return false;

  // return GlobA::Match(cbegin(), cend(), pattern.cbegin(), pattern.cend(),
  // casesense);
}
...
ByteString& ByteString::LeftChop(int32 len) {
  ...
  if (len > 0) {
    ResizeUninitialized(
        Len() - len);  // Resize() 함수는 Length가 음수일 경우, 0으로 취급하므로
                       // 상관 없음. 그져, 비어있는 ByteString()이 될뿐.
  }
  return *this;
}
...
ByteString ByteString::ToLower() const { return ByteString(*this).MakeLower(); }

ByteString ByteString::ToUpper() const { return ByteString(*this).MakeUpper(); }
...
ByteString& ByteString::Trim() {
  ...
  //왼쪽부터 처리하게 되면, 오프셋의 변화가 오게 되므로, 오른쪽 부터 처리하도록
  //함.
  if (r_space_count) {
...
ByteString& ByteString::Simplify() { return (*this = Simplified()); }

ByteString ByteString::Simplified() const {
  return StringAlgo<const ByteString>::Simplified(*this);
}

ByteString ByteString::LeftJustified(int32 width, char filler,
                                     bool truncate) const {
  fun_check(width >= 0);
...
ByteString ByteString::RightJustified(int32 width, char filler,
                                      bool truncate) const {
  fun_check(width >= 0);
...
ByteString& ByteString::Prepend(char ch) { return Insert(0, ch); }

ByteString& ByteString::Prepend(int32 count, char ch) {
  return Insert(0, count, ch);
}

ByteString& ByteString::Prepend(ByteStringView str) { return Insert(0, str); }
...
int32 ByteString::TryFindAndRemove(ByteStringView sub,
...
ByteString& ByteString::Replace(int32 before_pos, int32 old_len,
                                ByteStringView after) {
  if (old_len == after.Len() &&
      (before_pos + old_len <=
       Len())) {  // If the contents before and after the change are the same
                  // length and are in the range, overwrite
    Detach();
...
int32 ByteString::TryReplace(ByteStringView before, ByteStringView after,
                             CaseSensitivity casesense) {
...
Array<ByteString> ByteString::Split(char separator, int32 max_splits,
                                    StringSplitOptions split_options,
                                    CaseSensitivity casesense) const {
  Array<ByteString> list;
  Split(list, separator, max_splits, split_options, casesense);
  return list;
}
Array<ByteString> ByteString::Split(const Array<char>& separators,
                                    int32 max_splits,
                                    StringSplitOptions split_options,
                                    CaseSensitivity casesense) const {
  Array<ByteString> list;
...
Array<ByteString> ByteString::Split(std::initializer_list<char> separators,
                                    int32 max_splits,
                                    StringSplitOptions split_options,
                                    CaseSensitivity casesense) const {
  Array<ByteString> list;
...
Array<ByteString> ByteString::Split(ByteStringView separators, int32 max_splits,
                                    StringSplitOptions split_options,
                                    CaseSensitivity casesense) const {
  Array<ByteString> list;
  Split(list, separators, max_splits, split_options, casesense);
...
Array<ByteString> ByteString::Split(AsciiString separators, int32 max_splits,
                                    StringSplitOptions split_options,
                                    CaseSensitivity casesense) const {
  Array<ByteString> list;
  Split(list, separators, max_splits, split_options, casesense);
...
Array<ByteString> ByteString::Split(const Array<const char*>& separators,
                                    int32 max_splits,
                                    StringSplitOptions split_options,
                                    CaseSensitivity casesense) const {
  Array<ByteString> list;
...
Array<ByteString> ByteString::Split(const Array<ByteString>& separators,
                                    int32 max_splits,
                                    StringSplitOptions split_options,
                                    CaseSensitivity casesense) const {
  Array<ByteString> list;
...
Array<ByteString> ByteString::Split(const Array<ByteStringRef>& separators,
                                    int32 max_splits,
                                    StringSplitOptions split_options,
                                    CaseSensitivity casesense) const {
  Array<ByteString> list;
...
Array<ByteString> ByteString::Split(const Array<ByteStringView>& separators,
                                    int32 max_splits,
                                    StringSplitOptions split_options,
                                    CaseSensitivity casesense) const {
  Array<ByteString> list;
...
Array<ByteString> ByteString::Split(const Array<AsciiString>& separators,
                                    int32 max_splits,
                                    StringSplitOptions split_options,
                                    CaseSensitivity casesense) const {
  Array<ByteString> list;
...
Array<ByteString> ByteString::Split(
...
Array<ByteString> ByteString::SplitByWhitespaces(
    ByteString extra_separator, int32 max_splits,
    StringSplitOptions split_options, CaseSensitivity casesense) const {
  Array<ByteString> list;
...
Array<ByteString> ByteString::SplitLines(StringSplitOptions split_options,
                                         CaseSensitivity casesense) const {
  Array<ByteString> list;
...
#if FUN_USE_IMPLICIT_STRING_CONVERSION
Array<ByteString> ByteString::Split(UNICHAR separator, int32 max_splits,
                                    StringSplitOptions split_options,
                                    CaseSensitivity casesense) const {
  Array<ByteString> list;
  Split(list, separator, max_splits, split_options, casesense);
...
Array<ByteString> ByteString::Split(const Array<UNICHAR>& separators,
                                    int32 max_splits,
                                    StringSplitOptions split_options,
                                    CaseSensitivity casesense) const {
  Array<ByteString> list;
...
Array<ByteString> ByteString::Split(std::initializer_list<UNICHAR> separators,
                                    int32 max_splits,
                                    StringSplitOptions split_options,
                                    CaseSensitivity casesense) const {
  Array<ByteString> list;
...
Array<ByteString> ByteString::Split(UStringView separators, int32 max_splits,
                                    StringSplitOptions split_options,
                                    CaseSensitivity casesense) const {
  Array<ByteString> list;
  Split(list, separators, max_splits, split_options, casesense);
...
Array<ByteString> ByteString::Split(const Array<const UNICHAR*>& separators,
                                    int32 max_splits,
                                    StringSplitOptions split_options,
                                    CaseSensitivity casesense) const {
  Array<ByteString> list;
...
Array<ByteString> ByteString::Split(const Array<UString>& separators,
                                    int32 max_splits,
                                    StringSplitOptions split_options,
                                    CaseSensitivity casesense) const {
  Array<ByteString> list;
...
Array<ByteString> ByteString::Split(const Array<UStringRef>& separators,
                                    int32 max_splits,
                                    StringSplitOptions split_options,
                                    CaseSensitivity casesense) const {
  Array<ByteString> list;
...
Array<ByteString> ByteString::Split(const Array<UStringView>& separators,
                                    int32 max_splits,
                                    StringSplitOptions split_options,
                                    CaseSensitivity casesense) const {
  Array<ByteString> list;
...
Array<ByteString> ByteString::Split(std::initializer_list<UString> separators,
                                    int32 max_splits,
                                    StringSplitOptions split_options,
                                    CaseSensitivity casesense) const {
  Array<ByteString> list;
...
int32 ByteString::Split(Array<ByteString>& list, char separator,
                        int32 max_splits, StringSplitOptions split_options,
                        CaseSensitivity casesense) const {
  return SplitByCharacters<ByteString, ByteString, ByteStringView>(
      list, *this, &ByteString::Mid, ByteStringView(&separator, 1), max_splits,
      split_options, casesense);
}
int32 ByteString::Split(Array<ByteString>& list, const Array<char>& separators,
                        int32 max_splits, StringSplitOptions split_options,
                        CaseSensitivity casesense) const {
  return SplitByCharacters<ByteString, ByteString, ByteStringView>(
      list, *this, &ByteString::Mid,
      ByteStringView(separators.ConstData(), separators.Count()), max_splits,
      split_options, casesense);
}
int32 ByteString::Split(Array<ByteString>& list,
                        std::initializer_list<char> separators,
                        int32 max_splits, StringSplitOptions split_options,
                        CaseSensitivity casesense) const {
  return SplitByCharacters<ByteString, ByteString, ByteStringView>(
      list, *this, &ByteString::Mid,
      ByteStringView(separators.begin(), separators.size()), max_splits,
      split_options, casesense);
}
int32 ByteString::Split(Array<ByteString>& list, ByteStringView separators,
                        int32 max_splits, StringSplitOptions split_options,
                        CaseSensitivity casesense) const {
  return SplitByCharacters<ByteString, ByteString, ByteStringView>(
      list, *this, &ByteString::Mid, separators, max_splits, split_options,
      casesense);
}
int32 ByteString::Split(Array<ByteString>& list, AsciiString separators,
                        int32 max_splits, StringSplitOptions split_options,
                        CaseSensitivity casesense) const {
  return SplitByCharacters<ByteString, ByteString, ByteStringView>(
      list, *this, &ByteString::Mid,
      ByteStringView(separators.ConstData(), separators.Len()), max_splits,
      split_options, casesense);
}
...
int32 ByteString::Split(Array<ByteString>& list,
                        ...
                        StringSplitOptions split_options,
                        CaseSensitivity casesense) const {
  return SplitByStrings<ByteString, ByteString, const char*>(
      list, *this, &ByteString::Mid, separators.ConstData(), separators.Count(),
...
int32 ByteString::Split(Array<ByteString>& list,
                        ...
                        StringSplitOptions split_options,
                        CaseSensitivity casesense) const {
  return SplitByStrings<ByteString, ByteString, ByteString>(
      list, *this, &ByteString::Mid, separators.ConstData(), separators.Count(),
      max_splits, split_options, casesense);
}
int32 ByteString::Split(Array<ByteString>& list,
                        ...
                        int32 max_splits, StringSplitOptions split_options,
                        CaseSensitivity casesense) const {
  return SplitByStrings<ByteString, ByteString, ByteStringRef>(
      list, *this, &ByteString::Mid, separators.ConstData(), separators.Count(),
      max_splits, split_options, casesense);
}
int32 ByteString::Split(Array<ByteString>& list,
                        ...
                        int32 max_splits, StringSplitOptions split_options,
                        CaseSensitivity casesense) const {
  return SplitByStrings<ByteString, ByteString, ByteStringView>(
      list, *this, &ByteString::Mid, separators.ConstData(), separators.Count(),
      max_splits, split_options, casesense);
}
int32 ByteString::Split(Array<ByteString>& list,
                        ...
                        StringSplitOptions split_options,
                        CaseSensitivity casesense) const {
  return SplitByStrings<ByteString, ByteString, AsciiString>(
      list, *this, &ByteString::Mid, separators.ConstData(), separators.Count(),
      max_splits, split_options, casesense);
}
...
int32 ByteString::Split(Array<ByteString>& list,
                        ...
                        CaseSensitivity casesense) const {
  return SplitByStrings<ByteString, ByteString, const char*>(
      list, *this, &ByteString::Mid, separators.begin(), separators.size(),
      max_splits, split_options, casesense);
}
int32 ByteString::Split(Array<ByteString>& list,
...
int32 ByteString::SplitByWhitespaces(Array<ByteString>& list,
                                     ByteString extra_separator,
                                     int32 max_splits,
                                     StringSplitOptions split_options,
                                     CaseSensitivity casesense) const {
  if (extra_separator.IsEmpty()) {
    auto separators = {ByteStringView(" "), ByteStringView("\t"),
                       ByteStringView("\r"), ByteStringView("\n")};
    return Split(list, separators, max_splits, split_options, casesense);
  } else {
    auto separators = {ByteStringView(" "), ByteStringView("\t"),
                       ByteStringView("\r"), ByteStringView(extra_separator)};
    return Split(list, separators, max_splits, split_options, casesense);
  }
}

int32 ByteString::SplitLines(Array<ByteString>& out_lines,
                             StringSplitOptions split_options,
                             CaseSensitivity casesense) const {
...
#if FUN_USE_IMPLICIT_STRING_CONVERSION
int32 ByteString::Split(Array<ByteString>& list, UNICHAR separator,
                        int32 max_splits, StringSplitOptions split_options,
                        CaseSensitivity casesense) const {
  return SplitByCharacters<ByteString, ByteString, UStringView>(
      list, *this, &ByteString::Mid, UStringView(&separator, 1), max_splits,
      split_options, casesense);
}
int32 ByteString::Split(Array<ByteString>& list,
...
int32 ByteString::Split(Array<ByteString>& list, UStringView separators,
                        int32 max_splits, StringSplitOptions split_options,
                        CaseSensitivity casesense) const {
  return SplitByCharacters<ByteString, ByteString, UStringView>(
      list, *this, &ByteString::Mid, separators, max_splits, split_options,
...
int32 ByteString::Split(Array<ByteString>& list,
                        ...
                        int32 max_splits, StringSplitOptions split_options,
                        CaseSensitivity casesense) const {
  return SplitByStrings<ByteString, ByteString, const UNICHAR*>(
      list, *this, &ByteString::Mid, separators.ConstData(), separators.Count(),
      max_splits, split_options, casesense);
}
int32 ByteString::Split(Array<ByteString>& list,
                        ...
                        StringSplitOptions split_options,
                        CaseSensitivity casesense) const {
  return SplitByStrings<ByteString, ByteString, UString>(
      list, *this, &ByteString::Mid, separators.ConstData(), separators.Count(),
      max_splits, split_options, casesense);
}
int32 ByteString::Split(Array<ByteString>& list,
                        ...
                        StringSplitOptions split_options,
                        CaseSensitivity casesense) const {
  return SplitByStrings<ByteString, ByteString, UStringRef>(
      list, *this, &ByteString::Mid, separators.ConstData(), separators.Count(),
      max_splits, split_options, casesense);
}
int32 ByteString::Split(Array<ByteString>& list,
                        ...
                        StringSplitOptions split_options,
                        CaseSensitivity casesense) const {
  return SplitByStrings<ByteString, ByteString, UStringView>(
      list, *this, &ByteString::Mid, separators.ConstData(), separators.Count(),
      max_splits, split_options, casesense);
}
...
Array<ByteStringRef> ByteString::SplitRef(char separator, int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
...
Array<ByteStringRef> ByteString::SplitRef(const Array<char>& separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteString::SplitRef(
    std::initializer_list<char> separators, int32 max_splits,
    StringSplitOptions split_options, CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
  SplitRef(list, separators, max_splits, split_options, casesense);
  return list;
...
Array<ByteStringRef> ByteString::SplitRef(ByteStringView separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteString::SplitRef(AsciiString separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteString::SplitRef(const Array<const char*>& separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteString::SplitRef(const Array<ByteString>& separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteString::SplitRef(const Array<AsciiString>& separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteString::SplitByWhitespacesRef(
    ByteString extra_separator, int32 max_splits,
    StringSplitOptions split_options, CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteString::SplitLinesRef(
    StringSplitOptions split_options, CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
#if FUN_USE_IMPLICIT_STRING_CONVERSION
Array<ByteStringRef> ByteString::SplitRef(UNICHAR separator, int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
  SplitRef(list, separator, max_splits, split_options, casesense);
...
Array<ByteStringRef> ByteString::SplitRef(const Array<UNICHAR>& separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteString::SplitRef(
    std::initializer_list<UNICHAR> separators, int32 max_splits,
    StringSplitOptions split_options, CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
  SplitRef(list, separators, max_splits, split_options, casesense);
  return list;
...
Array<ByteStringRef> ByteString::SplitRef(UStringView separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteString::SplitRef(const Array<UString>& separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteString::SplitRef(const Array<UStringRef>& separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteString::SplitRef(const Array<UStringView>& separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteString::SplitRef(
    std::initializer_list<UString> separators, int32 max_splits,
    StringSplitOptions split_options, CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
  SplitRef(list, separators, max_splits, split_options, casesense);
  return list;
...
int32 ByteString::SplitRef(Array<ByteStringRef>& list, char separator,
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByCharacters<ByteStringRef, ByteString, ByteStringView>(
      list, *this, &ByteString::MidRef, ByteStringView(&separator, 1),
      max_splits, split_options, casesense);
}
int32 ByteString::SplitRef(Array<ByteStringRef>& list,
                           const Array<char>& separators, int32 max_splits,
                           StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByCharacters<ByteStringRef, ByteString, ByteStringView>(
      list, *this, &ByteString::MidRef,
      ByteStringView(separators.ConstData(), separators.Count()), max_splits,
      split_options, casesense);
}
int32 ByteString::SplitRef(Array<ByteStringRef>& list,
...
int32 ByteString::SplitRef(Array<ByteStringRef>& list,
                           ByteStringView separators, int32 max_splits,
                           StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByCharacters<ByteStringRef, ByteString, ByteStringView>(
      list, *this, &ByteString::MidRef, separators, max_splits, split_options,
      casesense);
}
int32 ByteString::SplitRef(Array<ByteStringRef>& list, AsciiString separators,
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByCharacters<ByteStringRef, ByteString, ByteStringView>(
      list, *this, &ByteString::MidRef,
...
int32 ByteString::SplitRef(Array<ByteStringRef>& list,
                           const Array<const char*>& separators,
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByStrings<ByteStringRef, ByteString, const char*>(
      list, *this, &ByteString::MidRef, separators.ConstData(),
      separators.Count(), max_splits, split_options, casesense);
}
int32 ByteString::SplitRef(Array<ByteStringRef>& list,
                           const Array<ByteString>& separators,
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByStrings<ByteStringRef, ByteString, ByteString>(
      list, *this, &ByteString::MidRef, separators.ConstData(),
      separators.Count(), max_splits, split_options, casesense);
}
int32 ByteString::SplitRef(Array<ByteStringRef>& list,
                           ...
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByStrings<ByteStringRef, ByteString, ByteStringRef>(
      list, *this, &ByteString::MidRef, separators.ConstData(),
      separators.Count(), max_splits, split_options, casesense);
}
int32 ByteString::SplitRef(Array<ByteStringRef>& list,
                           ...
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByStrings<ByteStringRef, ByteString, ByteStringView>(
      list, *this, &ByteString::MidRef, separators.ConstData(),
      separators.Count(), max_splits, split_options, casesense);
}
int32 ByteString::SplitRef(Array<ByteStringRef>& list,
                           const Array<AsciiString>& separators,
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByStrings<ByteStringRef, ByteString, AsciiString>(
      list, *this, &ByteString::MidRef, separators.ConstData(),
      separators.Count(), max_splits, split_options, casesense);
}
...
int32 ByteString::SplitRef(Array<ByteStringRef>& list,
                           ...
                           CaseSensitivity casesense) const {
  return SplitByStrings<ByteStringRef, ByteString, const char*>(
      list, *this, &ByteString::MidRef, separators.begin(), separators.size(),
      max_splits, split_options, casesense);
}
int32 ByteString::SplitRef(Array<ByteStringRef>& list,
                           std::initializer_list<ByteString> separators,
...
int32 ByteString::SplitByWhitespacesRef(Array<ByteStringRef>& list,
                                        ByteString extra_separator,
                                        int32 max_splits,
                                        StringSplitOptions split_options,
                                        CaseSensitivity casesense) const {
...
int32 ByteString::SplitLinesRef(Array<ByteStringRef>& out_lines,
                                StringSplitOptions split_options,
                                CaseSensitivity casesense) const {
...
#if FUN_USE_IMPLICIT_STRING_CONVERSION
int32 ByteString::SplitRef(Array<ByteStringRef>& list, UNICHAR separator,
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByCharacters<ByteStringRef, ByteString, UStringView>(
      list, *this, &ByteString::MidRef, UStringView(&separator, 1), max_splits,
      split_options, casesense);
}
int32 ByteString::SplitRef(Array<ByteStringRef>& list,
                           const Array<UNICHAR>& separators, int32 max_splits,
...
int32 ByteString::SplitRef(Array<ByteStringRef>& list, UStringView separators,
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByCharacters<ByteStringRef, ByteString, UStringView>(
      list, *this, &ByteString::MidRef, separators, max_splits, split_options,
...
int32 ByteString::SplitRef(Array<ByteStringRef>& list,
                           ...
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByStrings<ByteStringRef, ByteString, const UNICHAR*>(
      list, *this, &ByteString::MidRef, separators.ConstData(),
      separators.Count(), max_splits, split_options, casesense);
}
int32 ByteString::SplitRef(Array<ByteStringRef>& list,
                           ...
                           StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByStrings<ByteStringRef, ByteString, UString>(
      list, *this, &ByteString::MidRef, separators.ConstData(),
      separators.Count(), max_splits, split_options, casesense);
}
int32 ByteString::SplitRef(Array<ByteStringRef>& list,
                           const Array<UStringRef>& separators,
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByStrings<ByteStringRef, ByteString, UStringRef>(
      list, *this, &ByteString::MidRef, separators.ConstData(),
      separators.Count(), max_splits, split_options, casesense);
}
int32 ByteString::SplitRef(Array<ByteStringRef>& list,
                           const Array<UStringView>& separators,
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByStrings<ByteStringRef, ByteString, UStringView>(
      list, *this, &ByteString::MidRef, separators.ConstData(),
      separators.Count(), max_splits, split_options, casesense);
}
...
bool ByteString::Divide(ByteStringView delim, ByteString* out_left,
                        ByteString* out_right, bool trimming,
                        CaseSensitivity casesense) const {
...
bool ByteString::Divide(ByteStringView delim, ByteStringRef* out_left,
                        ByteStringRef* out_right, bool trimming,
                        CaseSensitivity casesense) const {
  const int32 pos = IndexOf(delim, casesense);
  if (pos == INVALID_INDEX) {
    return false;
...
bool ByteString::LastDivide(ByteStringView delim, ByteString* out_left,
                            ByteString* out_right, bool trimming,
                            CaseSensitivity casesense) const {
...
bool ByteString::LastDivide(ByteStringView delim, ByteStringRef* out_left,
                            ByteStringRef* out_right, bool trimming,
                            CaseSensitivity casesense) const {
  const int32 pos = LastIndexOf(delim, casesense);
  if (pos == INVALID_INDEX) {
...
ByteString ByteString::Join(const Array<const char*>& list,
                            ByteStringView separator) {
  return StringJoin<ByteString>(list.ConstData(), list.Count(), separator);
}
ByteString ByteString::Join(const Array<ByteString>& list,
                            ByteStringView separator) {
  return StringJoin<ByteString>(list.ConstData(), list.Count(), separator);
}
ByteString ByteString::Join(const Array<ByteStringRef>& list,
                            ByteStringView separator) {
  return StringJoin<ByteString>(list.ConstData(), list.Count(), separator);
}
ByteString ByteString::Join(const Array<ByteStringView>& list,
                            ByteStringView separator) {
  return StringJoin<ByteString>(list.ConstData(), list.Count(), separator);
}
ByteString ByteString::Join(const Array<AsciiString>& list,
                            ByteStringView separator) {
  return StringJoin<ByteString>(list.ConstData(), list.Count(), separator);
}
#if FUN_USE_IMPLICIT_STRING_CONVERSION
ByteString ByteString::Join(const Array<const UNICHAR*>& list,
                            ByteStringView separator) {
  return StringJoin<ByteString>(list.ConstData(), list.Count(), separator);
}
ByteString ByteString::Join(const Array<UString>& list,
                            ByteStringView separator) {
  return StringJoin<ByteString>(list.ConstData(), list.Count(), separator);
}
ByteString ByteString::Join(const Array<UStringRef>& list,
                            ByteStringView separator) {
  return StringJoin<ByteString>(list.ConstData(), list.Count(), separator);
}
ByteString ByteString::Join(const Array<UStringView>& list,
                            ByteStringView separator) {
  return StringJoin<ByteString>(list.ConstData(), list.Count(), separator);
}
#endif

ByteString ByteString::Join(std::initializer_list<const char*> list,
                            ByteStringView separator) {
  return StringJoin<ByteString>(list.begin(), list.size(), separator);
}
ByteString ByteString::Join(std::initializer_list<ByteString> list,
                            ByteStringView separator) {
  return StringJoin<ByteString>(list.begin(), list.size(), separator);
}
ByteString ByteString::Join(std::initializer_list<ByteStringRef> list,
                            ByteStringView separator) {
  return StringJoin<ByteString>(list.begin(), list.size(), separator);
}
ByteString ByteString::Join(std::initializer_list<ByteStringView> list,
                            ByteStringView separator) {
  return StringJoin<ByteString>(list.begin(), list.size(), separator);
}
ByteString ByteString::Join(std::initializer_list<AsciiString> list,
                            ByteStringView separator) {
  return StringJoin<ByteString>(list.begin(), list.size(), separator);
}
#if FUN_USE_IMPLICIT_STRING_CONVERSION
ByteString ByteString::Join(std::initializer_list<const UNICHAR*> list,
                            ByteStringView separator) {
  return StringJoin<ByteString>(list.begin(), list.size(), separator);
}
ByteString ByteString::Join(std::initializer_list<UString> list,
                            ByteStringView separator) {
  return StringJoin<ByteString>(list.begin(), list.size(), separator);
}
ByteString ByteString::Join(std::initializer_list<UStringRef> list,
                            ByteStringView separator) {
  return StringJoin<ByteString>(list.begin(), list.size(), separator);
}
ByteString ByteString::Join(std::initializer_list<UStringView> list,
                            ByteStringView separator) {
  return StringJoin<ByteString>(list.begin(), list.size(), separator);
}
#endif
...
ByteString ByteString::operator*(int32 times) const { return Repeated(times); }

namespace {

int64 ToIntegral_helper(const char* str, int32 len, bool* ok, int32 base,
                        int64 /*constrain*/) {
...
template <typename T>
FUN_ALWAYS_INLINE T ToIntegral(const char* str, int32 len, bool* ok,
                               int32 base) {
  const bool IS_UNSIGNED = T(0) < T(-1);
  typedef typename Conditional<IS_UNSIGNED, uint64, int64>::Result Int64;
  if (base != 0 && (base < 2 || base > 36)) {
    // TODO
    // fun_log(LogCore, Warning, "ByteString::ToIntegral: Invalid base {0}",
    // base);
    base = 10;
  }
...
double ByteString::ToDouble(bool* ok) const {
  ...
  const double double_value =
      AsciiToDouble(ConstData(), Len(), non_null_ok, processed);
  if (ok) {
    *ok = non_null_ok;
  }
  return double_value;
...
char* __ulltoa(char* p, uint64 n, int32 base) {
  if (base < 2 || base > 36) {
    // TODO
    // fun_log(LogCore, Warning, "ByteString::SetNumber: Invalid base {0}",
    // base);
    base = 10;
  }
...
ByteString& ByteString::SetNumber(double value, char f, int32 precision) {
  ...
  switch (f) {
    ...
    default:
      // TODO
      // fun_log(LogCore, Warning, "ByteString::SetNumber: Invalid format char
      // '%c'", f);
      break;
  }

  *this = WCHAR_TO_UTF8(
      *LocaleData::C()->DoubleToString(value, precision, form, -1, flags));
  return *this;
}
...
ByteString ByteString::ToBase64() const { return ToBase64(Base64Encoding); }
...
ByteString ByteString::ToBase64(Base64Options options) const {
  const char ALPHABET_BASE64[] =
      ...
      "wxyz0123"
      "456789+/";
  const char ALPHABET_BASE64_URL[] =
      "ABCDEFGH"
      "IJKLMNOP"
      "QRSTUVWX"
      "YZabcdef"
  ...
  const char* const ALPHABET =
      options & Base64UrlEncoding ? ALPHABET_BASE64_URL : ALPHABET_BASE64;
  const char pad_char = '=';
  int32 pad_len = 0;
  ...
  fun_check((options & OmitTrailingEquals) ||
            (dst == tmp.Len() + tmp.ConstData()));
...
ByteString ByteString::ToHex() const { return ToHex('\0'); }
...
ByteString ByteString::FromPercentEncoding(const ByteString& pct_encoded,
                                           char percent) {
...
void ToPercentEncoding_helper(ByteString* str, const char* dont_encode,
                              const char* also_encode, char percent) {
  if (str->IsEmpty()) {
    return;
  }
...
ByteString ByteString::ToPercentEncoding(const ByteString& exclude,
  ...
  ByteString result = *this;
  ToPercentEncoding_helper(&result, *exclude.ToNulTerminated(),
                           *include2.ToNulTerminated(), percent);
  return result;
}
...
bool ByteString::IsNumeric() const {
  return !IsEmpty() &&
         CStringTraitsA::IsNumeric(ConstData(), ConstData() + Len());
}
...
ByteString ByteString::ReplaceCharWithEscapedChar(
    const Array<char>* chars) const {
...
ByteString ByteString::ReplaceEscapedCharWithChar(
    const Array<char>* chars) const {
  if (Len() > 0 && (chars == nullptr || chars->Count() > 0)) {
    ByteString result(*this);
...
std::string ByteString::ToStdString() const {
  return std::string(ConstData(),
                     Len());  // 길이를 가지고 가므로, nul-term 일 필요는 없음.
}

ByteString ByteString::FromStdString(const std::string& std_string) {
  return ByteString(std_string.data(),
                    int32(std_string.size()));  // 길이를 가지고 가므로, 중간에
                                                // nul 문자가 있어도 문제 없음.
}
...
ByteStringRef::ByteStringRef() : string_(nullptr), position_(0), length_(0) {}
...
const ByteString* ByteStringRef::Str() const { return string_; }

int32 ByteStringRef::Position() const { return position_; }

int32 ByteStringRef::Len() const { return length_; }

bool ByteStringRef::IsEmpty() const { return length_ == 0; }
...
bool ByteStringRef::IsNull() const { return string_ == nullptr; }

const char* ByteStringRef::operator*() const { return ConstData(); }
...
ByteString ByteStringRef::ToString() const {
  return string_ ? ByteString(string_->ConstData() + position_, length_)
                 : ByteString();
}
...
char ByteStringRef::First() const { return At(0); }

char ByteStringRef::Last() const { return At(Len() - 1); }
...
int32 ByteStringRef::IndexOf(ByteStringView sub, CaseSensitivity casesense,
                             int32 from, int32* matched_len) const {
  if (matched_len) {
    *matched_len = sub.Len();
  }
...
int32 ByteStringRef::LastIndexOf(ByteStringView sub, CaseSensitivity casesense,
                                 int32 from, int32* matched_len) const {
  if (matched_len) {
    *matched_len = sub.Len();
  }
...
int32 ByteStringRef::IndexOfAny(const Array<const char*>& strings,
                                CaseSensitivity casesense, int32 from,
                                int32* matched_index,
                                int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.ConstData(), strings.Count(),
...
int32 ByteStringRef::IndexOfAny(const Array<ByteString>& strings,
                                CaseSensitivity casesense, int32 from,
                                int32* matched_index,
                                int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.ConstData(), strings.Count(),
...
int32 ByteStringRef::IndexOfAny(const Array<ByteStringRef>& strings,
                                CaseSensitivity casesense, int32 from,
                                int32* matched_index,
                                int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.ConstData(), strings.Count(),
...
int32 ByteStringRef::IndexOfAny(const Array<ByteStringView>& strings,
                                CaseSensitivity casesense, int32 from,
                                int32* matched_index,
                                int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.ConstData(), strings.Count(),
...
int32 ByteStringRef::IndexOfAny(const Array<AsciiString>& strings,
                                CaseSensitivity casesense, int32 from,
                                int32* matched_index,
                                int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.ConstData(), strings.Count(),
...
int32 ByteStringRef::IndexOfAny(std::initializer_list<const char*> strings,
                                CaseSensitivity casesense, int32 from,
                                int32* matched_index,
                                int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.begin(), strings.size(),
...
int32 ByteStringRef::IndexOfAny(std::initializer_list<ByteString> strings,
                                CaseSensitivity casesense, int32 from,
                                int32* matched_index,
                                int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.begin(), strings.size(),
...
int32 ByteStringRef::IndexOfAny(std::initializer_list<ByteStringRef> strings,
                                CaseSensitivity casesense, int32 from,
                                int32* matched_index,
                                int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.begin(), strings.size(),
...
int32 ByteStringRef::IndexOfAny(std::initializer_list<ByteStringView> strings,
                                CaseSensitivity casesense, int32 from,
                                int32* matched_index,
                                int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.begin(), strings.size(),
...
int32 ByteStringRef::IndexOfAny(std::initializer_list<AsciiString> strings,
                                CaseSensitivity casesense, int32 from,
                                int32* matched_index,
                                int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.begin(), strings.size(),
...
#if FUN_USE_IMPLICIT_STRING_CONVERSION
int32 ByteStringRef::IndexOfAny(const Array<const UNICHAR*>& strings,
                                CaseSensitivity casesense, int32 from,
                                int32* matched_index,
                                int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.ConstData(), strings.Count(),
...
int32 ByteStringRef::IndexOfAny(const Array<UString>& strings,
                                CaseSensitivity casesense, int32 from,
                                int32* matched_index,
                                int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.ConstData(), strings.Count(),
...
int32 ByteStringRef::IndexOfAny(const Array<UStringRef>& strings,
                                CaseSensitivity casesense, int32 from,
                                int32* matched_index,
                                int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.ConstData(), strings.Count(),
...
int32 ByteStringRef::IndexOfAny(const Array<UStringView>& strings,
                                CaseSensitivity casesense, int32 from,
                                int32* matched_index,
                                int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.ConstData(), strings.Count(),
...
int32 ByteStringRef::IndexOfAny(std::initializer_list<const UNICHAR*> strings,
                                CaseSensitivity casesense, int32 from,
                                int32* matched_index,
                                int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.begin(), strings.size(),
...
int32 ByteStringRef::IndexOfAny(std::initializer_list<UString> strings,
                                CaseSensitivity casesense, int32 from,
                                int32* matched_index,
                                int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.begin(), strings.size(),
...
int32 ByteStringRef::IndexOfAny(std::initializer_list<UStringRef> strings,
                                CaseSensitivity casesense, int32 from,
                                int32* matched_index,
                                int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.begin(), strings.size(),
...
int32 ByteStringRef::IndexOfAny(std::initializer_list<UStringView> strings,
                                CaseSensitivity casesense, int32 from,
                                int32* matched_index,
                                int32* matched_len) const {
  return IndexOfAnyStrings_helper(*this, strings.begin(), strings.size(),
...
int32 ByteStringRef::Count(ByteStringView sub,
                           CaseSensitivity casesense) const {
  int32 n = 0, pos = -int32(sub.Len());
...
ByteStringRef ByteStringRef::Left(int32 len) const {
  fun_check(len >= 0);
  fun_check(len <= Len());
  return uint32(len) < uint32(length_) ? ByteStringRef(string_, position_, len)
                                       : *this;
}
...
ByteStringRef ByteStringRef::Right(int32 len) const {
  ...
  return uint32(len) < uint32(length_)
             ? ByteStringRef(string_, position_ + length_ - len, len)
             : *this;
}
...
bool ByteStringRef::StartsWith(ByteStringView sub,
                               CaseSensitivity casesense) const {
  if (sub.IsEmpty()) {
    return true;
  }
  ...
  return StringCmp::Compare(ConstData(), sub.ConstData(), sub.Len(),
                            casesense) == 0;
}

bool ByteStringRef::EndsWith(ByteStringView sub,
                             CaseSensitivity casesense) const {
  if (sub.IsEmpty()) {
    return true;
  }
  ...
  return StringCmp::Compare(ConstData() + Len() - sub.Len(), sub.ConstData(),
                            sub.Len(), casesense) == 0;
}

bool ByteStringRef::GlobMatch(ByteStringView pattern,
                              CaseSensitivity casesense) const {
  // TODO
  fun_check(0);
  return false;
...
ByteStringRef& ByteStringRef::TrimRight() {
  if (string_) {
    ...
    const char* local_begin = ConstData();
    const char* local_end = ConstData() + length_;
    length_ = StringAlgo<ByteString>::RightTrimmedPositions(
...
ByteStringRef ByteStringRef::TrimmedLeft() const {
  if (string_) {
    ...
    const char* local_end = ConstData() + length_;
    StringAlgo<ByteString>::LeftTrimmedPositions(local_begin, local_end);
    return ByteStringRef(string_, local_begin - abs_begin,
...
Array<ByteStringRef> ByteStringRef::Split(char separator, int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
  Split(list, separator, max_splits, split_options, casesense);
...
Array<ByteStringRef> ByteStringRef::Split(const Array<char>& separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteStringRef::Split(
    std::initializer_list<char> separators, int32 max_splits,
    StringSplitOptions split_options, CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
  Split(list, separators, max_splits, split_options, casesense);
...
Array<ByteStringRef> ByteStringRef::Split(ByteStringView separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteStringRef::Split(AsciiString separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteStringRef::Split(const Array<const char*>& separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteStringRef::Split(const Array<ByteString>& separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteStringRef::Split(const Array<AsciiString>& separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteStringRef::SplitByWhitespaces(
    ByteString extra_separator, int32 max_splits,
    StringSplitOptions split_options, CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
  SplitByWhitespaces(list, extra_separator, max_splits, split_options,
...
Array<ByteStringRef> ByteStringRef::SplitLines(
    StringSplitOptions split_options, CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
  SplitLines(list, split_options, casesense);
  return list;
...
#if FUN_USE_IMPLICIT_STRING_CONVERSION
Array<ByteStringRef> ByteStringRef::Split(UNICHAR separator, int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
  Split(list, separator, max_splits, split_options, casesense);
...
Array<ByteStringRef> ByteStringRef::Split(const Array<UNICHAR>& separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteStringRef::Split(UStringView separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteStringRef::Split(const Array<UString>& separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteStringRef::Split(const Array<UStringRef>& separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
Array<ByteStringRef> ByteStringRef::Split(const Array<UStringView>& separators,
                                          int32 max_splits,
                                          StringSplitOptions split_options,
                                          CaseSensitivity casesense) const {
  Array<ByteStringRef> list;
...
int32 ByteStringRef::Split(Array<ByteStringRef>& list, char separator,
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByCharacters<ByteStringRef, ByteStringRef, ByteStringView>(
      list, *this, &ByteStringRef::Mid, ByteStringView(&separator, 1),
      max_splits, split_options, casesense);
}
int32 ByteStringRef::Split(Array<ByteStringRef>& list,
                           const Array<char>& separators, int32 max_splits,
                           StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByCharacters<ByteStringRef, ByteStringRef, ByteStringView>(
      list, *this, &ByteStringRef::Mid,
      ByteStringView(separators.ConstData(), separators.Count()), max_splits,
      split_options, casesense);
}
int32 ByteStringRef::Split(Array<ByteStringRef>& list,
...
int32 ByteStringRef::Split(Array<ByteStringRef>& list,
                           ...
                           CaseSensitivity casesense) const {
  return SplitByCharacters<ByteStringRef, ByteStringRef, ByteStringView>(
      list, *this, &ByteStringRef::Mid, separators, max_splits, split_options,
      casesense);
}
int32 ByteStringRef::Split(Array<ByteStringRef>& list, AsciiString separators,
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByCharacters<ByteStringRef, ByteStringRef, ByteStringView>(
      list, *this, &ByteStringRef::Mid,
...
int32 ByteStringRef::Split(Array<ByteStringRef>& list,
                           ...
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByStrings<ByteStringRef, ByteStringRef, const char*>(
      list, *this, &ByteStringRef::Mid, separators.ConstData(),
      separators.Count(), max_splits, split_options, casesense);
}
int32 ByteStringRef::Split(Array<ByteStringRef>& list,
                           const Array<ByteString>& separators,
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByStrings<ByteStringRef, ByteStringRef, ByteString>(
      list, *this, &ByteStringRef::Mid, separators.ConstData(),
      separators.Count(), max_splits, split_options, casesense);
}
int32 ByteStringRef::Split(Array<ByteStringRef>& list,
                           ...
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByStrings<ByteStringRef, ByteStringRef, ByteStringRef>(
      list, *this, &ByteStringRef::Mid, separators.ConstData(),
      separators.Count(), max_splits, split_options, casesense);
}
int32 ByteStringRef::Split(Array<ByteStringRef>& list,
                           ...
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByStrings<ByteStringRef, ByteStringRef, ByteStringView>(
      list, *this, &ByteStringRef::Mid, separators.ConstData(),
      separators.Count(), max_splits, split_options, casesense);
}
int32 ByteStringRef::Split(Array<ByteStringRef>& list,
                           ...
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByStrings<ByteStringRef, ByteStringRef, AsciiString>(
      list, *this, &ByteStringRef::Mid, separators.ConstData(),
      separators.Count(), max_splits, split_options, casesense);
}
...
int32 ByteStringRef::Split(Array<ByteStringRef>& list,
                           ...
                           CaseSensitivity casesense) const {
  return SplitByStrings<ByteStringRef, ByteStringRef, const char*>(
      list, *this, &ByteStringRef::Mid, separators.begin(), separators.size(),
      max_splits, split_options, casesense);
}
int32 ByteStringRef::Split(Array<ByteStringRef>& list,
                           std::initializer_list<ByteString> separators,
...
int32 ByteStringRef::SplitByWhitespaces(Array<ByteStringRef>& list,
                                        ByteString extra_separator,
                                        int32 max_splits,
                                        StringSplitOptions split_options,
                                        CaseSensitivity casesense) const {
...
int32 ByteStringRef::SplitLines(Array<ByteStringRef>& out_lines,
                                StringSplitOptions split_options,
                                CaseSensitivity casesense) const {
  auto separators = {ByteStringView("\r\n"), ByteStringView("\n"),
                     ByteStringView("\r")};
...
#if FUN_USE_IMPLICIT_STRING_CONVERSION
int32 ByteStringRef::Split(Array<ByteStringRef>& list, UNICHAR separator,
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByCharacters<ByteStringRef, ByteStringRef, UStringView>(
      list, *this, &ByteStringRef::Mid, UStringView(&separator, 1), max_splits,
      split_options, casesense);
}
int32 ByteStringRef::Split(Array<ByteStringRef>& list,
                           const Array<UNICHAR>& separators, int32 max_splits,
...
int32 ByteStringRef::Split(Array<ByteStringRef>& list, UStringView separators,
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByCharacters<ByteStringRef, ByteStringRef, UStringView>(
      list, *this, &ByteStringRef::Mid, separators, max_splits, split_options,
...
int32 ByteStringRef::Split(Array<ByteStringRef>& list,
                           ...
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByStrings<ByteStringRef, ByteStringRef, const UNICHAR*>(
      list, *this, &ByteStringRef::Mid, separators.ConstData(),
      separators.Count(), max_splits, split_options, casesense);
}
int32 ByteStringRef::Split(Array<ByteStringRef>& list,
                           ...
                           StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByStrings<ByteStringRef, ByteStringRef, UString>(
      list, *this, &ByteStringRef::Mid, separators.ConstData(),
      separators.Count(), max_splits, split_options, casesense);
}
int32 ByteStringRef::Split(Array<ByteStringRef>& list,
                           ...
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByStrings<ByteStringRef, ByteStringRef, UStringRef>(
      list, *this, &ByteStringRef::Mid, separators.ConstData(),
      separators.Count(), max_splits, split_options, casesense);
}
int32 ByteStringRef::Split(Array<ByteStringRef>& list,
                           ...
                           int32 max_splits, StringSplitOptions split_options,
                           CaseSensitivity casesense) const {
  return SplitByStrings<ByteStringRef, ByteStringRef, UStringView>(
      list, *this, &ByteStringRef::Mid, separators.ConstData(),
      separators.Count(), max_splits, split_options, casesense);
}
...
bool ByteStringRef::Divide(ByteStringView delim, ByteStringRef* out_left,
                           ByteStringRef* out_right, bool trimming,
                           CaseSensitivity casesense) const {
  const int32 pos = IndexOf(delim, casesense);
  if (pos == INVALID_INDEX) {
...
bool ByteStringRef::LastDivide(ByteStringView delim, ByteStringRef* out_left,
                               ByteStringRef* out_right, bool trimming,
                               CaseSensitivity casesense) const {
  const int32 pos = LastIndexOf(delim, casesense);
  if (pos == INVALID_INDEX) {
...
bool ByteStringRef::IsQuoted() const { return ByteString::IsQuoted(*this); }
...
VARARG_BODY(ByteString, ByteString::Format, const char*, VARARG_NONE)
{
  ...
  GET_VARARGS_RESULT_ANSI(buffer, buffer_size, buffer_size - 1, fmt, fmt,
result);
...
void ByteString::BytesToDebuggableString(ByteString& output,
  ...
  for (int32 i = 0; i < data.Len(); i += width) {
    output << indent
           << ByteString::Format("%4.4lX : ", i);  // TODO fix format specifier.
    ...
    for (int32 c = 0; c < width && (i + c) < data.Len(); ++c) {
      ...
      if (ascii_only && (i + c + 1) < data.Len() && data[i + c] == 0x0D &&
          data[i + c + 1] == 0x0A) {
        i += (c + 2 - width);
        break;
      }
      ...
      const char ascii_char =
          (data[i + c] >= 0x20) && (data[i + c] < 0x80) ? data[i + c] : '.';
      output << ascii_char;
      ...
      if (ascii_only && (i + c + 2) < data.Len() && data[i + c + 1] == 0x0D &&
          data[i + c + 2] == 0x0A) {
        i += (c + 3 - width);
        break;
      }
