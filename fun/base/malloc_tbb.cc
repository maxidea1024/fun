...
#if FUN_BUILD_DEBUG && !defined(NDEBUG) // Use !defined(NDEBUG) to check to see if we actually are linking with Debug third party libraries (bDebugBuildsActuallyUseDebugCRT)
...
void* MemoryAllocatorTBB::Realloc(void* ptr, size_t new_size, uint32 alignment) {
...
bool MemoryAllocatorTBB::GetAllocationSize(void* reported_ptr, size_t& out_allocation_size) {
