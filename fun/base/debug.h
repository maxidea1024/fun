...
class FUN_BASE_API Debug {
 public:
   ...
   */
  static void Assertion(const char* cond,
                        const char* file,
                        int32 line,
 ...
 protected:
  static String What( const char* msg,
                      const char* file,
                      int32 line,
                      const char* text = nullptr);
};
...
//TODO 메시지 출력이 필요한 경우에는 format을 지원하는게 좋을듯...
  ...
  #define fun_check_dbg(cond)                do { if (!(cond)) std::abort(); } while (0);
  #define fun_check_msg_dbg(cond, msg,...)   do { if (!(cond)) std::abort(); } while (0);
  #define fun_check(cond)                    do { if (!(cond)) std::abort(); } while (0);
  #define fun_check_msg(cond, msg,...)       do { if (!(cond)) std::abort(); } while (0);
  #define fun_check_ptr(ptr)                 do { if (!(ptr)) std::abort(); } while (0);
  #define fun_bugcheck()                     do { std::abort(); } while (0);
  #define fun_bugcheck_msg(msg,...)          do { std::abort(); } while (0);

#else // defined(__clang_analyzer__)

  #if defined(_DEBUG)
    #define fun_check_dbg(cond) \
        if (!(cond)) fun::Debug::Assertion(#cond, __FILE__, __LINE__); else (void)0;
    //TODO 가변 포맷 제대로 지원하자! (현재는 컴파일 오류만 안나게 해놓은 상태임)
    #define fun_check_msg_dbg(cond,msg,...) \
        if (!(cond)) fun::Debug::Assertion(#cond, __FILE__, __LINE__, msg); else (void)0;
  #else
    #define fun_check_dbg(cond,...)
    #define fun_check_msg_dbg(cond,msg,...)
  #endif
  ...
  #define fun_check_ptr(ptr) \
    if (!(ptr)) fun::Debug::NullPointer(#ptr, __FILE__, __LINE__); else (void)0;

  #define fun_bugcheck() \
    fun::Debug::Bugcheck(__FILE__, __LINE__);
...
#define fun_unexpected() \
  fun::Debug::Unexpected(__FILE__, __LINE__);

#define fun_debugger() \
  fun::Debug::Debugger(__FILE__, __LINE__);
...
#define fun_debugger_msg(msg,...) \
  ...
  #define fun_stderr_dbg(str) \
    std::cerr << __FILE__ << '(' << std::dec << __LINE__ << "):" << str << std::endl;
#else
  #define fun_stderr_dbg(str)
#endif
