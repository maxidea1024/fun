...
FileStatData AndroidStatToFunFileData(struct stat& file_info) {
  ...
  return FileStatData(ANDROID_EPOCH + Timespan(0, 0, file_info.st_ctime),
                      ANDROID_EPOCH + Timespan(0, 0, file_info.st_atime),
                      ANDROID_EPOCH + Timespan(0, 0, file_info.st_mtime),
                      file_size, is_directory, !!(file_info.st_mode & S_IWUSR));
}
...
// This function is declared in the Java-defined class, GameActivity.java:
// "public native void nativeSetObbInfo(String package_name, int Version, int
// PatchVersion);"
extern "C" void Java_com_mycorp_jarvis_GameActivity_nativeSetObbInfo(
    JNIEnv* jenv, jobject thiz, jstring project_name, jstring package_name,
    jint version, jint patch_version) {
...
const String& GetFileBasePath() {
  static String base_path = g_file_path_base + String(FILEBASE_DIRECTORY) +
                            AppInfo::GetGameName() + String("/");
  return base_path;
}
...
class FUN_BASE_API AndroidFile : public IFile {
 ...
 public:
...
#if FUN_ANDROID_FILE_64
    file_->handle =
        AAsset_openFileDescriptor64(file_->asset, &start_, &length_);
#else
    off_t out_start = start_;
    off_t out_length = length_;
    file_->handle =
        AAsset_openFileDescriptor(file_->asset, &out_start, &out_length);
    start_ = out_start;
    length_ = out_length;
#endif
    CheckValid();
  ...
  virtual int64 Tell() override {
    ...
    fun_check(pos != -1);
    return pos - start_;  // We are treating 'tell' as a virtual location from
                          // file start_
  }
  ...
  virtual bool SeekFromEnd(int64 relative_pos_to_end = 0) override {
    ...
    // We need to convert this to a virtual offset inside the file we are
    // interested in
  ...
  virtual bool Read(uint8* dst, int64 len_to_read) override {
    ...
    PlatformMisc::LowLevelOutputDebugStringf(
        ...
        CAndroidTLS::GetCurrentThreadId(), file_->handle, *(file_->path),
        int32(len_to_read));
#endif
...
#if LOG_ANDROID_FILE
      PlatformMisc::LowLevelOutputDebugStringf(
          "(%d/%d) AndroidFile:Read => path = %s, this_size = %d, destination "
          "= %X",
          CAndroidTLS::GetCurrentThreadId(), file_->handle, *(file_->path),
          int32(this_size), dst);
#endif
  ...
  virtual int64 Size() override { return length_; }
};
...
class ManifestReader {
 ...
 public:
  ManifestReader(const String& manifest_filename)
      : manifest_filename_(manifest_filename), is_initialized_(false) {}
  ...
  bool GetFileTimestamp(const String& filename, DateTime& out_timestamp) {
    ...
    if (result) {
      ...
      PlatformMisc::LowLevelOutputDebugStringf("found time stamp for '%s', %s",
                                               *filename,
                                               *out_timestamp.ToString());
#endif
      return true;
    }
#if LOG_ANDROID_FILE_MANIFEST
    PlatformMisc::LowLevelOutputDebugStringf("Didn't find time stamp for '%s'",
                                             *filename);
#endif
    return false;
  }
...
#if LOG_ANDROID_FILE_MANIFEST
    PlatformMisc::LowLevelOutputDebugStringf("SetFileTimestamp '%s', %s",
  ...
  void Read() {
...
#if LOG_ANDROID_FILE_MANIFEST
      PlatformMisc::LowLevelOutputDebugStringf(
          "Failed to open file for read'%s'", *manifest_filename_);
#endif
      return;
    }
...
#if LOG_ANDROID_FILE_MANIFEST
    PlatformMisc::LowLevelOutputDebugStringf("Loaded manifest file %s",
                                             *manifest_filename_);
    ...
    for (const auto& line : lines) {
      ...
      if (line.Divide("\t", &filename, &date_time_str)) {
...
#if LOG_ANDROID_FILE_MANIFEST
          PlatformMisc::LowLevelOutputDebugStringf(
              "Read time stamp '%s' %s", *filename, *modified_date.ToString());
#endif
...
#if LOG_ANDROID_FILE_MANIFEST
          PlatformMisc::LowLevelOutputDebugStringf(
              "Failed to parse date for file '%s' %s", *filename,
              *date_time_str);
#endif
        }
      }
        ...
        PlatformMisc::LowLevelOutputDebugStringf("Unable to split line '%s'",
                                                 *line);
      }
#endif
    }
  ...
  void Write() {
    ...
    int handle =
        open(TCHAR_TO_UTF8(*manifest_path), O_WRONLY | O_CREAT | O_TRUNC);
    if (handle == -1) {
#if LOG_ANDROID_FILE_MANIFEST
      PlatformMisc::LowLevelOutputDebugStringf(
          "Failed to open file for write '%s'", *manifest_filename_);
#endif
      return;
    }
...
class ZipUnionFile {
 public:
  struct Entry {
    ...
    Entry(SharedPtr<AndroidFile> file, const String& filename,
          int32 mod_time = 0)
        : file(file), filename(filename), mod_time(mod_time) {}
  };
  ...
  struct Directory {
    ...
    Directory(EntryMap& _entries, const String& _path)
        : current(_entries.CreateIterator()), path(_path) {
      if (!path.IsEmpty()) {
        path /= "";
      }
  ...
  void AddPatchFile(SharedPtr<AndroidFile> file) {
    ...
    // archive.
    uint16 entry_count =
        (buffer.GetValue<uint16>(eocd_index + kEOCDNumEntries));
    ...
    for (uint16 edntry_index = 0; edntry_index < entry_count; ++edntry_index) {
      ...
      fun_verify(
          directory_map.Read((uint8*)&when_modified, sizeof(when_modified)));

      uint32 uncompressed_length;
      fun_verify(directory_map.Seek(offset + kCDEUncompLen));
      fun_verify(directory_map.Read((uint8*)&uncompressed_length,
                                    sizeof(uncompressed_length)));

      uint16 filename_len;
      fun_verify(directory_map.Seek(offset + kCDENameLen));
      fun_verify(
          directory_map.Read((uint8*)&filename_len, sizeof(filename_len)));
      ...
      if (method == kCompressStored) {
        ...
        fun_verify(
            directory_map.Read((uint8*)&local_offset, sizeof(local_offset)));

        ByteBuffer filename_buffer(filename_len + 1);
        fun_verify(directory_map.Seek(offset + kCDELen));
        fun_verify(
            directory_map.Read(filename_buffer.data, filename_buffer.Size));
        ...
        fun_verify(
            file->Read((uint8*)&local_signature, sizeof(local_signature)));

        uint16 local_filename_len;
        fun_verify(file->Seek(local_offset + kLFHNameLen));
        fun_verify(file->Read((uint8*)&local_filename_len,
                              sizeof(local_filename_len)));

        uint16 local_extra_len;
        fun_verify(file->Seek(local_offset + kLFHExtraLen));
        fun_verify(
            file->Read((uint8*)&local_extra_len, sizeof(local_extra_len)));

        int64 entry_offset =
            local_offset + kLFHLen + local_filename_len + local_extra_len;

        // Add the entry.
#if LOG_ANDROID_FILE
        PlatformMisc::LowLevelOutputDebugStringf(
            "UnionZipFile::AddPatchFile.. FILE: '%s'", *filename);
#endif
        entries_.Add(filename,
                     MakeShareable(new Entry(
                         MakeShareable(new AndroidFile(*file, entry_offset,
                                                       uncompressed_length)),
                         filename, when_modified)));
...
#if LOG_ANDROID_FILE
            PlatformMisc::LowLevelOutputDebugStringf(
                "UnionZipFile::AddPatchFile.. DIR: '%s'", *dir_name);
#endif
            entries_.Add(dir_name,
                         MakeShareable(new Entry(nullptr, dir_name, 0)));
          }
          filename = CPaths::GetPath(filename);
        }
      }
  ...
  bool HasEntry(const String& path) { return entries_.Contains(path); }

  Entry& GetEntry(const String& path) { return *entries_[path]; }

  int64 GetEntryLength(const String& path) {
    return entries_[path]->file->Size();
  }

  int64 GetEntryModTime(const String& path) { return entries_[path]->mod_time; }
...
class FUN_BASE_API AndroidFileSystem : public IAndroidPlatformFS {
 public:
  ...
  AndroidFileSystem() : asset_mgr_(nullptr) {
  ...
  virtual bool Initialize(IFileSystem* inner, const char* cmdline) override {
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::Initialize(..)");
#endif
    if (!IPhysicalFileSystem::Initialize(inner, cmdline)) {
      PlatformMisc::LowLevelOutputDebugStringf(
          "AndroidFileSystem::Initialize failed");
      return false;
    }
    ...
    if (g_obb_bin_apk) {
      ...
      // Not searching for the OBB to mount is faster on some devices
      // (IterateDirectory above can be slow!)
      ...
      String obb_dir1 =
          g_file_path_base + String("/Android/obb/" + g_package_name);
      String obb_dir2 = g_file_path_base + String("/obb/" + g_package_name);
      String main_obb_name =
          String::Format("main.%d.%s.obb", g_package_version, *g_package_name);
      String patch_obb_name =
          String::Format("patch.%d.%s.obb", g_package_version, *g_package_name);
    ...
    // make sure the base path directory exists (FunGame and
    // FunGame/project_name)
  ...
  bool FileExists(const char* filename, bool allow_local) {
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::FileExists('%s')", filename);
#endif
    String local_path;
    String asset_path;
    ...
    if (!local_path.IsEmpty() &&
        (stat(TCHAR_TO_UTF8(*local_path), &file_info) == 0)) {
...
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::FileExists('%s') => %s\nResolved as %s", filename,
        result ? "TRUE" : "FALSE", *local_path);
#endif
    return result;
  }
  ...
  int64 FileSize(const char* filename, bool allow_local) {
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::file_size('%s')", filename);
#endif
    String local_path;
    String asset_path;
    PathToAndroidPaths(local_path, asset_path, filename, allow_local);
    ...
    if (!local_path.IsEmpty() &&
      ...
      AAsset* file = AAssetManager_open(asset_mgr_, TCHAR_TO_UTF8(*asset_path),
                                        AASSET_MODE_UNKNOWN);
      if (file) {
...
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::DeleteFile('%s')", filename);
#endif
    String local_path;
    String asset_path;
    PathToAndroidPaths(local_path, asset_path, filename, allow_local);
...
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::IsReadOnly('%s')", filename);
#endif
    String local_path;
    String asset_path;
    PathToAndroidPaths(local_path, asset_path, filename, allow_local);
  ...
  bool MoveFile(const char* to, const char* from, bool allow_local) {
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::MoveFile('%s', '%s')", to, from);
#endif
    ...
    if (IsLocal(from_local_path)) {
      return rename(TCHAR_TO_UTF8(*from_local_path),
                    TCHAR_TO_UTF8(*to_local_path)) != -1;
    }
    return false;
  }
...
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::SetReadOnly('%s')", filename);
#endif
    String local_path;
    String asset_path;
    PathToAndroidPaths(local_path, asset_path, filename, allow_local);
  ...
  DateTime GetTimestamp(const char* filename, bool allow_local) {
#if LOG_ANDROID_FILE_MANIFEST
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::GetTimestamp('%s')", filename);
#endif
    String local_path;
    String asset_path;
    PathToAndroidPaths(local_path, asset_path, filename, allow_local);
    ...
    if (IsLocal(local_path)) {
...
#if LOG_ANDROID_FILE_MANIFEST
      PlatformMisc::LowLevelOutputDebugStringf(
          "Failed to find time stamp in NonUFSManifest for file '%s'",
          filename);
#endif
      ...
      Timespan time_since_epoch(0, 0,
                                zip_resource_.GetEntryModTime(asset_path));
      return ANDROID_EPOCH + time_since_epoch;
    } else {
  ...
  virtual void SetTimestamp(const char* filename,
                            const DateTime& timestamp) override {
...
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::SetTimestamp('%s')", filename);
#endif
    String local_path;
    String asset_path;
    PathToAndroidPaths(local_path, asset_path, filename, allow_local);
  ...
  DateTime GetAccessTimestamp(const char* filename, bool allow_local) {
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::GetAccessTimestamp('%s')", filename);
#endif
    String local_path;
    String asset_path;
    PathToAndroidPaths(local_path, asset_path, filename, allow_local);
    ...
    if (IsLocal(local_path)) {
      ...
      // No TimeStamp for resources nor assets, so just return a default
      // timespan for now.
      return DateTime::Null;
    }
  }
  ...
  FileStatData GetStatData(const char* filename_or_directory,
                           bool allow_local) {
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::GetStatData('%s')", filename_or_directory);
#endif
    String local_path;
    String asset_path;
    PathToAndroidPaths(local_path, asset_path, filename_or_directory,
                       allow_local);
    ...
    if (IsLocal(local_path)) {
      ...
      if (file) {
        ...
        AAssetDir* sub_dir =
            AAssetManager_openDir(asset_mgr_, TCHAR_TO_UTF8(*asset_path));
        if (sub_dir) {
          is_directory = true;
        ...
        FileStatData stat_data(DateTime::Null,  // creation_time
                               DateTime::Null,  // access_time
                               DateTime::Null,  // modification_time
                               file_size,       // file_size
  ...
  virtual IFile* OpenRead(const char* filename,
                          bool allow_write = false) override {
  ...
  IFile* OpenRead(const char* filename, bool allow_local, bool allow_write) {
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::OpenRead('%s')", filename);
#endif
    String local_path;
    String asset_path;
    PathToAndroidPaths(local_path, asset_path, filename, allow_local);
    ...
    if (IsLocal(local_path)) {
      ...
      return new AndroidFile(*zip_resource_.GetEntry(asset_path).file, 0,
                             zip_resource_.GetEntry(asset_path).file->Size());
    } else {
      AAsset* asset = AAssetManager_open(asset_mgr_, TCHAR_TO_UTF8(*asset_path),
                                         AASSET_MODE_RANDOM);
      if (asset) {
  ...
  // packaged content.
  virtual IFile* OpenWrite(const char* filename, bool append,
                           bool allow_read) override {
    return OpenWrite(filename, append, allow_read, false);
  }

  IFile* OpenWrite(const char* filename, bool append, bool allow_read,
                   bool allow_local) {
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::OpenWrite('%s')", filename);
#endif
    String local_path;
    String asset_path;
    PathToAndroidPaths(local_path, asset_path, filename, allow_local);
  ...
  bool DirectoryExists(const char* directory, bool allow_local,
                       bool allow_asset) {
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::DirectoryExists('%s')", directory);
#endif
    String local_path;
    String asset_path;
    ...
    if (IsLocal(local_path)) {
#if LOG_ANDROID_FILE
      PlatformMisc::LowLevelOutputDebugStringf(
          "AndroidFileSystem::DirectoryExists('%s') => Check IsLocal: '%s'",
...
#if LOG_ANDROID_FILE
      PlatformMisc::LowLevelOutputDebugStringf(
          "AndroidFileSystem::DirectoryExists('%s') => found as resource: '%s'",
      ...
      if (found) {
        PlatformMisc::LowLevelOutputDebugStringf(
            "AndroidFileSystem::DirectoryExists('%s') => found as asset: '%s'",
            directory, *(asset_path));
      }
#endif
    }
    ...
    if (found) {
      PlatformMisc::LowLevelOutputDebugStringf(
          "AndroidFileSystem::DirectoryExists('%s') => FOUND", directory);
    } else {
      PlatformMisc::LowLevelOutputDebugStringf(
          "AndroidFileSystem::DirectoryExists('%s') => NOT", directory);
    }
#endif
    return found;
  }
...
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::CreateDirectory('%s')", directory);
#endif
    String local_path;
    String asset_path;
    PathToAndroidPaths(local_path, asset_path, directory, allow_local);
...
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::DeleteDirectory('%s')", directory);
#endif
    String local_path;
    String asset_path;
    PathToAndroidPaths(local_path, asset_path, directory, allow_local);
  ...
  bool IterateDirectory(const char* directory, DirectoryVisitor& visitor,
                        bool allow_local, bool allow_asset) {
    const String directory_str = directory;

    auto internal_visitor = [&](const String& local_path,
                                struct dirent* de) -> bool {
    ...
    return IterateDirectoryCommon(
        directory, internal_visitor, internal_resource_visitor,
        internal_asset_visitor, allow_local, allow_asset);
  }
  ...
  bool IterateDirectoryStat(const char* directory,
                            DirectoryStatVisitor& visitor, bool allow_local,
                            bool allow_asset) {
    const String directory_str = directory;
    ...
    auto internal_visitor = [&](const String& local_path,
      ...
      struct stat file_info;
      if (stat(TCHAR_TO_UTF8(*(local_path / UTF8_TO_TCHAR(de->d_name))),
               &file_info) != -1) {
    ...
    auto internal_resource_visitor = [&](const String& resource_name) -> bool {
      return visitor.Visit(*resource_name,
                           FileStatData(DateTime::Null,  // creation_time
                                        DateTime::Null,  // access_time
                                        DateTime::Null,  // modification_time
                                        zip_resource_.GetEntryLength(
                                            resource_name),  // file_size
                                        false,               // is_directory
                                        true                 // bIsReadOnly
                                        ));
    };
    ...
    auto internal_asset_visitor = [&](const char* asset_path) -> bool {
      ...
      if (!is_directory) {
        AAsset* file =
            AAssetManager_open(asset_mgr_, asset_path, AASSET_MODE_UNKNOWN);
        if (file) {
          file_size = AAsset_getLength(file);
        }
      ...
      return visitor.Visit(UTF8_TO_TCHAR(asset_path),
                           FileStatData(DateTime::Null,  // creation_time
                                        DateTime::Null,  // access_time
                                        DateTime::Null,  // modification_time
                                        file_size,       // file_size
  ...
  bool IterateDirectoryCommon(
      ...
      const FunctionRef<bool(const String&, struct dirent*)>& visitor,
      const FunctionRef<bool(const String&)>& resource_visitor,
      const FunctionRef<bool(const char*)>& asset_visitor, bool allow_local,
      bool allow_asset) {
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::IterateDirectory('%s')", directory);
#endif
    String local_path;
    String asset_path;
    PathToAndroidPaths(local_path, asset_path, directory, allow_local);
    ...
    if (IsLocal(local_path)) {
      ...
      if (handle) {
        ...
        while ((entry = readdir(handle)) && result == true) {
          if (CharTraits::Strcmp(UTF8_TO_TCHAR(entry->d_name), ".") &&
              CharTraits::Strcmp(UTF8_TO_TCHAR(entry->d_name), "..")) {
#if LOG_ANDROID_FILE
            PlatformMisc::LowLevelOutputDebugStringf(
                "AndroidFileSystem::IterateDirectory('%s').. LOCAL Visit: '%s'",
                directory, *(String(directory) / UTF8_TO_TCHAR(entry->d_name)));
#endif
      ...
      ZipUnionFile::Directory resource_dir =
          zip_resource_.OpenDirectory(asset_path);
      bool result = true;
      while (result && resource_dir.Next()) {
#if LOG_ANDROID_FILE
        PlatformMisc::LowLevelOutputDebugStringf(
            "AndroidFileSystem::IterateDirectory('%s').. RESOURCE Visit: '%s'",
            directory, *resource_dir.current.Key());
#endif
...
#if LOG_ANDROID_FILE
          PlatformMisc::LowLevelOutputDebugStringf(
              "AndroidFileSystem::IterateDirectory('%s').. ASSET Visit: '%s'",
              directory, UTF8_TO_TCHAR(fileName));
#endif
...
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::FileIsAsset('%s')", filename);
#endif
    String local_path;
    String asset_path;
...
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::FileStartOffset('%s')", filename);
#endif
    String local_path;
    String asset_path;
    PathToAndroidPaths(local_path, asset_path, filename, true);
...
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::FileRootPath('%s')", filename);
#endif
    String local_path;
    String asset_path;
    PathToAndroidPaths(local_path, asset_path, filename, true);
 ...
 private:
  ...
  void PathToAndroidPaths(String& local_path, String& asset_path,
                          const char* path, bool allow_local) {
    local_path.Empty();
    asset_path.Empty();
...
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::PathToAndroidPaths('%s') => android_path = '%s'",
        path, *android_path);
#endif
    if (!android_path.IsEmpty()) {
      if ((allow_local && android_path.StartsWith("/")) ||
          ...
          android_path.StartsWith(
              g_external_file_path.Left(android_path.Len()))) {
...
#if LOG_ANDROID_FILE
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::PathToAndroidPaths('%s') => local_path = '%s'",
        path, *local_path);
    PlatformMisc::LowLevelOutputDebugStringf(
        "AndroidFileSystem::PathToAndroidPaths('%s') => asset_path = '%s'",
        path, *asset_path);
#endif
  }
  ...
  bool IsLocal(const String& local_path) {
    return !local_path.IsEmpty() &&
           access(TCHAR_TO_UTF8(*local_path), F_OK) == 0;
  }
  ...
  class MountOBBVisitor : public IFileSystem::DirectoryVisitor {
   ...
   public:
    MountOBBVisitor(AndroidFileSystem& android_fs) : android_fs_(android_fs) {}
  ...
  void MountOBB(const char* filename) {
    AndroidFile* file =
        static_cast<AndroidFile*>(OpenRead(filename, true, false));
    fun_check_ptr(file);
