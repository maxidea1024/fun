...
enum class FileOpenFlag {
  ...
  /** Open for writing */
  Write = 0x02,
  /** When writing, keep the existing data, set the filepointer to the end of
     the existing data */
  Append = 0x40,
};
...
class IFileManager {
 ...
 public:
  ...
  virtual Archive* CreateFileReader(const char* filename,
                                    FileReadFlags read_flags = 0) = 0;

  /** Creates file writer archive. */
  virtual Archive* CreateFileWriter(const char* filename,
                                    FileWriteFlags write_flags = 0) = 0;

  // If you're writing to a debug file, you should use CreateDebugFileWriter,
  // and wrap the calling code in #if ALLOW_DEBUG_FILES.
#if ALLOW_DEBUG_FILES
  virtual Archive* CreateDebugFileWriter(const char* filename,
                                         FileWriteFlags write_flags = 0) = 0;
#endif
  ...
  /** Deletes a file. */
  virtual bool Delete(const char* filename, bool require_exists = false,
                      bool event_if_read_only = false, bool quiet = false) = 0;

  /** Copies a file. */
  virtual FileCopyResult Copy(
      const char* dst, const char* src, bool replace = true,
      bool event_if_read_only = false, bool copy_attributes = false,
      FileCopyProgress* progress = nullptr) = 0;  // utility

  /** Moves/renames a file. */
  virtual bool Move(const char* dst, const char* src, bool replace = true,
                    bool event_if_read_only = false,
                    bool copy_attributes = false,
                    bool do_not_retry_or_error = false) = 0;
  ...
  virtual void FindFiles(Array<String>& out_filenames, const char* filename,
                         bool with_files, bool with_directories) = 0;
  ...
  virtual void FindFiles(Array<String>& out_found_files, const char* directory,
                         const char* file_extension = nullptr) = 0;

  /** Finds file or directories recursively. */
  virtual void FindFilesRecursive(Array<String>& out_filenames,
                                  const char* start_directory,
                                  const char* filename, bool with_files,
                                  bool with_directories,
                                  bool clear_filenames = true) = 0;  // utility
  ...
  virtual bool IterateDirectory(const char* directory,
                                IPlatformFS::DirectoryVisitor& visitor) = 0;
  ...
  virtual bool IterateDirectoryRecursively(
      const char* directory, IPlatformFS::DirectoryVisitor& visitor) = 0;
  ...
  virtual bool IterateDirectoryStat(
      const char* directory, IPlatformFS::DirectoryStatVisitor& visitor) = 0;
  ...
  /**
   * @return the modification time of the given file (or DateTime::MinValue() on
   * failure)
   */
  ...
  /**
   * @return the last access time of the given file (or DateTime::MinValue() on
   * failure)
   */
  ...
  virtual bool SendMessageToServer(
      const char* message, IPlatformFS::IFileServerMessageHandler* handler) = 0;
