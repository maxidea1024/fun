...
class FUN_BASE_API FSManagerGeneric : public IFileManager {
  // instead of caching the LowLevel, we call the singleton each time to never
  // be incorrect
 ...
 public:
  ...
  Archive* CreateFileReader(const char* filename,
                            FileReadFlags read_flags = 0) override;
  Archive* CreateFileWriter(const char* filename,
                            FileWriteFlags write_flags = 0) override;

#if ALLOW_DEBUG_FILES
  Archive* CreateDebugFileWriter(const char* filename,
                                 FileWriteFlags write_flags = 0) override {
    return CreateFileWriter(filename, write_flags);
  }
#endif

  bool Delete(const char* filename, bool require_exists = false,
              bool event_if_read_only = false, bool quiet = false) override;
  bool IsReadOnly(const char* filename) override;
  bool Move(const char* dst, const char* src, bool replace = true,
            bool event_if_read_only = false, bool copy_attributes = false,
            bool do_not_retry_or_error = false) override;
  bool FileExists(const char* filename) override;
  bool DirectoryExists(const char* directory) override;
  void FindFiles(Array<String>& result, const char* filename, bool with_files,
                 bool with_directories) override;
  void FindFilesRecursive(Array<String>& out_filenames,
                          const char* start_directory, const char* filename,
                          bool with_files, bool with_directories,
                          bool clear_filenames = true) override;
  ...
  void GetTimestampPair(const char* path_a, const char* path_b,
                        DateTime& out_timestamp_a, DateTime& out_timestamp_b);
  ...
  virtual FileCopyResult Copy(const char* dest_file, const char* src_file,
                              bool replace_existing, bool event_if_read_only,
                              bool copy_attributes,
                              FileCopyProgress* progress) override;
  virtual bool MakeDirectory(const char* path, bool tree = false) override;
  virtual bool DeleteDirectory(const char* path, bool require_exists = false,
                               bool tree = false) override;

  virtual FileStatData GetStatData(const char* filename_or_directory) override;

  /**
   * Finds all the files within the given directory, with optional file
   * extension filter.
   *
   * @param directory - the absolute path to the directory to search. Ex:
   * "C:\Fun\Pictures"
   *
   * @param file_extension - If file_extension is NULL, or an empty string ""
   * then all files are found. Otherwise file_extension can be of the form .EXT
   * or just EXT and only files with that extension will be returned.
   *
   * @return out_found_files, All the files that matched the optional
   * file_extension filter, or all files if none was specified.
   */
  virtual void FindFiles(Array<String>& out_found_files, const char* directory,
                         const char* file_extension = nullptr) override;
  ...
  virtual bool SendMessageToServer(
      const char* message,
      IPlatformFS::IFileServerMessageHandler* handler) override {
 ...
 private:
   ...
   */
  FileCopyResult CopyWithProgress(const char* dest_file, const char* src_file,
                                  bool replace_existing,
                                  bool event_if_read_only, bool copy_attributes,
                                  FileCopyProgress* progress);

  void FindFilesRecursiveInternal(Array<String>& out_filenames,
...
class FUN_BASE_API ArchiveFileReaderGeneric : public Archive {
 ...
 protected:
   ...
   */
  virtual void ReadLowLevel(uint8* dst, int64 len_to_read,
...
class ArchiveFileWriterGeneric : public Archive {
 ...
 protected:
  ...
  /**
   * Logs I/O error
   * It is important to not call any platform API functions after the error
   * occurred and before calling this functions as the system error code may get
   * reset and will not be properly logged in this message.
   *
