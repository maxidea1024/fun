...
Archive* FSManagerGeneric::CreateFileReader(const char* filename, FileReadFlags flags) {
  IFile* handle = GetLowLevel().OpenRead(filename, !!(flags & FileReadFlag::AllowWrite));
  if (handle == nullptr) {
...
Archive* FSManagerGeneric::CreateFileWriter(const char* filename, FileWriteFlags flags) {
  ...
  //  fun_log(LogFileManager, Info, "Can't write to signed game file: %s", filename);
  ...
  IFile* handle = GetLowLevel().OpenWrite(filename, !!(flags & FileWriteFlag::Append), !!(flags & FileWriteFlag::AllowRead));
  if (handle == nullptr) {
    if (!!(flags & FileWriteFlag::NoFail)) {
      fun_log(LogFileManager, Fatal, TEXT("Failed to create file: %s"), filename);
    }
    return nullptr;
  }
...
FileCopyResult FSManagerGeneric::Copy(const char* dest_file, const char* src_file, bool replace_existing, bool event_if_read_only, bool copy_attributes, FileCopyProgress* progress) {
  FileCopyResult result = FileCopyResult::OK;
  if (CPaths::ConvertRelativePathToFull(dest_file) == CPaths::ConvertRelativePathToFull(src_file)) {
    result = FileCopyResult::Fail;
  } else if (progress) {
    result = CopyWithProgress(dest_file, src_file, replace_existing, event_if_read_only, copy_attributes, progress);
...
FileCopyResult FSManagerGeneric::CopyWithProgress(const char* dest_file, const char* src_file, bool replace_existing, bool event_if_read_only, bool copy_attributes, FileCopyProgress* progress) {
  FileCopyResult result = FileCopyResult::OK;
  ...
  if (progress == nullptr || progress->Poll(0.0)) {
    ...
    if (src == nullptr) {
      ...
      if (dst == nullptr) {
        ...
        for (int64 Total = 0; Total < size; Total += sizeof(Buffer)) {
          ...
          if (progress != nullptr && Percent != new_percent && !progress->Poll((float)new_percent / 100.f)) {
    ...
    if (progress != nullptr && result == FileCopyResult::OK && !progress->Poll(1.0)) {
      result = FileCopyResult::Canceled;
    }
  } else {
    result = FileCopyResult::Canceled;
...
bool FSManagerGeneric::Delete(const char* filename, bool require_exists, bool event_if_read_only, bool quiet) {
  // Only allow writes to files that are not signed
  // Except if the file is missing(that way corrupt ini files can be autogenerated by deleting them)
  //TODO?
  //if (CSHA1::GetFileSHAHash(filename, nullptr)) {
  //  if (!quiet) {
  //    fun_log(LogFileManager, Info, TEXT("Can't delete signed game file: %s"), filename);
  //  }
  //  return false;
  //}
  ...
  if (require_exists && !exists) {
    if (!quiet) {
      fun_log(LogFileManager, Warning, "Could not delete %s because it doesn't exist.", filename);
    }
    return false;
  }
  ...
  if (exists) {
    ...
    if (!GetLowLevel().DeleteFile(filename)) {
      if (!quiet) {
        fun_log(LogFileManager, Warning, "Error deleting file: %s (Error Code %i)", filename, PlatformMisc::GetLastError());
      }
      return false;
    }
  }
...
bool FSManagerGeneric::Move(const char* dst, const char* src, bool replace, bool event_if_read_only, bool copy_attributes, bool do_not_retry_or_error) {
  MakeDirectory(*CPaths::GetPath(dst), true);

  // Retry on failure, unless the file wasn't there anyway.
  if (GetLowLevel().FileExists(dst) && !GetLowLevel().DeleteFile(dst) && !do_not_retry_or_error) {
    // If the delete failed, throw a warning but retry before we throw an error
    fun_log(LogFileManager, Warning, TEXT("DeleteFile was unable to delete '%s', retrying in .5s..."), dst);
    ...
    if (!GetLowLevel().DeleteFile(dst)) {
      ...
      fun_log(LogFileManager, Warning, TEXT("DeleteFile recovered during retry!"));
    }
  }
  ...
  if (!GetLowLevel().MoveFile(dst, src)) {
    ...
    while (RetryCount--) {
      ...
      fun_log(LogFileManager, Warning, TEXT("MoveFile was unable to move '%s' to '%s', retrying in .5s..."), src, dst);
      ...
      if (success) {
        fun_log(LogFileManager, Warning, TEXT("MoveFile recovered during retry!"));
        break;
      }
    }
    if (!success) {
      fun_log(LogFileManager, Error, TEXT("Error moving file '%s' to '%s'."), src, dst);
      return false;
    }
  }
  return true;
...
bool FSManagerGeneric::DeleteDirectory(const char* path, bool require_exists, bool tree) {
  if (tree) {
    return GetLowLevel().DeleteDirectoryRecursively(path) || (!require_exists && !GetLowLevel().DirectoryExists(path));
  }
  return GetLowLevel().DeleteDirectory(path) || (!require_exists && !GetLowLevel().DirectoryExists(path));
}
...
void FSManagerGeneric::FindFiles(Array<String>& result, const char* InFilename, bool with_files, bool with_directories) {
  struct CFileMatch : public IPlatformFS::DirectoryVisitor {
    ...
    CFileMatch(Array<String>& result, const String& wildcard, bool with_files, bool with_directories)
      : result(result),
        wildcard(wildcard),
    ...
    virtual bool Visit(const char* filename_or_directory, bool is_directory) {
      if (((is_directory && with_directories) || (!is_directory && with_files))
  ...
  const bool find_all_files = clean_filename == TEXT("*") || clean_filename == TEXT("*.*");
  CFileMatch file_match(result, find_all_files ? TEXT("*") : clean_filename, with_files, with_directories);
  GetLowLevel().IterateDirectory(*CPaths::GetPath(filename), file_match);
}

//@maxidea: todo: TEXT로 안묶었는데, 이거 괜시리 인코딩 변환이 이루어지는거 아닌가??
void FSManagerGeneric::FindFiles(Array<String>& out_found_files, const char* directory, const char* file_extension) {
  ...
  // Don't modify the ext_str if the user supplied the form "*.EXT" or "*" or "*.*" or "Name.*"
  if (!ext_str.Contains("*")) {
    if (ext_str == "") {
      ext_str = "*.*";
    } else {
...
bool FSManagerGeneric::IterateDirectory(const char* directory, IPlatformFS::DirectoryVisitor& visitor) {
  return GetLowLevel().IterateDirectory(directory, visitor);
}

bool FSManagerGeneric::IterateDirectoryRecursively(const char* directory, IPlatformFS::DirectoryVisitor& visitor) {
  return GetLowLevel().IterateDirectoryRecursively(directory, visitor);
}

bool FSManagerGeneric::IterateDirectoryStat(const char* directory, IPlatformFS::DirectoryStatVisitor& visitor) {
  return GetLowLevel().IterateDirectoryStat(directory, visitor);
}

bool FSManagerGeneric::IterateDirectoryStatRecursively(const char* directory, IPlatformFS::DirectoryStatVisitor& visitor) {
...
void FSManagerGeneric::GetTimestampPair(const char* path_a, const char* path_b, DateTime& out_timestamp_a, DateTime& out_timestamp_b) {
  GetLowLevel().GetTimestampPair(path_a, path_b, out_timestamp_a, out_timestamp_b);
}

bool FSManagerGeneric::SetTimestamp(const char* filename, const DateTime& timestamp) {
...
String FSManagerGeneric::DefaultConvertToRelativePath(const char* filename) {
  ...
  while ((current_slash_pos = root_directory.LastIndexOf("/", CaseSensitivity::CaseSensitive)) != INVALID_INDEX) {
    ...
    const int32 pos_of_next_slash = root_directory.LastIndexOf("/", CaseSensitivity::CaseSensitive, current_slash_pos);
...
String FSManagerGeneric::ConvertToAbsolutePathForExternalAppForRead(const char* absolute_path) {
  return GetLowLevel().ConvertToAbsolutePathForExternalAppForRead(absolute_path);
}

String FSManagerGeneric::ConvertToAbsolutePathForExternalAppForWrite(const char* absolute_path) {
  return GetLowLevel().ConvertToAbsolutePathForExternalAppForWrite(absolute_path);
}

void FSManagerGeneric::FindFilesRecursive(Array<String>& out_filenames, const char* start_directory, const char* filename, bool with_files, bool with_directories, bool clear_filenames) {
  if (clear_filenames) {
    out_filenames.Clear();
  }
  FindFilesRecursiveInternal(out_filenames, start_directory, filename, with_files, with_directories);
}

void FSManagerGeneric::FindFilesRecursiveInternal(Array<String>& out_filenames, const char* start_directory, const char* filename, bool with_files, bool with_directories) {
  ...
  for (int32 sub_dir_index = 0; sub_dir_index < sub_dirs.Count(); ++sub_dir_index) {
    String sub_dir = String(start_directory) / sub_dirs[sub_dir_index];
    FindFilesRecursiveInternal(out_filenames, *sub_dir, filename, with_files, with_directories);
  }
}
...
ArchiveFileReaderGeneric::ArchiveFileReaderGeneric(IFile* handle, const char* filename, int64 size)
  : filename_(filename),
    size_(size),
    pos_(0),
    buffer_base_(0),
...
void ArchiveFileReaderGeneric::Seek(int64 new_pos) {
  fun_check_msg(new_pos >= 0, TEXT("Attempted to seek to a negative location (%lld/%lld), file: %s. The file is most likely corrupt."), new_pos, size_, *filename_);
  fun_check_msg(new_pos <= size_, TEXT("Attempted to seek past the end of file (%lld/%lld), file: %s. The file is most likely corrupt."), new_pos, size_, *filename_);
  if (!SeekLowLevel(new_pos)) {
    char error_msg[1024];
    ArIsError = true;
    fun_log(LogFileManager, Error, TEXT("SetFilePointer on %s Failed %lld/%lld: %lld %s"), *filename, new_pos, size_, pos_, PlatformMisc::GetSystemErrorMessage(error_msg, 1024, 0));
  }
  pos_ = new_pos;
  buffer_base_ = pos_;
  buffer_count_ = 0;
...
void ArchiveFileReaderGeneric::ReadLowLevel(uint8* dst, int64 len_to_read, int64& out_readed_len) {
...
bool ArchiveFileReaderGeneric::InternalPrecache(int64 precache_offset, int64 precache_size) {
  // Only precache at current position and avoid work if precaching same offset twice.
  if (pos_ == precache_offset && (buffer_base_ == 0 || buffer_count_ == 0 || buffer_base_ != pos_)) {
    buffer_base_ = pos_;
    buffer_count_ = MathBase::Min(MathBase::Min(precache_size, (int64)(countof(buffer_) - (pos_ & (countof(buffer_) - 1)))), size_ - pos_);
    buffer_count_ = MathBase::Max(buffer_count_, 0LL); // clamp to 0
    int64 count = 0;

    // Read data from device via Win32 ReadFile API.
    {
      CLOG(buffer_count_ > countof(buffer_) || buffer_count_ <= 0, LogFileManager, Fatal, "Invalid buffer_count_=%lld while reading %s. File is most likely corrupted, try deleting it if possible. Pos=%lld, Size=%lld, precache_size=%lld, precache_offset=%lld",
              buffer_count_, *filename, pos_, size_, precache_size, precache_offset);
    ...
    if (count != buffer_count_) {
      char error_msg[1024];
      ArIsError = true;
      fun_log(LogFileManager, Warning, TEXT("ReadFile failed: Count=%lld buffer_count_=%lld Error=%s"), count, buffer_count_, PlatformMisc::GetSystemErrorMessage(error_msg, 1024, 0));
    }
  }
  return true;
}
    ...
    if (len_to_copy <= 0) {
      if (len >= countof(buffer_)) {
        ...
        if (count != len) {
          char error_msg[1024];
          ArIsError = true;
          fun_log(LogFileManager, Warning, "ReadFile failed: Count=%lld Length=%lld Error=%s for file %s",
            count, len, PlatformMisc::GetSystemErrorMessage(error_msg, 1024, 0), *filename);
        }
        pos_ += len;
        return;
      }
      ...
      if (len_to_copy <= 0) {
        ArIsError = true;
        fun_log(LogFileManager, Error, "ReadFile beyond EOF %lld+%lld/%lld for file %s",
          pos_, len, size_, *filename);
      }
...
ArchiveFileWriterGeneric::ArchiveFileWriterGeneric(IFile* handle, const char* filename, int64 pos)
  : filename_(filename),
    pos_(pos),
    buffer_count_(0),
    handle_(handle),
...
bool ArchiveFileWriterGeneric::WriteLowLevel(const uint8* src, int64 len_to_write) {
...
void ArchiveFileWriterGeneric::LogWriteError(const char* message) {
  ...
  if (!is_logging_error_) {
    ...
    char error_msg[1024];
    fun_log(LogFileManager, Error, "%s: %s (%s)", message, *filename, PlatformMisc::GetSystemErrorMessage(error_msg, 1024, 0));
    is_logging_error_ = false;
  }
}
