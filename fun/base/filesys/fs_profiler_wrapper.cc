...
bool OsFSReadStats::Tick(float delta) {
  ...
  if (reads) {
    ...
    SET_FLOAT_STAT(STAT_LTAvgReadSize, (lifetime_read_size_/lifetime_read_calls_));
    SET_FLOAT_STAT(STAT_LTAvgReadSpeed, (lifetime_read_speed_/lifetime_read_calls_)/1024.f);
  }
...
bool OsFSReadStats::Initialize(IPlatformFS* inner, const char* cmdline) {
  ...
  fun_check_ptr(inner);
  lower_level_ = inner;
  Ticker::GetCoreTicker().AddTicker(TickerDelegate::CreateRaw(this, &OsFSReadStats::Tick), 0.f);
