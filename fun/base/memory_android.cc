...
void CAndroidMemory::Init() {
  ...
  const CPlatformMemoryConstants& MemoryConstants =
      PlatformMemory::GetConstants();
  CPlatformMemoryStats MemoryStats = GetStats();
  LOG(LogInit, Info,
      TEXT("Memory total: Physical=%.2fMB (%dGB approx) Available=%.2fMB "
           "PageSize=%.1fKB"),
...
CPlatformMemoryStats CAndroidMemory::GetStats() {
  ...
  // MemoryStats.AvailablePhysical = NumAvailPhysPages *
  // MemoryConstants.PageSize;

  MemoryStats.AvailablePhysical =
      MemoryConstants.TotalPhysical - GetNativeHeapAllocatedSize();
...
const CPlatformMemoryConstants& CAndroidMemory::GetConstants() {
  ...
  if (MemoryConstants.TotalPhysical == 0) {
    ...
    MemoryConstants.TotalPhysicalGB =
        (MemoryConstants.TotalPhysical + 1024 * 1024 * 1024 - 1) / 1024 / 1024 /
        1024;
  }
...
MemoryAllocator* CAndroidMemory::BaseAllocator() {
  const CPlatformMemoryConstants& MemoryConstants =
      PlatformMemory::GetConstants();
  // 1 << MathBase::CeilLogTwo(MemoryConstants.TotalPhysical) should really be
  // MathBase::RoundUpToPowerOfTwo, but that overflows to 0 when
  // MemoryConstants.TotalPhysical is close to 4GB, since CeilLogTwo returns 32
  // this then causes the MemoryLimit to be 0 and crashing the app
  uint64 MemoryLimit = MathBase::Min<uint64>(
      uint64(1) << MathBase::CeilLogTwo(MemoryConstants.TotalPhysical),
      0x100000000);
...
void CAndroidMemory::BinnedFreeToOS(void* ptr) { free(ptr); }
