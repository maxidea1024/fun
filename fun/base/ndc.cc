...
void Ndc::Dump(std::ostream& ostr) const {
  Dump(ostr, "\n");
}

void Ndc::Dump(std::ostream& ostr, const String& delimiter, bool name_only) const {
  //TODO
  fun_check(0);
...
//thread별로 해야하지 않으려나??
//그냥 별도로 만들어주어야할까??
String Ndc::Backtrace(int32 skip_end, int32 skip_begin, int32 stack_size, int32 buf_size) {
  String trace_buf;
    ...
    if (addr_len != 0) {
      ...
      if (begin > 1) { // 0 is this function
        ...
        if (skip == 1) {
          ...
          trace_buf.Append(" (entries 1-" + std::to_string(begin-1) + " skipped)");
        }
      }
      trace_buf.Append(":\n");
      ...
      if (end < addr_len) {
        if (end != addr_len-1) {
          trace_buf.Append("\n " + std::to_string(end).Append(1, '-') + std::to_string(addr_len-1) + " (skipped)");
        } else {
