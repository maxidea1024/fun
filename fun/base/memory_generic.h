...
struct CGenericPlatformMemoryConstants {
  ...
  /**
   * For platforms that support multiple page sizes this is non-zero and smaller
   * than PageSize. If non-zero, then BinnedAllocFromOS will take allocation
   * requests aligned to this size and return blocks aligned to PageSize
   */
  size_t OsAllocationGranularity;

  /**
   * AddressLimit - Second parameter is estimate of the range of addresses
   * expected to be returns by BinnedAllocFromOS(). Binned Malloc will adjust
   * its internal structures to make lookups for memory allocations O(1) for
   * this range. It is ok to go outside this range, lookups will just be a
   * little slower
   */
  uint64 AddressLimit;

  /** Approximate physical RAM in GB; 1 on everything except PC. Used for
   * "course tuning", like CPlatformMisc::NumberOfCores(). */
...
/**
 * ALLOCA/alloca implementation. This can't be a function, even inline'd because
 * there's no guarantee that the memory returned in a function will stick around
...
#define ALLOCA(size) \
  ((size == 0) ? 0 : (void*)(((intptr_t)__ALLOCA_Func(size + 15) + 15) & ~15))

/** Generic implementation for most platforms, these tend to be unused and
 * unimplemented. */
struct FUN_BASE_API CGenericPlatformMemory {
  ...
  /** Set to alignment of allocation that triggered out of memory, zero
   * otherwise. */
  static uint32 OOMAllocationAlignment;

  /** Preallocated buffer to delete on out of memory. Used by OOM handling and
   * crash reporting. */
  ...
  /**
   * Various memory regions that can be used with memory stats. The exact
   * meaning of the enums are relatively platform-dependent, although the
   * general ones (Physical, GPU) are straightforward. A platform can add more
   * of these, and it won't affect other platforms, other than a minuscule
   * amount of memory for the StatManager to track the max available memory for
   * each region (uses an array PlatformMemory::MCR_MAX big)
   */
   ...
   */
  enum ESharedMemoryAccess { Read = 0x02, Write = 0x04 };
  ...
  struct CSharedMemoryRegion {
    ...
    CSharedMemoryRegion(const String& InName, uint32 InAccessMode,
                        void* InAddress, size_t InSize);
   ...
   */
  static bool SupportBackupMemoryPool() { return false; }
  ...
  /**
   * Writes all platform specific current memory statistics in the format usable
   * by the malloc profiler.
   */
   ...
   * address space.
   *
   * @param Ptr - Address to the starting page of the region of pages whose
   * access protection attributes are to be changed.
   * @param size - The size of the region whose access protection attributes are
   * to be changed, in bytes.
   ...
   */
  static bool PageProtect(void* const Ptr, const size_t size,
                          const bool can_read, const bool can_write);
  ...
  static void* GPUMalloc(size_t Count, uint32 Alignment = 0) {
    return nullptr;
  };
  static void* GPURealloc(void* Original, size_t Count, uint32 Alignment = 0) {
    return nullptr;
  };
  ...
  /** Dumps basic platform memory statistics and allocator specific statistics
  ...
  /**
   * Copies count bytes of characters from Src to Dest. If some regions of the
   * source area and the destination overlap, memmove ensures that the original
   * source bytes in the overlapping region are copied before being overwritten.
   * NOTE: make sure that the destination buffer is the same size or larger than
   * the source buffer!
   */
  ...
  static inline void* BigBlockMemcpy(void* Dest, const void* Src,
                                     size_t Count) {
    return memcpy(Dest, Src, Count);
  }

  /** On some platforms memcpy optimized for big blocks that avoid L2 cache
   * pollution are available */
  static inline void* StreamingMemcpy(void* Dest, const void* Src,
                                      size_t Count) {
    return memcpy(Dest, Src, Count);
  }
 ...
 public:
  ...
  /**
   * Maps a named shared memory region into process address space (creates or
   * opens it)
   *
   * @param Name - unique name of the shared memory region (should not contain
   * [back]slashes to remain cross-platform).
   * @param bCreate - whether we're creating it or just opening existing
   * (created by some other process).
   * @param AccessMode - mode which we will be accessing it (use values from
   * ESharedMemoryAccess)
   * @param size - size of the buffer (should be >0. Also, the real size is
   * subject to platform limitations and may be increased to match page size)
   *
   * @return pointer to CSharedMemoryRegion (or its descendants) if successful,
   * nullptr if not.
   */
  static CSharedMemoryRegion* MapNamedSharedMemoryRegion(const String& Name,
                                                         bool bCreate,
                                                         uint32 AccessMode,
                                                         size_t size);
   ...
   *
   * @param MemoryRegion - an object that encapsulates a shared memory region
   * (will be destroyed even if function fails!)
   *
 ...
 protected:
  ...
  /** Updates platform specific stats. This method is called through
   * CGenericStatsUpdater from the task graph thread. */
