...
// Value we fill a memory block with after it is free, in FUN_BUILD_DEBUG and
// FUN_BUILD_DEVELOPMENT
#define DEBUG_FILL_FREED (0xdd)
// Value we fill a new memory block with, in FUN_BUILD_DEBUG and
// FUN_BUILD_DEVELOPMENT
...
void* MemoryAllocatorJemalloc::Realloc(void* ptr, size_t new_size,
                                       uint32 alignment) {
  fun_check_msg(
      alignment == DEFAULT_ALIGNMENT,
      "alignment with realloc is not supported with MemoryAllocatorJemalloc");
  MEM_TIME(mem_time_ -= SystemTime::Seconds());
...
void MemoryAllocatorJemalloc::DumpAllocatorStats(Printer& out) {
  ...
  // "g" omits static stats, "a" omits per-arena stats, "l" omits large object
  // stats see
  // http://www.canonware.com/download/jemalloc/jemalloc-latest/doc/jemalloc.html
  // for detailed opts explanation.
  je_malloc_stats_print(JemallocStatsPrintCallback, &out, "gla");
}

bool MemoryAllocatorJemalloc::GetAllocationSize(void* reported_ptr,
