...
bool FileHelper::ReadAllBytes(ByteArray& out_bytes,
                              const char* filename,
                              FileReadFlags flags) {
  UniquePtr<Archive> reader(IFileManager::Get().CreateFileReader(filename, flags));
  if (!reader.IsValid()) {
    if (!(flags & FileReadFlag::Silent)) {
      //TODO
      //fun_log(LogStreaming, Warning, "Failed to read file '{0}' error.", filename);
    }
...
bool FileHelper::ReadAllBytes(Array<uint8>& out_bytes,
                              const char* filename,
                              FileReadFlags flags) {
  UniquePtr<Archive> reader(IFileManager::Get().CreateFileReader(filename, flags));
  if (!reader.IsValid()) {
...
bool FileHelper::WriteAllBytes( const ByteArray& bytes,
                                const char* filename,
                                IFileManager* fm,
                                FileWriteFlags flags) {
  return WriteAllBytes((const uint8*)bytes.ConstData(), bytes.Len(), filename, fm, flags);
}

bool FileHelper::WriteAllBytes( const Array<uint8>& bytes,
                                const char* filename,
                                IFileManager* fm,
                                FileWriteFlags flags) {
...
bool FileHelper::WriteAllBytes( const uint8* bytes,
                                size_t length,
                                const char* filename,
                                IFileManager* fm,
                                FileWriteFlags flags) {
...
bool FileHelper::BytesToString( String& out_string,
                                const ByteArray& bytes,
                                TextFileEncoding* out_detected_encoding) {
  return BytesToString(out_string, (const uint8*)bytes.ConstData(), bytes.Len(), out_detected_encoding);
}

bool FileHelper::BytesToString( UString& out_string,
                                const Array<uint8>& bytes,
                                TextFileEncoding* out_detected_encoding) {
  return BytesToString(out_string, bytes.ConstData(), bytes.Count(), out_detected_encoding);
}
...
bool FileHelper::BytesToString( UString& out_string,
  ...
  if (length >= 4 && !(length & 3) && UnsafeMemory::Memcmp(bytes, UTF32LE_BOM, sizeof(UTF32LE_BOM)) == 0) { // UTF32LE
    if (out_detected_encoding) {
    ...
    const int32 utf16_len = Utf::LengthAsUtf16((const UTF32CHAR*)bytes, utf32_len);
    out_string.ResizeUninitialized(utf16_len);
    Utf::Convert((const UTF32CHAR*)bytes, utf32_len, out_string.MutableData(), out_string.Len());
#else
    ...
    const int32 utf16_len = Utf::LengthAsUtf16(utf32_buf.ConstData(), utf32_buf.Count());
    out_string.Resize(utf16_len);
    Utf::Convert(utf32_buf.ConstData(), utf32_buf.Count(), out_string.MutableData(), out_string.Len());
#endif
  }
  else if (length >= 4 && !(length & 3) && UnsafeMemory::Memcmp(bytes, UTF32BE_BOM, sizeof(UTF32BE_BOM)) == 0) { // UTF32BE
    if (out_detected_encoding) {
  ...
  else if (length >= 2 && !(length & 1) && UnsafeMemory::Memcmp(bytes, UTF16LE_BOM, sizeof(UTF16LE_BOM)) == 0) { // UTF16LE
    if (out_detected_encoding) {
    ...
    for (int32 i = 0; i < char_count*2; i += 2) {
      *p++ = CharCast<TCHAR>((UCS2CHAR)((uint16)bytes[i] + (uint16)bytes[i + 1] * 256));
    }
  }
  else if (length >= 2 && !(length & 1) && UnsafeMemory::Memcmp(bytes, UTF16BE_BOM, sizeof(UTF16BE_BOM)) == 0) { // UTF16BE
    if (out_detected_encoding) {
    ...
    TCHAR* p = out_string.MutableData();
    for (int32 i = 0; i < char_count*2; i += 2) {
      *p++ = CharCast<TCHAR>((UCS2CHAR)((uint16)bytes[i + 1] + (uint16)bytes[i] * 256));
    }
  }
  else if (length >= 3 && UnsafeMemory::Memcmp(bytes, UTF8_BOM, sizeof(UTF8_BOM)) == 0) { // Utf8
    if (out_detected_encoding) {
    ...
    // Utf8 -> TCHAR
    // 현재, TCHAR는 UTF16LE라고 암묵적으로 가정하고 있는 차후에 시스템 전반적으로 정리가 필요해보임.
    // UCS2를 사용하는 윈도우즈에서는 아무런 문제가 되지 않으나, wchar_t가 32비트인 unix계열에서는
    // 문제가 발생함.
    ...
    const int32 utf16_len = MultiByteToWideChar(CP_ACP, 0, (char*)bytes, length, nullptr, 0);
    out_string.ResizeUninitialized(utf16_len);
    MultiByteToWideChar(CP_ACP, 0, (char*)bytes, length, out_string.MutableData(), out_string.Len());
  }
...
void FileHelper::StringToBytes( ByteArray& out_bytes,
                                const UString& String,
                                TextFileEncoding encoding) {
  if (encoding == TextFileEncoding::AutoDetect) {
    const bool is_pure_ansi = string.IndexOfIf([](TCHAR Ch) ->bool { return Ch > 0x7F; }) == INVALID_INDEX;
    encoding = is_pure_ansi ? TextFileEncoding::ANSI : TextFileEncoding::UTF16LE;
  }
  ...
  case TextFileEncoding::ANSI: {
    ...
    const int32 mbcs_len = WideCharToMultiByte(CP_ACP, 0, string.ConstData(), string.Len(), nullptr, 0, nullptr, nullptr);
    out_bytes.ResizeUninitialized(mbcs_len);
    WideCharToMultiByte(CP_ACP, 0, string.ConstData(), string.Len(), out_bytes.MutableData(), out_bytes.Len(), nullptr, nullptr);
    break;
  }
  ...
  case TextFileEncoding::UTF32LE: {
    const int32 utf32_len = Utf::LengthAsUtf32(string.ConstData(), string.Len());
    Array<UTF32CHAR> utf32_buf(utf32_len, NoInit);
    Utf::Convert(string.ConstData(), string.Len(), utf32_buf.MutableData(), utf32_buf.Count());
  ...
  case TextFileEncoding::UTF8WithoutBOM: {
    const int32 BOM_SIZE = encoding == TextFileEncoding::Utf8 ? sizeof(UTF8_BOM) : 0;
    const int32 utf8_len = Utf::LengthAsUtf8(string.ConstData(), string.Len());
    ...
    Utf::Convert(string.ConstData(), string.Len(), dst, out_bytes.Len() - BOM_SIZE);
    break;
  }
...
void FileHelper::StringToBytes( Array<uint8>& out_bytes,
                                const UString& string,
                                TextFileEncoding encoding) {
  //TODO
  //코드 중복을 피할 수 없을까??
  ...
  if (encoding == TextFileEncoding::AutoDetect) {
    const bool is_pure_ansi = string.IndexOfByIf([](TCHAR Ch) ->bool { return Ch > 0x7F; }) == INVALID_INDEX;
    encoding = is_pure_ansi ? TextFileEncoding::ANSI : TextFileEncoding::UTF16LE;
  }
  ...
  case TextFileEncoding::ANSI: {
    ...
    WideCharToMultiByte(CP_ACP, 0, string.ConstData(), string.Len(), (char*)out_bytes.MutableData(), out_bytes.Count(), nullptr, nullptr);
    break;
  }
  ...
  case TextFileEncoding::UTF8WithoutBOM: {
    ...
    Utf::Convert(string.ConstData(), string.Len(), dst, out_bytes.Count() - BOM_SIZE);
    break;
  }
...
bool FileHelper::ReadAllText( UString& out_text,
                              const char* filename,
  ...
  return BytesToString(out_text, (const uint8*)bytes.ConstData(), bytes.Len(), out_encoding);
}

bool FileHelper::ReadAllLines(Array<UString>& out_lines,
                              const char* filename,
                              HashOptions verify_flags,
...
bool FileHelper::WriteAllText(const UString& text,
                              const char* filename,
                              TextFileEncoding encoding,
                              IFileManager* fm,
                              FileWriteFlags flags) {
...
bool FileHelper::WriteAllLines( const Array<UString>& lines,
                                ...
                                IFileManager* fm,
                                FileWriteFlags flags) {
  const int32 LINE_TERMINATOR_LENGTH = CStringTraitsA::Strlen(FUN_LINE_TERMINATOR);
...
bool FileHelper::CreateBitmap(const char* pattern,
                              int32 data_width,
                              int32 data_height,
                              const class Color* data,
                              const class IntRect* sub_rect,
                              IFileManager* fm,
                              UString* out_filename,
                              bool write_alpha) {
...
#pragma pack (pop)
    ...
    uint32 info_header_size = sizeof(BITMAPINFOHEADER) + (write_alpha ? sizeof(BITMAPV4HEADER) : 0);

    // filename header.
    fh.bfType = ByteOrder::ToLittleEndian((uint16)('B' + 256 * 'M'));
    fh.bfSize = ByteOrder::ToLittleEndian((uint32)(sizeof(BITMAPFILEHEADER) + info_header_size + bytes_per_line * height));
    fh.bfReserved1 = ByteOrder::ToLittleEndian((uint16)0);
    fh.bfReserved2 = ByteOrder::ToLittleEndian((uint16)0);
    fh.bfOffBits = ByteOrder::ToLittleEndian((uint32)(sizeof(BITMAPFILEHEADER) + info_header_size));
    ...
    for (int32 i = sub_rect->max.y - 1; i >= sub_rect->min.y; --i) {
      ...
      for (uint32 pad_index = width * bytes_per_pixel; pad_index < bytes_per_line; ++pad_index) {
        uint8 b = 0;
        ar->Serialize(&b, 1);
      }
    }
...
bool FileHelper::GenerateNextBitmapFilename(const String& pattern,
  ...
  for (int32 test_bitmap_index = g_last_screenshot_bitmap_index + 1; test_bitmap_index < 100000; ++test_bitmap_index) {
    filename = UString::Format(TEXT("%s%05i.%s"), *pattern, test_bitmap_index, *extension);
...
bool FileHelper::LoadANSITextFileToStrings(const char* filename,
  ...
  if (text_file.IsValid()) {
    ...
    //TODO
    //fun_log(LogStreaming, Warning, "Failed to open ANSI TEXT file {0}", filename);
    return false;
  }
}
