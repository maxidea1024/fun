...
AutoConsoleCommand InvalidateCachedShaders(
  ...
  "'MallocLeak Stop'  Dumps outstanding unique callstacks and stops
accumulation.  Also clears data.\n"
  "'MallocLeak Clear'  Clears all accumulated data.  Does not change start/stop
state.\n"
  "'MallocLeak Dump size' Dumps oustanding unique callstacks with optional size
filter in bytes.\n",
...
void MemoryAllocatorLeakDetection::HandleMallocLeakCommandInternal(
  ...
  if (args.Count() >= 1) {
    if (args[0].Compare("Start", CaseSensitivity::IgnoreCase) == 0) {
      ...
      fun_log(LogConsoleResponse, Info,
              "Stopping allocation tracking and clearing data.");
      ...
      fun_log(LogConsoleResponse, Info,
              "Dumping unique calltacks with %i bytes or more oustanding.",
              filter_size);
...
void MemoryAllocatorLeakDetection::HandleMallocLeakCommand(
    const Array<String>& args) {
...
void MemoryAllocatorLeakDetection::AddCallstack(CallstackTrack& callstack) {
  ...
  uint32 callstack_hash =
      Crc::Crc32(callstack.callstack, sizeof(callstack.callstack), 0);
  CallstackTrack& unique_callstack =
      unique_callstacks_.FindOrAdd(callstack_hash);
  // if we had a hash collision bail and lose the data rather than corrupting
  // existing data.
...
void MemoryAllocatorLeakDetection::DumpOpenCallstacks(uint32 filter_size) {
  ...
  ScopedLock<FastMutex> pointers_guard(allocated_pointers_mutex_);
  CPlatformMisc::LowLevelOutputDebugStringf(
      "Dumping out of %i possible open callstacks filtered with more than %u "
      "bytes on frame: %i\n",
  ...
  for (const auto& pair : unique_callstacks_) {
    ...
    if (callstack.size >= filter_size) {
      CPlatformMisc::LowLevelOutputDebugStringf(
          "alloc_size: %i, Num: %i, FirstFrameEverAllocated: %i\n",
          callstack.size, callstack.Count, callstack.frame_number);
      for (int32 i = 0; i < CallstackTrack::Depth; ++i) {
        CPlatformStackWalk::ProgramCounterToHumanReadableString(
            i, callstack.CallStack[i], callstack_string,
            MAX_CALLSTACK_LINE_CHARS);
        // convert ansi -> tchar without mallocs in case we are in OOM handler.
        for (int32 cur_char = 0; cur_char < MAX_CALLSTACK_LINE_CHARS;
             ++cur_char) {
...
bool MemoryAllocatorLeakDetection::Exec(RuntimeEnv* env, const char* cmd,
                                        Printer& ar) {
  return false;
}
  ...
  if (ptr) {
    if (capture_allocs_) {
      ...
      if (!recursive_) {
        ...
        CPlatformStackWalk::CaptureStackBackTrace(callstack.callstack,
                                                  CallstackTrack::Depth);
        // TODO client에서나 의미 있는 값인데... 서버에서는 어떤식으로
        // 처리하는게 바람직할까? 없애는게 좋을듯...??
...
void MemoryAllocatorLeakDetection::Realloc(void* old_ptr, void* new_ptr,
                                           size_t size) {
  if (capture_allocs_) {
    ScopedLock<FastMutex> pointers_guard(allocated_pointers_mutex_);
    Free(old_ptr);
...
MemoryAllocatorLeakDetectionProxy::MemoryAllocatorLeakDetectionProxy(
    MemoryAllocator* malloc)
    : inner_malloc_(malloc), verify_(MemoryAllocatorLeakDetection::Get()) {}
