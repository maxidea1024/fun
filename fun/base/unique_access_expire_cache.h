 ...
 * allows to define expiration per CacheEntry. Each ValueType object must thus
 * offer the following method:
 *
 *    const Timespan& GetTimeout() const;
 *
 * which returns the relative timespan for how long the entry should be valid
 * without being accessed! The absolute expire timepoint is calculated as Now()
 * + GetTimeout(). Accessing an object will update this absolute expire
 * timepoint. You can use the AccessExpirationDecorator to add the GetExpiration
