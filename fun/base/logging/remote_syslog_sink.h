...
class FUN_BASE_API RemoteSyslogSink : public LogSink {
  ...
  /**
   * Creates a RemoteSyslogSink with the given target address, name, and facility.
   * If bsd_format is true, messages are formatted according to RFC 3164.
   */
  RemoteSyslogSink( const String& address,
                    const String& name,
                    int facility = SYSLOG_USER,
                    bool bsd_format = false);
