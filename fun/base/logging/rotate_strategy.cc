...
bool RotateByIntervalStrategy::MustRotate(LogFile* file) {
  ...
  if (last_rotated_at_ == 0 || file->GetSize() == 0) {
    if (file->GetSize() != 0) {
      ...
      fun::InputLineEndingConverter converter(istr, fun::LineEnding::NEWLINE_LF);
      String tag;
      std::getline(converter, tag);
      if (tag.compare(0, ROTATE_TEXT.Len(), ROTATE_TEXT) == 0) {
        String timestamp(tag, ROTATE_TEXT.Len());
        int tzd;
        last_rotated_at_ = DateTimeParser::Parse(DateTimeFormat::RFC1036_FORMAT, timestamp, tzd).timestamp();
      } else {
        last_rotated_at_ = file->GetCreationDate();
      }
    } else {
      last_rotated_at_.Update();
      String tag(ROTATE_TEXT);
      DateTimeFormatter::append(tag, last_rotated_at_, DateTimeFormat::RFC1036_FORMAT);
      file->Write(tag);
    }
  }
  */
