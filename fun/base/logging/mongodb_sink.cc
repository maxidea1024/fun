...
MongoDbSink::MongoDbSink( const String& connector,
...
void MongoDbSink::Open() {
  if (connector_.IsEmpty() || connect_.IsEmpty()) {
    throw IllegalStateException("Connector and connect string must be non-IsEmpty.");
  }
...
void MongoDbSink::LogAsync(const LogMessage& msg) {
  ...
  if (Wait() == 0 && !log_statement_->Done() && !log_statement_->IsInitialized()) {
    if (throw_) {
...
void MongoDbSink::InitLogStatement() {
	...
	String sql = String::Format("INSERT INTO %s VALUES (?,?,?,?,?,?,?,?)", table_);
...
void MongoDbSink::RegisterSink() {
	LoggingFactory::DefaultFactory().RegisterSinkClass("MongoDbSink",
                new Instantiator<MongoDbSink, LogSink>);
}
