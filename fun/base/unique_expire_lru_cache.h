...
/**
 * A UniqueExpireLRUCache combines LRU caching and time based per entry expire
 * caching. One can define for each cache entry a separate timepoint but also
 * limit the size of the cache (per default: 1024). Each TValue object must thus
