...
class AsyncResultHolder : public RefCountedObject {
 public:
  AsyncResultHolder()
      : data_(nullptr), exception_(nullptr), event_(EventResetType::Manual) {
   ...
   */
  bool TryWait(int32 milliseconds) { return event_.TryWait(milliseconds); }
   ...
   */
  void Wait(int32 milliseconds) { event_.Wait(milliseconds); }
   ...
   * Sets the exception.
   */
  void SetError(const String& error) { SetError(UnhandledException(error)); }
...
class AsyncResultHolder<void> : public RefCountedObject {
 public:
   ...
   */
  AsyncResultHolder() : exception_(nullptr), event_(EventResetType::Manual) {
...
class AsyncResult {
 public:
   ...
   * Copy constructor.
   */
  AsyncResult(const AsyncResult& rhs) : holder_(rhs.holder_) {
    // if (holder_) {
    //  holder_->AddRef();
    //}
    fun_check_ptr(holder_);
  ...
  void Swap(AsyncResult& other) { fun::Swap(holder_, other.holder_); }

  /**
   * Returns a reference to the result data.
   */
  ResultType& Data() const { return holder_->Data(); }

  void SetData(ResultType* data) { holder_->SetData(data); }

  /**
   * Pauses the caller until the result becomes available.
   */
  void Wait() { holder_->Wait(); }
   ...
   * available, false otherwise.
   */
  bool TryWait(int32 milliseconds) { return holder_->TryWait(milliseconds); }
   ...
   * result did not became available.
   */
  void Wait(int32 milliseconds) { holder_->Wait(milliseconds); }

  /**
   * Returns true if a result is available.
   */
  bool Available() const { return holder_->TryWait(0); }

  /**
   * Returns true if the active method failed (and threw an exception).
   * Information about the exception can be obtained by calling GetError().
   */
  bool Failed() const { return holder_->Failed(); }
   ...
   * active method completed successfully.
   */
  String GetError() const { return holder_->GetError(); }

  /**
   * If the active method threw an exception, a clone of the exception
   * object is returned, otherwise null.
   */
  Exception* GetException() const { return holder_->GetException(); }

  /**
   * Notifies the invoking thread that the result became available.
   * For internal use only.
   */
  void Notify() FUN_INTERNAL_USE_ONLY { holder_->Notify(); }
   ...
   */
  void SetError(const String& error) { holder_->SetError(error); }

  /**
   * Sets the failed flag and the exception message.
   */
  void SetError(const Exception& e) { holder_->SetError(e); }
