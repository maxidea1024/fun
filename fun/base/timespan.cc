...
String Timespan::ToString() const {
  ...
  const String value =
      (dd == 0) ? String::Format("%d:%02d:%02d.%03d.%06d", hh, mm, ss, ms, us)
                : String::Format("%d.%02d:%02d:%02d.%03d.%06d", dd, hh, mm, ss,
                                 ms, us);
  return sign + value;
}
...
bool Timespan::TryAssign(int32 days, int32 hours, int32 minutes, int32 seconds,
                         int32 milliseconds, int32 microseconds) {
  const int64 ticks = days * DateTimeConstants::TICKS_PER_DAY +
...
void Timespan::Assign(int32 days, int32 hours, int32 minutes, int32 seconds,
                      int32 milliseconds, int32 microseconds) {
...
uint32 HashOf(const Timespan& timespan) { return HashOf(timespan.ticks_); }
