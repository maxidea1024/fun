...
struct UniqueLock {
  ...
  UniqueLock(MutexType& mutex, const std::chrono::duration<Rep, Period>& timeout)
    : mutex_(&mutex)
  ...
  UniqueLock(MutexType& mutex, const std::chrono::time_point<Clock, Duration>& timeout)
    : mutex_(&mutex)
  ...
  MutexType* GetMutex() const noexcept
  {
    return mutex_;
  }
