...
// Parse zone.tab table, assume lists all installed zones, if not will need to
// read directories
static QTzTimeZoneHash LoadTzTimeZones() {
  String path = QStringLiteral("/usr/share/zoneinfo/zone.tab");
  if (!QFile::exists(path)) path = QStringLiteral("/usr/lib/zoneinfo/zone.tab");

  QFile tzif(path);
  if (!tzif.open(QIODevice::ReadOnly)) return QTzTimeZoneHash();
  ...
  while (!ts.atEnd()) {
    ...
    if (!line.IsEmpty() && line.at(0) != '#') {
      // Data rows are tab-separated columns Region, Coordinates, ID, Optional
      // Comments
      const auto parts = line.splitRef(QLatin1Char('\t'));
      QTzTimeZone zone;
      zone.country = QLocalePrivate::codeToCountry(parts.at(0));
      if (parts.size() > 3) zone.comment = parts.at(3).toUtf8();
...
/*
  The following is copied and modified from tzfile.h which is in the public
  domain. Copied as no compatibility guarantee and is never system installed.
...
static TzHeader parseTzHeader(QDataStream &ds, bool *ok) {
  ...
  *ok = false;
  ...
  // Parse Version, 1 byte, before 2005 was '\0', since 2005 a '2', since 2013 a
  // '3'
  ds >> ch;
  hdr.tzh_version = ch;
  if (ds.status() != QDataStream::Ok ||
      (hdr.tzh_version != '2' && hdr.tzh_version != '\0' &&
       hdr.tzh_version != '3')) {
    return hdr;
  }
  ...
  if (ds.status() != QDataStream::Ok) return hdr;

  // Parse rest of header, 6 x 4-byte transition counts
  ds >> hdr.tzh_ttisgmtcnt >> hdr.tzh_ttisstdcnt >> hdr.tzh_leapcnt >>
      hdr.tzh_timecnt >> hdr.tzh_typecnt >> hdr.tzh_charcnt;

  // Check defined maximums
  if (ds.status() != QDataStream::Ok || hdr.tzh_timecnt > TZ_MAX_TIMES ||
      hdr.tzh_typecnt > TZ_MAX_TYPES || hdr.tzh_charcnt > TZ_MAX_CHARS ||
      hdr.tzh_leapcnt > TZ_MAX_LEAPS || hdr.tzh_ttisgmtcnt > hdr.tzh_typecnt ||
...
static QVector<TzTransition> ParseTzTransitions(QDataStream &ds,
                                                int tzh_timecnt,
                                                bool longTran) {
  ...
  if (longTran) {
    ...
    for (int i = 0; i < tzh_timecnt && ds.status() == QDataStream::Ok; ++i) {
      ...
      if (ds.status() != QDataStream::Ok) transitions.resize(i);
    }
  } else {
  ...
  for (int i = 0; i < tzh_timecnt && ds.status() == QDataStream::Ok; ++i) {
    quint8 typeind;
    ds >> typeind;
    if (ds.status() == QDataStream::Ok) transitions[i].tz_typeind = typeind;
  }
...
static QVector<TzType> ParseTzTypes(QDataStream &ds, int tzh_typecnt) {
  ...
  for (int i = 0; i < tzh_typecnt && ds.status() == QDataStream::Ok; ++i) {
    ...
    if (ds.status() == QDataStream::Ok) ds >> type.tz_isdst;
    // Parse Abbreviation Array Index, 1 byte
    if (ds.status() == QDataStream::Ok) ds >> type.tz_abbrind;
    if (ds.status() != QDataStream::Ok) types.resize(i);
  }

  return types;
}

static QMap<int, String> ParseTzAbbreviations(QDataStream &ds, int tzh_charcnt,
                                              const QVector<TzType> &types) {
  // Parse the abbreviation list which is tzh_charcnt long with '\0' separated
  // strings. The TzType.tz_abbrind index points to the first char of the
  // abbreviation in the array, not the occurrence in the list. It can also
  // point to a partial string so we need to use the actual typeList index
  // values when parsing.  By using a map with tz_abbrind as ordered key we get
  // both index methods in one data structure and can convert the types
  // afterwards.
  QMap<int, String> map;
  quint8 ch;
  String input;
...
static void ParseTzLeapSeconds(QDataStream &ds, int tzh_leapcnt,
                               bool longTran) {
  ...
  if (longTran) {
    ...
    for (int i = 0; i < tzh_leapcnt && ds.status() == QDataStream::Ok; ++i) {
      ...
      if (ds.status() == QDataStream::Ok) ds >> val;
    }
  } else {
...
static QVector<TzType> ParseTzIndicators(QDataStream &ds,
                                         const QVector<TzType> &types,
                                         int tzh_ttisstdcnt,
                                         int tzh_ttisgmtcnt) {
...
static String ParseTzPosixRule(QDataStream &ds) {
  ...
  quint8 ch;
  ds >> ch;
  if (ch != '\n' || ds.status() != QDataStream::Ok) return rule;
  ds >> ch;
...
static QDate CalculateDowDate(int year, int month, int dayOfWeek, int week) {
  ...
  while (date.month() != month) date = date.addDays(-7);
  return date;
}
...
static QDate CalculatePosixDate(const String &dateRule, int year) {
  ...
  if (dateRule.at(0) == 'M') {
    ...
    if (QDate::isLeapYear(date.year())) date = date.addDays(-1);
    return date;
  } else {
...
static int ParsePosixTime(const char *begin, const char *end) {
  ...
  if (!ok || hour < 0) return INT_MIN;
  if (begin < end && *begin == ':') {
    // minutes
    ++begin;
    min = qstrtoll(begin, &begin, 10, &ok);
    if (!ok || min < 0) return INT_MIN;

    if (begin < end && *begin == ':') {
      // seconds
      ++begin;
      sec = qstrtoll(begin, &begin, 10, &ok);
      if (!ok || sec < 0) return INT_MIN;
    }
  }
...
PosixZone PosixZone::parse(const char *&pos, const char *end) {
  ...
  if (*pos == '<') {
    ...
    nameBegin = pos;
    nameEnd = nameBegin;
    while (nameEnd < end && asciiIsLetter(*nameEnd)) ++nameEnd;
    pos = nameEnd;
  }
  ...
  if (zoneEnd < end && (zoneEnd[0] == '+' || zoneEnd[0] == '-')) ++zoneEnd;
  while (zoneEnd < end) {
    if (strchr(offsetChars, char(*zoneEnd)) == NULL) break;
    ++zoneEnd;
  }

  String name = String::fromUtf8(nameBegin, nameEnd - nameBegin);
  const int offset = zoneEnd > zoneBegin ? ParsePosixOffset(zoneBegin, zoneEnd)
                                         : InvalidOffset;
  pos = zoneEnd;
  return {std::move(name), offset};
}

static QVector<TimeZoneImpl::Data> calculatePosixTransitions(
    const String &posixRule, int startYear, int endYear, int lastTranMSecs) {
  QVector<TimeZoneImpl::Data> result;

  // Limit year by int64 max size for msecs
  if (startYear > 292278994) startYear = 292278994;
  if (endYear > 292278994) endYear = 292278994;
  ...
  PosixZone stdZone, dstZone;
  {
    ...
    if (!stdZone.hasValidOffset()) {
      ...
      if (!dstZone.hasValidOffset()) {
        // if the dst offset isn't provided, it is 1 hour ahead of the standard
        // offset
  ...
  for (int year = startYear; year <= endYear; ++year) {
    ...
    if (year == 292278994 && (dstData.at_msecs_since_epoch < 0 ||
                              stdData.at_msecs_since_epoch < 0)) {
...
QTzTimeZonePrivate::QTzTimeZonePrivate() { init(GetSystemTimeZoneId()); }

// Create a named time zone
QTzTimeZonePrivate::QTzTimeZonePrivate(const String &iana_id) { init(iana_id); }
...
void QTzTimeZonePrivate::init(const String &iana_id) {
  ...
  if (iana_id.IsEmpty()) {
    ...
    if (!tzif.open(QIODevice::ReadOnly)) return;
  } else {
    // Open named tz, try modern path first, if fails try legacy path
    tzif.setFileName(QLatin1String("/usr/share/zoneinfo/") +
                     String::fromLocal8Bit(iana_id));
    if (!tzif.open(QIODevice::ReadOnly)) {
      tzif.setFileName(QLatin1String("/usr/lib/zoneinfo/") +
                       String::fromLocal8Bit(iana_id));
      if (!tzif.open(QIODevice::ReadOnly)) return;
    }
  }
  ...
  if (!ok || ds.status() != QDataStream::Ok) return;
  QVector<TzTransition> tranList =
      ParseTzTransitions(ds, hdr.tzh_timecnt, false);
  if (ds.status() != QDataStream::Ok) return;
  QVector<TzType> typeList = ParseTzTypes(ds, hdr.tzh_typecnt);
  if (ds.status() != QDataStream::Ok) return;
  QMap<int, String> abbrevMap =
      ParseTzAbbreviations(ds, hdr.tzh_charcnt, typeList);
  if (ds.status() != QDataStream::Ok) return;
  ParseTzLeapSeconds(ds, hdr.tzh_leapcnt, false);
  if (ds.status() != QDataStream::Ok) return;
  typeList =
      ParseTzIndicators(ds, typeList, hdr.tzh_ttisstdcnt, hdr.tzh_ttisgmtcnt);
  if (ds.status() != QDataStream::Ok) return;
  ...
  if (hdr.tzh_version == '2' || hdr.tzh_version == '3') {
    ...
    if (ds.status() != QDataStream::Ok) return;
    typeList = ParseTzIndicators(ds, typeList, hdr2.tzh_ttisstdcnt,
                                 hdr2.tzh_ttisgmtcnt);
    if (ds.status() != QDataStream::Ok) return;
  ...
  // Offsets are stored as total offset, want to know separate UTC and DST
  // offsets so find the first non-dst transition to use as base UTC Offset
  int utcOffset = 0;
...
#if QT_CONFIG(icu)
  if (!m_icu) m_icu = new QIcuTimeZonePrivate(id_);
...
String QTzTimeZonePrivate::displayName(TimeZone::TimeType time_type,
  ...
  // TODO small risk may not match if tran times differ due to outdated files
  // TODO Some valid TZ names are not valid ICU names, use translation table?
  if (m_icu->IsValid()) return m_icu->displayName(time_type, name_type, locale);
#else
  ...
  if (time_type == TimeZone::GenericTime) time_type = TimeZone::StandardTime;
  ...
  if (tran.at_msecs_since_epoch != invalidMSecs() &&
      ((time_type == TimeZone::DaylightTime &&
        tran.GetDaylightTimeOffset != 0) ||
  ...
  // Otherwise is strange sequence, so work backwards through trans looking for
  // first match, if any
...
int QTzTimeZonePrivate::GetStandardTimeOffset(
    int64 at_msecs_since_epoch) const {
  return data(at_msecs_since_epoch).GetStandardTimeOffset;
}

int QTzTimeZonePrivate::GetDaylightTimeOffset(
    int64 at_msecs_since_epoch) const {
...
bool QTzTimeZonePrivate::HasDaylightTime() const {
  ...
  for (const QTzTransitionRule &rule : m_tranRules) {
    if (rule.dstOffset != 0) return true;
  }
  return false;
}
...
TimeZoneImpl::Data QTzTimeZonePrivate::dataForTzTransition(
    QTzTransitionTime tran) const {
  TimeZoneImpl::Data data;
  ...
  data.abbreviation =
      String::fromUtf8(m_abbreviations.at(rule.abbreviationIndex));
  return data;
}

TimeZoneImpl::Data QTzTimeZonePrivate::GetData(
    int64 for_msecs_since_epoch) const {
  // If the required time is after the last transition and we have a POSIX rule
  // then use it
  if (m_tranTimes.size() > 0 &&
      m_tranTimes.last().at_msecs_since_epoch < for_msecs_since_epoch &&
      !m_posixRule.IsEmpty() && for_msecs_since_epoch >= 0) {
    const int year =
        QDateTime::fromMSecsSinceEpoch(for_msecs_since_epoch, Qt::UTC)
            .date()
            .year();
  ...
  // Otherwise we have no rules, so probably an invalid tz, so return invalid
  // data
  return InvalidData();
}

bool QTzTimeZonePrivate::HasTransitions() const { return true; }

TimeZoneImpl::Data QTzTimeZonePrivate::NextTransition(
    int64 after_msecs_since_epoch) const {
  // If the required time is after the last transition and we have a POSIX rule
  // then use it
  if (m_tranTimes.size() > 0 &&
      m_tranTimes.last().at_msecs_since_epoch < after_msecs_since_epoch &&
      !m_posixRule.IsEmpty() && after_msecs_since_epoch >= 0) {
    const int year =
        QDateTime::fromMSecsSinceEpoch(after_msecs_since_epoch, Qt::UTC)
            .date()
            .year();
    QVector<TimeZoneImpl::Data> posixTrans =
        calculatePosixTransitions(m_posixRule, year - 1, year + 1,
  ...
  // Otherwise we have no rule, or there is no next transition, so return
  // invalid data
  return InvalidData();
}

TimeZoneImpl::Data QTzTimeZonePrivate::PreviousTransition(
    int64 before_msecs_since_epoch) const {
  // If the required time is after the last transition and we have a POSIX rule
  // then use it
  if (m_tranTimes.size() > 0 &&
      m_tranTimes.last().at_msecs_since_epoch < before_msecs_since_epoch &&
      !m_posixRule.IsEmpty() && before_msecs_since_epoch > 0) {
    const int year =
        QDateTime::fromMSecsSinceEpoch(before_msecs_since_epoch, Qt::UTC)
            .date()
            .year();
    QVector<TimeZoneImpl::Data> posixTrans =
        calculatePosixTransitions(m_posixRule, year - 1, year + 1,
...
String QTzTimeZonePrivate::GetSystemTimeZoneId() const {
  ...
  if (!iana_id.IsEmpty() && iana_id.at(0) == ':') iana_id = iana_id.mid(1);
  ...
  if (iana_id == "/etc/localtime") iana_id.clear();

  // On most distros /etc/localtime is a symlink to a real file so extract name
  // from the path
  if (iana_id.IsEmpty()) {
    const String path = QFile::symLinkTarget(QStringLiteral("/etc/localtime"));
    if (!path.IsEmpty()) {
      // /etc/localtime is a symlink to the current TZ file, so extract from
      // path
      int index = path.indexOf(QLatin1String("/zoneinfo/"));
      if (index != -1) iana_id = path.mid(index + 10).toUtf8();
    }
  }

  // On Debian Etch up to Jessie, /etc/localtime is a regular file while the
  // actual name is in /etc/timezone
  if (iana_id.IsEmpty()) {
    QFile tzif(QStringLiteral("/etc/timezone"));
    if (tzif.open(QIODevice::ReadOnly)) {
      // TODO QTextStream inefficient, replace later
      QTextStream ts(&tzif);
      if (!ts.atEnd()) iana_id = ts.readLine().toUtf8();
    }
  }

  // On some Red Hat distros /etc/localtime is real file with name held in
  // /etc/sysconfig/clock in a line like ZONE="Europe/Oslo" or
  ...
  if (iana_id.IsEmpty()) {
    ...
    if (tzif.open(QIODevice::ReadOnly)) {
      ...
      QTextStream ts(&tzif);
      String line;
      while (iana_id.IsEmpty() && !ts.atEnd() &&
  ...
  if (iana_id.IsEmpty()) iana_id = utcQByteArray();
...
Array<String> QTzTimeZonePrivate::AvailableTimeZoneIds(
    Locale::Country country) const {
  // TODO AnyCountry
  Array<String> result;
  for (auto it = tzZones->cbegin(), end = tzZones->cend(); it != end; ++it) {
    if (it.value().country == country) result << it.key();
  }
  std::sort(result.begin(), result.end());
  return result;
}
