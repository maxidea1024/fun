...
MonitoredProcess::MonitoredProcess(const String& url, const String& params,
                                   bool hidden, bool create_pipes)
    : cancelling_(false),
...
Timespan MonitoredProcess::GetDuration() const {
  return IsRunning() ? (DateTime::UtcNow() - start_time_)
                     : (end_time_ - start_time_);
}

bool MonitoredProcess::IsRunning() const { return !!thread_; }
...
bool MonitoredProcess::Launch() {
  ...
  process_handle_ = CPlatformProcess::CreateNewProcess(
      *url_, *params_, false, hidden_, hidden_, nullptr, 0, *CPaths::RootDir(),
      write_pipe_);
  ...
  const String MonitoredProcessName =
      String::Format(TEXT("MonitoredProcess %d"), MonitoredProcessIndex);
  MonitoredProcessIndex++;

  thread_ = RunnableThread::Create(this, *MonitoredProcessName, 128 * 1024,
