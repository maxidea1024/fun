...
SHA1Digester::~SHA1Digester() { Reset(); }

int32 SHA1Digester::GetDigestLength() { return DIGEST_SIZE; }
...
const Digester::Digest& SHA1Digester::GetDigest() {
  ...
  for (count = 0; count < DIGEST_SIZE; count++) {
    hash[count] =
        (uint8)((context_.digest[count >> 2]) >> (8 * (3 - (count & 0x3)))) &
        0xff;
  }
  digest_.Clear();
...
#define expand(count)                                            \
  W[count] = S(1, (W[count - 3] ^ W[count - 8] ^ W[count - 14] ^ \
