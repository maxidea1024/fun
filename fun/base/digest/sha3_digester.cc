...
static const uint32 KeccakRhoOffsets[25] = {0,  1, 62, 28, 27, 36, 44, 6,  55,
...
#ifdef FUN_PTR_IS_64_BIT
void KeccakP1600_AddBytes(void *state, const uint8 *data, uint32 offset,
                          uint32 length) {
...
void KeccakP1600_ExtractBytes(const void *state, uint8 *data, uint32 offset,
                              uint32 length) {
  fun_check(offset < 200);
  fun_check(offset + length <= 200);
...
void KeccakP1600_AddBytesInLane(void *state, uint32 lanePosition,
...
void KeccakP1600_AddBytes(void *state, const uint8 *data, uint32 offset,
  ...
  while (length > 0) {
    ...
    if (bytesInLane > length) bytesInLane = length;
    KeccakP1600_AddBytesInLane(state, lanePosition, data, offsetInLane,
                               bytesInLane);
    length -= bytesInLane;
    lanePosition++;
    offsetInLane = 0;
...
void ROL64(uint32 inEven, uint32 inOdd, uint32 *outEven, uint32 *outOdd,
           uint32 offset) {
  if ((offset % 2) == 0) {
...
static void rho(uint32 *A) {
  ...
  for (x = 0; x < 5; x++)
    for (y = 0; y < 5; y++)
      ROL64(A[index(x, y, 0)], A[index(x, y, 1)], &(A[index(x, y, 0)]),
            &(A[index(x, y, 1)]), KeccakRhoOffsets[5 * y + x]);
}
...
void KeccakP1600_ExtractBytesInLane(const void *state, uint32 lanePosition,
                                    uint8 *data, uint32 offset, uint32 length) {
  if ((lanePosition < 25) && (offset < 8) && (offset + length <= 8)) {
    uint32 *stateAsHalfLanes = (uint32 *)state;
    uint32 lane[2];
    UInt8 laneAsBytes[8];
    fromBitInterleaving(stateAsHalfLanes[lanePosition * 2],
                        stateAsHalfLanes[lanePosition * 2 + 1], lane, lane + 1);
...
void KeccakP1600_ExtractBytes(const void *state, uint8 *data, uint32 offset,
  ...
  while (length > 0) {
    uint32 bytesInLane = 8 - offsetInLane;
    if (bytesInLane > length) bytesInLane = length;
    KeccakP1600_ExtractBytesInLane(state, lanePosition, data, offsetInLane,
                                   bytesInLane);
    length -= bytesInLane;
    lanePosition++;
    offsetInLane = 0;
...
int32 SHA3Digester::GetDigestLength() { return (int32)algorithm_ / 8; }

int KeccakWidth1600_SpongeInitialize(KeccakWidth1600_SpongeInstance *instance,
                                     uint32 rate, uint32 capacity) {
...
int KeccakWidth1600_SpongeAbsorbLastFewBits(
    KeccakWidth1600_SpongeInstance *instance, uint8 delimitedData) {
...
int SpongeAbsorbLastFewBits(KeccakWidth1600_SpongeInstance *instance,
                            uint8 delimitedData) {
  uint32 rateInBytes = instance->rate / 8;
  if (delimitedData == 0) return 1;
  if (instance->squeezing) return 1; /* Too late for additional input */
...
int KeccakWidth1600_SpongeSqueeze(KeccakWidth1600_SpongeInstance *instance,
                                  uint8 *data, size_t dataByteLen) {
  size_t i, j;
  uint32 partialBlock;
  uint32 rateInBytes = instance->rate / 8;
  uint8 *curData;

  if (!instance->squeezing) SpongeAbsorbLastFewBits(instance, 0x01);
  ...
  while (i < dataByteLen) {
    if ((instance->byteIOIndex == rateInBytes) &&
      ...
      KeccakP1600_ExtractBytes(instance->state, curData, instance->byteIOIndex,
                               partialBlock);
...
const Digester::Digest &SHA3Digester::GetDigest() {
  ...
  if (KeccakWidth1600_SpongeAbsorbLastFewBits(&context->sponge,
                                              context->delimitedSuffix) == 0) {
    KeccakWidth1600_SpongeSqueeze(&context->sponge, hash,
                                  context->fixedOutputLength / 8);
  }
...
void SHA3Digester::UpdateImpl(const void *input, size_t input_length) {
  ...
  while (i < input_length) {
    if (((&context->sponge)->byteIOIndex == 0) &&
        (input_length >= (i + rateInBytes))) {
      for (j = input_length - i; j >= rateInBytes; j -= rateInBytes) {
        KeccakP1600_AddBytes((&context->sponge)->state, curData, 0,
                             rateInBytes);
      ...
      KeccakP1600_AddBytes((&context->sponge)->state, curData,
