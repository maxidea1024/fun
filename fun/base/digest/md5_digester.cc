// TODO libsodium으로 교체하는게 좋을듯...
#include "fun/base/digest/md5_digester.h"

namespace fun {

MD5Disgester::MD5Disgester() {
  digest_.Reserve(16);
  Reset();
}

MD5Disgester::~MD5Disgester() { Reset(); }

int32 MD5Disgester::GetDigestLength() { return DIGEST_SIZE; }

void MD5Disgester::Reset() {
...
const Digester::Digest& MD5Disgester::GetDigest() {
...
void MD5Disgester::UpdateImpl(const void* _input, size_t input_len) {
  ...
  if ((context_.count[0] += ((uint32)input_len << 3)) <
      ((uint32)input_len << 3)) {
    context_.count[1]++;
  }
...
void MD5Disgester::Transform(uint32 state[4], const uint8 block[64]) {
  ...
  /* Zeroize sensitive information. */
  UnsafeMemory::Memzero(x, sizeof(x));
}

void MD5Disgester::Encode(uint8* output, const uint32* input, int32 len) {
  uint32 i, j;
...
void MD5Disgester::Decode(uint32* output, const uint8* input, int32 len) {
  uint32 i, j;
