...
typedef struct {
  ...
  SHA2Digester::Algorithm size;
  uint8 buffer[128];
} HASHCONTEXT;
static const uint32 K32[] = {
...
#define GET_UINT64(n, b, i)                                             \
  {                                                                     \
    (n) = ((uint64)(b)[(i)]) | ((uint64)(b)[(i) + 1] << 8) |            \
          ((uint64)(b)[(i) + 2] << 16) | ((uint64)(b)[(i) + 3] << 24) | \
          ((uint64)(b)[(i) + 4] << 32) | ((uint64)(b)[(i) + 5] << 40) | \
...
#define GET_UINT32(n, b, i)                                         \
  do {                                                              \
    (n) = ((uint32)(b)[(i)] << 24) | ((uint32)(b)[(i) + 1] << 16) | \
...
SHA2Digester::SHA2Digester(Algorithm algorithm)
    : context_(nullptr), algorithm_(algorithm) {
  digest_.Reserve(GetDigestLength());
  Reset();
}

SHA2Digester::~SHA2Digester() {
  Reset();
  UnsafeMemory::Free(context_);
}

int32 SHA2Digester::GetDigestLength() { return (int32)algorithm_ / 8; }

void SHA2Digester::Reset() {
  if (context_ != NULL) {
    free(context_);
  }
  context_ = calloc(1, sizeof(HASHCONTEXT));
  HASHCONTEXT* pContext = (HASHCONTEXT*)context_;
  pContext->size = algorithm_;
  if (algorithm_ == SHA2_224) {
...
const Digester::Digest& SHA2Digester::GetDigest() {
  digest_.Clear();
  if (context_ == NULL) return digest_;
  HASHCONTEXT* pContext = (HASHCONTEXT*)context_;
  size_t last, padn;
  uint8 hash[64];
  UnsafeMemory::Memset(hash, 0, 64);
  if (pContext->size > SHA2_256) {
    uint8 msglen[16];
    uint64 high =
        (pContext->total.total64[0] >> 61) | (pContext->total.total64[1] << 3);
    uint64 low = (pContext->total.total64[0] << 3);
    ...
    uint8 msglen[8];
    uint32 high =
        (pContext->total.total32[0] >> 29) | (pContext->total.total32[1] << 3);
    uint32 low = (pContext->total.total32[0] << 3);
    ...
    if (pContext->size > SHA2_224)
      PUT_UINT32(pContext->state.state32[7], hash, 28);
  }
...
void _sha256_process(HASHCONTEXT* pContext, const uint8 data[64]) {
  uint32 i;
  uint32 temp1, temp2, temp3[8], W[64];
  for (i = 0; i < 8; i++) temp3[i] = pContext->state.state32[i];
  for (i = 0; i < 16; i++) {
    GET_UINT32(W[i], data, 4 * i);
  }
  for (i = 0; i < 16; i += 8) {
    P32(temp3[0], temp3[1], temp3[2], temp3[3], temp3[4], temp3[5], temp3[6],
        temp3[7], W[i + 0], K32[i + 0]);
    P32(temp3[7], temp3[0], temp3[1], temp3[2], temp3[3], temp3[4], temp3[5],
        temp3[6], W[i + 1], K32[i + 1]);
    P32(temp3[6], temp3[7], temp3[0], temp3[1], temp3[2], temp3[3], temp3[4],
        temp3[5], W[i + 2], K32[i + 2]);
    P32(temp3[5], temp3[6], temp3[7], temp3[0], temp3[1], temp3[2], temp3[3],
        temp3[4], W[i + 3], K32[i + 3]);
    P32(temp3[4], temp3[5], temp3[6], temp3[7], temp3[0], temp3[1], temp3[2],
        temp3[3], W[i + 4], K32[i + 4]);
    P32(temp3[3], temp3[4], temp3[5], temp3[6], temp3[7], temp3[0], temp3[1],
        temp3[2], W[i + 5], K32[i + 5]);
    P32(temp3[2], temp3[3], temp3[4], temp3[5], temp3[6], temp3[7], temp3[0],
        temp3[1], W[i + 6], K32[i + 6]);
    P32(temp3[1], temp3[2], temp3[3], temp3[4], temp3[5], temp3[6], temp3[7],
        temp3[0], W[i + 7], K32[i + 7]);
  }
  for (i = 16; i < 64; i += 8) {
    P32(temp3[0], temp3[1], temp3[2], temp3[3], temp3[4], temp3[5], temp3[6],
        temp3[7], R32(i + 0), K32[i + 0]);
    P32(temp3[7], temp3[0], temp3[1], temp3[2], temp3[3], temp3[4], temp3[5],
        temp3[6], R32(i + 1), K32[i + 1]);
    P32(temp3[6], temp3[7], temp3[0], temp3[1], temp3[2], temp3[3], temp3[4],
        temp3[5], R32(i + 2), K32[i + 2]);
    P32(temp3[5], temp3[6], temp3[7], temp3[0], temp3[1], temp3[2], temp3[3],
        temp3[4], R32(i + 3), K32[i + 3]);
    P32(temp3[4], temp3[5], temp3[6], temp3[7], temp3[0], temp3[1], temp3[2],
        temp3[3], R32(i + 4), K32[i + 4]);
    P32(temp3[3], temp3[4], temp3[5], temp3[6], temp3[7], temp3[0], temp3[1],
        temp3[2], R32(i + 5), K32[i + 5]);
    P32(temp3[2], temp3[3], temp3[4], temp3[5], temp3[6], temp3[7], temp3[0],
        temp3[1], R32(i + 6), K32[i + 6]);
    P32(temp3[1], temp3[2], temp3[3], temp3[4], temp3[5], temp3[6], temp3[7],
        temp3[0], R32(i + 7), K32[i + 7]);
  }
  for (i = 0; i < 8; i++) pContext->state.state32[i] += temp3[i];
}

void _sha512_process(HASHCONTEXT* pContext, const uint8 data[128]) {
  int i;
  uint64 temp1, temp2, temp3[8], W[80];
  for (i = 0; i < 16; i++) {
    GET_UINT64(W[i], data, i << 3);
  }
  for (; i < 80; i++) {
    W[i] = S641(W[i - 2]) + W[i - 7] + S640(W[i - 15]) + W[i - 16];
  }
  for (i = 0; i < 8; i++) temp3[i] = pContext->state.state64[i];
  i = 0;
  do {
    P64(temp3[0], temp3[1], temp3[2], temp3[3], temp3[4], temp3[5], temp3[6],
        temp3[7], W[i], K64[i]);
    i++;
    P64(temp3[7], temp3[0], temp3[1], temp3[2], temp3[3], temp3[4], temp3[5],
        temp3[6], W[i], K64[i]);
    i++;
    P64(temp3[6], temp3[7], temp3[0], temp3[1], temp3[2], temp3[3], temp3[4],
        temp3[5], W[i], K64[i]);
    i++;
    P64(temp3[5], temp3[6], temp3[7], temp3[0], temp3[1], temp3[2], temp3[3],
        temp3[4], W[i], K64[i]);
    i++;
    P64(temp3[4], temp3[5], temp3[6], temp3[7], temp3[0], temp3[1], temp3[2],
        temp3[3], W[i], K64[i]);
    i++;
    P64(temp3[3], temp3[4], temp3[5], temp3[6], temp3[7], temp3[0], temp3[1],
        temp3[2], W[i], K64[i]);
    i++;
    P64(temp3[2], temp3[3], temp3[4], temp3[5], temp3[6], temp3[7], temp3[0],
        temp3[1], W[i], K64[i]);
    i++;
    P64(temp3[1], temp3[2], temp3[3], temp3[4], temp3[5], temp3[6], temp3[7],
        temp3[0], W[i], K64[i]);
    i++;
  } while (i < 80);
...
void SHA2Digester::UpdateImpl(const void* input, size_t input_len) {
  if (context_ == NULL || input == NULL || input_len == 0) return;
  uint32 left = 0;
  HASHCONTEXT* pContext = (HASHCONTEXT*)context_;
  uint8* data = (uint8*)input;
  if (pContext->size > SHA2_256) {
    left = (uint32)(pContext->total.total64[0] & 0x7F);
    size_t fill = 128 - left;
    pContext->total.total64[0] += (uint64)input_len;
    if (pContext->total.total64[0] < (uint64)input_len)
      pContext->total.total64[1]++;
    if (left && input_len >= fill) {
      UnsafeMemory::Memcpy((void*)(pContext->buffer + left), data, fill);
    ...
    left = (uint32)(pContext->total.total32[0] & 0x3F);
    size_t fill = 64 - left;
    pContext->total.total32[0] += (uint32)input_len;
    pContext->total.total32[0] &= 0xFFFFFFFF;
    if (pContext->total.total32[0] < (uint32)input_len)
      pContext->total.total32[1]++;
    if (left && input_len >= fill) {
      UnsafeMemory::Memcpy((void*)(pContext->buffer + left), data, fill);
  ...
  if (input_len > 0) {
    UnsafeMemory::Memcpy((void*)(pContext->buffer + left), data, input_len);
  }
}

// TODO remove this
void SHA2Digester::Transform() {
  // NOOP
}
