...
typename std::conditional<IsMoveCallable<Func>::value, Arg&&, const Arg&>::type
tryMove(Arg& arg) {
  return std::move(arg);
}
...
struct Core {
  Core(State _state, TypeId _id)
      : state(_state), exc(), mtx(), requests(), id(_id) {}
...
class Deferred {
 public:
  ...
  bool resolve(U&& arg) {
    ...
    static_assert(
        std::is_same<T, CleanU>::value || std::is_convertible<U, T>::value,
        "Types mismatch");
...
struct When {
 ...
 private:
  ...
  struct WhenContinuation {
    WhenContinuation(Data data_) : data(std::move(data_)) {}
...
struct WhenAllRange {
 ...
 private:
  ...
  struct WhenContinuation {
    ...
    WhenContinuation(const D& data_, size_t _index)
        : data(data_), index(_index) {}
  ...
  struct WhenContinuation<void, Dummy> {
    ...
    WhenContinuation(const D& data_, size_t) : data(data_) {}
