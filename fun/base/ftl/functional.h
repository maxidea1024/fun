...
struct ReversePredicate {
  ReversePredicate(const Predicate& pred)
    : pred_(pred) {}
...
FUN_ALWAYS_INLINE auto DereferenceIfNecessary(Callable&& callable)
  -> typename EnableIf<PointerIsConvertibleFromTo<typename Decay<Callable>::Type, typename Decay<Base>::Type>::Value, decltype((Callable&&)callable)>::Type {
...
FUN_ALWAYS_INLINE decltype(auto) Invoke(R C::*pdm, Callable&& callable) {
  return Invoke_internal::DereferenceIfNecessary<C>(Forward<Callable>(callable)).*pdm;
}

template <typename R, typename C, typename... PMFArgs, typename Callable, typename... Args>
FUN_ALWAYS_INLINE decltype(auto) Invoke(R (C::*method)(PMFArgs...), Callable&& callable, Args&&... args) {
  return (Invoke_internal::DereferenceIfNecessary<C>(Forward<Callable>(callable)).*method)(Forward<Args>(args)...);
}

template <typename R, typename C, typename... PMFArgs, typename Callable, typename... Args>
FUN_ALWAYS_INLINE decltype(auto) Invoke(R (C::*method)(PMFArgs...) const, Callable&& callable, Args&&... args) {
  return (Invoke_internal::DereferenceIfNecessary<C>(Forward<Callable>(callable)).*method)(Forward<Args>(args)...);
}
...
struct IsInvocable_impl<typename Void<decltype(Invoke(DeclVal<Callable>(), DeclVal<Args>()...))>::Type, Callable, Args...> {
  enum { Value = true };
};
 ...
 */
template <typename Callable, typename... Args>
struct IsInvocable : IsInvocable_internal::IsInvocable_impl<void, Callable, Args...> {};
...
FUN_ALWAYS_INLINE uint32 HashOf(const uint8 x) {
  return x;
}

FUN_ALWAYS_INLINE uint32 HashOf(const int8 x) {
  return x;
}

FUN_ALWAYS_INLINE uint32 HashOf(const uint16 x) {
  return x;
}

FUN_ALWAYS_INLINE uint32 HashOf(const int16 x) {
  return x;
}

FUN_ALWAYS_INLINE uint32 HashOf(const int32 x) {
  return x;
}

FUN_ALWAYS_INLINE uint32 HashOf(const uint32 x) {
  return x;
}
...
FUN_ALWAYS_INLINE uint32 HashOf(const char* x) {
...
FUN_ALWAYS_INLINE uint32 HashOf(const void* x) {
  return PointerHash(x);
}

FUN_ALWAYS_INLINE uint32 HashOf(void* x) {
  return PointerHash(x);
}

template <typename Enum> FUN_ALWAYS_INLINE
typename EnableIf<IsEnum<Enum>::Value, uint32>::Type HashOf(Enum e) {
  return HashOf((__underlying_type(Enum))e);
}

//template <typename WrappedEnum> FUN_ALWAYS_INLINE
//typename EnableIf<IsWrappedEnum<WrappedEnum>::Value, uint32>::Type HashOf(WrappedEnum e)
//{
