...
template <typename... Ts> struct TupleSize<TupleTypes<Ts...>> : public IntegralConstant<size_t, sizeof...(Ts)> {};
template <typename... Ts> struct TupleSize<Tuple<Ts...>> : public IntegralConstant<size_t, sizeof...(Ts)> {};
...
class TupleSize<internal::TupleImpl<Indices, Ts...>> : public IntegralConstant<size_t, sizeof...(Ts)> {};
...
class TupleElement<I, internal::TupleImpl<Indices, Ts...>> : public TupleElement<I, Tuple<Ts...>> {};

template <size_t I, typename Indices, typename... Ts>
class TupleElement<I, const internal::TupleImpl<Indices, Ts...>> : public TupleElement<I, const Tuple<Ts...>> {};

template <size_t I, typename Indices, typename... Ts>
class TupleElement<I, volatile internal::TupleImpl<Indices, Ts...>> : public TupleElement<I, volatile Tuple<Ts...>> {};

template <size_t I, typename Indices, typename... Ts>
class TupleElement<I, const volatile internal::TupleImpl<Indices, Ts...>> : public TupleElement<I, const volatile Tuple<Ts...>> {};
...
struct TupleIndex<T, TupleTypes<T, TsRest...>> {
  ...
  static_assert(IsVoid<typename TupleIndex<T, TupleTypes<TsRest...>>::DuplicateTypeCheck>::Value, "duplicate type T in tuple_vector::Get<T>(); unique types must be provided in declaration, or only use Get<size_t>()");
...
struct TupleIndex<T, internal::TupleImpl<Indices, Ts...>> : public TupleIndex<T, TupleTypes<Ts...>> {};


namespace internal {

// TupleLeaf
template <size_t I, typename ValueType, bool IsEmpty = fun::IsEmpty<ValueType>::Value>
class TupleLeaf;

template <size_t I, typename ValueType, bool IsEmpty>
FUN_ALWAYS_INLINE void Swap(TupleLeaf<I, ValueType, IsEmpty>& a, TupleLeaf<I, ValueType, IsEmpty>& b) {
...
template <size_t I, typename Indices, typename... Ts>
ConstTupleElement_T<I, TupleImpl<Indices, Ts...>>& Get(const TupleImpl<Indices, Ts...>& t);
...
struct TupleImpl<IntegerSequence<size_t, Indices...>, Ts...> : public TupleLeaf<Indices, Ts>... {
  ...
  // IndexSequence changed to IntegerSequence due to issues described below in VS2015 CTP 6.
  ...
  TupleImpl(OtherTuple&& t)
    : TupleLeaf<Indices, Ts>(Forward<TupleElement_T<Indices, MakeTupleTypes_T<OtherTuple>>>(Get<Indices>(t)))... {
  }

  template <typename OtherTuple>
  TupleImpl& operator=(OtherTuple&& t) {
    Swallow(TupleLeaf<Indices, Ts>::operator=(
      Forward<TupleElement_T<Indices, MakeTupleTypes_T<OtherTuple>>>(Get<Indices>(t)))...);
    return *this;
  }
...
template <size_t I, typename Indices, typename... Ts>
FUN_ALWAYS_INLINE TupleElement_T<I, TupleImpl<Indices, Ts...>>& Get(TupleImpl<Indices, Ts...>& t) {
  typedef TupleElement_T<I, TupleImpl<Indices, Ts...>> Type;
  return static_cast<internal::TupleLeaf<I, Type>&>(t).GetInternal();
}

template <size_t I, typename Indices, typename... Ts>
FUN_ALWAYS_INLINE ConstTupleElement_T<I, TupleImpl<Indices, Ts...>>& Get(const TupleImpl<Indices, Ts...>& t) {
  typedef TupleElement_T<I, TupleImpl<Indices, Ts...>> Type;
...
template <typename TargetTupleType, typename... FromTypes>
using TupleImplicitlyConvertible_T = EnableIf_T<TupleImplicitlyConvertible<TargetTupleType, FromTypes...>::Value, bool>;
...
template <typename TargetTupleType, typename... FromTypes>
using TupleExplicitlyConvertible_T = EnableIf_T<TupleExplicitlyConvertible<TargetTupleType, FromTypes...>::Value, bool>;
...
struct TupleCat<Tuple1, Tuple2, TuplesRest...> {
  ...
  static FUN_ALWAYS_INLINE ResultType DoCat(TupleArg1&& t1, TupleArg2&& t2, TupleArgsRest&&... ts) {
...
class Tuple<T, Ts...> {
 public:
  ...
  constexpr Tuple(const T& t, const Ts&... ts)
    : impl_(MakeIndexSequence<sizeof...(Ts) + 1>{}, internal::MakeTupleTypes_T<Tuple>{}, t, ts...) {
  }
  ...
  template <typename U, typename... Us,
    ...
    : impl_(MakeIndexSequence<sizeof...(Us) + 1>{}, internal::MakeTupleTypes_T<Tuple>{}, Forward<U>(u),
      Forward<Us>(us)...) {
  }
  ...
  template <typename OtherTuple,
        typename EnableIf<internal::TupleConvertible<OtherTuple, Tuple>::Value, bool>::Type = false>
  Tuple(OtherTuple&& t)
    : impl_(Forward<OtherTuple>(t)) {
  }

  template <typename OtherTuple,
        typename EnableIf<internal::TupleAssignable<Tuple, OtherTuple>::Value, bool>::Type = false>
 ...
 private:
  typedef internal::TupleImpl<MakeIndexSequence<sizeof...(Ts) + 1>, T, Ts...> Impl;
  Impl impl_;
...
template <size_t I, typename... Ts>
FUN_ALWAYS_INLINE ConstTupleElement_T<I, Tuple<Ts...>>& Get(const Tuple<Ts...>& t) {
  return Get<I>(t.impl_);
}
...
template <typename... T1s, typename... T2s>
FUN_ALWAYS_INLINE bool operator == (const Tuple<T1s...>& t1, const Tuple<T2s...>& t2) {
...
template <typename... Ts>
FUN_ALWAYS_INLINE constexpr Tuple<internal::MakeTupleReturn_T<Ts>...> MakeTuple(Ts&&... values) {
...
// Specialize ignore_t IsAssignable type trait due to yet another VS2013 type traits bug
template <typename U>
...
template <typename... Tuples>
FUN_ALWAYS_INLINE typename internal::TupleCat<Tuples...>::ResultType
TupleCat(Tuples&&... ts) {
...
constexpr decltype(auto) Apply(F&& f, Tuple&& t) {
  return detail::ApplyImpl(Forward<F>(f), Forward<Tuple>(t),
...
FUN_ALWAYS_INLINE uint32 HashOf(const Tuple<Ts...>& tuple) {
  return internal::TupleHashOfHelper<1u, sizeof...(Ts)>::Do(HashOf(Get<0>(tuple)), tuple);
}
