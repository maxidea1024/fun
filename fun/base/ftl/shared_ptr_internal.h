...
/**
 * SPMode is used select between either 'fast' or 'thread safe' shared pointer
 * types. This is only used by templates at compile time to generate one code
 * path or another.
 */
enum class SPMode {
  /**
   ...
   * It's better to change it here, instead of replacing SPMode::Fast to
   * SPMode::ThreadSafe throughout the code.
   */
...
/**
 * Forward declarations.  Note that in the interest of fast performance, thread
 * safety features are mostly turned off (Mode = SPMode::Fast).  If you need to
 * access your object on multiple threads, you should use SPMode::ThreadSafe!
 */
template <typename ObjectType, SPMode Mode = SPMode::Fast>
class SharedRef;
template <typename ObjectType, SPMode Mode = SPMode::Fast>
class SharedPtr;
template <typename ObjectType, SPMode Mode = SPMode::Fast>
...
class ReferenceControllerBase {
 public:
  ...
  FUN_ALWAYS_INLINE explicit ReferenceControllerBase(void* object)
      : shared_reference_count_(1), weak_reference_count_(1), object_(object) {}
...
class ReferenceControllerWithDeleter : private DeleterType,
                                       public ReferenceControllerBase {
 public:
  explicit ReferenceControllerWithDeleter(void* object, DeleterType&& deleter)
      : DeleterType(MoveTemp(deleter)), ReferenceControllerBase(object) {}

  virtual void DestroyObject() {
    (*static_cast<DeleterType*>(this))(
        (ObjectType*)static_cast<ReferenceControllerBase*>(this)->object_);
  }
};
...
struct DefaultDeleter {
  FUN_ALWAYS_INLINE void operator()(Type* object) const { delete object; }
};
...
FUN_ALWAYS_INLINE ReferenceControllerBase* CreateDefaultReferenceController(
    ObjectType* object) {
  return new ReferenceControllerWithDeleter<ObjectType,
                                            DefaultDeleter<ObjectType>>(
 ...
 */
template <typename ObjectType, typename DeleterType>
FUN_ALWAYS_INLINE ReferenceControllerBase* CreateCustomReferenceController(
    ObjectType* object, DeleterType&& deleter) {
  return new ReferenceControllerWithDeleter<
      ObjectType, typename RemoveReference<DeleterType>::Type>(
...
struct RawPtrProxy {
  ...
  FUN_ALWAYS_INLINE RawPtrProxy(ObjectType* object, DeleterType&& deleter)
      : object(object),
        reference_controller(CreateCustomReferenceController(
            object, Forward<DeleterType>(deleter))) {}
};
...
struct ReferenceControllerOps<SPMode::ThreadSafe> {
   ...
   */
  static FUN_ALWAYS_INLINE const int32
  GetSharedReferenceCount(const ReferenceControllerBase* reference_controller) {
    // This reference count may be accessed by multiple threads
    return static_cast<int32 const volatile&>(
   ...
   */
  static FUN_ALWAYS_INLINE void AddSharedReference(
      ReferenceControllerBase* reference_controller) {
  ...
  static bool ConditionallyAddSharedReference(
      ReferenceControllerBase* reference_controller) {
    for (;;) {
      // Peek at the current shared reference count.  Remember, this value may
      // be updated by multiple threads.
      const int32 original_count = static_cast<int32 const volatile&>(
          reference_controller->shared_reference_count_);
      if (original_count == 0) {
      ...
      const int32 actual_original_count = Atomics::CompareExchange(
          &reference_controller->shared_reference_count_, original_count + 1,
          original_count);

      // We need to make sure that we never revive a counter that has already
      // expired, so if the actual value what we expected (because it was
      // touched by another thread), then we'll try again.  Note that only in
      // very unusual cases will this actually have to loop.
   ...
   */
  static FUN_ALWAYS_INLINE void ReleaseSharedReference(
      ReferenceControllerBase* reference_controller) {
    fun_check_dbg(reference_controller->shared_reference_count_ > 0);

    if (Atomics::Decrement(&reference_controller->shared_reference_count_) ==
        0) {
      // Last shared reference was released!  Destroy the referenced object.
      reference_controller->DestroyObject();

      // No more shared referencers, so decrement the weak reference count by
      // one.  When the weak reference count reaches zero, this object will be
      // deleted.
   ...
   */
  static FUN_ALWAYS_INLINE void AddWeakReference(
      ReferenceControllerBase* reference_controller) {
   ...
   */
  static void ReleaseWeakReference(
      ReferenceControllerBase* reference_controller) {
...
struct ReferenceControllerOps<SPMode::NotThreadSafe> {
  ...
  /**
   * Adds a shared reference to this counter ONLY if there is already at least
   * one reference
   *
   * \return True if the shared reference was added successfully
   */
...
/**
 * SharedReferencer is a wrapper around a pointer to a reference controller that
 * is used by either a SharedRef or a SharedPtr to keep track of a referenced
 * object's lifetime
 */
template <SPMode Mode>
class SharedReferencer {
 ...
 public:
   ...
   */
  FUN_ALWAYS_INLINE SharedReferencer() : reference_controller_(nullptr) {}

  /**
   * Constructor that counts a single reference to the specified object
   */
  FUN_ALWAYS_INLINE explicit SharedReferencer(
      ReferenceControllerBase* reference_controller)
  ...
  FUN_ALWAYS_INLINE SharedReferencer(SharedReferencer const& shared_ref)
    ...
    // If the incoming reference had an object associated with it, then go ahead
    // and increment the shared reference count
  ...
  /**
   * Creates a shared referencer object from a weak referencer object.  This
   * will only result in a valid object reference if the object already has at
   * least one other shared referencer.
   */
  SharedReferencer(WeakReferencer<Mode> const& weak_ref)
    ...
    // and increment the shared reference count
    if (reference_controller_) {
      // Attempt to elevate a weak reference to a shared one.  For this to work,
      // the object this weak counter is associated with must already have at
      // least one shared reference.  We'll never revive a pointer that has
      // already expired!
  ...
  FUN_ALWAYS_INLINE ~SharedReferencer() {
    if (reference_controller_) {
      // Tell the reference counter object that we're no longer referencing the
      // object with this shared pointer
      Ops::ReleaseSharedReference(reference_controller_);
    }
  }

  /**
   * Assignment operator adds a reference to the assigned object.  If this
   * counter was previously referencing an object, that reference will be
   * released.
   */
  FUN_ALWAYS_INLINE SharedReferencer& operator=(
      SharedReferencer const& shared_ref) {
  ...
  FUN_ALWAYS_INLINE const int32 GetSharedReferenceCount() const {
    return reference_controller_
               ? Ops::GetSharedReferenceCount(reference_controller_)
               : 0;
  }
...
class WeakReferencer {
 ...
 public:
   ...
   */
  FUN_ALWAYS_INLINE WeakReferencer() : reference_controller_(nullptr) {}
  ...
  FUN_ALWAYS_INLINE WeakReferencer(
      SharedReferencer<Mode> const& shared_referencer)
   ...
   */
  FUN_ALWAYS_INLINE WeakReferencer& operator=(
      WeakReferencer const& weak_referencer) {
   ...
   */
  FUN_ALWAYS_INLINE WeakReferencer& operator=(
      WeakReferencer&& weak_referencer) {
    auto old_reference_controller = reference_controller_;
    reference_controller_ = weak_referencer.reference_controller_;
    weak_referencer.reference_controller_ = nullptr;
    if (old_reference_controller) {
  ...
  FUN_ALWAYS_INLINE const bool IsValid() const {
    return reference_controller_ &&
           Ops::GetSharedReferenceCount(reference_controller_) > 0;
  }
 ...
 private:
   ...
   */
  FUN_ALWAYS_INLINE void AssignReferenceController(
      ReferenceControllerBase* new_reference_controller) {
 ...
 */
template <typename SharedPtrType, typename ObjectType, typename OtherType,
          SPMode Mode>
FUN_ALWAYS_INLINE void EnableSharedFromThis(
    SharedPtr<SharedPtrType, Mode> const* shared_ptr, ObjectType const* object,
    SharedFromThis<OtherType, Mode> const* shareable) {
  if (shareable) {
    shareable->UpdateWeakReferenceInternal(shared_ptr,
...
template <typename SharedPtrType, typename ObjectType, typename OtherType,
          SPMode Mode>
FUN_ALWAYS_INLINE void EnableSharedFromThis(
    SharedPtr<SharedPtrType, Mode>* shared_ptr, ObjectType const* object,
    SharedFromThis<OtherType, Mode> const* shareable) {
  if (shareable) {
    shareable->UpdateWeakReferenceInternal(shared_ptr,
                                           const_cast<ObjectType*>(object));
 ...
 * from an object instance
 */
template <typename SharedRefType, typename ObjectType, typename OtherType,
          SPMode Mode>
FUN_ALWAYS_INLINE void EnableSharedFromThis(
    SharedRef<SharedRefType, Mode> const* shared_ref, ObjectType const* object,
    SharedFromThis<OtherType, Mode> const* shareable) {
  if (shareable) {
    shareable->UpdateWeakReferenceInternal(shared_ref,
                                           const_cast<ObjectType*>(object));
  }
}
...
template <typename SharedRefType, typename ObjectType, typename OtherType,
          SPMode Mode>
FUN_ALWAYS_INLINE void EnableSharedFromThis(
    SharedRef<SharedRefType, Mode>* shared_ref, ObjectType const* object,
    SharedFromThis<OtherType, Mode> const* shareable) {
  if (shareable) {
    shareable->UpdateWeakReferenceInternal(shared_ref,
                                           const_cast<ObjectType*>(object));
