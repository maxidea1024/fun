 ...
 *
 * This class is non-copyable - ownership can only be transferred via a move
 * operation, e.g.:
 *
 * UniquePtr<MyClass> ptr1(new MyClass);    // The MyClass object is owned by
 * ptr1. UniquePtr<MyClass> ptr2(ptr1);           // Error - UniquePtr is not
 * copyable UniquePtr<MyClass> ptr3(MoveTemp(ptr1)); // ptr3 now owns the
 * MyClass object - ptr1 is now nullptr.
 */
...
class UniquePtr : private DeleterType {
 ...
 public:
  ...
  FUN_ALWAYS_INLINE UniquePtr(UniquePtr&& other)
      : DeleterType(MoveTemp(other.GetDeleter())), ptr_(other.ptr_) {
    other.ptr_ = nullptr;
  }
  ...
  template <typename OtherType, typename OtherDeleterType>
  FUN_ALWAYS_INLINE
      typename EnableIf<!IsCppArray<OtherType>::Value, UniquePtr&>::Type
  ...
  FUN_ALWAYS_INLINE ~UniquePtr() { GetDeleter()(ptr_); }
  ...
  FUN_ALWAYS_INLINE T* operator->() const { return ptr_; }

  FUN_ALWAYS_INLINE T& operator*() const { return *ptr_; }

  FUN_ALWAYS_INLINE T* Get() const { return ptr_; }
 ...
 public:
  ...
  template <typename OtherType, typename OtherDeleterType,
            typename = typename EnableIf<
                PointerIsConvertibleFromTo<OtherType[], T[]>::Value>::Type>
  FUN_ALWAYS_INLINE UniquePtr(UniquePtr<OtherType, OtherDeleterType>&& other)
      : DeleterType(MoveTemp(other.GetDeleter())), ptr_(other.ptr_) {
    other.ptr_ = nullptr;
  }
  ...
  template <typename OtherType, typename OtherDeleterType>
  FUN_ALWAYS_INLINE typename EnableIf<
      PointerIsConvertibleFromTo<OtherType[], T[]>::Value>::Type
  operator=(UniquePtr<OtherType, OtherDeleterType>&& other) {
    if (FUN_LIKELY(&other != this)) {
      T* old_ptr = ptr_;
  ...
  FUN_ALWAYS_INLINE T& operator[](size_t index) const { return ptr_[index]; }
 ...
 *
 * \return true if the two UniquePtrs are logically substitutable for each
 * other, false otherwise.
 */
template <typename LhsType, typename RhsType>
FUN_ALWAYS_INLINE bool operator==(const UniquePtr<LhsType>& lhs,
                                  const UniquePtr<RhsType>& rhs) {
  return lhs.Get() == rhs.Get();
}

template <typename T>
FUN_ALWAYS_INLINE bool operator==(const UniquePtr<T>& lhs,
                                  const UniquePtr<T>& rhs) {
  return lhs.Get() == rhs.Get();
}
 ...
 * \param rhs - The second UniquePtr to compare.
 *
 * \return false if the two UniquePtrs are logically substitutable for each
...
/**
 * Constructs a new object with the given arguments and returns it as a
 * UniquePtr.
 *
 * \param args - The arguments to pass to the constructor of T.
 *
 * \return A UniquePtr which points to a newly-constructed T with the specified
 * args.
 */
...
template <typename T, typename... Args>
typename EnableIf<IsBoundedCppArray<T>::Value, UniquePtr<T>>::Type MakeUnique(
    Args&&... args) = delete;
