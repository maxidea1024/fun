...
/**
 * Casts a shared reference of one type to another type. (static_cast)  Useful
 * for down-casting.
 *
 * \param shared_ref - The shared reference to cast
 */
template <typename CastToType, typename CastFromType, SPMode Mode>
FUN_ALWAYS_INLINE SharedRef<CastToType, Mode> StaticCastSharedRef(
    SharedRef<CastFromType, Mode> const& shared_ref) {
  return SharedRef<CastToType, Mode>(shared_ref,
                                     SharedPtr_internal::StaticCastTag());
}

class FObjectBase;

/**
 * SharedRef is a non-nullable, non-intrusive reference-counted authoritative
 * object reference.
 *
 * This shared reference will be conditionally thread-safe when the optional
 * Mode template argument is set to ThreadSafe. OTE: SharedRef is an FUN
 * extension to standard smart pointer feature set
 */
template <typename ObjectType, SPMode Mode>
class SharedRef {
  // TSharedRefs with FObjects are illegal.
  static_assert(
      !PointerIsConvertibleFromTo<ObjectType, const FObjectBase>::Value,
      "You cannot use SharedRef with FObjects.");

 public:
  // NOTE: SharedRef has no default constructor as it does not support empty
  // references.  You must
  //       initialize your SharedRef to a valid object at construction time.

  /**
   * Constructs a shared reference that owns the specified object.  Must not be
   * nullptr.
   *
  ...
  FUN_ALWAYS_INLINE explicit SharedRef(OtherType* object)
      : object_(object),
        shared_referencer_(
            SharedPtr_internal::CreateDefaultReferenceController(object)) {
    Init(object);
  }
   ...
   * \param deleter - Deleter object used to destroy the object when it is no
   * longer referenced.
   */
  template <typename OtherType, typename DeleterType>
  FUN_ALWAYS_INLINE SharedRef(OtherType* object, DeleterType&& deleter)
      : object_(object),
        shared_referencer_(SharedPtr_internal::CreateCustomReferenceController(
  ...
  /**
   * Constructs default shared reference that owns the default object for
   * specified type.
   *
  ...
  SharedRef()
      : object_(new ObjectType()),
        shared_referencer_(
            SharedPtr_internal::CreateDefaultReferenceController(object_)) {
   ...
   * (See MakeShareable()) Must not be nullptr.
   *
   * \param raw_ptr_proxy - Proxy raw pointer that contains the object that the
   * new shared reference will reference
   *
   * NOTE: The following is an FUN extension to standard shared_ptr behavior
   */
  template <typename OtherType>
  FUN_ALWAYS_INLINE SharedRef(
      SharedPtr_internal::RawPtrProxy<OtherType> const& raw_ptr_proxy)
      : object_(raw_ptr_proxy.object),
        shared_referencer_(raw_ptr_proxy.reference_controller) {
    // If the following assert goes off, it means a SharedRef was initialized
    // from a nullptr object pointer. Shared references must never be nullptr,
    // so either pass a valid object or consider using SharedPtr instead.
    fun_check_ptr(raw_ptr_proxy.object_);

    // If the object happens to be derived from SharedFromThis, the following
    // method will prime the object with a weak pointer to itself.
    SharedPtr_internal::EnableSharedFromThis(this, raw_ptr_proxy.object,
                                             raw_ptr_proxy.object);
  }

  /**
   * Constructs a shared reference as a reference to an existing shared
   * reference's object. This constructor is needed so that we can implicitly
   * upcast to base classes.
   *
   * \param shared_ref - The shared reference whose object we should create an
  ...
  /**
   * Special constructor used internally to statically cast one shared reference
   * type to another.  You should never call this constructor directly. Instead,
   * use the StaticCastSharedRef() function. This constructor creates a shared
   * reference as a shared reference to an existing shared reference after
   * statically casting that reference's object.  This constructor is needed for
   * static casts.
   *
   * \param shared_ref - The shared reference whose object we should create an
   * additional reference to
  ...
  /**
   * Special constructor used internally to cast a 'const' shared reference a
   * 'mutable' reference.  You should never call this constructor directly.
   * Instead, use the ConstCastSharedRef() function. This constructor creates a
   * shared reference as a shared reference to an existing shared reference
   * after const casting that reference's object.  This constructor is needed
   * for const casts.
   *
   * \param shared_ref - The shared reference whose object we should create an
   * additional reference to
  ...
  template <typename OtherType>
  FUN_ALWAYS_INLINE SharedRef(SharedRef<OtherType, Mode> const& shared_ref,
                              SharedPtr_internal::ConstCastTag)
      : object_(const_cast<ObjectType*>(shared_ref.object_)),
        shared_referencer_(shared_ref.shared_referencer_) {}

  /**
   * Special constructor used internally to create a shared reference from an
   * existing shared reference, while using the specified object reference
   * instead of the incoming shared reference's object pointer.  This is used by
   * with the SharedFromThis feature (by UpdateWeakReferenceInternal)
   *
   * \param other_shared_ref - The shared reference whose reference count
   * \param object - The object pointer to use (instead of the incoming shared
   * reference's object)
   */
  template <typename OtherType>
  FUN_ALWAYS_INLINE SharedRef(
      SharedRef<OtherType, Mode> const& other_shared_ref, ObjectType* object)
      : object_(object),
  ...
  FUN_ALWAYS_INLINE SharedRef(SharedRef&& shared_ref)
      : object_(shared_ref.object_),
        shared_referencer_(shared_ref.shared_referencer_) {
    // We're intentionally not moving here, because we don't want to leave
    // shared_ref in a null state, because that breaks the class invariant.  But
    // we provide a move constructor anyway in case the compiler complains that
    // we have a move assign but no move construct.
  }

  /**
   * Assignment operator replaces this shared reference with the specified
   * shared reference.  The object currently referenced by this shared reference
   * will no longer be referenced and will be deleted if there are no other
   * referencers.
   *
   ...
   * shared reference.  The object currently referenced by this shared reference
   * will no longer be referenced and will be deleted if there are no other
   * referencers.  Must not be nullptr.
   *
   * \param raw_ptr_proxy - Proxy object used to assign the object (see
   * MakeShareable helper function)
   *
   * NOTE: The following is an FUN extension to standard shared_ptr behavior
   */
  template <typename OtherType>
  FUN_ALWAYS_INLINE SharedRef& operator=(
      SharedPtr_internal::RawPtrProxy<OtherType> const& raw_ptr_proxy) {
    // If the following assert goes off, it means a SharedRef was initialized
    // from a nullptr object pointer. Shared references must never be nullptr,
    // so either pass a valid object or consider using SharedPtr instead.
  ...
  /**
   * Dereference operator returns a reference to the object this shared pointer
   * points to
   *
   ...
   *
   * IMPORTANT: Not necessarily fast!  Should only be used for debugging
   * purposes!
   *
   * \return Number of shared references to the object (including this
   * reference.)
   */
   ...
   * purposes!
   *
   * \return True if there is only one shared reference to the object, and this
   * is it!
   */
 ...
 private:
  ...
  /**
   * Converts a shared pointer to a shared reference.  The pointer *must* be
   * valid or an assertion will trigger. NOTE: This explicit conversion
   * constructor is intentionally private.  Use 'ToSharedRef()' instead.
   *
   * \return Reference to the object
   */
  template <typename OtherType>
  FUN_ALWAYS_INLINE explicit SharedRef(
      SharedPtr<OtherType, Mode> const& shared_ptr)
      : object_(shared_ptr.object_),
        shared_referencer_(shared_ptr.shared_referencer_) {
    // If this assert goes off, it means a shared reference was created from a
    // shared pointer that was nullptr. Shared references are never allowed to
    // be null.  Consider using SharedPtr instead.
    fun_check(IsValid());
  }
   ...
   * NOTE: This validity test is intentionally private because shared references
   * must always be valid.
   *
   * \return True if the shared reference is valid and can be dereferenced
   */
  FUN_ALWAYS_INLINE const bool IsValid() const { return !!object_; }
  ...
  // friend HashResult& operator << (HashResult& result, const
  // SharedRef<ObjectType, Mode>& shared_ref)
  //{
  //  return result << fun::PointerHash(shared_ref.object_);
  //}

  // We declare ourselves as a friend (templated using OtherType) so we can
  // access members as needed
  template <typename OtherType, SPMode OtherMode>
 ...
 private:
  ...
  /**
   * Interface to the reference counter for this object.  Note that the actual
   * reference controller object is shared by all shared and weak pointers that
   * refer to the object
   */
 ...
 * This shared pointer will be conditionally thread-safe when the optional Mode
 * template argument is set to ThreadSafe.
 */
template <typename ObjectType, SPMode Mode>
class SharedPtr {
  // TSharedPtrs with FObjects are illegal.
  // static_assert(!PointerIsConvertibleFromTo<ObjectType, const
  // FObjectBase>::Value, "You cannot use SharedPtr or WeakPtr with FObjects.
  // Consider a UProperty() pointer or TWeakObjectPtr.");

  enum {
    ObjectTypeHasSameModeSharedFromThis =
        PointerIsConvertibleFromTo<ObjectType,
                                   SharedFromThis<ObjectType, Mode>>::Value,
    ObjectTypeHasOppositeModeSharedFromThis = PointerIsConvertibleFromTo<
        ObjectType,
        SharedFromThis<ObjectType, (Mode == SPMode::NotThreadSafe)
                                       ? SPMode::ThreadSafe
                                       : SPMode::NotThreadSafe>>::Value
  };

  // SharedPtr of one mode to a type which has a SharedFromThis only of another
  // mode is illegal. A type which does not inherit SharedFromThis at all is ok.
  static_assert(SharedPtr::ObjectTypeHasSameModeSharedFromThis ||
                    !SharedPtr::ObjectTypeHasOppositeModeSharedFromThis,
                "You cannot use a SharedPtr of one mode with a type which "
                "inherits SharedFromThis of another mode.");
 ...
 public:
  ...
  /**
   * Constructs a shared pointer that owns the specified object.  Note that
   * passing nullptr here will still create a tracked reference to a nullptr
   * pointer. (Consistent with std::shared_ptr)
   *
   * \param object - object this shared pointer to retain a reference to
   */
  template <typename OtherType>
  ...
  /**
   * Constructs a shared pointer using a proxy reference to a raw pointer. (See
   * MakeShareable())
   *
   * \param raw_ptr_proxy - Proxy raw pointer that contains the object that the
   * new shared pointer will reference
   */
  // NOTE: The following is an FUN extension to standard shared_ptr behavior
  template <typename OtherType>
  FUN_ALWAYS_INLINE SharedPtr(
      SharedPtr_internal::RawPtrProxy<OtherType> const& raw_ptr_proxy)
      : object_(raw_ptr_proxy.object),
        shared_referencer_(raw_ptr_proxy.reference_controller) {
    // If the object happens to be derived from SharedFromThis, the following
  ...
  /**
   * Constructs a shared pointer as a shared reference to an existing shared
   * pointer's object. This constructor is needed so that we can implicitly
   * upcast to base classes.
   *
   * \param shared_ptr - The shared pointer whose object we should create an
   * additional reference to
   */
  template <typename OtherType>
  ...
  /**
   * Implicitly converts a shared reference to a shared pointer, adding a
   * reference to the object. NOTE: We allow an implicit conversion from
   * SharedRef to SharedPtr because it's always a safe conversion.
   *
   * \param shared_ref - The shared reference that will be converted to a shared
   * pointer
   *
   * NOTE: The following is an FUN extension to standard shared_ptr behavior
   */
  ...
  FUN_ALWAYS_INLINE SharedPtr(SharedRef<OtherType, Mode> const& shared_ref)
      : object_(shared_ref.object_),
        shared_referencer_(shared_ref.shared_referencer_) {
    // There is no rvalue overload of this constructor, because 'stealing' the
    // pointer from a SharedRef would leave it as null, which would invalidate
    // its invariant.
  }

  /**
   * Special constructor used internally to statically cast one shared pointer
   * type to another.  You should never call this constructor directly. Instead,
   * use the StaticCastSharedPtr() function. This constructor creates a shared
   * pointer as a shared reference to an existing shared pointer after
   * statically casting that pointer's object.  This constructor is needed for
   * static casts.
   *
   * \param shared_ptr - The shared pointer whose object we should create an
   * additional reference to
  ...
  /**
   * Special constructor used internally to cast a 'const' shared pointer a
   * 'mutable' pointer.  You should never call this constructor directly.
   * Instead, use the ConstCastSharedPtr() function. This constructor creates a
   * shared pointer as a shared reference to an existing shared pointer after
   * const casting that pointer's object.  This constructor is needed for const
   * casts.
   *
   * \param shared_ptr - The shared pointer whose object we should create an
   * additional reference to
  ...
  /**
   * Special constructor used internally to create a shared pointer from an
   * existing shared pointer, while using the specified object pointer instead
   * of the incoming shared pointer's object pointer.  This is used by with the
   * SharedFromThis feature (by UpdateWeakReferenceInternal)
   *
  ...
  template <typename OtherType>
  FUN_ALWAYS_INLINE SharedPtr(
      SharedPtr<OtherType, Mode> const& other_shared_ptr, ObjectType* object)
      : object_(object),
        shared_referencer_(other_shared_ptr.shared_referencer_) {}

  /**
   * Assignment to a nullptr pointer.  The object currently referenced by this
   * shared pointer will no longer be referenced and will be deleted if there
   * are no other referencers. NOTE: The following is an FUN extension to
   * standard shared_ptr behavior
   */
  ...
  /**
   * Assignment operator replaces this shared pointer with the specified shared
   * pointer.  The object currently referenced by this shared pointer will no
   * longer be referenced and will be deleted if there are no other referencers.
   *
   ...
   */
  template <typename OtherType>
  FUN_ALWAYS_INLINE SharedPtr& operator=(
      SharedPtr_internal::RawPtrProxy<OtherType> const& raw_ptr_proxy) {
  ...
  /**
   * Converts a shared pointer to a shared reference.  The pointer *must* be
   * valid or an assertion will trigger.
   *
   * \return Reference to the object
   *
   * NOTE: The following is an FUN extension to standard shared_ptr behavior
  ...
  /**
   * Returns the object referenced by this pointer, or nullptr if no object is
   * reference
   *
   * \return The object owned by this shared pointer, or nullptr
   */
  FUN_ALWAYS_INLINE ObjectType* Get() const { return object_; }
   ...
   * \return Reference to the object
   */
  FUN_ALWAYS_INLINE typename MakeReferenceTo<ObjectType>::Type operator*()
      const {
    fun_check(IsValid());
    return *object_;
  }
  ...
  /**
   * Arrow operator returns a pointer to the object this shared pointer
   * references
   *
   ...
   */
  FUN_ALWAYS_INLINE void Reset() { *this = SharedPtr<ObjectType, Mode>(); }
  ...
  /**
   * Returns true if this is the only shared reference to this object.  Note
   * that there may be outstanding weak references left.
   *
   * IMPORTANT: Not necessarily fast!  Should only be used for debugging
   * purposes!
   *
 ...
 private:
  /**
   * Constructs a shared pointer from a weak pointer, allowing you to access the
   * object (if it hasn't expired yet.)  Remember, if there are no more shared
   * references to the object, the shared pointer will not be valid.  You should
   * always check to make sure this shared pointer is valid before trying to
   * dereference the shared pointer!
   *
   * NOTE: This constructor is private to force users to be explicit when
   * converting a weak pointer to a shared pointer.  Use the weak pointer's
   * Lock() method instead!
   */
  template <typename OtherType>
  FUN_ALWAYS_INLINE explicit SharedPtr(WeakPtr<OtherType, Mode> const& weak_ptr)
      : object_(nullptr), shared_referencer_(weak_ptr.weak_reference_count_) {
    // Check that the shared reference was created from the weak reference
    // successfully. We'll only cache a pointer to the object if we have a valid
    // shared reference.
...
template <typename ObjectType, SPMode Mode>
struct IsZeroConstructible<SharedPtr<ObjectType, Mode>> {
  enum { Value = true };
};
...
class WeakPtr {
 public:
  ...
  FUN_ALWAYS_INLINE WeakPtr(SharedPtr_internal::NullTag* = nullptr)
      : object_(nullptr), weak_reference_count_() {}
   ...
   */
  template <typename OtherType>
  FUN_ALWAYS_INLINE WeakPtr& operator=(
      WeakPtr<OtherType, Mode> const& weak_ptr) {
    object_ = weak_ptr.Lock().Get();
    weak_reference_count_ = weak_ptr.weak_reference_count_;
    return *this;
  }
   ...
   *
   * \param shared_ref - The shared reference used to assign to this weak
   * pointer
   *
   * NOTE: The following is an FUN extension to standard shared_ptr behavior
   */
  template <typename OtherType>
  FUN_ALWAYS_INLINE WeakPtr& operator=(
      SharedRef<OtherType, Mode> const& shared_ref) {
    object_ = shared_ref.object_;
    weak_reference_count_ = shared_ref.shared_referencer_;
    return *this;
  }
   ...
   * dereference the shared pointer!
   *
   * \return Shared pointer for this object (will only be valid if still
   * referenced!)
   */
   ...
   *
   * \return True if the weak pointer is valid and a pin operator would have
   * succeeded
   */
   ...
   */
  FUN_ALWAYS_INLINE void Reset() { *this = WeakPtr<ObjectType, Mode>(); }
 ...
 private:
  ...
  // Declare ourselves as a friend of SharedPtr so we can access members as
  // needed
  template <typename OtherType, SPMode OtherMode>
  friend class SharedPtr;
...
template <typename T, SPMode Mode>
struct IsWeakPointerType<WeakPtr<T, Mode>> {
  enum { Value = true };
};
template <typename T, SPMode Mode>
struct IsZeroConstructible<WeakPtr<T, Mode>> {
  enum { Value = true };
};
...
class SharedFromThis {
 public:
  ...
  SharedRef<ObjectType, Mode> AsShared() {
    ...
    //
    //     - You tried to request a shared pointer before the object was ever
    //     assigned to one. (e.g. constructor)
    //     - You tried to request a shared pointer while the object is being
    //     destroyed (destructor chain)
    //
    // To fix this, make sure you create at least one shared reference to your
    // object instance before requested, and also avoid calling this function
    // from your object's destructor.
    //
    fun_check(shared_this.Get() == this);

    // Now that we've verified the shared pointer is valid, we'll convert it to
    // a shared reference and return it!
 ...
 protected:
   ...
   */
  template <typename OtherType>
  FUN_ALWAYS_INLINE static SharedRef<OtherType, Mode> SharedThis(
      OtherType* this_ptr) {
   ...
   */
  template <typename OtherType>
  FUN_ALWAYS_INLINE static SharedRef<OtherType const, Mode> SharedThis(
      const OtherType* this_ptr) {
    return StaticCastSharedRef<OtherType const>(this_ptr->AsShared());
  }

 public:  // @todo: Ideally this would be private, but template sharing problems
          // prevent it
  /**
  ...
  FUN_ALWAYS_INLINE void UpdateWeakReferenceInternal(
      SharedPtr<SharedPtrType, Mode> const* shared_ptr,
      OtherType* object) const {
  ...
  FUN_ALWAYS_INLINE void UpdateWeakReferenceInternal(
      SharedRef<SharedRefType, Mode> const* shared_ref,
      OtherType* object) const {
    if (!weak_this_.IsValid()) {
 ...
 protected:
   ...
   */
  FUN_ALWAYS_INLINE SharedFromThis& operator=(SharedFromThis const&) {
    return *this;
  }
...
FUN_ALWAYS_INLINE bool operator==(
    SharedRef<ObjectTypeA, Mode> const& shared_ref_a,
    SharedRef<ObjectTypeB, Mode> const& shared_ref_b) {
...
template <typename ObjectTypeA, typename ObjectTypeB, SPMode Mode>
FUN_ALWAYS_INLINE bool operator==(
    SharedPtr<ObjectTypeA, Mode> const& shared_ptr_a,
    SharedPtr<ObjectTypeB, Mode> const& shared_ptr_b) {
...
template <typename ObjectTypeA, typename ObjectTypeB, SPMode Mode>
FUN_ALWAYS_INLINE bool operator==(
    SharedRef<ObjectTypeA, Mode> const& shared_ref,
    SharedPtr<ObjectTypeB, Mode> const& shared_ptr) {
...
template <typename ObjectTypeA, typename ObjectTypeB, SPMode Mode>
FUN_ALWAYS_INLINE bool operator==(
    WeakPtr<ObjectTypeA, Mode> const& weak_ptr_a,
    WeakPtr<ObjectTypeB, Mode> const& weak_ptr_b) {
...
FUN_ALWAYS_INLINE bool operator==(WeakPtr<ObjectTypeA, Mode> const& weak_ptr_a,
                                  decltype(nullptr)) {
...
FUN_ALWAYS_INLINE bool operator==(
    decltype(nullptr), WeakPtr<ObjectTypeB, Mode> const& weak_ptr_b) {
 ...
 */
template <typename CastToType, typename CastFromType, SPMode Mode>
FUN_ALWAYS_INLINE SharedPtr<CastToType, Mode> StaticCastSharedPtr(
    SharedPtr<CastFromType, Mode> const& shared_ptr) {
  return SharedPtr<CastToType, Mode>(shared_ptr,
                                     SharedPtr_internal::StaticCastTag());
}
 ...
 */
template <typename CastToType, typename CastFromType, SPMode Mode>
FUN_ALWAYS_INLINE SharedRef<CastToType, Mode> ConstCastSharedRef(
    SharedRef<CastFromType, Mode> const& shared_ref) {
  return SharedRef<CastToType, Mode>(shared_ref,
                                     SharedPtr_internal::ConstCastTag());
}
 ...
 */
template <typename CastToType, typename CastFromType, SPMode Mode>
FUN_ALWAYS_INLINE SharedPtr<CastToType, Mode> ConstCastSharedPtr(
    SharedPtr<CastFromType, Mode> const& shared_ptr) {
  return SharedPtr<CastToType, Mode>(shared_ptr,
                                     SharedPtr_internal::ConstCastTag());
}
...
FUN_ALWAYS_INLINE SharedPtr_internal::RawPtrProxy<ObjectType> MakeShareable(
    ObjectType* object) {
...
FUN_ALWAYS_INLINE SharedPtr_internal::RawPtrProxy<ObjectType> MakeShareable(
    ObjectType* object, DeleterType&& deleter) {
  return SharedPtr_internal::RawPtrProxy<ObjectType>(
      object, Forward<DeleterType>(deleter));
}
