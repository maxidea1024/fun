...
struct DecayedFrontOfParameterPackIsSameType {
  enum {
    Value = IsSame<T, typename Decay<typename NthTypeFromParameterPack<
                          0, Ts...>::Type>::Type>::Value
  };
};
...
struct TupleElement {
  template <
      typename... Args,
      typename = typename EnableIf<AndValue<
          sizeof...(Args) != 0,
          OrValue<sizeof...(Args) != 1,
                  Not<Tuple_internal::DecayedFrontOfParameterPackIsSameType<
                      TupleElement, Args...>>>>::Value>::Type>
  explicit TupleElement(Args&&... args) : value(Forward<Args>(args)...) {}
...
struct TupleElementHelperImpl<IterIndex, Index, ElementType, Ts...>
    : TupleElementHelperImpl<IterIndex + 1, Index, Ts...> {};
...
struct EqualityHelper {
  template <typename TupleType>
  FUN_ALWAYS_INLINE static bool Compare(const TupleType& lhs,
                                        const TupleType& rhs) {
    return lhs.template Get<ArgToCompare>() ==
               rhs.template Get<ArgToCompare>() &&
           EqualityHelper<ArgCount, ArgToCompare + 1>::Compare(lhs, rhs);
  }
};
...
template <size_t ArgCount, size_t ArgToCompare = 0,
          bool Last = ArgToCompare + 1 == ArgCount>
struct LessThanHelper {
  ...
  FUN_ALWAYS_INLINE static bool Do(const TupleType& lhs, const TupleType& rhs) {
    return lhs.template Get<ArgToCompare>() <
               rhs.template Get<ArgToCompare>() ||
           (!(rhs.template Get<ArgToCompare>() <
              lhs.template Get<ArgToCompare>()) &&
            LessThanHelper<ArgCount, ArgToCompare + 1>::Do(lhs, rhs));
  }
};
...
struct TupleStorage<IntegerSequence<size_t, Indices...>, Ts...>
    : TupleElement<Ts, Indices>... {
  template <
      typename... Args,
      typename = typename EnableIf<AndValue<
  ...
  FUN_ALWAYS_INLINE const typename TupleElementHelper<Index, Ts...>::Type& Get()
      const {
    return TupleElementHelper<Index, Ts...>::Get(*this);
  }
  template <size_t Index>
  FUN_ALWAYS_INLINE typename TupleElementHelper<Index, Ts...>::Type& Get() {
    return TupleElementHelper<Index, Ts...>::Get(*this);
  }
};
...
struct TupleStorage<IntegerSequence<size_t, 0, 1>, _KeyType, _ValueType> {
 ...
 private:
  ...
  struct GetHelper<1, Dummy> {
    ...
    static const ValueType& Get(const TupleStorage& tuple) {
      return tuple.value;
    }
 ...
 public:
  ...
  template <size_t Index>
  FUN_ALWAYS_INLINE const typename GetHelper<Index, void>::ResultType& Get()
      const {
    return GetHelper<Index, void>::Get(*this);
  }
  template <size_t Index>
  FUN_ALWAYS_INLINE typename GetHelper<Index, void>::ResultType& Get() {
    return GetHelper<Index, void>::Get(*this);
  }
...
struct TupleImpl<IntegerSequence<size_t, Indices...>, Ts...>
    : TupleStorage<IntegerSequence<size_t, Indices...>, Ts...> {
 private:
 ...
 public:
  ...
  template <
      ...
      typename = typename EnableIf<AndValue<
          ...
          OrValue<sizeof...(Args) != 1,
                  Not<Tuple_internal::DecayedFrontOfParameterPackIsSameType<
                      TupleImpl, Args...>>>>::Value>::Type>
  explicit TupleImpl(Args&&... args) : Super(Forward<Args>(args)...) {}
  ...
  // TODO ApplyBefore / ApplyAfter ���� ���̰� �ִ°ǰ�??
  ...
  FUN_ALWAYS_INLINE friend Archive& operator&(Archive& ar, TupleImpl& tuple) {
    // This should be implemented with a fold expression when our compilers
    // support it
  ...
  // TODO
  // FUN_ALWAYS_INLINE friend void operator & (StructuredArchive::Slot slot,
  // TupleImpl& tuple) {
  //  // This should be implemented with a fold expression when our compilers
  //  support it StructuredArchive::Stream stream = slot.EnterStream(); int
  //  tmp[] = { 0, (stream.EnterElement() & tuple.template Get<Indices>(), 0)...
  //  }; (void)tmp;
  //}

  FUN_ALWAYS_INLINE friend bool operator==(const TupleImpl& lhs,
                                           const TupleImpl& rhs) {
    // This could be implemented with a fold expression when our compilers
    // support it
...
//#ifdef _MSC_VER
//
//  // Not strictly necessary, but some VC versions give a 'syntax error:
//  <fake-expression>' error
//  // for empty tuples.
//  template <>
...
//    }
//
//    FUN_ALWAYS_INLINE friend Archive& operator & (Archive& ar, TupleImpl&
//    tuple) {
//      return ar;
//    }
//
//    FUN_ALWAYS_INLINE friend bool operator == (const TupleImpl& lhs, const
//    TupleImpl& rhs) {
//      return true;
//    }
//
...
template <typename... Ts>
FUN_ALWAYS_INLINE Tuple<typename Decay<Ts>::Type...> MakeTupleImpl(
    Ts&&... args) {
...
struct TransformTuple_Impl<IntegerSequence<size_t, Indices...>> {
  ...
  static decltype(auto) Do(TupleType&& tuple, FuncType func) {
    return MakeTupleImpl(
        func(Forward<TupleType>(tuple).template Get<Indices>())...);
  }
};
...
struct VisitTupleElements_Impl<IntegerSequence<size_t, Indices...>> {
  ...
  FUN_ALWAYS_INLINE static void InvokeFunc(FuncType&& func,
                                           TupleTypes&&... tuples) {
    Invoke(Forward<FuncType>(func),
           Forward<TupleTypes>(tuples).template Get<Index>()...);
  }
  ...
  static void Do(FuncType&& func, TupleTypes&&... tuples) {
    // This should be implemented with a fold expression when our compilers
    // support it
    int tmp[] = {0, (InvokeFunc<Indices>(Forward<FuncType>(func),
                                         Forward<TupleTypes>(tuples)...),
                     0)...};
    (void)tmp;
  }
};
...
template <typename... Ts>
struct Tuple
    : Tuple_internal::TupleImpl<MakeIntegerSequence<size_t, sizeof...(Ts)>,
                                Ts...> {
 private:
  using Super =
      Tuple_internal::TupleImpl<MakeIntegerSequence<size_t, sizeof...(Ts)>,
 ...
 public:
  template <
      ...
      typename = typename EnableIf<AndValue<
          ...
          OrValue<sizeof...(Args) != 1,
                  Not<Tuple_internal::DecayedFrontOfParameterPackIsSameType<
                      Tuple, Args...>>>>::Value>::Type>
  explicit Tuple(Args&&... args) : Super(Forward<Args>(args)...) {
 ...
 *   // Equivalent to:
 *   // Tuple<float, char, const char*>
 * transformed_tuple(Overloaded(my_tuple.Get<0>()),
 * Overloaded(my_tuple.Get<1>()), Overloaded(my_tuple.Get<2>()))); auto
 * transformed_tuple = TransformTuple(my_tuple, [](const auto& arg) { return
 * Overloaded(arg); });
 * }
 */
template <typename FuncType, typename... Ts>
FUN_ALWAYS_INLINE decltype(auto) TransformTuple(Tuple<Ts...>&& tuple,
                                                FuncType func) {
  return Tuple_internal::TransformTuple_Impl<
      MakeIntegerSequence<size_t, sizeof...(Ts)>>::Do(MoveTemp(tuple),
                                                      MoveTemp(func));
}

template <typename FuncType, typename... Ts>
FUN_ALWAYS_INLINE decltype(auto) TransformTuple(const Tuple<Ts...>& tuple,
                                                FuncType func) {
  return Tuple_internal::TransformTuple_Impl<
      MakeIntegerSequence<size_t, sizeof...(Ts)>>::Do(tuple, MoveTemp(func));
}

/**
 * Visits each element in the specified tuples in parallel and applies them as
 * arguments to the functor. All specified tuples must have the same number of
 * elements.
 *
 * @param func    The functor to apply.
 ...
 * Example:
 *
 * void func(const Tuple<int32, const char*, String>& tuple1, const Tuple<bool,
 * float, String>& tuple2) {
 *   // Equivalent to:
...
FUN_ALWAYS_INLINE void VisitTupleElements(FuncType&& func,
                                          FirstTupleType&& first_tuple,
                                          TupleTypes&&... tuples) {
  Tuple_internal::VisitTupleElements_Impl<MakeIntegerSequence<
      size_t, TupleArity<typename Decay<FirstTupleType>::Type>::Value>>::
      Do(Forward<FuncType>(func), Forward<FirstTupleType>(first_tuple),
         Forward<TupleTypes>(tuples)...);
}
...
struct TupleHashOfHelper {
  ...
  FUN_ALWAYS_INLINE static uint32 Do(uint32 hash, const TupleType& tuple) {
    return TupleHashOfHelper<ArgToCombine + 1, ArgCount>::Do(
        HashCombine(hash, HashOf(tuple.template Get<ArgToCombine>())), tuple);
  }
};
...
FUN_ALWAYS_INLINE uint32 HashOf(const Tuple<Ts...>& tuple) {
  return TupleHashOf_internal::TupleHashOfHelper<1u, sizeof...(Ts)>::Do(
      HashOf(tuple.template Get<0>()), tuple);
}

FUN_ALWAYS_INLINE uint32 HashOf(const Tuple<>& tuple) { return 0; }
