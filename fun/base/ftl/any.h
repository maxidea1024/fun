...
class Any {
  ...
  template <typename ValueType> friend const ValueType* AnyCast(const Any* any) noexcept;
  template <typename ValueType> friend ValueType* AnyCast(Any* any) noexcept;
  template <typename ValueType> friend ValueType AnyCast(const Any& any) noexcept;
  template <typename ValueType> friend ValueType AnyCast(Any& any) noexcept;
  template <typename ValueType> friend ValueType AnyCast(Any&& any) noexcept;
  ...
  template <typename ValueType> friend const ValueType* UnsafeAnyCast(const Any* any) noexcept;
  template <typename ValueType> friend ValueType* UnsafeAnyCast(Any* any) noexcept;
  ...
  struct StorageHandlerInternal {
    ...
    static void* HandlerFunc(StorageOperation op, const Any* this_ptr, Any* other_ptr) {
      switch (op) {
  ...
  struct StorageHandlerExternal {
    ...
    template <typename... Args>
    static void ConstructInPlace(Storage& s, Args... args) {
      s.external_storage = internal::DefaultConstruct<T>(Forward<Args>(args)...);
    }

    template <typename NT, typename U, typename... Args>
    static void ConstructInPlace(Storage& s, std::initializer_list<U> il, Args&&... args) {
      s.external_storage = internal::DefaultConstruct<NT>(il, Forward<Args>(args)...);
    }

    static inline void Destroy(Any& any_ref) {
      internal::DefaultDestroy(static_cast<T*>(refAny.storage_.external_storage));
    ...
    static void* HandlerFunc(StorageOperation op, const Any* this_ptr, Any* other_ptr) {
      switch (op) {
        ...
        case StorageOperation::Copy:
          func_check_ptr(this_ptr);
          func_check_ptr(other_ptr);
          Construct(other_ptr->storage_, *static_cast<T*>(this_ptr->storage_.external_storage));
          break;

        case StorageOperation::Move:
          func_check_ptr(this_ptr);
          func_check_ptr(other_ptr);
          Construct(other_ptr->storage_, MoveTemp(*(T*)(this_ptr->storage_.external_storage)));
          Destroy(const_cast<Any&>(*this_ptr));
          break;
 ...
 public:
  ...
  Any(ValueType&& value,
      typename EnableIf<!IsSame<typename Decay<ValueType>::Type, Any>::Value>::Type* = 0) {
    using DecayedValueType = typename Decay<ValueType>::Type;
    static_assert(IsCopyConstructible<DecayedValueType>::Value, "ValueType must be copy-constructible");
    StorageHandler<DecayedValueType>::Construct(storage_, Forward<ValueType>(value));
  ...
  explicit Any(in_place_type_t<T>, Args&&... args) {
    ...
    static_assert(IsConstructible<T, Args...>::Value, "T must be constructible with Args...");
  ...
  explicit Any(in_place_type_t<T>,
               ...
               Args&&... args,
               typename EnableIf<IsConstructible<T, std::initializer_list<U>&, Args...>::Value,
                                         void>::Type* = 0) {
    typedef StorageHandler<typename Decay<T>::Type> StorageHandlerT;
  ...
  Any& operator = (ValueType&& value) {
    static_assert(IsCopyConstructible<typename Decay<ValueType>::Type>::Value, "ValueType must be copy-constructible");
  ...
  template <typename NT, typename U, typename... Args>
  typename EnableIf<IsConstructible<NT, std::initializer_list<U>&, Args...>::Value, void>::Type
...
inline ValueType AnyCast(const Any& operand) {
  static_assert(IsReference<ValueType>::Value || IsCopyConstructible<ValueType>::Value,
                "ValueType must be a reference or copy constructible");

  auto* p = AnyCast<typename AddConst<typename RemoveReference<ValueType>::Type>::Type>(&operand);
...
template <typename ValueType>
inline ValueType AnyCast(any&& operand) {
  static_assert(IsReference<ValueType>::Value || eastl::IsCopyConstructible<ValueType>::Value,
                "ValueType must be a reference or copy constructible");
...
inline const ValueType* AnyCast(const Any* any) noexcept {
  return (any && any->handler_ //== &Any::StorageHandler<typename Decay<ValueType>::Type>::handler_func
      #if FUN_RTTI_ENABLED
        && any->Type() == typeid(typename RemoveReference<ValueType>::Type)
      #endif
      ) ?
         static_cast<const ValueType*>(any->handler_(Any::StorageOperation::Get, any, nullptr)) :
         nullptr;
}
...
inline ValueType* AnyCast(Any* any) noexcept {
  return (any && any->handler_ //== &Any::StorageHandler<typename Decay<ValueType>::Type>::handler_func
        ...
        && any->Type() == typeid(typename RemoveReference<ValueType>::Type)
         ...
         static_cast<ValueType*>(any->handler_(Any::StorageOperation::Get, any, nullptr)) :
         nullptr;
}
...
inline ValueType* UnsafeAnyCast(Any* any) noexcept {
  return static_cast<ValueType*>(any->handler_(Any::StorageOperation::Get, any, nullptr));
}
