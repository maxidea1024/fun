...
struct IsFundamental
    : BoolConstant<IsArithmetic<T>::Value || IsVoid<T>::Value ||
                   IsNullPointer<T>::Value> {};

template <typename T>
struct IsObject : BoolConstant<!IsCppFunction<T>::Value &&
                               !IsReference<T>::Value && !IsVoid<T>::Value> {};
...
template <typename T>
struct IsMemberPointer : BoolConstant<std::is_member_pointer_v<T>> {};

template <typename T>
struct IsScalar
    : BoolConstant<IsArithmetic<T>::Value || IsEnum<T>::Value ||
...
struct IsCopyConstructible
    : BoolConstant<__is_constructible(
          T, typename AddLValueReference<const T>::Type)> {};
...
template <typename T>
struct _IsImplicitlyDefaultConstructible<
    T, Void_T<decltype(_ImplicitlyDefaultConstruct<T>({}))>> {};
...
struct IsCopyAssignable
    : BoolConstant<__is_assignable(typename AddLValueReference<T>::Type,
...
struct IsTriviallyCopyConstructible
    : BoolConstant<__is_trivially_constructible(
          T, typename AddLValueReference<const T>::Type)> {};
...
struct IsTriviallyCopyAssignable
    : BoolConstant<__is_trivially_assignable(
          typename AddLValueReference<T>::Type,
...
struct IsTriviallyMoveAssignable
    : BoolConstant<__is_trivially_assignable(
          typename AddLValueReference<T>::Type, T)> {};
...
template <typename T>
struct IsNothrowMoveAssignable : BoolConstant<__is_nothrow_assignable(
                                     typename AddLValueReference<T>::Type, T)> {
};
