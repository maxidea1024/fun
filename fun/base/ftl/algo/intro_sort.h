...
void IntroSortInternal(T* first, size_t count, Projection proj,
                       Predicate pred) {
  struct Stack {
    T* min;
    T* max;
  ...
  // Stack recursion_stack[32] = {{first, first + count - 1,
  // (uint32)(MathBase::Loge((float)count) * 2.f)}}, current, inner;
  Stack recursion_stack[32] = {{first, first + count - 1,
                                (uint32)(MathBase::Loge((float)count) * 2.f)}},
        current, inner;
  for (Stack* stack_top = recursion_stack; stack_top >= recursion_stack;
       --stack_top) {
    current = *stack_top;
  ...
  Loop:
    ...
    if (count <= 8) {
      ...
      while (current.max > current.min) {
        T *max, *item;
        for (max = current.min, item = current.min + 1; item <= current.max;
             ++item) {
      ...
      // Grab middle element so sort doesn't exhibit worst-case behavior with
      // presorted lists.
      Swap(current.min[count / 2], current.min[0]);

      // Divide list into two halves, one with items <=current.min, the other
      // with items >current.max.
      inner.min = current.min;
      inner.max = current.max + 1;
      for (;;) {
        while (++inner.min <= current.max &&
               !pred(Invoke(proj, *current.min), Invoke(proj, *inner.min)))
          ;
        while (--inner.max > current.min &&
               !pred(Invoke(proj, *inner.max), Invoke(proj, *current.min)))
          ;
...
FUN_ALWAYS_INLINE void IntroSort(Range& range) {
  impl::IntroSortInternal(GetMutableData(range), GetCount(range),
