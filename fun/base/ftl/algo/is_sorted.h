...
기본적으로 오름차순인지 여부를 체크하도록 되어 있다. 만약 내림차순으로 정렬된
상태를 확인하고 싶다면, Predicate에서 Less<T> 대신 Greater<T>를 지정하면 된다.

Predicate를 지정하지 않으면 Less<T>가 사용된다. 즉, 위에서 얘기한것처럼
오름차순인지를 확인하는 것으로 간주한다.
*/
...
bool IsSortedByInternal(const T* range, size_t range_count, Projection proj,
                        Predicate pred) {
  if (range_count == 0) {
    return true;
  }
...
FUN_ALWAYS_INLINE bool IsSorted(const Range& range) {
  return impl::IsSortedByInternal(GetConstData(range), GetCount(range),
