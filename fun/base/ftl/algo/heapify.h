...
FUN_ALWAYS_INLINE void Heapify(Range& range) {
  impl::HeapifyInternal(GetMutableData(range), GetCount(range), IdentityFunctor(), Less<>());
}

template <typename Range, typename Predicate>
FUN_ALWAYS_INLINE void Heapify(Range& range, Predicate pred) {
  impl::HeapifyInternal(GetMutableData(range), GetCount(range), IdentityFunctor(), pred);
}
...
template <typename Range, typename Projection, typename Predicate>
FUN_ALWAYS_INLINE void HeapifyBy(Range& range, Projection proj, Predicate pred) {
