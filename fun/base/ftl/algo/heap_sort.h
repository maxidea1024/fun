...
FUN_ALWAYS_INLINE void HeapSort(Range& range) {
  impl::HeapSortInternal(GetMutableData(range), GetCount(range),
...
FUN_ALWAYS_INLINE void HeapSortBy(Range& range, Projection proj) {
  impl::HeapSortInternal(GetMutableData(range), GetCount(range), proj,
                         Less<>());
}
...
FUN_ALWAYS_INLINE void HeapSortBy(Range& range, Projection proj,
                                  Predicate pred) {
  impl::HeapSortInternal(GetMutableData(range), GetCount(range), MoveTemp(proj),
                         MoveTemp(pred));
}
