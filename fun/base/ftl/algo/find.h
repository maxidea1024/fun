...
typename RangePointerType<Range>::Type FindBy(Range& range, const Value& value,
                                              Projection proj) {
  for (auto& elem : range) {
...
typename RangePointerType<Range>::Type FindIf(Range& range, Predicate pred) {
  for (auto& elem : range) {
    if (Invoke(pred, elem)) {
      return &elem;
    }
...
template <typename Range, typename Value, typename Projection>
FUN_ALWAYS_INLINE decltype(auto) FindBy(Range& range, const Value& value,
                                        Projection proj) {
