...
bool IsHeapInternal(RangeValue* heap, size_t count, Projection proj, Predicate pred) {
...
FUN_ALWAYS_INLINE bool IsHeap(Range& range) {
  return impl::IsHeapInternal(GetMutableData(range), GetCount(range), IdentityFunctor(), Less<>());
}

template <typename Range, typename Projection>
FUN_ALWAYS_INLINE bool IsHeapBy(Range& range, Projection proj) {
  return impl::IsHeapInternal(GetMutableData(range), GetCount(range), MoveTemp(proj), Less<>());
}
...
FUN_ALWAYS_INLINE bool IsHeapBy(Range& range, Projection proj, Predicate pred) {
  return impl::IsHeapInternal(GetMutableData(range), GetCount(range), MoveTemp(proj), MoveTemp(pred));
}
