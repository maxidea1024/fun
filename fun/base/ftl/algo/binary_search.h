 ...
 * \param proj Called on values in array to get type that can be compared to
 * value \param sort_pred Predicate for sort comparison
 *
 * \returns Position of the first element >= value, may be == count
 */
template <typename RangeValue, typename PredicateValue, typename Projection,
          typename SortPredicate>
FUN_ALWAYS_INLINE size_t LowerBoundInternal(RangeValue* first,
                                            const size_t count,
  ...
  // With this method, if size is even it will do one more comparison than
  // necessary, but because size can be predicted by the CPU it is faster in
  // practice
  while (size > 0) {
...
template <typename RangeValue, typename PredicateValue, typename Projection,
          typename SortPredicate>
FUN_ALWAYS_INLINE size_t UpperBoundInternal(RangeValue* first,
                                            const size_t count,
                                            const PredicateValue& value,
                                            const Projection& proj,
                                            const SortPredicate& sort_pred) {
 ...
 *
 * \returns Position of the first element >= value, may be position after last
 * element in range
 */
template <typename Range, typename Value, typename SortPredicate>
FUN_ALWAYS_INLINE int32 LowerBound(Range& range, const Value& value,
                                   const SortPredicate& sort_pred) {
  return impl::LowerBoundInternal(GetMutableData(range), GetCount(range), value,
                                  IdentityFunctor(), sort_pred);
}
...
/**
 * Performs binary search, resulting in position of the first element with
 * projected value >= value using predicate
 *
 * \param range range to search through, must be already sorted by sort_pred
 * \param value value to look for
 * \param proj Functor or data member pointer, called via Invoke to compare to
 * value \param sort_pred Predicate for sort comparison, defaults to <
 *
 * \returns Position of the first element >= value, may be position after last
 * element in range
 */
template <typename Range, typename Value, typename Projection,
          typename SortPredicate>
FUN_ALWAYS_INLINE int32 LowerBoundBy(Range& range, const Value& value,
                                     const Projection& proj,
                                     const SortPredicate& sort_pred) {
  return impl::LowerBoundInternal(GetMutableData(range), GetCount(range), value,
                                  proj, sort_pred);
...
FUN_ALWAYS_INLINE int32 UpperBound(Range& range, const Value& value,
                                   const SortPredicate& sort_pred) {
  return impl::UpperBoundInternal(GetMutableData(range), GetCount(range), value,
                                  IdentityFunctor(), sort_pred);
}
...
template <typename Range, typename Value, typename Projection,
          typename SortPredicate>
FUN_ALWAYS_INLINE int32 UpperBoundBy(Range& range, const Value& value,
                                     const Projection& proj,
                                     const SortPredicate& sort_pred) {
  return impl::UpperBoundInternal(GetMutableData(range), GetCount(range), value,
                                  proj, sort_pred);
...
/**
 * Returns index to the first found element matching a value in a range, the
 * range must be sorted by <
 *
 ...
 * \param value The value to search for
 * \param proj Functor or data member pointer, called via Invoke to compare to
 * value \param sort_pred Predicate for sort comparison, defaults to < \return
 * Index of found element, or INVALID_INDEX
 */
template <typename Range, typename Value, typename Projection,
          typename SortPredicate>
FUN_ALWAYS_INLINE int32 BinarySearchBy(Range& range, const Value& value,
                                       const Projection& proj,
                                       const SortPredicate& sort_pred) {
