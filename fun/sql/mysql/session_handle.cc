...
#define FUN_MYSQL_VERSION_NUMBER                          \
...
class ThreadCleanupHelper {
 public:
  ...
  static ThreadCleanupHelper& Instance() { return *sh_.Get(); }

  static void Cleanup(void* data) { mysql_thread_end(); }
...
void SessionHandle::Reset() {
    ...
    ((defined(MARIADB_PACKAGE_VERSION_ID)) &&                       \
     (MARIADB_PACKAGE_VERSION_ID >= 30000))
  if (mysql_reset_connection(handle_) != 0) {
#else
  if (mysql_refresh(handle_, REFRESH_TABLES | REFRESH_STATUS | REFRESH_THREADS |
                                 REFRESH_READ_LOCK) != 0) {
#endif
