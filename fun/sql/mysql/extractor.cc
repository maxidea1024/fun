...
bool Extractor::Extract(size_t pos, String& val) {
  ...
  if (column_type != fun::sql::MetaColumn::FDT_STRING && column_type != fun::sql::MetaColumn::FDT_BLOB) {
    throw MySqlException("Extractor: not a string");
  }

  if (column_type == fun::sql::MetaColumn::FDT_BLOB && metadata_.length(pos) > 0 && metadata_.GetRawData(pos) == NULL) {
    std::vector<char> buffer(metadata_.length(pos), 0);
    bool ret = RealExtractFixedBlob(pos, metadata_.Row()[pos].buffer_type, buffer.data(), buffer.size());
    if (ret) {
  ...
  val.Assign(reinterpret_cast<const char*>(metadata_.rawData(pos)), metadata_.length(pos));
  return true;
}
...
bool Extractor::Extract(size_t pos, fun::sql::BLOB& val) {
  ...
  if (column_type == fun::sql::MetaColumn::FDT_BLOB && metadata_.length(pos) > 0 && metadata_.rawData(pos) == NULL) {
...
bool Extractor::Extract(size_t pos, fun::sql::CLOB& val) {
  ...
  val.AssignRaw(reinterpret_cast<const char*>(metadata_.rawData(pos)), metadata_.length(pos));
  return true;
}
...
bool Extractor::RealExtractFixed(size_t pos, enum_field_types type, void* buffer, bool is_unsigned) {
  MYSQL_BIND bind = {0};
  my_bool IsNull = 0;
...
bool Extractor::RealExtractFixedBlob(size_t pos, enum_field_types type, void* buffer, size_t len) {
  MYSQL_BIND bind = {0};
  my_bool IsNull = 0;
