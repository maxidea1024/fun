 ...
 *
 *     Session ses(SessionFactory::Instance().Create(connector, connection_string));
 *
...
class FUN_SQL_API SessionFactory {
 public:
   ...
   * Always takes ownership of parameter connector.
   */
   ...
   */
  Session Create( const String& key,
                  const String& connection_string,
                  size_t timeout = Session::LOGIN_TIMEOUT_DEFAULT);

  /**
   * Creates a Session for the given URI (must be in key:///connection_string format).
   * Throws a fun:sql::UnknownDataBaseException if no Connector is registered for the key.
   */
