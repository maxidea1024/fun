...
RecordSet::RecordSet(Session& session, const String& query,
                     RowFormatter::Ptr row_formatter)
...
RecordSet::RecordSet(RecordSet&& other) : Statement(MoveTemp(other)) {
  current_row_ = other.current_row_;
  ...
  row_map_ = MoveTemp(other.row_map_);
  other.row_map_.clear();
}
...
fun::dynamic::Var RecordSet::value(size_t col, size_t data_row,
  ...
  switch (columnType(col)) {
    case MetaColumn::FDT_BOOL:
      return value<bool>(col, data_row, use_filter);
    case MetaColumn::FDT_INT8:
      return value<int8>(col, data_row, use_filter);
    case MetaColumn::FDT_UINT8:
      return value<uint8>(col, data_row, use_filter);
    case MetaColumn::FDT_INT16:
      return value<int16>(col, data_row, use_filter);
    case MetaColumn::FDT_UINT16:
      return value<uint16>(col, data_row, use_filter);
    case MetaColumn::FDT_INT32:
      return value<int32>(col, data_row, use_filter);
    case MetaColumn::FDT_UINT32:
      return value<uint32>(col, data_row, use_filter);
    case MetaColumn::FDT_INT64:
      return value<int64>(col, data_row, use_filter);
    case MetaColumn::FDT_UINT64:
      return value<uint64>(col, data_row, use_filter);
    case MetaColumn::FDT_FLOAT:
      return value<float>(col, data_row, use_filter);
    case MetaColumn::FDT_DOUBLE:
      return value<double>(col, data_row, use_filter);
    case MetaColumn::FDT_STRING:
      return value<String>(col, data_row, use_filter);
    case MetaColumn::FDT_WSTRING:
      return value<UString>(col, data_row, use_filter);
    case MetaColumn::FDT_BLOB:
      return value<BLOB>(col, data_row, use_filter);
    case MetaColumn::FDT_CLOB:
      return value<CLOB>(col, data_row, use_filter);
    case MetaColumn::FDT_DATE:
      return value<Date>(col, data_row, use_filter);
    case MetaColumn::FDT_TIME:
      return value<Time>(col, data_row, use_filter);
    case MetaColumn::FDT_TIMESTAMP:
      return value<DateTime>(col, data_row);
    default:
      throw UnknownTypeException("Data type not supported.");
  }
}

fun::dynamic::Var RecordSet::value(const String& name, size_t data_row,
                                   bool use_filter) const {
  if (use_filter && IsFiltered() && !IsAllowed(data_row)) {
    throw InvalidAccessException("Row not allowed");
  }
  ...
  switch (columnType(name)) {
    case MetaColumn::FDT_BOOL:
      return Value<bool>(name, data_row, use_filter);
    case MetaColumn::FDT_INT8:
      return Value<int8>(name, data_row, use_filter);
    case MetaColumn::FDT_UINT8:
      return Value<uint8>(name, data_row, use_filter);
    case MetaColumn::FDT_INT16:
      return Value<int16>(name, data_row, use_filter);
    case MetaColumn::FDT_UINT16:
      return Value<uint16>(name, data_row, use_filter);
    case MetaColumn::FDT_INT32:
      return Value<int32>(name, data_row, use_filter);
    case MetaColumn::FDT_UINT32:
      return Value<uint32>(name, data_row, use_filter);
    case MetaColumn::FDT_INT64:
      return Value<int64>(name, data_row, use_filter);
    case MetaColumn::FDT_UINT64:
      return Value<uint64>(name, data_row, use_filter);
    case MetaColumn::FDT_FLOAT:
      return Value<float>(name, data_row, use_filter);
    case MetaColumn::FDT_DOUBLE:
      return Value<double>(name, data_row, use_filter);
    case MetaColumn::FDT_STRING:
      return Value<String>(name, data_row, use_filter);
    case MetaColumn::FDT_WSTRING:
      return Value<UString>(name, data_row, use_filter);
    case MetaColumn::FDT_BLOB:
      return Value<BLOB>(name, data_row, use_filter);
    case MetaColumn::FDT_DATE:
      return Value<Date>(name, data_row, use_filter);
    case MetaColumn::FDT_TIME:
      return Value<Time>(name, data_row, use_filter);
    case MetaColumn::FDT_TIMESTAMP:
      return Value<DateTime>(name, data_row, use_filter);
    default:
      throw UnknownTypeException("Data type not supported.");
  }
}
...
Row& RecordSet::row(size_t pos) {
  ...
  if (it == row_map_.end()) {
    if (row_map_.size()) {
      ...
      row_ptr =
          new Row(row_map_.begin()->second->names(),
                  row_map_.begin()->second->GetSortMap(), GetRowFormatter());
      ...
      for (size_t col = 0; col < columns; ++col) {
        row_ptr->append(metaColumn(static_cast<uint32>(col)).name(),
                        value(col, pos));
      }
    }
...
std::ostream& RecordSet::CopyValues(std::ostream& os, size_t offset,
                                    size_t length) const {
  if (begin() == end()) return os;

  RowIterator it = *begin_ + offset;
  RowIterator itEnd =
      (RowIterator::POSITION_END != length) ? it + length : *end_;
...
std::ostream& RecordSet::Copy(std::ostream& os, size_t offset,
                              size_t length) const {
  if (begin() == end()) {
    return os;
  }
...
bool RecordSet::IsFiltered() const { return filter_ && !filter_->IsEmpty(); }
