...
class BulkExtraction : public ExtractionBase {
 public:
  ...
  BulkExtraction(C& result_ref, const CValType& def, uint32 limit, const Position& pos = Position(0))
    : ExtractionBase(typeid(C).name(), limit, pos.value(), true),
      result_(result_ref),
      default_(def) {
    if (static_cast<uint32>(result_ref.size()) != limit) {
  ...
  PreparationBase::Ptr CreatePreparation(PreparatorBase::Ptr& prep, size_t col) {
...
class InternalBulkExtraction: public BulkExtraction<C> {
 public:
   ...
   */
  InternalBulkExtraction( C& result_ref,
                          Column<C>* pColumn,
                          uint32 limit,
 ...
 */
template <typename T>
ExtractionBase::Ptr into(std::vector<T>& t, const Bulk& bulk, const Position& pos = Position(0)) {
 ...
 */
template <typename T>
ExtractionBase::Ptr into(std::vector<T>& t, BulkFnType, const Position& pos = Position(0)) {
 ...
 */
template <typename T>
ExtractionBase::Ptr into(std::deque<T>& t, const Bulk& bulk, const Position& pos = Position(0)) {
 ...
 */
template <typename T>
ExtractionBase::Ptr into(std::deque<T>& t, BulkFnType, const Position& pos = Position(0)) {
  uint32 size = static_cast<uint32>(t.size());
  if (0 == size) throw InvalidArgumentException("Zero length not allowed.");
 ...
 */
template <typename T>
ExtractionBase::Ptr into(std::list<T>& t, const Bulk& bulk, const Position& pos = Position(0)) {
 ...
 */
template <typename T>
ExtractionBase::Ptr into(std::list<T>& t, BulkFnType, const Position& pos = Position(0)) {
  uint32 size = static_cast<uint32>(t.size());
  if (0 == size) throw InvalidArgumentException("Zero length not allowed.");
