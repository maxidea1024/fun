...
class FUN_POSTGRESQL_API Binder : public fun::sql::BinderBase {
 public:
  ...
  virtual void Bind(size_t pos, const String& val, Direction dir = PD_IN,
                    const WhenNullCb& null_cb = WhenNullCb());
  virtual void Bind(size_t pos, const fun::sql::BLOB& val,
                    Direction dir = PD_IN,
                    const WhenNullCb& null_cb = WhenNullCb());
  virtual void Bind(size_t pos, const fun::sql::CLOB& val,
                    Direction dir = PD_IN,
                    const WhenNullCb& null_cb = WhenNullCb());
  ...
  virtual void Bind(size_t pos, const NullData& val, Direction dir = PD_IN,
                    const std::type_info& bind_type = typeid(void));
  ...
  void Bind(size_t pos, const std::vector<T>& val, Direction dir = PD_IN) {
    ...
    typename std::vector<T>::iterator first =
        const_cast<std::vector<T>&>(val).begin();
    typename std::vector<T>::iterator last =
        const_cast<std::vector<T>&>(val).end();
  ...
  void Bind(size_t pos, const std::deque<T>& val, Direction dir = PD_IN) {
    ...
    typename std::deque<T>::iterator first =
        const_cast<std::deque<T>&>(val).begin();
    typename std::deque<T>::iterator last =
        const_cast<std::deque<T>&>(val).end();
  ...
  void Bind(size_t pos, const std::list<T>& val, Direction dir = PD_IN) {
    ...
    typename std::list<T>::iterator first =
        const_cast<std::list<T>&>(val).begin();
  ...
  /**
   * obtain the current version of the bound data and update the internal
   * representation
   */
 ...
 private:
   ...
   */
  void RealBind(size_t position,
                fun::sql::MetaColumn::ColumnDataType field_type,
                const void* aBufferPtr, size_t aLength);
    ...
    try {
      ...
      for (; first != last; ++first) {
        ...
        for (; paramsIt != ParamsItEnd; ++paramsIt) {
          ...
          prepared_string.Append(
              static_cast<const char*>(paramsIt->pInternalRepresentation()));
        }
      }
