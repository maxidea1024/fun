...
class FUN_POSTGRESQL_API Connector : public fun::sql::Connector {
 public:
  ...
  /**
   * Creates a PostgreSQL SessionImpl object and initializes it with the given
   * connection_string.
   */
  virtual fun::sql::SessionImpl::Ptr CreateSession(
...
#if defined(_MSC_VER)
extern "C" const struct FUN_POSTGRESQL_API PostgreSqlConnectorRegistrator
    funPostgreSqlConnectorRegistrator;
#if defined(FUN_POSTGRESQL_EXPORTS)
#if defined(_WIN64)
#define FUN_DATA_POSTGRESQL_FORCE_SYMBOL(s) \
  __pragma(comment(linker, "/export:" #s))
#elif defined(_WIN32)
#define FUN_DATA_POSTGRESQL_FORCE_SYMBOL(s) \
...
#define FUN_DATA_POSTGRESQL_FORCE_SYMBOL(s) \
  extern "C" const struct PostgreSqlConnectorRegistrator s;
