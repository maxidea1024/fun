...
void Binder::Bind(size_t pos, const char& val, Direction dir,
                  const WhenNullCb& null_cb) {
  fun_check(dir == PD_IN);
  // USING UINT8 because fun::sql::MetaColumn does not have a single character
  // type, just String
...
void Binder::Bind(size_t pos, const String& val, Direction dir,
                  const WhenNullCb& null_cb) {
  fun_check(dir == PD_IN);
  RealBind(pos, fun::sql::MetaColumn::FDT_STRING, &val,
...
void Binder::Bind(size_t pos, const fun::sql::BLOB& val, Direction dir,
                  const WhenNullCb& null_cb) {
  fun_check(dir == PD_IN);
  RealBind(pos, fun::sql::MetaColumn::FDT_BLOB, &val,
           static_cast<int>(val.size()));
}
...
void Binder::Bind(size_t pos, const fun::sql::CLOB& val, Direction dir,
                  const WhenNullCb& null_cb) {
  fun_check(dir == PD_IN);
  RealBind(pos, fun::sql::MetaColumn::FDT_CLOB, &val,
           static_cast<int>(val.size()));
}
...
void Binder::Bind(size_t pos, const fun::DateTime& val, Direction dir,
                  const WhenNullCb& null_cb) {
  fun_check(dir == PD_IN);
  RealBind(pos, fun::sql::MetaColumn::FDT_TIMESTAMP, &val,
           sizeof(fun::DateTime));
}
...
InputParameterVector Binder::BindVector() const { return bind_vector_; }
  ...
  for (; itr != itrEnd; ++itr) {
    switch (itr->fieldType()) {
      case fun::sql::MetaColumn::FDT_INT8:
        itr->SetStringVersionRepresentation(fun::NumberFormatter::Format(
            *static_cast<const int8*>(itr->pData())));
        break;

      case fun::sql::MetaColumn::FDT_UINT8:
        itr->SetStringVersionRepresentation(fun::NumberFormatter::Format(
            *static_cast<const uint8*>(itr->pData())));
        break;

      case fun::sql::MetaColumn::FDT_INT16:
        itr->SetStringVersionRepresentation(fun::NumberFormatter::Format(
            *static_cast<const int16*>(itr->pData())));
        break;

      case fun::sql::MetaColumn::FDT_UINT16:
        itr->SetStringVersionRepresentation(fun::NumberFormatter::Format(
            *static_cast<const uint16*>(itr->pData())));
        break;

      case fun::sql::MetaColumn::FDT_INT32:
        itr->SetStringVersionRepresentation(fun::NumberFormatter::Format(
            *static_cast<const int32*>(itr->pData())));
        break;

      case fun::sql::MetaColumn::FDT_UINT32:
        itr->SetStringVersionRepresentation(fun::NumberFormatter::Format(
            *static_cast<const uint32*>(itr->pData())));
        break;

      case fun::sql::MetaColumn::FDT_INT64:
        itr->SetStringVersionRepresentation(fun::NumberFormatter::Format(
            *static_cast<const int64*>(itr->pData())));
        break;

      case fun::sql::MetaColumn::FDT_UINT64:
        itr->SetStringVersionRepresentation(fun::NumberFormatter::Format(
            *static_cast<const uint64*>(itr->pData())));
        break;

      case fun::sql::MetaColumn::FDT_BOOL: {
        const bool currentBoolValue = *static_cast<const bool*>(itr->pData());
        itr->SetStringVersionRepresentation(currentBoolValue ? "TRUE"
                                                             : "FALSE");
      } break;

      case fun::sql::MetaColumn::FDT_FLOAT:
        itr->SetStringVersionRepresentation(fun::NumberFormatter::Format(
            *static_cast<const float*>(itr->pData())));
        break;

      case fun::sql::MetaColumn::FDT_DOUBLE:
        itr->SetStringVersionRepresentation(fun::NumberFormatter::Format(
            *static_cast<const double*>(itr->pData())));
        break;

        //  case fun::sql::MetaColumn::FDT_CHAR:
        //    itr->SetStringVersionRepresentation(String(static_cast<const
        //    char*>(itr->pData()), 1));  // single character string break;

      case fun::sql::MetaColumn::FDT_STRING:
        itr->SetStringVersionRepresentation(
            *static_cast<const String*>(itr->pData()));
        break;

      case fun::sql::MetaColumn::FDT_TIMESTAMP: {
        const fun::DateTime& datetime =
            *static_cast<const fun::DateTime*>(itr->pData());
        itr->SetStringVersionRepresentation(DateTimeFormatter::Format(
            datetime, fun::DateTimeFormat::ISO8601_FRAC_FORMAT));
      } break;

      case fun::sql::MetaColumn::FDT_DATE: {
        const fun::sql::Date& date =
            *static_cast<const fun::sql::Date*>(itr->pData());
        itr->SetStringVersionRepresentation(DateTimeFormatter::Format(
            fun::DateTime(date.Year(), date.Month(), date.Day()), "%Y-%m-%d"));
      } break;

      case fun::sql::MetaColumn::FDT_TIME: {
        const fun::sql::Time& time =
            *static_cast<const fun::sql::Time*>(itr->pData());
        itr->SetStringVersionRepresentation(DateTimeFormatter::Format(
            fun::DateTime(0, 1, 1, time.Hour(), time.Minute(), time.Second()),
            "%H:%M:%s%z"));
      } break;

      case fun::sql::MetaColumn::FDT_BLOB: {
        const fun::sql::BLOB& blob =
            *static_cast<const fun::sql::BLOB*>(itr->pData());
        itr->SetNonStringVersionRepresentation(
            static_cast<const void*>(blob.GetRawContent()), blob.size());
      } break;

      case fun::sql::MetaColumn::FDT_CLOB: {
        const fun::sql::CLOB& clob =
            *static_cast<const fun::sql::CLOB*>(itr->pData());
        itr->SetNonStringVersionRepresentation(
            static_cast<const void*>(clob.GetRawContent()), clob.size());
      }
...
void Binder::RealBind(size_t position,
                      fun::sql::MetaColumn::ColumnDataType field_type,
...
void Binder::Bind(size_t pos, const std::vector<int8>& val, Direction dir) {
  ...
  std::vector<int8>::iterator first =
      const_cast<std::vector<int8>&>(val).begin();
...
void Binder::Bind(size_t pos, const std::vector<uint8>& val, Direction dir) {
  ...
  std::vector<uint8>::iterator first =
      const_cast<std::vector<uint8>&>(val).begin();
  std::vector<uint8>::iterator last =
      const_cast<std::vector<uint8>&>(val).end();
...
void Binder::Bind(size_t pos, const std::deque<uint8>& val, Direction dir) {
  ...
  std::deque<uint8>::iterator first =
      const_cast<std::deque<uint8>&>(val).begin();
...
void Binder::Bind(size_t pos, const std::vector<int16>& val, Direction dir) {
  ...
  std::vector<int16>::iterator first =
      const_cast<std::vector<int16>&>(val).begin();
  std::vector<int16>::iterator last =
      const_cast<std::vector<int16>&>(val).end();
...
void Binder::Bind(size_t pos, const std::deque<int16>& val, Direction dir) {
  ...
  std::deque<int16>::iterator first =
      const_cast<std::deque<int16>&>(val).begin();
...
void Binder::Bind(size_t pos, const std::vector<uint16>& val, Direction dir) {
  ...
  std::vector<uint16>::iterator first =
      const_cast<std::vector<uint16>&>(val).begin();
  std::vector<uint16>::iterator last =
      const_cast<std::vector<uint16>&>(val).end();
...
void Binder::Bind(size_t pos, const std::deque<uint16>& val, Direction dir) {
  ...
  std::deque<uint16>::iterator first =
      const_cast<std::deque<uint16>&>(val).begin();
  std::deque<uint16>::iterator last =
      const_cast<std::deque<uint16>&>(val).end();
...
void Binder::Bind(size_t pos, const std::list<uint16>& val, Direction dir) {
  ...
  std::list<uint16>::iterator first =
      const_cast<std::list<uint16>&>(val).begin();
...
void Binder::Bind(size_t pos, const std::vector<int32>& val, Direction dir) {
  ...
  std::vector<int32>::iterator first =
      const_cast<std::vector<int32>&>(val).begin();
  std::vector<int32>::iterator last =
      const_cast<std::vector<int32>&>(val).end();
...
void Binder::Bind(size_t pos, const std::deque<int32>& val, Direction dir) {
  ...
  std::deque<int32>::iterator first =
      const_cast<std::deque<int32>&>(val).begin();
...
void Binder::Bind(size_t pos, const std::vector<uint32>& val, Direction dir) {
  ...
  std::vector<uint32>::iterator first =
      const_cast<std::vector<uint32>&>(val).begin();
  std::vector<uint32>::iterator last =
      const_cast<std::vector<uint32>&>(val).end();
...
void Binder::Bind(size_t pos, const std::deque<uint32>& val, Direction dir) {
  ...
  std::deque<uint32>::iterator first =
      const_cast<std::deque<uint32>&>(val).begin();
  std::deque<uint32>::iterator last =
      const_cast<std::deque<uint32>&>(val).end();
...
void Binder::Bind(size_t pos, const std::list<uint32>& val, Direction dir) {
  ...
  std::list<uint32>::iterator first =
      const_cast<std::list<uint32>&>(val).begin();
...
void Binder::Bind(size_t pos, const std::vector<int64>& val, Direction dir) {
  ...
  std::vector<int64>::iterator first =
      const_cast<std::vector<int64>&>(val).begin();
  std::vector<int64>::iterator last =
      const_cast<std::vector<int64>&>(val).end();
...
void Binder::Bind(size_t pos, const std::deque<int64>& val, Direction dir) {
  ...
  std::deque<int64>::iterator first =
      const_cast<std::deque<int64>&>(val).begin();
...
void Binder::Bind(size_t pos, const std::vector<uint64>& val, Direction dir) {
  ...
  std::vector<uint64>::iterator first =
      const_cast<std::vector<uint64>&>(val).begin();
  std::vector<uint64>::iterator last =
      const_cast<std::vector<uint64>&>(val).end();
...
void Binder::Bind(size_t pos, const std::deque<uint64>& val, Direction dir) {
  ...
  std::deque<uint64>::iterator first =
      const_cast<std::deque<uint64>&>(val).begin();
  std::deque<uint64>::iterator last =
      const_cast<std::deque<uint64>&>(val).end();
...
void Binder::Bind(size_t pos, const std::list<uint64>& val, Direction dir) {
  ...
  std::list<uint64>::iterator first =
      const_cast<std::list<uint64>&>(val).begin();
...
void Binder::Bind(size_t pos, const std::vector<float>& val, Direction dir) {
  ...
  std::vector<float>::iterator first =
      const_cast<std::vector<float>&>(val).begin();
  std::vector<float>::iterator last =
      const_cast<std::vector<float>&>(val).end();
...
void Binder::Bind(size_t pos, const std::deque<float>& val, Direction dir) {
  ...
  std::deque<float>::iterator first =
      const_cast<std::deque<float>&>(val).begin();
...
void Binder::Bind(size_t pos, const std::vector<double>& val, Direction dir) {
  ...
  std::vector<double>::iterator first =
      const_cast<std::vector<double>&>(val).begin();
  std::vector<double>::iterator last =
      const_cast<std::vector<double>&>(val).end();
...
void Binder::Bind(size_t pos, const std::deque<double>& val, Direction dir) {
  ...
  std::deque<double>::iterator first =
      const_cast<std::deque<double>&>(val).begin();
  std::deque<double>::iterator last =
      const_cast<std::deque<double>&>(val).end();
...
void Binder::Bind(size_t pos, const std::list<double>& val, Direction dir) {
  ...
  std::list<double>::iterator first =
      const_cast<std::list<double>&>(val).begin();
...
void Binder::Bind(size_t pos, const std::vector<char>& val, Direction dir) {
  ...
  std::vector<char>::iterator first =
      const_cast<std::vector<char>&>(val).begin();
...
void Binder::Bind(size_t pos, const std::vector<fun::sql::BLOB>& val,
  ...
  std::vector<fun::sql::BLOB>::iterator first =
      const_cast<std::vector<fun::sql::BLOB>&>(val).begin();
  std::vector<fun::sql::BLOB>::iterator last =
      const_cast<std::vector<fun::sql::BLOB>&>(val).end();
  RealContainerBind<std::vector<fun::sql::BLOB>::iterator, fun::sql::BLOB>(
      pos, first, last);
}
...
void Binder::Bind(size_t pos, const std::deque<fun::sql::BLOB>& val,
  ...
  std::deque<fun::sql::BLOB>::iterator first =
      const_cast<std::deque<fun::sql::BLOB>&>(val).begin();
  std::deque<fun::sql::BLOB>::iterator last =
      const_cast<std::deque<fun::sql::BLOB>&>(val).end();
  RealContainerBind<std::deque<fun::sql::BLOB>::iterator, fun::sql::BLOB>(
      pos, first, last);
}
...
void Binder::Bind(size_t pos, const std::list<fun::sql::BLOB>& val,
  ...
  std::list<fun::sql::BLOB>::iterator first =
      const_cast<std::list<fun::sql::BLOB>&>(val).begin();
  std::list<fun::sql::BLOB>::iterator last =
      const_cast<std::list<fun::sql::BLOB>&>(val).end();
  RealContainerBind<std::list<fun::sql::BLOB>::iterator, fun::sql::BLOB>(
      pos, first, last);
}
...
void Binder::Bind(size_t pos, const std::vector<fun::sql::CLOB>& val,
  ...
  std::vector<fun::sql::CLOB>::iterator first =
      const_cast<std::vector<fun::sql::CLOB>&>(val).begin();
  std::vector<fun::sql::CLOB>::iterator last =
      const_cast<std::vector<fun::sql::CLOB>&>(val).end();
  RealContainerBind<std::vector<fun::sql::CLOB>::iterator, fun::sql::CLOB>(
      pos, first, last);
}
...
void Binder::Bind(size_t pos, const std::deque<fun::sql::CLOB>& val,
  ...
  std::deque<fun::sql::CLOB>::iterator first =
      const_cast<std::deque<fun::sql::CLOB>&>(val).begin();
  std::deque<fun::sql::CLOB>::iterator last =
      const_cast<std::deque<fun::sql::CLOB>&>(val).end();
  RealContainerBind<std::deque<fun::sql::CLOB>::iterator, fun::sql::CLOB>(
      pos, first, last);
}
...
void Binder::Bind(size_t pos, const std::list<fun::sql::CLOB>& val,
  ...
  std::list<fun::sql::CLOB>::iterator first =
      const_cast<std::list<fun::sql::CLOB>&>(val).begin();
  std::list<fun::sql::CLOB>::iterator last =
      const_cast<std::list<fun::sql::CLOB>&>(val).end();
  RealContainerBind<std::list<fun::sql::CLOB>::iterator, fun::sql::CLOB>(
      pos, first, last);
}
...
void Binder::Bind(size_t pos, const std::vector<fun::DateTime>& val,
  ...
  std::vector<fun::DateTime>::iterator first =
      const_cast<std::vector<fun::DateTime>&>(val).begin();
  std::vector<fun::DateTime>::iterator last =
      const_cast<std::vector<fun::DateTime>&>(val).end();
  RealContainerBind<std::vector<fun::DateTime>::iterator, fun::DateTime>(
      pos, first, last);
}
...
void Binder::Bind(size_t pos, const std::deque<fun::DateTime>& val,
  ...
  std::deque<fun::DateTime>::iterator first =
      const_cast<std::deque<fun::DateTime>&>(val).begin();
  std::deque<fun::DateTime>::iterator last =
      const_cast<std::deque<fun::DateTime>&>(val).end();
  RealContainerBind<std::deque<fun::DateTime>::iterator, fun::DateTime>(
      pos, first, last);
}
...
void Binder::Bind(size_t pos, const std::list<fun::DateTime>& val,
  ...
  std::list<fun::DateTime>::iterator first =
      const_cast<std::list<fun::DateTime>&>(val).begin();
  std::list<fun::DateTime>::iterator last =
      const_cast<std::list<fun::DateTime>&>(val).end();
  RealContainerBind<std::list<fun::DateTime>::iterator, fun::DateTime>(
      pos, first, last);
}
...
void Binder::Bind(size_t pos, const std::vector<fun::sql::Date>& val,
  ...
  std::vector<fun::sql::Date>::iterator first =
      const_cast<std::vector<fun::sql::Date>&>(val).begin();
  std::vector<fun::sql::Date>::iterator last =
      const_cast<std::vector<fun::sql::Date>&>(val).end();
  RealContainerBind<std::vector<fun::sql::Date>::iterator, fun::sql::Date>(
      pos, first, last);
}
...
void Binder::Bind(size_t pos, const std::deque<fun::sql::Date>& val,
  ...
  std::deque<fun::sql::Date>::iterator first =
      const_cast<std::deque<fun::sql::Date>&>(val).begin();
  std::deque<fun::sql::Date>::iterator last =
      const_cast<std::deque<fun::sql::Date>&>(val).end();
  RealContainerBind<std::deque<fun::sql::Date>::iterator, fun::sql::Date>(
      pos, first, last);
}
...
void Binder::Bind(size_t pos, const std::list<fun::sql::Date>& val,
  ...
  std::list<fun::sql::Date>::iterator first =
      const_cast<std::list<fun::sql::Date>&>(val).begin();
  std::list<fun::sql::Date>::iterator last =
      const_cast<std::list<fun::sql::Date>&>(val).end();
  RealContainerBind<std::list<fun::sql::Date>::iterator, fun::sql::Date>(
      pos, first, last);
}
...
void Binder::Bind(size_t pos, const std::vector<fun::sql::Time>& val,
  ...
  std::vector<fun::sql::Time>::iterator first =
      const_cast<std::vector<fun::sql::Time>&>(val).begin();
  std::vector<fun::sql::Time>::iterator last =
      const_cast<std::vector<fun::sql::Time>&>(val).end();
  RealContainerBind<std::vector<fun::sql::Time>::iterator, fun::sql::Time>(
      pos, first, last);
}
...
void Binder::Bind(size_t pos, const std::deque<fun::sql::Time>& val,
  ...
  std::deque<fun::sql::Time>::iterator first =
      const_cast<std::deque<fun::sql::Time>&>(val).begin();
  std::deque<fun::sql::Time>::iterator last =
      const_cast<std::deque<fun::sql::Time>&>(val).end();
  RealContainerBind<std::deque<fun::sql::Time>::iterator, fun::sql::Time>(
      pos, first, last);
}
...
void Binder::Bind(size_t pos, const std::list<fun::sql::Time>& val,
  ...
  std::list<fun::sql::Time>::iterator first =
      const_cast<std::list<fun::sql::Time>&>(val).begin();
  std::list<fun::sql::Time>::iterator last =
      const_cast<std::list<fun::sql::Time>&>(val).end();
  RealContainerBind<std::list<fun::sql::Time>::iterator, fun::sql::Time>(
      pos, first, last);
}

void Binder::Bind(size_t pos, const std::vector<fun::sql::NullData>& val,
                  Direction dir, const std::type_info&) {
  throw NotImplementedException();
}
...
void Binder::Bind(size_t pos, const std::vector<String>& val, Direction dir) {
  ...
  std::vector<String>::iterator first =
      const_cast<std::vector<String>&>(val).begin();
  std::vector<String>::iterator last =
      const_cast<std::vector<String>&>(val).end();
...
void Binder::Bind(size_t pos, const std::deque<String>& val, Direction dir) {
  ...
  std::deque<String>::iterator first =
      const_cast<std::deque<String>&>(val).begin();
  std::deque<String>::iterator last =
      const_cast<std::deque<String>&>(val).end();
...
void Binder::Bind(size_t pos, const std::list<String>& val, Direction dir) {
  ...
  std::list<String>::iterator first =
      const_cast<std::list<String>&>(val).begin();
