...
void Binder::Bind(size_t pos, const String& val, Direction dir,
                  const WhenNullCb& null_cb) {
  int rc = sqlite3_bind_text(stmt_, (int)pos, val.c_str(),
                             (int)val.size() * sizeof(char), SQLITE_TRANSIENT);
  CheckReturn(rc);
}
...
void Binder::Bind(size_t pos, const Time& val, Direction dir,
                  const WhenNullCb& null_cb) {
  DateTime dt;
  dt.Assign(dt.Year(), dt.Month(), dt.Day(), val.Hour(), val.Minute(),
            val.Second());
...
void Binder::Bind(size_t pos, const NullData&, Direction,
