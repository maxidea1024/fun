...
class FUN_SQLITE_API Binder : public fun::sql::BinderBase {
 public:
  ...
  void Bind(size_t pos, const fun::sql::BLOB& val, Direction dir, const WhenNullCb& null_cb);
  void Bind(size_t pos, const fun::sql::CLOB& val, Direction dir, const WhenNullCb& null_cb);
 ...
 private:
  ...
  template <typename T>
  void BindLOB(size_t pos, const fun::sql::LOB<T>& val, Direction dir, const WhenNullCb& null_cb) {
    ...
    int rc = sqlite3_bind_blob(stmt_, static_cast<int>(pos), data, data_len, SQLITE_STATIC); // no deep copy, do not free memory
    CheckReturn(rc);
  }
...
inline void Binder::Bind(size_t pos, const char* const &pVal, Direction dir, const WhenNullCb& null_cb) {
  String val(pVal);
  Bind(pos, val, dir, null_cb);
}

inline void Binder::Bind(size_t pos, const fun::sql::BLOB& val, Direction dir, const WhenNullCb& null_cb) {
  BindLOB<fun::sql::BLOB::ValueType>(pos, val, dir, null_cb);
}

inline void Binder::Bind(size_t pos, const fun::sql::CLOB& val, Direction dir, const WhenNullCb& null_cb) {
