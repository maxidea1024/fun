...
Binder::Binder(const StatementHandle& stmt,
  size_t maxFieldSize,
  Binder::ParameterBinding data_binding,
  TypeInfo* pDataTypes,
  bool insert_only) :
  stmt_(stmt),
...
void Binder::Bind(size_t pos, const UString& val, Direction dir, const WhenNullCb& null_cb) {
  ...
  if (IsOutBound(dir)) {
    ...
    out_params_.insert(ParamMap::value_type(pVal, size));
    _utf16Strings.insert(UTF16StringMap::value_type(pChar, const_cast<UString*>(& val)));
  }
  else if (IsInBound(dir)) {
    pVal = (SQLPOINTER)val.c_str();
    in_params_.insert(ParamMap::value_type(pVal, size));
...
void Binder::Bind(size_t pos, const NullData& val, Direction dir, const std::type_info& bind_type) {
  ...
  *pLenIn  = SQL_NULL_DATA;
  ...
  const SQLSMALLINT colType = (bind_type == typeid(void)     ||
                 bind_type == typeid(NullData) ||
...
void Binder::Bind(size_t pos, const char* const &/*pVal*/, Direction dir, const WhenNullCb& null_cb) {
  throw NotImplementedException("char* binding not implemented, Use String instead.");
}
...
SQLSMALLINT Binder::ToOdbcDirection(Direction dir) const {
  ...
  else throw fun::IllegalStateException("Binder not bound (must be [in] OR [out]).");
...
void Binder::Reset() {
  ...
  if (in_params_.size() > 0)
    in_params_.clear();
  if (out_params_.size() > 0)
    out_params_.clear();
  if (dates_.size() > 0)
    dates_.clear();
  if (times_.size() > 0)
...
void Binder::GetColSizeAndPrecision(size_t pos,
  ...
  if (_pTypeInfo) {
    ...
    if (colSize && actualSize > colSize) {
      throw LengthExceededException(fun::Format("Error binding column %z size=%z, max size=%ld)",
  ...
  if (parameters_.size() <= pos || !parameters_[pos].IsDefined()) {
    if (parameters_.size() <= pos)
      parameters_.resize(pos + 1);
    parameters_[pos] = ParamDescriptor(0, cDataType, 0);

    try {
      {
        Parameter p(stmt_, pos);
        parameters_[pos] = ParamDescriptor(static_cast<SQLINTEGER>(p.columnSize()), cDataType, static_cast<SQLSMALLINT>(p.decimalDigits()));
      }
    } catch (StatementException&) {
      try {
        OdbcMetaColumn c(stmt_, pos);
        parameters_[pos] = ParamDescriptor(static_cast<SQLINTEGER>(c.length()), cDataType, static_cast<SQLSMALLINT>(c.precision()));
      } catch (StatementException&) {}
    }
  }
...
//On Linux, PostgreSQL driver segfaults on SQLGetDescField, so this is disabled for now
    ...
    SQLHDESC hIPD = 0;
    if (!Utility::IsError(fun::sql::odbc::SQLGetStmtAttr(stmt_, SQL_ATTR_IMP_PARAM_DESC, &hIPD, SQL_IS_POINTER, 0))) {
      SQLUINTEGER sz = 0;
      if (!Utility::IsError(fun::sql::odbc::SQLGetDescField(hIPD, (SQLSMALLINT)pos + 1, SQL_DESC_LENGTH, &sz, SQL_IS_UINTEGER, 0)) &&
          sz > 0) {
        size = sz;
      }
    }
...
void Binder::SetParamSetSize(size_t length) {
  if (0 == param_set_size_) {
    if (Utility::IsError(fun::sql::odbc::SQLSetStmtAttr(stmt_, SQL_ATTR_PARAM_BIND_TYPE, SQL_PARAM_BIND_BY_COLUMN, SQL_IS_UINTEGER)) ||
        Utility::IsError(fun::sql::odbc::SQLSetStmtAttr(stmt_, SQL_ATTR_PARAMSET_SIZE, (SQLPOINTER) length, SQL_IS_UINTEGER))) {
