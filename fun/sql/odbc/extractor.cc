...
const String Extractor::FLD_SIZE_EXCEEDED_FMT = "Specified data size (%z bytes) "
...
bool Extractor::ExtractBoundImpl<UString>(size_t pos, UString& val) {
  ...
  if (ti == typeid(CharT*)) {
    ...
    throw fun::sql::ExtractException("Unsupported string type: " + String(ti.name()));
  }
...
template <>
bool Extractor::ExtractBoundImpl<fun::sql::Date>(size_t pos, fun::sql::Date& val) {
  if (IsNull(pos)) {
    return false;
  }
...
template <>
bool Extractor::ExtractBoundImplContainer<std::vector<fun::sql::Date> >(size_t pos,
  std::vector<fun::sql::Date>& val) {
  std::vector<SQL_DATE_STRUCT>& ds = RefAnyCast<std::vector<SQL_DATE_STRUCT> >(preparator_->at(pos));
  Utility::DateSync(val, ds);
  return true;
}

template <>
bool Extractor::ExtractBoundImplContainer<std::deque<fun::sql::Date> >(size_t pos,
  std::deque<fun::sql::Date>& val) {
  std::vector<SQL_DATE_STRUCT>& ds = RefAnyCast<std::vector<SQL_DATE_STRUCT> >(preparator_->at(pos));
  Utility::DateSync(val, ds);
  return true;
}
...
template <>
bool Extractor::ExtractBoundImplContainer<std::list<fun::sql::Date> >(size_t pos,
  std::list<fun::sql::Date>& val) {
  std::vector<SQL_DATE_STRUCT>& ds = RefAnyCast<std::vector<SQL_DATE_STRUCT> >(preparator_->at(pos));
  Utility::DateSync(val, ds);
  return true;
}
...
template <>
bool Extractor::ExtractBoundImpl<fun::sql::Time>(size_t pos, fun::sql::Time& val) {
  if (IsNull(pos)) {
    return false;
  }
...
template <>
bool Extractor::ExtractBoundImplContainer<std::vector<fun::sql::Time> >(size_t pos,
  std::vector<fun::sql::Time>& val) {
  std::vector<SQL_TIME_STRUCT>& ds = RefAnyCast<std::vector<SQL_TIME_STRUCT> >(preparator_->at(pos));
  Utility::TimeSync(val, ds);
  return true;
}

template <>
bool Extractor::ExtractBoundImplContainer<std::deque<fun::sql::Time> >(size_t pos,
  std::deque<fun::sql::Time>& val) {
  std::vector<SQL_TIME_STRUCT>& ds = RefAnyCast<std::vector<SQL_TIME_STRUCT> >(preparator_->at(pos));
  Utility::TimeSync(val, ds);
  return true;
}
...
template <>
bool Extractor::ExtractBoundImplContainer<std::list<fun::sql::Time> >(size_t pos,
  std::list<fun::sql::Time>& val) {
  std::vector<SQL_TIME_STRUCT>& ds = RefAnyCast<std::vector<SQL_TIME_STRUCT> >(preparator_->at(pos));
  Utility::TimeSync(val, ds);
  return true;
}
...
template <>
bool Extractor::ExtractBoundImpl<fun::DateTime>(size_t pos, fun::DateTime& val) {
  if (IsNull(pos)) {
    return false;
  }
  ...
  size_t dataSize = preparator_->actualDataSize(pos);
  CheckDataSize(dataSize);
  SQL_TIMESTAMP_STRUCT& tss = *AnyCast<SQL_TIMESTAMP_STRUCT>(&preparator_->at(pos));
...
template <>
bool Extractor::ExtractBoundImplContainer<std::vector<fun::DateTime> >(size_t pos,
  std::vector<fun::DateTime>& val) {
  std::vector<SQL_TIMESTAMP_STRUCT>& ds = RefAnyCast<std::vector<SQL_TIMESTAMP_STRUCT> >(preparator_->at(pos));
  Utility::DateTimeSync(val, ds);
  return true;
}

template <>
bool Extractor::ExtractBoundImplContainer<std::deque<fun::DateTime> >(size_t pos,
  std::deque<fun::DateTime>& val) {
  std::vector<SQL_TIMESTAMP_STRUCT>& ds = RefAnyCast<std::vector<SQL_TIMESTAMP_STRUCT> >(preparator_->at(pos));
  Utility::DateTimeSync(val, ds);
  return true;
}
...
template <>
bool Extractor::ExtractBoundImplContainer<std::list<fun::DateTime> >(size_t pos,
  std::list<fun::DateTime>& val) {
  std::vector<SQL_TIMESTAMP_STRUCT>& ds = RefAnyCast<std::vector<SQL_TIMESTAMP_STRUCT> >(preparator_->at(pos));
  Utility::DateTimeSync(val, ds);
  return true;
}
...
template <>
bool Extractor::ExtractBoundImplContainer<std::vector<bool> >(size_t pos,
  std::vector<bool>& val) {
...
template <>
bool Extractor::ExtractBoundImplContainer<std::deque<bool> >(size_t pos,
  std::deque<bool>& val) {
  size_t length = preparator_->GetLength();
  bool** p = AnyCast<bool*>(&preparator_->at(pos));
  val.Assign(*p, *p + length);
...
template <>
bool Extractor::ExtractBoundImplContainer<std::list<bool> >(size_t pos,
  std::list<bool>& val) {
  size_t length = preparator_->GetLength();
  bool** p = AnyCast<bool*>(&preparator_->at(pos));
  val.Assign(*p, *p + length);
...
template <>
bool Extractor::extractManualImpl<String>(size_t pos, String& val, SQLSMALLINT cType) {
  return extractManualStringImpl(pos, val, cType);
}

template <>
bool Extractor::extractManualImpl<UString>(size_t pos, UString& val, SQLSMALLINT cType) {
  return extractManualStringImpl(pos, val, cType);
}
...
template <typename T>
bool Extractor::extractManualLOBImpl( size_t pos,
                                      fun::sql::LOB<T>& val,
                                      SQLSMALLINT cType) {
  ...
  do {
    ...
    if (fetchedSize > 0)
      UnsafeMemory::Memset(pChar, 0, fetchedSize);
    len = 0;
    rc = SQLGetData(stmt_,
      (SQLUSMALLINT) pos + 1,
      cType, //C data type
...
template <>
bool Extractor::extractManualImpl<fun::DateTime>(size_t pos,
  fun::DateTime& val,
  SQLSMALLINT cType) {
...
bool Extractor::Extract(size_t pos, String& val) {
  if (Preparator::DE_MANUAL == data_extraction_ || preparator_->IsPotentiallyHuge(pos))
