#include "fun/sql/odbc/unicode_unix_odbc.h"
#include <iostream>
#include "fun/base/buffer.h"
#include "fun/base/exception.h"
#include "fun/sql/odbc/odbc.h"
#include "fun/text_converter.h"
#include "fun/utf16_encoding.h"
#include "fun/utf8_encoding.h"

using fun::Buffer;
using fun::InvalidArgumentException;
using fun::NotImplementedException;
using fun::TextConverter;
using fun::UTF16Encoding;
using fun::UTF8Encoding;

#ifdef FUN_ODBC_UNICODE
...
void makeUTF16(SQLCHAR* pSQLChar, SQLINTEGER length, String& target) {
  int len = length;
  if (SQL_NTS == len) len = (int)std::strlen((const char*)pSQLChar);
...
void makeUTF8(fun::Buffer<SQLWCHAR>& buffer, SQLINTEGER length,
...
SQLRETURN SQLColAttribute(SQLHSTMT hstmt, SQLUSMALLINT iCol,
  ...
  if (IsString(pCharAttr, cbCharAttrMax)) {
    ...
    SQLRETURN rc = SQLColAttributeW(hstmt, iCol, iField, buffer.begin(),
  ...
  return SQLColAttributeW(hstmt, iCol, iField, pCharAttr, cbCharAttrMax,
                          pcbCharAttr, pNumAttr);
}

SQLRETURN SQLColAttributes(SQLHSTMT hstmt, SQLUSMALLINT icol,
                           SQLUSMALLINT fDescType, SQLPOINTER rgbDesc,
                           SQLSMALLINT cbDescMax, SQLSMALLINT* pcbDesc,
                           SQLLEN* pfDesc) {
  return SQLColAttribute(hstmt, icol, fDescType, rgbDesc, cbDescMax, pcbDesc,
                         pfDesc);
}

SQLRETURN SQLConnect(SQLHDBC hdbc, SQLCHAR* szDSN, SQLSMALLINT cbDSN,
  ...
  return SQLConnectW(hdbc, (SQLWCHAR*)sqlDSN.c_str(), cbDSN,
                     (SQLWCHAR*)sqlUID.c_str(), cbUID,
                     (SQLWCHAR*)sqlPWD.c_str(), cbAuthStr);
}

SQLRETURN SQLDescribeCol(SQLHSTMT hstmt, SQLUSMALLINT icol, SQLCHAR* szColName,
                         SQLSMALLINT cbColNameMax, SQLSMALLINT* pcbColName,
                         SQLSMALLINT* pfSqlType, SQLULEN* pcbColDef,
                         SQLSMALLINT* pibScale, SQLSMALLINT* pfNullable) {
  Buffer<SQLWCHAR> buffer(cbColNameMax);
  SQLRETURN rc = SQLDescribeColW(hstmt, icol, (SQLWCHAR*)buffer.begin(),
                                 (SQLSMALLINT)buffer.size(), pcbColName,
...
SQLRETURN SQLError(SQLHENV henv, SQLHDBC hdbc, SQLHSTMT hstmt,
...
SQLRETURN SQLExecDirect(SQLHSTMT hstmt, SQLCHAR* szSqlStr,
                        SQLINTEGER cbSqlStr) {
  String sqlStr;
...
SQLRETURN SQLGetConnectAttr(SQLHDBC hdbc, SQLINTEGER fAttribute,
  ...
  if (IsString(rgbValue, cbValueMax)) {
    ...
    SQLRETURN rc = SQLGetConnectAttrW(hdbc, fAttribute, buffer.begin(),
                                      (SQLINTEGER)buffer.sizeBytes(), pcbValue);

    makeUTF8(buffer, *pcbValue, rgbValue, cbValueMax);
    return rc;
  }

  return SQLGetConnectAttrW(hdbc, fAttribute, rgbValue, cbValueMax, pcbValue);
}

SQLRETURN SQLGetCursorName(SQLHSTMT hstmt, SQLCHAR* szCursor,
                           SQLSMALLINT cbCursorMax, SQLSMALLINT* pcbCursor) {
  throw NotImplementedException("Not implemented");
}

SQLRETURN SQLSetDescField(SQLHDESC hdesc, SQLSMALLINT iRecord,
  ...
  if (IsString(rgbValue, cbValueMax)) {
    ...
    return SQLSetDescFieldW(hdesc, iRecord, iField, (SQLPOINTER)str.c_str(),
                            (SQLINTEGER)str.size() * sizeof(SQLWCHAR));
  }

  return SQLSetDescFieldW(hdesc, iRecord, iField, rgbValue, cbValueMax);
}

SQLRETURN SQLGetDescField(SQLHDESC hdesc, SQLSMALLINT iRecord,
                          SQLSMALLINT iField, SQLPOINTER rgbValue,
                          SQLINTEGER cbValueMax, SQLINTEGER* pcbValue) {
  if (IsString(rgbValue, cbValueMax)) {
    Buffer<SQLWCHAR> buffer(stringLength(rgbValue, cbValueMax));

    SQLRETURN rc = SQLGetDescFieldW(hdesc, iRecord, iField, buffer.begin(),
                                    (SQLINTEGER)buffer.sizeBytes(), pcbValue);
  ...
  return SQLGetDescFieldW(hdesc, iRecord, iField, rgbValue, cbValueMax,
                          pcbValue);
}

SQLRETURN SQLGetDescRec(SQLHDESC hdesc, SQLSMALLINT iRecord, SQLCHAR* szName,
                        SQLSMALLINT cbNameMax, SQLSMALLINT* pcbName,
                        SQLSMALLINT* pfType, SQLSMALLINT* pfSubType,
                        SQLLEN* pLength, SQLSMALLINT* pPrecision,
                        SQLSMALLINT* pScale, SQLSMALLINT* pNullable) {
  throw NotImplementedException();
}

SQLRETURN SQLGetDiagField(SQLSMALLINT fHandleType, SQLHANDLE handle,
  ...
  if (IsString(rgbDiagInfo, cbDiagInfoMax)) {
    ...
    SQLRETURN rc = SQLGetDiagFieldW(
        fHandleType, handle, iRecord, fDiagField, buffer.begin(),
        (SQLSMALLINT)buffer.sizeBytes(), pcbDiagInfo);
  ...
  return SQLGetDiagFieldW(fHandleType, handle, iRecord, fDiagField, rgbDiagInfo,
                          cbDiagInfoMax, pcbDiagInfo);
}

SQLRETURN SQLGetDiagRec(SQLSMALLINT fHandleType, SQLHANDLE handle,
  ...
  SQLRETURN rc = SQLGetDiagRecW(fHandleType, handle, iRecord, bufState.begin(),
                                pfNativeError, bufErr.begin(),
                                (SQLSMALLINT)bufErr.size(), pcbErrorMsg);
...
SQLRETURN SQLPrepare(SQLHSTMT hstmt, SQLCHAR* szSqlStr, SQLINTEGER cbSqlStr) {
  ...
  return SQLPrepareW(hstmt, (SQLWCHAR*)sqlStr.c_str(),
                     (SQLINTEGER)sqlStr.size());
}

SQLRETURN SQLSetConnectAttr(SQLHDBC hdbc, SQLINTEGER fAttribute,
                            SQLPOINTER rgbValue, SQLINTEGER cbValue) {
  if (IsString(rgbValue, cbValue)) {
    String str;
    makeUTF16((SQLCHAR*)rgbValue, cbValue, str);

    return SQLSetConnectAttrW(hdbc, fAttribute, (SQLWCHAR*)str.c_str(),
                              (SQLINTEGER)str.size() * sizeof(SQLWCHAR));
  }
...
SQLRETURN SQLSetCursorName(SQLHSTMT hstmt, SQLCHAR* szCursor,
                           SQLSMALLINT cbCursor) {
  throw NotImplementedException("Not implemented");
}

SQLRETURN SQLSetStmtAttr(SQLHSTMT hstmt, SQLINTEGER fAttribute,
                         SQLPOINTER rgbValue, SQLINTEGER cbValueMax) {
  if (IsString(rgbValue, cbValueMax)) {
    String str;
    makeUTF16((SQLCHAR*)rgbValue, cbValueMax, str);

    return SQLSetStmtAttrW(hstmt, fAttribute, (SQLWCHAR*)str.c_str(),
                           (SQLINTEGER)str.size());
  }
...
SQLRETURN SQLGetStmtAttr(SQLHSTMT hstmt, SQLINTEGER fAttribute,
                         SQLPOINTER rgbValue, SQLINTEGER cbValueMax,
                         SQLINTEGER* pcbValue) {
  if (IsString(rgbValue, cbValueMax)) {
    Buffer<SQLWCHAR> buffer(stringLength(rgbValue, cbValueMax));

    return SQLGetStmtAttrW(hstmt, fAttribute, (SQLPOINTER)buffer.begin(),
                           (SQLINTEGER)buffer.sizeBytes(), pcbValue);
  }
...
SQLRETURN SQLColumns(SQLHSTMT hstmt, SQLCHAR* szCatalogName,
...
SQLRETURN SQLGetConnectOption(SQLHDBC hdbc, SQLUSMALLINT fOption,
                              SQLPOINTER pvParam) {
  throw NotImplementedException();
}

SQLRETURN SQLGetInfo(SQLHDBC hdbc, SQLUSMALLINT fInfoType,
                     SQLPOINTER rgbInfoValue, SQLSMALLINT cbInfoValueMax,
                     SQLSMALLINT* pcbInfoValue) {
  if (cbInfoValueMax) {
    Buffer<SQLWCHAR> buffer(cbInfoValueMax);

    SQLRETURN rc = SQLGetInfoW(hdbc, fInfoType, (SQLPOINTER)buffer.begin(),
                               (SQLSMALLINT)buffer.sizeBytes(), pcbInfoValue);
  ...
  return SQLGetInfoW(hdbc, fInfoType, rgbInfoValue, cbInfoValueMax,
                     pcbInfoValue);
}
...
SQLRETURN SQLSetConnectOption(SQLHDBC hdbc, SQLUSMALLINT fOption,
                              SQLULEN vParam) {
  throw NotImplementedException();
}

SQLRETURN SQLSpecialColumns(SQLHSTMT hstmt, SQLUSMALLINT fColType,
...
SQLRETURN SQLStatistics(SQLHSTMT hstmt, SQLCHAR* szCatalogName,
                        SQLSMALLINT cbCatalogName, SQLCHAR* szSchemaName,
                        SQLSMALLINT cbSchemaName, SQLCHAR* szTableName,
...
SQLRETURN SQLTables(SQLHSTMT hstmt, SQLCHAR* szCatalogName,
                    SQLSMALLINT cbCatalogName, SQLCHAR* szSchemaName,
                    SQLSMALLINT cbSchemaName, SQLCHAR* szTableName,
...
SQLRETURN SQLDataSources(SQLHENV henv, SQLUSMALLINT fDirection, SQLCHAR* szDSN,
                         SQLSMALLINT cbDSNMax, SQLSMALLINT* pcbDSN,
                         SQLCHAR* szDesc, SQLSMALLINT cbDescMax,
                         SQLSMALLINT* pcbDesc) {
  Buffer<SQLWCHAR> bufDSN(cbDSNMax);
  Buffer<SQLWCHAR> bufDesc(cbDescMax);

  SQLRETURN rc = SQLDataSourcesW(
      henv, fDirection, bufDSN.begin(), (SQLSMALLINT)bufDSN.size(), pcbDSN,
      bufDesc.begin(), (SQLSMALLINT)bufDesc.size(), pcbDesc);
...
SQLRETURN SQLDriverConnect(SQLHDBC hdbc, SQLHWND hwnd, SQLCHAR* szConnStrIn,
  ...
  SQLRETURN rc = SQLDriverConnectW(
      hdbc, hwnd, (SQLWCHAR*)connStrIn.c_str(), (SQLSMALLINT)connStrIn.size(),
      out.begin(), cbConnStrOutMax, pcbConnStrOut, fDriverCompletion);

  makeUTF8(out, *pcbConnStrOut * sizeof(SQLWCHAR), pcbConnStrOut,
           cbConnStrOutMax);

  return rc;
}

SQLRETURN SQLBrowseConnect(SQLHDBC hdbc, SQLCHAR* szConnStrIn,
                           SQLSMALLINT cbConnStrIn, SQLCHAR* szConnStrOut,
                           SQLSMALLINT cbConnStrOutMax,
                           SQLSMALLINT* pcbConnStrOut) {
  String str;
  ...
  SQLRETURN rc = SQLBrowseConnectW(
      hdbc, (SQLWCHAR*)str.c_str(), (SQLSMALLINT)str.size(),
      bufConnStrOut.begin(), (SQLSMALLINT)bufConnStrOut.size(), pcbConnStrOut);

  makeUTF8(bufConnStrOut, *pcbConnStrOut * sizeof(SQLWCHAR), szConnStrOut,
           cbConnStrOutMax);
...
SQLRETURN SQLNativeSql(SQLHDBC hdbc, SQLCHAR* szSqlStrIn, SQLINTEGER cbSqlStrIn,
  ...
  SQLRETURN rc =
      SQLNativeSqlW(hdbc, (SQLWCHAR*)str.c_str(), (SQLINTEGER)str.size(),
                    bufSQLOut.begin(), (SQLINTEGER)bufSQLOut.size(), pcbSqlStr);
...
SQLRETURN SQLDrivers(SQLHENV henv, SQLUSMALLINT fDirection,
  ...
  SQLRETURN rc = SQLDriversW(henv, fDirection, bufDriverDesc.begin(),
                             (SQLSMALLINT)bufDriverDesc.size(), pcbDriverDesc,
                             bufDriverAttr.begin(),
                             (SQLSMALLINT)bufDriverAttr.size(), pcbDrvrAttr);

  makeUTF8(bufDriverDesc, *pcbDriverDesc * sizeof(SQLWCHAR), szDriverDesc,
           cbDriverDescMax);
  makeUTF8(bufDriverAttr, *pcbDrvrAttr * sizeof(SQLWCHAR), szDriverAttributes,
           cbDrvrAttrMax);
