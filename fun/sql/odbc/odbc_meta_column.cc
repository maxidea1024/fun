...
void OdbcMetaColumn::GetDescription() {
  ...
  if (Utility::IsError(fun::sql::odbc::SQLDescribeCol(
          stmt_,
          (SQLUSMALLINT)position() + 1,  // ODBC columns are 1-based
          column_desc_.name, NAME_BUFFER_LENGTH, &column_desc_.nameBufferLength,
          &column_desc_.dataType, &column_desc_.size,
          &column_desc_.decimalDigits, &column_desc_.IsNullable))) {
...
bool OdbcMetaColumn::IsUnsigned() const {
  SQLLEN val = 0;
  if (Utility::IsError(fun::sql::odbc::SQLColAttribute(
          stmt_,
          (SQLUSMALLINT)position() + 1,  // ODBC columns are 1-based
          SQL_DESC_UNSIGNED, 0, 0, 0, &val))) {
    throw StatementException(stmt_);
  }
...
void OdbcMetaColumn::Init() {
  ...
  if (Utility::IsError(fun::sql::odbc::SQLColAttribute(
          stmt_,
          (SQLUSMALLINT)position() + 1,  // ODBC columns are 1-based
          SQL_DESC_LENGTH, 0, 0, 0, &data_length_))) {
    throw StatementException(stmt_);
  }
  ...
  switch (column_desc_.dataType) {
    ...
    case -370:  // IBM DB2 XML, documentation advises to bind it as BLOB, not
                // CLOB
      SetType(MetaColumn::FDT_BLOB);
      break;

    case -99:  // IBM DB2 CLOB
      SetType(MetaColumn::FDT_CLOB);
      break;

    case SQL_TYPE_DATE:
      SetType(MetaColumn::FDT_DATE);
      break;

    case SQL_TYPE_TIME:
      SetType(MetaColumn::FDT_TIME);
      break;

    case SQL_TYPE_TIMESTAMP:
      SetType(MetaColumn::FDT_TIMESTAMP);
      break;
