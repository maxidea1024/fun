...
/**
 * Class encapsulating ODBC diagnostic record collection. Collection is
 * generated during construction. Class provides access and string generation
 * for the collection as well as individual diagnostic records.
 */
...
class Error {
 public:
   ...
   */
  const Diagnostics<H, handleType>& diagnostics() const { return diagnostics_; }

  /**
   * Returns the count of diagnostic records.
   */
  int count() const { return (int)diagnostics_.count(); }
  ...
  String& ToString(int index, String& str) const {
    ...
    fun::Format(s,
                ...
                index + 1, diagnostics_.sqlState(index),
                diagnostics_.nativeError(index), diagnostics_.message(index));
  ...
  String ToString() const {
    ...
    fun::Format(str, "Connection:%s\nServer:%s\n",
                diagnostics_.connectionName(), diagnostics_.serverName());
