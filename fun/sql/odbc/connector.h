 ...
 */
class FUN_ODBC_API Connector : public fun::sql::Connector {
 public:
  /**
...
inline const String& Connector::GetName() const {
  static const String n(FUN_DATA_ODBC_CONNECTOR_NAME);
  return n;
}
 ...
 * ODBC connector with central fun Data registry.
 */
struct FUN_ODBC_API OdbcConnectorRegistrator {
  /**
  ...
  ~OdbcConnectorRegistrator() {
    try {
      fun::sql::odbc::Connector::UnregisterConnector();
    } catch (...) {
      fun_unexpected();
    }
  }
