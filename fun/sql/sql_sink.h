 ...
 * Other than DateTime filed name used for optional time-based archiving
 * purposes, currently the field names are not mandated. However, it is
 * recommended to use names as specified above.
 *
 * To provide as non-intrusive operation as possible, the log entries are cached
 * and inserted into the target database asynchronously by default . The
 * blocking, however, will occur before the next entry insertion with default
 * timeout of 1 second. The default settings can be overriden (see async,
 * timeout and throw properties for details). If throw property is false,
 * insertion timeouts are ignored, otherwise a TimeoutException is thrown. To
 * force insertion of every entry, set timeout to 0. This setting, however,
 * introduces a risk of long blocking periods in case of remote server
...
class FUN_SQL_API SqlSink : public fun::Logsink {
 public:
  ...
  /**
   * Creates a SqlSink with the given connector, connect string, timeout, table
   * and name. The connector must be already registered.
   */
   ...
   *                  Table must exist in the target database. To disable
   * archiving, set this property to empty string.
   *
   *     * async:     Indicates asynchronous execution. When executing
   * asynchronously, messages are sent to the target using asynchronous
   * execution. However, prior to the next message being processed and sent to
   *                  the target, the previous operation must have been either
   * completed or timed out (see timeout and throw properties for details on how
   * abnormal conditions are handled).
   *
   *     * timeout:   Timeout (ms) to wait for previous log operation
   * completion. Values "0" and "" mean no timeout. Only valid when logging is
   * asynchronous, otherwise ignored.
   *
   *     * throw:     Boolean value indicating whether to throw in case of
   * timeout. Setting this property to false may result in log entries being
   * lost. True values are (case insensitive) "true", "t", "yes", "y". Anything
   * else yields false.
   */
...
inline bool SqlSink::IsTrue(const String& value) const {
  return ((0 == icompare(value, "true")) || (0 == icompare(value, "t")) ||
          (0 == icompare(value, "yes")) || (0 == icompare(value, "y")));
}
