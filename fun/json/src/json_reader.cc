...
bool Reader::Parse( const String& document,
                    JValue& root,
...
bool Reader::Parse( const char* begin,
                    const char* end,
                    JValue& root,
                    bool collect_comments) {
...
bool Reader::ReadValue() {
  ...
  if (nodes_.Count() >= features_.stack_limit) {
    throw RangeException(StringLiteral("Exceeded StackLimit in ReadValue()."));
  }
...
bool Reader::ReadObject(Token& token_start) {
  ...
  while (ReadToken(token_name)) {
    ...
    if (token_name.type == TokenType::ObjectEnd && name.IsEmpty()) { // empty object
      return true;
    }
    ...
    } else if (token_name.type == TokenType::Number && features_.allow_numeric_keys) {
      JValue number_name;
    ...
    Token comma;
    if (!ReadToken(comma) ||
        ( comma.type != TokenType::ObjectEnd &&
  ...
  return AddErrorAndRecover("Missing '}' or object member name",
                            token_name, TokenType::ObjectEnd);
}
...
bool Reader::DecodeNumber(Token& token, JValue& decoded) {
  ...
  const uint64 max_integer_value =
        is_neg ? std::numeric_limits<int64>::max() + 1:
  ...
  while (cur != token.end) {
    ...
    if (value >= threshold) {
      if (value > threshold || cur != token.end || digit > (max_integer_value % 10)) {
        return DecodeDouble(token, decoded);
      }
    }
  ...
  */
  ...
  if (is_neg && value == max_integer_value) { // hmm...
    ...
    decoded = -(int64)(value);
  } else if (value <= (uint64)std::numeric_limits<int32>::max()) { // positive int64
    decoded = (int64)(value);
  } else { // unsigned int64
    decoded = value;
  }
...
bool Reader::DecodeUnicodeCodepoint(Token& token,
  ...
  if (unicode >= 0xD800 && unicode <= 0xDBFF) {
    // surrogate pairs
    if ((end - cur) < 6) {
      return AddError("additional six characters expected to parse unicode surrogate pair.", token, cur);
    }
    ...
    if (*cur == '\\' && *(cur + 1) == 'u') {
      ...
      return AddError("expecting another \\u token to begin the second half of a unicode surrogate pair", token, cur);
    }
  }
...
bool Reader::DecodeUnicodeEscapeSequence( Token& token,
                                          ...
                                          uint32& out_unicode) {
  if (end - cur < 4) {
    return AddError("Bad unicode escape sequence in string: four digits expected.", token, cur);
  }
  ...
  for (int32 index = 0; index < 4; ++index) {
    ...
    if (ch >= '0' && ch <= '9') {
      ...
      return AddError("Bad unicode escape sequence in string: hexadecimal digit expected.", token, cur);
    }
  }
...
bool Reader::RecoverFromError(TokenType skip_until_token) {
  ...
  for (;;) {
    if (!ReadToken(skip)) {
      errors_.Resize(encountered_error_count); // discard errors caused by recovery
    }

    if (skip.type == skip_until_token || skip.type == TokenType::EndOfStream) {
      break;
    }
  }
...
String Reader::GetFormattedErrorMessages() const {
  ...
  for (const auto& error : errors_) {
    formatted_message += "* " + GetLocationLineAndColumn(error.token.start) + "\n";
    formatted_message += "  " + error.message + "\n";
    if (error.extra) {
      formatted_message += "See " + GetLocationLineAndColumn(error.extra) + " for detail.\n";
    }
  }
...
bool Reader::PushError( const JValue& value,
                        const String& message,
                        const JValue& extra) {
  const ptrdiff_t len = end_ - begin_;

  if (value.GetOffsetStart() > len ||
      value.GetOffsetLimit() > len ||
