...
WinRegistryKey::WinRegistryKey( const String& key,
                                bool read_only,
                                REGSAM extra_sam)
  : hkey_(0),
...
WinRegistryKey::WinRegistryKey( HKEY hroot_key,
...
void WinRegistryKey::SetString(const String& name, const String& value) {
  ...
  if (RegSetValueExW( hkey_,
                      uname.c_str(),
                      0,
                      REG_SZ,
                      (CONST BYTE*)uvalue.c_str(),
...
String WinRegistryKey::GetString(const String& name) {
  ...
  DWORD size;
  UString uname = UString::FromUtf8(name);
  if (RegQueryValueExW(hkey_, uname.c_str(), NULL, &type, NULL, &size) != ERROR_SUCCESS || (type != REG_SZ && type != REG_EXPAND_SZ && type != REG_LINK)) {
  ...
  if (size > 0) {
    ...
    RegQueryValueExW(hkey_, uname.c_str(), NULL, NULL, (BYTE*)buffer.MutableData(), &size);
    buffer[len] = 0;
    //TODO optimize
...
void WinRegistryKey::SetStringExpand(const String& name, const String& value) {
  ...
  UString uname = UString::FromUtf8(name);
  UString uvalue = UString::FromUtf8(value);
  if (RegSetValueExW(hkey_, uname.c_str(), 0, REG_EXPAND_SZ, (CONST BYTE*) uvalue.c_str(), (DWORD)(uvalue.Len() + 1) * sizeof(wchar_t)) != ERROR_SUCCESS) {
    HandleSetError(name);
  }
}
...
String WinRegistryKey::GetStringExpand(const String& name) {
  ...
  if (size > 0) {
    ...
    ExpandEnvironmentStringsW(buffer.ConstData(), exp_buffer.MutableData(), exp_size);
...
void WinRegistryKey::SetBinary(const String& name, const Array<char>& value) {
  ...
  UString uname = UString::FromUtf8(name);
  if (RegSetValueExW(hkey_, uname.c_str(), 0, REG_BINARY, (CONST BYTE*)&value[0], (DWORD)value.Count()) != ERROR_SUCCESS) {
    HandleSetError(name);
  }
}
...
Array<char> WinRegistryKey::GetBinary(const String& name) {
  ...
  Array<char> result;
  UString uname = UString::FromUtf8(name);
  if (RegQueryValueExW(hkey_, uname.c_str(), NULL, &type, NULL, &size) != ERROR_SUCCESS || type != REG_BINARY) {
    throw NotFoundException(GetKey(name));
  }

  if (size > 0) {
    result.Resize(size);
    RegQueryValueExW(hkey_, uname.c_str(), NULL, NULL, (BYTE*)&result[0], &size);
  }
...
void WinRegistryKey::SetInt(const String& name, int32 value) {
  ...
  DWORD data = value;
  UString uname = UString::FromUtf8(name);
  if (RegSetValueExW(hkey_, uname.c_str(), 0, REG_DWORD, (CONST BYTE*) &data, sizeof(data)) != ERROR_SUCCESS) {
    HandleSetError(name);
  }
}
...
int32 WinRegistryKey::GetInt(const String& name) {
  ...
  UString uname = UString::FromUtf8(name);
  if (RegQueryValueExW(hkey_, uname.c_str(), NULL, &type, (BYTE*)&data, &size) != ERROR_SUCCESS || (type != REG_DWORD && type != REG_DWORD_BIG_ENDIAN)) {
    throw NotFoundException(GetKey(name));
  }
...
void WinRegistryKey::SetInt64(const String& name, int64 value) {
  ...
  UString uname = UString::FromUtf8(name);
  if (RegSetValueExW(hkey_, uname.c_str(), 0, REG_QWORD, (CONST BYTE*) &value, sizeof(value)) != ERROR_SUCCESS) {
    HandleSetError(name);
  }
}
...
int64 WinRegistryKey::GetInt64(const String& name) {
  ...
  DWORD size = sizeof(data);
  UString uname = UString::FromUtf8(name);
  if (RegQueryValueExW(hkey_, uname.c_str(), NULL, &type, (BYTE*)&data, &size) != ERROR_SUCCESS || type != REG_QWORD) {
    throw NotFoundException(GetKey(name));
  }
...
void WinRegistryKey::DeleteKey() {
  ...
  // Therefore, if we need to call RegDeleteKeyEx (extra_sam_ != 0) we need to check for
  ...
  if (extra_sam_ != 0) {
    ...
    if (adv_api32.GetHandle()) {
      RegDeleteKeyExWFunc pRegDeleteKeyExW = reinterpret_cast<RegDeleteKeyExWFunc>(GetProcAddress(adv_api32.GetHandle() , "RegDeleteKeyExW"));
      if (pRegDeleteKeyExW) {
        if ((*pRegDeleteKeyExW)(hroot_key_, usub_key.c_str(), extra_sam_, 0) != ERROR_SUCCESS) {
...
bool WinRegistryKey::Exists() {
  HKEY hkey;
  UString usub_key = UString::FromUtf8(sub_key_);
  if (RegOpenKeyExW(hroot_key_, usub_key.c_str(), 0, KEY_READ | extra_sam_, &hkey) == ERROR_SUCCESS) {
    RegCloseKey(hkey);
    return true;
  }
...
WinRegistryKey::Type WinRegistryKey::GetType(const String& name) {
  ...
  DWORD size;
  UString uname = UString::FromUtf8(name);
  if (RegQueryValueExW(hkey_, uname.c_str(), NULL, &type, NULL, &size) != ERROR_SUCCESS) {
    throw NotFoundException(GetKey(name));
  }
...
void WinRegistryKey::Open() {
  if (!hkey_) {
    UString usub_key = UString::FromUtf8(sub_key_);
    if (read_only_) {
      if (RegOpenKeyExW(hroot_key_, usub_key.c_str(), 0, KEY_READ | extra_sam_, &hkey_) != ERROR_SUCCESS) {
        throw NotFoundException("Cannot Open registry key: ", GetKey());
      }
    } else {
      if (RegCreateKeyExW(hroot_key_, usub_key.c_str(), 0, NULL, REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE | extra_sam_, NULL, &hkey_, NULL) != ERROR_SUCCESS) {
...
void WinRegistryKey::GetSubKeys(WinRegistryKey::Keys& keys) {
  ...
  DWORD value_count = 0;
  if (RegQueryInfoKey(hkey_, NULL, NULL, NULL, &sub_key_count, NULL, NULL, &value_count, NULL, NULL, NULL, NULL) != ERROR_SUCCESS) {
    return;
  }
  ...
  for (DWORD i = 0; i< sub_key_count; ++i) {
    if (RegEnumKeyExW(hkey_, i, buf, &buf_size, NULL, NULL, NULL, NULL) == ERROR_SUCCESS) {
...
void WinRegistryKey::GetValues(WinRegistryKey::Values& vals) {
  ...
  DWORD value_count = 0;
  if (RegQueryInfoKey(hkey_, NULL, NULL, NULL, NULL, NULL, NULL, &value_count, NULL, NULL, NULL, NULL) != ERROR_SUCCESS) {
    return ;
  }
  ...
  for (DWORD i = 0; i< value_count; ++i) {
    if (RegEnumValueW(hkey_, i, buf, &buf_size, NULL, NULL, NULL, NULL) == ERROR_SUCCESS) {
