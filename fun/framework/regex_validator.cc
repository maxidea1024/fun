...
RegexValidator::RegexValidator(const String& regex) : regex_(regex) {}
...
void RegexValidator::Validate(const Option& option, const String& value) {
  if (!Regex::Match(value, regex_, Regex::RE_ANCHORED | Regex::RE_UTF8)) {
    throw InvalidArgumentException(
        String::Format("argument for {0} does not match regular expression {1}",
                       option.GetFullName(), regex_));
  }
}
