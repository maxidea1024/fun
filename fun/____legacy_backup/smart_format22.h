...
class BasicStringRef
{
 public:
  ...
  const CharType* ConstData() const { return data_; }
...
class BasicCStringRef
{
 public:
  ...
  const CharType* c_str() const { return data_; }
...
void MemoryBuffer<T, SIZE, Allocator>::Grow(size_t len)
{
...
{
 public:
  explicit ThousandsSep(::fun::sf::CStringRefA sep)
    : sep_(sep)
    , digit_index_(0)
  {
  }
...
// ThousandsSep is a functor that is called after writing each char to
// add a thousands separator if necessary.
template <typename UIntT, typename CharType, typename ThousandsSep>
...
// We check if ``lconv`` contains ``ThousandsSep`` because on Android
// ``lconv`` is stubbed as an empty struct.
template <typename LConv>
inline CStringRefA ThousandsSep(LConv* LC, LConvCheck<char* LConv::*, &LConv::ThousandsSep> = 0)
{
  return LC->ThousandsSep;
}

//기본적으로 3자리마다 ','를 넣어줌.
//만약, 로케일에 따라서 처리가 된 경우에는 ',' 대신에 각 로케일에 맞는 문자가 반영될것임.
inline CStringRefA ThousandsSep(...) { return ","; }
...
class ArgFormatterBase : public ArgVisitor<Impl, void>
{
 ...
 public:
  ...
  template <typename T>
  void VisitAnyInt(T value) { writer_.WriteInt(value, spec_); }

  template <typename T>
  void VisitAnyDouble(T value) { writer_.WriteDouble(value, spec_); }
...
class BasicWriter
{
 private:
  ...
  static CharPtr FillPadding(CharPtr buffer, uint32 total_len, size_t content_len, wchar_t fill);
  ...
  CharPtr PrepareIntBuffer(uint32 digit_count, const EmptySpec&, const char* prefix, uint32 prefix_len)
  {
  ...
  CharPtr PrepareIntBuffer(uint32 digit_count, const SpecT& spec, const char* prefix, uint32 prefix_len);
...
typename BasicWriter<CharType>::CharPtr
  BasicWriter<CharType>::WriteString(const StrChar* s, size_t len, const AlignSpec& spec)
{
  CharPtr out = CharPtr();
  if (spec.Width() > len)
  {
...
template <typename CharType>
template <typename StrChar, typename SpecT>
void BasicWriter<CharType>::WriteString(const internal::Arg::CStringValue<StrChar>& s, const SpecT& spec)
{
...
template <typename CharType>
typename BasicWriter<CharType>::CharPtr
  BasicWriter<CharType>::FillPadding(CharPtr buffer, uint32 total_len, size_t content_len, wchar_t fill)
{
...
void BasicWriter<CharType>::WriteInt(T value, SpecT spec)
{
  ...
  case 'n': {
    ...
    sep = internal::ThousandsSep(std::localeconv());
#endif
    ...
    internal::FormatDecimal(Get(p), abs_value, 0, internal::ThousandsSep(sep));
    break;
  }
...
class FormatInt
{
 public:
  ...
  size_t Len() const { return internal::ToUnsigned(buffer_ - str_ + BUFFER_LEN - 1); }
  const char* ConstData() const { return str_; }
  const char* c_str() const { buffer_[BUFFER_LEN - 1] = '\0'; return str_; }
